...
if heteroagentoptions.useCustomModelStats==1
    if heteroagentoptions.CustomModelStats_origgrids==0
        CustomStats2=heteroagentoptions.CustomModelStats(V,Policy,StationaryDist,Parameters,FnsToEvaluate,n_d,n_a,n_z,N_j,d_grid,a_grid,z_gridvals_J,pi_z_J,heteroagentoptions,heteroagentoptions.CustomModelStatsInputs.vfoptions,heteroagentoptions.CustomModelStatsInputs.simoptions);
...
if caliboptions.verbose==1
    ...
    if heteroagentoptions.useCustomModelStats==1
        fprintf('Current CustomModelStats variables (from heteroagentoptions): \n')
        for ii=1:length(customstatnames)
            fprintf('	%s: %8.4f \n',customstatnames{ii},CustomStats2.(customstatnames{ii}))
        end
    end
