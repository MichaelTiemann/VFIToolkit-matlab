function StationaryDist=StationaryDist_FHorz_ExpAsseteSemiExo(jequaloneDist,AgeWeightParamNames,Policy,n_d,n_a,n_semiz,n_z,N_j,pi_semiz_J,pi_z_J,Parameters,simoptions)
...
n_d2=n_d(end-simoptions.l_dexperienceassete-simoptions.l_dsemiz+1:end-simoptions.l_dsemiz); % decision variables that controls experience asset
if length(n_d)>2
    n_d1=n_d(1:end-2);
    l_d1=length(n_d1);
else
...
%%
% e is required for experienceassete, so n_bothze always contains n_e
if N_z==0
...
Policy_aprime=zeros(N_a,N_bothze,2,N_j,simoptions.indexT,'gpuArray'); % the lower grid point
