function StationaryDist=StationaryDist_FHorz_ExpAssetzSemiExo(jequaloneDist,AgeWeightParamNames,Policy,n_d,n_a,n_semiz,n_z,N_j,pi_semiz_J,z_gridvals_J,pi_z_J,Parameters,simoptions)
...
n_d2=n_d(end-simoptions.l_dexperienceassetz-simoptions.l_dsemiz+1:end-simoptions.l_dsemiz); % decision variables that controls experience asset
if length(n_d)>2
    n_d1=n_d(1:end-2);
    l_d1=length(n_d1);
else
...
%%
% z is required for experienceassetz, so n_bothze always contains n_z
if N_e==0
