...
PolicyProbs=reshape(gather(PolicyProbs),[N_a*N_semiz*N_z*N_e,N_probs,N_j]);
...
for jj=1:(N_j-1)
    ...
    Gammatranspose=sparse(Policy_aprimesemizz_jj,II2,PolicyProbs_jj,N_a*N_bothz,N_a*N_bothz*N_e); % Note: sparse() will accumulate at repeated indices [only relevant at grid end points]
    ...
    pi_e=sparse(pi_e_J(:,jj+1)); % pi_e_J(:,jj+1) is the distribution of the e realized in period jj+1
