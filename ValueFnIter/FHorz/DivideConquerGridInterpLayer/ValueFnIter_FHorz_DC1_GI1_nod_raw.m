function [V, Policy]=ValueFnIter_FHorz_DC1_GI1_nod_raw(n_a,n_z,N_j, a_grid, z_gridvals_J,pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)

N_a=prod(n_a);
N_z=prod(n_z);

V=zeros(N_a,N_z,N_j,vfoptions.precision,'gpuArray');
Policy=zeros(2,N_a,N_z,N_j,vfoptions.indexT,'gpuArray'); % first dim indexes the optimal choice for aprime and aprime2 (in GI layer)
