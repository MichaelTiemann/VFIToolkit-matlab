...
% Continuation value interpolation: (n_a x G x n_z)
EV_next = V_next * (pi_z_j');
EV_next_pad = [EV_next; EV_next(end, :)];
tau_3d = reshape(tau_vec, [1, G, 1]);
EV_dense_3d = (1 - tau_3d) .* reshape(EV_next, [n_a, 1, n_z]) + ...
...
% Permute choices to trailing dimensions: (n_anchors, n_z, n_d, n_a, G)
n_choices_full = n_d * n_a * G;
RHS_m1 = reshape(RHS_1, [n_anchors * n_z, n_choices_full]);
...
if n_rem > 0
    ...
    ev_cand_lin = (coarse_cand_idx - 1) + (tau_sub_idx - 1) .* n_a + (z_sub_idx - 1) .* (n_a * G) + 1;
    V_cont_5d = EV_dense_3d(ev_cand_lin);
    
    RHS_2 = F_2 + beta_j .* V_cont_5d;
    
    n_choices_cand = n_d * n_cand_coarse * G;
    RHS_m2 = reshape(permute(RHS_2, [1, 2, 3, 4, 5]), [n_rem * n_z, n_choices_cand]);
    ...
    coarse_cand_2d = reshape(coarse_cand_idx, [n_rem * n_z, n_cand_coarse]);
    state_lin2 = (1:(n_rem * n_z))';
    chosen_coarse_lin = sub2ind([n_rem * n_z, n_cand_coarse], state_lin2, coarse_offset_opt2);
