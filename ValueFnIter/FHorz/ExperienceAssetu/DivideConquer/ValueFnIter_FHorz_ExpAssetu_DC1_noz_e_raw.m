function [V,Policy]=ValueFnIter_FHorz_ExpAssetu_DC1_noz_e_raw(n_d1, n_d2,n_a1,n_a2,n_e,n_u,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid,e_gridvals_J, u_gridvals, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
Policy=zeros(N_a,N_e,N_j,vfoptions.indexT,'gpuArray'); %first dim indexes the optimal choice for d and a1prime rest of dimensions a,z
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV=DiscountFactorParamsVec*reshape(EV,[N_d2,N_a1,1,N_a2]); % (d2,a1prime,1,a2); d1-dim is implicit singleton, broadcasts at use sites
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2]
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap+1,1,N_a2]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV(d2aprime),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap+1,1,N_a2]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2]
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d2aprime),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V(curraindex,e_c,jj)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                    ...
                    d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,1,1,N_a2]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2]
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V(curraindex,e_c,jj)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
