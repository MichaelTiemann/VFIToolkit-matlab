function [V,Policy]=ValueFnIter_FHorz_ExpAssetu_DC2A_noz_raw(n_d1, n_d2, n_a1, n_a2, n_a3, n_u, N_j, d_gridvals, d2_gridvals, a1_grid, a2_gridvals, a3_grid, u_gridvals, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d2aprime),[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d2aprime),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii,1);
            dind   =rem(maxindex-1,N_d)+1;
            a2pind =floor((maxindex-1)/N_d)+1;
...
for reverse_j=1:N_j-1
    ...
    Vlower=reshape(V(aprimeIndex(:),jj+1),    [N_d2*N_a1*N_a2,N_a3,N_u]);
    Vupper=reshape(V(aprimeplus1Index(:),jj+1),[N_d2*N_a1*N_a2,N_a3,N_u]);
    skipinterp=(Vlower==Vupper);
    aprimeProbs(skipinterp)=0;
