function [V,Policy]=ValueFnIter_FHorz_ExpAssetu_DC2A_nod1_noz_raw(n_d2, n_a1, n_a2, n_a3, n_u, N_j, d2_gridvals, a1_grid, a2_gridvals, a3_grid, u_gridvals, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d2aprime),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d2aprime),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii,1);
            d2ind  =rem(maxindex-1,N_d2)+1;
            a2pind =floor((maxindex-1)/N_d2)+1;
