function [V,Policy]=ValueFnIter_FHorz_ExpAssetu_DC2A_GI2A_nod1_raw(n_d2, n_a1, n_a2, n_a3, n_z, n_u, N_j, d2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, u_gridvals, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% DC on the first standard endogenous state (n_a1, scalar),
% folded standard middle endogenous states (n_a2, may be multi-dim),
% experience asset as the last endogenous state (n_a3, scalar),
% then Grid Interpolation Layer fine pass over the DC-derived midpoint.
% _nod1: only d2 (no d1).
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii,[N_d2*n2long*N_a2,N_a,N_z]);
        linidx_lower=d_ind                   + N_d2*n2long*(maxindexL2a2-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*zindB;
        linidx_upper=d_ind + N_d2*(n2long-1) + N_d2*n2long*(maxindexL2a2-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*zindB;
...
for reverse_j=1:N_j-1
    ...
    Vlower=reshape(V(aprimeIndex(:),:,jj+1),    [N_d2*N_a1*N_a2,N_a3,N_u,N_z]);
    Vupper=reshape(V(aprimeplus1Index(:),:,jj+1),[N_d2*N_a1*N_a2,N_a3,N_u,N_z]);
    skipinterp=(Vlower==Vupper);
    aprimeProbs(skipinterp)=0;
    EV=aprimeProbs.*Vlower+(1-aprimeProbs).*Vupper;
    EV=squeeze(sum((EV.*pi_u),3)); % integrate out u
