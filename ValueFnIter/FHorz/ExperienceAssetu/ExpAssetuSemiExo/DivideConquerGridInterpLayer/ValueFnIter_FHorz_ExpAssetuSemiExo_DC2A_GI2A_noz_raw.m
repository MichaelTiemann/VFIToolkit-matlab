...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3=reshape(ReturnMatrix_ii_d3+DiscountedEVinterp(aprimez),[N_d12*n2long*N_a2,N_a,N_semiz]);
    ...
    d12sel=reshape(d12_ford3_jj(idx),[1,N_a,N_semiz]);
    Policy4(1,:,:,N_j)=rem(d12sel-1,N_d1)+1; % d1
    Policy4(2,:,:,N_j)=ceil(d12sel/N_d1); % d2
    Policy4(4,:,:,N_j)=reshape(joint_ford3_jj(idx),[1,N_a,N_semiz]); % joint(a1prime midpoint,a2prime)
    Policy4(5,:,:,N_j)=reshape(L2a1_ford3_jj(idx),[1,N_a,N_semiz]); % a1primeL2ind
...
for reverse_j=1:N_j-1
    ...
    Policy4(4,:,:,jj)=reshape(joint_ford3_jj(idx),[1,N_a,N_semiz]); % joint(a1prime midpoint,a2prime)
    Policy4(5,:,:,jj)=reshape(L2a1_ford3_jj(idx),[1,N_a,N_semiz]); % a1primeL2ind
