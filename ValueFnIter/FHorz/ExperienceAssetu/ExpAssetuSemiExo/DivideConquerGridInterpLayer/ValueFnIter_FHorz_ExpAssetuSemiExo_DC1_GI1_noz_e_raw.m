...
Policy=zeros(5,N_a,N_semiz,N_e,N_j,vfoptions.indexT,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    Policy(2,:,:,:,N_j)=reshape(Policy4_ford3_jj(2+temp),[1,N_a,N_semiz,N_e]);
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind; % [N_d12,maxgap+1,1,N_a2,N_semiz,N_e]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2,N_semiz]
            ...
            entireRHS_ii_d3=ReturnMatrix_ii_d3+reshape(DiscountedEVinterp(d2a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz,N_e]);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind; % [N_d12,maxgap+1,1,N_a2,N_semiz]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2,N_semiz]
                        entireRHS_ii_d3=ReturnMatrix_ii_d3+DiscountedEV(d2aprimez);
                        [~,maxindex]=max(entireRHS_ii_d3,[],2);
                        midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
                    else
                ...
                d2a1primea2semiz=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*semizind; % Note: EV does not depend on d1, but this does still have d1 as part of the first dimension
                entireRHS_ii_d3=ReturnMatrix_ii_d3+reshape(DiscountedEVinterp(d2a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
                V_ford3_jj(:,:,e_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d12)+1;
                allind=d_ind+N_d12*aind+N_d12*N_a*semizBind; % midpoint is n_d12-by-1-by-n_a1-by-n_a2-by-n_semiz
...
for reverse_j=1:N_j-1
    ...
    Policy(2,:,:,:,jj)=reshape(Policy4_ford3_jj(2+temp),[1,N_a,N_semiz,N_e]);
...
adjust=cast2index(Policy(5,:,:,:,:)<1+n2short+1); % if second layer is choosing below midpoint
