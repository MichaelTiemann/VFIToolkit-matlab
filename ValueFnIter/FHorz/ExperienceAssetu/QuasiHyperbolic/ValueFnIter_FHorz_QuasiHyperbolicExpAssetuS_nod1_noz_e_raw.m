function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuS_nod1_noz_e_raw(n_d2,n_a1,n_a2,n_e,n_u, N_j, d2_gridvals, a1_gridvals, a2_grid, e_gridvals_J, u_gridvals, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2,n_a1,n_a1,n_a2,n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,0,0); % Level=0, Refine=0
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2,n_a1,n_a1,n_a2,special_n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,0,0); % Level=0, Refine=0
            %Calc the max and it's index
            [Vtemp,maxindex]=max(ReturnMatrix,[],1);
            Vhat(:,e_c,N_j)=Vtemp;
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_hat=ReturnMatrix+beta0beta*repelem(EV,1,N_a1,1);
        [Vtemp,maxindex]=max(entireRHS_hat,[],1);
        entireRHS_under=ReturnMatrix+beta*repelem(EV,1,N_a1,1);
        maxindexfull=maxindex+N_d2*N_a1*(0:1:N_a-1)+shiftdim(N_d2*N_a1*N_a*(0:1:N_e-1),-1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            e_val=e_gridvals_J(e_c,:,N_j);
            ReturnMatrix_e=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2,n_a1,n_a1,n_a2,special_n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,0,0); % Level=0, Refine=0

            % hat: argmax of F + beta0*beta*EV; under: the beta-RHS gathered at that argmax
            entireRHS_hat=ReturnMatrix_e+beta0beta*repelem(EV,1,N_a1,1);
            [Vtemp,maxindex]=max(entireRHS_hat,[],1);
            entireRHS_under=ReturnMatrix_e+beta*repelem(EV,1,N_a1,1);
            maxindexfull=maxindex+N_d2*N_a1*(0:1:N_a-1);
            Vhat(:,e_c,N_j)=shiftdim(Vtemp,1);
            Vunderbar(:,e_c,N_j)=shiftdim(entireRHS_under(maxindexfull),1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2,n_a1,n_a1,n_a2,n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,0,0); % Level=0, Refine=0
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Vunderbar(:,e_c,jj)=shiftdim(entireRHS_under(maxindexfull),1);
