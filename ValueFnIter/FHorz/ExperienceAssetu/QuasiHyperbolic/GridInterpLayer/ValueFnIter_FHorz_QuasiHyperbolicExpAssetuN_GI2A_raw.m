function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuN_GI2A_raw(n_d1, n_d2, n_a1, n_a2, n_a3, n_z, n_u, N_j, d_gridvals, d2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, u_gridvals, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_ExpAssetu_GI2A_raw.
...
% d2-component of each d slot (d1 cycles fastest, d2 next).
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        midpoint=max(min(maxindex,N_a1-1),2); % [N_d,1,N_a2,N_a1,N_a2,N_a3,N_z]
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_iialt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, d_gridvals, a1prime_grid(a1primeindexesalt), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
        ...
        entireRHS_iialt=reshape(ReturnMatrix_iialt+beta*entireEVinterp(aprimezalt),[N_d*n2long*N_a2,N_a,N_z]);
        ...
        entireRHS_ii=reshape(ReturnMatrix_ii+beta0beta*entireEVinterp(aprimez),[N_d*n2long*N_a2,N_a,N_z]);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_zalt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, d_gridvals, a1prime_grid(a1primeindexesalt), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
            aprime_zalt=d2ind_vec + N_d2*(a1primeindexesalt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
            entireRHS_ii_zalt=reshape(ReturnMatrix_ii_zalt+beta*entireEVinterp_z(aprime_zalt),[N_d*n2long*N_a2,N_a]);
            ...
            entireRHS_ii_z=reshape(ReturnMatrix_ii_z+beta0beta*entireEVinterp_z(aprime_z),[N_d*n2long*N_a2,N_a]);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_iialt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, d_gridvals, a1prime_grid(a1primeindexesalt), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
        aprimezalt=d2ind_vec + N_d2*(a1primeindexesalt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
        entireRHS_iialt=reshape(ReturnMatrix_iialt+beta*entireEVinterp(aprimezalt),[N_d*n2long*N_a2,N_a,N_z]);
        [Vtempii,maxindexL2alt]=max(entireRHS_iialt,[],1);
