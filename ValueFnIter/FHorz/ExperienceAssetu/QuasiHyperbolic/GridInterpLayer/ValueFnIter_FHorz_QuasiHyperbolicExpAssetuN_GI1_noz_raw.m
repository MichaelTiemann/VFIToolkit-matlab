function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuN_GI1_noz_raw(n_d1, n_d2,n_a1,n_a2,n_u,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, u_gridvals, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_ExpAssetu_GI1_noz_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, n_d1,n_d2, n_a1, n_a1,n_a2, d_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, ReturnFnParamsVec,1,0); % Level=1, Refine=0
    ...
    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, n_d1, n_d2, n2long, n_a1,n_a2, d_gridvals, a1prime_grid(aprimeindexes), a1_gridvals, a2_gridvals, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
    ...
    ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, n_d1, n_d2, n_a1, n_a1,n_a2, d_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, ReturnFnParamsVec,1,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=1, Refine=0

    %% Valt (beta) -- capture Policyalt (exponential discounter's choice)
    entireRHSalt=ReturnMatrix+beta*repelem(entireEV,N_d1,1); % autofill 3rd dim to N_a1
    ...
    % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2
    ReturnMatrix_iialt=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, n_d1, n_d2, n2long, n_a1,n_a2, d_gridvals, a1prime_grid(a1primeindexesfinealt), a1_gridvals, a2_gridvals, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
    da1primea2alt=(1:1:N_d)'+N_d*(a1primeindexesfinealt-1)+N_d*N_a1prime*a2ind;
    entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(entireEVinterp(da1primea2alt(:)),[N_d*n2long,N_a1*N_a2]);
    ...
    %% Vtilde (beta0*beta)
    entireRHS=ReturnMatrix+beta0beta*repelem(entireEV,N_d1,1); % autofill 3rd dim to N_a1
    ...
    entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(entireEVinterp(da1primea2(:)),[N_d*n2long,N_a1*N_a2]);
...
for reverse_j=1:N_j-1
    ...
    % Interpolate EV over aprime_grid
    entireEVinterp=permute(interp1(a1_gridvals,permute(entireEV,[2,1,3,4]),a1prime_grid),[2,1,3,4]); % [N_d2,N_a1prime,1,N_a2]
    entireEVinterp=repelem(entireEVinterp,N_d1,1);
