function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuS_GI1_e_raw(n_d1,n_d2,n_a1,n_a2,n_z,n_e,n_u,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, z_gridvals_J, e_gridvals_J, u_gridvals, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_ExpAssetu_GI1_e_raw.
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                EVfine_z=reshape(entireEVinterp_z(da1primea2(:)),[N_d*n2long,N_a1*N_a2]);
                entireRHS_ii=ReturnMatrix_ii+beta0beta*EVfine_z;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                Vhat(:,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d)+1;
                allind=d_ind+N_d*aind; % midpoint is n_d-by-1-by-n_a1-by-n_a2
                ...
                Vunderbar(:,z_c,e_c,N_j)=Vhat(:,z_c,e_c,N_j)+(beta-beta0beta)*EV_at_policy_z;
            end
        end
    end
end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                linidx_z=reshape(maxindexL2,[1,N_a])+N_d*n2long*(0:N_a-1);
                EV_at_policy_z=reshape(EVfine_z(linidx_z),[N_a,1]);
                Vunderbar(:,z_c,e_c,jj)=Vhat(:,z_c,e_c,jj)+(beta-beta0beta)*EV_at_policy_z;
            end
        end
    end
end
