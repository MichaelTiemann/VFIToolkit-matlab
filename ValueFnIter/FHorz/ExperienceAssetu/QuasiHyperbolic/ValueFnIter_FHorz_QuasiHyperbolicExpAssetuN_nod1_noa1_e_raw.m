function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuN_nod1_noa1_e_raw(n_d2,n_a2,n_z,n_e,n_u,N_j, d2_gridvals, a2_grid,z_gridvals_J,e_gridvals_J,u_gridvals, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_alt=ReturnMatrix_ze+beta*EV_z;
                [Vtemp_alt,maxindex_alt]=max(entireRHS_alt,[],1);
                Valt(:,z_c,e_c,N_j)=shiftdim(Vtemp_alt,1);
                Policyalt(:,z_c,e_c,N_j)=shiftdim(maxindex_alt,1);
                entireRHS_tilde=ReturnMatrix_ze+beta0beta*EV_z;
                [Vtemp_tilde,maxindex_tilde]=max(entireRHS_tilde,[],1);
...
for reverse_j=1:N_j-1
    ...
    EVpre=sum(shiftdim(pi_e_J(:,jj+1),-2).*reshape(Valt(:,:,:,jj+1),[N_a,N_z,N_e]),3);
