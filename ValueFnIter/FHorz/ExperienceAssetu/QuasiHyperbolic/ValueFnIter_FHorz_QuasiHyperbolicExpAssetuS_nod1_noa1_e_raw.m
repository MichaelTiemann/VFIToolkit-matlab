function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuS_nod1_noa1_e_raw(n_d2,n_a2,n_z,n_e,n_u,N_j, d2_gridvals, a2_grid,z_gridvals_J,e_gridvals_J,u_gridvals, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        maxindexfull=maxindex+N_d2*(0:1:N_a-1)+shiftdim(N_d2*N_a*(0:1:N_z-1),-1)+shiftdim(N_d2*N_a*N_z*(0:1:N_e-1),-2);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            % hat: argmax of F + beta0*beta*EV; under: the beta-RHS gathered at that argmax
            entireRHS_hat=ReturnMatrix_e+beta0beta*EV;
            [Vtemp,maxindex]=max(entireRHS_hat,[],1);
            entireRHS_under=ReturnMatrix_e+beta*EV;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                % hat: argmax of F + beta0*beta*EV; under: the beta-RHS gathered at that argmax
                entireRHS_hat=ReturnMatrix_ze+beta0beta*EV_z;
                [Vtemp,maxindex]=max(entireRHS_hat,[],1);
                entireRHS_under=ReturnMatrix_ze+beta*EV_z;
...
for reverse_j=1:N_j-1
    ...
    EVpre=sum(shiftdim(pi_e_J(:,jj+1),-2).*reshape(Vunderbar(:,:,:,jj+1),[N_a,N_z,N_e]),3);
