function [Vhat,Policy3,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuSemiExoS_DC1_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,n_u,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, u_gridvals, pi_z_J, pi_semiz_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            dind=rem(maxindex2-1,N_d23)+1; % Do I need this shiftdim(), can probably delete all these
            Policy3(1,curraindex,zind,N_j)=rem(dind-1,N_d2)+1;
            Policy3(2,curraindex,zind,N_j)=ceil(dind/N_d2);
            Policy3(3,curraindex,zind,N_j)=ceil(maxindex2/N_d23);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy3(1,curraindex,zind,N_j)=rem(dind-1,N_d2)+1;
                    Policy3(2,curraindex,zind,N_j)=ceil(dind/N_d2);
                    Policy3(3,curraindex,zind,N_j)=ceil(maxindex/N_d23+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
                    % Just use aprime(ii) for everything
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_bothz
            ...
            Policy3(3,curraindex,z_c,N_j)=ceil(maxindex2/N_d23);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy3(1,curraindex,z_c,N_j)=rem(dind-1,N_d2)+1;
                    Policy3(2,curraindex,z_c,N_j)=ceil(dind/N_d2);
                    Policy3(3,curraindex,z_c,N_j)=ceil(maxindex/N_d23+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
                    % Just use aprime(ii) for everything
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_z
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                    ...
                    loweredge=maxindex1(:,1,ii,:,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level=2, Refine=0
    ...
    elseif vfoptions.lowmemory==1
        aprimeProbs_full=repmat(a2primeProbs,N_a1,1,1,N_bothz);  % [N_d2*N_a1,N_a2,N_u,N_bothz]
        for d3_c=1:N_d3
            % d3_val=d3_grid(d3_c);
            d23_gridvals_val=[d2_gridvals,repelem(d3_grid(d3_c),N_d2,1)];
            ...
            % Skip interpolation when upper and lower are equal (otherwise can cause numerical rounding errors)
            skipinterp=(EV1==EV2);
            aprimeProbs_full_d3=aprimeProbs_full; % fresh per d3: skipinterp varies with d3_c, so the zeroing must not accumulate
            aprimeProbs_full_d3(skipinterp)=0; % effectively skips interpolation

            % Apply the aprimeProbs
            EV=EV1.*aprimeProbs_full_d3+EV2.*(1-aprimeProbs_full_d3); % probability of lower grid point+ probability of upper grid point
            EV(aprimeProbs_full_d3==0)=EV2(aprimeProbs_full_d3==0); % includes the skipinterp positions; a zero weight against an infinite node gives 0*(-Inf)=NaN
            EV(aprimeProbs_full_d3==1)=EV1(aprimeProbs_full_d3==1);
            % Already applied the probabilities from interpolating onto grid
            EV=squeeze(sum((EV.*pi_u),3)); % (d2,a1prime,a2,bothz)
            ...
            for z_c=1:N_z
                ...
                entireRHS_ii_z=ReturnMatrix_ii_z+DiscountedEV_z_hat;
                entireRHS_ii_z_under=ReturnMatrix_ii_z+DiscountedEV_z_under;

                % First, we want a1prime conditional on (d,1,a)
                [~,maxindex1]=max(entireRHS_ii_z,[],2);

                % Now, get and store the full (d,aprime)
                entireRHS_ii_z_flat=reshape(entireRHS_ii_z,[N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                [Vtempii,maxindex2]=max(entireRHS_ii_z_flat,[],1);

                % Store
                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                V_ford3_hat(curraindex,zind,d3_c)=shiftdim(Vtempii,1);
                entireRHS_ii_z_under_flat=reshape(entireRHS_ii_z_under,[N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_semiz
                        ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_semiz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                        ...
                        entireRHS_ii=reshape(ReturnMatrix_ii_z+DiscountedEV_z_hat(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        entireRHS_ii_under=reshape(ReturnMatrix_ii_z+DiscountedEV_z_under(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_hat(curraindex,zind,d3_c)=shiftdim(Vtempii,1);
                        ...
                        Policy_ford3_hat(curraindex,zind,d3_c)=shiftdim(maxindex+N_d2*(loweredge(allind)-1),1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
                        % Just use aprime(ii) for everything
                        ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],1,level1iidiff(ii),n_a2,special_n_semiz, d23_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                        ...
                        entireRHS_ii=reshape(ReturnMatrix_ii_z+DiscountedEV_z_hat(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
                        entireRHS_ii_under=reshape(ReturnMatrix_ii_z+DiscountedEV_z_under(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_hat(curraindex,zind,d3_c)=shiftdim(Vtempii,1);
    ...
    elseif vfoptions.lowmemory==2
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                DiscountedEV_z_hat=beta0beta*reshape(EV_z,[N_d2,N_a1,1,N_a2]); % (d,a1prime,1,a2)   % QH-perceived
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2
                        ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_bothz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                        ...
                        entireRHS_ii_z=reshape(ReturnMatrix_ii_z+DiscountedEV_z_hat(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                        entireRHS_ii_z_under=reshape(ReturnMatrix_ii_z+DiscountedEV_z_under(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                        ...
                        Policy_ford3_hat(curraindex,z_c,d3_c)=shiftdim(maxindex+N_d2*(loweredge(allind)-1),1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                        % Just use aprime(ii) for everything
                        ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],1,level1iidiff(ii),n_a2,special_n_bothz, d23_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                        ...
                        entireRHS_ii_z=reshape(ReturnMatrix_ii_z+DiscountedEV_z_hat(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                        entireRHS_ii_z_under=reshape(ReturnMatrix_ii_z+DiscountedEV_z_under(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                        [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
                        V_ford3_hat(curraindex,z_c,d3_c)=shiftdim(Vtempii,1);
    ...
    d2a1prime_ind=reshape(Policy_ford3_hat((1:1:N_a*N_semiz*N_z)'+(N_a*N_semiz*N_z)*(maxindex-1)),[1,N_a,N_semiz*N_z]);
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_z
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                    d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3); % [N_d2,maxgap+1,1,N_a2,N_bothz]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_bothz]
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_hat(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                    entireRHS_ii_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    ...
                    loweredge=maxindex1(:,1,ii,:,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                    d2aprimez=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3); % [N_d2,1,1,N_a2,N_bothz]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_bothz]
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_hat(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_bothz]);
                    entireRHS_ii_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_bothz]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
