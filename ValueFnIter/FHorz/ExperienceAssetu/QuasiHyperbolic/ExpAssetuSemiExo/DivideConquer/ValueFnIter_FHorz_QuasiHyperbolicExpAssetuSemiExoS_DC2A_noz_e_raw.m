...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3_under_flat=reshape(entireRHS_ii_d3_under,[N_d12*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_semiz,N_e]);
            maxindexfull=maxindex2 + N_d12*N_a1*N_a2*(0:vfoptions.level1n*N_a2*N_a3-1) + N_d12*N_a1*N_a2*vfoptions.level1n*N_a2*N_a3*shiftdim((0:N_semiz-1),-1) + N_d12*N_a1*N_a2*vfoptions.level1n*N_a2*N_a3*N_semiz*shiftdim((0:N_e-1),-2);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_hat(d2aprimez),[N_d12*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz,N_e]);
                    entireRHS_ii_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d12*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    maxindexfull=maxindex + N_d12*(maxgap(ii)+1)*N_a2*(0:level1iidiff(ii)*N_a2*N_a3-1) + N_d12*(maxgap(ii)+1)*N_a2*level1iidiff(ii)*N_a2*N_a3*shiftdim((0:N_semiz-1),-1) + N_d12*(maxgap(ii)+1)*N_a2*level1iidiff(ii)*N_a2*N_a3*N_semiz*shiftdim((0:N_e-1),-2);
                    ...
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_hat(d2aprimez),[N_d12*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz,N_e]);
                    entireRHS_ii_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d12*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    maxindexfull=maxindex + N_d12*1*N_a2*(0:level1iidiff(ii)*N_a2*N_a3-1) + N_d12*1*N_a2*level1iidiff(ii)*N_a2*N_a3*shiftdim((0:N_semiz-1),-1) + N_d12*1*N_a2*level1iidiff(ii)*N_a2*N_a3*N_semiz*shiftdim((0:N_e-1),-2);
                    Vtempii_under=entireRHS_ii_under(maxindexfull);
                    V_ford3_hat(curraindex,:,:,d3_c)=shiftdim(Vtempii,1);
                    V_ford3_under(curraindex,:,:,d3_c)=shiftdim(Vtempii_under,1);
                    dind   =rem(maxindex-1,N_d12)+1;
    ...
    d12aprime_ind=reshape(Policy_ford3_hat((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindex-1)),[1,N_a,N_semiz,N_e]);
