function [Vtilde,Policy3,Valt,Policy3alt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuSemiExoN_noa1_raw(n_d1,n_d2,n_d3,n_a2,n_z,n_semiz,n_u,N_j, d12_gridvals, d2_gridvals, d3_grid, a2_grid, z_gridvals_J, semiz_gridvals_J, u_gridvals, pi_z_J, pi_semiz_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% noa1 version of ValueFnIter_FHorz_ExpAssetuSemiExo_raw (d1, z, noe, with u).
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            Policy3alt(3,:,zind,N_j)=ceil(maxindex/N_d12);
        end
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            EV1=reshape(EV(aprimeIndex,:),[N_d2,N_a2,N_u,N_bothz]);
            EV2=reshape(EV(aprimeplus1Index,:),[N_d2,N_a2,N_u,N_bothz]);
            aprimeProbs_full=repmat(a2primeProbs,1,1,1,N_bothz);
            skipinterp=(EV1==EV2);
            aprimeProbs_full(skipinterp)=0;
            entireEV=EV1.*aprimeProbs_full+EV2.*(1-aprimeProbs_full);
            entireEV=squeeze(sum((entireEV.*pi_u),3)); % integrate out u -> (d2,a2,bothz)
            ...
            for z_c=1:N_z
                ...
                entireRHS_alt=ReturnMatrix_d3z+beta*repelem(entireEV_z,N_d1,1,1);
                [Vtemp_alt,maxindex_alt]=max(entireRHS_alt,[],1);
                V_ford3_alt(:,zind,d3_c)=shiftdim(Vtemp_alt,1);
                Policy_ford3_alt(:,zind,d3_c)=shiftdim(maxindex_alt,1);
                entireRHS_tilde=ReturnMatrix_d3z+beta0beta*repelem(entireEV_z,N_d1,1,1);
                [Vtemp_tilde,maxindex_tilde]=max(entireRHS_tilde,[],1);
