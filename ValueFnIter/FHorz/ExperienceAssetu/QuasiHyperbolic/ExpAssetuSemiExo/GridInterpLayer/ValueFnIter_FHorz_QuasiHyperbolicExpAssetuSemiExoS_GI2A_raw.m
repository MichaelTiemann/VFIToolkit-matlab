...
Policy4_ford3_hat=zeros(4,N_a,N_semiz*N_z,N_d3,'gpuArray'); % 1=d1, 2=d2, 3=joint(a1prime midpoint,a2prime), 4=a1primeL2ind
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                Policy4_ford3_hat(3,:,zind,d3_c)=midpoint(allind)+N_a1*(maxindexL2a2-1); % joint(a1prime midpoint,a2prime)
                Policy4_ford3_hat(4,:,zind,d3_c)=maxindexL2a1; % a1primeL2ind
    ...
    Policy(1,:,:,N_j)=reshape(Policy4_ford3_hat(1+temp),[1,N_a,N_bothz]); % d1
    Policy(2,:,:,N_j)=reshape(Policy4_ford3_hat(2+temp),[1,N_a,N_bothz]); % d2
    Policy(4,:,:,N_j)=reshape(Policy4_ford3_hat(3+temp),[1,N_a,N_bothz]); % joint(a1prime,a2prime)
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            midpoint=max(min(maxindex,N_a1-1),2);
            a1primeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                midpoint=max(min(maxindex,N_a1-1),2);
                a1primeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_semiz, d123_gridvals, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                ...
                EVfine=reshape(entireEVinterp_z(aprime_z),[N_d12*n2long*N_a2,N_a,N_semiz]);
                entireRHS_ii_z=reshape(ReturnMatrix_ii_z,[N_d12*n2long*N_a2,N_a,N_semiz])+beta0beta*EVfine;
    ...
    elseif vfoptions.lowmemory==2
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                midpoint=max(min(maxindex,N_a1-1),2);
                a1primeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_bothz, d123_gridvals, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            midpoint=max(min(maxindex,N_a1-1),2);
            a1primeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
            aprimez=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            EVfine=reshape(entireEVinterp(aprimez),[N_d12*n2long*N_a2,N_a,N_bothz]);
            entireRHS_ii=reshape(ReturnMatrix_ii,[N_d12*n2long*N_a2,N_a,N_bothz])+beta0beta*EVfine;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    maxindex=reshape(maxindex,[N_a*N_bothz,1]);
    temp=4*( (1:1:N_a*N_bothz)'+(N_a*N_bothz)*(maxindex-1) -1);
    Policy(1,:,:,jj)=reshape(Policy4_ford3_hat(1+temp),[1,N_a,N_bothz]); % d1
    Policy(2,:,:,jj)=reshape(Policy4_ford3_hat(2+temp),[1,N_a,N_bothz]); % d2
    Policy(4,:,:,jj)=reshape(Policy4_ford3_hat(3+temp),[1,N_a,N_bothz]); % joint(a1prime,a2prime)
