...
% integrated out with pi_u when EV is formed, upstream of the maximisation.
%
% Sophisticated: Vhat_j      = max_{d,a1'} F + beta_0*beta*E[Vunderbar_{j+1}]
%                Vunderbar_j = Vhat_j + (beta - beta_0*beta)*EVfine_at_optimal_choice
% where F is the return function.
% EVfine is the (undiscounted) interpolated continuation actually added to the layer-2 RHS,
% so the a2 lottery and the u shock are already baked in and the gather needs no lottery
% handling. The gather is taken at the FINAL layer-2 argmax (maxindexL2), and then at the
% chosen d3.
...
Policy3_ford3_hat=zeros(3,N_a,N_semiz*N_z,N_e,N_d3,'gpuArray');
flag_ford3_hat=2*ones(N_a,N_semiz*N_z,N_e,N_d3,'gpuArray');
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,special_n_bothz,special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,2,0); % [N_d2,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
