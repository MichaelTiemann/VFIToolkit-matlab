...
V_ford3_under=zeros(N_a,N_semiz,N_d3,'gpuArray'); % beta-value gathered at the hat argmax
Policy4_ford3_hat=zeros(4,N_a,N_semiz,N_d3,'gpuArray');
flag_ford3_hat=2*ones(1,N_a,N_semiz,N_d3,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_semiz
                ...
                Policy4_ford3_hat(3,:,z_c,d3_c)=shiftdim(squeeze(midpoint(allind)),-1); % a1prime midpoint
                Policy4_ford3_hat(4,:,z_c,d3_c)=shiftdim(ceil(maxindexL2/N_d12),-1); % a1primeL2ind
                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                L2offset=ceil(maxindexL2/N_d12);
    ...
    Policy(1,:,:,N_j)=reshape(Policy4_ford3_hat(1+temp),[1,N_a,N_semiz]);
    Policy(2,:,:,N_j)=reshape(Policy4_ford3_hat(2+temp),[1,N_a,N_semiz]);
    Policy(4,:,:,N_j)=reshape(Policy4_ford3_hat(3+temp),[1,N_a,N_semiz]);
    Policy(5,:,:,N_j)=reshape(Policy4_ford3_hat(4+temp),[1,N_a,N_semiz]);
    flat_idx=(1:1:N_a*N_semiz)'+(N_a*N_semiz)*(maxindex-1);
    PolicyL2flag(1,:,:,N_j)=reshape(flag_ford3_hat(flat_idx),[1,N_a,N_semiz]);
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,n_semiz, d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, semiz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level=3, Refine=0
            ...
            % aprime possibilities are n_d12-by-n2long-by-n_a1-by-n_a2-by-n_semiz
            ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,n_semiz, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, semiz_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d2,N_a1prime,N_a1,N_a2,N_semiz]; Level=2, Refine=0
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for z_c=1:N_semiz
                ...
                entireRHS_ii_d3_z=ReturnMatrix_ii_d3_z+repelem(beta0beta*entireEV_z,N_d1,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_d3_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_semiz, d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0); % Level=3, Refine=0
                        d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d12,maxgap+1,1,N_a2]; linear index into entireEV_z [N_d2,N_a1,1,N_a2]
                        entireRHS_ii_d3_z=ReturnMatrix_ii_d3_z+beta0beta*entireEV_z(d2aprime);
                ...
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint
                % aprime possibilities are n_d12-by-n2long-by-n_a1-by-n_a2
                ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,special_n_semiz, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, ReturnFnParamsVec,2,0); % [N_d2,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
                ...
                EVfine=reshape(entireEVinterp_z(d2a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                entireRHS_ii_d3=ReturnMatrix_ii_d3+beta0beta*EVfine;
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
                V_ford3_hat(:,z_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d12)+1;
    ...
    Vunderbar(:,:,N_j)=reshape(V_ford3_under(flat_idx),[N_a,N_semiz]);
end
...
for reverse_j=1:N_j-1
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_semiz
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,n_semiz, d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, semiz_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level=3, Refine=0
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind; % [N_d12,maxgap+1,1,N_a2,N_semiz]; linear index into entireEV [N_d2,N_a1,1,N_a2,N_semiz]
                    entireRHS_ii_d3=ReturnMatrix_ii_d3+beta0beta*entireEV(d2aprimez);
                    [~,maxindex]=max(entireRHS_ii_d3,[],2);
                    midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
            ...
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint
            % aprime possibilities are n_d12-by-n2long-by-n_a1-by-n_a2-by-n_semiz
            ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,n_semiz, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, semiz_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0); % [N_d2,N_a1prime,N_a1,N_a2,N_semiz]; Level=2, Refine=0
            d2a1primea2semiz=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*semizind; % Note: EV does not depend on d1, but this does still have d1 as part of the first dimension
            EVfine=reshape(entireEVinterp(d2a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
            entireRHS_ii_d3=ReturnMatrix_ii_d3+beta0beta*EVfine;
            [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
    ...
    maxindex=reshape(maxindex,[N_a*N_semiz,1]); % This is the value of d that corresponds, make it this shape for addition just below
    temp=4*( (1:1:N_a*N_semiz)'+(N_a*N_semiz)*(maxindex-1) -1);
    Policy(1,:,:,jj)=reshape(Policy4_ford3_hat(1+temp),[1,N_a,N_semiz]);
    Policy(2,:,:,jj)=reshape(Policy4_ford3_hat(2+temp),[1,N_a,N_semiz]);
    Policy(4,:,:,jj)=reshape(Policy4_ford3_hat(3+temp),[1,N_a,N_semiz]);
    Policy(5,:,:,jj)=reshape(Policy4_ford3_hat(4+temp),[1,N_a,N_semiz]);
    flat_idx=(1:1:N_a*N_semiz)'+(N_a*N_semiz)*(maxindex-1);
    PolicyL2flag(1,:,:,jj)=reshape(flag_ford3_hat(flat_idx),[1,N_a,N_semiz]);
    % Vunderbar at the d3 chosen by the hat max
    Vunderbar(:,:,jj)=reshape(V_ford3_under(flat_idx),[N_a,N_semiz]);
end
