function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuSemiExoS_DC2A_GI2A_nod1_e_raw(n_d2, n_d3, n_a1, n_a2, n_a3, n_z, n_semiz, n_e, n_u, N_j, d2_gridvals, d3_grid, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, u_gridvals, pi_z_J, pi_semiz_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting + ExperienceAssetu + SemiExo, two standard endogenous
...
Vunderbar=zeros(N_a,N_semiz*N_z,N_e,N_j,'gpuArray'); % the beta-discounted value gathered at the hat argmax
...
V_ford3_under=zeros(N_a,N_semiz*N_z,N_e,N_d3,'gpuArray'); % beta-value gathered at the hat argmax, per d3
Policy3_ford3_hat=zeros(3,N_a,N_semiz*N_z,N_e,N_d3,'gpuArray'); % d2, joint(a1prime-midpoint,a2prime), a1primeL2ind
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Vhat(:,zind,e_c,N_j)=shiftdim(Vtempii,1);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            EV(isnan(EV))=0;
            EV=squeeze(sum(EV,3));
            entireEV=reshape(EV,[N_d2,N_a1,N_a2,1,1,N_a3,N_bothz]); % undiscounted; beta0beta is applied at the use sites and the undiscounted EVfine is reused for the Vunderbar gather
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                    ...
                    entireRHS_d3=ReturnMatrix_d3+beta0beta*entireEV(d2aprimez);
            ...
            EVfine=reshape(entireEVinterp(aprimez),[N_d2*n2long*N_a2,N_a,N_bothz,N_e]);
            entireRHS_ii_d3=reshape(ReturnMatrix_ii_d3,[N_d2*n2long*N_a2,N_a,N_bothz,N_e])+beta0beta*EVfine;
            ...
            Policy3_ford3_hat(3,:,:,:,d3_c)=maxindexL2a1; % a1primeL2ind
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                        d2aprimez=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                        entireRHS_d3=ReturnMatrix_d3+beta0beta*entireEV(d2aprimez);
                        [~,maxindex_inner]=max(entireRHS_d3,[],2);
                        midpoint(:,1,:,curra1inner,:,:,:)=maxindex_inner+(loweredge-1);
                ...
                entireRHS_ii_d3=reshape(ReturnMatrix_ii_d3,[N_d2*n2long*N_a2,N_a,N_bothz])+beta0beta*EVfine;
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
                ...
                Policy3_ford3_hat(3,:,:,e_c,d3_c)=maxindexL2a1; % a1primeL2ind
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_d3=ReturnMatrix_d3+beta0beta*entireEV_z(d2aprimez);
                            [~,maxindex_inner]=max(entireRHS_d3,[],2);
                            midpoint(:,1,:,curra1inner,:,:,:)=maxindex_inner+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                    ...
                    EVfine=reshape(entireEVinterp_z(aprimez),[N_d2*n2long*N_a2,N_a,N_semiz]);
                    ...
                    Policy3_ford3_hat(2,:,zind,e_c,d3_c)=midpoint(allind)+N_a1*(maxindexL2a2-1); % joint(a1prime midpoint,a2prime)
                    Policy3_ford3_hat(3,:,zind,e_c,d3_c)=maxindexL2a1; % a1primeL2ind
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                            d2aprime=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                            entireRHS_d3=ReturnMatrix_d3+beta0beta*entireEV_z(d2aprime);
                            [~,maxindex_inner]=max(entireRHS_d3,[],2);
                            midpoint(:,1,:,curra1inner,:,:)=maxindex_inner+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                    ...
                    EVfine=reshape(entireEVinterp_z(aprimez),[N_d2*n2long*N_a2,N_a]);
                    entireRHS_ii_d3=reshape(ReturnMatrix_ii_d3,[N_d2*n2long*N_a2,N_a])+beta0beta*EVfine;
                    [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
                    ...
                    Policy3_ford3_hat(3,:,z_c,e_c,d3_c)=maxindexL2a1; % a1primeL2ind
    ...
    Policy(1,:,:,:,N_j)=reshape(Policy3_ford3_hat(1+temp),[1,N_a,N_bothz,N_e]); % d2
    Policy(3,:,:,:,N_j)=reshape(Policy3_ford3_hat(2+temp),[1,N_a,N_bothz,N_e]); % joint(a1prime midpoint,a2prime)
    Policy(4,:,:,:,N_j)=reshape(Policy3_ford3_hat(3+temp),[1,N_a,N_bothz,N_e]); % a1primeL2ind
...
for reverse_j=1:N_j-1
    ...
    EVpre=squeeze(sum(Vunderbar(:,:,:,jj+1).*shiftdim(pi_e_J(:,jj+1),-2),3)); % [N_a,N_bothz]  -- continuation is Vunderbar (the beta-discounted value at the hat argmax)
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                    d2aprimez=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                    entireRHS_d3=ReturnMatrix_d3+beta0beta*entireEV(d2aprimez);
                    [~,maxindex_inner]=max(entireRHS_d3,[],2);
                    midpoint(:,1,:,curra1inner,:,:,:,:)=maxindex_inner+(loweredge-1);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                        d2aprimez=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                        entireRHS_d3=ReturnMatrix_d3+beta0beta*entireEV(d2aprimez);
                        [~,maxindex_inner]=max(entireRHS_d3,[],2);
                        midpoint(:,1,:,curra1inner,:,:,:)=maxindex_inner+(loweredge-1);
    ...
    maxindex=reshape(maxindex,[N_a*N_bothz*N_e,1]); % This is the value of d3 that corresponds, make it this shape for addition just below
    temp=3*( (1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1) -1);
    Policy(1,:,:,:,jj)=reshape(Policy3_ford3_hat(1+temp),[1,N_a,N_bothz,N_e]); % d2
    Policy(3,:,:,:,jj)=reshape(Policy3_ford3_hat(2+temp),[1,N_a,N_bothz,N_e]); % joint(a1prime midpoint,a2prime)
    Policy(4,:,:,:,jj)=reshape(Policy3_ford3_hat(3+temp),[1,N_a,N_bothz,N_e]); % a1primeL2ind
