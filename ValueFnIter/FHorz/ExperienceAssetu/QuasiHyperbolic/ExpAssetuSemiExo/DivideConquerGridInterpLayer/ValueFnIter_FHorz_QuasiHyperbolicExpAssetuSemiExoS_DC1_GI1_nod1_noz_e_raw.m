...
V_ford3_under=zeros(N_a,N_semiz,N_e,N_d3,'gpuArray'); % beta-value gathered at the hat argmax
Policy3_ford3_hat=zeros(3,N_a,N_semiz,N_e,N_d3,'gpuArray');
flag_ford3_hat=2*ones(1,N_a,N_semiz,N_e,N_d3,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    PolicyL2flag(1,:,:,:,N_j)=reshape(flag_ford3_hat(flat_idx),[1,N_a,N_semiz,N_e]);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireEV=reshape(EV,[N_d2,N_a1,1,N_a2,N_semiz]); % undiscounted, beta0beta applied at use sites
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind; % [N_d2,maxgap+1,1,N_a2,N_semiz,N_e]; linear index into entireEV [N_d2,N_a1,1,N_a2,N_semiz]
            ...
            EVfine=reshape(entireEVinterp(d2a1primea2semiz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz,N_e]);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind; % [N_d2,maxgap+1,1,N_a2,N_semiz]; linear index into entireEV [N_d2,N_a1,1,N_a2,N_semiz]
                        entireRHS_ii_d3=ReturnMatrix_ii_d3+beta0beta*entireEV(d2aprimez);
                        [~,maxindex]=max(entireRHS_ii_d3,[],2);
                        midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
                    else
                ...
                d2a1primea2semiz=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*semizind;
                EVfine=reshape(entireEVinterp(d2a1primea2semiz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
                entireRHS_ii_d3=ReturnMatrix_ii_d3+beta0beta*EVfine;
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
                V_ford3_hat(:,:,e_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d2)+1;
    ...
    Vunderbar(:,:,:,N_j)=reshape(V_ford3_under(flat_idx),[N_a,N_semiz,N_e]);
end
...
for reverse_j=1:N_j-1
    ...
    flat_idx=(1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindex-1);
    PolicyL2flag(1,:,:,:,jj)=reshape(flag_ford3_hat(flat_idx),[1,N_a,N_semiz,N_e]);
    % Vunderbar at the d3 chosen by the hat max
    Vunderbar(:,:,:,jj)=reshape(V_ford3_under(flat_idx),[N_a,N_semiz,N_e]);
end
