    ...
    elseif vfoptions.lowmemory==2
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                entireRHS_alt=ReturnMatrix_d3z+beta*kron(EV_z,ones(1,N_a1));
                [Vtemp_alt,maxindex_alt]=max(entireRHS_alt,[],1);
                V_ford3_alt(:,z_c,d3_c)=Vtemp_alt;
                Policy_ford3_alt(:,z_c,d3_c)=maxindex_alt;
                entireRHS_tilde=ReturnMatrix_d3z+beta0beta*kron(EV_z,ones(1,N_a1));
                [Vtemp_tilde,maxindex_tilde]=max(entireRHS_tilde,[],1);
    ...
    elseif vfoptions.lowmemory==2
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                %Calc the condl expectation term (except beta), which depends on z but not on control variables
                EV_z=EVpre.*shiftdim(pi_bothz_d3(z_c,:)',-1);
                EV_z(isnan(EV_z))=0; %multiplications of -Inf with 0 gives NaN, this replaces them with zeros (as the zeros come from the transition probabilities)
                EV_z=sum(EV_z,2);
                ...
                entireRHS_alt=ReturnMatrix_d3z+beta*repelem(EV_z,1,N_a1);
                [Vtemp_alt,maxindex_alt]=max(entireRHS_alt,[],1);
                V_ford3_alt(:,z_c,d3_c)=shiftdim(Vtemp_alt,1);
                Policy_ford3_alt(:,z_c,d3_c)=shiftdim(maxindex_alt,1);
                entireRHS_tilde=ReturnMatrix_d3z+beta0beta*repelem(EV_z,1,N_a1);
                [Vtemp_tilde,maxindex_tilde]=max(entireRHS_tilde,[],1);
