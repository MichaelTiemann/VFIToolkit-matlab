function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuS_e_raw(n_d1,n_d2,n_a1,n_a2,n_z,n_e,n_u,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, z_gridvals_J, e_gridvals_J, u_gridvals, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        maxindexfull=maxindex+N_d12*N_a1*(0:1:N_a-1)+shiftdim(N_d12*N_a1*N_a*(0:1:N_z-1),-1)+shiftdim(N_d12*N_a1*N_a*N_z*(0:1:N_e-1),-2);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            % hat: argmax of F + beta0*beta*EV; under: the beta-RHS gathered at that argmax
            entireRHS_hat=ReturnMatrix_e+DiscountedEV_hat;
            [Vtemp,maxindex]=max(entireRHS_hat,[],1);
            entireRHS_under=ReturnMatrix_e+DiscountedEV_under;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                % hat: argmax of F + beta0*beta*EV; under: the beta-RHS gathered at that argmax
                entireRHS_hat=ReturnMatrix_ze+DiscountedEV_z_hat;
                [Vtemp,maxindex]=max(entireRHS_hat,[],1);
                entireRHS_under=ReturnMatrix_ze+DiscountedEV_z_under;
                maxindexfull=maxindex+N_d12*N_a1*(0:1:N_a-1);
                Vhat(:,z_c,e_c,N_j)=shiftdim(Vtemp,1);
                Vunderbar(:,z_c,e_c,N_j)=shiftdim(entireRHS_under(maxindexfull),1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_under=ReturnMatrix_ze+DiscountedEV_z_under;
                maxindexfull=maxindex+N_d12*N_a1*(0:1:N_a-1);
                Vhat(:,z_c,e_c,jj)=shiftdim(Vtemp,1);
                Vunderbar(:,z_c,e_c,jj)=shiftdim(entireRHS_under(maxindexfull),1);
