function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuS_nod1_noa1_noz_raw(n_d2,n_a2,n_u,N_j, d2_gridvals, a2_grid, u_gridvals, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
for reverse_j=1:N_j-1
    ...
    Vlower=reshape(Vunderbar(a2primeIndex,jj+1),[N_d2,N_a2,N_u]);
    Vupper=reshape(Vunderbar(a2primeIndex+1,jj+1),[N_d2,N_a2,N_u]);
    % Skip interpolation when upper and lower are equal (otherwise can cause numerical rounding errors)
    skipinterp=(Vlower==Vupper);
    a2primeProbs(skipinterp)=0; % effectively skips interpolation
