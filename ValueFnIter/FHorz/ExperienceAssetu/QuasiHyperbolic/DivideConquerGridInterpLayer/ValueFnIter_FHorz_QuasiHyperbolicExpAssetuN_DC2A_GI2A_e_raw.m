function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuN_DC2A_GI2A_e_raw(n_d1, n_d2, n_a1, n_a2, n_a3, n_z, n_e, n_u, N_j, d_gridvals, d2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, e_gridvals_J, u_gridvals, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_ExpAssetu_DC2A_GI2A_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_iialt=reshape(ReturnMatrix_ii_gialt+beta*entireEVinterp(aprimezalt),[N_d*n2long*N_a2,N_a,N_z,N_e]);
        ...
        entireRHS_ii=reshape(ReturnMatrix_ii_gi+beta0beta*entireEVinterp(aprimez),[N_d*n2long*N_a2,N_a,N_z,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            entireRHS_ii_zalt=reshape(ReturnMatrix_ii_z_gialt+beta*entireEVinterp_z(aprimealt),[N_d*n2long*N_a2,N_a,1,N_e]);
            ...
            entireRHS_ii_z=reshape(ReturnMatrix_ii_z_gi+beta0beta*entireEVinterp_z(aprime),[N_d*n2long*N_a2,N_a,1,N_e]);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 1);
                %% Valt (beta) -- capture Policyalt (exponential discounter's choice)
                entireRHS_ii_zealt=ReturnMatrix_ii_ze+beta*repelem(entireEV_z,N_d1,1,1,1,1,1);
                ...
                aprimealt=d2ind_vec + N_d2*(a1primeindexesfinealt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
                entireRHS_ii_zealt=reshape(ReturnMatrix_ii_ze_gialt+beta*entireEVinterp_z(aprimealt),[N_d*n2long*N_a2,N_a]);
                ...
                %% Vtilde (beta0*beta)
                entireRHS_ii_ze=ReturnMatrix_ii_ze+beta0beta*repelem(entireEV_z,N_d1,1,1,1,1,1);
                ...
                aprime=d2ind_vec + N_d2*(a1primeindexesfine-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
                entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze_gi+beta0beta*entireEVinterp_z(aprime),[N_d*n2long*N_a2,N_a]);
