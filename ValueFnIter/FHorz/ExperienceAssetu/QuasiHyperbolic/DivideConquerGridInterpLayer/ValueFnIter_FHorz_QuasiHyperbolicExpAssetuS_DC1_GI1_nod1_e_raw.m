...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    entireEV=reshape(EV,[N_d2,N_a1,1,N_a2,N_z]); % (d2,a1prime,1,a2,zprime); undiscounted, beta0beta applied at use sites
    % Interpolate EV over aprime_grid
    entireEVinterp=permute(interp1(a1_gridvals,permute(entireEV,[2,1,3,4,5]),a1prime_grid),[2,1,3,4,5]);   % [N_d2,N_a1prime,1,N_a2]
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ReturnMatrix_ii+beta0beta*entireEV; % autofill e for DiscountedentireEV
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_z-by-n_e
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d2,maxgap+1,1,N_a2,N_z,N_e]; linear index into entireEV [N_d2,N_a1,1,N_a2,N_z]
        ...
        da1primea2z=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind; % [N_d2,n2long,N_a1,N_a2,N_z,N_e]; linear index into entireEVinterp [N_d2,N_a1prime,1,N_a2,N_z]
        EVfine=reshape(entireEVinterp(da1primea2z),[N_d2*n2long,N_a1*N_a2,N_z,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii_e=ReturnMatrix_ii_e+beta0beta*entireEV;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_z
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d2,maxgap+1,1,N_a2,N_z]; linear index into entireEV [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii_e=ReturnMatrix_ii_e+beta0beta*entireEV(d2aprimez);
            ...
            da1primea2z=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind; % [N_d2,n2long,N_a1,N_a2,N_z]; linear index into entireEVinterp [N_d2,N_a1prime,1,N_a2,N_z]
            EVfine=reshape(entireEVinterp(da1primea2z),[N_d2*n2long,N_a1*N_a2,N_z]);
            entireRHS_ii=ReturnMatrix_ii+beta0beta*EVfine;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            Vhat(:,:,e_c,N_j)=shiftdim(Vtempii,1);
            d_ind=rem(maxindexL2-1,N_d2)+1;
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii_ze=ReturnMatrix_ii_z+beta0beta*entireEV_z(d2aprime);
                ...
                da1primea2=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d2,n2long,N_a1,N_a2]; linear index into entireEVinterp_z [N_d2,N_a1prime,1,N_a2]
                EVfine=reshape(entireEVinterp_z(da1primea2),[N_d2*n2long,N_a1*N_a2]);
                entireRHS_ii=ReturnMatrix_ii+beta0beta*EVfine;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                Vhat(:,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d2)+1;
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_z
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d2,maxgap+1,1,N_a2,N_z]; linear index into entireEV [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii_e=ReturnMatrix_ii+beta0beta*entireEV(d2aprimez);
                    [~,maxindex]=max(entireRHS_ii_e,[],2);
                    midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_z,special_n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                        d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d2,maxgap+1,1,N_a2]; linear index into entireEV_z [N_d2,N_a1,1,N_a2]
                        entireRHS_ii_ze=ReturnMatrix_ii_ze+beta0beta*entireEV_z(d2aprime);
                        [~,maxindex]=max(entireRHS_ii_ze,[],2);
                        midpoint(:,1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
