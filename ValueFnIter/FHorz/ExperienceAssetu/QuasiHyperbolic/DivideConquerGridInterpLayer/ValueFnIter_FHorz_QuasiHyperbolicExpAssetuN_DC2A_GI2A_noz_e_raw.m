function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuN_DC2A_GI2A_noz_e_raw(n_d1, n_d2, n_a1, n_a2, n_a3, n_e, n_u, N_j, d_gridvals, d2_gridvals, a1_grid, a2_gridvals, a3_grid, e_gridvals_J, u_gridvals, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_ExpAssetu_DC2A_GI2A_noz_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_iialt=ReturnMatrix_ii+beta*repelem(entireEV,N_d1,1,1,1,1,1);
        ...
        entireRHS_iialt=reshape(ReturnMatrix_ii_gialt+beta*entireEVinterp(aprimealt),[N_d*n2long*N_a2,N_a,N_e]);
        ...
        %% Vtilde (beta0*beta)
        entireRHS_ii=ReturnMatrix_ii+beta0beta*repelem(entireEV,N_d1,1,1,1,1,1);
        ...
        entireRHS_ii=reshape(ReturnMatrix_ii_gi+beta0beta*entireEVinterp(aprime),[N_d*n2long*N_a2,N_a,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 1);
            %% Valt (beta) -- capture Policyalt (exponential discounter's choice)
            entireRHS_ii_ealt=ReturnMatrix_ii_e+beta*repelem(entireEV,N_d1,1,1,1,1,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    d2aprimealt=d2ind_vec + N_d2*(a1primeindexesalt-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_ealt=ReturnMatrix_ii_e_dcalt+beta*entireEV(d2aprimealt);
            ...
            aprimealt=d2ind_vec + N_d2*(a1primeindexesfinealt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
            entireRHS_ii_ealt=reshape(ReturnMatrix_ii_e_gialt+beta*entireEVinterp(aprimealt),[N_d*n2long*N_a2,N_a]);
            ...
            %% Vtilde (beta0*beta)
            entireRHS_ii_e=ReturnMatrix_ii_e+beta0beta*repelem(entireEV,N_d1,1,1,1,1,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_e=ReturnMatrix_ii_e_dc+beta0beta*entireEV(d2aprime);
            ...
            aprime=d2ind_vec + N_d2*(a1primeindexesfine-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
            entireRHS_ii_e=reshape(ReturnMatrix_ii_e_gi+beta0beta*entireEVinterp(aprime),[N_d*n2long*N_a2,N_a]);
