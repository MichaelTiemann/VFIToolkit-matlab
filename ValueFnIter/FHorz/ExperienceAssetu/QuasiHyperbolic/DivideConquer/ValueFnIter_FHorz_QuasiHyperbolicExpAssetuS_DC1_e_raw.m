function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuS_DC1_e_raw(n_d1,n_d2,n_a1,n_a2,n_z,n_e,n_u,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, z_gridvals_J, e_gridvals_J, u_gridvals, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV_hat=beta0beta*reshape(EV,[N_d2,N_a1,1,N_a2,N_z]); % (d2,a1prime,1,a2,zprime); d1-dim is implicit singleton, broadcasts at use sites   % QH-perceived
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ReturnMatrix_ii+repelem(DiscountedEV_hat,N_d1,1,1,1,1); % autofill e for DiscountedentireEV
        entireRHS_ii_under=ReturnMatrix_ii+repelem(DiscountedEV_under,N_d1,1,1,1,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d,maxgap+1,1,N_a2,N_z,N_e]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_z]
                ...
                d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d,1,1,N_a2,N_z,N_e]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_z]
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii=ReturnMatrix_ii+repelem(DiscountedEV_hat,N_d1,1,1,1,1);
            entireRHS_ii_under=ReturnMatrix_ii+repelem(DiscountedEV_under,N_d1,1,1,1,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d,maxgap+1,1,N_a2,N_z]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprimez),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                    entireRHS_ii_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprimez),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    ...
                    d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d,1,1,N_a2,N_z]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprimez),[N_d,level1iidiff(ii)*N_a2,N_z]);
                    entireRHS_ii_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprimez),[N_d,level1iidiff(ii)*N_a2,N_z]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii_z=ReturnMatrix_ii_z+repelem(DiscountedEV_hat_z,N_d1,1,1,1);
                entireRHS_ii_z_under=ReturnMatrix_ii_z+repelem(DiscountedEV_under_z,N_d1,1,1,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii_z=reshape(ReturnMatrix_ii_z+DiscountedEV_hat_z(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                        entireRHS_ii_z_under=reshape(ReturnMatrix_ii_z+DiscountedEV_under_z(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                        [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
                        Vhat(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        maxindexfull=maxindex+N_d*(0:level1iidiff(ii)*N_a2-1);
                        Vtempii_under=entireRHS_ii_z_under(maxindexfull);
                        Vunderbar(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii_under,1);
                        % maxindex does not need reworking, as with expasset there is no a2prime
                        %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
...
for reverse_j=1:N_j-1
    ...
    EVpre=sum(shiftdim(pi_e_J(:,jj+1),-2).*Vunderbar(:,:,:,jj+1),3); % First, switch V_Jplus1 into Kron form
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii_e=ReturnMatrix_ii_e+repelem(DiscountedEV_hat,N_d1,1,1,1,1);
            entireRHS_ii_e_under=ReturnMatrix_ii_e+repelem(DiscountedEV_under,N_d1,1,1,1,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d,maxgap+1,1,N_a2,N_z]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat(d2aprimez),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                    entireRHS_ii_e_under=reshape(ReturnMatrix_ii_e+DiscountedEV_under(d2aprimez),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                    ...
                    d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d,1,1,N_a2,N_z]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat(d2aprimez),[N_d,level1iidiff(ii)*N_a2,N_z]);
                    entireRHS_ii_e_under=reshape(ReturnMatrix_ii_e+DiscountedEV_under(d2aprimez),[N_d,level1iidiff(ii)*N_a2,N_z]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    Vhat(curraindex,:,e_c,jj)=shiftdim(Vtempii,1);
                    maxindexfull=maxindex+N_d*(0:level1iidiff(ii)*N_a2-1)+N_d*level1iidiff(ii)*N_a2*shiftdim((0:N_z-1),-1);
                    Vtempii_under=entireRHS_ii_e_under(maxindexfull);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_hat_z,N_d1,1,1,1);
                entireRHS_ii_ze_under=ReturnMatrix_ii_ze+repelem(DiscountedEV_under_z,N_d1,1,1,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind; % [N_d,1,1,N_a2]; linear index into DiscountedEV_hat_z [N_d2,N_a1,1,N_a2]
                        entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze+DiscountedEV_hat_z(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                        entireRHS_ii_ze_under=reshape(ReturnMatrix_ii_ze+DiscountedEV_under_z(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                        [Vtempii,maxindex]=max(entireRHS_ii_ze,[],1);
                        Vhat(curraindex,z_c,e_c,jj)=shiftdim(Vtempii,1);
                        maxindexfull=maxindex+N_d*(0:level1iidiff(ii)*N_a2-1);
                        Vtempii_under=entireRHS_ii_ze_under(maxindexfull);
