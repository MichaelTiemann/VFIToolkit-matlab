...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_hat_flat=reshape(entireRHS_hat,[N_d*N_a1*N_a2,M,N_e]);
        [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
        entireRHS_under=ReturnMatrix_ii+repelem(DiscountedEV_under,N_d1,1,1,1,1,1);
        entireRHS_under_flat=reshape(entireRHS_under,[N_d*N_a1*N_a2,M,N_e]);
        maxindexfull=maxindex2 + (N_d*N_a1*N_a2)*(0:M-1) + (N_d*N_a1*N_a2)*M*shiftdim((0:N_e-1),-1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_hat=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprime),[firstdim,Mblock,N_e]);
                entireRHS_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprime),[firstdim,Mblock,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            e_val=e_gridvals_J(e_c,:,N_j);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 1);
            entireRHS_hat_e=ReturnMatrix_ii_e+repelem(DiscountedEV_hat,N_d1,1,1,1,1,1);
            [~,maxindex1]=max(entireRHS_hat_e,[],2);
            M=vfoptions.level1n*N_a2*N_a3;
            entireRHS_hat_flat=reshape(entireRHS_hat_e,[N_d*N_a1*N_a2,M]);
            [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
            entireRHS_under_e=ReturnMatrix_ii_e+repelem(DiscountedEV_under,N_d1,1,1,1,1,1);
            entireRHS_under_flat=reshape(entireRHS_under_e,[N_d*N_a1*N_a2,M]);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    firstdim=N_d*(maxgap(ii)+1)*N_a2;
                    Mblock=level1iidiff(ii)*N_a2*N_a3;
                    entireRHS_hat_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat(d2aprime),[firstdim,Mblock]);
                    entireRHS_under_e=reshape(ReturnMatrix_ii_e+DiscountedEV_under(d2aprime),[firstdim,Mblock]);
