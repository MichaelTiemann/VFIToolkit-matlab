function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuS_DC1_nod1_noz_e_raw(n_d2,n_a1,n_a2,n_e,n_u,N_j, d2_gridvals, a1_gridvals, a2_grid, e_gridvals_J, u_gridvals, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV_hat=beta0beta*reshape(EV,[N_d2,N_a1,1,N_a2]); % (d,a1prime,1,a2)   % QH-perceived
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
                ...
                d2aprime=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,1,1,N_a2,N_e]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,maxgap+1,1,N_a2]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    entireRHS_ii_e_under=reshape(ReturnMatrix_ii_e+DiscountedEV_under(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    ...
                    d2aprime=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,1,1,N_a2]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                    entireRHS_ii_e_under=reshape(ReturnMatrix_ii_e+DiscountedEV_under(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    Vhat(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
