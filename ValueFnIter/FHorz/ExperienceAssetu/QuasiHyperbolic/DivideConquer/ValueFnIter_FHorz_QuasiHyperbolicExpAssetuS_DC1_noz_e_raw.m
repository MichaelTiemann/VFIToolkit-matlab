function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuS_DC1_noz_e_raw(n_d1, n_d2,n_a1,n_a2,n_e,n_u,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid,e_gridvals_J, u_gridvals, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV_hat=beta0beta*reshape(EV,[N_d2,N_a1,1,N_a2]); % (d2,a1prime,1,a2); d1-dim is implicit singleton, broadcasts at use sites   % QH-perceived
    DiscountedEV_under=beta*reshape(EV,[N_d2,N_a1,1,N_a2]);   % exponential

    if vfoptions.lowmemory==0
        % n-Monotonicity
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,n_a1,vfoptions.level1n,n_a2,n_e, d_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0); % Level=1, Refine=0

        entireRHS_ii=ReturnMatrix_ii+repelem(DiscountedEV_hat,N_d1,1,1,1); % autofill e for DiscountedentireEV
        entireRHS_ii_under=ReturnMatrix_ii+repelem(DiscountedEV_under,N_d1,1,1,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
                ...
                d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,1,1,N_a2,N_e]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii_e=ReturnMatrix_ii_e+repelem(DiscountedEV_hat,N_d1,1,1,1);
            entireRHS_ii_e_under=ReturnMatrix_ii_e+repelem(DiscountedEV_under,N_d1,1,1,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap+1,1,N_a2]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat(d2aprime),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    entireRHS_ii_e_under=reshape(ReturnMatrix_ii_e+DiscountedEV_under(d2aprime),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    ...
                    d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,1,1,N_a2]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    entireRHS_ii_e_under=reshape(ReturnMatrix_ii_e+DiscountedEV_under(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    Vhat(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
...
for reverse_j=1:N_j-1
    ...
    EVpre=sum(pi_e_J(:,jj+1)'.*Vunderbar(:,:,jj+1),2);    % Expectations over e
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap+1,1,N_a2]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprime),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    entireRHS_ii_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprime),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat(curraindex,e_c,jj)=shiftdim(Vtempii,1);
                    maxindexfull=maxindex+N_d*(maxgap(ii)+1)*(0:level1iidiff(ii)*N_a2-1);
                    Vtempii_under=entireRHS_ii_under(maxindexfull);
                    ...
                    d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,1,1,N_a2]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2]
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    entireRHS_ii_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat(curraindex,e_c,jj)=shiftdim(Vtempii,1);
                    maxindexfull=maxindex+N_d*(0:level1iidiff(ii)*N_a2-1);
                    Vtempii_under=entireRHS_ii_under(maxindexfull);
