function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuN_DC1_nod1_noz_raw(n_d2,n_a1,n_a2,n_u,N_j, d2_gridvals, a1_gridvals, a2_grid, u_gridvals, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    % n-Monotonicity
    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, 0, n_d2, n_a1, vfoptions.level1n, n_a2, d2_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, ReturnFnParamsVec,1,0); % Level=1, Refine=0
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, 0,n_d2, maxgap(ii)+1, level1iidiff(ii), n_a2, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, ReturnFnParamsVec,2,0); % Level=2, Refine=0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, 0, n_d2, 1, level1iidiff(ii), n_a2, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, ReturnFnParamsVec,2,0); % Level=2, Refine=0
            [Vtempii,maxindex]=max(ReturnMatrix_ii,[],1);
            Valt(curraindex,N_j)=shiftdim(Vtempii,1);
            % maxindex does not need reworking, as with expasset there is no a2prime
            %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
    ...
    % Now, get and store the full (d,aprime)
    [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii,[N_d2*N_a1,vfoptions.level1n*N_a2]),[],1);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap_V(ii)>0
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, 0, n_d2, maxgap_V(ii)+1, level1iidiff(ii), n_a2, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, ReturnFnParamsVec,3,0); % Level=2, Refine=0
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d2*(maxgap_V(ii)+1),level1iidiff(ii)*N_a2]);
            ...
            loweredge=maxindex1(:,1,ii,:);
            % Just use aprime(ii) for everything
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, 0, n_d2, 1, level1iidiff(ii), n_a2, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, ReturnFnParamsVec,3,0); % Level=2, Refine=0
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
            [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
            Valt(curraindex,N_j)=shiftdim(Vtempii,1);
            % maxindexalt does not need reworking, as with expasset there is no a2prime
            %  the a1prime is relative to loweredge(allindalt), need to 'add' the loweredge
    ...
    % Now, get and store the full (d,aprime)
    [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d2*N_a1,vfoptions.level1n*N_a2]),[],1);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            a1primeindexes=loweredge+(0:1:maxgap(ii));
            % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, 0, n_d2, maxgap(ii)+1, level1iidiff(ii), n_a2, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, ReturnFnParamsVec,3,0); % Level=2, Refine=0
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            Vtilde(curraindex,N_j)=shiftdim(Vtempii,1);
            % maxindex does not need reworking, as with expasset there is no a2prime
            %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
