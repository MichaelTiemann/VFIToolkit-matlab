function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetuN_nod1_noa1_raw(n_d2,n_a2,n_z,n_u,N_j, d2_gridvals, a2_grid,z_gridvals_J, u_gridvals, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            entireRHS_alt=ReturnMatrix_z+beta*EV_z;
            [Vtemp_alt,maxindex_alt]=max(entireRHS_alt,[],1);
            Valt(:,z_c,N_j)=shiftdim(Vtemp_alt,1);
            Policyalt(:,z_c,N_j)=shiftdim(maxindex_alt,1);
            entireRHS_tilde=ReturnMatrix_z+beta0beta*EV_z;
            [Vtemp_tilde,maxindex_tilde]=max(entireRHS_tilde,[],1);
...
for reverse_j=1:N_j-1
    ...
    Vlower=reshape(Valt(a2primeIndex,:,jj+1),[N_d2,N_a2,N_u,N_z]);
    Vupper=reshape(Valt(a2primeIndex+1,:,jj+1),[N_d2,N_a2,N_u,N_z]);
    % Skip interpolation when upper and lower are equal (otherwise can cause numerical rounding errors)
    skipinterp=(Vlower==Vupper);
    a2primeProbs(skipinterp)=0; % effectively skips interpolation
