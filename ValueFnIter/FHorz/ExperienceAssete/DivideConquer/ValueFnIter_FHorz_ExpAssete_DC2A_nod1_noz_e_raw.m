function [V,Policy]=ValueFnIter_FHorz_ExpAssete_DC2A_nod1_noz_e_raw(n_d2, n_a1, n_a2, n_a3, n_e, N_j, d2_gridvals, a1_grid, a2_gridvals, a3_grid, e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% nod1 noz_e DC2A analog of ValueFnIter_FHorz_ExpAssete_DC1_nod1_noz_e_raw (e-dep aprimeFn, no Markov z, i.i.d. e).
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d2aprimee),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_e]);
                ...
                entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d2aprimee),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,:,N_j)=shiftdim(Vtempii,1);
                d2ind  =rem(maxindex-1,N_d2)+1;
                a2pind =floor((maxindex-1)/N_d2)+1;
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_e(d2aprime),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                    ...
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_e(d2aprime),[N_d2*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    V(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                    d2ind  =rem(maxindex-1,N_d2)+1;
                    a2pind =floor((maxindex-1)/N_d2)+1;
