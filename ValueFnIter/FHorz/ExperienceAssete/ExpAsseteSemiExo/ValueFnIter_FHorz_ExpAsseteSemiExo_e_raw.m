...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        Policy4(4,:,:,:,N_j)=ceil(maxindex/N_d); % a1prime
    ...
    % aprime depends on (d2, a1, a2, current_e); independent of d3, semiz, z -- compute once
    ...
    % aprimeIndex/aprimeProbs are [N_d2,N_a2,N_e]
    ...
    % Integrate e' first (e is i.i.d. start-of-period); EVpre is [N_a, N_bothz]
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            EV(isnan(EV))=0; % multiplications of -Inf with 0 gives NaN
            EV=sum(EV,2); % sum over zprime, leaving singular second dim

            EV_byzcur=reshape(EV,[N_a,N_bothz]); % [N_a, N_bothzcur]

            % aprime depends on (d2,a1,a2,e), independent of bothz -- index [N_a] per (d2,a1,a2,e), then expand over bothzcur
            Vlower=reshape(EV_byzcur(aprimeIndex(:),:),[N_d2*N_a1,N_a2,N_e,N_bothz]); % (d2*a1prime,a2,e_cur,bothzcur)
            Vupper=reshape(EV_byzcur(aprimeplus1Index(:),:),[N_d2*N_a1,N_a2,N_e,N_bothz]);
            skipinterp=(Vlower==Vupper);
            aprimeProbs_d3=repmat(aprimeProbs_d2a1a2e,1,1,1,N_bothz); % [N_d2*N_a1,N_a2,N_e,N_bothz]
            aprimeProbs_d3(skipinterp)=0;
            entireEV=aprimeProbs_d3.*Vlower+(1-aprimeProbs_d3).*Vupper; % (d2*a1prime,a2,e_cur,bothzcur)
            entireEV=permute(entireEV,[1,2,4,3]); % (d2*a1prime,a2,bothzcur,e_cur) -- match return-fn dim order
