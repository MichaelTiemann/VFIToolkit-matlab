function [V,Policy3]=ValueFnIter_FHorz_ExpAsseteSemiExo_DC1_nod1_e_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,n_e,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% d2 determines experience asset, d3 determines semi-exog state
% a1 is standard endogenous state, a2 is experience asset
% z is exogenous markov state (required), semiz is semi-exog state, e is i.i.d. start-of-period (required); no d1
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                allind=dind+N_d23*a2Bind+N_d23*N_a2*bothzBind+N_d23*N_a2*N_bothz*eBind;
                Policy3(1,curraindex,:,:,N_j)=rem(dind-1,N_d2)+1;
                Policy3(2,curraindex,:,:,N_j)=ceil(dind/N_d2);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    dind=(rem(maxindex-1,N_d23)+1);
                    a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                    allind=dind+N_d23*a2Bind+N_d23*N_a2*bothzBind;
                    Policy3(1,curraindex,:,e_c,N_j)=rem(dind-1,N_d2)+1;
                    Policy3(2,curraindex,:,e_c,N_j)=ceil(dind/N_d2);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3=ReturnMatrix_ii_d3+reshape(DiscountedEV,[N_d2,N_a1,1,N_a2,N_bothz,N_e]);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind+N_d2*N_a*N_bothz*eind;
                    ...
                    a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                    allind=dind+N_d2*a2Bind+N_d2*N_a2*bothzBind+N_d2*N_a2*N_bothz*eBind;
                    ...
                    d2aprimez=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind+N_d2*N_a*N_bothz*eind;
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii_d3e=ReturnMatrix_ii_d3e+reshape(DiscountedEV_e,[N_d2,N_a1,1,N_a2,N_bothz]);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind;
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3e+DiscountedEV_e(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_jj(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d2)+1);
                        a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        allind=dind+N_d2*a2Bind+N_d2*N_a2*bothzBind;
                        ...
                        d2aprimez=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind;
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3e+DiscountedEV_e(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_jj(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d2)+1);
                        a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+reshape(DiscountedEV_ze,[N_d2,N_a1,1,N_a2,N_semiz]);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_ze(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d2)+1);
                            a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                            allind=dind+N_d2*a2Bind+N_d2*N_a2*semizBind;
                            Policy_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(maxindex+N_d2*(loweredge(allind)-1),1);
                        else
                            loweredge=maxindex1(:,1,ii,:,:);
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_ze(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d2)+1);
                            a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    d2a1prime_ind=reshape(Policy_ford3_jj((1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1)),[1,N_a,N_bothz,N_e]);
