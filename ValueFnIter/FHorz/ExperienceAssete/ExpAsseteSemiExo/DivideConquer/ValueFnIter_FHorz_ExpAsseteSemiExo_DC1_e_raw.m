...
if vfoptions.lowmemory==0
    eBind=shiftdim((0:1:N_e-1),-2); % for allind
    eind=shiftdim((0:1:N_e-1),-4); % for d2aprimez stride into 6D DiscountedEV
else
...
elseif vfoptions.lowmemory==3
    ...
    bothzind=shiftdim((0:1:N_bothz-1),-3); % for d2aprimez stride into 6D DiscountedEV
    bothzBind=shiftdim((0:1:N_bothz-1),-1); % for allind
end
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
                [Vtempii,maxindex]=max(ReturnMatrix_ii,[],1);
                V(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                dind=(rem(maxindex-1,N_d)+1);
                a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                allind=dind+N_d*a2Bind+N_d*N_a2*bothzBind+N_d*N_a2*N_bothz*eBind;
                d12_ind=rem(dind-1,N_d12)+1;
                Policy4(1,curraindex,:,:,N_j)=rem(d12_ind-1,N_d1)+1;
                Policy4(2,curraindex,:,:,N_j)=ceil(d12_ind/N_d1);
                Policy4(3,curraindex,:,:,N_j)=ceil(dind/N_d12);
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,1,level1iidiff(ii),n_a2,n_bothz,n_e, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
                [Vtempii,maxindex]=max(ReturnMatrix_ii,[],1);
                V(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                dind=(rem(maxindex-1,N_d)+1);
                a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,n_a1,vfoptions.level1n,n_a2,n_bothz,special_n_e, d_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2,0);
                    [Vtempii,maxindex]=max(ReturnMatrix_ii_e,[],1);
                    V(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d)+1);
                    a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                    allind=dind+N_d*a2Bind+N_d*N_a2*bothzBind;
                    d12_ind=rem(dind-1,N_d12)+1;
                    Policy4(1,curraindex,:,e_c,N_j)=rem(d12_ind-1,N_d1)+1;
                    Policy4(2,curraindex,:,e_c,N_j)=ceil(d12_ind/N_d1);
                    Policy4(3,curraindex,:,e_c,N_j)=ceil(dind/N_d12);
                    ...
                    loweredge=maxindex1(:,1,ii,:,:);
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2,0);
                    [Vtempii,maxindex]=max(ReturnMatrix_ii_e,[],1);
                    V(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d)+1);
                    a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,n_a1,vfoptions.level1n,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,1,0);
                ...
                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                V(curraindex,semizblock,e_c,N_j)=shiftdim(Vtempii,1);
                dind=rem(maxindex2-1,N_d)+1;
                d12_ind=rem(dind-1,N_d12)+1;
                Policy4(1,curraindex,semizblock,e_c,N_j)=rem(d12_ind-1,N_d1)+1;
                Policy4(2,curraindex,semizblock,e_c,N_j)=ceil(d12_ind/N_d1);
                Policy4(3,curraindex,semizblock,e_c,N_j)=ceil(dind/N_d12);
                Policy4(4,curraindex,semizblock,e_c,N_j)=ceil(maxindex2/N_d);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,2,0);
                        ...
                        Policy4(2,curraindex,semizblock,e_c,N_j)=ceil(d12_ind/N_d1);
                        Policy4(3,curraindex,semizblock,e_c,N_j)=ceil(dind/N_d12);
                        Policy4(4,curraindex,semizblock,e_c,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
                        ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,2,0);
                        [Vtempii,maxindex]=max(ReturnMatrix_ii_ze,[],1);
                        V(curraindex,semizblock,e_c,N_j)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d)+1);
                        a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    elseif vfoptions.lowmemory==3 % joint bothz, inner e
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,n_a1,vfoptions.level1n,n_a2,special_n_bothz,special_n_e, d_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
                [~,maxindex1]=max(ReturnMatrix_ii_ze,[],2);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_bothz,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,2,0);
                        [Vtempii,maxindex]=max(ReturnMatrix_ii_ze,[],1);
                        V(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d)+1);
                        a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        ...
                        loweredge=maxindex1(:,1,ii,:);
                        ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d23,1,level1iidiff(ii),n_a2,special_n_bothz,special_n_e, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,2,0);
                        [Vtempii,maxindex]=max(ReturnMatrix_ii_ze,[],1);
                        V(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d)+1);
                        a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    EVpre=sum(reshape(vfoptions.V_Jplus1,[N_a,N_bothz,N_e]).*shiftdim(pi_e_J(:,N_j),-2),3); % [N_a, N_bothz]
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireEV=aprimeProbs_d3.*Vlower+(1-aprimeProbs_d3).*Vupper; % (d2*a1prime,a2,e_cur,bothzcur)
            entireEV=permute(entireEV,[1,2,4,3]); % (d2*a1prime,a2,bothzcur,e_cur)
            DiscountedEV=DiscountFactorParamsVec*reshape(entireEV,[N_d2,N_a1,1,N_a2,N_bothz,N_e]); % (d2,a1prime,1,a2,bothz,e)
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind+N_d2*N_a*N_bothz*eind; % [N_d12,maxgap+1,1,N_a2,N_bothz,N_e]
                    ...
                    a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                    allind=dind+N_d12*a2Bind+N_d12*N_a2*bothzBind+N_d12*N_a2*N_bothz*eBind;
                    ...
                    d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind+N_d2*N_a*N_bothz*eind;
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                [Vtempii,maxindex2]=max(reshape(entireRHS_ii_d3e,[N_d12*N_a1,vfoptions.level1n*N_a2,N_bothz]),[],1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind; % [N_d12,maxgap+1,1,N_a2,N_bothz]
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3e+DiscountedEV_e(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_jj(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d12)+1);
                        a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        allind=dind+N_d12*a2Bind+N_d12*N_a2*bothzBind;
                        ...
                        d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind;
    ...
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    DiscountedEV_ze=DiscountedEV(:,:,:,:,semizblock,e_c);
                    ...
                    [Vtempii,maxindex2]=max(reshape(entireRHS_ii_d3ze,[N_d12*N_a1,vfoptions.level1n*N_a2,N_semiz]),[],1);

                    curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                    V_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                    Policy_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(maxindex2,1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_ze(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d12)+1);
                            a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                            allind=dind+N_d12*a2Bind+N_d12*N_a2*semizBind;
                            Policy_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(maxindex+N_d12*(loweredge(allind)-1),1);
                        else
                            loweredge=maxindex1(:,1,ii,:,:);
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_ze(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d12)+1);
                            a2Bind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind+N_d2*N_a*N_bothz*eind;
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford3_jj(curraindex,:,:,d3_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d12)+1);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*bothzind;
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3e+DiscountedEV_e(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_jj(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d12)+1);
