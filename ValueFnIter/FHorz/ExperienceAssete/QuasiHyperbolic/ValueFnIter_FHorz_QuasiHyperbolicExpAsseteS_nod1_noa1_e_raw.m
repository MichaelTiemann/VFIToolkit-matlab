function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteS_nod1_noa1_e_raw(n_d2,n_a2,n_z,n_e,N_j, d2_gridvals, a2_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Sophisticated QH + ExpAssete (e dep aprimeFn), baseline (no DC, no GI), no d1, no a1, with z.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            entireRHS_hat_e=ReturnMatrix_e+beta0beta*EV(:,:,:,e_c);
            %Calc the max and it's index
            [Vtemp,maxindex]=max(entireRHS_hat_e,[],1);
            Vhat(:,:,e_c,N_j)=shiftdim(Vtemp,1);
            Policy(:,:,e_c,N_j)=shiftdim(maxindex,1);

            entireRHS_under_e=ReturnMatrix_e+beta*EV(:,:,:,e_c);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_hat_ze=ReturnMatrix_ze+beta0beta*EV(:,:,z_c,e_c);
                %Calc the max and it's index
                [Vtemp,maxindex]=max(entireRHS_hat_ze,[],1);
                Vhat(:,z_c,e_c,N_j)=Vtemp;
                Policy(:,z_c,e_c,N_j)=maxindex;

                entireRHS_under_ze=ReturnMatrix_ze+beta*EV(:,:,z_c,e_c);
