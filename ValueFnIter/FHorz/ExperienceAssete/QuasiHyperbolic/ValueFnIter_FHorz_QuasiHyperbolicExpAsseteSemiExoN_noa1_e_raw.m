function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteSemiExoN_noa1_e_raw(n_d1,n_d2,n_d3,n_a2,n_z,n_semiz,n_e,N_j, d12_gridvals, d2_gridvals, d3_grid, a2_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% noa1 version of ValueFnIter_FHorz_QuasiHyperbolicExpAsseteSemiExoN_e_raw (d1, z, e).
% Naive quasi-hyperbolic + ExperienceAssete + SemiExo (d1, no a1).
...
%
% Naive QH dual pass over the same argmax axis the exponential SemiExo ze noa1 (d1) raw maxes over:
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Valt(:,:,e_c,N_j)=Vtemp;
            d12_ind=rem(maxindex-1,N_d12)+1;
            Policyalt(1,:,:,e_c,N_j)=rem(d12_ind-1,N_d1)+1; % d1
            Policyalt(2,:,:,e_c,N_j)=ceil(d12_ind/N_d1);    % d2
            Policyalt(3,:,:,e_c,N_j)=ceil(maxindex/N_d12);  % d3
        end
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                d12_ind=rem(maxindex-1,N_d12)+1;
                Policyalt(1,:,semizblock,e_c,N_j)=rem(d12_ind-1,N_d1)+1; % d1
                Policyalt(2,:,semizblock,e_c,N_j)=ceil(d12_ind/N_d1);    % d2
                Policyalt(3,:,semizblock,e_c,N_j)=ceil(maxindex/N_d12);  % d3
            end
        end
    elseif vfoptions.lowmemory==3 % joint bothz, inner e
        for e_c=1:N_e
            ...
            for z_c=1:N_bothz
                ...
                d12_ind=rem(maxindex-1,N_d12)+1;
                Policyalt(1,:,z_c,e_c,N_j)=rem(d12_ind-1,N_d1)+1; % d1
                Policyalt(2,:,z_c,e_c,N_j)=ceil(d12_ind/N_d1);    % d2
                Policyalt(3,:,z_c,e_c,N_j)=ceil(maxindex/N_d12);  % d3
            end
        end
    end
        ...
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                for z_c=1:N_bothz
                    ...
                    % alt (exponential): F + beta*EV
                    entireRHS_d3ze=ReturnMatrix_d3ze+beta*entireEVpart(:,:,z_c,e_c);
                    [Vtemp,maxindex]=max(entireRHS_d3ze,[],1);
                    V_ford3_alt(:,z_c,e_c,d3_c)=Vtemp;
                    Policy_ford3_alt(:,z_c,e_c,d3_c)=maxindex;

                    % tilde (QH-perceived): F + beta0*beta*EV
                    entireRHS_d3ze=ReturnMatrix_d3ze+beta0beta*entireEVpart(:,:,z_c,e_c);
                    [Vtemp,maxindex]=max(entireRHS_d3ze,[],1);
