...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    entireEV=repelem(entireEV,N_d1,1);% [N_d1*N_d2,N_a1,1,N_a2,N_z,N_e]
    entireEVinterp=repelem(entireEVinterp,N_d1,1); % [N_d1*N_d2,N_a1prime,1,N_a2,N_z,N_e]
    ...
    if vfoptions.lowmemory==0
        ...
        da1primea2zalt=(1:1:N_d)'+N_d*(a1primeindexesfinealt-1)+N_d*N_a1prime*a2ind+N_d*N_a1prime*N_a2*zind+N_d*N_a1prime*N_a2*N_z*shiftdim((0:1:N_e-1),-4);
        ...
        da1primea2z=(1:1:N_d)'+N_d*(a1primeindexesfine-1)+N_d*N_a1prime*a2ind+N_d*N_a1prime*N_a2*zind+N_d*N_a1prime*N_a2*N_z*shiftdim((0:1:N_e-1),-4);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(entireEVinterp_e(da1primea2zalt(:)),[N_d*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2alt]=max(entireRHS_iialt,[],1);
            Valt(:,:,e_c,N_j)=shiftdim(Vtempii,1);
            d_indalt=rem(maxindexL2alt-1,N_d)+1;
            ...
            entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(entireEVinterp_e(da1primea2z(:)),[N_d*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                entireEV_ze=entireEV_z(:,:,:,:,1,e_c);
                entireEVinterp_ze=entireEVinterp_z(:,:,:,:,1,e_c);
                ...
                %% Valt (beta) -- capture Policyalt (exponential discounter's choice)
                entireRHS_zealt=ReturnMatrix_ze+beta*entireEV_ze; % autofill 3rd dim to N_a1
                ...
                entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(entireEVinterp_ze(da1primea2alt(:)),[N_d*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2alt]=max(entireRHS_iialt,[],1);
                Valt(:,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                d_indalt=rem(maxindexL2alt-1,N_d)+1;
                ...
                entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(entireEVinterp_ze(da1primea2(:)),[N_d*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
