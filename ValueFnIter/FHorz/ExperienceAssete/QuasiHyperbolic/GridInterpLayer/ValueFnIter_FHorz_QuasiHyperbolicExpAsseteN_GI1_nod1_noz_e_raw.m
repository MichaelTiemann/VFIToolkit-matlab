function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteN_GI1_nod1_noz_e_raw(n_d2,n_a1,n_a2,n_e, N_j, d2_gridvals, a1_gridvals, a2_grid, e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_ExpAssete_GI1_nod1_noz_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2,n_a1,n_a1,n_a2,n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0); % Level=1, Refine=0
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2, n2long, n_a1,n_a2,n_e, d2_gridvals, a1prime_grid(aprimeindexes), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2,n_a1,n_a1,n_a2,special_n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,1,0); % Level=1, Refine=0
            % Calc the max and it's index
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2, n2long, n_a1,n_a2,special_n_e, d2_gridvals, a1prime_grid(aprimeindexes), a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
            [Vtempii,maxindexL2]=max(ReturnMatrix_ii,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_iialt=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2, n2long, n_a1,n_a2,n_e, d2_gridvals, a1prime_grid(a1primeindexesfinealt), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        d2a1primea2alt=(1:1:N_d2)'+N_d2*(a1primeindexesfinealt-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*eind;
        entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(entireEVinterp(d2a1primea2alt(:)),[N_d2*n2long,N_a1*N_a2,N_e]);
        [Vtempii,maxindexL2alt]=max(entireRHS_iialt,[],1);
        Valt(:,:,N_j)=shiftdim(Vtempii,1);
        d_indalt=rem(maxindexL2alt-1,N_d2)+1;
        allindalt=d_indalt+N_d2*aind+N_d2*N_a*eindB; % midpoint is n_d-by-1-by-n_a1-by-n_a2-by-n_e
        ...
        % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2-by-n_e
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2, n2long, n_a1,n_a2,n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        d2a1primea2=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*eind;
        entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(entireEVinterp(d2a1primea2(:)),[N_d2*n2long,N_a1*N_a2,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        Vtilde(:,:,N_j)=shiftdim(Vtempii,1);
        d_ind=rem(maxindexL2-1,N_d2)+1;
        allind=d_ind+N_d2*aind+N_d2*N_a*eindB; % midpoint is n_d-by-1-by-n_a1-by-n_a2-by-n_e
        Policy(1,:,:,N_j)=d_ind; % d2
        Policy(2,:,:,N_j)=shiftdim(squeeze(midpoint(allind)),-1); % a1prime midpoint
        Policy(3,:,:,N_j)=shiftdim(ceil(maxindexL2/N_d2),-1); % a1primeL2ind
        % L2 flag: detect -Inf on the coarse a1 neighbour we'd put weight on (at chosen d)
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            a1primeindexesfinealt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint
            % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2
            ReturnMatrix_iialt=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2, n2long, n_a1,n_a2, special_n_e, d2_gridvals, a1prime_grid(a1primeindexesfinealt), a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
            d2a1primea2alt=(1:1:N_d2)'+N_d2*(a1primeindexesfinealt-1)+N_d2*N_a1prime*a2ind;
            entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(entireEVinterp_e(d2a1primea2alt(:)),[N_d2*n2long,N_a1*N_a2]);
            [Vtempii,maxindexL2alt]=max(entireRHS_iialt,[],1);
            Valt(:,e_c,N_j)=shiftdim(Vtempii,1);
            d_indalt=rem(maxindexL2alt-1,N_d2)+1;
            ...
            entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(entireEVinterp_e(d2a1primea2(:)),[N_d2*n2long,N_a1*N_a2]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2,n_a1,n_a1,n_a2,n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,1,0); % Level=1, Refine=0
        ...
        a1primeindexesfinealt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint
        % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2-by-n_e
        ReturnMatrix_iialt=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2, n2long, n_a1,n_a2,n_e, d2_gridvals, a1prime_grid(a1primeindexesfinealt), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        d2a1primea2alt=(1:1:N_d2)'+N_d2*(a1primeindexesfinealt-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*eind;
        entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(entireEVinterp(d2a1primea2alt(:)),[N_d2*n2long,N_a1*N_a2,N_e]);
        [Vtempii,maxindexL2alt]=max(entireRHS_iialt,[],1);
        ...
        a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint
        % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2-by-n_e
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0, n_d2, n2long, n_a1,n_a2,n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        d2a1primea2=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*eind;
        entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(entireEVinterp(d2a1primea2(:)),[N_d2*n2long,N_a1*N_a2,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
