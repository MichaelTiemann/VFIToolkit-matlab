function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteS_GI2A_e_raw(n_d1, n_d2, n_a1, n_a2, n_a3, n_z, n_e, N_j, d_gridvals, d2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_ExpAssete_GI2A_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        EVfine=reshape(entireEVinterp(aprimez),[N_d*n2long*N_a2,N_a,N_z,N_e]);
        entireRHS_ii=reshape(ReturnMatrix_ii,[N_d*n2long*N_a2,N_a,N_z,N_e])+beta0beta*EVfine;
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            EVfine_e=reshape(entireEVinterp_e(aprime),[N_d*n2long*N_a2,N_a,N_z,1]);
            entireRHS_ii_e=reshape(ReturnMatrix_ii_e,[N_d*n2long*N_a2,N_a,N_z,1])+beta0beta*EVfine_e;
            ...
            PolicyL2flag(1,:,:,e_c,N_j)=2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
            % --- Vunderbar: exponential value at the QH-chosen (interpolated) point ---
            linidx_e=reshape(maxindexL2,[1,N_a*N_z])+size(EVfine_e,1)*(0:N_a*N_z-1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                EVfine_ze=reshape(entireEVinterp_ze(aprime),[N_d*n2long*N_a2,N_a]);
                entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze,[N_d*n2long*N_a2,N_a])+beta0beta*EVfine_ze;
