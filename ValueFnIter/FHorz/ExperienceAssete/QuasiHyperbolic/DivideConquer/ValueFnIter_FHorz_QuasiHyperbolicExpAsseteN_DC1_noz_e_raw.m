function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteN_DC1_noz_e_raw(n_d1, n_d2,n_a1,n_a2,n_e,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid,e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d,maxgap_V+1,1,N_a2,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_e]
                ...
                d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d,1,1,N_a2,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_e]
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_e]
                ...
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,1,level1iidiff(ii),n_a2,n_e, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d,1,1,N_a2,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_e]
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap_V+1,1,N_a2]; linear index into DiscountedEV_alt_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_dc_e+DiscountedEV_alt_e(d2aprime),[N_d*(maxgap_V(ii)+1),level1iidiff(ii)*N_a2]);
                    ...
                    d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,1,1,N_a2]; linear index into DiscountedEV_alt_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_dc_e+DiscountedEV_alt_e(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindexalt]=max(entireRHS_ii_e,[],1);
                    Valt(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindexalt does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allindalt), need to 'add' the loweredge
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap+1,1,N_a2]; linear index into DiscountedEV_tilde_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_dc_e+DiscountedEV_tilde_e(d2aprime),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    ...
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_dc_e=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,1,level1iidiff(ii),n_a2,special_n_e, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                    d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,1,1,N_a2]; linear index into DiscountedEV_tilde_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_dc_e+DiscountedEV_tilde_e(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    Vtilde(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap_V+1,1,N_a2]; linear index into DiscountedEV_alt_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt_e(d2aprime),[N_d*(maxgap_V(ii)+1),level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                    Valt(curraindex,e_c,jj)=shiftdim(Vtempii,1);
                    % maxindexalt does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allindalt), need to 'add' the loweredge
                    ...
                    d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,1,1,N_a2]; linear index into DiscountedEV_alt_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt_e(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                    Valt(curraindex,e_c,jj)=shiftdim(Vtempii,1);
                    % maxindexalt does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allindalt), need to 'add' the loweredge
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,maxgap+1,1,N_a2]; linear index into DiscountedEV_tilde_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde_e(d2aprime),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vtilde(curraindex,e_c,jj)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,1,level1iidiff(ii),n_a2,special_n_e, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                    d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d,1,1,N_a2]; linear index into DiscountedEV_tilde_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde_e(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vtilde(curraindex,e_c,jj)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
