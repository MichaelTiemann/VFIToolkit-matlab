function [Vhat,Policy4,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteSemiExoS_DC2A_nod1_noz_e_raw(n_d2, n_d3, n_a1, n_a2, n_a3, n_semiz, n_e, N_j, d2_gridvals, d3_grid, a1_grid, a2_gridvals, a3_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            DiscountedEV_under=beta*reshape(entireEV,[N_d2,N_a1,N_a2,1,1,N_a3,N_semiz,N_e]);    % exponential
            DiscountedEV_hat=beta0beta*reshape(entireEV,[N_d2,N_a1,N_a2,1,1,N_a3,N_semiz,N_e]); % QH-perceived
            ...
            entireRHS_hat_flat=reshape(entireRHS_hat,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_semiz,N_e]);
            [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
            entireRHS_under=ReturnMatrix_ii_d3+DiscountedEV_under;
            entireRHS_under_flat=reshape(entireRHS_under,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_semiz,N_e]);
            M=vfoptions.level1n*N_a2*N_a3;
            maxindexfull=maxindex2 + (N_d2*N_a1*N_a2)*(0:M-1) + (N_d2*N_a1*N_a2)*M*shiftdim((0:N_semiz-1),-1) + (N_d2*N_a1*N_a2)*M*N_semiz*shiftdim((0:N_e-1),-2);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            entireEV=aprimeProbs_d3.*Vlower+(1-aprimeProbs_d3).*Vupper; % (d2*a1prime*a2prime,a3,e_cur,semizcur)
            entireEV=permute(entireEV,[1,2,4,3]); % (d2*a1prime*a2prime,a3,semizcur,e_cur)
            DiscountedEV_under=beta*reshape(entireEV,[N_d2,N_a1,N_a2,1,1,N_a3,N_semiz,N_e]);
            DiscountedEV_hat=beta0beta*reshape(entireEV,[N_d2,N_a1,N_a2,1,1,N_a3,N_semiz,N_e]);
    ...
    d2aprime_ind=reshape(Policy_ford3_hat((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(d3maxindex_lin-1)),[1,N_a,N_semiz,N_e]);
