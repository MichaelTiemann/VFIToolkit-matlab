...
if vfoptions.lowmemory==0
    % precompute
    eind=shiftdim((0:1:N_e-1),-4); % already includes -1; for d2aprimez e-stride into 6D DiscountedEV
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind+N_d2*N_a*N_z*eind; % [N_d,1,1,N_a2,N_z,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_z,N_e]
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,1,level1iidiff(ii),n_a2,n_z,n_e, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind+N_d2*N_a*N_z*eind; % [N_d,1,1,N_a2,N_z,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_z,N_e]
