...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_hat_flat=reshape(entireRHS_hat,[N_d12*N_a1,vfoptions.level1n*N_a2,N_bothz,N_e]);
            ...
            entireRHS_under_flat=reshape(entireRHS_under,[N_d12*N_a1,vfoptions.level1n*N_a2,N_bothz,N_e]);
            M=vfoptions.level1n*N_a2;
            maxindexfull=maxindex2 + (N_d12*N_a1)*(0:M-1) + (N_d12*N_a1)*M*shiftdim((0:N_bothz-1),-1) + (N_d12*N_a1)*M*N_bothz*shiftdim((0:N_e-1),-2);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                [~,maxindex1]=max(entireRHS_hat,[],2);
                entireRHS_hat_flat=reshape(entireRHS_hat,[N_d12*N_a1,vfoptions.level1n*N_a2,N_bothz]);
                [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
                entireRHS_under=ReturnMatrix_ii_d3e+repelem(DiscountedEV_under_e,N_d1,1,1,1,1);
                entireRHS_under_flat=reshape(entireRHS_under,[N_d12*N_a1,vfoptions.level1n*N_a2,N_bothz]);
                M=vfoptions.level1n*N_a2;
                maxindexfull=maxindex2 + (N_d12*N_a1)*(0:M-1) + (N_d12*N_a1)*M*shiftdim((0:N_bothz-1),-1);
                Vtempii_under=entireRHS_under_flat(maxindexfull);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_hat=ReturnMatrix_ii_d3ze+repelem(DiscountedEV_hat_zbe,N_d1,1,1,1,1);
                    [~,maxindex1]=max(entireRHS_hat,[],2);
                    entireRHS_hat_flat=reshape(entireRHS_hat,[N_d12*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                    [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
                    entireRHS_under=ReturnMatrix_ii_d3ze+repelem(DiscountedEV_under_zbe,N_d1,1,1,1,1);
    ...
    d12a1prime_ind=reshape(Policy_ford3_hat((1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(d3maxindex_lin-1)),[1,N_a,N_bothz,N_e]);
