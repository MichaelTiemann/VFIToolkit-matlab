function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteS_DC1_nod1_noz_e_raw(n_d2,n_a1,n_a2,n_e, N_j, d2_gridvals, a1_gridvals, a2_grid, e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV_hat=beta0beta*reshape(EV,[N_d2,N_a1,1,N_a2,N_e]); % (d,a1prime,1,a2,e_cur)   % QH-perceived
    ...
    if vfoptions.lowmemory==0
        ...
        % Now, get and store the full (d,aprime)
        entireRHS_ii_flat=reshape(entireRHS_ii,[N_d2*N_a1,vfoptions.level1n*N_a2,N_e]);
        [Vtempii,maxindex2]=max(entireRHS_ii_flat,[],1);

        % Store
        curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
        Vhat(curraindex,:,N_j)=shiftdim(Vtempii,1);
        entireRHS_ii_under_flat=reshape(entireRHS_ii_under,[N_d2*N_a1,vfoptions.level1n*N_a2,N_e]);
        maxindexfull=maxindex2+N_d2*N_a1*(0:vfoptions.level1n*N_a2-1)+N_d2*N_a1*vfoptions.level1n*N_a2*shiftdim((0:N_e-1),-1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d2,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_e]);
                entireRHS_ii_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vhat(curraindex,:,N_j)=shiftdim(Vtempii,1);
                maxindexfull=maxindex+N_d2*(maxgap(ii)+1)*(0:level1iidiff(ii)*N_a2-1)+N_d2*(maxgap(ii)+1)*level1iidiff(ii)*N_a2*shiftdim((0:N_e-1),-1);
                ...
                d2aprime=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d2,1,1,N_a2,N_e]; linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprime),[N_d2,level1iidiff(ii)*N_a2,N_e]);
                entireRHS_ii_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprime),[N_d2,level1iidiff(ii)*N_a2,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vhat(curraindex,:,N_j)=shiftdim(Vtempii,1);
                maxindexfull=maxindex+N_d2*(0:level1iidiff(ii)*N_a2-1)+N_d2*level1iidiff(ii)*N_a2*shiftdim((0:N_e-1),-1);
                Vtempii_under=entireRHS_ii_under(maxindexfull);
                Vunderbar(curraindex,:,N_j)=shiftdim(Vtempii_under,1);
                % maxindex does not need reworking, as with expasset there is no a2prime
                %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii_e=ReturnMatrix_ii_e+DiscountedEV_hat(:,:,:,:,e_c);
            entireRHS_ii_e_under=ReturnMatrix_ii_e+DiscountedEV_under(:,:,:,:,e_c);

            % First, we want a1prime conditional on (d,1,a)
            [~,maxindex1]=max(entireRHS_ii_e,[],2);

            % Now, get and store the full (d,aprime)
            entireRHS_ii_e_flat=reshape(entireRHS_ii_e,[N_d2*N_a1,vfoptions.level1n*N_a2]);
            [Vtempii,maxindex2]=max(entireRHS_ii_e_flat,[],1);

            % Store
            curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
            Vhat(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
            entireRHS_ii_e_under_flat=reshape(entireRHS_ii_e_under,[N_d2*N_a1,vfoptions.level1n*N_a2]);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,maxgap+1,1,N_a2]; linear index into DiscountedEV_hat(:,:,:,:,e_c) [N_d2,N_a1,1,N_a2]
                    DiscountedEV_hat_e=DiscountedEV_hat(:,:,:,:,e_c);
                    DiscountedEV_under_e=DiscountedEV_under(:,:,:,:,e_c);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat_e(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    entireRHS_ii_e_under=reshape(ReturnMatrix_ii_e+DiscountedEV_under_e(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    ...
                    d2aprime=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,1,1,N_a2]; linear index into DiscountedEV_hat(:,:,:,:,e_c) [N_d2,N_a1,1,N_a2]
                    DiscountedEV_hat_e=DiscountedEV_hat(:,:,:,:,e_c);
                    DiscountedEV_under_e=DiscountedEV_under(:,:,:,:,e_c);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat_e(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                    entireRHS_ii_e_under=reshape(ReturnMatrix_ii_e+DiscountedEV_under_e(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    Vhat(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
