function [Vhat,Policy3,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteSemiExoS_DC1_nod1_noz_e_raw(n_d2,n_d3,n_a1,n_a2,n_semiz,n_e,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_hat_flat=reshape(entireRHS_hat,[N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz,N_e]);
            [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
            entireRHS_under=ReturnMatrix_ii_d3+DiscountedEV_under;
            entireRHS_under_flat=reshape(entireRHS_under,[N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz,N_e]);
            M=vfoptions.level1n*N_a2;
            maxindexfull=maxindex2 + (N_d2*N_a1)*(0:M-1) + (N_d2*N_a1)*M*shiftdim((0:N_semiz-1),-1) + (N_d2*N_a1)*M*N_semiz*shiftdim((0:N_e-1),-2);
