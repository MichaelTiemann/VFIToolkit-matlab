...
% lowmemory=0 full vectorization; lowmemory=1 loop over e (z vectorized); lowmemory=2 nested z+e.
...
Vtilde=zeros(N_a,N_z,N_e,N_j,'gpuArray'); % QH-perceived value fn (beta0*beta)
Policy=zeros(3,N_a,N_z,N_e,N_j,'gpuArray');
Policyalt=zeros(3,N_a,N_z,N_e,N_j,'gpuArray'); % exponential-discounter policy (Valt is computed at this)
...
elseif vfoptions.lowmemory==1
    ...
    zind=shiftdim((0:1:N_z-1),-1); % z at dim 3 of [1,L,N_z,1]
else
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    EVpre=squeeze(sum(reshape(vfoptions.V_Jplus1,[N_a,N_z,N_e]).*shiftdim(pi_e_J(:,N_j+1),-2),3)); % integrate out eprime: [N_a,N_z(zprime)]
    ...
    aprimeProbs=repmat(a3primeProbs,N_a1*N_a2,1,1,N_z); % [N_d2*N_a1*N_a2,N_a3,N_e,N_z] (replicate over zprime)

    Vlower=reshape(EVpre(aprimeIndex(:),:),    [N_d2*N_a1*N_a2,N_a3,N_e,N_z]); % (d2*a1prime*a2prime,a3,e_cur,zprime)
    ...
    if vfoptions.lowmemory==0
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 1);
        %% Valt (beta)
        entireRHS_ii_V=ReturnMatrix_ii+repelem(DiscountedEV_alt,N_d1,1,1,1,1,1,1);
        [~,maxindex1_V]=max(entireRHS_ii_V,[],2);
        [Vtempii_V,maxindex2_V]=max(reshape(entireRHS_ii_V,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,N_e]),[],1);
        curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                 + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                 + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
        dind_V   =rem(maxindex2_V-1,N_d)+1;
        a1pind_V =rem(floor((maxindex2_V-1)/N_d),N_a1)+1;
        a2pind_V =floor((maxindex2_V-1)/(N_d*N_a1))+1;
        Valt(curraindex,:,:,N_j)       =shiftdim(Vtempii_V,1);
        Policyalt(1,curraindex,:,:,N_j)=dind_V;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                entireRHS_ii_V=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprimeze),[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                [Vtempii_V,maxindex_V]=max(entireRHS_ii_V,[],1);
                Valt(curraindex,:,:,N_j)=shiftdim(Vtempii_V,1);
                dind_V      =rem(maxindex_V-1,N_d)+1;
                a1localind_V=rem(floor((maxindex_V-1)/N_d),maxgap_V(ii)+1)+1;
                a2pind_V    =floor((maxindex_V-1)/(N_d*(maxgap_V(ii)+1)))+1;
                a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                loweredge_idx=dind_V + N_d*(a2pind_V-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat + N_d*N_a2*N_a2*N_a3*zind + N_d*N_a2*N_a2*N_a3*N_z*eind;
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d2aprimeze=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5) + N_d2*N_a1*N_a2*N_a3*N_z*shiftdim((0:1:N_e-1),-6);
                entireRHS_ii_V=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprimeze),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                [Vtempii_V,maxindex_V]=max(entireRHS_ii_V,[],1);
                Valt(curraindex,:,:,N_j)=shiftdim(Vtempii_V,1);
                dind_V   =rem(maxindex_V-1,N_d)+1;
                a2pind_V =floor((maxindex_V-1)/N_d)+1;
                a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                loweredge_idx=dind_V + N_d*(a2pind_V-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat + N_d*N_a2*N_a2*N_a3*zind + N_d*N_a2*N_a2*N_a3*N_z*eind;
                Policyalt(1,curraindex,:,:,N_j)=dind_V;
        ...
        [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,N_e]),[],1);
        curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                 + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                 + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
        dind   =rem(maxindex2-1,N_d)+1;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d2aprimeze=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5) + N_d2*N_a1*N_a2*N_a3*N_z*shiftdim((0:1:N_e-1),-6);
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprimeze),[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d2aprimeze=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5) + N_d2*N_a1*N_a2*N_a3*N_z*shiftdim((0:1:N_e-1),-6);
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprimeze),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                dind   =rem(maxindex-1,N_d)+1;
                a2pind =floor((maxindex-1)/N_d)+1;
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
            %% Valt (beta)
            entireRHS_ii_e_V=ReturnMatrix_ii_e+repelem(DiscountedEV_alt_e,N_d1,1,1,1,1,1);
            [~,maxindex1_V]=max(entireRHS_ii_e_V,[],2);
            [Vtempii_V,maxindex2_V]=max(reshape(entireRHS_ii_e_V,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]),[],1);
            curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                     + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                     + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
            dind_V   =rem(maxindex2_V-1,N_d)+1;
            ...
            Policyalt(1,curraindex,:,e_c,N_j)=dind_V;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1_V(:,1,:,ii,:,:,:,:),N_a1-maxgap_V(ii));
                    a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                    ...
                    entireRHS_ii_e_V=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_alt_e(d2aprimez),[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    ...
                    a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                    a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                    loweredge_idx=dind_V + N_d*(a2pind_V-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat + N_d*N_a2*N_a2*N_a3*zind;
                    a1prime_rec=a1localind_V+loweredge(loweredge_idx)-1;
                    Policyalt(1,curraindex,:,e_c,N_j)=dind_V;
                    ...
                    loweredge=maxindex1_V(:,1,:,ii,:,:,:,:);
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                    ...
                    entireRHS_ii_e_V=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_alt_e(d2aprimez),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii_V,maxindex_V]=max(entireRHS_ii_e_V,[],1);
                    Valt(curraindex,:,e_c,N_j)=shiftdim(Vtempii_V,1);
                    dind_V   =rem(maxindex_V-1,N_d)+1;
                    a2pind_V =floor((maxindex_V-1)/N_d)+1;
            ...
            %% Vtilde (beta0*beta)
            entireRHS_ii_e=ReturnMatrix_ii_e+repelem(DiscountedEV_tilde_e,N_d1,1,1,1,1,1);
            [~,maxindex1]=max(entireRHS_ii_e,[],2);
            [Vtempii,maxindex2]=max(reshape(entireRHS_ii_e,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]),[],1);
            curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                     + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                     + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
            dind   =rem(maxindex2-1,N_d)+1;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                    d2aprimez=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_tilde_e(d2aprimez),[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    ...
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                    d2aprimez=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_tilde_e(d2aprimez),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    Vtilde(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    dind   =rem(maxindex-1,N_d)+1;
                    a2pind =floor((maxindex-1)/N_d)+1;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 1);
                %% Valt (beta)
                entireRHS_ii_ze_V=ReturnMatrix_ii_ze+repelem(DiscountedEV_alt_ze,N_d1,1,1,1,1,1);
                [~,maxindex1_V]=max(entireRHS_ii_ze_V,[],2);
                [Vtempii_V,maxindex2_V]=max(reshape(entireRHS_ii_ze_V,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
                curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                         + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                         + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
                dind_V   =rem(maxindex2_V-1,N_d)+1;
                a1pind_V =rem(floor((maxindex2_V-1)/N_d),N_a1)+1;
                a2pind_V =floor((maxindex2_V-1)/(N_d*N_a1))+1;
                Valt(curraindex,z_c,e_c,N_j)       =shiftdim(Vtempii_V,1);
                Policyalt(1,curraindex,z_c,e_c,N_j)=dind_V;
                Policyalt(2,curraindex,z_c,e_c,N_j)=a1pind_V;
                Policyalt(3,curraindex,z_c,e_c,N_j)=a2pind_V;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        entireRHS_ii_ze_V=reshape(ReturnMatrix_ii_ze_dc+DiscountedEV_alt_ze(d2aprime),[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        [Vtempii_V,maxindex_V]=max(entireRHS_ii_ze_V,[],1);
                        Valt(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii_V,1);
                        dind_V      =rem(maxindex_V-1,N_d)+1;
                        a1localind_V=rem(floor((maxindex_V-1)/N_d),maxgap_V(ii)+1)+1;
                        a2pind_V    =floor((maxindex_V-1)/(N_d*(maxgap_V(ii)+1)))+1;
                        a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                        a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                        loweredge_idx=dind_V + N_d*(a2pind_V-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat;
                        a1prime_rec=a1localind_V+loweredge(loweredge_idx)-1;
                        Policyalt(1,curraindex,z_c,e_c,N_j)=dind_V;
                        ...
                        loweredge=maxindex1_V(:,1,:,ii,:,:,:,:);
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                        d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_ii_ze_V=reshape(ReturnMatrix_ii_ze_dc+DiscountedEV_alt_ze(d2aprime),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        [Vtempii_V,maxindex_V]=max(entireRHS_ii_ze_V,[],1);
                        Valt(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii_V,1);
                        dind_V   =rem(maxindex_V-1,N_d)+1;
                        a2pind_V =floor((maxindex_V-1)/N_d)+1;
                ...
                %% Vtilde (beta0*beta)
                entireRHS_ii_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_tilde_ze,N_d1,1,1,1,1,1);
                [~,maxindex1]=max(entireRHS_ii_ze,[],2);
                [Vtempii,maxindex2]=max(reshape(entireRHS_ii_ze,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
                curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                         + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                         + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
                dind   =rem(maxindex2-1,N_d)+1;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                        d2aprime=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze_dc+DiscountedEV_tilde_ze(d2aprime),[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        ...
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                        d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze_dc+DiscountedEV_tilde_ze(d2aprime),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        [Vtempii,maxindex]=max(entireRHS_ii_ze,[],1);
                        Vtilde(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        dind   =rem(maxindex-1,N_d)+1;
                        a2pind =floor((maxindex-1)/N_d)+1;
...
for reverse_j=1:N_j-1
    ...
    EVpre=squeeze(sum(Valt(:,:,:,jj+1).*shiftdim(pi_e_J(:,jj+1),-2),3)); % integrate out eprime: [N_a,N_z(zprime)]
    ...
    if vfoptions.lowmemory==0
        ...
        Policyalt(1,curraindex,:,:,jj)=dind_V;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                loweredge=maxindex1_V(:,1,:,ii,:,:,:,:);
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                d2aprimeze=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5) + N_d2*N_a1*N_a2*N_a3*N_z*shiftdim((0:1:N_e-1),-6);
                entireRHS_ii_V=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprimeze),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                [Vtempii_V,maxindex_V]=max(entireRHS_ii_V,[],1);
                Valt(curraindex,:,:,jj)=shiftdim(Vtempii_V,1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Policyalt(1,curraindex,:,e_c,jj)=dind_V;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1_V(:,1,:,ii,:,:,:,:),N_a1-maxgap_V(ii));
                    a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                    d2aprimez=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_ii_e_V=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_alt_e(d2aprimez),[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii_V,maxindex_V]=max(entireRHS_ii_e_V,[],1);
                    Valt(curraindex,:,e_c,jj)=shiftdim(Vtempii_V,1);
                    ...
                    loweredge=maxindex1_V(:,1,:,ii,:,:,:,:);
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                    d2aprimez=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_ii_e_V=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_alt_e(d2aprimez),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii_V,maxindex_V]=max(entireRHS_ii_e_V,[],1);
                    Valt(curraindex,:,e_c,jj)=shiftdim(Vtempii_V,1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                a1pind_V =rem(floor((maxindex2_V-1)/N_d),N_a1)+1;
                a2pind_V =floor((maxindex2_V-1)/(N_d*N_a1))+1;
                Valt(curraindex,z_c,e_c,jj)       =shiftdim(Vtempii_V,1);
                Policyalt(1,curraindex,z_c,e_c,jj)=dind_V;
                Policyalt(2,curraindex,z_c,e_c,jj)=a1pind_V;
                Policyalt(3,curraindex,z_c,e_c,jj)=a2pind_V;
