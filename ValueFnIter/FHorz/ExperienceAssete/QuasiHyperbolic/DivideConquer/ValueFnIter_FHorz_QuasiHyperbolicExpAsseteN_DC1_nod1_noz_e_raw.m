function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteN_DC1_nod1_noz_e_raw(n_d2,n_a1,n_a2,n_e, N_j, d2_gridvals, a1_gridvals, a2_grid, e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if vfoptions.lowmemory==0
    % precompute
    eind=shiftdim((0:1:N_e-1),-1); % already includes -1; for allind in Policy assignment
else
...
if ~isfield(vfoptions,'V_Jplus1')
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,2,0); % Level=2, Refine=0
                    [Vtempii,maxindex]=max(ReturnMatrix_ii,[],1);
                    Valt(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                    ...
                    loweredge=maxindex1(:,1,ii,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,1,level1iidiff(ii),n_a2,special_n_e, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,2,0); % Level=2, Refine=0
                    [Vtempii,maxindex]=max(ReturnMatrix_ii,[],1);
                    Valt(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
    ...
    DiscountedEV_alt=beta*reshape(EV,[N_d2,N_a1,1,N_a2,N_e]); % (d,a1prime,1,a2,e_cur)   % exponential
    ...
    if vfoptions.lowmemory==0
        ...
        % Now, get and store the full (d,aprime)
        [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii,[N_d2*N_a1,vfoptions.level1n*N_a2,N_e]),[],1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d2,maxgap_V+1,1,N_a2,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d2*(maxgap_V(ii)+1),level1iidiff(ii)*N_a2,N_e]);
                ...
                a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii)); % already includes -1
                allindalt=dindalt+N_d2*a2ind+N_d2*N_a2*eind; % loweredge is n_d-by-1-by-1-by-n_a2-by-n_e
                Policyalt(curraindex,:,N_j)=shiftdim(maxindexalt+N_d2*(loweredge(allindalt)-1),1);
            else
                loweredge=maxindex1(:,1,ii,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d2,1,1,N_a2,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d2,level1iidiff(ii)*N_a2,N_e]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,N_j)=shiftdim(Vtempii,1);
                % maxindexalt does not need reworking, as with expasset there is no a2prime
                %  the a1prime is relative to loweredge(allindalt), need to 'add' the loweredge
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d2,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_e]);
                ...
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d2,1,1,N_a2,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime),[N_d2,level1iidiff(ii)*N_a2,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde(curraindex,:,N_j)=shiftdim(Vtempii,1);
                % maxindex does not need reworking, as with expasset there is no a2prime
                %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii_e=ReturnMatrix_ii_e+DiscountedEV_alt(:,:,:,:,e_c);

            % First, we want a1prime conditional on (d,1,a)
            [~,maxindex1]=max(entireRHS_ii_e,[],2);

            % Now, get and store the full (d,aprime)
            [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii_e,[N_d2*N_a1,vfoptions.level1n*N_a2]),[],1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_dc_e=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,special_n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                    d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,maxgap_V+1,1,N_a2]; linear index into DiscountedEV_alt(:,:,:,:,e_c) [N_d2,N_a1,1,N_a2]
                    DiscountedEV_alt_e=DiscountedEV_alt(:,:,:,:,e_c);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_dc_e+DiscountedEV_alt_e(d2aprime),[N_d2*(maxgap_V(ii)+1),level1iidiff(ii)*N_a2]);
                    ...
                    loweredge=maxindex1(:,1,ii,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_dc_e=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,1,level1iidiff(ii),n_a2,special_n_e, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                    d2aprime=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,1,1,N_a2]; linear index into DiscountedEV_alt(:,:,:,:,e_c) [N_d2,N_a1,1,N_a2]
                    DiscountedEV_alt_e=DiscountedEV_alt(:,:,:,:,e_c);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_dc_e+DiscountedEV_alt_e(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindexalt]=max(entireRHS_ii_e,[],1);
                    Valt(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindexalt does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allindalt), need to 'add' the loweredge
            ...
            entireRHS_ii_e=ReturnMatrix_ii_e+DiscountedEV_tilde(:,:,:,:,e_c);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2
                    ReturnMatrix_ii_dc_e=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                    d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,maxgap+1,1,N_a2]; linear index into DiscountedEV_tilde(:,:,:,:,e_c) [N_d2,N_a1,1,N_a2]
                    DiscountedEV_tilde_e=DiscountedEV_tilde(:,:,:,:,e_c);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_dc_e+DiscountedEV_tilde_e(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    ...
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_dc_e=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,1,level1iidiff(ii),n_a2,special_n_e, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0); % Level=2, Refine=0
                    d2aprime=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2); % [N_d2,1,1,N_a2]; linear index into DiscountedEV_tilde(:,:,:,:,e_c) [N_d2,N_a1,1,N_a2]
                    DiscountedEV_tilde_e=DiscountedEV_tilde(:,:,:,:,e_c);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_dc_e+DiscountedEV_tilde_e(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    Vtilde(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                % aprime possibilities are n_d-by-maxgap_V(ii)+1-by-1-by-n_a2-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d2,maxgap_V+1,1,N_a2,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d2*(maxgap_V(ii)+1),level1iidiff(ii)*N_a2,N_e]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,jj)=shiftdim(Vtempii,1);
                ...
                loweredge=maxindex1(:,1,ii,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d2,1,1,N_a2,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d2,level1iidiff(ii)*N_a2,N_e]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,jj)=shiftdim(Vtempii,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level=2, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_e-1),-3); % [N_d2,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde(curraindex,:,jj)=shiftdim(Vtempii,1);
