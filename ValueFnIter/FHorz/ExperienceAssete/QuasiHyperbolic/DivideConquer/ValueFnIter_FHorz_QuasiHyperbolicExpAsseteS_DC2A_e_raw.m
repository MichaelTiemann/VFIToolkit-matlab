function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteS_DC2A_e_raw(n_d1, n_d2, n_a1, n_a2, n_a3, n_z, n_e, N_j, d_gridvals, d2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Sophisticated quasi-hyperbolic discounting + ExperienceAssete, with-d1 DC2A pattern.
...
Vhat=zeros(N_a,N_z,N_e,N_j,'gpuArray'); % QH-perceived value fn (beta0*beta)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_hat_flat=reshape(entireRHS_hat,[N_d*N_a1*N_a2,M,N_z,N_e]);
        ...
        entireRHS_under_flat=reshape(entireRHS_under,[N_d*N_a1*N_a2,M,N_z,N_e]);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_hat=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprimeze),[firstdim,Mblock,N_z,N_e]);
                entireRHS_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprimeze),[firstdim,Mblock,N_z,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
            entireRHS_hat_e=ReturnMatrix_ii_e+repelem(DiscountedEV_hat_e,N_d1,1,1,1,1,1);
            [~,maxindex1]=max(entireRHS_hat_e,[],2);
            M=vfoptions.level1n*N_a2*N_a3;
            entireRHS_hat_flat=reshape(entireRHS_hat_e,[N_d*N_a1*N_a2,M,N_z,1]);
            [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
            entireRHS_under_e=ReturnMatrix_ii_e+repelem(DiscountedEV_under_e,N_d1,1,1,1,1,1);
            entireRHS_under_flat=reshape(entireRHS_under_e,[N_d*N_a1*N_a2,M,N_z,1]);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    firstdim=N_d*(maxgap(ii)+1)*N_a2;
                    Mblock=level1iidiff(ii)*N_a2*N_a3;
                    entireRHS_hat_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat_e(d2aprimez),[firstdim,Mblock,N_z,1]);
                    entireRHS_under_e=reshape(ReturnMatrix_ii_e+DiscountedEV_under_e(d2aprimez),[firstdim,Mblock,N_z,1]);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                M=vfoptions.level1n*N_a2*N_a3;
                entireRHS_hat_flat=reshape(entireRHS_hat_ze,[N_d*N_a1*N_a2,M]);
                [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
                entireRHS_under_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_under_ze,N_d1,1,1,1,1,1);
                entireRHS_under_flat=reshape(entireRHS_under_ze,[N_d*N_a1*N_a2,M]);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        firstdim=N_d*(maxgap(ii)+1)*N_a2;
                        Mblock=level1iidiff(ii)*N_a2*N_a3;
                        entireRHS_hat_ze=reshape(ReturnMatrix_ii_ze+DiscountedEV_hat_ze(d2aprime),[firstdim,Mblock]);
                        entireRHS_under_ze=reshape(ReturnMatrix_ii_ze+DiscountedEV_under_ze(d2aprime),[firstdim,Mblock]);
...
for reverse_j=1:N_j-1
    ...
    EVpre=squeeze(sum(Vunderbar(:,:,:,jj+1).*shiftdim(pi_e_J(:,jj+1),-2),3)); % integrate out eprime: [N_a,N_z(zprime)]
