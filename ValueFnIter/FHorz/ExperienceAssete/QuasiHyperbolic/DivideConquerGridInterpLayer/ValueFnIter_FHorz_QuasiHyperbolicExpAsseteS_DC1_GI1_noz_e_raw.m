function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteS_DC1_GI1_noz_e_raw(n_d1, n_d2,n_a1,n_a2,n_e,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid,e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting + ExperienceAssete, Divide-and-Conquer (DC1 over a1prime) with grid interpolation layer (GI1) on a1.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    entireEV=reshape(EV,[N_d2,N_a1,1,N_a2,N_e]); % (d2,a1prime,1,a2,e_cur); d1-dim is implicit singleton, broadcasts at use sites; undiscounted, beta0beta applied at use sites
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ReturnMatrix_ii+repelem(beta0beta*entireEV,N_d1,1,1,1,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_e
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*eind; % [N_d,maxgap+1,1,N_a2,N_e]; linear index into entireEV [N_d2,N_a1,1,N_a2,N_e]
        ...
        da1primea2=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*eind; % [N_d,n2long,N_a1,N_a2,N_e]; linear index into entireEVinterp [N_d2,N_a1prime,1,N_a2,N_e]
        EVfine=reshape(entireEVinterp(da1primea2),[N_d*n2long,N_a1*N_a2,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii_e=ReturnMatrix_ii_e+repelem(beta0beta*entireEV_e,N_d1,1,1,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d,maxgap+1,1,N_a2]; linear index into entireEV_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii=ReturnMatrix_ii+beta0beta*entireEV_e(d2aprime);
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoint(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,n2long,n_a1,n_a2,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
            da1primea2=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d,n2long,N_a1,N_a2]; linear index into entireEVinterp_e [N_d2,N_a1prime,1,N_a2]
            EVfine=reshape(entireEVinterp_e(da1primea2),[N_d*n2long,N_a1*N_a2]);
            entireRHS_ii=ReturnMatrix_ii+beta0beta*EVfine;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            Vhat(:,e_c,N_j)=shiftdim(Vtempii,1);
            d_ind=rem(maxindexL2-1,N_d)+1;
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_e
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*eind; % [N_d,maxgap+1,1,N_a2,N_e]; linear index into entireEV [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=ReturnMatrix_ii+beta0beta*entireEV(d2aprime);
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
