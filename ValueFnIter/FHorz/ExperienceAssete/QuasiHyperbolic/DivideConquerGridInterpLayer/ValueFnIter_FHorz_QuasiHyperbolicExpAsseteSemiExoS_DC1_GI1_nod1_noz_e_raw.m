...
Policy3_ford3=zeros(3,N_a,N_semiz,N_e,N_d3,'gpuArray'); % (d2, midpoint, L2ind)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    Policy(1,:,:,:,N_j)=reshape(Policy3_ford3(1+temp),[1,N_a,N_semiz,N_e]);
    Policy(3,:,:,:,N_j)=reshape(Policy3_ford3(2+temp),[1,N_a,N_semiz,N_e]);
    Policy(4,:,:,:,N_j)=reshape(Policy3_ford3(3+temp),[1,N_a,N_semiz,N_e]);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            EVfine=reshape(EVinterp(d2a1primea2bothze(:)),[N_d2*n2long,N_a1*N_a2,N_semiz,N_e]);
            ...
            linidx=reshape(maxindexL2,[1,N_a*N_semiz*N_e])+N_d2*n2long*(0:N_a*N_semiz*N_e-1);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                EVfine=reshape(EVinterp_e(d2a1primea2bothz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
                entireRHS_L2=ReturnMatrix_L2+beta0beta*EVfine;
                [Vtempii,maxindexL2]=max(entireRHS_L2,[],1);
                Vhat_ford3(:,:,e_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d2)+1;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % hat (QH-perceived): F + beta0*beta*EV, full divide-conquer + grid-interp
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_semiz,n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,1,0);
            entireRHS_ii=ReturnMatrix_ii+beta0beta*EVreshape;
            [~,maxindex1]=max(entireRHS_ii,[],2);
            midpoint(:,1,level1ii,:,:,:)=maxindex1;
            maxgap=squeeze(max(max(max(max(maxindex1(:,1,2:end,:,:,:)-maxindex1(:,1,1:end-1,:,:,:),[],6),[],5),[],4),[],1));
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                % hat (QH-perceived): F + beta0*beta*EV, full divide-conquer + grid-interp
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_semiz,special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,1,0);
                entireRHS_ii=ReturnMatrix_ii+beta0beta*EVreshape_e;
                [~,maxindex1]=max(entireRHS_ii,[],2);
                midpoint(:,1,level1ii,:,:)=maxindex1;
                maxgap=squeeze(max(max(max(maxindex1(:,1,2:end,:,:)-maxindex1(:,1,1:end-1,:,:),[],5),[],4),[],1));
    ...
    temp=3*((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindex-1)-1);
    Policy(1,:,:,:,jj)=reshape(Policy3_ford3(1+temp),[1,N_a,N_semiz,N_e]);
    Policy(3,:,:,:,jj)=reshape(Policy3_ford3(2+temp),[1,N_a,N_semiz,N_e]);
    Policy(4,:,:,:,jj)=reshape(Policy3_ford3(3+temp),[1,N_a,N_semiz,N_e]);
    flat_idx=(1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindex-1);
