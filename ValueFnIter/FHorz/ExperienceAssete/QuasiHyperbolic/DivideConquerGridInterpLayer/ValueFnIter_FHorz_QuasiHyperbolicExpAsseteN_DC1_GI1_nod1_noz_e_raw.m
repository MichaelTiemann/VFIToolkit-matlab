function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteN_DC1_GI1_nod1_noz_e_raw(n_d2,n_a1,n_a2,n_e, N_j, d2_gridvals, a1_gridvals, a2_grid, e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV_alt=beta*reshape(EV,[N_d2,N_a1,1,N_a2,N_e]); % (d2,a1prime,1,a2,e_cur); exponential
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*eind; % [N_d2,maxgap_V+1,1,N_a2,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_e]
        ...
        % aprime possibilities are n_d2-by-n2long-by-n_a1-by-n_a2-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,n2long,n_a1,n_a2,n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        da1primea2=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*eind; % [N_d2,n2long,N_a1,N_a2,N_e]; linear index into DiscountedEVinterp_alt [N_d2,N_a1prime,1,N_a2,N_e]
        entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(da1primea2),[N_d2*n2long,N_a1*N_a2,N_e]);
        [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
        Valt(:,:,N_j)=shiftdim(Vtempii,1);
        d_indalt=rem(maxindexL2alt-1,N_d2)+1;
        allindalt=d_indalt+N_d2*aind+N_d2*N_a*eindB; % midpoint is n_d2-by-1-by-n_a1-by-n_a2-by-n_e
        ...
        linidx_loweralt = d_indalt                  + N_d2*n2long*aind + N_d2*n2long*N_a*eindB;
        linidx_upperalt = d_indalt + N_d2*(n2long-1) + N_d2*n2long*aind + N_d2*n2long*N_a*eindB;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*eind; % [N_d2,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_e]
        ...
        % aprime possibilities are n_d2-by-n2long-by-n_a1-by-n_a2-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,n2long,n_a1,n_a2,n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        da1primea2=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*eind; % [N_d2,n2long,N_a1,N_a2,N_e]; linear index into DiscountedEVinterp_tilde [N_d2,N_a1prime,1,N_a2,N_e]
        entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde(da1primea2),[N_d2*n2long,N_a1*N_a2,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,special_n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d2,maxgap_V+1,1,N_a2]; linear index into DiscountedEV_alt_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=ReturnMatrix_ii_dc+DiscountedEV_alt_e(d2aprime);
            ...
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint, fine index
            % aprime possibilities are n_d2-by-n2long-by-n_a1-by-n_a2
            ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,n2long,n_a1,n_a2,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
            da1primea2=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d2,n2long,N_a1,N_a2]; linear index into DiscountedEVinterp_alt_e [N_d2,N_a1prime,1,N_a2]
            entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt_e(da1primea2),[N_d2*n2long,N_a1*N_a2]);
            [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
            Valt(:,e_c,N_j)=shiftdim(Vtempii,1);
            d_indalt=rem(maxindexL2alt-1,N_d2)+1;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d2,maxgap+1,1,N_a2]; linear index into DiscountedEV_tilde_e [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=ReturnMatrix_ii_dc+DiscountedEV_tilde_e(d2aprime);
                    [~,maxindex]=max(entireRHS_ii_e,[],2);
                    midpoint(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
            ...
            % aprime possibilities are n_d2-by-n2long-by-n_a1-by-n_a2
            ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,n2long,n_a1,n_a2,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
            da1primea2=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d2,n2long,N_a1,N_a2]; linear index into DiscountedEVinterp_tilde_e [N_d2,N_a1prime,1,N_a2]
            entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde_e(da1primea2),[N_d2*n2long,N_a1*N_a2]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                % aprime possibilities are n_d-by-maxgap_V(ii)+1-by-1-by-n_a2-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*eind; % [N_d2,maxgap_V+1,1,N_a2,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime);
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
        ...
        a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint, fine index
        % aprime possibilities are n_d2-by-n2long-by-n_a1-by-n_a2-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,n2long,n_a1,n_a2,n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        da1primea2=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*eind; % [N_d2,n2long,N_a1,N_a2,N_e]; linear index into DiscountedEVinterp_alt [N_d2,N_a1prime,1,N_a2,N_e]
        entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(da1primea2),[N_d2*n2long,N_a1*N_a2,N_e]);
        [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_e, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprime=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*eind; % [N_d2,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_e]
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime);
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
