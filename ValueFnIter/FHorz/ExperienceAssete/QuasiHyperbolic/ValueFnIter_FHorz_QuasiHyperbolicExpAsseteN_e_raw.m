function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAsseteN_e_raw(n_d1,n_d2,n_a1,n_a2,n_z,n_e,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Naive QH + ExpAssete (e dep aprimeFn), baseline (no DC, no GI), with a1, with z.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    EV=permute(EV,[1,2,4,3]); % (d2*a1prime,a2,zcur,e_cur)  -- match ReturnMatrix dim order

    entireEV=repelem(EV,N_d1,N_a1,1,1); % undiscounted; beta/beta0beta applied below
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_ze=ReturnMatrix_ze+beta*entireEV_ze;

                % Calc the max and its index
                [Vtemp,maxindexalt]=max(entireRHS_ze,[],1);

                Valt(:,z_c,e_c,N_j)=shiftdim(Vtemp,1);
                Policyalt(:,z_c,e_c,N_j)=shiftdim(maxindexalt,1);

                entireRHS_ze=ReturnMatrix_ze+beta0beta*entireEV_ze;

                [Vtemp,maxindex]=max(entireRHS_ze,[],1);

                Vtilde(:,z_c,e_c,N_j)=shiftdim(Vtemp,1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Valt(:,z_c,e_c,jj)=shiftdim(Vtemp,1);
                Policyalt(:,z_c,e_c,jj)=shiftdim(maxindexalt,1);

                entireRHS_ze=ReturnMatrix_ze+beta0beta*entireEV_ze;

                [Vtemp,maxindex]=max(entireRHS_ze,[],1);

                Vtilde(:,z_c,e_c,jj)=shiftdim(Vtemp,1);
