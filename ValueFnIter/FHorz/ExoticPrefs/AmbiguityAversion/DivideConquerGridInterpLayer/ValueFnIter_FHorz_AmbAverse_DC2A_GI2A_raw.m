function [V, Policy]=ValueFnIter_FHorz_AmbAverse_DC2A_GI2A_raw(n_ambiguity, n_d,n_a,n_z, N_j, d_gridvals, a_grid, z_gridvals_J, ambiguity_pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    EVpre=reshape(vfoptions.V_Jplus1,[N_a,N_z]); % Using V_Jplus1
    ...
    DiscountedEVinterp=min(interp1(a1_grid,DiscountFactorParamsVec*reshape(ambEV,[N_a1,N_a2,1,1,N_z,size(ambEV,4)]),a1prime_grid),[],6); % interpolate each prior's discounted EV over a1prime_grid (conditional on the prior), then take the worst case over the priors
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprimez,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) in 4th-dim
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+DiscountedEV_z(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 4th-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
