function [V, Policy]=ValueFnIter_FHorz_AmbAverse_DC1_GI1_nod_raw(n_ambiguity, n_a,n_z,N_j, a_grid, z_gridvals_J,ambiguity_pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    EVinterp=min(interp1(a_grid,ambEV,aprime_grid'),[],4); % interpolate each prior's EV over aprime_grid, then take the worst case over the priors (the interpolation is conditional on the prior). Note: the transposed (column) query keeps the output orientation stable if ambEV ever collapses to a vector (interp1 returns the query's shape for vector inputs).
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+DiscountFactorParamsVec*EV_z(aprimeindexes); % autoexpand the level1iidiff(ii) in 2nd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,curraindex)=shiftdim(maxindex+(loweredge-1),1);
                else
                    loweredge=maxindex1(1,ii);
