function [V,Policy]=ValueFnIter_FHorz_AmbAverse_DC2A_GI2A_nod_noz_e_raw(n_ambiguity, n_a, n_e, N_j, a_grid, e_gridvals_J, ambiguity_pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEVinterp=min(interp1(a1_grid,DiscountFactorParamsVec*reshape(ambEV,[N_a1,N_a2,1,1,size(ambEV,2)]),a1prime_grid),[],5); % interpolate each prior's discounted EV over a1prime_grid (conditional on the prior), then take the worst case over the priors
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2,N_e]));  % autoexpand level1iidiff(ii) in 3rd-dim
        ...
        entireRHS_ii=ReturnMatrix_ii+reshape(DiscountedEVinterp(aprime),[n2long*N_a2,N_a,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_val, ReturnFnParamsVec,3);
                    aprime=aprimeindexes+N_a1*a2ind;
                    entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2]));  % autoexpand level1iidiff(ii) in 3rd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(1,:,ii,:);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,e_val, ReturnFnParamsVec,2);
            aprime=a1primeindexes+N_a1fine*a2ind;
            entireRHS_ii=ReturnMatrix_ii+reshape(DiscountedEVinterp(aprime),[n2long*N_a2,N_a]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            maxindexL2a1=rem(maxindexL2-1,n2long)+1;
            maxindexL2a2=ceil(maxindexL2/n2long);
            V(:,e_c,N_j)=shiftdim(Vtempii,1);
