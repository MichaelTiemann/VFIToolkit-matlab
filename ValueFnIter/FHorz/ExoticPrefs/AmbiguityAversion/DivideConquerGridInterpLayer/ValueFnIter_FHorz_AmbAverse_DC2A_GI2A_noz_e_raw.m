function [V, Policy]=ValueFnIter_FHorz_AmbAverse_DC2A_GI2A_noz_e_raw(n_ambiguity, n_d,n_a,n_e, N_j, d_gridvals, a_grid, e_gridvals_J, ambiguity_pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEVinterp=min(interp1(a1_grid,DiscountFactorParamsVec*reshape(ambEV,[N_a1,N_a2,size(ambEV,2)]),a1prime_grid),[],3); % interpolate each prior's discounted EV over a1prime_grid (conditional on the prior), then take the worst case over the priors
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_e])); % autoexpand level1iidiff(ii) in 4th-dim
        ...
        entireRHS_ii=ReturnMatrix_ii+reshape(DiscountedEVinterp(aprime),[N_d*n2long*N_a2,N_a,N_e]);
