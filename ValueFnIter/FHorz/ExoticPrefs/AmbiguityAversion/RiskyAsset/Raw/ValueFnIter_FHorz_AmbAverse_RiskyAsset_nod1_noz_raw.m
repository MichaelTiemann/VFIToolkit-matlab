function [V,Policy]=ValueFnIter_FHorz_AmbAverse_RiskyAsset_nod1_noz_raw(n_ambiguity, n_d2,n_d3,n_a1,n_a2,n_u, N_j, d2_grid, d3_grid, a1_grid,a2_grid, u_grid, ambiguity_pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    EVpreu=EV; % (d23 & a1prime, u): keep the pre-sum EV, each prior's expectation is taken from it
    EV=squeeze(sum((EVpreu.*shiftdim(ambiguity_pi_u(:,1),-1)),2));
    for amb_cu=2:n_ambiguity(N_j)
        EV=min(EV,squeeze(sum((EVpreu.*shiftdim(ambiguity_pi_u(:,amb_cu),-1)),2)));
    end
