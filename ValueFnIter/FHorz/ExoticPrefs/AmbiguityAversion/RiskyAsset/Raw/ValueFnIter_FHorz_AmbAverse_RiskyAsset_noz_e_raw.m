...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for amb_c=1:n_ambiguity(N_j)
        ...
        EV1=aprimeProbsK.*reshape(V_Jplus1(aprimeIndex),[N_d23*N_a1,N_u]); % (d&a1prime,u), the lower aprime
        EV2=(1-aprimeProbsK).*reshape(V_Jplus1(aprimeplus1Index),[N_d23*N_a1,N_u]); % (d&a1prime,u), the upper aprime
...
for reverse_j=1:N_j-1
    ...
    for amb_c=1:n_ambiguity(jj)
        ...
        % Switch EV from being in terms of aprime to being in terms of d (in expectation because of the u shocks)
        EV1=aprimeProbsK.*reshape(EV(aprimeIndex),[N_d23*N_a1,N_u]); % (d&a1prime,u), the lower aprime
        EV2=(1-aprimeProbsK).*reshape(EV(aprimeplus1Index),[N_d23*N_a1,N_u]); % (d&a1prime,u), the upper aprime
        % Already applied the probabilities from interpolating onto grid
