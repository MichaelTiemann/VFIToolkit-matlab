...
Policy=zeros(3,N_a,N_z,N_e,N_j,'gpuArray'); % d1,d2,d3
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, n_d13, n_a, n_z, special_n_e, d13_gridvals, a_gridvals, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
            % Calc the max and it's index
    ...
    elseif vfoptions.lowmemory==2
        ...
        for e_c=1:N_e
            ...
            for z_c=1:N_z
                ...
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, n_d13, n_a, special_n_z, special_n_e, d13_gridvals, a_gridvals, z_val, e_val, ReturnFnParamsVec);
                % Calc the max and it's index
                [Vtemp,maxindex]=max(ReturnMatrix_ze,[],1);
                V(:,z_c,e_c,N_j)=Vtemp;
                Policy(1,:,z_c,e_c,N_j)=shiftdim(rem(maxindex-1,N_d1)+1,-1);
                Policy(2,:,z_c,e_c,N_j)=1; % is meaningless anyway
                Policy(3,:,z_c,e_c,N_j)=shiftdim(ceil(maxindex/N_d1),-1);
            end
        end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                V(:,z_c,e_c,N_j)=Vtemp;
                Policy(3,:,z_c,e_c,N_j)=shiftdim(maxindex,1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, n_d13, n_a, n_z, n_e, d13_gridvals, a_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,aprime,a,z,e)
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, n_d13, n_a, n_z, special_n_e, d13_gridvals, a_gridvals, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec);
            % (d,aprime,a,z)
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                [Vtemp,maxindex]=max(entireRHS_ze,[],1);
                V(:,z_c,e_c,jj)=Vtemp;
                Policy(3,:,z_c,e_c,jj)=shiftdim(maxindex,1);
