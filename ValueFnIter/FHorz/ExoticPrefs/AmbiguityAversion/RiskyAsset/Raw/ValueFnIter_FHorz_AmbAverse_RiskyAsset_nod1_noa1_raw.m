function [V,Policy]=ValueFnIter_FHorz_AmbAverse_RiskyAsset_nod1_noa1_raw(n_ambiguity, n_d2,n_d3,n_a,n_z,n_u,N_j, d2_grid, d3_grid, a_grid, z_gridvals_J, u_grid, ambiguity_pi_z_J, ambiguity_pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
Policy=zeros(2,N_a,N_z,N_j,'gpuArray'); % two: d2, d3
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d3, n_a, n_z, d3_gridvals, a_gridvals, z_gridvals_J(:,:,N_j), ReturnFnParamsVec);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d3, n_a, n_z, d3_gridvals, a_gridvals, z_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,aprime,a,z)
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            [Vtemp,maxindex]=max(entireRHS_z,[],1);
            V(:,z_c,jj)=Vtemp;
            Policy(2,:,z_c,jj)=shiftdim(maxindex,1); % d3
            Policy(1,:,z_c,jj)=shiftdim(d2index(maxindex),1); % d2
