function [V,Policy]=ValueFnIter_FHorz_AmbAverse_RiskyAsset_nod1_raw(n_ambiguity, n_d2,n_d3,n_a1,n_a2,n_z,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, z_gridvals_J, u_grid, ambiguity_pi_z_J, ambiguity_pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            % Now put together entireRHS, which just depends on d3
            entireRHS_z=ReturnMatrix_z+DiscountedEV_onlyd3_z;

            %Calc the max and it's index
            [Vtemp,maxindex]=max(entireRHS_z,[],1);
            V(:,z_c,N_j)=Vtemp;
            Policy(2,:,z_c,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1); % d3
            Policy(3,:,z_c,N_j)=shiftdim(ceil(maxindex/N_d3),-1);
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            Policy(1,:,z_c,jj)=shiftdim(d2index(maxindex),1);
        end
    end
end
