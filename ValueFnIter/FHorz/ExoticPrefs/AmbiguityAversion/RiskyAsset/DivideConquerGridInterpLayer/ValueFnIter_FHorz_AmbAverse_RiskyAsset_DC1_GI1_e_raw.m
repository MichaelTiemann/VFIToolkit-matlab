...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        [Vtempii,maxindexL2]=max(reshape(ReturnMatrix_ii,[N_d13*n2long,N_a1*N_a2,N_z,N_e]),[],1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                [Vtempii,maxindexL2]=max(reshape(ReturnMatrix_ii,[N_d13*n2long,N_a1*N_a2,1]),[],1);
    ...
    % Get EV in terms of next period endogenous states; integrate out e first, then zprime
    ...
    if vfoptions.lowmemory==0
        % Layer 1
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,n_z,n_e, d13_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
        % [N_d13, level1n, N_a1, N_a2, N_z, N_e]
        ReturnMatrix_ii=reshape(ReturnMatrix_ii,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2,N_z,N_e]);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,n_e, d13_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*shiftdim(zindB,-2); % [N_d13,maxgap+1,1,N_a2,N_z]; e broadcasts
        ...
        entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_z,N_e])+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_z,N_e]),[N_d13*n2long,N_a1*N_a2,N_z,N_e]);
        ...
        a1mid=squeeze(midpoint_jj(allind)); % [N_a,N_z,N_e]
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            % Layer 1
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,n_z,special_n_e, d13_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1,0);
            ReturnMatrix_ii_e=reshape(ReturnMatrix_ii_e,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2,N_z]);
            DEV=reshape(DiscountedEV,[1,N_d3,N_a1,1,1,N_z]);
            entireRHS_ii_e=ReturnMatrix_ii_e+DEV;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                % Layer 1
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,special_n_z,special_n_e, d13_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
                ReturnMatrix_ii_e=reshape(ReturnMatrix_ii_e,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2,1]);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii_e=ReturnMatrix_ii_e+DiscountedEV_z(d3aprimez);
                        [~,maxindex]=max(entireRHS_ii_e,[],2);
                        midpoint_jj(:,1,curraindex,:,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
                ...
                entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,1])+reshape(DiscountedEVinterp_z(da1primez),[N_d13,n2long,N_a1,N_a2,1]),[N_d13*n2long,N_a1*N_a2,1]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                V(:,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d13)+1;
                allind=d_ind+N_d13*aind;
                ...
                Policy(2,:,z_c,e_c,N_j)=d2index_z(linlookup);
            end
        end
    end
end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                a1mid=midpoint_jj(allind);
                linlookup=d3part+N_d3*(a1mid-1);
                Policy(2,:,z_c,e_c,jj)=d2index_z(linlookup);
            end
        end
    end
end
