function [V,Policy]=ValueFnIter_FHorz_AmbAverse_RiskyAsset_DC1_GI1_nod1_raw(n_ambiguity, n_d2,n_d3,n_a1,n_a2,n_z,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, z_gridvals_J, u_grid, ambiguity_pi_z_J, ambiguity_pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Ambiguity aversion: multiple priors over ambiguity_pi_u (the risky return distribution is ambiguous, not known risk), pi_z;
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        RM=reshape(ReturnMatrix_ii,[N_d3,N_a1,vfoptions.level1n,N_a2,N_z]);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d3,n2long,N_a1,N_a2])+reshape(DiscountedEVinterp_z(da1prime),[N_d3,n2long,N_a1,N_a2]),[N_d3*n2long,N_a1*N_a2]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            V(:,z_c,N_j)=shiftdim(Vtempii,1);
            d3_ind=rem(maxindexL2-1,N_d3)+1;
            allind=d3_ind+N_d3*aind;
            ...
            Policy(1,:,z_c,N_j)=shiftdim(d2index_z(linlookup),-1);
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            a1mid=squeeze(midpoint_jj(allind));
            linlookup=d3_ind+N_d3*(a1mid-1);
            Policy(1,:,z_c,jj)=shiftdim(d2index_z(linlookup),-1);
        end
    end
end
