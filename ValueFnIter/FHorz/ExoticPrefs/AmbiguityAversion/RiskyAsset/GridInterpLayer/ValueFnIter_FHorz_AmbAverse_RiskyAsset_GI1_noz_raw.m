function [V,Policy]=ValueFnIter_FHorz_AmbAverse_RiskyAsset_GI1_noz_raw(n_ambiguity, n_d1,n_d2,n_d3,n_a1,n_a2,n_u,N_j, d1_grid, d2_grid, d3_grid, a1_grid, a2_grid, u_grid, ambiguity_pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Ambiguity aversion: multiple priors over ambiguity_pi_u (the risky return distribution is ambiguous, not known risk);
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for amb_cu=2:n_ambiguity(N_j)
        EV=min(EV,sum(EV1.*ambiguity_pi_u(:,amb_cu)',2)+sum(EV2.*ambiguity_pi_u(:,amb_cu)',2));
    end
