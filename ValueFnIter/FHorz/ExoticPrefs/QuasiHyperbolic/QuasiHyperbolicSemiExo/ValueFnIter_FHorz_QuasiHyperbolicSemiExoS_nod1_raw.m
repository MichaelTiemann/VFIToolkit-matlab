function [Vhat, Policy, Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_nod1_raw(n_d2, n_a, n_z, n_semiz, N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic with semi-exogenous shock, no d1.
...
Policy=zeros(2,N_a,N_semiz*N_z,N_j,'gpuArray'); % d2, aprime
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for z_c=1:N_bothz
            ...
            Vhat(:,z_c,N_j)=Vtemp;
            d_ind=shiftdim(rem(maxindex-1,N_d)+1,-1);
            Policy(1,:,z_c,N_j)=shiftdim(d_ind,-1);
            Policy(2,:,z_c,N_j)=shiftdim(ceil(maxindex/N_d),-1);
        end
    end
    Vunderbar=Vhat;
else
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            % entireRHS_alt shape: (1, N_a, N_bothz); maxindex shape: (1, N_a, N_bothz) indexing the aprime dim
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                entireRHS_alt=ReturnMatrix_d2z+beta*EV_d2z; % [N_a(aprime),N_a,N_semiz]
                maxindexfull=maxindex+N_a*(0:1:N_a-1)+shiftdim(N_a*N_a*(0:1:N_semiz-1),-1);
                Vunderbar_ford2_jj(:,semizblock,d2_c)=shiftdim(entireRHS_alt(maxindexfull),1);
            end
        end
        [Vhat_jj,maxindex]=max(Vhat_ford2_jj,[],3);
        Vhat(:,:,N_j)=Vhat_jj;
