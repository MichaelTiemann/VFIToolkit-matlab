function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC2A_nod1_raw(n_d2,n_a,n_z,n_semiz, N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive QH + SemiExo + DC2A (divide-and-conquer in the first endo state, a2 enumerated in full), no d1, with z, no e.
%
      ...
      if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d2, n_bothz, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,5,0);
                ...
                loweredge=maxindex1(1,:,ii,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d2, n_bothz, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,5,0);
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d2, [n_semiz,special_n_z], d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_valblock, ReturnFnParamsVec,5,0);
                    ...
                    maxindexfix=a1primeind+N_a1*(a2primeind-1);
                    allind=a2primeind+N_a2*repelem(a2ind,1,level1iidiff(ii))+N_a2*N_a2*semizind; % loweredge is 1-by-n_a2-by-1-by-n_a2-by-n_semiz
                    Policy_V_ford2_jj(curraindex,semizblock,d2_c)=shiftdim(maxindexfix+loweredge(allind)-1,1);
                else
                    loweredge=maxindex1(1,:,ii,:,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d2, [n_semiz,special_n_z], d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_valblock, ReturnFnParamsVec,5,0);
        ...
        for z_c=1:N_bothz
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d2, special_n_bothz, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsVec,5,0);
                    ...
                    maxindexfix=a1primeind+N_a1*(a2primeind-1);
                    allind=a2primeind+N_a2*repelem(a2ind,1,level1iidiff(ii)); % loweredge is 1-by-n_a2-by-1-by-n_a2
                    Policy_V_ford2_jj(curraindex,z_c,d2_c)=shiftdim(maxindexfix+loweredge(allind)-1,1);
                else
                    loweredge=maxindex1(1,:,ii,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d2, special_n_bothz, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsVec,5,0);
      ...
      if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                % aprime possibilities are (maxgap_V(ii)+1)-n_a2-by-1-by-n_a2-by-n_bothz
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d2, n_bothz, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,5,0);
                aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*bothzBind; % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_bothz]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,d2_c)=shiftdim(Vtempii,1);
                ...
                loweredge=maxindex1(1,:,ii,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d2, n_bothz, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,5,0);
                aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*bothzBind; % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[1*N_a2,level1iidiff(ii)*N_a2,N_bothz]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,d2_c)=shiftdim(Vtempii,1);
