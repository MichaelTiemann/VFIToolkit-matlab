function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_DC2A_nod1_raw(n_d2,n_a,n_z,n_semiz, N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated QH + SemiExo + DC2A (divide-and-conquer in the first endo state, a2 enumerated in full), no d1, with z, no e.
%
    ...
    for d2_c=1:N_d2
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+DiscountedEV_d2z(reshape(aprime,[(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat_ford2_jj(curraindex,semizblock,d2_c)=shiftdim(Vtempii,1);
                    a1primeind=rem(maxindex-1,maxgap(ii)+1)+1;
                    a2primeind=ceil(maxindex/(maxgap(ii)+1));
                    ...
                    entireRHS_ii=ReturnMatrix_ii+DiscountedEV_d2z(reshape(aprime,[1*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat_ford2_jj(curraindex,semizblock,d2_c)=shiftdim(Vtempii,1);
                    a1primeind=1;
                    a2primeind=maxindex;
            ...
            % Vunderbar = Vhat + (beta - beta0beta)*EV_at_policy
            aprime_ind_d2=Policy_ford2_jj(:,semizblock,d2_c);
      ...
      elseif vfoptions.lowmemory==2 % joint loop over bothz
        for z_c=1:N_bothz
            ...
            % Vunderbar = Vhat + (beta - beta0beta)*EV_at_policy
            aprime_ind_d2=Policy_ford2_jj(:,z_c,d2_c);
