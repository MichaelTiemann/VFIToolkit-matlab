function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_DC1_noz_e_raw(n_d1,n_d2,n_a,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated QH + SemiExo + DC, with d1, no z, with e.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            [Vtempii,maxindex2]=max(reshape(entireRHS_Vh,[N_d1*N_a,vfoptions.level1n,N_semiz,N_e]),[],1);
            Vhat_ford2_jj(level1ii,:,:,d2_c)=shiftdim(Vtempii,1);
            Policy_ford2_jj(level1ii,:,:,d2_c)=shiftdim(maxindex2,1);
            maxgap=squeeze(max(max(max(maxindex1(:,1,2:end,:,:)-maxindex1(:,1,1:end-1,:,:),[],5),[],4),[],1));
            for ii=1:(vfoptions.level1n-1)
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_Vh=ReturnMatrix_d2ii+beta0beta*shiftdim(EV_d2semiz,-1);
                    [~,maxindex1]=max(entireRHS_Vh,[],2);
                ...
                aprime_ind_d2semiz=ceil(Policy_ford2_jj(:,semiz_c,:,d2_c)/N_d1);
