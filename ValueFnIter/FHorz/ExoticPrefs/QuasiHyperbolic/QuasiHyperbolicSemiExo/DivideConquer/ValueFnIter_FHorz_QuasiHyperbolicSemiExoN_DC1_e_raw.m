function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC1_e_raw(n_d1,n_d2,n_a,n_z,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% Naive QH + SemiExo + DC, with d1, z, e. Dual-Valt; cross-d2 max on Vtilde.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            [Vtempii_V,maxindex2_V]=max(reshape(entireRHS_V,[N_d1*N_a,vfoptions.level1n,N_bothz,N_e]),[],1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    allindalt=dindalt+N_d1*bothzind+N_d1*N_bothz*eind;
                    ...
                    entireRHS_ii=ReturnMatrix_ii+beta*reshape(EV_d2(aprimez),[N_d1,1,N_bothz,N_e]);
                    [Vtempii,maxindex2alt]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                    dindalt=(rem(maxindex2alt-1,N_d1)+1);
                    allindalt=dindalt+N_d1*bothzind+N_d1*N_bothz*eind;
            ...
            [Vtempii,maxindex2]=max(reshape(entireRHS_Vt,[N_d1*N_a,vfoptions.level1n,N_bothz,N_e]),[],1);
            Vtilde_ford2_jj(level1ii,:,:,d2_c)=shiftdim(Vtempii,1);
            Policy_ford2_jj(level1ii,:,:,d2_c)=shiftdim(maxindex2,1);
            maxgap=squeeze(max(max(max(maxindex1(:,1,2:end,:,:)-maxindex1(:,1,1:end-1,:,:),[],5),[],4),[],1));
            for ii=1:(vfoptions.level1n-1)
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    %% Valt slab (beta)
                    entireRHS_V=ReturnMatrix_d2iize+beta*shiftdim(EV_d2z,-1);
                    [~,maxindex1_V]=max(entireRHS_V,[],2);
                    ...
                    Policy_V_ford2_jj(level1ii,semizblock,e_c,d2_c)=shiftdim(maxindex2_V,1);
                    maxgap_V=squeeze(max(max(maxindex1_V(:,1,2:end,:)-maxindex1_V(:,1,1:end-1,:),[],4),[],1));
                    for ii=1:(vfoptions.level1n-1)
                        curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                        if maxgap_V(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iize+beta*reshape(EV_d2z(aprimez),[N_d1*(maxgap_V(ii)+1),1,N_semiz]);
                            [Vtempii,maxindex2alt]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                            dindalt=(rem(maxindex2alt-1,N_d1)+1);
                            allindalt=dindalt+N_d1*semizind;
                            Policy_V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(maxindex2alt+N_d1*(loweredge(allindalt)-1));
                        else
                            loweredge=maxindex1_V(:,1,ii,:);
                            ReturnMatrix_iize=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, [n_semiz,special_n_z], special_n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, e_val, ReturnFnParamsVec,2);
                            aprimez=loweredge+N_a*semizBind;
                            entireRHS_ii=ReturnMatrix_iize+beta*reshape(EV_d2z(aprimez),[N_d1,1,N_semiz]);
                            [Vtempii,maxindex2alt]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                            dindalt=(rem(maxindex2alt-1,N_d1)+1);
                            allindalt=dindalt+N_d1*semizind;
                    ...
                    %% Vtilde slab (beta0*beta) -> Policy
                    entireRHS_Vt=ReturnMatrix_d2iize+beta0beta*shiftdim(EV_d2z,-1);
                    [~,maxindex1]=max(entireRHS_Vt,[],2);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    Policy_V_ford2_jj(level1ii,z_c,e_c,d2_c)=shiftdim(maxindex2_V,1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_V(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iize+beta*reshape(EV_d2z(aprimez),[N_d1*(maxgap_V(ii)+1),1]);
                            [Vtempii,maxindex2alt]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            dindalt=(rem(maxindex2alt-1,N_d1)+1);
                            Policy_V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(maxindex2alt+N_d1*(reshape(loweredge(dindalt),size(maxindex2alt))-1));
                        else
                            loweredge=maxindex1_V(:,1,ii);
                            ReturnMatrix_iize=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, ReturnFnParamsVec,2);
                            aprimez=loweredge;
                            entireRHS_ii=ReturnMatrix_iize+beta*reshape(EV_d2z(aprimez),[N_d1,1]);
                            [Vtempii,maxindex2alt]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            dindalt=(rem(maxindex2alt-1,N_d1)+1);
                            Policy_V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(maxindex2alt+N_d1*(reshape(loweredge(dindalt),size(maxindex2alt))-1));
    ...
    d1aprime_ind_alt=reshape(Policy_V_ford2_jj((1:1:NN)'+NN*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
