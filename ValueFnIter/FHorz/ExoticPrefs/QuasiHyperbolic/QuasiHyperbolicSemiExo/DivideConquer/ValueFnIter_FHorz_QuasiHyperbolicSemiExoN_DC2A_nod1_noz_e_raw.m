function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC2A_nod1_noz_e_raw(n_d2,n_a,n_semiz,n_e, N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive QH + SemiExo + DC2A (divide-and-conquer in the first endo state, a2 enumerated in full), no d1, no z, with e.
%
  ...
  elseif vfoptions.lowmemory==2 % outer semiz / inner e
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        EV_d2=reshape(EV_d2,[N_a1,N_a2,1,1,N_semiz]); % autoexpand (a,semiz,e)
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                % aprime possibilities are (maxgap_V(ii)+1)-n_a2-by-1-by-n_a2-by-n_semiz-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,5,0);
                aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_semiz,N_e]));
                ...
                loweredge=maxindex1(1,:,ii,:,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,5,0);
                aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[1*N_a2,level1iidiff(ii)*N_a2,N_semiz,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,5,0);
                aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2_tilde(reshape(aprime,[(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_semiz,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,5,0);
                aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2_tilde(reshape(aprime,[1*N_a2,level1iidiff(ii)*N_a2,N_semiz,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
      ...
      elseif vfoptions.lowmemory==1 % loop e, vectorise semiz
        ...
        EV_d2(isnan(EV_d2))=0; %multiplications of -Inf with 0 gives NaN, this replaces them with zeros (as the zeros come from the transition probabilities)
        EV_d2=sum(EV_d2,2); % sum over z', leaving a singular second dimension
        EV_d2=reshape(EV_d2,[N_a1,N_a2,1,1,N_semiz]); % autoexpand (a,semiz)
        DiscountedEV_d2=beta*EV_d2;
        DiscountedEV_d2_tilde=beta0beta*EV_d2;
        for e_c=1:N_e
            e_val=e_gridvals_J(e_c,:,N_j);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,5,0);
                    aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    a1primeind=rem(maxindex-1,maxgap_V(ii)+1)+1;
                    a2primeind=ceil(maxindex/(maxgap_V(ii)+1));
                    ...
                    loweredge=maxindex1(1,:,ii,:,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,5,0);
                    aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[1*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    a1primeind=1;
                    a2primeind=maxindex;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,5,0);
                    aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2_tilde(reshape(aprime,[(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vtilde_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    a1primeind=rem(maxindex-1,maxgap(ii)+1)+1;
                    a2primeind=ceil(maxindex/(maxgap(ii)+1));
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,5,0);
                    aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2_tilde(reshape(aprime,[1*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vtilde_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    a1primeind=1;
                    a2primeind=maxindex;
      ...
      elseif vfoptions.lowmemory==2 % outer semiz / inner e
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem(a2ind',vfoptions.level1n,1);
                V_ford2_jj(curraindex,semiz_c,e_c,d2_c)=shiftdim(Vtempii,1);
                Policy_V_ford2_jj(curraindex,semiz_c,e_c,d2_c)=shiftdim(maxindex2,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, special_n_semiz, special_n_e, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_val, e_val, ReturnFnParamsVec,5,0);
                        ...
                        maxindexfix=a1primeind+N_a1*(a2primeind-1);
                        allind=a2primeind+N_a2*repelem(a2ind,1,level1iidiff(ii)); % loweredge is 1-by-n_a2-by-1-by-n_a2
                        Policy_V_ford2_jj(curraindex,semiz_c,e_c,d2_c)=shiftdim(maxindexfix+loweredge(allind)-1,1);
                    else
                        loweredge=maxindex1(1,:,ii,:);
                        % Just use aprime(ii) for everything
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, special_n_semiz, special_n_e, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_val, e_val, ReturnFnParamsVec,5,0);
                ...
                [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_a1*N_a2,vfoptions.level1n*N_a2]),[],1);
                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem(a2ind',vfoptions.level1n,1);
                Vtilde_ford2_jj(curraindex,semiz_c,e_c,d2_c)=shiftdim(Vtempii,1);
                Policy_ford2_jj(curraindex,semiz_c,e_c,d2_c)=shiftdim(maxindex2,1);
                maxgap=squeeze(max(max(maxindex1(1,:,2:end,:)-maxindex1(1,:,1:end-1,:),[],4),[],2));
                for ii=1:(vfoptions.level1n-1)
                    curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2ind',level1iidiff(ii),1);
    ...
    maxindexalt_lin=reshape(maxindexalt_d2,[N_a*N_semiz*N_e,1]); % This is the value of d that corresponds, make it this shape for addition just below
      ...
      if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                % aprime possibilities are (maxgap_V(ii)+1)-n_a2-by-1-by-n_a2-by-n_semiz-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,5,0);
                aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_semiz,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                ...
                loweredge=maxindex1(1,:,ii,:,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,5,0);
                aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[1*N_a2,level1iidiff(ii)*N_a2,N_semiz,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                    % aprime possibilities are (maxgap_V(ii)+1)-n_a2-by-1-by-n_a2-by-n_semiz
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,5,0);
                    aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    ...
                    loweredge=maxindex1(1,:,ii,:,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,5,0);
                    aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*semizBind; % the current aprimeii(ii):aprimeii(ii+1)
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2(reshape(aprime,[1*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
