function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_DC2A_raw(n_d1, n_d2,n_a,n_z,n_semiz, N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated QH + SemiExo + DC2A (divide-and-conquer in the first endo state, a2 enumerated in full), with d1, with z, no e.
%
...
if ~isfield(vfoptions,'V_Jplus1')
  ...
  elseif vfoptions.lowmemory==1 % split: outer z, vectorise semiz
    for d2_c=1:N_d2
        ...
        for z_c=1:N_z
            ...
            curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem(a2ind',vfoptions.level1n,1);
            Vhat_ford2_jj(curraindex,semizblock,d2_c)=shiftdim(Vtempii,1);
  ...
  elseif vfoptions.lowmemory==2 % joint over bothz
    for d2_c=1:N_d2
        ...
        for z_c=1:N_bothz
            ...
            curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem(a2ind',vfoptions.level1n,1);
            Vhat_ford2_jj(curraindex,z_c,d2_c)=shiftdim(Vtempii,1);
    ...
    [V_jj,maxindex]=max(Vhat_ford2_jj,[],3); % max over d2
    Vhat(:,:,N_j)=V_jj;
  ...
  if vfoptions.lowmemory==0
    for d2_c=1:N_d2
        ...
        EV_at_policy_d2=reshape(EVd2_2d(aprime_ind_d2+N_a*reshape(0:1:N_bothz-1,1,N_bothz)),[N_a,N_bothz]);
  ...
  elseif vfoptions.lowmemory==1 % split: outer z, vectorise semiz
    for d2_c=1:N_d2
        ...
        for z_c=1:N_z
            ...
            EV_at_policy_d2=reshape(EVd2z_2d(aprime_ind_d2+N_a*reshape(0:1:N_semiz-1,1,N_semiz)),[N_a,N_semiz]);
  ...
  elseif vfoptions.lowmemory==2 % joint over bothz
    for d2_c=1:N_d2
        ...
        for z_c=1:N_bothz
            ...
            EV_at_policy_d2=EV_d2z(aprime_ind_d2);
            Vunderbar_ford2_jj(:,z_c,d2_c)=Vhat_ford2_jj(:,z_c,d2_c)+(beta-beta0beta)*EV_at_policy_d2;
        end
    end
  end
    % Now we just max over d2, and keep the policy that corresponded to that (including modify the policy to include the d2 decision)
    [V1_jj,maxindex]=max(Vhat_ford2_jj,[],3); % max over d2
    Vhat(:,:,N_j)=V1_jj;
    Policy(2,:,:,N_j)=shiftdim(maxindex,-1); % d2 is just maxindex
