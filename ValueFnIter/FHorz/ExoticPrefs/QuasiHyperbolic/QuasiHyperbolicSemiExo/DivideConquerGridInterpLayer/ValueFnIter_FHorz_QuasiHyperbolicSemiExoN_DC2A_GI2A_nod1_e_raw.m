function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC2A_GI2A_nod1_e_raw(n_d2, n_a, n_z, n_semiz, n_e, N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
...
Policyalt=zeros(4,N_a,N_bothz,N_e,N_j,'gpuArray'); % exponential discounter's optimal choice
  ...
  elseif vfoptions.lowmemory==3 % joint bothz / inner e
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        maxgap_V=squeeze(max(max(max(max(maxindex1(1,1,:,2:end,:,:,:)-maxindex1(1,1,:,1:end-1,:,:,:),[],3),[],5),[],6),[],7));
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            if maxgap_V(ii)>0
                loweredge=min(maxindex1(1,1,:,ii,:,:,:),N_a1-maxgap_V(ii));
                a1primeindexes=loweredge+shiftdim((0:1:maxgap_V(ii))',-1);
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3, 1);
                aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[(maxgap_V(ii)+1)*1,N_a2,1,N_a2,N_bothz,N_e]));
        ...
        midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
        a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, n_e, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2, 0);
        aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
        entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEVinterp(aprime),[n2long*N_a2,N_a,N_bothz,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        maxindexL2a1=rem(maxindexL2-1,n2long)+1;
        maxindexL2a2=ceil(maxindexL2/n2long);

        V_ford2alt(:,:,:,d2_c)=shiftdim(Vtempii,1);
        mid_ford2alt(:,:,:,d2_c)=midpoints_jj(maxindexL2a2+N_a2*a12ind+N_a2*N_a*zind+N_a2*N_a*N_bothz*eind);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3, 1);
                aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[(maxgap(ii)+1)*1,N_a2,1,N_a2,N_bothz,N_e]));
                [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                midpoints_jj(1,1,:,curraindex,:,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
            else
                loweredge=maxindex1(1,1,:,ii,:,:,:);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1(1,1,:,ii,:,:),N_a1-maxgap_V(ii));
                    a1primeindexes=loweredge+shiftdim((0:1:maxgap_V(ii))',-1);
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3, 1);
                    aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*zBind;
            ...
            midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
            a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, special_n_e, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 2, 0);
            aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
            ...
            mid_ford2alt(:,:,e_c,d2_c)=midpoints_jj(maxindexL2a2+N_a2*a12ind+N_a2*N_a*zind);
            ...
            inLowerStrict = (maxindexL2a1 >= 2)         & (maxindexL2a1 <= n2short+1);
            inUpperStrict = (maxindexL2a1 >= n2short+3) & (maxindexL2a1 <= n2long-1);
            L2flag_ford2alt(:,:,e_c,d2_c) = squeeze(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper));
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        loweredge=min(maxindex1(1,1,:,ii,:,:),N_a1-maxgap_V(ii));
                        a1primeindexes=loweredge+shiftdim((0:1:maxgap_V(ii))',-1);
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_valblock, e_val, ReturnFnParamsVec, 3, 1);
                ...
                midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
                a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, z_valblock, e_val, ReturnFnParamsVec, 2, 0);
                ...
                mid_ford2alt(:,semizblock,e_c,d2_c)=midpoints_jj(maxindexL2a2+N_a2*a12ind+N_a2*N_a*semizind);
                ...
                inLowerStrict = (maxindexL2a1 >= 2)         & (maxindexL2a1 <= n2short+1);
                inUpperStrict = (maxindexL2a1 >= n2short+3) & (maxindexL2a1 <= n2long-1);
                L2flag_ford2alt(:,semizblock,e_c,d2_c) = squeeze(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper));
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        loweredge=min(maxindex1(1,1,:,ii,:),N_a1-maxgap_V(ii));
                        a1primeindexes=loweredge+shiftdim((0:1:maxgap_V(ii))',-1);
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_val, e_val, ReturnFnParamsVec, 3, 1);
                ...
                midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
                a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, z_val, e_val, ReturnFnParamsVec, 2, 0);
                ...
                mid_ford2alt(:,z_c,e_c,d2_c)=midpoints_jj(maxindexL2a2+N_a2*a12ind);
                ...
                inLowerStrict = (maxindexL2a1 >= 2)         & (maxindexL2a1 <= n2short+1);
                inUpperStrict = (maxindexL2a1 >= n2short+3) & (maxindexL2a1 <= n2long-1);
                L2flag_ford2alt(:,z_c,e_c,d2_c) = shiftdim(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper), 1);
    ...
    Policyalt(2,:,:,:,N_j)=reshape(mid_ford2alt(idxalt), [1,N_a,N_bothz,N_e]);
    Policyalt(3,:,:,:,N_j)=reshape(L2a2_ford2alt(idxalt),[1,N_a,N_bothz,N_e]);
    Policyalt(4,:,:,:,N_j)=reshape(L2a1_ford2alt(idxalt),[1,N_a,N_bothz,N_e]);
...
for reverse_j=1:N_j-1
      ...
      if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                loweredge=min(maxindex1(1,1,:,ii,:,:,:),N_a1-maxgap_V(ii));
                a1primeindexes=loweredge+shiftdim((0:1:maxgap_V(ii))',-1);
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3, 1);
                aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[(maxgap_V(ii)+1),N_a2,1,N_a2,N_bothz,N_e]));
                [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                midpoints_jj(1,1,:,curraindex,:,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
            else
                loweredge=maxindex1(1,1,:,ii,:,:,:);
        ...
        midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
        a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, n_e, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 2, 0);
        aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
        entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEVinterp(aprime),[n2long*N_a2,N_a,N_bothz,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        maxindexL2a1=rem(maxindexL2-1,n2long)+1;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3, 1);
                aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2,N_bothz,N_e]));
                [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                midpoints_jj(1,1,:,curraindex,:,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
            else
                loweredge=maxindex1(1,1,:,ii,:,:,:);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1(1,1,:,ii,:,:),N_a1-maxgap_V(ii));
                    a1primeindexes=loweredge+shiftdim((0:1:maxgap_V(ii))',-1);
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3, 1);
                    aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*zBind;
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[(maxgap_V(ii)+1),N_a2,1,N_a2,N_bothz]));
                    [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                    midpoints_jj(1,1,:,curraindex,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
                else
                    loweredge=maxindex1(1,1,:,ii,:,:);
            ...
            midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
            a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, special_n_e, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 2, 0);
            aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
            entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEVinterp(aprime),[n2long*N_a2,N_a,N_bothz]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            maxindexL2a1=rem(maxindexL2-1,n2long)+1;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3, 1);
                    aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*zBind;
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2,N_bothz]));
                    [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                    midpoints_jj(1,1,:,curraindex,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
                else
                    loweredge=maxindex1(1,1,:,ii,:,:);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_valblock, e_val, ReturnFnParamsVec, 3, 1);
                        aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*semizBind;
                        entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2z(reshape(aprime,[(maxgap_V(ii)+1),N_a2,1,N_a2,N_semiz]));
                        [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                        midpoints_jj(1,1,:,curraindex,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
                    else
                        loweredge=maxindex1(1,1,:,ii,:,:);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_valblock, e_val, ReturnFnParamsVec, 3, 1);
                        aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*semizBind;
                        entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2z(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2,N_semiz]));
                        [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                        midpoints_jj(1,1,:,curraindex,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
                    else
                        loweredge=maxindex1(1,1,:,ii,:,:);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_val, e_val, ReturnFnParamsVec, 3, 1);
                        aprime=a1primeindexes+N_a1*a2ind;
                        entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2z(reshape(aprime,[(maxgap_V(ii)+1),N_a2,1,N_a2]));
                        [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                        midpoints_jj(1,1,:,curraindex,:)=shiftdim(maxindex,-1)+(loweredge-1);
                    else
                        loweredge=maxindex1(1,1,:,ii,:);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_val, e_val, ReturnFnParamsVec, 3, 1);
                        aprime=a1primeindexes+N_a1*a2ind;
                        entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_d2z(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2]));
                        [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                        midpoints_jj(1,1,:,curraindex,:)=shiftdim(maxindex,-1)+(loweredge-1);
                    else
                        loweredge=maxindex1(1,1,:,ii,:);
    ...
    d2_maxalt_lin=reshape(d2_maxalt,[M,1]);
    idxalt=(1:M)'+M*(d2_maxalt_lin-1);
    Policyalt(2,:,:,:,jj)=reshape(mid_ford2alt(idxalt), [1,N_a,N_bothz,N_e]);
    Policyalt(3,:,:,:,jj)=reshape(L2a2_ford2alt(idxalt),[1,N_a,N_bothz,N_e]);
    Policyalt(4,:,:,:,jj)=reshape(L2a1_ford2alt(idxalt),[1,N_a,N_bothz,N_e]);
