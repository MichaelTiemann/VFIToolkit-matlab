function [Vhat, Policy, Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_DC1_GI1_e_raw(n_d1,n_d2,n_a,n_z,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated QH + SemiExo + DC + GI raw, with d1, z, e. Output: (Vunderbar, Policy, Vhat) -- matches plain QH SemiExo S raw convention.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2 % parallel semiz, outer z, inner e
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                [Vtempii,maxindexL2]=max(ReturnMatrix_ii,[],1);
                Vhat(:,semizblock,e_c,N_j)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d)+1;
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            EVfine=reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_bothz,N_e]);
            ...
            linidx=reshape(maxindex,[1,N_a*N_bothz*N_e])+N_d1*n2long*(0:N_a*N_bothz*N_e-1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                linidx=reshape(maxindex,[1,N_a*N_bothz])+N_d1*n2long*(0:N_a*N_bothz-1);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    EVfine=reshape(EVinterp_d2z(aprimez),[N_d1*n2long,N_a,N_semiz]);
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*EVfine;
                    [Vtemp,maxindex]=max(entireRHS_L2,[],1);

                    Vhat_ford2_jj(:,semizblock,e_c,d2_c)=shiftdim(Vtemp,1);
                    ...
                    Vunderbar_ford2_jj(:,semizblock,e_c,d2_c)=Vhat_ford2_jj(:,semizblock,e_c,d2_c)+(beta-beta0beta)*EV_at_policy;
                end
            end
        end
    elseif vfoptions.lowmemory==3 % joint bothz, inner e
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    EVfine=reshape(EVinterp_d2z(aprimeindexes),[N_d1*n2long,N_a]);
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*EVfine;
                    [Vtemp,maxindex]=max(entireRHS_L2,[],1);

                    Vhat_ford2_jj(:,z_c,e_c,d2_c)=shiftdim(Vtemp,1);
                    ...
                    linidx=reshape(maxindex,[1,N_a])+N_d1*n2long*(0:N_a-1);
                    EV_at_policy=reshape(EVfine(linidx),[N_a,1]);
                    Vunderbar_ford2_jj(:,z_c,e_c,d2_c)=Vhat_ford2_jj(:,z_c,e_c,d2_c)+(beta-beta0beta)*EV_at_policy;
                end
            end
        end
    end
    ...
    Vunderbar(:,:,:,N_j)=reshape(Vunderbar_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex-1)),[N_a,N_semiz*N_z,N_e]);
...
for reverse_j=1:N_j-1
    ...
    EV=sum(Vunderbar(:,:,:,jj+1).*pi_e_J(1,1,:,jj+1),3);
    EV=reshape(EV,[N_a,N_semiz*N_z]);
    ...
    maxindex=reshape(maxindex,[N_a*N_semiz*N_z*N_e,1]);
    Vunderbar(:,:,:,jj)=reshape(Vunderbar_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex-1)),[N_a,N_semiz*N_z,N_e]);
    d1aprimeL2_ind=reshape(Policy_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex-1)),[1,N_a,N_semiz*N_z*N_e]);
