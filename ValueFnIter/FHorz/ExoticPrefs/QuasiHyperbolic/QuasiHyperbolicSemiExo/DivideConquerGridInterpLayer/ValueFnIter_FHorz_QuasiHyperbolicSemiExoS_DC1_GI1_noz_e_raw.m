function [Vhat, Policy, Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_DC1_GI1_noz_e_raw(n_d1,n_d2,n_a,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated QH + SemiExo + DC + GI raw, with d1, no z, with e. Output: (Vunderbar, Policy, Vhat) -- matches plain QH SemiExo S raw convention.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            linidx=reshape(maxindex,[1,N_a*N_semiz*N_e])+N_d1*n2long*(0:N_a*N_semiz*N_e-1);
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    aprimez=aprimeindexes;
                    EVfine=reshape(EVinterp_d2semiz(aprimez),[N_d1*n2long,N_a,1]);
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*EVfine;
                    [Vtemp,maxindex]=max(entireRHS_L2,[],1);
