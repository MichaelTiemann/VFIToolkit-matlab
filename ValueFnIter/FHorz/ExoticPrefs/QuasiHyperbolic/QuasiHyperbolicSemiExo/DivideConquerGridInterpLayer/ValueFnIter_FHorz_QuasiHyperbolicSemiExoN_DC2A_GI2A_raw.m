function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC2A_GI2A_raw(n_d1, n_d2, n_a, n_z, n_semiz, N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
      ...
      if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, n_bothz, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3, 0);
        ...
        midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
        a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, n_bothz, d12c_gridvals, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2, 0);
        ...
        mid_ford2alt(:,:,d2_c)=midpoints_jj(shiftdim(maxindexL2d1,1)+N_d1*(shiftdim(maxindexL2a2,1)-1)+N_d1*N_a2*a12ind+N_d1*N_a2*N_a*bothzind);
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap_V(ii));
                    a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_valblock, ReturnFnParamsVec, 3, 0);
            ...
            midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
            a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, z_valblock, ReturnFnParamsVec, 2, 0);
            ...
            d1_ford2alt(:,semizblock,d2_c)=shiftdim(maxindexL2d1,1);
            mid_ford2alt(:,semizblock,d2_c)=midpoints_jj(shiftdim(maxindexL2d1,1)+N_d1*(shiftdim(maxindexL2a2,1)-1)+N_d1*N_a2*a12ind+N_d1*N_a2*N_a*semizind);
            ...
            inLowerStrict = (a1f >= 2)         & (a1f <= n2short+1);
            inUpperStrict = (a1f >= n2short+3) & (a1f <= n2long-1);
            L2flag_ford2alt(:,semizblock,d2_c) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
        ...
        for z_c=1:N_bothz
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:),N_a1-maxgap_V(ii));
                    a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, special_n_bothz, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_val, ReturnFnParamsVec, 3, 0);
            ...
            midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
            a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, special_n_bothz, d12c_gridvals, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, z_val, ReturnFnParamsVec, 2, 0);
            ...
            d1_ford2alt(:,z_c,d2_c)=shiftdim(maxindexL2d1,1);
            mid_ford2alt(:,z_c,d2_c)=midpoints_jj(shiftdim(maxindexL2d1,1)+N_d1*(shiftdim(maxindexL2a2,1)-1)+N_d1*N_a2*a12ind);
            ...
            inLowerStrict = (a1f >= 2)         & (a1f <= n2short+1);
            inUpperStrict = (a1f >= n2short+3) & (a1f <= n2long-1);
            L2flag_ford2alt(:,z_c,d2_c) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
      ...
      if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap_V(ii));
                a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, n_bothz, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 3, 0);
                aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*bothzBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[N_d1,(maxgap_V(ii)+1),N_a2,1,N_a2,N_bothz]));
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoints_jj(:,1,:,curraindex,:,:)=maxindex+(loweredge-1);
        ...
        midpoints_jj=max(min(midpoints_jj,n_a1-1),2);
        a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, n_bothz, d12c_gridvals, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 2, 0);
        aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*bothzBind;
        entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEVinterp(aprime),[N_d1*n2long*N_a2,N_a,N_bothz]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        maxindexL2d1=rem(maxindexL2-1,N_d1)+1;
