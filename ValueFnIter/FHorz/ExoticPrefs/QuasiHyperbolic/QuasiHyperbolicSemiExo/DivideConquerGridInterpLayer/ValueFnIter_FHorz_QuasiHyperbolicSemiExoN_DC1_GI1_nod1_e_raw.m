function [Vtilde, Policy, Valt, Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC1_GI1_nod1_e_raw(n_d2,n_a,n_z,n_semiz, n_e,N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2 % parallel semiz, outer z, inner e
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, [n_semiz,special_n_z], special_n_e, d2_gridvals, a_grid, a_grid(level1ii), z_valblock, e_val, ReturnFnParamsVec,1);
                [~,maxindex1]=max(ReturnMatrix_ii,[],2);
                midpoints_Nj(:,1,level1ii,:)=maxindex1;
                maxgap=squeeze(max(max(maxindex1(:,1,2:end,:)-maxindex1(:,1,1:end-1,:),[],4),[],1));
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii));
                        aprimeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, [n_semiz,special_n_z], special_n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, e_val, ReturnFnParamsVec,3);
                        [~,maxindex]=max(ReturnMatrix_ii,[],2);
                        midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                ...
                Policy(1,:,semizblock,e_c,N_j)=d_ind;
                Policy(2,:,semizblock,e_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1);
                Policy(3,:,semizblock,e_c,N_j)=shiftdim(ceil(maxindexL2/N_d2),-1);
    ...
    elseif vfoptions.lowmemory==3 % joint bothz, inner e
        ...
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, special_n_bothz, special_n_e, d2_gridvals, a_grid, a_grid(level1ii), z_val, e_val, ReturnFnParamsVec,1);
                [~,maxindex1]=max(ReturnMatrix_ii,[],2);
                midpoints_Nj(:,1,level1ii)=maxindex1;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii),n_a-maxgap(ii));
                        aprimeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, special_n_bothz, special_n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, ReturnFnParamsVec,3);
                        [~,maxindex]=max(ReturnMatrix_ii,[],2);
                        midpoints_Nj(:,1,curraindex)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii);
                ...
                Policy(2,:,z_c,e_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1);
                Policy(3,:,z_c,e_c,N_j)=shiftdim(ceil(maxindexL2/N_d2),-1);
    ...
    elseif vfoptions.lowmemory==2 % parallel semiz, outer z, inner e
        midpoints_jj_ze=zeros(1,N_a,N_semiz,'gpuArray');
        for d2_c=1:N_d2
            d2_val=d2_gridvals(d2_c,:);
            pi_bothz=kron(pi_z_J(:,:,N_j), pi_semiz_J(:,:,d2_c,N_j));
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a_grid, a_grid(level1ii), z_valblock, e_val, ReturnFnParamsVec,4);

                    %% Valt (beta)
                    entireRHS_d2ii_V=ReturnMatrix_d2ii+beta*EV_d2z;
                    [~,maxindex1]=max(entireRHS_d2ii_V,[],1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_V(ii)>0
                            loweredge=min(maxindex1(1,ii,:),n_a-maxgap_V(ii));
                            aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, e_val, ReturnFnParamsVec,5);
                            aprimez=aprimeindexes+N_a*semizind;
                            entireRHS_ii=ReturnMatrix_ii+beta*reshape(EV_d2z(aprimez),[(maxgap_V(ii)+1),1,N_semiz]);
                            [~,maxindex]=max(entireRHS_ii,[],1);
                            midpoints_jj_ze(1,curraindex,:)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(1,ii,:);
                            midpoints_jj_ze(1,curraindex,:)=repelem(loweredge,1,length(curraindex),1);
                        end
                    end
                    ...
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, z_valblock, e_val, ReturnFnParamsVec,5);
                    aprimez=aprimeindexes+n2aprime*semizind;
                    entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2z(aprimez),[n2long,N_a,N_semiz]);
                    [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
                    Valt_ford2_jj(:,semizblock,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_V_ford2_jj(:,semizblock,e_c,d2_c)=shiftdim(maxindexL2alt,1);
                    midpointV_ford2_jj(:,semizblock,e_c,d2_c)=squeeze(midpoints_jj_ze);
                    isInfLoweralt    = (ReturnMatrix_L2(1,     :,:) == -Inf);
                    isInfUpperalt    = (ReturnMatrix_L2(n2long,:,:) == -Inf);
                    inLowerStrictalt = (maxindexL2alt >= 2)         & (maxindexL2alt <= n2short+1);
                    inUpperStrictalt = (maxindexL2alt >= n2short+3) & (maxindexL2alt <= n2long-1);
                    ...
                    %% Vtilde (beta0*beta)
                    entireRHS_d2ii_T=ReturnMatrix_d2ii+beta0beta*EV_d2z;
                    [~,maxindex1]=max(entireRHS_d2ii_T,[],1);
                    midpoints_jj_ze(1,level1ii,:)=maxindex1;
                    maxgap=squeeze(max(maxindex1(1,2:end,:)-maxindex1(1,1:end-1,:),[],3));
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, e_val, ReturnFnParamsVec,5);
                            aprimez=aprimeindexes+N_a*semizind;
                            entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EV_d2z(aprimez),[(maxgap(ii)+1),1,N_semiz]);
                            [~,maxindex]=max(entireRHS_ii,[],1);
                            midpoints_jj_ze(1,curraindex,:)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(1,ii,:);
                    ...
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, z_valblock, e_val, ReturnFnParamsVec,5);
                    aprimez=aprimeindexes+n2aprime*semizind;
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2z(aprimez),[n2long,N_a,N_semiz]);
                    [Vtemp,maxindex]=max(entireRHS_L2,[],1);

                    V_ford2_jj(:,semizblock,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_ford2_jj(:,semizblock,e_c,d2_c)=shiftdim(maxindex,1);

                    midpoint_ford2_jj(:,semizblock,e_c,d2_c)=squeeze(midpoints_jj_ze);
    ...
    elseif vfoptions.lowmemory==3 % joint bothz, inner e
        midpoints_jj_ze=zeros(1,N_a,'gpuArray');
        for d2_c=1:N_d2
            d2_val=d2_gridvals(d2_c,:);
            pi_bothz=kron(pi_z_J(:,:,N_j), pi_semiz_J(:,:,d2_c,N_j));
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, a_grid, a_grid(level1ii), z_val, e_val, ReturnFnParamsVec,4);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_V(ii)>0
                            loweredge=min(maxindex1(1,ii,:),n_a-maxgap_V(ii));
                            aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, ReturnFnParamsVec,5);
                            entireRHS_ii=ReturnMatrix_ii+beta*reshape(EV_d2z(aprimeindexes),[(maxgap_V(ii)+1),1]);
                            [~,maxindex]=max(entireRHS_ii,[],1);
                            midpoints_jj_ze(1,curraindex)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(1,ii,:);
                    ...
                    midpoints_jj_ze=max(min(midpoints_jj_ze,n_a-1),2);
                    aprimeindexes=(midpoints_jj_ze+(midpoints_jj_ze-1)*n2short)+(-n2short-1:1:1+n2short)';
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, z_val, e_val, ReturnFnParamsVec,5);
                    entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2z(aprimeindexes),[n2long,N_a]);
                    [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
                    Valt_ford2_jj(:,z_c,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_V_ford2_jj(:,z_c,e_c,d2_c)=shiftdim(maxindexL2alt,1);
                    midpointV_ford2_jj(:,z_c,e_c,d2_c)=squeeze(midpoints_jj_ze);
                    isInfLoweralt    = (ReturnMatrix_L2(1,     :) == -Inf);
                    isInfUpperalt    = (ReturnMatrix_L2(n2long,:) == -Inf);
                    inLowerStrictalt = (maxindexL2alt >= 2)         & (maxindexL2alt <= n2short+1);
                    inUpperStrictalt = (maxindexL2alt >= n2short+3) & (maxindexL2alt <= n2long-1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            aprimeindexes=loweredge+(0:1:maxgap(ii))';
                            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, ReturnFnParamsVec,5);
                            entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EV_d2z(aprimeindexes),[(maxgap(ii)+1),1]);
                            [~,maxindex]=max(entireRHS_ii,[],1);
                            midpoints_jj_ze(1,curraindex)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(1,ii,:);
                    ...
                    aprimeindexes=(midpoints_jj_ze+(midpoints_jj_ze-1)*n2short)+(-n2short-1:1:1+n2short)';
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, z_val, e_val, ReturnFnParamsVec,5);
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2z(aprimeindexes),[n2long,N_a]);
                    [Vtemp,maxindex]=max(entireRHS_L2,[],1);

                    V_ford2_jj(:,z_c,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_ford2_jj(:,z_c,e_c,d2_c)=shiftdim(maxindex,1);

                    midpoint_ford2_jj(:,z_c,e_c,d2_c)=squeeze(midpoints_jj_ze);
