function [Vtilde, Policy, Valt, Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC1_GI1_noz_raw(n_d1,n_d2,n_a,n_semiz,N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% Naive QH + SemiExo + DC + GI raw, with d1, no z, no e. Output: (Vtilde=Vtilde, Policy, Valt=Valt).
...
Policyalt=zeros(4,N_a,N_semiz,N_j,vfoption.'gpuArray'); % exponential discounter optimal [d1; d2; midpoint; aprimeL2ind]
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for d2_c=1:N_d2
        ...
        ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1(ReturnFn, special_n_d, n_semiz, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
    ...
    Policyalt(3,:,:,N_j)=reshape(midpointV_ford2_jj((1:1:N_a*N_semiz)'+(N_a*N_semiz)*(maxindexalt_lin-1)),[1,N_a,N_semiz]);
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
        ...
        midpoints_jj=max(min(midpoints_jj,n_a-1),2);
        aprimeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1(ReturnFn, special_n_d, n_semiz, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
        aprimez=aprimeindexes+n2aprime*semizind2;
        entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_semiz]);
        [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
        Valt_ford2_jj(:,:,d2_c)=shiftdim(Vtemp,1);
    ...
    Policyalt(3,:,:,jj)=reshape(midpointV_ford2_jj((1:1:N_a*N_semiz)'+(N_a*N_semiz)*(maxindexalt_lin-1)),[1,N_a,N_semiz]);
