function [Vtilde, Policy, Valt, Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC1_GI1_nod1_noz_e_raw(n_d2,n_a,n_semiz, n_e,N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% Naive QH + SemiExo + DC + GI raw, no d1, no z, with e. Output: (Vtilde=Vtilde, Policy, Valt=Valt).
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, n_semiz, n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, n_semiz, special_n_e, d2_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1);
            [~,maxindex1]=max(ReturnMatrix_ii,[],2);
            midpoints_Nj(:,1,level1ii,:)=maxindex1;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii));
                    aprimeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, n_semiz, special_n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3);
                    [~,maxindex]=max(ReturnMatrix_ii,[],2);
                    midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
    ...
    elseif vfoptions.lowmemory==2
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, special_n_semiz, special_n_e, d2_gridvals, a_grid, a_grid(level1ii), semiz_val, e_val, ReturnFnParamsVec,1);
                [~,maxindex1]=max(ReturnMatrix_ii,[],2);
                midpoints_Nj(:,1,level1ii)=maxindex1;
                maxgap=squeeze(max(max(maxindex1(:,1,2:end,:)-maxindex1(:,1,1:end-1,:),[],4),[],1));
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii));
                        aprimeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, special_n_semiz, special_n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, ReturnFnParamsVec,3);
                        [~,maxindex]=max(ReturnMatrix_ii,[],2);
                        midpoints_Nj(:,1,curraindex)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                ...
                Policy(2,:,semiz_c,e_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1);
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,5);
            aprimez=aprimeindexes+n2aprime*semizind;
            entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2(aprimez),[n2long,N_a,N_semiz,N_e]);
            ...
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,5);
            aprimez=aprimeindexes+n2aprime*semizind;
            entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2(aprimez),[n2long,N_a,N_semiz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        midpoints_jj_e=zeros(1,N_a,N_semiz,'gpuArray');
        for d2_c=1:N_d2
            d2_val=d2_gridvals(d2_c,:);
            pi_bothz=pi_semiz_J(:,:,d2_c,N_j);
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,5);
                aprimez=aprimeindexes+n2aprime*semizind;
    ...
    elseif vfoptions.lowmemory==2
        midpoints_jj_se=zeros(1,N_a,'gpuArray');
        for d2_c=1:N_d2
            d2_val=d2_gridvals(d2_c,:);
            pi_bothz=pi_semiz_J(:,:,d2_c,N_j);
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    %% Valt (beta)
                    entireRHS_d2ii_V=ReturnMatrix_d2ii+beta*EV_d2semiz;
                    [~,maxindex1]=max(entireRHS_d2ii_V,[],1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_V(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_ii+beta*reshape(EV_d2semiz(aprimeindexes),[(maxgap_V(ii)+1),1]);
                            [~,maxindex]=max(entireRHS_ii,[],1);
                            midpoints_jj_se(1,curraindex)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(1,ii,:);
                            midpoints_jj_se(1,curraindex)=repelem(loweredge,1,length(curraindex),1);
                        end
                    end
                    ...
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_val, e_val, ReturnFnParamsVec,5);
                    entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2semiz(aprimeindexes),[n2long,N_a]);
                    [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
                    Valt_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_V_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(maxindexL2alt,1);
                    midpointV_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(midpoints_jj_se,1);
                    isInfLoweralt    = (ReturnMatrix_L2(1,     :) == -Inf);
                    isInfUpperalt    = (ReturnMatrix_L2(n2long,:) == -Inf);
                    inLowerStrictalt = (maxindexL2alt >= 2)         & (maxindexL2alt <= n2short+1);
                    inUpperStrictalt = (maxindexL2alt >= n2short+3) & (maxindexL2alt <= n2long-1);
                    PolicyL2flagV_ford2_jj(:,semiz_c,e_c,d2_c) = shiftdim(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt),1);

                    %% Vtilde (beta0*beta)
                    entireRHS_d2ii_T=ReturnMatrix_d2ii+beta0beta*EV_d2semiz;
                    [~,maxindex1]=max(entireRHS_d2ii_T,[],1);
                    midpoints_jj_se(1,level1ii)=maxindex1;
                    maxgap=squeeze(max(maxindex1(1,2:end,:)-maxindex1(1,1:end-1,:),[],3));
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            aprimeindexes=loweredge+(0:1:maxgap(ii))';
                            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_semiz, special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, ReturnFnParamsVec,5);
                            entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EV_d2semiz(aprimeindexes),[(maxgap(ii)+1),1]);
                            [~,maxindex]=max(entireRHS_ii,[],1);
                            midpoints_jj_se(1,curraindex)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(1,ii,:);
                    ...
                    aprimeindexes=(midpoints_jj_se+(midpoints_jj_se-1)*n2short)+(-n2short-1:1:1+n2short)';
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_val, e_val, ReturnFnParamsVec,5);
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2semiz(aprimeindexes),[n2long,N_a]);
                    [Vtemp,maxindex]=max(entireRHS_L2,[],1);

                    V_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(maxindex,1);

                    midpoint_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(midpoints_jj_se,1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            midpoints_jj=max(min(midpoints_jj,n_a-1),2);
            aprimeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,5);
            aprimez=aprimeindexes+n2aprime*semizind;
            entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2(aprimez),[n2long,N_a,N_semiz,N_e]);
            [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
            Valt_ford2_jj(:,:,:,d2_c)=shiftdim(Vtemp,1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                midpoints_jj_e=max(min(midpoints_jj_e,n_a-1),2);
                aprimeindexes=(midpoints_jj_e+(midpoints_jj_e-1)*n2short)+(-n2short-1:1:1+n2short)';
                ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,5);
                aprimez=aprimeindexes+n2aprime*semizind;
                entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2(aprimez),[n2long,N_a,N_semiz]);
                [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
                Valt_ford2_jj(:,:,e_c,d2_c)=shiftdim(Vtemp,1);
