function [Vtilde, Policy, Valt, Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC1_GI1_noz_e_raw(n_d1,n_d2,n_a,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% Naive QH + SemiExo + DC + GI raw, with d1, no z, with e. Output: (Vtilde=Vtilde, Policy, Valt=Valt).
...
Valt=zeros(N_a,N_semiz,N_e,N_j,vfoptions.precision,'gpuArray');
Vtilde=zeros(N_a,N_semiz,N_e,N_j,vfoptions.precision,'gpuArray');
Policy=zeros(4,N_a,N_semiz,N_e,N_j,'gpuArray');
PolicyL2flag=2*ones(1,N_a,N_semiz,N_e,N_j,'gpuArray');
Policyalt=zeros(4,N_a,N_semiz,N_e,N_j,'gpuArray'); % exponential discounter optimal [d1; d2; midpoint; aprimeL2ind]
...
Valt_ford2_jj=zeros(N_a,N_semiz,N_e,N_d2,vfoptions.precision,'gpuArray');
...
PolicyL2flagV_ford2_jj=2*ones(N_a,N_semiz,N_e,N_d2,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, special_n_semiz, special_n_e, d_gridvals, a_grid, a_grid(level1ii), semiz_val, e_val, ReturnFnParamsVec,1);
                [~,maxindex1]=max(ReturnMatrix_ii,[],2);
                midpoints_Nj(:,1,level1ii,:)=maxindex1;
                maxgap=squeeze(max(max(maxindex1(:,1,2:end,:)-maxindex1(:,1,1:end-1,:),[],4),[],1));
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii));
                        aprimeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, special_n_semiz, special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, ReturnFnParamsVec,3);
                        [~,maxindex]=max(ReturnMatrix_ii,[],2);
                        midpoints_Nj(:,1,curraindex,:)=shiftdim(maxindex+(loweredge-1),1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                        midpoints_Nj(:,1,curraindex,:)=repelem(loweredge,1,1,length(curraindex),1,1);
                    end
                ...
                [Vtempii,maxindexL2]=max(ReturnMatrix_ii,[],1);
                Valt(:,semiz_c,e_c,N_j)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d)+1;
                ...
                Policy(3,:,semiz_c,e_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1);
                Policy(4,:,semiz_c,e_c,N_j)=shiftdim(ceil(maxindexL2/N_d),-1);
    ...
    EV=sum(reshape(vfoptions.V_Jplus1,[N_a,N_semiz,N_e]).*pi_e_J(1,1,:,N_j+1),3);
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_semiz,N_e]);
            ...
            allindalt=d1_indalt+N_d1*aind+N_d1*N_a*semizind+N_d1*N_a*N_semiz*eind;
            midpointV_ford2_jj(:,:,:,d2_c)=squeeze(midpoints_jj(allindalt));
            L2offsetalt = ceil(maxindexL2alt/N_d1);
            linidx_loweralt = d1_indalt                  + N_d1*n2long*aind + N_d1*n2long*N_a*semizind + N_d1*n2long*N_a*N_semiz*eind;
            linidx_upperalt = d1_indalt + N_d1*(n2long-1) + N_d1*n2long*aind + N_d1*n2long*N_a*semizind + N_d1*n2long*N_a*N_semiz*eind;
            ...
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, n_semiz, n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
            aprimez=aprimeindexes+n2aprime*semizind2;
            entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_semiz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                aprimez=aprimeindexes+n2aprime*semizind2;
                entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_semiz]);
                [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
                ...
                ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2);
                aprimez=aprimeindexes+n2aprime*semizind2;
                entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_semiz]);
                [Vtemp,maxindex]=max(entireRHS_L2,[],1);
    ...
    elseif vfoptions.lowmemory==2
        midpoints_jj_se=zeros(N_d1,1,N_a,'gpuArray');
        for d2_c=1:N_d2
            d12c_gridvals=d12_gridvals(:,:,d2_c);
            pi_bothz=pi_semiz_J(:,:,d2_c,N_j);
            ...
            for semiz_c=1:N_semiz
                semiz_val=semiz_gridvals_J(semiz_c,:,N_j);
                EV_d2semiz=EV.*pi_bothz(semiz_c,:);
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid, a_grid(level1ii), semiz_val, e_val, ReturnFnParamsVec,1);

                    %% Valt (beta)
                    entireRHS_ii=ReturnMatrix_d2ii+beta*shiftdim(EV_d2semiz,-1);
                    [~,maxindex1]=max(entireRHS_ii,[],2);
                    midpoints_jj_se(:,1,level1ii)=maxindex1;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_V(ii)>0
                            loweredge=min(maxindex1(:,1,ii),n_a-maxgap_V(ii));
                            aprimeindexes=loweredge+(0:1:maxgap_V(ii));
                            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, ReturnFnParamsVec,6);
                            aprimez=aprimeindexes;
                            entireRHS_ii=ReturnMatrix_ii+beta*reshape(EV_d2semiz(aprimez),[N_d1,(maxgap_V(ii)+1),1,1]);
                            [~,maxindex]=max(entireRHS_ii,[],2);
                            midpoints_jj_se(:,1,curraindex)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,ii);
                            midpoints_jj_se(:,1,curraindex)=repelem(loweredge,1,1,length(curraindex));
                        end
                    end
                    midpoints_jj_se=max(min(midpoints_jj_se,n_a-1),2);
                    aprimeindexes=(midpoints_jj_se+(midpoints_jj_se-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_val, e_val, ReturnFnParamsVec,2);
                    aprimez=aprimeindexes;
                    entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2semiz(aprimez),[N_d1*n2long,N_a,1]);
                    [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
                    Valt_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_V_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(maxindexL2alt,1);
                    d1_indalt=rem(maxindexL2alt-1,N_d1)+1;
                    allindalt=d1_indalt+N_d1*aind;
                    midpointV_ford2_jj(:,semiz_c,e_c,d2_c)=squeeze(midpoints_jj_se(allindalt));
                    L2offsetalt = ceil(maxindexL2alt/N_d1);
                    ...
                    inLowerStrictalt = (L2offsetalt >= 2)         & (L2offsetalt <= n2short+1);
                    inUpperStrictalt = (L2offsetalt >= n2short+3) & (L2offsetalt <= n2long-1);
                    PolicyL2flagV_ford2_jj(:,semiz_c,e_c,d2_c) = squeeze(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt));

                    %% Vtilde (beta0*beta)
                    entireRHS_ii=ReturnMatrix_d2ii+beta0beta*shiftdim(EV_d2semiz,-1);
                    [~,maxindex1]=max(entireRHS_ii,[],2);
                    midpoints_jj_se(:,1,level1ii)=maxindex1;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, ReturnFnParamsVec,6);
                            aprimez=aprimeindexes;
                            entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EV_d2semiz(aprimez),[N_d1,(maxgap(ii)+1),1,1]);
                            [~,maxindex]=max(entireRHS_ii,[],2);
                            midpoints_jj_se(:,1,curraindex)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,ii);
                    ...
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_val, e_val, ReturnFnParamsVec,2);
                    aprimez=aprimeindexes;
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2semiz(aprimez),[N_d1*n2long,N_a,1]);
                    [Vtemp,maxindex]=max(entireRHS_L2,[],1);
                    ...
                    midpoint_ford2_jj(:,semiz_c,e_c,d2_c)=squeeze(midpoints_jj_se(allind));
    ...
    d1aprimeL2_ind_alt=reshape(Policy_V_ford2_jj((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_e]);
    Policyalt(1,:,:,:,N_j)=reshape(rem(d1aprimeL2_ind_alt-1,N_d1)+1,[N_a,N_semiz,N_e]);
    Policyalt(4,:,:,:,N_j)=reshape(ceil(d1aprimeL2_ind_alt/N_d1),[N_a,N_semiz,N_e]);
    Policyalt(3,:,:,:,N_j)=reshape(midpointV_ford2_jj((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz,N_e]);
    PolicyL2flagalt(1,:,:,:,N_j)=reshape(PolicyL2flagV_ford2_jj((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz,N_e]);
end
...
for reverse_j=1:N_j-1
    ...
    maxindexalt_lin=reshape(maxindexalt_d2,[N_a*N_semiz*N_e,1]);
    d1aprimeL2_ind_alt=reshape(Policy_V_ford2_jj((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_e]);
    Policyalt(1,:,:,:,jj)=reshape(rem(d1aprimeL2_ind_alt-1,N_d1)+1,[N_a,N_semiz,N_e]);
    Policyalt(4,:,:,:,jj)=reshape(ceil(d1aprimeL2_ind_alt/N_d1),[N_a,N_semiz,N_e]);
    Policyalt(3,:,:,:,jj)=reshape(midpointV_ford2_jj((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz,N_e]);
    PolicyL2flagalt(1,:,:,:,jj)=reshape(PolicyL2flagV_ford2_jj((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz,N_e]);
end
