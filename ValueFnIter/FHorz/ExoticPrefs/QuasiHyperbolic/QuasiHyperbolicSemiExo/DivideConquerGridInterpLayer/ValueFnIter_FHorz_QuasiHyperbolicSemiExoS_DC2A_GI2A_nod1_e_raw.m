function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_DC2A_GI2A_nod1_e_raw(n_d2, n_a, n_z, n_semiz, n_e, N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
  ...
  elseif vfoptions.lowmemory==3 % joint bothz / inner e
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        EVfine=reshape(EVundinterp(aprime),[n2long*N_a2,N_a,N_bothz,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*zBind;
                    entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprime,[(maxgap(ii)+1)*1,N_a2,1,N_a2,N_bothz]));
                    [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                    midpoints_jj(1,1,:,curraindex,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
                else
                    loweredge=maxindex1(1,1,:,ii,:,:);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_bothz, special_n_e, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 2, 0);
            aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
            EVfine=reshape(EVundinterp(aprime),[n2long*N_a2,N_a,N_bothz]);
            entireRHS_ii=ReturnMatrix_ii+beta0beta*EVfine;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            maxindexL2a1=rem(maxindexL2-1,n2long)+1;
            maxindexL2a2=ceil(maxindexL2/n2long);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii+DiscountedEV_d2z(reshape(aprime,[(maxgap(ii)+1)*1,N_a2,1,N_a2,N_semiz]));
                        [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                        midpoints_jj(1,1,:,curraindex,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
                    else
                        loweredge=maxindex1(1,1,:,ii,:,:);
                ...
                EVfine=reshape(EV_d2zundinterp(aprime),[n2long*N_a2,N_a,N_semiz]);
                entireRHS_ii=ReturnMatrix_ii+beta0beta*EVfine;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                maxindexL2a1=rem(maxindexL2-1,n2long)+1;
                maxindexL2a2=ceil(maxindexL2/n2long);
                ...
                Vunderbar_ford2(:,semizblock,e_c,d2_c)=V_ford2(:,semizblock,e_c,d2_c)+(beta-beta0beta)*EV_at_policy;
                mid_ford2(:,semizblock,e_c,d2_c)=midpoints_jj(maxindexL2a2+N_a2*a12ind+N_a2*N_a*semizind);
                L2a1_ford2(:,semizblock,e_c,d2_c)=shiftdim(maxindexL2a1,1);
                L2a2_ford2(:,semizblock,e_c,d2_c)=shiftdim(maxindexL2a2,1);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii+DiscountedEV_d2z(reshape(aprime,[(maxgap(ii)+1)*1,N_a2,1,N_a2]));
                        [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                        midpoints_jj(1,1,:,curraindex,:)=shiftdim(maxindex,-1)+(loweredge-1);
                    else
                        loweredge=maxindex1(1,1,:,ii,:);
                ...
                Vunderbar_ford2(:,z_c,e_c,d2_c)=V_ford2(:,z_c,e_c,d2_c)+(beta-beta0beta)*EV_at_policy;
                mid_ford2(:,z_c,e_c,d2_c)=midpoints_jj(maxindexL2a2+N_a2*a12ind);
                L2a1_ford2(:,z_c,e_c,d2_c)=shiftdim(maxindexL2a1,1);
                L2a2_ford2(:,z_c,e_c,d2_c)=shiftdim(maxindexL2a2,1);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*zBind;
                    entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2,N_bothz]));
                    [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                    midpoints_jj(1,1,:,curraindex,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
                else
                    loweredge=maxindex1(1,1,:,ii,:,:);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_valblock, e_val, ReturnFnParamsVec, 3, 1);
                        aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*semizBind;
                        entireRHS_ii=ReturnMatrix_ii+DiscountedEV_d2z(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2,N_semiz]));
                        [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                        midpoints_jj(1,1,:,curraindex,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
                    else
                        loweredge=maxindex1(1,1,:,ii,:,:);
