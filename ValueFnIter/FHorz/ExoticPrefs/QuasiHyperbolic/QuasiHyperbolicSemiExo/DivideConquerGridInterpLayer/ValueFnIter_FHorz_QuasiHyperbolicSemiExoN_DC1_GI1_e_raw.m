function [Vtilde, Policy, Valt, Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC1_GI1_e_raw(n_d1,n_d2,n_a,n_z,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% Naive QH + SemiExo + DC + GI raw, with d1, z, e. Output: (Vtilde=Vtilde, Policy, Valt=Valt).
...
Policyalt=zeros(4,N_a,N_semiz*N_z,N_e,N_j,'gpuArray'); % exponential discounter optimal [d1; d2; midpoint; aprimeL2ind]
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, n_bothz, n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii));
                    aprimeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, n_bothz, special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3);
                    [~,maxindex]=max(ReturnMatrix_ii,[],2);
                    midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii));
                        aprimeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, [n_semiz,special_n_z], special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, e_val, ReturnFnParamsVec,3);
                        [~,maxindex]=max(ReturnMatrix_ii,[],2);
                        midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                ...
                [Vtempii,maxindexL2]=max(ReturnMatrix_ii,[],1);
                Valt(:,semizblock,e_c,N_j)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d)+1;
                ...
                Policy(3,:,semizblock,e_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1);
                Policy(4,:,semizblock,e_c,N_j)=shiftdim(ceil(maxindexL2/N_d),-1);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii),n_a-maxgap(ii));
                        aprimeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, special_n_bothz, special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, ReturnFnParamsVec,3);
                        [~,maxindex]=max(ReturnMatrix_ii,[],2);
                        midpoints_Nj(:,1,curraindex)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii);
                ...
                Policy(3,:,z_c,e_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1);
                Policy(4,:,z_c,e_c,N_j)=shiftdim(ceil(maxindexL2/N_d),-1);
    ...
    EV=sum(reshape(vfoptions.V_Jplus1,[N_a,N_semiz*N_z,N_e]).*pi_e_J(1,1,:,N_j+1),3);
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+beta*reshape(EV_d2(aprimez),[N_d1,(maxgap_V(ii)+1),1,N_bothz,N_e]);
            ...
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, n_bothz, n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
            aprimez=aprimeindexes+n2aprime*bothzind2;
            entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_bothz,N_e]);
            ...
            allindalt=d1_indalt+N_d1*aind+N_d1*N_a*bothzind+N_d1*N_a*N_bothz*eind;
            midpointV_ford2_jj(:,:,:,d2_c)=squeeze(midpoints_jj(allindalt));
            L2offsetalt = ceil(maxindexL2alt/N_d1);
            linidx_loweralt = d1_indalt                  + N_d1*n2long*aind + N_d1*n2long*N_a*bothzind + N_d1*n2long*N_a*N_bothz*eind;
            linidx_upperalt = d1_indalt + N_d1*(n2long-1) + N_d1*n2long*aind + N_d1*n2long*N_a*bothzind + N_d1*n2long*N_a*N_bothz*eind;
            ...
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, n_bothz, n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
            aprimez=aprimeindexes+n2aprime*bothzind2;
            entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_bothz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        midpoints_jj_e=zeros(N_d1,1,N_a,N_semiz*N_z,'gpuArray');
        for d2_c=1:N_d2
            d12c_gridvals=d12_gridvals(:,:,d2_c);
            pi_bothz=kron(pi_z_J(:,:,N_j), pi_semiz_J(:,:,d2_c,N_j));
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2);
                aprimez=aprimeindexes+n2aprime*bothzind2;
                ...
                midpointV_ford2_jj(:,:,e_c,d2_c)=squeeze(midpoints_jj_e(allindalt));
                L2offsetalt = ceil(maxindexL2alt/N_d1);
    ...
    elseif vfoptions.lowmemory==2 % parallel semiz, outer z, inner e
        midpoints_jj_ze=zeros(N_d1,1,N_a,N_semiz,'gpuArray');
        for d2_c=1:N_d2
            d12c_gridvals=d12_gridvals(:,:,d2_c);
            pi_bothz=kron(pi_z_J(:,:,N_j), pi_semiz_J(:,:,d2_c,N_j));
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    %% Valt (beta)
                    entireRHS_ii=ReturnMatrix_d2ii+beta*shiftdim(EV_d2z,-1);
                    [~,maxindex1]=max(entireRHS_ii,[],2);
                    midpoints_jj_ze(:,1,level1ii,:)=maxindex1;
                    maxgap_V=squeeze(max(max(maxindex1(:,1,2:end,:)-maxindex1(:,1,1:end-1,:),[],4),[],1));
                    for ii=1:(vfoptions.level1n-1)
                        curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                        if maxgap_V(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_ii+beta*reshape(EV_d2z(aprimez),[N_d1,(maxgap_V(ii)+1),1,N_semiz]);
                            [~,maxindex]=max(entireRHS_ii,[],2);
                            midpoints_jj_ze(:,1,curraindex,:)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,ii,:);
                            midpoints_jj_ze(:,1,curraindex,:)=repelem(loweredge,1,1,length(curraindex),1);
                        end
                    end
                    midpoints_jj_ze=max(min(midpoints_jj_ze,n_a-1),2);
                    aprimeindexes=(midpoints_jj_ze+(midpoints_jj_ze-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, [n_semiz,special_n_z], special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, z_valblock, e_val, ReturnFnParamsVec,2);
                    aprimez=aprimeindexes+n2aprime*semizind2;
                    entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2z(aprimez),[N_d1*n2long,N_a,N_semiz]);
                    [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
                    Valt_ford2_jj(:,semizblock,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_V_ford2_jj(:,semizblock,e_c,d2_c)=shiftdim(maxindexL2alt,1);
                    d1_indalt=rem(maxindexL2alt-1,N_d1)+1;
                    allindalt=d1_indalt+N_d1*aind+N_d1*N_a*semizind;
                    midpointV_ford2_jj(:,semizblock,e_c,d2_c)=squeeze(midpoints_jj_ze(allindalt));
                    L2offsetalt = ceil(maxindexL2alt/N_d1);
                    linidx_loweralt = d1_indalt                  + N_d1*n2long*aind + N_d1*n2long*N_a*semizind;
                    linidx_upperalt = d1_indalt + N_d1*(n2long-1) + N_d1*n2long*aind + N_d1*n2long*N_a*semizind;
                    isInfLoweralt    = (ReturnMatrix_L2(linidx_loweralt) == -Inf);
                    isInfUpperalt    = (ReturnMatrix_L2(linidx_upperalt) == -Inf);
                    inLowerStrictalt = (L2offsetalt >= 2)         & (L2offsetalt <= n2short+1);
                    inUpperStrictalt = (L2offsetalt >= n2short+3) & (L2offsetalt <= n2long-1);
                    PolicyL2flagV_ford2_jj(:,semizblock,e_c,d2_c) = squeeze(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt));

                    %% Vtilde (beta0*beta)
                    entireRHS_ii=ReturnMatrix_d2ii+beta0beta*shiftdim(EV_d2z,-1);
                    [~,maxindex1]=max(entireRHS_ii,[],2);
                    midpoints_jj_ze(:,1,level1ii,:)=maxindex1;
                    maxgap=squeeze(max(max(maxindex1(:,1,2:end,:)-maxindex1(:,1,1:end-1,:),[],4),[],1));
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, [n_semiz,special_n_z], special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, e_val, ReturnFnParamsVec,6);
                            aprimez=aprimeindexes+N_a*semizind2;
                            entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EV_d2z(aprimez),[N_d1,(maxgap(ii)+1),1,N_semiz]);
                            [~,maxindex]=max(entireRHS_ii,[],2);
                            midpoints_jj_ze(:,1,curraindex,:)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,ii,:);
                    ...
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, [n_semiz,special_n_z], special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, z_valblock, e_val, ReturnFnParamsVec,2);
                    aprimez=aprimeindexes+n2aprime*semizind2;
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2z(aprimez),[N_d1*n2long,N_a,N_semiz]);
                    [Vtemp,maxindex]=max(entireRHS_L2,[],1);
                    ...
                    midpoint_ford2_jj(:,semizblock,e_c,d2_c)=squeeze(midpoints_jj_ze(allind));
    ...
    elseif vfoptions.lowmemory==3 % joint bothz, inner e
        midpoints_jj_ze=zeros(N_d1,1,N_a,'gpuArray');
        for d2_c=1:N_d2
            d12c_gridvals=d12_gridvals(:,:,d2_c);
            pi_bothz=kron(pi_z_J(:,:,N_j), pi_semiz_J(:,:,d2_c,N_j));
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_V(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_ii+beta*reshape(EV_d2z(aprimez),[N_d1,(maxgap_V(ii)+1)]);
                            [~,maxindex]=max(entireRHS_ii,[],2);
                            midpoints_jj_ze(:,1,curraindex)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,ii);
                            midpoints_jj_ze(:,1,curraindex)=repelem(loweredge,1,1,length(curraindex));
                        end
                    end
                    midpoints_jj_ze=max(min(midpoints_jj_ze,n_a-1),2);
                    aprimeindexes=(midpoints_jj_ze+(midpoints_jj_ze-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, z_val, e_val, ReturnFnParamsVec,2);
                    entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2z(aprimeindexes),[N_d1*n2long,N_a]);
                    [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
                    Valt_ford2_jj(:,z_c,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_V_ford2_jj(:,z_c,e_c,d2_c)=shiftdim(maxindexL2alt,1);
                    d1_indalt=rem(maxindexL2alt-1,N_d1)+1;
                    allindalt=d1_indalt+N_d1*aind;
                    midpointV_ford2_jj(:,z_c,e_c,d2_c)=squeeze(midpoints_jj_ze(allindalt));
                    L2offsetalt = ceil(maxindexL2alt/N_d1);
                    linidx_loweralt = d1_indalt                  + N_d1*n2long*aind;
                    linidx_upperalt = d1_indalt + N_d1*(n2long-1) + N_d1*n2long*aind;
                    isInfLoweralt    = (ReturnMatrix_L2(linidx_loweralt) == -Inf);
                    isInfUpperalt    = (ReturnMatrix_L2(linidx_upperalt) == -Inf);
                    inLowerStrictalt = (L2offsetalt >= 2)         & (L2offsetalt <= n2short+1);
                    inUpperStrictalt = (L2offsetalt >= n2short+3) & (L2offsetalt <= n2long-1);
                    PolicyL2flagV_ford2_jj(:,z_c,e_c,d2_c) = squeeze(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt));
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, ReturnFnParamsVec,6);
                            aprimez=aprimeindexes;
                            entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EV_d2z(aprimez),[N_d1,(maxgap(ii)+1)]);
                            [~,maxindex]=max(entireRHS_ii,[],2);
                            midpoints_jj_ze(:,1,curraindex)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,ii);
                    ...
                    aprimeindexes=(midpoints_jj_ze+(midpoints_jj_ze-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, z_val, e_val, ReturnFnParamsVec,2);
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*reshape(EVinterp_d2z(aprimeindexes),[N_d1*n2long,N_a]);
                    [Vtemp,maxindex]=max(entireRHS_L2,[],1);
                    ...
                    midpoint_ford2_jj(:,z_c,e_c,d2_c)=squeeze(midpoints_jj_ze(allind));
    ...
    d1aprimeL2_ind_alt=reshape(Policy_V_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z*N_e]);
    Policyalt(1,:,:,:,N_j)=reshape(rem(d1aprimeL2_ind_alt-1,N_d1)+1,[N_a,N_semiz*N_z,N_e]);
    Policyalt(4,:,:,:,N_j)=reshape(ceil(d1aprimeL2_ind_alt/N_d1),[N_a,N_semiz*N_z,N_e]);
    Policyalt(3,:,:,:,N_j)=reshape(midpointV_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            midpoints_jj=max(min(midpoints_jj,n_a-1),2);
            aprimeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, n_bothz, n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
            aprimez=aprimeindexes+n2aprime*bothzind2;
            entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_bothz,N_e]);
            [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
            Valt_ford2_jj(:,:,:,d2_c)=shiftdim(Vtemp,1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                midpoints_jj_e=max(min(midpoints_jj_e,n_a-1),2);
                aprimeindexes=(midpoints_jj_e+(midpoints_jj_e-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2);
                aprimez=aprimeindexes+n2aprime*bothzind2;
                entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_bothz]);
                [Vtemp,maxindexL2alt]=max(entireRHS_L2,[],1);
                Valt_ford2_jj(:,:,e_c,d2_c)=shiftdim(Vtemp,1);
    ...
    maxindexalt_lin=reshape(maxindexalt_d2,[N_a*N_semiz*N_z*N_e,1]);
    d1aprimeL2_ind_alt=reshape(Policy_V_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z*N_e]);
    Policyalt(1,:,:,:,jj)=reshape(rem(d1aprimeL2_ind_alt-1,N_d1)+1,[N_a,N_semiz*N_z,N_e]);
    Policyalt(4,:,:,:,jj)=reshape(ceil(d1aprimeL2_ind_alt/N_d1),[N_a,N_semiz*N_z,N_e]);
    Policyalt(3,:,:,:,jj)=reshape(midpointV_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
