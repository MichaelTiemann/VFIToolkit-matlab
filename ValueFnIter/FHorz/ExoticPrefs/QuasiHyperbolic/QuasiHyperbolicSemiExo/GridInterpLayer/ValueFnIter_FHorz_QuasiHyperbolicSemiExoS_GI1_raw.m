function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_GI1_raw(n_d1, n_d2, n_a, n_z, n_semiz, N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated QH + SemiExo + GI: with d1, with z.
% Vhat_search yields midpoint+L2 policy; Vunderbar = Vhat + (beta-beta0beta)*EV_at_policy.
% L2flag detection runs on Vhat's (beta0beta) Return matrix.
% Output order matches plain QuasiHyperbolicSemiExoS_raw: (Vunderbar, Policy, Vhat).
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        for z_c=1:N_z
            ...
            [Vtempii,maxindexL2]=max(ReturnMatrix_ii,[],1);
            Vhat(:,semizblock,N_j)=shiftdim(Vtempii,1);
            d_ind=rem(maxindexL2-1,N_d)+1;
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            % Vunderbar = Vhat + (beta-beta0beta)*EV_d2_at_policy
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                EVfine=reshape(EVinterp_z(aprimez),[N_d1*n2long,N_a,N_semiz]);
                entireRHS_ii=ReturnMatrix_d2ii+beta0beta*EVfine;
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                linidx_z=reshape(maxindex,[1,N_a])+N_d1*n2long*(0:N_a-1);
    ...
    %% Cross-d2: winner is Vhat; gather Vunderbar at the same winning d2.
    [Vhat_jj,maxindex]=max(Vhat_ford2_jj,[],3);
    Vhat(:,:,N_j)=Vhat_jj;
    Policy(2,:,:,N_j)=shiftdim(maxindex,-1);
    maxindex_lin=reshape(maxindex,[N_a*N_semiz*N_z,1]);
    d1aprimeL2_ind=reshape(Policy_ford2_jj((1:1:N_a*N_semiz*N_z)'+(N_a*N_semiz*N_z)*(maxindex_lin-1)),[1,N_a,N_semiz*N_z]);
