function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_GI2A_noz_e_raw(n_d1, n_d2, n_a, n_semiz, n_e, N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
  ...
  elseif vfoptions.lowmemory==2 % outer semiz / inner e
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        EVfine=reshape(EVinterp(aprime),[N_d1*n2long*N_a2,N_a,N_semiz,N_e]);
        ...
        linidx=reshape(maxindexL2,[1,N_a*N_semiz*N_e])+N_d1*n2long*N_a2*(0:N_a*N_semiz*N_e-1);
      ...
      elseif vfoptions.lowmemory==2 % outer semiz / inner e
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                % Vunderbar for this (d2,semiz,e)
                linidx=reshape(maxindexL2,[1,N_a])+N_d1*n2long*N_a2*(0:N_a-1);
                EV_at_policy=reshape(EVfine(linidx),[N_a,1]);
                Vunderbar_ford2(:,semiz_c,e_c,d2_c)=shiftdim(Vtempii,1)+(beta-beta0beta)*EV_at_policy;
            end
        end
      end
    end
