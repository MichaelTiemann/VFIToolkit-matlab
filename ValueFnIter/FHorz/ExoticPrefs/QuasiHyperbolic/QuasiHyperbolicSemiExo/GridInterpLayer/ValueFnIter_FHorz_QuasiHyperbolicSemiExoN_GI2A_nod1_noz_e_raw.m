function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_GI2A_nod1_noz_e_raw(n_d2, n_a, n_semiz, n_e, N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive QH + SemiExo + GI2A (grid interpolation layer on the first endogenous
% state only, a2 enumerated in full), no d1, no z, with e.
%
  ...
  elseif vfoptions.lowmemory==2 % outer semiz / inner e
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2, 0);
        aprimealt=a1primeindexesalt+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
        entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[n2long*N_a2,N_a,N_semiz,N_e]);
        ...
        % L2 flag (per d2): detect -Inf on the coarse a1 neighbour we'd put weight on (at chosen a2prime)
        linidx_loweralt  = 1      + n2long*(maxindexL2a2alt-1) + n2long*N_a2*a12ind + n2long*N_a2*N_a*semizind + n2long*N_a2*N_a*N_semiz*eind;
        linidx_upperalt  = n2long + n2long*(maxindexL2a2alt-1) + n2long*N_a2*a12ind + n2long*N_a2*N_a*semizind + n2long*N_a2*N_a*N_semiz*eind;
        ...
        mid_ford2alt(:,:,:,d2_c)=midpointalt(maxindexL2a2alt+N_a2*a12ind+N_a2*N_a*semizind+N_a2*N_a*N_semiz*eind);
        ...
        entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EVinterp(aprime),[n2long*N_a2,N_a,N_semiz,N_e]);
      ...
      elseif vfoptions.lowmemory==1 % loop e, vectorise semiz
        ...
        for e_c=1:N_e
            ...
            a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 2, 0);
            aprimealt=a1primeindexesalt+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
            ...
            inLowerStrictalt = (maxindexL2a1alt >= 2)         & (maxindexL2a1alt <= n2short+1);
            inUpperStrictalt = (maxindexL2a1alt >= n2short+3) & (maxindexL2a1alt <= n2long-1);
            flag_ford2alt(:,:,e_c,d2_c) = shiftdim(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt), 1);

            V_ford2alt(:,:,e_c,d2_c)=shiftdim(Vtempiialt,1);
            mid_ford2alt(:,:,e_c,d2_c)=midpointalt(maxindexL2a2alt+N_a2*a12ind+N_a2*N_a*semizind);
      ...
      elseif vfoptions.lowmemory==2 % outer semiz / inner e
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, special_n_semiz, special_n_e, d2_val, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_val, e_val, ReturnFnParamsVec, 2, 0);
                ...
                inLowerStrictalt = (maxindexL2a1alt >= 2)         & (maxindexL2a1alt <= n2short+1);
                inUpperStrictalt = (maxindexL2a1alt >= n2short+3) & (maxindexL2a1alt <= n2long-1);
                flag_ford2alt(:,semiz_c,e_c,d2_c) = shiftdim(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt), 1);

                V_ford2alt(:,semiz_c,e_c,d2_c)=shiftdim(Vtempiialt,1);
                mid_ford2alt(:,semiz_c,e_c,d2_c)=midpointalt(maxindexL2a2alt+N_a2*a12ind);
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, n_e, d2_val, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 2, 0);
        aprimealt=a1primeindexesalt+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
        entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[n2long*N_a2,N_a,N_semiz,N_e]);
        [Vtempiialt,maxindexL2alt]=max(entireRHS_iialt,[],1);
        maxindexL2a1alt=rem(maxindexL2alt-1,n2long)+1;
      ...
      elseif vfoptions.lowmemory==1 % loop e, vectorise semiz
        ...
        for e_c=1:N_e
            ...
            a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 2, 0);
            aprimealt=a1primeindexesalt+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
            entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[n2long*N_a2,N_a,N_semiz]);
            [Vtempiialt,maxindexL2alt]=max(entireRHS_iialt,[],1);
            maxindexL2a1alt=rem(maxindexL2alt-1,n2long)+1;
