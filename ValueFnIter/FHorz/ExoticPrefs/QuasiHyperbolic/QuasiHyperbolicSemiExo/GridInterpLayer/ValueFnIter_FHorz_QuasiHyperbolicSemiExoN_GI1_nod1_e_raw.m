function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_GI1_nod1_e_raw(n_d2, n_a, n_z, n_semiz, n_e, N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive QH + SemiExo + GI: no d1, with z, with e.
...
flagV_ford2_jj=2*ones(N_a,N_semiz*N_z,N_e,N_d2,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2 % outer z / inner e, vectorize semiz
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy(2,:,semizblock,e_c,N_j)=shiftdim(squeeze(midpoint(allind)),-1);
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            ReturnMatrix_d2iiV=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_bothz, n_e, d2_val, aprime_grid(aprimeindexesV), a_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,5);
            aprimezV=aprimeindexesV+n2aprime*bothzind;
            entireRHS_iiV=ReturnMatrix_d2iiV+beta*reshape(EVinterp(aprimezV),[n2long,N_a,N_bothz,N_e]);
            ...
            entireRHS_ii=ReturnMatrix_d2ii+beta0beta*reshape(EVinterp(aprimez),[n2long,N_a,N_bothz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                midpointV=max(min(maxindex,n_a-1),2);
                aprimeindexesV=(midpointV+(midpointV-1)*n2short)+(-n2short-1:1:1+n2short)';
                ReturnMatrix_d2iiV=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_bothz, special_n_e, d2_val, aprime_grid(aprimeindexesV), a_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,5);
                aprimezV=aprimeindexesV+n2aprime*bothzind;
                entireRHS_iiV=ReturnMatrix_d2iiV+beta*reshape(EVinterp(aprimezV),[n2long,N_a,N_bothz]);
                [Vtemp,maxindexL2alt]=max(entireRHS_iiV,[],1);
                ...
                aprimez=aprimeindexes+n2aprime*bothzind;
                entireRHS_ii=ReturnMatrix_d2ii+beta0beta*reshape(EVinterp(aprimez),[n2long,N_a,N_bothz]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    %% Valt (beta)
                    entireRHS=ReturnMatrix_d2ze+beta*EV_d2z;
                    [~,maxindex]=max(entireRHS,[],1);
                    midpointV=max(min(maxindex,n_a-1),2);
                    aprimeindexesV=(midpointV+(midpointV-1)*n2short)+(-n2short-1:1:1+n2short)';
                    ReturnMatrix_d2iiV=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, aprime_grid(aprimeindexesV), a_grid, z_valblock, e_val, ReturnFnParamsVec,5);
                    ...
                    inLowerStrictalt = (maxindexL2alt >= 2)         & (maxindexL2alt <= n2short+1);
                    inUpperStrictalt = (maxindexL2alt >= n2short+3) & (maxindexL2alt <= n2long-1);
                    flagV_ford2_jj(:,semizblock,e_c,d2_c) = shiftdim(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt), 1);
                    midpointV_ford2_jj(:,semizblock,e_c,d2_c)=squeeze(midpointV);

                    %% Vtilde (beta0beta)
                    entireRHS=ReturnMatrix_d2ze+beta0beta*EV_d2z;
                    [~,maxindex]=max(entireRHS,[],1);
                    midpoint=max(min(maxindex,n_a-1),2);
                    aprimeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short)';
                    ...
                    midpoint_ford2_jj(:,semizblock,e_c,d2_c)=squeeze(midpoint);
                end
            end
        end
    elseif vfoptions.lowmemory==3 % joint bothz, inner e
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    midpointV=max(min(maxindex,n_a-1),2);
                    aprimeindexesV=(midpointV+(midpointV-1)*n2short)+(-n2short-1:1:1+n2short)';
                    ReturnMatrix_d2iiV=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, aprime_grid(aprimeindexesV), a_grid, z_val, e_val, ReturnFnParamsVec,5);
                    entireRHS_iiV=ReturnMatrix_d2iiV+beta*reshape(EVinterp(aprimeindexesV),[n2long,N_a]);
                    [Vtemp,maxindexL2alt]=max(entireRHS_iiV,[],1);
                    ...
                    inLowerStrictalt = (maxindexL2alt >= 2)         & (maxindexL2alt <= n2short+1);
                    inUpperStrictalt = (maxindexL2alt >= n2short+3) & (maxindexL2alt <= n2long-1);
                    flagV_ford2_jj(:,z_c,e_c,d2_c) = shiftdim(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt), 1);
                    midpointV_ford2_jj(:,z_c,e_c,d2_c)=squeeze(midpointV);
                    ...
                    entireRHS_ii=ReturnMatrix_d2ii+beta0beta*reshape(EVinterp(aprimeindexes),[n2long,N_a]);
                    [Vtemp,maxindex]=max(entireRHS_ii,[],1);
                    ...
                    midpoint_ford2_jj(:,z_c,e_c,d2_c)=squeeze(midpoint);
                end
            end
        end
    end
    ...
    PolicyL2flagalt(1,:,:,:,N_j)=reshape(flagV_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            midpointV=max(min(maxindex,n_a-1),2);
            aprimeindexesV=(midpointV+(midpointV-1)*n2short)+(-n2short-1:1:1+n2short)';
            ReturnMatrix_d2iiV=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_bothz, n_e, d2_val, aprime_grid(aprimeindexesV), a_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,5);
            aprimezV=aprimeindexesV+n2aprime*bothzind;
            entireRHS_iiV=ReturnMatrix_d2iiV+beta*reshape(EVinterp(aprimezV),[n2long,N_a,N_bothz,N_e]);
            [Vtemp,maxindexL2alt]=max(entireRHS_iiV,[],1);
            V_ford2_jj(:,:,:,d2_c)=shiftdim(Vtemp,1);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                midpointV=max(min(maxindex,n_a-1),2);
                aprimeindexesV=(midpointV+(midpointV-1)*n2short)+(-n2short-1:1:1+n2short)';
                ReturnMatrix_d2iiV=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_bothz, special_n_e, d2_val, aprime_grid(aprimeindexesV), a_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,5);
                aprimezV=aprimeindexesV+n2aprime*bothzind;
                entireRHS_iiV=ReturnMatrix_d2iiV+beta*reshape(EVinterp(aprimezV),[n2long,N_a,N_bothz]);
                [Vtemp,maxindexL2alt]=max(entireRHS_iiV,[],1);
                V_ford2_jj(:,:,e_c,d2_c)=shiftdim(Vtemp,1);
    ...
    PolicyL2flagalt(1,:,:,:,jj)=reshape(flagV_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
    [Vtilde_jj,maxindex]=max(Valt_ford2_jj,[],4);
    Vtilde(:,:,:,jj)=Vtilde_jj;
