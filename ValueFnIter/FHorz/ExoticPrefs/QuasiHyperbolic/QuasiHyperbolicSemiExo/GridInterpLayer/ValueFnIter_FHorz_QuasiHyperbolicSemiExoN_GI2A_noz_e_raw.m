function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_GI2A_noz_e_raw(n_d1, n_d2, n_a, n_semiz, n_e, N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive QH + SemiExo + GI2A (grid interpolation layer on the first endogenous
% state only, a2 enumerated in full), with d1, no z, with e.
%
...
flag_ford2alt=2*ones(N_a,N_semiz,N_e,N_d2,'gpuArray');
  ...
  elseif vfoptions.lowmemory==2 % outer semiz / inner e
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_semiz, n_e, d12c_gridvals, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2, 0);
        aprimealt=a1primeindexesalt+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
        entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[N_d1*n2long*N_a2,N_a,N_semiz,N_e]);
        ...
        linidx_loweralt  = mL2d1alt                  + N_d1*n2long*(mL2a2alt-1) + N_d1*n2long*N_a2*a12ind + N_d1*n2long*N_a2*N_a*semizind + N_d1*n2long*N_a2*N_a*N_semiz*eind;
        linidx_upperalt  = mL2d1alt + N_d1*(n2long-1) + N_d1*n2long*(mL2a2alt-1) + N_d1*n2long*N_a2*a12ind + N_d1*n2long*N_a2*N_a*semizind + N_d1*n2long*N_a2*N_a*N_semiz*eind;
        ...
        mid_ford2alt(:,:,:,d2_c)=midpointalt(shiftdim(maxindexL2d1alt,1)+N_d1*(shiftdim(maxindexL2a2alt,1)-1)+N_d1*N_a2*a12ind+N_d1*N_a2*N_a*semizind+N_d1*N_a2*N_a*N_semiz*eind);
        ...
        entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EVinterp(aprime),[N_d1*n2long*N_a2,N_a,N_semiz,N_e]);
      ...
      elseif vfoptions.lowmemory==1 % loop e, vectorise semiz
        ...
        for e_c=1:N_e
            ...
            a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 2, 0);
            aprimealt=a1primeindexesalt+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
            ...
            mid_ford2alt(:,:,e_c,d2_c)=midpointalt(shiftdim(maxindexL2d1alt,1)+N_d1*(shiftdim(maxindexL2a2alt,1)-1)+N_d1*N_a2*a12ind+N_d1*N_a2*N_a*semizind);
      ...
      elseif vfoptions.lowmemory==2 % outer semiz / inner e
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_val, e_val, ReturnFnParamsVec, 2, 0);
                ...
                inLowerStrictalt = (mL2a1alt >= 2)         & (mL2a1alt <= n2short+1);
                inUpperStrictalt = (mL2a1alt >= n2short+3) & (mL2a1alt <= n2long-1);
                flag_ford2alt(:,semiz_c,e_c,d2_c) = 2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt);

                V_ford2alt(:,semiz_c,e_c,d2_c)=shiftdim(Vtempiialt,1);
                d1_ford2alt(:,semiz_c,e_c,d2_c)=shiftdim(maxindexL2d1alt,1);
                mid_ford2alt(:,semiz_c,e_c,d2_c)=midpointalt(shiftdim(maxindexL2d1alt,1)+N_d1*(shiftdim(maxindexL2a2alt,1)-1)+N_d1*N_a2*a12ind);
                L2a1_ford2alt(:,semiz_c,e_c,d2_c)=shiftdim(maxindexL2a1alt,1);
                L2a2_ford2alt(:,semiz_c,e_c,d2_c)=shiftdim(maxindexL2a2alt,1);
    ...
    PolicyL2flagalt(1,:,:,:,N_j)=reshape(flag_ford2alt(idxalt),[1,N_a,N_semiz,N_e]);
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_semiz, n_e, d12c_gridvals, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 2, 0);
        aprimealt=a1primeindexesalt+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
        entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[N_d1*n2long*N_a2,N_a,N_semiz,N_e]);
        [Vtempiialt,maxindexL2alt]=max(entireRHS_iialt,[],1);
        maxindexL2d1alt=rem(maxindexL2alt-1,N_d1)+1;
      ...
      elseif vfoptions.lowmemory==1 % loop e, vectorise semiz
        ...
        for e_c=1:N_e
            ...
            a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 2, 0);
            aprimealt=a1primeindexesalt+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
            entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[N_d1*n2long*N_a2,N_a,N_semiz]);
            [Vtempiialt,maxindexL2alt]=max(entireRHS_iialt,[],1);
            maxindexL2d1alt=rem(maxindexL2alt-1,N_d1)+1;
    ...
    PolicyL2flagalt(1,:,:,:,jj)=reshape(flag_ford2alt(idxalt),[1,N_a,N_semiz,N_e]);
