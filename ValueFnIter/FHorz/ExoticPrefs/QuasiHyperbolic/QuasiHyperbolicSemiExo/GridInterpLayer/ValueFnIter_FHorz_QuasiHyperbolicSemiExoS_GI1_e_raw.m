function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_GI1_e_raw(n_d1, n_d2, n_a, n_z, n_semiz, n_e, N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated QH + SemiExo + GI: with d1, with z, with e.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            EVfine=reshape(EVinterp(aprimez),[N_d1*n2long,N_a,N_bothz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                aprimez=aprimeindexes+n2aprime*bothzBind;
                EVfine=reshape(EVinterp(aprimez),[N_d1*n2long,N_a,N_bothz]);
                entireRHS_ii=ReturnMatrix_d2ii+beta0beta*EVfine;
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);
                ...
                linidx_e=reshape(maxindex,[1,N_a*N_bothz])+N_d1*n2long*(0:N_a*N_bothz-1);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    linidx_e=reshape(maxindex,[1,N_a*N_semiz])+N_d1*n2long*(0:N_a*N_semiz-1);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    EVfine=reshape(EVinterp(aprimeindexes),[N_d1*n2long,N_a]);
                    entireRHS_ii=ReturnMatrix_d2ii+beta0beta*EVfine;
                    [Vtemp,maxindex]=max(entireRHS_ii,[],1);
                    ...
                    linidx_e=reshape(maxindex,[1,N_a])+N_d1*n2long*(0:N_a-1);
    ...
    d1aprimeL2_ind=reshape(Policy_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex_lin-1)),[1,N_a,N_semiz*N_z*N_e]);
    ...
    Policy(3,:,:,:,N_j)=reshape(midpoint_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
...
for reverse_j=1:N_j-1
    ...
    Policy(3,:,:,:,jj)=reshape(midpoint_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
