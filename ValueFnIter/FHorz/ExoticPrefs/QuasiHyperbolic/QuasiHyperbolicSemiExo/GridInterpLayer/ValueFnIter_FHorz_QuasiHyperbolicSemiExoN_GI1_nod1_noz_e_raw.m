function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_GI1_nod1_noz_e_raw(n_d2, n_a, n_semiz, n_e, N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    %% Valt (beta)
                    entireRHS=ReturnMatrix_d2semize+beta*EV_d2semiz;
                    [~,maxindex]=max(entireRHS,[],1);
                    midpointV=max(min(maxindex,n_a-1),2);
                    aprimeindexesV=(midpointV+(midpointV-1)*n2short)+(-n2short-1:1:1+n2short)';
                    ReturnMatrix_d2semizeiiV=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexesV), a_grid, semiz_val, e_val, ReturnFnParamsVec,5);
                    entireRHS_iiV=ReturnMatrix_d2semizeiiV+beta*reshape(EVinterp_d2semiz(aprimeindexesV),[n2long,N_a]);
                    [Vtemp,maxindexL2alt]=max(entireRHS_iiV,[],1);
                    V_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(Vtemp,1);
                    Policy_V_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(maxindexL2alt,1);
                    isInfLoweralt    = (ReturnMatrix_d2semizeiiV(1,      :) == -Inf);
                    isInfUpperalt    = (ReturnMatrix_d2semizeiiV(n2long, :) == -Inf);
                    inLowerStrictalt = (maxindexL2alt >= 2)         & (maxindexL2alt <= n2short+1);
                    inUpperStrictalt = (maxindexL2alt >= n2short+3) & (maxindexL2alt <= n2long-1);
                    flagV_ford2_jj(:,semiz_c,e_c,d2_c) = shiftdim(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt), 1);
                    midpointV_ford2_jj(:,semiz_c,e_c,d2_c)=squeeze(midpointV);
                    ...
                    entireRHS_ii=ReturnMatrix_d2semizeii+beta0beta*reshape(EVinterp_d2semiz(aprimeindexes),[n2long,N_a]);
                    [Vtemp,maxindex]=max(entireRHS_ii,[],1);
