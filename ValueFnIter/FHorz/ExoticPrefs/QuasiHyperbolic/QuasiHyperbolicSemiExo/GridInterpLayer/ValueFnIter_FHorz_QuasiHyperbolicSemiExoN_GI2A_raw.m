function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_GI2A_raw(n_d1, n_d2, n_a, n_z, n_semiz, N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive QH + SemiExo + GI2A (grid interpolation layer on the first endogenous
% state only, a2 enumerated in full), with d1, with z, no e.
%
...
d1_ford2alt=zeros(N_a,N_bothz,N_d2,'gpuArray');
  ...
  elseif vfoptions.lowmemory==2 % joint loop over bothz
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, n_bothz, d12c_gridvals, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2, 0);
        ...
        mid_ford2alt(:,:,d2_c)=midpointalt(shiftdim(maxindexL2d1alt,1)+N_d1*(shiftdim(maxindexL2a2alt,1)-1)+N_d1*N_a2*a12ind+N_d1*N_a2*N_a*bothzind);
      ...
      elseif vfoptions.lowmemory==1 % split: outer z, vectorise semiz
        for z_c=1:N_z
            ...
            a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, z_valblock, ReturnFnParamsVec, 2, 0);
            ...
            inLowerStrictalt = (mL2a1alt >= 2)         & (mL2a1alt <= n2short+1);
            inUpperStrictalt = (mL2a1alt >= n2short+3) & (mL2a1alt <= n2long-1);
            flag_ford2alt(:,semizblock,d2_c) = 2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt);

            V_ford2alt(:,semizblock,d2_c)=shiftdim(Vtempiialt,1);
            d1_ford2alt(:,semizblock,d2_c)=shiftdim(maxindexL2d1alt,1);
            mid_ford2alt(:,semizblock,d2_c)=midpointalt(shiftdim(maxindexL2d1alt,1)+N_d1*(shiftdim(maxindexL2a2alt,1)-1)+N_d1*N_a2*a12ind+N_d1*N_a2*N_a*semizind);
      ...
      elseif vfoptions.lowmemory==2 % joint loop over bothz
        for z_c=1:N_bothz
            ...
            a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, special_n_bothz, d12c_gridvals, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, z_val, ReturnFnParamsVec, 2, 0);
            ...
            inLowerStrictalt = (mL2a1alt >= 2)         & (mL2a1alt <= n2short+1);
            inUpperStrictalt = (mL2a1alt >= n2short+3) & (mL2a1alt <= n2long-1);
            flag_ford2alt(:,z_c,d2_c) = 2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt);

            V_ford2alt(:,z_c,d2_c)=shiftdim(Vtempiialt,1);
            d1_ford2alt(:,z_c,d2_c)=shiftdim(maxindexL2d1alt,1);
            mid_ford2alt(:,z_c,d2_c)=midpointalt(shiftdim(maxindexL2d1alt,1)+N_d1*(shiftdim(maxindexL2a2alt,1)-1)+N_d1*N_a2*a12ind);
    ...
    Policyalt(1,:,:,N_j)=reshape(d1_ford2alt(idxalt), [1,N_a,N_bothz]);
    Policyalt(3,:,:,N_j)=reshape(mid_ford2alt(idxalt),[1,N_a,N_bothz]);
    Policyalt(4,:,:,N_j)=reshape(L2a2_ford2alt(idxalt),[1,N_a,N_bothz]);
    Policyalt(5,:,:,N_j)=reshape(L2a1_ford2alt(idxalt),[1,N_a,N_bothz]);
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, n_bothz, d12c_gridvals, a1prime_grid(a1primeindexesalt), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 2, 0);
        aprimealt=a1primeindexesalt+N_a1fine*a2ind+N_a1fine*N_a2*bothzBind;
        entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[N_d1*n2long*N_a2,N_a,N_bothz]);
        [Vtempiialt,maxindexL2alt]=max(entireRHS_iialt,[],1);
        maxindexL2d1alt=rem(maxindexL2alt-1,N_d1)+1;
    ...
    d2_maxalt_lin=reshape(d2_maxalt,[Malt,1]);
    idxalt=(1:Malt)'+Malt*(d2_maxalt_lin-1);
    Policyalt(1,:,:,jj)=reshape(d1_ford2alt(idxalt), [1,N_a,N_bothz]);
    Policyalt(3,:,:,jj)=reshape(mid_ford2alt(idxalt),[1,N_a,N_bothz]);
    Policyalt(4,:,:,jj)=reshape(L2a2_ford2alt(idxalt),[1,N_a,N_bothz]);
    Policyalt(5,:,:,jj)=reshape(L2a1_ford2alt(idxalt),[1,N_a,N_bothz]);
