function [Vhat, Policy, Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoS_nod1_noz_e_raw(n_d2, n_a, n_semiz, n_e, N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic with semi-exogenous shock and iid e shock, no d1, no z.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2 % outer loop semiz, inner loop e
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                d_ind=shiftdim(rem(maxindex-1,N_d)+1,-1);
                Policy(1,:,semiz_c,e_c,N_j)=shiftdim(d_ind,-1);
                Policy(2,:,semiz_c,e_c,N_j)=shiftdim(ceil(maxindex/N_d),-1);
            end
        end
    end
    Vunderbar=Vhat;
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            maxindexfull=maxindex+N_a*(0:1:N_a-1)+shiftdim(N_a*N_a*(0:1:N_semiz-1),-1)+shiftdim(N_a*N_a*N_semiz*(0:1:N_e-1),-2);
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_e=ReturnMatrix_d2semize+beta0beta*EV_d2semiz;
                    [Vtemp,maxindex]=max(entireRHS_e,[],1);
                    Vhat_ford2_jj(:,semiz_c,e_c,d2_c)=Vtemp;
                    Policy_ford2_jj(:,semiz_c,e_c,d2_c)=maxindex;

                    entireRHS_alt=ReturnMatrix_d2semize+beta*EV_d2semiz;
