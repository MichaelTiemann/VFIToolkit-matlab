function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicS_nod_e_raw(n_a,n_z,n_e,N_j, a_grid, z_gridvals_J,e_gridvals_J,pi_z_J,pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
Policy=zeros(N_a,N_z,N_e,N_j,vfoptions.indexT,'gpuArray'); % indexes the optimal choice for aprime, rest of dimensions a,z
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        % First Vhat
        entireRHS=ReturnMatrix+beta0beta*EV; %*repmat(EV,1,N_a,1,N_e);  % Use the today-to-tomorrow discount factor
        ...
        % Now Vstar
        entireRHS=ReturnMatrix+beta*EV; %*repmat(EV,1,N_a,1,N_e); % Use the two-future-periods discount factor
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            % and then we compute Vunderbar.
            % First Vhat
            entireRHS_e=ReturnMatrix_e+beta0beta*EV; %*EV.*ones(1,N_a,1);  % Use the today-to-tomorrow discount factor
            [Vtemp,maxindex]=max(entireRHS_e,[],1);
            Vhat(:,:,e_c,N_j)=shiftdim(Vtemp,1); % Note that this is Vhat when sophisticated
            Policy(:,:,e_c,N_j)=shiftdim(maxindex,1); % This is the policy from solving the problem of Vhat
            % Now Vstar
            entireRHS_e=ReturnMatrix_e+beta*EV; %*EV.*ones(1,N_a,1); % Use the two-future-periods discount factor
    ...
    elseif vfoptions.lowmemory==2
        for e_c=1:N_e
            ...
            for z_c=1:N_z
                ...
                % and then we compute Vunderbar.
                % First Vhat
                entireRHS_ez=ReturnMatrix_ze+beta0beta*EV_z; %*EV_z.*ones(1,N_a,1);  % Use the today-to-tomorrow discount factor
                [Vtemp,maxindex]=max(entireRHS_ez,[],1);
                Vhat(:,z_c,e_c,N_j)=Vtemp; % Note that this is Vhat when sophisticated
                Policy(:,z_c,e_c,N_j)=maxindex; % This is the policy from solving the problem of Vhat
                % Now Vstar
                entireRHS_ez=ReturnMatrix_ze+beta*EV_z; %*EV_z.*ones(1,N_a,1); % Use the two-future-periods discount factor
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_e=ReturnMatrix_e+beta0beta*EV; %*EV.*ones(1,N_a,1);  % Use the today-to-tomorrow discount factor
            [Vtemp,maxindex]=max(entireRHS_e,[],1);
            Vhat(:,:,e_c,jj)=shiftdim(Vtemp,1); % Note that this is Vhat when sophisticated
    ...
    elseif vfoptions.lowmemory==2
        for e_c=1:N_e
            ...
            for z_c=1:N_z
                ...
                entireRHS_ez=ReturnMatrix_ze+beta0beta*EV_z; %*EV_z.*ones(1,N_a,1);  % Use the today-to-tomorrow discount factor
                [Vtemp,maxindex]=max(entireRHS_ez,[],1);
                Vhat(:,z_c,e_c,jj)=Vtemp; % Note that this is Vhat when sophisticated
