function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_nod_e_raw(n_a,n_z,n_e,N_j, a_grid, z_gridvals_J,e_gridvals_J,pi_z_J,pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        % For naive, we compute Valt which is the exponential discounter case, and then from this we get Vtilde and
        ...
        entireRHS=ReturnMatrix+beta0beta*EV; %*repmat(EV,1,N_a,1,N_e); % Use today-to-tomorrow discount factor
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            % Now Vtilde and Policy
            entireRHS_e=ReturnMatrix_e+beta0beta*EV; %*EV.*ones(1,N_a,1);
    ...
    elseif vfoptions.lowmemory==2
        for e_c=1:N_e
            ...
            for z_c=1:N_z
                ...
                [Vtemp,maxindexalt]=max(entireRHS_ez,[],1);
                Valt(:,z_c,e_c,N_j)=Vtemp;
                Policyalt(:,z_c,e_c,N_j)=maxindexalt;
                % Now Vtilde and Policy
                entireRHS_ez=ReturnMatrix_ze+beta0beta*EV_z; %*EV_z.*ones(1,N_a,1);
