function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_DC1_e_raw(n_d,n_a,n_z,n_e,N_j, d_gridvals, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    Policyalt(curraindex,:,e_c,N_j)=shiftdim(maxindexalt+N_d*(loweredge(allindalt)-1),1);
                else
                    loweredge=maxindex1(:,1,ii,:);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Valt(level1ii,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                Policyalt(level1ii,z_c,e_c,N_j)=shiftdim(maxindex2alt,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        Valt(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        dindalt=(rem(maxindexalt-1,N_d)+1);
                        Policyalt(curraindex,z_c,e_c,N_j)=shiftdim(maxindexalt+N_d*(reshape(loweredge(dindalt),size(dindalt))-1),1);
                    else
                        loweredge=maxindex1(:,1,ii);
