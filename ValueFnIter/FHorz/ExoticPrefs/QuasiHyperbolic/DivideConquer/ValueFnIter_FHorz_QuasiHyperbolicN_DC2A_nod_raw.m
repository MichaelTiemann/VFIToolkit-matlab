function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_DC2A_nod_raw(n_a,n_z,N_j, a_grid, z_gridvals_J,pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_nod_raw.
% No d. Has z (z-expectation via pi_z_J). No e. lowmemory: =0 vectorize over z, =1 loop over z. GPU (parallel==2 only).
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        [Vtempii,maxindex2]=max(reshape(ReturnMatrix_ii,[N_a1*N_a2,vfoptions.level1n*N_a2,N_z]),[],1);
        % Store
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_z, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                aprimez=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                ...
                loweredge=maxindex1(1,:,ii,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_z, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                aprimez=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_z, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                aprimez=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprimez,[(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde(curraindex,:,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_z, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                aprimez=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprimez,[1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde(curraindex,:,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_z, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, ReturnFnParamsVec,2);
                    aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2ind;
                    entireRHS_ii=ReturnMatrix_ii_z_dc+reshape(DiscountedEV_z(aprime),[(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Valt(curraindex,z_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
                    ...
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                    allind=a2primeind+N_a2*repelem(a2ind,1,level1iidiff(ii)); % loweredge is 1-by-n_a2-by-1-by-n_a2
                    Policyalt(curraindex,z_c,N_j)=shiftdim(maxindexfix+loweredge(allind)-1,1);
                else
                    loweredge=maxindex1(1,:,ii,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_z, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, ReturnFnParamsVec,2);
                    aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2ind;
                    entireRHS_ii=ReturnMatrix_ii_z_dc+reshape(DiscountedEV_z(aprime),[1*N_a2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Valt(curraindex,z_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_z, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, ReturnFnParamsVec,2);
                    aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2ind;
                    entireRHS_ii=ReturnMatrix_ii_z_dc+reshape(DiscountedEV_tilde_z(aprime),[(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vtilde(curraindex,z_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                    ...
                    ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_z, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, ReturnFnParamsVec,2);
                    aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2ind;
                    entireRHS_ii=ReturnMatrix_ii_z_dc+reshape(DiscountedEV_tilde_z(aprime),[1*N_a2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vtilde(curraindex,z_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                % aprime possibilities are (maxgap_V(ii)+1)-n_a2-by-1-by-n_a2-by-n_z
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_z, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                aprimez=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,jj)=shiftdim(Vtempii,1);
                ...
                loweredge=maxindex1(1,:,ii,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_z, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                aprimez=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,jj)=shiftdim(Vtempii,1);
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                    allind=a2primeind+N_a2*repelem(a2ind,1,level1iidiff(ii)); % loweredge is 1-by-n_a2-by-1-by-n_a2
                    Policyalt(curraindex,z_c,jj)=shiftdim(maxindexfix+loweredge(allind)-1,1);
                else
                    loweredge=maxindex1(1,:,ii,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_z, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, ReturnFnParamsVec,2);
