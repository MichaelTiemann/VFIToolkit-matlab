function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_DC2A_noz_raw(n_d,n_a,N_j, d_gridvals, a_grid, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_noz_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_noz(ReturnFn, n_d, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,2,0);
            [Vtempii,maxindex]=max(ReturnMatrix_ii_dc,[],1);
            Valt(curraindex,N_j)=shiftdim(Vtempii,1);
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_noz(ReturnFn, n_d, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,2,0);
            [Vtempii,maxindex]=max(ReturnMatrix_ii_dc,[],1);
            Valt(curraindex,N_j)=shiftdim(Vtempii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
    ...
    EV=reshape(V_Jplus1,[1,N_a1,N_a2,1,1]); % will autoexand d in 1st-dim
    DiscountedEV=beta*EV;
    DiscountedEV_tilde=beta0beta*EV;
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap_V(ii)>0
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,]));
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            Valt(curraindex,N_j)=shiftdim(Vtempii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_noz(ReturnFn, n_d, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,2,0);
            aprime=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind;
            entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprime,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            Vtilde(curraindex,N_j)=shiftdim(Vtempii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_noz(ReturnFn, n_d, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,2,0);
            aprime=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind;
            entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            Vtilde(curraindex,N_j)=shiftdim(Vtempii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
...
for reverse_j=1:N_j-1
    ...
    EV=reshape(Valt(:,jj+1),[1,N_a1,N_a2,1,1]); % will autoexand d in 1st-dim
    DiscountedEV=beta*EV;
    DiscountedEV_tilde=beta0beta*EV;
