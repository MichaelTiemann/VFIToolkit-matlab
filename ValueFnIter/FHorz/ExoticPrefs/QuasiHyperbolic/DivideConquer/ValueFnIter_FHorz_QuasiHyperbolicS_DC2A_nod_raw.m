function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicS_DC2A_nod_raw(n_a,n_z,N_j, a_grid, z_gridvals_J,pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_nod_raw.
% divide-and-conquer in the first endo state. No d. Has z. No e. GPU (parallel==2 only).
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii_z+reshape(DiscountedEV_z(aprime),[(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat(curraindex,z_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                    ...
                    entireRHS_ii=ReturnMatrix_ii_z+reshape(DiscountedEV_z(aprime),[1*N_a2,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat(curraindex,z_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
