function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_DC1_nod_noz_raw(n_a, N_j, a_grid, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for ii=1:(vfoptions.level1n-1)
        curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
        a_range=Policyalt(level1ii(ii),N_j):Policyalt(level1ii(ii+1),N_j);
        ...
        Valt(curraindex,N_j)=shiftdim(Vtempii,1);
        Policyalt(curraindex,N_j)=shiftdim(maxindex,1)+Policyalt(level1ii(ii),N_j)-1;
    end
    %% Vtilde (beta0*beta)
...
for reverse_j=1:N_j-1
    ...
    for ii=1:(vfoptions.level1n-1)
        curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
        a_range=Policyalt(level1ii(ii),jj):Policyalt(level1ii(ii+1),jj);
        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC1_nod_noz(ReturnFn, a_grid(a_range), a_grid(curraindex), ReturnFnParamsVec);
        entireRHS_ii=ReturnMatrix_ii_dc+beta*EV(a_range);
        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
