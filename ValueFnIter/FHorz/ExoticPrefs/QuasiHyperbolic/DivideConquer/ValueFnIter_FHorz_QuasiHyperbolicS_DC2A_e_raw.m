function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicS_DC2A_e_raw(n_d,n_a,n_z,n_e,N_j, d_gridvals, a_grid, z_gridvals_J,e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_e_raw.
% divide-and-conquer in the first endo state. Has d. Has z. Has e. GPU (parallel==2 only).
...
zindC=gpuArray(0:1:N_z-1); % 1-by-N_z, for EV_at_policy gather
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV=beta0beta*reshape(EV,[1,N_a1,N_a2,1,1,N_z]); % autoexpand d into 1st-dim
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprimez,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z,N_e]));
                ...
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vhat(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_vals, ReturnFnParamsVec,2,0);
                    aprimez=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_e+DiscountedEV(reshape(aprimez,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                    ...
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_vals, ReturnFnParamsVec,2,0);
                    aprimez=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_e+DiscountedEV(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vhat(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii_ze+DiscountedEV_z(reshape(aprime,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        Vhat(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        % maxindex needs to be reworked:
                        %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                        ...
                        entireRHS_ii=ReturnMatrix_ii_ze+DiscountedEV_z(reshape(aprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        Vhat(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        % maxindex needs to be reworked:
                        %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
