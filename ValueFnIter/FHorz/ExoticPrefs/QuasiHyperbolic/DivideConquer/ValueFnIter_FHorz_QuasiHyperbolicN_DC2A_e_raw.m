function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_DC2A_e_raw(n_d,n_a,n_z,n_e,N_j, d_gridvals, a_grid, z_gridvals_J,e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_e_raw.
% divide-and-conquer in the first endo state. Has d, z, and e. lowmemory: =0 vectorize, =1 loop over e, =2 loop over e and z. GPU (parallel==2 only).
%
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV=beta*reshape(EV,[1,N_a1,N_a2,1,1,N_z]); % autoexpand d into 1st-dim
    DiscountedEV_tilde=beta0beta*reshape(EV,[1,N_a1,N_a2,1,1,N_z]); % autoexpand d into 1st-dim
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
                aprimez=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
                dind=(rem(maxindex-1,N_d)+1);
                a1primeind=rem(ceil(maxindex/N_d)-1,maxgap_V(ii)+1)+1-1; % already includes -1
                a2primeind=ceil(maxindex/(N_d*(maxgap_V(ii)+1)))-1; % already includes -1
                maxindexfix=dind+N_d*a1primeind+N_d*N_a1*a2primeind; % put maxindex back together, using N_a1 to determine a2prime, rather than using (maxgap_V(ii)+1) which is what it originally was in maxindex
                %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                allind=dind+N_d*a2primeind+N_d*N_a2*repelem(a2ind,1,level1iidiff(ii))+N_d*N_a2*N_a2*zind+N_d*N_a2*N_a2*N_z*eind; % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2-by-n_z-by-n_e
                Policyalt(curraindex,:,:,N_j)=shiftdim(maxindexfix+N_d*(loweredge(allind)-1),1);
            else
                loweredge=maxindex1(:,1,:,ii,:,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
                aprimez=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
                aprimez=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprimez,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
                aprimez=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_vals, ReturnFnParamsVec,2,0);
                    aprimez=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_e_dc+DiscountedEV(reshape(aprimez,[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Valt(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
                    ...
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                    allind=dind+N_d*a2primeind+N_d*N_a2*repelem(a2ind,1,level1iidiff(ii))+N_d*N_a2*N_a2*zind; % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2-by-n_z
                    Policyalt(curraindex,:,e_c,N_j)=shiftdim(maxindexfix+N_d*(loweredge(allind)-1),1);
                else
                    loweredge=maxindex1(:,1,:,ii,:,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_vals, ReturnFnParamsVec,2,0);
                    aprimez=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_e_dc+DiscountedEV(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Valt(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_vals, ReturnFnParamsVec,2,0);
                    aprimez=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_e_dc+DiscountedEV_tilde(reshape(aprimez,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vtilde(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                    ...
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_vals, ReturnFnParamsVec,2,0);
                    aprimez=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_e_dc+DiscountedEV_tilde(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Vtilde(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, special_n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, e_vals, ReturnFnParamsVec,2,0);
                        aprime=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind;
                        entireRHS_ii=ReturnMatrix_ii_ze_dc+DiscountedEV_z(reshape(aprime,[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        Valt(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        % maxindex needs to be reworked:
                        %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
                        ...
                        %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                        allind=dind+N_d*a2primeind+N_d*N_a2*repelem(a2ind,1,level1iidiff(ii)); % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2
                        Policyalt(curraindex,z_c,e_c,N_j)=shiftdim(maxindexfix+N_d*(loweredge(allind)-1),1);
                    else
                        loweredge=maxindex1(:,1,:,ii,:);
                        % Just use aprime(ii) for everything
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, special_n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, e_vals, ReturnFnParamsVec,2,0);
                        aprime=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind;
                        entireRHS_ii=ReturnMatrix_ii_ze_dc+DiscountedEV_z(reshape(aprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        Valt(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        % maxindex needs to be reworked:
                        %  the a2prime is only an 'after maxgap_V(ii)+1, but needs to be after N_a1'
                ...
                %% Vtilde (beta0*beta)
                entireRHS_ii=ReturnMatrix_ii_ze+DiscountedEV_tilde_z; % autofill e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, special_n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, e_vals, ReturnFnParamsVec,2,0);
                        aprime=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind;
                        entireRHS_ii=ReturnMatrix_ii_ze_dc+DiscountedEV_tilde_z(reshape(aprime,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        Vtilde(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        % maxindex needs to be reworked:
                        %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                        ...
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, special_n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, e_vals, ReturnFnParamsVec,2,0);
                        aprime=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind;
                        entireRHS_ii=ReturnMatrix_ii_ze_dc+DiscountedEV_tilde_z(reshape(aprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        Vtilde(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        % maxindex needs to be reworked:
                        %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        Policyalt(curraindex,:,:,jj)=shiftdim(maxindex2,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                % aprime possibilities are n_d-by-maxgap_V(ii)+1-by-n_a2-by-1-by-n_a2-by-n_z-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0);
                aprimez=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,:,jj)=shiftdim(Vtempii,1);
                ...
                %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                allind=dind+N_d*a2primeind+N_d*N_a2*repelem(a2ind,1,level1iidiff(ii))+N_d*N_a2*N_a2*zind+N_d*N_a2*N_a2*N_z*eind; % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2-by-n_z-by-n_e
                Policyalt(curraindex,:,:,jj)=shiftdim(maxindexfix+N_d*(loweredge(allind)-1),1);
            else
                loweredge=maxindex1(:,1,:,ii,:,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0);
                aprimez=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,:,jj)=shiftdim(Vtempii,1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Policyalt(curraindex,:,e_c,jj)=shiftdim(maxindex2,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                    % aprime possibilities are n_d-by-maxgap_V(ii)+1-by-n_a2-by-1-by-n_a2-by-n_z
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), e_vals, ReturnFnParamsVec,2,0);
                    aprimez=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_e_dc+DiscountedEV(reshape(aprimez,[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Valt(curraindex,:,e_c,jj)=shiftdim(Vtempii,1);
                    ...
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                    allind=dind+N_d*a2primeind+N_d*N_a2*repelem(a2ind,1,level1iidiff(ii))+N_d*N_a2*N_a2*zind; % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2-by-n_z
                    Policyalt(curraindex,:,e_c,jj)=shiftdim(maxindexfix+N_d*(loweredge(allind)-1),1);
                else
                    loweredge=maxindex1(:,1,:,ii,:,:);
                    % Just use aprime(ii) for everything
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), e_vals, ReturnFnParamsVec,2,0);
                    aprimez=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_e_dc+DiscountedEV(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    Valt(curraindex,:,e_c,jj)=shiftdim(Vtempii,1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policyalt(curraindex,z_c,e_c,jj)=shiftdim(maxindex2,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                        allind=dind+N_d*a2primeind+N_d*N_a2*repelem(a2ind,1,level1iidiff(ii)); % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2
                        Policyalt(curraindex,z_c,e_c,jj)=shiftdim(maxindexfix+N_d*(loweredge(allind)-1),1);
                    else
                        loweredge=maxindex1(:,1,:,ii,:);
                        % Just use aprime(ii) for everything
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, n_d, special_n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, e_vals, ReturnFnParamsVec,2,0);
