function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicS_DC2A_GI2A_nod_raw(n_a, n_z, N_j, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_GI2A_nod_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    EV=reshape(EV,[N_a1,N_a2,1,1,N_z]);  % autoexpand (a,z)
    ...
    if vfoptions.lowmemory==0
        ...
        EVfine=reshape(EVinterp(aprimez),[n2long*N_a2,N_a,N_z]);
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            %% Vhat (beta0*beta)
            entireRHS_ii=ReturnMatrix_ii_z+beta0beta*EV_z;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii_z_dc+beta0beta*EV_z(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 3rd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(1,:,ii,:);
            ...
            EVfine_z=reshape(EVinterp_z(aprime),[n2long*N_a2,N_a]);
            entireRHS_L2=ReturnMatrix_L2_z+beta0beta*EVfine_z;
            [Vtempii,maxindexL2]=max(entireRHS_L2,[],1);
            maxindexL2a1=rem(maxindexL2-1,n2long)+1;
            maxindexL2a2=ceil(maxindexL2/n2long);
            Vhat(:,z_c,N_j)=shiftdim(Vtempii,1);
...
for reverse_j=1:N_j-1
    ...
    EV=Vunderbar(:,:,jj+1); % sophisticated: continuation is Vunderbar
