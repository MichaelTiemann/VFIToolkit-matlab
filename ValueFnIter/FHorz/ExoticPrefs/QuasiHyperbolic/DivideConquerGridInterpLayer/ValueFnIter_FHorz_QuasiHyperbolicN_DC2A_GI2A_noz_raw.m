...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_noz(ReturnFn, n_d, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,3,0);
    ...
    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_noz(ReturnFn,n_d,d_gridvals,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid,ReturnFnParamsVec,2,0);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap_V(ii)>0
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[N_d,(maxgap_V(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 4th-dim
    ...
    entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp(aprime),[N_d*n2long*N_a2,N_a]);
    ...
    Policyalt(2,:,N_j)=midpoints_jj(maxindexL2dalt+N_d*(maxindexL2a2alt-1)+N_d*N_a2*a12ind); % a1prime midpoint
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_noz(ReturnFn, n_d, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,3,0);
            aprime=a1primeindexes+N_a1*a2ind;
            entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 4th-dim
            [~,maxindex]=max(entireRHS_ii,[],2);
            midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
        else
            loweredge=maxindex1(:,1,:,ii,:);
    ...
    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_noz(ReturnFn,n_d,d_gridvals,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,ReturnFnParamsVec,2,0);
    aprime=a1primeindexes+N_a1fine*a2ind;
    entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde(aprime),[N_d*n2long*N_a2,N_a]);
...
for reverse_j=1:N_j-1
    ...
    DiscountedEV_tilde=beta0beta*reshape(Valt(:,jj+1),[N_a1,N_a2]); % will autoexand d in 1st-dim
    % Interpolate EV over aprime_grid
    DiscountedEVinterp=interp1(a1_grid,DiscountedEV,a1prime_grid);
    DiscountedEVinterp_tilde=interp1(a1_grid,DiscountedEV_tilde,a1prime_grid);
    DiscountedEV=shiftdim(DiscountedEV,-1); % will autoexand d in 1st-dim
    ...
    Policyalt(2,:,jj)=midpoints_jj(maxindexL2dalt+N_d*(maxindexL2a2alt-1)+N_d*N_a2*a12ind); % a1prime midpoint
