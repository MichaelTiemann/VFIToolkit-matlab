function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicS_DC2A_GI2A_nod_e_raw(n_a,n_z,n_e, N_j, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_GI2A_nod_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    EV=sum(reshape(vfoptions.V_Jplus1,[N_a,N_z,N_e]).*pi_e_J(1,1,:,N_j+1),3); % Using V_Jplus1 (should be Vunderbar for sophisticated)
    ...
    if vfoptions.lowmemory==0
        ...
        %% Vhat (beta0*beta)
        entireRHS_ii=ReturnMatrix_ii+beta0beta*EV; % autofill e
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV(reshape(aprimez,[(maxgap(ii)+1),N_a2,1,N_a2,N_z,N_e])); % autoexpand level1iidiff(ii) in 3rd-dim
        ...
        EVfine=reshape(EVinterp(aprimez),[n2long*N_a2,N_a,N_z,N_e]);
        ...
        linidx=reshape(maxindexL2,[1,N_a*N_z*N_e])+n2long*N_a2*(0:N_a*N_z*N_e-1);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    aprimez=aprimeindexes+N_a1*a2ind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV(reshape(aprimez,[(maxgap(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) in 3rd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,:,curraindex,:,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(1,:,ii,:,:);
            ...
            aprimez=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
            EVfine_e=reshape(EVinterp(aprimez),[n2long*N_a2,N_a,N_z]);
            ...
            % Vunderbar: gather the interpolated continuation at the (interpolated) optimal choice
            linidx=reshape(maxindexL2,[1,N_a*N_z])+n2long*N_a2*(0:N_a*N_z-1);
            EV_at_policy=reshape(EVfine_e(linidx),[N_a,N_z]);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                %% Vhat (beta0*beta)
                entireRHS_ii=ReturnMatrix_ii+beta0beta*EV_z; % autofill e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV_z(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 3rd-dim
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,:,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,:,ii,:);
                ...
                EVfine_ze=reshape(EVinterp_z(aprime),[n2long*N_a2,N_a]);
                ...
                EV_at_policy=reshape(EVfine_ze(linidx),[N_a,1]);
...
for reverse_j=1:N_j-1
    ...
    EV=sum(Vunderbar(:,:,:,jj+1).*pi_e_J(1,1,:,jj+1),3); % sophisticated: continuation is Vunderbar
