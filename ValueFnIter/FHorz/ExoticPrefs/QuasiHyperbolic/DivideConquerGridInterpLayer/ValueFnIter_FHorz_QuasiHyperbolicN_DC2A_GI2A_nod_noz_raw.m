function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_DC2A_GI2A_nod_noz_raw(n_a, N_j, a_grid, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_GI2A_nod_noz_raw.
% divide-and-conquer in the first endo state (a2 enumerated in full), plus grid interpolation layer.
% No d. No z. No e. GPU (parallel==2 only).
%
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod_noz(ReturnFn, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,1);
    ...
    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_nod_noz(ReturnFn,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid,ReturnFnParamsVec,2);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap_V(ii)>0
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprime,[(maxgap_V(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 3rd-dim
    ...
    entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp(aprime),[n2long*N_a2,N_a]);
    ...
    Valt(:,N_j)=shiftdim(Vtempii,1);
    Policyalt(1,:,N_j)=midpoints_jj(maxindexL2a2alt+N_a2*a12ind); % a1prime midpoint
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod_noz(ReturnFn, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,1);
            aprime=aprimeindexes+N_a1*a2ind;
            entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 3rd-dim
            [~,maxindex]=max(entireRHS_ii,[],1);
            midpoints_jj(1,:,curraindex,:)=maxindex+(loweredge-1);
        else
            loweredge=maxindex1(1,:,ii,:,:);
    ...
    ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_nod_noz(ReturnFn,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,ReturnFnParamsVec,2);
    aprime=a1primeindexes+N_a1fine*a2ind;
    entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde(aprime),[n2long*N_a2,N_a]);
...
for reverse_j=1:N_j-1
    ...
    DiscountedEV=beta*reshape(Valt(:,jj+1),[N_a1,N_a2]);
    DiscountedEV_tilde=beta0beta*reshape(Valt(:,jj+1),[N_a1,N_a2]);
    % Interpolate EV over aprime_grid
    DiscountedEVinterp=interp1(a1_grid,DiscountedEV,a1prime_grid);
    DiscountedEVinterp_tilde=interp1(a1_grid,DiscountedEV_tilde,a1prime_grid);
    ...
    Policyalt(1,:,jj)=midpoints_jj(maxindexL2a2alt+N_a2*a12ind); % a1prime midpoint
