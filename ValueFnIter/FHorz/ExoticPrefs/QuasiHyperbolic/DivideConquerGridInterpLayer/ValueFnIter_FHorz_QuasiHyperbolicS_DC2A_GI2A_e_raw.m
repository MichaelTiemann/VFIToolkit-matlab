function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicS_DC2A_GI2A_e_raw(n_d,n_a,n_z,n_e, N_j, d_gridvals, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_GI2A_e_raw.
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV(reshape(aprimez,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_z,N_e])); % autoexpand level1iidiff(ii) into 4th-dim
        ...
        EVfine=reshape(EVinterp(aprime),[N_d*n2long*N_a2,N_a,N_z,N_e]);
        ...
        linidx=reshape(maxindexL2,[1,N_a*N_z*N_e])+N_d*n2long*N_a2*(0:N_a*N_z*N_e-1);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    aprimez=a1primeindexes+N_a1*a2Bind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV(reshape(aprimez,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) into 4th-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:,:);
            ...
            aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
            EVfine_e=reshape(EVinterp(aprime),[N_d*n2long*N_a2,N_a,N_z]);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV_z(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) into 4th-dim
                        [~,maxindex]=max(entireRHS_ii,[],2);
                        midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,:,ii,:);
                ...
                EVfine_ze=reshape(EVinterp_z(aprime),[N_d*n2long*N_a2,N_a]);
