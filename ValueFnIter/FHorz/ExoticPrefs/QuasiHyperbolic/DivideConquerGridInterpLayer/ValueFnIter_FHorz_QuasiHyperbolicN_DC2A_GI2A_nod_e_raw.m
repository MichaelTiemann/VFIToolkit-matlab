function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_DC2A_GI2A_nod_e_raw(n_a,n_z,n_e, N_j, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_GI2A_nod_e_raw.
% divide-and-conquer in the first endo state (a2 enumerated in full), plus grid interpolation layer.
% No d. Has z (z-expectation via pi_z_J). Has e (iid, e-expectation via pi_e_J). GPU (parallel==2 only).
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV=beta*reshape(EV,[N_a1,N_a2,1,1,N_z]);
    DiscountedEV_tilde=beta0beta*reshape(EV,[N_a1,N_a2,1,1,N_z]);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn, n_z, n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
                aprimez=aprimeindexes+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[(maxgap_V(ii)+1),N_a2,1,N_a2,N_z,N_e])); % autoexpand level1iidiff(ii) in 3rd-dim
        ...
        a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)'; % aprime points either side of midpoint
        % aprime possibilities are n2long-by-n_a2-by-n_a1-by-n_a2-by-n_z-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn,n_z,n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
        aprimez=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
        entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp(aprimez),[n2long*N_a2,N_a,N_z,N_e]);
        ...
        Valt(:,:,:,N_j)=shiftdim(Vtempii,1);
        Policyalt(1,:,:,:,N_j)=midpoints_jj(maxindexL2a2alt+N_a2*a12ind+N_a2*N_a*zind+N_a2*N_a*N_z*eind); % a1prime midpoint
        Policyalt(2,:,:,:,N_j)=maxindexL2a2alt; % a2prime
        Policyalt(3,:,:,:,N_j)=maxindexL2a1alt; % a1primeL2ind

        % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
        linidx_loweralt = 1      + n2long*(maxindexL2a2alt-1) + n2long*N_a2*a12ind + n2long*N_a2*N_a*zind + n2long*N_a2*N_a*N_z*eind;
        linidx_upperalt = n2long + n2long*(maxindexL2a2alt-1) + n2long*N_a2*a12ind + n2long*N_a2*N_a*zind + n2long*N_a2*N_a*N_z*eind;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn, n_z, n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
                aprimez=aprimeindexes+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprimez,[(maxgap(ii)+1),N_a2,1,N_a2,N_z,N_e])); % autoexpand level1iidiff(ii) in 3rd-dim
                [~,maxindex]=max(entireRHS_ii,[],1);
                midpoints_jj(1,:,curraindex,:,:,:)=maxindex+(loweredge-1);
            else
                loweredge=maxindex1(1,:,ii,:,:,:);
        ...
        ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn,n_z,n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
        aprimez=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
        entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde(aprimez),[n2long*N_a2,N_a,N_z,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn, n_z, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3);
                    aprimez=aprimeindexes+N_a1*a2ind+N_a*zBind;
            ...
            a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)'; % aprime points either side of midpoint
            % aprime possibilities are n2long-by-n_a2-by-n_a1-by-n_a2-by-n_z
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn,n_z,special_n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2);
            aprimez=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
            ...
            maxindexL2a2alt=ceil(maxindexL2alt/n2long);
            Valt(:,:,e_c,N_j)=shiftdim(Vtempii,1);
            Policyalt(1,:,:,e_c,N_j)=midpoints_jj(maxindexL2a2alt+N_a2*a12ind+N_a2*N_a*zind); % a1prime midpoint
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn, special_n_z, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, e_val, ReturnFnParamsVec,3);
                        aprime=aprimeindexes+N_a1*a2ind;
                        entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_z(reshape(aprime,[(maxgap_V(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 3rd-dim
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,:,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,:,ii,:);
                ...
                a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)'; % aprime points either side of midpoint
                % aprime possibilities are n2long-by-n_a2-by-n_a1-by-n_a2
                ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn,special_n_z,special_n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,z_val, e_val, ReturnFnParamsVec,2);
                aprime=a1primeindexes+N_a1fine*a2ind;
                entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_z(aprime),[n2long*N_a2,N_a]);
                [Vtempii,maxindexL2alt]=max(entireRHS_L2,[],1);
                maxindexL2a1alt=rem(maxindexL2alt-1,n2long)+1;
                maxindexL2a2alt=ceil(maxindexL2alt/n2long);
                Valt(:,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                Policyalt(1,:,z_c,e_c,N_j)=midpoints_jj(maxindexL2a2alt+N_a2*a12ind); % a1prime midpoint
                Policyalt(2,:,z_c,e_c,N_j)=maxindexL2a2alt; % a2prime
                Policyalt(3,:,z_c,e_c,N_j)=maxindexL2a1alt; % a1primeL2ind
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn, special_n_z, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, e_val, ReturnFnParamsVec,3);
                        aprime=aprimeindexes+N_a1*a2ind;
                        entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde_z(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 3rd-dim
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,:,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,:,ii,:);
                ...
                ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn,special_n_z,special_n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,z_val, e_val, ReturnFnParamsVec,2);
                aprime=a1primeindexes+N_a1fine*a2ind;
                entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde_z(aprime),[n2long*N_a2,N_a]);
                [Vtempii,maxindexL2]=max(entireRHS_L2,[],1);
                maxindexL2a1=rem(maxindexL2-1,n2long)+1;
                maxindexL2a2=ceil(maxindexL2/n2long);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                % aprime possibilities are (maxgap_V(ii)+1)-n_a2-by-1-by-n_a2-by-n_z-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn, n_z, n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3);
                aprimez=aprimeindexes+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[(maxgap_V(ii)+1),N_a2,1,N_a2,N_z,N_e])); % autoexpand level1iidiff(ii) in 3rd-dim
                [~,maxindex]=max(entireRHS_ii,[],1);
                midpoints_jj(1,:,curraindex,:,:,:)=maxindex+(loweredge-1);
        ...
        a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)'; % aprime points either side of midpoint
        % aprime possibilities are n2long-by-n_a2-by-n_a1-by-n_a2-by-n_z-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn,n_z,n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,z_gridvals_J(:,:,jj),e_gridvals_J(:,:,jj),  ReturnFnParamsVec,2);
        aprimez=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
        entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp(aprimez),[n2long*N_a2,N_a,N_z,N_e]);
        [Vtempii,maxindexL2alt]=max(entireRHS_L2,[],1);
        maxindexL2a1alt=rem(maxindexL2alt-1,n2long)+1;
        ...
        Policyalt(1,:,:,:,jj)=midpoints_jj(maxindexL2a2alt+N_a2*a12ind+N_a2*N_a*zind+N_a2*N_a*N_z*eind); % a1prime midpoint
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                    % aprime possibilities are (maxgap_V(ii)+1)-n_a2-by-1-by-n_a2-by-n_z
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn, n_z, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3);
                    aprimez=aprimeindexes+N_a1*a2ind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[(maxgap_V(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) in 3rd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,:,curraindex,:,:)=maxindex+(loweredge-1);
            ...
            a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)'; % aprime points either side of midpoint
            % aprime possibilities are n2long-by-n_a2-by-n_a1-by-n_a2-by-n_z
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A_nod_e(ReturnFn,n_z,special_n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2);
            aprimez=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
            entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp(aprimez),[n2long*N_a2,N_a,N_z]);
            [Vtempii,maxindexL2alt]=max(entireRHS_L2,[],1);
            maxindexL2a1alt=rem(maxindexL2alt-1,n2long)+1;
            ...
            Policyalt(1,:,:,e_c,jj)=midpoints_jj(maxindexL2a2alt+N_a2*a12ind+N_a2*N_a*zind); % a1prime midpoint
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policyalt(1,:,z_c,e_c,jj)=midpoints_jj(maxindexL2a2alt+N_a2*a12ind); % a1prime midpoint
                Policyalt(2,:,z_c,e_c,jj)=maxindexL2a2alt; % a2prime
                Policyalt(3,:,z_c,e_c,jj)=maxindexL2a1alt; % a1primeL2ind
