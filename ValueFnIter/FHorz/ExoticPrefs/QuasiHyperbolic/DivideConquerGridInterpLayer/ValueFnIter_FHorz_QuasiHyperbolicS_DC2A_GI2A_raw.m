function [Vhat, Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicS_DC2A_GI2A_raw(n_d,n_a,n_z, N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_GI2A_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    EV=reshape(vfoptions.V_Jplus1,[N_a,N_z]); % Using V_Jplus1 (should be Vunderbar for sophisticated)

    EV=EV.*shiftdim(pi_z_J(:,:,N_j)',-1);
    EV(isnan(EV))=0; %multiplications of -Inf with 0 gives NaN, this replaces them with zeros (as the zeros come from the transition probabilities)
    EV=sum(EV,2); % sum over z', leaving a singular second dimension

    EV=reshape(EV,[N_a1,N_a2,1,1,N_z]); % will autoexand d in 1st-dim
    % Interpolate EV over aprime_grid
    EVinterp=interp1(a1_grid,EV,a1prime_grid);
    EV=shiftdim(EV,-1); % will autoexand d in 1st-dim
    EVinterp=shiftdim(EVinterp,-1); % will autoexand d in 1st-dim
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV(reshape(aprimez,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) in 4th-dim
        ...
        EVfine=reshape(EVinterp(aprime),[N_d*n2long*N_a2,N_a,N_z]);
        ...
        linidx=reshape(maxindexL2,[1,N_a*N_z])+N_d*n2long*N_a2*(0:N_a*N_z-1);
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV_z(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 4th-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
            ...
            EVfine_z=reshape(EVinterp_z(aprime),[N_d*n2long*N_a2,N_a]);
            ...
            EV_at_policy=reshape(EVfine_z(linidx),[N_a,1]);
            Vunderbar(:,z_c,N_j)=Vhat(:,z_c,N_j)+(beta-beta0beta)*EV_at_policy;
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    EV=Vunderbar(:,:,jj+1).*shiftdim(pi_z_J(:,:,jj)',-1); % sophisticated: continuation is Vunderbar
    EV(isnan(EV))=0; %multiplications of -Inf with 0 gives NaN, this replaces them with zeros (as the zeros come from the transition probabilities)
    EV=sum(EV,2); % sum over z', leaving a singular second dimension
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            linidx=reshape(maxindexL2,[1,N_a])+N_d*n2long*N_a2*(0:N_a-1);
            EV_at_policy=reshape(EVfine_z(linidx),[N_a,1]);
            Vunderbar(:,z_c,jj)=Vhat(:,z_c,jj)+(beta-beta0beta)*EV_at_policy;
        end
    end
end
