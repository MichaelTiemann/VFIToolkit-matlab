function [Vtilde, Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_DC2A_GI2A_raw(n_d,n_a,n_z, N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_GI2A_raw.
% divide-and-conquer in the first endo state (a2 enumerated in full), plus grid interpolation layer.
% Has d. Has z (z-expectation via pi_z_J). No e. GPU (parallel==2 only).
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                aprimez=a1primeindexes+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[N_d,(maxgap_V(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) in 4th-dim
        ...
        a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint
        % aprime possibilities are n_d-by-n2long-by-n_a2-by-n_a1-by-n_a2-by-n_z
        ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A(ReturnFn,n_d,n_z,d_gridvals,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
        ...
        Policyalt(2,:,:,N_j)=midpoints_jj(maxindexL2dalt+N_d*(maxindexL2a2alt-1)+N_d*N_a2*a12ind+N_d*N_a2*N_a*zind); % a1prime midpoint
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                aprimez=a1primeindexes+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(reshape(aprimez,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) in 4th-dim
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoints_jj(:,1,:,curraindex,:,:)=maxindex+(loweredge-1);
            else
                loweredge=maxindex1(:,1,:,ii,:,:);
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, special_n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsVec,3,0);
                    aprime=a1primeindexes+N_a1*a2Bind;
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_z(reshape(aprime,[N_d,(maxgap_V(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 4th-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
            ...
            a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint
            % aprime possibilities are n_d-by-n2long-by-n_a2-by-n_a1-by-n_a2
            ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A(ReturnFn,n_d,special_n_z,d_gridvals,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid, z_val, ReturnFnParamsVec,2,0);
            ...
            maxindexL2a2alt=ceil(maxindexL2aalt/n2long);
            Valt(:,z_c,N_j)=shiftdim(Vtempii,1);
            Policyalt(1,:,z_c,N_j)=maxindexL2dalt; % d
            Policyalt(2,:,z_c,N_j)=midpoints_jj(maxindexL2dalt+N_d*(maxindexL2a2alt-1)+N_d*N_a2*a12ind); % a1prime midpoint
            Policyalt(3,:,z_c,N_j)=maxindexL2a2alt; % a2prime
            Policyalt(4,:,z_c,N_j)=maxindexL2a1alt; % a1primeL2ind
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, special_n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsVec,3,0);
                    aprime=a1primeindexes+N_a1*a2Bind;
                    entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde_z(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 4th-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
...
for reverse_j=1:N_j-1
    ...
    EV=Valt(:,:,jj+1).*shiftdim(pi_z_J(:,:,jj)',-1); % naive: continuation is the exponential value fn
    EV(isnan(EV))=0; %multiplications of -Inf with 0 gives NaN, this replaces them with zeros (as the zeros come from the transition probabilities)
    EV=sum(EV,2); % sum over z', leaving a singular second dimension
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                % aprime possibilities are n_d-by-maxgap_V(ii)+1-by-n_a2-by-1-by-n_a2-by-n_z
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                aprimez=a1primeindexes+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV(reshape(aprimez,[N_d,(maxgap_V(ii)+1),N_a2,1,N_a2,N_z]));% autoexpand level1iidiff(ii) in 4th-dim
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoints_jj(:,1,:,curraindex,:,:)=maxindex+(loweredge-1);
        ...
        a1primeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint
        % aprime possibilities are n_d-by-n2long-by-n_a2-by-n_a1-by-n_a2-by-n_z
        ReturnMatrix_L2=CreateReturnFnMatrix_Disc_DC2A(ReturnFn,n_d,n_z,d_gridvals,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0);
        aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
        entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp(aprime),[N_d*n2long*N_a2,N_a,N_z]);
        [Vtempii,maxindexL2alt]=max(entireRHS_L2,[],1);
        maxindexL2dalt=rem(maxindexL2alt-1,N_d)+1;
        ...
        Policyalt(2,:,:,jj)=midpoints_jj(maxindexL2dalt+N_d*(maxindexL2a2alt-1)+N_d*N_a2*a12ind+N_d*N_a2*N_a*zind); % a1prime midpoint
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            Policyalt(1,:,z_c,jj)=maxindexL2dalt; % d
            Policyalt(2,:,z_c,jj)=midpoints_jj(maxindexL2dalt+N_d*(maxindexL2a2alt-1)+N_d*N_a2*a12ind); % a1prime midpoint
            Policyalt(3,:,z_c,jj)=maxindexL2a2alt; % a2prime
            Policyalt(4,:,z_c,jj)=maxindexL2a1alt; % a1primeL2ind
