function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicS_DC2A_GI2A_noz_e_raw(n_d,n_a,n_e, N_j, d_gridvals, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_GI2A_noz_e_raw.
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_e])); % autoexpand level1iidiff(ii) in 4th-dim
        ...
        EVfine=reshape(EVinterp(aprime),[N_d*n2long*N_a2,N_a,N_e]);
        ...
        linidx=reshape(maxindexL2,[1,N_a*N_e])+N_d*n2long*N_a2*(0:N_a*N_e-1);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_val, ReturnFnParamsVec,3,0);
                    aprime=a1primeindexes+N_a1*a2Bind;
                    entireRHS_ii=ReturnMatrix_ii_e_dc+beta0beta*EV(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 4th-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
            ...
            ReturnMatrix_L2_e=CreateReturnFnMatrix_Disc_DC2A(ReturnFn,n_d,special_n_e,d_gridvals,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,e_val, ReturnFnParamsVec,2,0);
            aprime=a1primeindexes+N_a1fine*a2ind;
            EVfine_e=reshape(EVinterp(aprime),[N_d*n2long*N_a2,N_a]);
