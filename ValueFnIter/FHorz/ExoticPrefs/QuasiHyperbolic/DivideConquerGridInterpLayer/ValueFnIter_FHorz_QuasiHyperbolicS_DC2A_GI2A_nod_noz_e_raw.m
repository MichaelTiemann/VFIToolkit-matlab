function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicS_DC2A_GI2A_nod_noz_e_raw(n_a, n_e, N_j, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic discounting variant of ValueFnIter_FHorz_DC2A_GI2A_nod_noz_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    EV=sum(reshape(vfoptions.V_Jplus1,[N_a,N_e]).*pi_e_J(1,:,N_j+1),2); % Using V_Jplus1 (should be Vunderbar for sophisticated)

    EV=reshape(EV,[N_a1,N_a2,1,1]);  % autoexpand (a)
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2,N_e]));  % autoexpand level1iidiff(ii) in 3rd-dim
        ...
        EVfine=reshape(EVinterp(aprime),[n2long*N_a2,N_a,N_e]);
        ...
        linidx=reshape(maxindexL2,[1,N_a*N_e])+n2long*N_a2*(0:N_a*N_e-1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            %% Vhat (beta0*beta)
            entireRHS_ii=ReturnMatrix_ii_e+beta0beta*EV;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_val, ReturnFnParamsVec,3);
                    aprime=aprimeindexes+N_a1*a2ind;
                    entireRHS_ii=ReturnMatrix_ii_e_dc+beta0beta*EV(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2]));  % autoexpand level1iidiff(ii) in 3rd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(1,:,ii,:);
            ...
            ReturnMatrix_L2_e=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,e_val, ReturnFnParamsVec,2);
            aprime=a1primeindexes+N_a1fine*a2ind;
            EVfine_e=reshape(EVinterp(aprime),[n2long*N_a2,N_a]);
            entireRHS_L2=ReturnMatrix_L2_e+beta0beta*EVfine_e;
            [Vtempii,maxindexL2]=max(entireRHS_L2,[],1);
            maxindexL2a1=rem(maxindexL2-1,n2long)+1;
            maxindexL2a2=ceil(maxindexL2/n2long);
            Vhat(:,e_c,N_j)=shiftdim(Vtempii,1);
            ...
            EV_at_policy=reshape(EVfine_e(linidx),[N_a,1]);
...
for reverse_j=1:N_j-1
    ...
    EV=sum(Vunderbar(:,:,jj+1).*pi_e_J(1,:,jj+1),2); % sophisticated: continuation is Vunderbar
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            %% Vhat (beta0*beta)
            entireRHS_ii=ReturnMatrix_ii_e+beta0beta*EV; % autofill e
