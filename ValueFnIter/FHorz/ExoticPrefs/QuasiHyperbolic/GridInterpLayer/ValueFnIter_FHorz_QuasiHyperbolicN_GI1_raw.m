function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_GI1_raw(n_d,n_a,n_z,N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            ReturnMatrix=CreateReturnFnMatrix_Disc(ReturnFn, n_d, n_a, special_n_z, d_gridvals, a_grid, z_val, ReturnFnParamsVec,1);
            [~,maxindex]=max(ReturnMatrix,[],2);
            midpoint=max(min(maxindex,n_a-1),2);
            aprimeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_L2alt=CreateReturnFnMatrix_Disc_DC1(ReturnFn,n_d,n_z,d_gridvals,aprime_grid(aprimeindexesalt),a_grid,z_gridvals_J(:,:,N_j),ReturnFnParamsVec,2);
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            %% Valt (beta) -- capture Policyalt (exponential discounter's choice)
            entireRHS=ReturnMatrix+beta*shiftdim(EV_z,-1);
            [~,maxindexalt]=max(entireRHS,[],2);
            midpointalt=max(min(maxindexalt,n_a-1),2);
            aprimeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_L2alt=CreateReturnFnMatrix_Disc_DC1(ReturnFn,n_d,special_n_z,d_gridvals,aprime_grid(aprimeindexesalt),a_grid,z_val,ReturnFnParamsVec,2);
            ...
            Policyalt(2,:,z_c,N_j)=shiftdim(squeeze(midpointalt(allindalt)),-1);
            Policyalt(3,:,z_c,N_j)=shiftdim(ceil(maxindexL2alt/N_d),-1);
            %% Vtilde (beta0*beta)
            entireRHS=ReturnMatrix+beta0beta*shiftdim(EV_z,-1);
            [~,maxindex]=max(entireRHS,[],2);
            midpoint=max(min(maxindex,n_a-1),2);
            aprimeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
