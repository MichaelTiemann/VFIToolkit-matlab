function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_GI2A_nod_noz_e_raw(n_a,n_e, N_j, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_GI2A_nod_noz_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    % aprime possibilities are n2long-by-n_a2-by-n_a1-by-n_a2-by-n_e
    ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn,n_e,a1prime_grid(a1primeindexesalt),a2_grid, a1_grid, a2_grid,e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
    aprimealt=a1primeindexesalt+N_a1fine*a2ind;
    entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[n2long*N_a2,N_a,N_e]);
    ...
    linidx_upperalt  = n2long*maxindexL2a2alt                             + n2long*N_a2*a12ind + n2long*N_a2*N_a*eind;
    ...
    Valt(:,:,N_j)=shiftdim(Vtempii,1);
    Policyalt(1,:,:,N_j)=midpointalt(maxindexL2a2alt+N_a2*a12ind+N_a2*N_a*eind); % a1prime midpoint
    ...
    entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EVinterp(aprime),[n2long*N_a2,N_a,N_e]);
...
for reverse_j=1:N_j-1
    ...
    EV=sum(Valt(:,:,jj+1).*pi_e_J(:,:,jj+1),2); % naive: continuation is the exponential value fn
    ...
    a1primeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short)'; % aprime points either side of midpoint
    % aprime possibilities are n2long-by-n_a2-by-n_a1-by-n_a2-by-n_e
    ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn,n_e,a1prime_grid(a1primeindexesalt),a2_grid, a1_grid, a2_grid,e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
    aprimealt=a1primeindexesalt+N_a1fine*a2ind;
    entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[n2long*N_a2,N_a,N_e]);
    [Vtempii,maxindexL2alt]=max(entireRHS_iialt,[],1);
    maxindexL2a1alt=rem(maxindexL2alt-1,n2long)+1;
    ...
    Policyalt(1,:,:,jj)=midpointalt(maxindexL2a2alt+N_a2*a12ind+N_a2*N_a*eind); % a1prime midpoint
