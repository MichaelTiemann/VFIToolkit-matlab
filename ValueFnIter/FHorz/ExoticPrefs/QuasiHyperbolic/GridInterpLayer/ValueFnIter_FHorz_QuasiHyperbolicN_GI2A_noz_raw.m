function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicN_GI2A_noz_raw(n_d,n_a, N_j, d_gridvals, a_grid, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_GI2A_noz_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    % aprime possibilities are n_d-by-n2long-by-n_a2-by-n_a1-by-n_a2
    ReturnMatrix_iialt=CreateReturnFnMatrix_Disc_DC2A_noz(ReturnFn,n_d,d_gridvals,a1prime_grid(a1primeindexesalt),a2_grid, a1_grid, a2_grid,ReturnFnParamsVec,2,0);
    aprimealt=a1primeindexesalt+N_a1fine*a2ind;
    entireRHS_iialt=ReturnMatrix_iialt+beta*reshape(EVinterp(aprimealt),[N_d*n2long*N_a2,N_a]);
    ...
    Policyalt(2,:,N_j)=midpointalt(maxindexL2dalt+N_d*(maxindexL2a2alt-1)+N_d*N_a2*a12ind); % a1prime midpoint
    ...
    entireRHS_ii=ReturnMatrix_ii+beta0beta*reshape(EVinterp(aprime),[N_d*n2long*N_a2,N_a]);
...
for reverse_j=1:N_j-1
    ...
    Policyalt(2,:,jj)=midpointalt(maxindexL2dalt+N_d*(maxindexL2a2alt-1)+N_d*N_a2*a12ind); % a1prime midpoint
