function [V, Policy]=ValueFnIter_FHorz_GulPesendorfer_GI1_nod_e_raw(n_a,n_z,n_e,N_j, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            %Calc the max and it's index
            [~,maxindex]=max(ReturnMatrix_e+TemptationMatrix_e,[],1);

            % Most-tempting term: two-stage max of v over the FINE grid, around v's own coarse argmax
            [~,maxindexT]=max(TemptationMatrix_e,[],1);
            midpointT=max(min(maxindexT,n_a-1),2);
            aprimeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short)';
            ...
            TemptationMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,n_z,special_n_e,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,N_j),e_val,TemptationFnParamsVec,2);
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                %Calc the max and it's index
                [~,maxindex]=max(ReturnMatrix_ze+TemptationMatrix_ze,[],1);

                % Most-tempting term: two-stage max of v over the FINE grid, around v's own coarse argmax
                [~,maxindexT]=max(TemptationMatrix_ze,[],1);
                midpointT=max(min(maxindexT,n_a-1),2);
                aprimeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short)';
                ...
                TemptationMatrix_ii_ze=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,special_n_z,special_n_e,aprime_grid(aprimeindexes),a_grid,z_val,e_val,TemptationFnParamsVec,2);
                Ftemp_ii_ze=ReturnMatrix_ii_ze+TemptationMatrix_ii_ze;
                [Vtempii,maxindexL2]=max(Ftemp_ii_ze,[],1);

                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                isInfLower    = (Ftemp_ii_ze(1,     :) == -Inf);
                isInfUpper    = (Ftemp_ii_ze(n2long,:) == -Inf);
                inLowerStrict = (maxindexL2 >= 2)         & (maxindexL2 <= n2short+1);
                inUpperStrict = (maxindexL2 >= n2short+3) & (maxindexL2 <= n2long-1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_e=ReturnMatrix_e+TemptationMatrix_e+DiscountFactorParamsVec*EV;
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_ze=ReturnMatrix_ze+TemptationMatrix_ze+DiscountFactorParamsVec*EV_z;
