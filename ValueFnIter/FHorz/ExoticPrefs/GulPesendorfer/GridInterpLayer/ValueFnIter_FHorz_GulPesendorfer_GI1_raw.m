function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_GI1_raw(n_d,n_a,n_z,N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc(TemptationFn, n_d, n_a, n_z, d_gridvals, a_grid, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            TemptationMatrix_z=CreateReturnFnMatrix_Disc(TemptationFn, n_d, n_a, special_n_z, d_gridvals, a_grid, z_val, TemptationFnParamsVec,1);
            % Calc the max and it's index
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            TemptationMatrix_z=CreateReturnFnMatrix_Disc(TemptationFn, n_d, n_a, special_n_z, d_gridvals, a_grid, z_val, TemptationFnParamsVec,1);
            entireRHS_z=ReturnMatrix_z+TemptationMatrix_z+DiscountFactorParamsVec*shiftdim(EV_z,-1); %*ones(1,N_a,1);
            ...
            TemptationMatrix_ii_z=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,special_n_z,d_gridvals,aprime_grid(aprimeindexes),a_grid,z_val,TemptationFnParamsVec,2);
            Ftemp_ii_z=ReturnMatrix_ii_z+TemptationMatrix_ii_z;
            % aprimez=aprimeindexes;
            entireRHS_ii_z=Ftemp_ii_z+DiscountFactorParamsVec*reshape(EVinterp_z(aprimeindexes(:)),[N_d*n2long,N_a]);
            ...
            linidx_lower = d_ind + N_d*n2long*aind;
            linidx_upper = d_ind + N_d*(n2long-1) + N_d*n2long*aind;
            isInfLower = (Ftemp_ii_z(linidx_lower) == -Inf);
            isInfUpper = (Ftemp_ii_z(linidx_upper) == -Inf);
            inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
            inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
            PolicyL2flag(1,:,z_c,N_j) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
