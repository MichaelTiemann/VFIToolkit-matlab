function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_GI1_e_raw(n_d,n_a,n_z,n_e,N_j, d_gridvals, a_grid, z_gridvals_J, e_gridvals_J,pi_z_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d, n_a, n_z, n_e, d_gridvals, a_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d, n_a, n_z, special_n_e, d_gridvals, a_grid, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
            % Calc the max and it's index
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_ze=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d, n_a, special_n_z, special_n_e, d_gridvals, a_grid, z_val, e_val, TemptationFnParamsVec,1);
                % Calc the max and it's index
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            TemptationMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn,n_d,n_z,special_n_e,d_gridvals,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,N_j),e_val,TemptationFnParamsVec,2);
            Ftemp_ii_e=ReturnMatrix_ii_e+TemptationMatrix_ii_e;
            aprimez=aprimeindexes+n2aprime*zBind;
            entireRHS_ii_e=Ftemp_ii_e+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[N_d*n2long,N_a,N_z]);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_ze=ReturnMatrix_ze+TemptationMatrix_ze+DiscountFactorParamsVec*shiftdim(EV_z,-1);
                ...
                TemptationMatrix_ii_ze=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn,n_d,special_n_z,special_n_e,d_gridvals,aprime_grid(aprimeindexes),a_grid,z_val,e_val,TemptationFnParamsVec,2);
                Ftemp_ii_ze=ReturnMatrix_ii_ze+TemptationMatrix_ii_ze;
                entireRHS_ii_ze=Ftemp_ii_ze+DiscountFactorParamsVec*reshape(EVinterp_z(aprimeindexes(:)),[N_d*n2long,N_a]);
                ...
                linidx_lower = d_ind + N_d*n2long*aind;
                linidx_upper = d_ind + N_d*(n2long-1) + N_d*n2long*aind;
                isInfLower = (Ftemp_ii_ze(linidx_lower) == -Inf);
                isInfUpper = (Ftemp_ii_ze(linidx_upper) == -Inf);
                inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
                inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
                PolicyL2flag(1,:,z_c,e_c,N_j) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d, n_a, n_z, n_e, d_gridvals, a_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
        % (d,aprime,a,z,e)
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d, n_a, n_z, special_n_e, d_gridvals, a_grid, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,1);
            % (d,aprime,a,z)
            ...
            TemptationMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn,n_d,n_z,special_n_e,d_gridvals,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,jj),e_val,TemptationFnParamsVec,2);
            Ftemp_ii_e=ReturnMatrix_ii_e+TemptationMatrix_ii_e;
            aprimez=aprimeindexes+n2aprime*zBind;
            entireRHS_ii_e=Ftemp_ii_e+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[N_d*n2long,N_a,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii_e,[],1);
