function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_GI2A_nod_noz_e_raw(n_a,n_e, N_j, a_grid, e_gridvals_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn,n_e, a1_grid, a2_grid, a1_grid, a2_grid, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1);
    TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_nod(TemptationFn,n_e, a1_grid, a2_grid, a1_grid, a2_grid, e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
    ...
    linidx_upper  = n2long*maxindexL2a2                          + n2long*N_a2*a12ind + n2long*N_a2*N_a*eind;
    ...
    Policy(1,:,:,N_j)=midpoint(maxindexL2a2+N_a2*a12ind+N_a2*N_a*eind); % a1prime midpoint
    ...
    entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprime),[n2long*N_a2,N_a,N_e]);
...
for reverse_j=1:N_j-1
    ...
    ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_e, a1_grid, a2_grid, a1_grid, a2_grid, e_gridvals_J(:,:,jj), ReturnFnParamsVec,1);
    TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_nod(TemptationFn, n_e, a1_grid, a2_grid, a1_grid, a2_grid, e_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
    entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*EV;
    ...
    Policy(1,:,:,jj)=midpoint(maxindexL2a2+N_a2*a12ind+N_a2*N_a*eind); % a1prime midpoint
