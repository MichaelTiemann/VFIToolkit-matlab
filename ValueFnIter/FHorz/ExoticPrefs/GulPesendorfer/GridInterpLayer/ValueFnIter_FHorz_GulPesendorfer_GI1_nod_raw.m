function [V, Policy]=ValueFnIter_FHorz_GulPesendorfer_GI1_nod_raw(n_a,n_z,N_j, a_grid, z_gridvals_J,pi_z_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            TemptationMatrix=CreateReturnFnMatrix_Disc(TemptationFn, 0, n_a, special_n_z, 0, a_grid, z_val, TemptationFnParamsVec,0);
            %Calc the max and it's index
            [~,maxindex]=max(ReturnMatrix+TemptationMatrix,[],1);
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            entireRHS_z=ReturnMatrix_z+TemptationMatrix_z+DiscountFactorParamsVec*EV_z;

            %Calc the max and it's index
            [~,maxindex]=max(entireRHS_z,[],1);

            % Most-tempting term: two-stage max of v over the FINE grid, around v's own coarse argmax
            [~,maxindexT]=max(TemptationMatrix_z,[],1);
            midpointT=max(min(maxindexT,n_a-1),2);
            aprimeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short)';
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn,special_n_z,aprime_grid(aprimeindexesT),a_grid,z_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
