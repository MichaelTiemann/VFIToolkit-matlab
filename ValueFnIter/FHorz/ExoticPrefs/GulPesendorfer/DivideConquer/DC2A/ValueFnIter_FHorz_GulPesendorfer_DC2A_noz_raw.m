function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_DC2A_noz_raw(n_d,n_a,N_j, d_gridvals, a_grid, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_noz(TemptationFn, n_d, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, TemptationFnParamsVec,1,0);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_noz(TemptationFn, n_d, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, TemptationFnParamsVec,1,0);
        ...
        if maxgap(ii)>0
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_noz(TemptationFn, n_d, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, TemptationFnParamsVec,2,0);
            entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_noz(TemptationFn, n_d, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, TemptationFnParamsVec,2,0);
            entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
            % maxindex needs to be reworked:
    ...
    DiscountedEV=DiscountFactorParamsVec*reshape(V_Jplus1,[1,N_a1,N_a2,1,1]); % will autoexand d in 1st-dim
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
            ...
            entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
...
for reverse_j=1:N_j-1
    ...
    DiscountedEV=DiscountFactorParamsVec*reshape(V(:,jj+1),[1,N_a1,N_a2,1,1]); % will autoexand d in 1st-dim
