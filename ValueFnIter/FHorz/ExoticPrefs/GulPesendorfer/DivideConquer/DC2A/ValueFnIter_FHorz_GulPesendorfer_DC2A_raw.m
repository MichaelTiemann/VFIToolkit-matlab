function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_DC2A_raw(n_d,n_a,n_z,N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0);
        MostTempting1=max(reshape(TemptationMatrix_ii,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2,N_z]),[],1); % full joint (d,a1prime,a2prime) grid at the level-1 stations
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0);
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0);
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,:,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
                % maxindex needs to be reworked:
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_gridvals_J(z_c,:,N_j), TemptationFnParamsVec,1,0);
            MostTempting1=max(reshape(TemptationMatrix_ii,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2]),[],1); % full joint (d,a1prime,a2prime) grid at the level-1 stations
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2ind',level1iidiff(ii),1);
                % Most-tempting term over the FULL joint (d,a1prime,a2prime) grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(z_c,:,N_j), TemptationFnParamsVec,1,0);
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(z_c,:,N_j), TemptationFnParamsVec,2,0);
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(z_c,:,N_j), TemptationFnParamsVec,2,0);
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V(curraindex,z_c,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
                    % maxindex needs to be reworked:
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,:,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,:,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_z(reshape(aprime,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V(curraindex,z_c,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                    ...
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_z(reshape(aprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V(curraindex,z_c,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
                    % maxindex needs to be reworked:
                    %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_gridvals_J(:,:,jj), TemptationFnParamsVec,1,0);
        MostTempting1=max(reshape(TemptationMatrix_ii,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2,N_z]),[],1); % full joint (d,a1prime,a2prime) grid at the level-1 stations
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2ind',level1iidiff(ii),1);
            % Most-tempting term over the FULL joint (d,a1prime,a2prime) grid for these a (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), TemptationFnParamsVec,1,0);
            MostTempting_ii=max(reshape(TemptationMatrix_full,[N_d*N_a1*N_a2,level1iidiff(ii)*N_a2,N_z]),[],1);
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2-by-n_z
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0);
                aprimez=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0);
                aprimez=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,:,jj)=shiftdim(Vtempii-MostTempting_ii,1);
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_gridvals_J(z_c,:,jj), TemptationFnParamsVec,1,0);
            MostTempting1=max(reshape(TemptationMatrix_ii,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2]),[],1); % full joint (d,a1prime,a2prime) grid at the level-1 stations
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2ind',level1iidiff(ii),1);
                % Most-tempting term over the FULL joint (d,a1prime,a2prime) grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(z_c,:,jj), TemptationFnParamsVec,1,0);
                MostTempting_ii=max(reshape(TemptationMatrix_full,[N_d*N_a1*N_a2,level1iidiff(ii)*N_a2]),[],1);
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(z_c,:,jj), TemptationFnParamsVec,2,0);
                    aprime=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_z(reshape(aprime,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(z_c,:,jj), TemptationFnParamsVec,2,0);
                    aprime=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_z(reshape(aprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V(curraindex,z_c,jj)=shiftdim(Vtempii-MostTempting_ii,1);
