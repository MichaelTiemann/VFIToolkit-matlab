function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_DC2A_nod_noz_raw(n_a, N_j, a_grid, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    % n-Monotonicity
    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_noz(ReturnFn, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, ReturnFnParamsVec,1);
    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_noz(TemptationFn, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, TemptationFnParamsVec,1);
    MostTempting1=max(reshape(TemptationMatrix_ii,[N_a1*N_a2,vfoptions.level1n*N_a2]),[],1); % full joint (a1prime,a2prime) grid at the level-1 stations
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_nod_noz(TemptationFn, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, TemptationFnParamsVec,1);
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_noz(ReturnFn, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,2);
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_noz(TemptationFn, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, TemptationFnParamsVec,2);
            entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_noz(ReturnFn, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsVec,2);
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_noz(TemptationFn, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, TemptationFnParamsVec,2);
            entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
            % maxindex needs to be reworked:
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
            ...
            entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[1*N_a2,level1iidiff(ii)*N_a2]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
