function [V, Policy]=ValueFnIter_FHorz_GulPesendorfer_DC1_GI1_nod_e_raw(n_a,n_z,n_e,N_j, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    midpointsT_jj=zeros(1,N_a,N_z,N_e,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
elseif vfoptions.lowmemory==1 % loops over e
    midpoints_jj=zeros(1,N_a,N_z,'gpuArray');
    midpointsT_jj=zeros(1,N_a,N_z,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,n_z,n_e,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,N_j),e_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,n_z,n_e,aprime_grid(aprimeindexesT),a_grid,z_gridvals_J(:,:,N_j),e_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,n_z,special_n_e,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,N_j),e_val,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
            aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,n_z,special_n_e,aprime_grid(aprimeindexesT),a_grid,z_gridvals_J(:,:,N_j),e_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,special_n_z,special_n_e,aprime_grid(aprimeindexes),a_grid,z_val,e_val,TemptationFnParamsVec,2);
                Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,special_n_z,special_n_e,aprime_grid(aprimeindexesT),a_grid,z_val,e_val,TemptationFnParamsVec,2);
                MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[n2long,N_a,N_z,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            aprimez=aprimeindexes+n2aprime*zind;
            entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[n2long,N_a,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
            aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,special_n_z,special_n_e,aprime_grid(aprimeindexes),a_grid,z_val,e_val,TemptationFnParamsVec,2);
                Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_z(aprimeindexes(:)),[n2long,N_a]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,n_z,n_e,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,jj),e_gridvals_J(:,:,jj),TemptationFnParamsVec,2);
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        aprimez=aprimeindexes+n2aprime*zind;
        entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[n2long,N_a,N_z,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
        aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,n_z,n_e,aprime_grid(aprimeindexesT),a_grid,z_gridvals_J(:,:,jj),e_gridvals_J(:,:,jj),TemptationFnParamsVec,2);
        MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,n_z,special_n_e,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,jj),e_val,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            aprimez=aprimeindexes+n2aprime*zind;
            entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[n2long,N_a,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
            aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_nod_e(TemptationFn,n_z,special_n_e,aprime_grid(aprimeindexesT),a_grid,z_gridvals_J(:,:,jj),e_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
