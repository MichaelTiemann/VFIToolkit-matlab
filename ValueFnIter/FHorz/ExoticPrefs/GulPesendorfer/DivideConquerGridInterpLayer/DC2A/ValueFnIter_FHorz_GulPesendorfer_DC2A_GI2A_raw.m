function [V, Policy]=ValueFnIter_FHorz_GulPesendorfer_DC2A_GI2A_raw(n_d,n_a,n_z, N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0);
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,3,0);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,n_z,d_gridvals,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,n_z,d_gridvals,a1prime_grid(a1primeindexesT),a2_grid,a1_grid,a2_grid, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0);
        MostTempting=max(TemptationMatrix_Tii,[],1);
        V(:,:,N_j)=shiftdim(Vtempii-MostTempting,1);
        Policy(1,:,:,N_j)=maxindexL2d; % d
        Policy(2,:,:,N_j)=midpoints_jj(maxindexL2d+N_d*(maxindexL2a2-1)+N_d*N_a2*a12ind+N_d*N_a2*N_a*zind); % a1prime midpoint
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_val, TemptationFnParamsVec,1,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_val, TemptationFnParamsVec,1,0);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_jj(:,1,:,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, TemptationFnParamsVec,3,0);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,special_n_z,d_gridvals,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid, z_val, TemptationFnParamsVec,2,0);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            maxindexL2d=rem(maxindexL2-1,N_d)+1;
            maxindexL2a=ceil(maxindexL2/N_d);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
            a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,special_n_z,d_gridvals,a1prime_grid(a1primeindexesT),a2_grid,a1_grid,a2_grid, z_val, TemptationFnParamsVec,2,0);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            V(:,z_c,N_j)=shiftdim(Vtempii-MostTempting,1);
            Policy(1,:,z_c,N_j)=maxindexL2d; % d
            Policy(2,:,z_c,N_j)=midpoints_jj(maxindexL2d+N_d*(maxindexL2a2-1)+N_d*N_a2*a12ind); % a1prime midpoint
            Policy(3,:,z_c,N_j)=maxindexL2a2; % a2prime
            Policy(4,:,z_c,N_j)=maxindexL2a1; % a1primeL2ind
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) in 4th-dim
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_z(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 4th-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_gridvals_J(:,:,jj), TemptationFnParamsVec,1,0);
            [~,maxindexT]=max(TemptationMatrix_full,[],2);
            midpointsT_jj(:,1,:,curraindex,:,:)=maxindexT;
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-n_a2-by-1-by-n_a2-by-n_z
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), TemptationFnParamsVec,3,0);
                aprimez=a1primeindexes+N_a1*a2Bind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_z]));% autoexpand level1iidiff(ii) in 4th-dim
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoints_jj(:,1,:,curraindex,:,:)=maxindex+(loweredge-1);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,n_z,d_gridvals,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid, z_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0);
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprime),[N_d*n2long*N_a2,N_a,N_z]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        Policy(2,:,:,jj)=midpoints_jj(maxindexL2d+N_d*(maxindexL2a2-1)+N_d*N_a2*a12ind+N_d*N_a2*N_a*zind); % a1prime midpoint
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,special_n_z,d_gridvals,a1prime_grid(a1primeindexesT),a2_grid, a1_grid, a2_grid, z_val, TemptationFnParamsVec,2,0);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            V(:,z_c,jj)=shiftdim(Vtempii-MostTempting,1);
            Policy(1,:,z_c,jj)=maxindexL2d; % d
            Policy(2,:,z_c,jj)=midpoints_jj(maxindexL2d+N_d*(maxindexL2a2-1)+N_d*N_a2*a12ind); % a1prime midpoint
            Policy(3,:,z_c,jj)=maxindexL2a2; % a2prime
            Policy(4,:,z_c,jj)=maxindexL2a1; % a1primeL2ind
