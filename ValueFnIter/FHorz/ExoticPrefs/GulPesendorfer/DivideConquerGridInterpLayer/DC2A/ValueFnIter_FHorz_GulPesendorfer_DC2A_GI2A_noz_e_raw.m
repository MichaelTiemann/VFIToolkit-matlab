function [V, Policy]=ValueFnIter_FHorz_GulPesendorfer_DC2A_GI2A_noz_e_raw(n_d,n_a,n_e, N_j, d_gridvals, a_grid, e_gridvals_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    midpointsT_jj=zeros(N_d,1,N_a2,N_a1,N_a2,N_e,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_e, d_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0);
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_gridvals_J(:,:,N_j), TemptationFnParamsVec,3,0);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,n_e,d_gridvals,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid, e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,n_e,d_gridvals,a1prime_grid(a1primeindexesT),a2_grid,a1_grid,a2_grid, e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            % n-Monotonicity
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, special_n_e, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, e_val, ReturnFnParamsVec,1,0);
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_e, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, e_val, TemptationFnParamsVec,1,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_e, d_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, e_val, TemptationFnParamsVec,1,0);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_jj(:,1,:,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_val, ReturnFnParamsVec,3,0);
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_val, TemptationFnParamsVec,3,0);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn,n_d,special_n_e,d_gridvals,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid, e_val, ReturnFnParamsVec,2,0);
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,special_n_e,d_gridvals,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid, e_val, TemptationFnParamsVec,2,0);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            maxindexL2d=rem(maxindexL2-1,N_d)+1;
            maxindexL2a=ceil(maxindexL2/N_d);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
            a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,special_n_e,d_gridvals,a1prime_grid(a1primeindexesT),a2_grid,a1_grid,a2_grid, e_val, TemptationFnParamsVec,2,0);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_e])); % autoexpand level1iidiff(ii) in 4th-dim
        ...
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprime),[N_d*n2long*N_a2,N_a,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_val, TemptationFnParamsVec,3,0);
                    aprime=a1primeindexes+N_a1*a2Bind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 4th-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
            ...
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            aprime=a1primeindexes+N_a1fine*a2ind;
            entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprime),[N_d*n2long*N_a2,N_a]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            maxindexL2d=rem(maxindexL2-1,N_d)+1;
            maxindexL2a=ceil(maxindexL2/N_d);
            maxindexL2a1=rem(maxindexL2a-1,n2long)+1;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_e, d_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, e_gridvals_J(:,:,jj), TemptationFnParamsVec,1,0);
            [~,maxindexT]=max(TemptationMatrix_full,[],2);
            midpointsT_jj(:,1,:,curraindex,:,:)=maxindexT;
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-n_a2-by-1-by-n_a2-by-n_e
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, n_d, n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_gridvals_J(:,:,jj), TemptationFnParamsVec,3,0);
                aprime=a1primeindexes+N_a1*a2Bind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[N_d,(maxgap(ii)+1),N_a2,1,N_a2,N_e])); % autoexpand level1iidiff(ii) in 4th-dim
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoints_jj(:,1,:,curraindex,:,:)=maxindex+(loweredge-1);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,n_e,d_gridvals,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,e_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0);
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        aprime=a1primeindexes+N_a1fine*a2ind;
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprime),[N_d*n2long*N_a2,N_a,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
        a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn,n_d,n_e,d_gridvals,a1prime_grid(a1primeindexesT),a2_grid, a1_grid, a2_grid,e_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0);
        MostTempting=max(TemptationMatrix_Tii,[],1);
