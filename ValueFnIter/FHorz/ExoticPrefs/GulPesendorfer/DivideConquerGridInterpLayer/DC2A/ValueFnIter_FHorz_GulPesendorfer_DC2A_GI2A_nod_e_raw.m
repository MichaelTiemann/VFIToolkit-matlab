function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_DC2A_GI2A_nod_e_raw(n_a,n_z,n_e, N_j, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    midpointsT_jj=zeros(1,N_a2,N_a1,N_a2,N_z,N_e,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
...
elseif vfoptions.lowmemory==1
    ...
    midpointsT_jj=zeros(1,N_a2,N_a1,N_a2,N_z,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
    zind=shiftdim(gpuArray(0:1:N_z-1),-1); % already includes -1
    zBind=shiftdim(gpuArray(0:1:N_z-1),-3); % already includes -1
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, n_e, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,3);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,n_z,n_e,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid,z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,n_z,n_e,a1prime_grid(a1primeindexesT),a2_grid,a1_grid,a2_grid,z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, special_n_e, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, special_n_e, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],1);
                midpointsT_jj(1,:,curraindex,:,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(1,:,ii,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,3);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],1);
                    midpoints_jj(1,:,curraindex,:,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(1,:,ii,:,:);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,n_z,special_n_e,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid,z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            maxindexL2a1=rem(maxindexL2-1,n2long)+1;
            maxindexL2a2=ceil(maxindexL2/n2long);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
            a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,n_z,special_n_e,a1prime_grid(a1primeindexesT),a2_grid,a1_grid,a2_grid,z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, special_n_z, special_n_e, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_val, e_val, TemptationFnParamsVec,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, special_n_z, special_n_e, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_val, e_val, TemptationFnParamsVec,1);
                    [~,maxindexT]=max(TemptationMatrix_full,[],1);
                    midpointsT_jj(1,:,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(1,:,ii,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, special_n_z, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, e_val, TemptationFnParamsVec,3);
                        [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],1);
                        midpoints_jj(1,:,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,:,ii,:);
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,special_n_z,special_n_e,a1prime_grid(a1primeindexes),a2_grid,a1_grid,a2_grid,z_val, e_val, TemptationFnParamsVec,2);
                Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
                maxindexL2a1=rem(maxindexL2-1,n2long)+1;
                maxindexL2a2=ceil(maxindexL2/n2long);
                ...
                midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
                a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,special_n_z,special_n_e,a1prime_grid(a1primeindexesT),a2_grid,a1_grid,a2_grid,z_val, e_val, TemptationFnParamsVec,2);
                MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[(maxgap(ii)+1),N_a2,1,N_a2,N_z,N_e])); % autoexpand level1iidiff(ii) in 3rd-dim
        ...
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprimez),[n2long*N_a2,N_a,N_z,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,3);
                    aprimez=aprimeindexes+N_a1*a2ind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[(maxgap(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) in 3rd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,:,curraindex,:,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(1,:,ii,:,:);
            ...
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            aprimez=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
            entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprimez),[n2long*N_a2,N_a,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            maxindexL2a1=rem(maxindexL2-1,n2long)+1;
            maxindexL2a2=ceil(maxindexL2/n2long);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_z(reshape(aprime,[(maxgap(ii)+1),N_a2,1,N_a2])); % autoexpand level1iidiff(ii) in 3rd-dim
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,:,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,:,ii,:);
                ...
                entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp_z(aprime),[n2long*N_a2,N_a]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                maxindexL2a1=rem(maxindexL2-1,n2long)+1;
                maxindexL2a2=ceil(maxindexL2/n2long);
                % Most-tempting term: fine refinement of v around v's own coarse argmax
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, n_e, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
            [~,maxindexT]=max(TemptationMatrix_full,[],1);
            midpointsT_jj(1,:,curraindex,:,:,:)=maxindexT;
            if maxgap(ii)>0
                loweredge=min(maxindex1(1,:,ii,:,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,3);
                aprimez=aprimeindexes+N_a1*a2ind+N_a*zBind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[(maxgap(ii)+1),N_a2,1,N_a2,N_z,N_e])); % autoexpand level1iidiff(ii) in 3rd-dim
                [~,maxindex]=max(entireRHS_ii,[],1);
                midpoints_jj(1,:,curraindex,:,:,:)=maxindex+(loweredge-1);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,n_z,n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,z_gridvals_J(:,:,jj),e_gridvals_J(:,:,jj),  TemptationFnParamsVec,2);
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        aprimez=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprimez),[n2long*N_a2,N_a,N_z,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
        a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,n_z,n_e,a1prime_grid(a1primeindexesT),a2_grid, a1_grid, a2_grid,z_gridvals_J(:,:,jj),e_gridvals_J(:,:,jj),  TemptationFnParamsVec,2);
        MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, special_n_e, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, special_n_e, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],1);
                midpointsT_jj(1,:,curraindex,:,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(1,:,ii,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn, n_z, special_n_e, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,3);
                    aprimez=aprimeindexes+N_a1*a2ind+N_a*zBind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprimez,[(maxgap(ii)+1),N_a2,1,N_a2,N_z])); % autoexpand level1iidiff(ii) in 3rd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,:,curraindex,:,:)=maxindex+(loweredge-1);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,n_z,special_n_e,a1prime_grid(a1primeindexes),a2_grid, a1_grid, a2_grid,z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            aprimez=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*zBind;
            entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprimez),[n2long*N_a2,N_a,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
            a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A_nod_e(TemptationFn,n_z,special_n_e,a1prime_grid(a1primeindexesT),a2_grid, a1_grid, a2_grid,z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_z;
