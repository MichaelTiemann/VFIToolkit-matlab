function [V, Policy]=ValueFnIter_FHorz_GulPesendorfer_DC1_GI1_nod_raw(n_a,n_z,N_j, a_grid, z_gridvals_J,pi_z_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, n_z, a_grid, a_grid(level1ii), z_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, n_z, a_grid, a_grid(curraindex), z_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, n_z, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_gridvals_J(:,:,N_j), TemptationFnParamsVec,2);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn,n_z,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn,n_z,aprime_grid(aprimeindexesT),a_grid,z_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            TemptationMatrix_ii_z=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, special_n_z, a_grid, a_grid(level1ii), z_val, TemptationFnParamsVec,1);

            %Calc the max and it's index
            [~,maxindex1]=max(ReturnMatrix_ii_z+TemptationMatrix_ii_z,[],1);

            % Just keep the 'midpoint' version of maxindex1 [as GI]
            midpoints_jj(1,level1ii)=maxindex1;
            % v's own coarse argmax at the level-1 stations (full aprime grid)
            [~,maxindexT1]=max(TemptationMatrix_ii_z,[],1);
            midpointsT_jj(1,level1ii)=maxindexT1;
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, special_n_z, a_grid, a_grid(curraindex), z_val, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],1);
                midpointsT_jj(1,curraindex)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(1,ii),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii_z=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, special_n_z, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, TemptationFnParamsVec,2);
                    [~,maxindex]=max(ReturnMatrix_ii_z+TemptationMatrix_ii_z,[],1);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn,special_n_z,aprime_grid(aprimeindexes),a_grid,z_val,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
            aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn,special_n_z,aprime_grid(aprimeindexesT),a_grid,z_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*EV(reshape(aprimez,[(maxgap(ii)+1),1,N_z]));  % autoexpand the level1iidiff(ii) in 2nd-dim
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, special_n_z, a_grid, a_grid(level1ii), z_val, TemptationFnParamsVec,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, special_n_z, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, TemptationFnParamsVec,2);
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*EV_z(aprimeindexes); % autoexpand the level1iidiff(ii) in 2nd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,curraindex)=shiftdim(maxindex+(loweredge-1),1);
                else
                    loweredge=maxindex1(1,ii);
            ...
            TemptationMatrix_ii_z=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn,special_n_z,aprime_grid(aprimeindexes),a_grid,z_val,TemptationFnParamsVec,2);
            Ftemp_ii_z=ReturnMatrix_ii_z+TemptationMatrix_ii_z;
            entireRHS_ii_z=Ftemp_ii_z+DiscountFactorParamsVec*reshape(EVinterp_z(aprimeindexes(:)),[n2long,N_a]);
            ...
            Policy(2,:,z_c,N_j)=shiftdim(maxindexL2,-1); % aprimeL2ind
            % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
            isInfLower    = (Ftemp_ii_z(1,     :) == -Inf);
            isInfUpper    = (Ftemp_ii_z(n2long,:) == -Inf);
            inLowerStrict = (maxindexL2 >= 2)         & (maxindexL2 <= n2short+1);
            inUpperStrict = (maxindexL2 >= n2short+3) & (maxindexL2 <= n2long-1);
            PolicyL2flag(1,:,z_c,N_j) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
        end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, n_z, a_grid, a_grid(level1ii), z_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL aprime grid for these a (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, n_z, a_grid, a_grid(curraindex), z_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
            [~,maxindexT]=max(TemptationMatrix_full,[],1);
            midpointsT_jj(1,curraindex,:)=maxindexT;
            if maxgap(ii)>0
                loweredge=min(maxindex1(1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn, n_z, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_gridvals_J(:,:,jj), TemptationFnParamsVec,2);
                aprimez=aprimeindexes+N_a*zind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*EV(reshape(aprimez,[(maxgap(ii)+1),1,N_z])); % autoexpand the level1iidiff(ii) in 2nd-dim
                [~,maxindex]=max(entireRHS_ii,[],1);
                midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_nod(TemptationFn,n_z,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,jj),TemptationFnParamsVec,2);
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        aprimez=aprimeindexes+n2aprime*zind;
        entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[n2long,N_a,N_z]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
