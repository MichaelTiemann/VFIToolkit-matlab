function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_nod1_e_raw(n_d2,n_a,n_z,n_semiz, n_e,N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
% FULL (d2,aprime) choice set: a per-d2 full max is collected alongside each inner solve
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d2, n_a, n_bothz, n_e, d2_gridvals, a_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,0);
        MostTempting=max(TemptationMatrix,[],1); % full (d2,aprime) choice set
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d2, n_a, n_bothz, special_n_e, d2_gridvals, a_grid, bothz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,0);
            MostTempting_e=max(TemptationMatrix_e,[],1); % full (d2,aprime) choice set
            % Calc the max and it's index
            [Vtemp,maxindex]=max(ReturnMatrix_e+TemptationMatrix_e,[],1);
    ...
    elseif vfoptions.lowmemory==2 % outer z / inner e, vectorize semiz
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_ze=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d2, n_a, [n_semiz,special_n_z], special_n_e, d2_gridvals, a_grid, z_valblock, e_val, TemptationFnParamsVec,0);
                MostTempting_ze=max(TemptationMatrix_ze,[],1); % full (d2,aprime) choice set
                [Vtemp,maxindex]=max(ReturnMatrix_ze+TemptationMatrix_ze,[],1);
                V(:,semizblock,e_c,N_j)=Vtemp-MostTempting_ze;
    ...
    elseif vfoptions.lowmemory==3 % joint bothz, inner e
        ...
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_ze=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d2, n_a, special_n_bothz, special_n_e, d2_gridvals, a_grid, z_val, e_val, TemptationFnParamsVec,0);
                MostTempting_ze=max(TemptationMatrix_ze,[],1); % full (d2,aprime) choice set
                [Vtemp,maxindex]=max(ReturnMatrix_ze+TemptationMatrix_ze,[],1);
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            MostTempting_ford2_jj(:,:,:,d2_c)=shiftdim(max(TemptationMatrix_d2,[],1),1); % full aprime for this d2
            ...
            entireRHS=ReturnMatrix_d2+TemptationMatrix_d2+DiscountFactorParamsVec*EV_d2; %*repmat(entireEV,1,N_a,1,N_e);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                MostTempting_ford2_jj(:,:,e_c,d2_c)=shiftdim(max(TemptationMatrix_d2e,[],1),1); % full aprime for this d2

                entireRHS_d2e=ReturnMatrix_d2e+TemptationMatrix_d2e+DiscountFactorParamsVec*EV_d2; %.*ones(1,N_a,1);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    MostTempting_ford2_jj(:,semizblock,e_c,d2_c)=shiftdim(max(TemptationMatrix_d2ze,[],1),1); % full aprime for this d2
                    entireRHS_ze=ReturnMatrix_d2ze+TemptationMatrix_d2ze+DiscountFactorParamsVec*EV_d2z;
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    MostTempting_ford2_jj(:,z_c,e_c,d2_c)=max(TemptationMatrix_d2ze,[],1); % full aprime for this d2
                    entireRHS_ze=ReturnMatrix_d2ze+TemptationMatrix_d2ze+DiscountFactorParamsVec*EV_d2z;
                    [Vtemp,maxindex]=max(entireRHS_ze,[],1);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                % (d,aprime,a,z)
                TemptationMatrix_e=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d2, n_a, n_bothz, special_n_e, d2_val, a_grid, bothz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,0);
                MostTempting_ford2_jj(:,:,e_c,d2_c)=shiftdim(max(TemptationMatrix_e,[],1),1); % full aprime for this d2

                entireRHS_e=ReturnMatrix_e+TemptationMatrix_e+DiscountFactorParamsVec*EV_d2; %.*ones(1,N_a,1);
