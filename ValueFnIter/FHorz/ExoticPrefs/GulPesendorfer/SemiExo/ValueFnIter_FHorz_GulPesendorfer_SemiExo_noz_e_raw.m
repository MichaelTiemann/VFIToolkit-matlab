function [V,Policy3]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_noz_e_raw(n_d1,n_d2,n_a,n_semiz,n_e, N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d, n_a, n_semiz, n_e, d_gridvals, a_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,0);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d, n_a, n_semiz, special_n_e, d_gridvals, a_grid, semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,0);
            MostTempting_e=max(TemptationMatrix_e,[],1); % full (d1,d2,aprime) choice set
            %Calc the max and it's index
    ...
    elseif vfoptions.lowmemory==2
        ...
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_semize=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d, n_a, special_n_semiz, special_n_e, d_gridvals, a_grid, semiz_val, e_val, TemptationFnParamsVec,0);
                MostTempting_semize=max(TemptationMatrix_semize,[],1); % full (d1,d2,aprime) choice set
                %Calc the max and it's index
                [Vtemp,maxindex]=max(ReturnMatrix_semize+TemptationMatrix_semize,[],1);
                V(:,semiz_c,e_c,N_j)=Vtemp-MostTempting_semize;
                d_ind=shiftdim(rem(maxindex-1,N_d)+1,-1);
                Policy3(1,:,semiz_c,e_c,N_j)=shiftdim(rem(d_ind-1,N_d1)+1,-1);
                Policy3(2,:,semiz_c,e_c,N_j)=shiftdim(ceil(d_ind/N_d1),-1);
                Policy3(3,:,semiz_c,e_c,N_j)=shiftdim(ceil(maxindex/N_d),-1);
            end
        end
    end
else
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            % (d,aprime,a,z)
            TemptationMatrix_d2=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d, n_a, n_semiz,n_e, d12c_gridvals, a_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,0);
            ...
            entireRHS=ReturnMatrix_d2+TemptationMatrix_d2+DiscountFactorParamsVec*entireEV_d2; %repmat(entireEV,1,N_a,1);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                % (d,aprime,a,z)
                TemptationMatrix_d2e=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d, n_a, n_semiz,special_n_e, d12c_gridvals, a_grid, semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,0);
                MostTempting_ford2_jj(:,:,e_c,d2_c)=shiftdim(max(TemptationMatrix_d2e,[],1),1); % full (d1,aprime) for this d2

                entireRHSe=ReturnMatrix_d2e+TemptationMatrix_d2e+DiscountFactorParamsVec*entireEV_d2; %repmat(entireEV,1,N_a,1);
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    TemptationMatrix_d2semize=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d, n_a, special_n_semiz, special_n_e, d12c_gridvals, a_grid, semiz_val, e_val, TemptationFnParamsVec,0);
                    MostTempting_ford2_jj(:,semiz_c,e_c,d2_c)=max(TemptationMatrix_d2semize,[],1); % full (d1,aprime) for this d2

                    entireRHS=ReturnMatrix_d2semize+TemptationMatrix_d2semize+DiscountFactorParamsVec*entireEV_d2semiz;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            % (d,aprime,a,z)
            TemptationMatrix_d2=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d, n_a, n_semiz,n_e, d12c_gridvals, a_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,0);
            MostTempting_ford2_jj(:,:,:,d2_c)=shiftdim(max(TemptationMatrix_d2,[],1),1); % full (d1,aprime) for this d2
            ...
            entireEV_d2=repelem(EV_d2,N_d1,1,1);
            entireRHS=ReturnMatrix_d2+TemptationMatrix_d2+DiscountFactorParamsVec*entireEV_d2; % autofill the N_a and N_e
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_d2z=CreateReturnFnMatrix_Disc_e(ReturnFn, special_n_d, n_a, n_semiz, special_n_e, d12c_gridvals, a_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,0);
                TemptationMatrix_d2z=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d, n_a, n_semiz, special_n_e, d12c_gridvals, a_grid, semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,0);
                MostTempting_ford2_jj(:,:,e_c,d2_c)=max(TemptationMatrix_d2z,[],1); % full (d1,aprime) for this d2

                entireRHS_z=ReturnMatrix_d2z+TemptationMatrix_d2z+DiscountFactorParamsVec*entireEV_d2;
