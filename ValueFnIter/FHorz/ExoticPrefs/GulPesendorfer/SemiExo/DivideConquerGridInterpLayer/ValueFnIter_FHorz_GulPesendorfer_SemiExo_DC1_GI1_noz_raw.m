function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC1_GI1_noz_raw(n_d1,n_d2,n_a,n_semiz,N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz,N_d2,'gpuArray'); % per-d2 full (d1,aprime-fine) max of the temptation
% Preallocate
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    midpointsT_Nj=zeros(N_d,1,N_a,N_semiz,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, n_semiz, d_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, n_semiz, d_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, n_semiz, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,3);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,n_semiz,d_gridvals,aprime_grid(aprimeindexes),a_grid,semiz_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,n_semiz,d_gridvals,aprime_grid(aprimeindexesT),a_grid,semiz_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
    ...
    elseif vfoptions.lowmemory==1 % loop semiz
        ...
        for semiz_c=1:N_semiz
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, special_n_semiz, d_gridvals, a_grid, a_grid(level1ii), semiz_val, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer
            [~,maxindex1]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, special_n_semiz, d_gridvals, a_grid, a_grid(curraindex), semiz_val, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_Nj(:,1,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, special_n_semiz, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, TemptationFnParamsVec,3);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,special_n_semiz,d_gridvals,aprime_grid(aprimeindexes),a_grid,semiz_val,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_Nj=max(min(midpointsT_Nj,n_a-1),2);
            aprimeindexesT=(midpointsT_Nj+(midpointsT_Nj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,special_n_semiz,d_gridvals,aprime_grid(aprimeindexesT),a_grid,semiz_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    for d2_c=1:N_d2
        ...
        TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL aprime grid for these a (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            [~,maxindexT]=max(TemptationMatrix_full,[],2);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,3);
                aprimez=aprimeindexes+N_a*semizind2;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,(maxgap(ii)+1),1,N_semiz]);  % autoexpand level1iidiff(ii) in 3rd-dim
        ...
        TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,2);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, aprime_grid(aprimeindexesT), a_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,2);
        MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==1 % loop semiz
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_semiz, d12c_gridvals, a_grid, a_grid(level1ii), semiz_val, TemptationFnParamsVec,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_semiz, d12c_gridvals, a_grid, a_grid(curraindex), semiz_val, TemptationFnParamsVec,1);
                    [~,maxindexT]=max(TemptationMatrix_full,[],2);
                    midpointsT_jj(:,1,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_semiz, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, TemptationFnParamsVec,3);
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2z(aprimeindexes),[N_d1,(maxgap(ii)+1)]);  % autoexpand curraindex in 3rd-dim
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_semiz, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_val, TemptationFnParamsVec,2);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimeindexes),[N_d1*n2long,N_a]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);

                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_semiz, d12c_gridvals, aprime_grid(aprimeindexesT), a_grid, semiz_val, TemptationFnParamsVec,2);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting_ford2_jj(:,semiz_c,d2_c)=shiftdim(MostTempting,1); % full (d1,aprime-fine) for this d2
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
        ...
        TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
        entireRHS_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii+DiscountFactorParamsVec*shiftdim(EV_d2,-1);
        % Treat standard problem as just being the first layer
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL aprime grid for these a (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
            [~,maxindexT]=max(TemptationMatrix_full,[],2);
            midpointsT_jj(:,1,curraindex,:)=maxindexT;
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,3);
                aprimez=aprimeindexes+N_a*semizind2;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,(maxgap(ii)+1),1,N_semiz]);  % autoexpand level1iidiff(ii) in 3rd-dim
                [~,maxindex]=max(entireRHS_ii,[],2); % Note: only over aprime (not over d1)
                midpoints_jj(:,1,curraindex,:)=maxindex+(loweredge-1);
        ...
        TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,2);
        Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
        aprimez=aprimeindexes+n2aprime*semizind2; % the current aprime
        entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_semiz]);
        [Vtemp,maxindex]=max(entireRHS_ii,[],1);
        ...
        midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
        aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_semiz, d12c_gridvals, aprime_grid(aprimeindexesT), a_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,2);
        MostTempting=max(TemptationMatrix_Tii,[],1);
        MostTempting_ford2_jj(:,:,d2_c)=shiftdim(MostTempting,1); % full (d1,aprime-fine) for this d2
