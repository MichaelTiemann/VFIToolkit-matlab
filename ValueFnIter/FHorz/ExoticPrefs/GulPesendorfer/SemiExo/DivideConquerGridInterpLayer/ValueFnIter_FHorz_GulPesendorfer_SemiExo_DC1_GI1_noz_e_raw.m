function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC1_GI1_noz_e_raw(n_d1,n_d2,n_a,n_semiz,n_e, N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz,N_e,N_d2,'gpuArray'); % per-d2 full (d1,aprime-fine) max of the temptation
if vfoptions.lowmemory==0
    midpoints_jj=zeros(N_d1,1,N_a,N_semiz,N_e,'gpuArray');
    midpointsT_jj=zeros(N_d1,1,N_a,N_semiz,N_e,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        midpoints_Nj=zeros(N_d,1,N_a,N_semiz,N_e,'gpuArray'); % N_d
        midpointsT_Nj=zeros(N_d,1,N_a,N_semiz,N_e,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d, n_semiz, n_e, d_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
        % Treat standard problem as just being the first layer: we want aprime conditional on (d,1,a,z,e)
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d, n_semiz, n_e, d_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, n_semiz, n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,6);
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d, n_semiz, n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,6);
        ...
        allind=d_ind+N_d*aind+N_d*N_a*semizind+N_d*N_a*N_semiz*eind; % midpoint_ford2_Nj is n_d-by-1-by-n_a-by-n_semiz-by-n_e
    ...
    elseif vfoptions.lowmemory==1
        midpoints_Nj=zeros(N_d,1,N_a,N_semiz,'gpuArray'); % N_d
        midpointsT_Nj=zeros(N_d,1,N_a,N_semiz,'gpuArray'); % v's own coarse argmax (for the most-tempting term)

        for e_c=1:N_e
            e_val=e_gridvals_J(e_c,:,N_j);
            % n-Monotonicity
            ReturnMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, n_semiz, special_n_e, d_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1);
            TemptationMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d, n_semiz, special_n_e, d_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer: we want aprime conditional on (d,1,a,z,e)
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d, n_semiz, special_n_e, d_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_Nj(:,1,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, n_semiz, special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,6);
                    TemptationMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d, n_semiz, special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,6);
    ...
    elseif vfoptions.lowmemory==2
        midpoints_Nj=zeros(N_d,1,N_a,'gpuArray'); % N_d
        ...
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                % n-Monotonicity
                ReturnMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, special_n_semiz, special_n_e, d_gridvals, a_grid, a_grid(level1ii), semiz_val, e_val, ReturnFnParamsVec,1);
                TemptationMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d, special_n_semiz, special_n_e, d_gridvals, a_grid, a_grid(level1ii), semiz_val, e_val, TemptationFnParamsVec,1);
                % Treat standard problem as just being the first layer: we want aprime conditional on (d,1,a,z,e)
                [~,maxindex1]=max(ReturnMatrix_ii_e+TemptationMatrix_ii_e,[],2);
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d, special_n_semiz, special_n_e, d_gridvals, a_grid, a_grid(curraindex), semiz_val, e_val, TemptationFnParamsVec,1);
                    [~,maxindexT]=max(TemptationMatrix_full,[],2);
                    midpointsT_Nj(:,1,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        ReturnMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d, special_n_semiz, special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, ReturnFnParamsVec,6);
                        TemptationMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d, special_n_semiz, special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, TemptationFnParamsVec,6);
                        [~,maxindex]=max(ReturnMatrix_ii_e+TemptationMatrix_ii_e,[],2);
                        midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                ...
                Policy(3,:,semiz_c,e_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1); % midpoint
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,6);
                    aprimez=aprimeindexes+N_a*semizind2;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,(maxgap(ii)+1),1,N_semiz,N_e]); % autoexpand level1iidiff(ii) in 3rd-dim
            ...
            entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_semiz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_d2iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
                entireRHS_ii=ReturnMatrix_d2iie+TemptationMatrix_d2iie+DiscountFactorParamsVec*shiftdim(EV_d2,-1);
                % First, we want aprime conditional on (d,1,a,z,e)
                [~,maxindex1]=max(entireRHS_ii,[],2);
                ...
                % v's own coarse argmax at the level-1 stations (full aprime grid)
                [~,maxindexT1]=max(TemptationMatrix_d2iie,[],2);
                midpointsT_jj(:,1,level1ii,:)=maxindexT1;
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
                    [~,maxindexT]=max(TemptationMatrix_full,[],2);
                    midpointsT_jj(:,1,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        aprimeindexes=loweredge+(0:1:maxgap(ii));
                        % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_semiz
                        ReturnMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,6);
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,6);
                        aprimez=aprimeindexes+N_a*semizind2;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,(maxgap(ii)+1),1,N_semiz]); % autoexpand level1iidiff(ii) in 3rd-dim
                        [~,maxindex]=max(entireRHS_ii,[],2);
                        midpoints_jj(:,1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                ...
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                aprimez=aprimeindexes+n2aprime*semizind2; % the current aprime
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_semiz]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    TemptationMatrix_d2iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid, a_grid(level1ii), semiz_val, e_val, TemptationFnParamsVec,1);
                    entireRHS_ii=ReturnMatrix_d2iie+TemptationMatrix_d2iie+DiscountFactorParamsVec*shiftdim(EV_d2semiz,-1);
                    % First, we want aprime conditional on (d,1,a,z,e)
                    [~,maxindex1]=max(entireRHS_ii,[],2);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                        % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                        TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid, a_grid(curraindex), semiz_val, e_val, TemptationFnParamsVec,1);
                        [~,maxindexT]=max(TemptationMatrix_full,[],2);
                        midpointsT_jj(:,1,curraindex,:)=maxindexT;
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                            ...
                            aprimeindexes=loweredge+(0:1:maxgap(ii));
                            % aprime possibilities are n_d-by-maxgap(ii)+1
                            ReturnMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, ReturnFnParamsVec,6);
                            TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, TemptationFnParamsVec,6);
                            aprimez=aprimeindexes;
                            entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2semiz(aprimez),[N_d1,(maxgap(ii)+1),1,1]); % autoexpand level1iidiff(ii) in 3rd-dim
                            [~,maxindex]=max(entireRHS_ii,[],2);
                            midpoints_jj(:,1,curraindex,:)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,ii,:);
                    ...
                    aprimeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint
                    % aprime possibilities are n_d-by-n2long-by-n_a
                    ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_val, e_val, ReturnFnParamsVec,2);
                    TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, semiz_val, e_val, TemptationFnParamsVec,2);
                    Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                    aprimez=aprimeindexes; % the current aprime
                    entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2semiz(aprimez),[N_d1*n2long,N_a,1]);
                    [Vtemp,maxindex]=max(entireRHS_ii,[],1);
                    ...
                    MostTempting=max(TemptationMatrix_Tii,[],1);
                    MostTempting_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(MostTempting,1); % full (d1,aprime-fine) for this d2
                    ...
                    midpoint_ford2_jj(:,semiz_c,e_c,d2_c)=squeeze(midpoints_jj(allind));
                    ...
                    inLowerStrict = (L2offset_d2 >= 2)         & (L2offset_d2 <= n2short+1);
                    inUpperStrict = (L2offset_d2 >= n2short+3) & (L2offset_d2 <= n2long-1);
                    PolicyL2flag_ford2_jj(:,semiz_c,e_c,d2_c) = squeeze(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper));
                end
            end
        end
        % Now we just max over d2, and keep the policy that corresponded to that (including modify the policy to include the d2 decision)
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz,n_e, d12c_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
            entireRHS_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii+DiscountFactorParamsVec*shiftdim(EV_d2,-1);
            % First, we want aprime conditional on (d,1,a,z,e)
            [~,maxindex1]=max(entireRHS_ii,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz,n_e, d12c_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_jj(:,1,curraindex,:,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    % loweredge is n_d1-by-1-by-1-by-n_semiz-by-n_e
                    aprimeindexes=loweredge+(0:1:maxgap(ii));
                    % aprime possibilities are n_d1-by-maxgap(ii)+1-by-1-by-n_semiz-by-n_e
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,6);
                    aprimez=aprimeindexes+N_a*semizind2;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,(maxgap(ii)+1),1,N_semiz,N_e]); % autoexpand level1iidiff(ii) in 3rd-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,curraindex,:,:)=maxindex+(loweredge-1);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_d2iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,1);
                    [~,maxindexT]=max(TemptationMatrix_full,[],2);
                    midpointsT_jj(:,1,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        aprimeindexes=loweredge+(0:1:maxgap(ii));
                        % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_semiz
                        ReturnMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,6);
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,6);
                        aprimez=aprimeindexes+N_a*semizind2;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,(maxgap(ii)+1),1,N_semiz]); % autoexpand level1iidiff(ii) in 3rd-dim
                        [~,maxindex]=max(entireRHS_ii,[],2);
                        midpoints_jj(:,1,curraindex,:)=maxindex+(loweredge-1);
