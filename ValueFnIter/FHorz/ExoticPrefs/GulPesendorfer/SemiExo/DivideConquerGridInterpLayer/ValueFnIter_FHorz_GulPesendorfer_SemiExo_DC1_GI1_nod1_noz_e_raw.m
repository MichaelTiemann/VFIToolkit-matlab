function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC1_GI1_nod1_noz_e_raw(n_d2,n_a,n_semiz,n_e, N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz,N_e,N_d2,'gpuArray'); % per-d2 full (aprime-fine) max of the temptation
% Preallocate
if vfoptions.lowmemory==0
    midpoints_jj=zeros(1,N_a,N_semiz,N_e,'gpuArray');
    midpointsT_jj=zeros(1,N_a,N_semiz,N_e,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        midpointsT_Nj=zeros(N_d2,1,N_a,N_semiz,N_e,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d2, n_semiz, n_e, d2_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d2, n_semiz, n_e, d2_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d2, n_semiz, n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,6);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn,n_d2,n_semiz,n_e,d2_gridvals,aprime_grid(aprimeindexes),a_grid,semiz_gridvals_J(:,:,N_j),e_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn,n_d2,n_semiz,n_e,d2_gridvals,aprime_grid(aprimeindexesT),a_grid,semiz_gridvals_J(:,:,N_j),e_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            TemptationMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d2, n_semiz, special_n_e, d2_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d2, n_semiz, special_n_e, d2_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_Nj(:,1,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d2, n_semiz, special_n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,6);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn,n_d2,n_semiz,special_n_e,d2_gridvals,aprime_grid(aprimeindexes),a_grid,semiz_gridvals_J(:,:,N_j),e_val,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_Nj=max(min(midpointsT_Nj,n_a-1),2);
            aprimeindexesT=(midpointsT_Nj+(midpointsT_Nj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn,n_d2,n_semiz,special_n_e,d2_gridvals,aprime_grid(aprimeindexesT),a_grid,semiz_gridvals_J(:,:,N_j),e_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==2
        ...
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_ii_semize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d2, special_n_semiz, special_n_e, d2_gridvals, a_grid, a_grid(level1ii), semiz_val, e_val, TemptationFnParamsVec,1);
                % Treat standard problem as just being the first layer
                [~,maxindex1]=max(ReturnMatrix_ii_semize+TemptationMatrix_ii_semize,[],2);

                % Just keep the 'midpoint' version of maxindex1 [as GI]
                midpoints_Nj(:,1,level1ii)=maxindex1;

                % v's own coarse argmax at the level-1 stations (full aprime grid)
                [~,maxindexT1]=max(TemptationMatrix_ii_semize,[],2);
                midpointsT_Nj(:,1,level1ii)=maxindexT1;
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d2, special_n_semiz, special_n_e, d2_gridvals, a_grid, a_grid(curraindex), semiz_val, e_val, TemptationFnParamsVec,1);
                    [~,maxindexT]=max(TemptationMatrix_full,[],2);
                    midpointsT_Nj(:,1,curraindex)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_ii_semize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, n_d2, special_n_semiz, special_n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, TemptationFnParamsVec,6);
                        [~,maxindex]=max(ReturnMatrix_ii_semize+TemptationMatrix_ii_semize,[],2);
                        midpoints_Nj(:,1,curraindex)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                        midpoints_Nj(:,1,curraindex)=repelem(loweredge,1,1,length(curraindex),1); % unfortunately doesn't autofill
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn,n_d2,special_n_semiz,special_n_e,d2_gridvals,aprime_grid(aprimeindexes),a_grid,semiz_val,e_val,TemptationFnParamsVec,2);
                Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointsT_Nj=max(min(midpointsT_Nj,n_a-1),2);
                aprimeindexesT=(midpointsT_Nj+(midpointsT_Nj-1)*n2short)+(-n2short-1:1:1+n2short);
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn,n_d2,special_n_semiz,special_n_e,d2_gridvals,aprime_grid(aprimeindexesT),a_grid,semiz_val,e_val,TemptationFnParamsVec,2);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                ...
                inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
                inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
                PolicyL2flag(1,:,semiz_c,e_c,N_j) = shiftdim(squeeze(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper)),-1);
            end
        end
    end
else
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,4);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,4);
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
                    aprimez=aprimeindexes+N_a*semizind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[(maxgap(ii)+1),1,N_semiz,N_e]); % autoexpand level1iidiff(ii) in 2nd-dim
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
            Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
            aprimez=aprimeindexes+n2aprime*semizind; % the current aprime
            entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimez),[n2long,N_a,N_semiz,N_e]);
            ...
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, aprime_grid(aprimeindexesT), a_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_d2iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,4);
                entireRHS_d2iie=ReturnMatrix_d2iie+TemptationMatrix_d2iie+DiscountFactorParamsVec*EV_d2;
                % Treat standard problem as just being the first layer
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,4);
                    [~,maxindexT]=max(TemptationMatrix_full,[],1);
                    midpointsT_jj(1,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,5);
                        aprimez=aprimeindexes+N_a*semizind;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[(maxgap(ii)+1),1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,ii,:);
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,5);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                aprimez=aprimeindexes+n2aprime*semizind; % the current aprime
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimez),[n2long,N_a,N_semiz]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);

                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexesT), a_grid, semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,5);
                MostTempting=max(TemptationMatrix_Tii,[],1);
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    TemptationMatrix_d2semiie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_semiz, special_n_e, d2_val, a_grid, a_grid(level1ii), semiz_val, e_val, TemptationFnParamsVec,4);
                    entireRHS_d2semiie=ReturnMatrix_d2semiie+TemptationMatrix_d2semiie+DiscountFactorParamsVec*EV_d2semiz;
                    % Treat standard problem as just being the first layer
                    ...
                    % v's own coarse argmax at the level-1 stations (full aprime grid)
                    [~,maxindexT1]=max(TemptationMatrix_d2semiie,[],1);
                    midpointsT_jj(1,level1ii,semiz_c,e_c)=maxindexT1;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                        % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                        TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_semiz, special_n_e, d2_val, a_grid, a_grid(curraindex), semiz_val, e_val, TemptationFnParamsVec,4);
                        [~,maxindexT]=max(TemptationMatrix_full,[],1);
                        midpointsT_jj(1,curraindex,semiz_c,e_c)=maxindexT;
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                            ...
                            TemptationMatrix_iisemize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_semiz, special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, TemptationFnParamsVec,5);
                            entireRHS_ii=ReturnMatrix_iisemize+TemptationMatrix_iisemize+DiscountFactorParamsVec*reshape(EV_d2semiz(aprimeindexes),[(maxgap(ii)+1),1]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [~,maxindex]=max(entireRHS_ii,[],1);
                    ...
                    TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_val, e_val, TemptationFnParamsVec,5);
                    Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                    entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2semiz(aprimeindexes),[n2long,N_a]);
                    [Vtemp,maxindex]=max(entireRHS_ii,[],1);

                    % Most-tempting term: fine refinement of v around v's own coarse argmax
                    midpointsT_semize=max(min(midpointsT_jj(1,:,semiz_c,e_c),n_a-1),2); % slice to the current (semiz,e), mirroring midpoints_semize
                    aprimeindexesT=(midpointsT_semize+(midpointsT_semize-1)*n2short)+(-n2short-1:1:1+n2short)';
                    TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexesT), a_grid, semiz_val, e_val, TemptationFnParamsVec,5);
                    MostTempting=max(TemptationMatrix_Tii,[],1);
                    MostTempting_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz,n_e, d2_val, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,4);
            entireRHS_d2ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii+DiscountFactorParamsVec*EV_d2;
            % Treat standard problem as just being the first layer
            [~,maxindex1]=max(entireRHS_d2ii,[],1); % no d1
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz,n_e, d2_val, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,4);
                [~,maxindexT]=max(TemptationMatrix_full,[],1);
                midpointsT_jj(1,curraindex,:,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(1,ii,:,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
                    aprimez=aprimeindexes+N_a*semizind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[(maxgap(ii)+1),1,N_semiz,N_e]); % autoexpand level1iidiff(ii) in 2nd-dim
                    [~,maxindex]=max(entireRHS_ii,[],1);
                    midpoints_jj(1,curraindex,:,:)=maxindex+(loweredge-1);
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
            Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
            aprimez=aprimeindexes+n2aprime*semizind; % the current aprime
            entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimez),[n2long,N_a,N_semiz,N_e]);
            [Vtemp,maxindex]=max(entireRHS_ii,[],1);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
            aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, aprime_grid(aprimeindexesT), a_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting_ford2_jj(:,:,:,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_d2iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,4);
                entireRHS_d2iie=ReturnMatrix_d2iie+TemptationMatrix_d2iie+DiscountFactorParamsVec*EV_d2;
                % Treat standard problem as just being the first layer
                [~,maxindex1]=max(entireRHS_d2iie,[],1); % no d1
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,4);
                    [~,maxindexT]=max(TemptationMatrix_full,[],1);
                    midpointsT_jj(1,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,5);
                        aprimez=aprimeindexes+N_a*semizind;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[(maxgap(ii)+1),1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,5);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                aprimez=aprimeindexes+n2aprime*semizind; % the current aprime
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimez),[n2long,N_a,N_semiz]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);
                ...
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexesT), a_grid, semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,5);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting_ford2_jj(:,:,e_c,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2
