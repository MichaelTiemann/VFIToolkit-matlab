function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC2A_GI2A_nod1_noz_raw(n_d2, n_a, n_semiz, N_j, d2_gridvals, a_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2=zeros(N_a,N_semiz,N_d2,'gpuArray'); % per-d2 full (a1prime-fine,a2prime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
  ...
  if vfoptions.lowmemory==0
    for d2_c=1:N_d2
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 1, 0);
        [~,maxindex1]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2); % max over a1prime
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 1, 0);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(curraindex), a2_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 1, 0);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 2, 0);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1prime_grid(a1primeindexesT), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 2, 0);
  ...
  elseif vfoptions.lowmemory==1 % loop semiz
    for d2_c=1:N_d2
        ...
        for semiz_c=1:N_semiz
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, special_n_semiz, d2_val, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, semiz_val, TemptationFnParamsVec, 1, 0);
            [~,maxindex1]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2); % max over a1prime
            midpoints_jj(1,1,:,level1ii,:)=maxindex1;
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, special_n_semiz, d2_val, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, semiz_val, TemptationFnParamsVec, 1, 0);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_jj(1,1,:,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(1,1,:,ii,:),N_a1-maxgap(ii));
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, special_n_semiz, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(curraindex), a2_grid, semiz_val, TemptationFnParamsVec, 1, 0);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_jj(1,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(1,1,:,ii,:);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, special_n_semiz, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, semiz_val, TemptationFnParamsVec, 2, 0);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            maxindexL2a1=rem(maxindexL2-1,n2long)+1;
            maxindexL2a2=ceil(maxindexL2/n2long);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
            a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, special_n_semiz, d2_val, a1prime_grid(a1primeindexesT), a2_grid, a1_grid, a2_grid, semiz_val, TemptationFnParamsVec, 2, 0);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting_ford2(:,semiz_c,d2_c)=shiftdim(MostTempting,1); % full (a1prime-fine,a2prime) for this d2
    ...
    for d2_c=1:N_d2
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(curraindex), a2_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 3, 1);
                aprime=aprimeindexes+N_a1*a2ind+N_a1*N_a2*semizBind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[(maxgap(ii)+1)*1,N_a2,1,N_a2,N_semiz]));
        ...
        for semiz_c=1:N_semiz
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, special_n_semiz, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(curraindex), a2_grid, semiz_val, TemptationFnParamsVec, 3, 1);
                    aprime=aprimeindexes+N_a1*a2ind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[(maxgap(ii)+1)*1,N_a2,1,N_a2]));
                    [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                    midpoints_jj(1,1,:,curraindex,:)=shiftdim(maxindex,-1)+(loweredge-1);
                else
                    loweredge=maxindex1(1,1,:,ii,:);
...
for reverse_j=1:N_j-1
  ...
  if vfoptions.lowmemory==0
    for d2_c=1:N_d2
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec, 1, 0);
        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+shiftdim(DiscountedEV,-1);
        [~,maxindex1]=max(entireRHS_ii,[],2);
        midpoints_jj(1,1,:,level1ii,:,:)=maxindex1;
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec, 1, 0);
            [~,maxindexT]=max(TemptationMatrix_full,[],2);
            midpointsT_jj(1,1,:,curraindex,:,:)=maxindexT;
            if maxgap(ii)>0
                loweredge=min(maxindex1(1,1,:,ii,:,:),N_a1-maxgap(ii));
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1_grid(aprimeindexes), a2_grid, a1_grid(curraindex), a2_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec, 3, 1);
                aprime=aprimeindexes+N_a1*a2ind+N_a1*N_a2*semizBind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[(maxgap(ii)+1)*1,N_a2,1,N_a2,N_semiz]));
                [~,maxindex]=max(entireRHS_ii,[],1); % max over a1prime
                midpoints_jj(1,1,:,curraindex,:,:)=shiftdim(maxindex,-1)+(loweredge-1);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec, 2, 0);
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprime),[n2long*N_a2,N_a,N_semiz]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
        a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1prime_grid(a1primeindexesT), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec, 2, 0);
        MostTempting=max(TemptationMatrix_Tii,[],1);
        MostTempting_ford2(:,:,d2_c)=shiftdim(MostTempting,1); % full (a1prime-fine,a2prime) for this d2
