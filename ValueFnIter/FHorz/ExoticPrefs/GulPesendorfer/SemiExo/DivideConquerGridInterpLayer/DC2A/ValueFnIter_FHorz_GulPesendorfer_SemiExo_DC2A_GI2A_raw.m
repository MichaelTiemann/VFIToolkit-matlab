function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC2A_GI2A_raw(n_d1, n_d2, n_a, n_z, n_semiz, N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2=zeros(N_a,N_bothz,N_d2,'gpuArray'); % per-d2 full (d1,a1prime-fine,a2prime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
  ...
  if vfoptions.lowmemory==0
    for d2_c=1:N_d2
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 1, 0);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 3, 0);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 2, 0);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1prime_grid(a1primeindexesT), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 2, 0);
    ...
    for d2_c=1:N_d2
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_valblock, TemptationFnParamsVec, 1, 0);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_jj(:,1,:,curraindex,:,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii));
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_valblock, TemptationFnParamsVec, 3, 0);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:,:);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, z_valblock, TemptationFnParamsVec, 2, 0);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            maxindexL2d1=rem(maxindexL2-1,N_d1)+1;
            maxindexL2a=ceil(maxindexL2/N_d1);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
            a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a1prime_grid(a1primeindexesT), a2_grid, a1_grid, a2_grid, z_valblock, TemptationFnParamsVec, 2, 0);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting_ford2(:,semizblock,d2_c)=shiftdim(MostTempting,1); % full (d1,a1prime-fine,a2prime) for this d2
  ...
  elseif vfoptions.lowmemory==2 % joint loop over bothz
    for d2_c=1:N_d2
        ...
        for z_c=1:N_bothz
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, z_val, TemptationFnParamsVec, 1, 0);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_jj(:,1,:,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:),N_a1-maxgap(ii));
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, z_val, TemptationFnParamsVec, 3, 0);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_jj(:,1,:,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, z_val, TemptationFnParamsVec, 2, 0);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            maxindexL2d1=rem(maxindexL2-1,N_d1)+1;
            maxindexL2a=ceil(maxindexL2/N_d1);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
            a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, a1prime_grid(a1primeindexesT), a2_grid, a1_grid, a2_grid, z_val, TemptationFnParamsVec, 2, 0);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting_ford2(:,z_c,d2_c)=shiftdim(MostTempting,1); % full (d1,a1prime-fine,a2prime) for this d2
    ...
    V(:,:,N_j)=V_jj-max(MostTempting_ford2,[],3); % subtract the full (d1,d2,a1prime-fine,a2prime) most-tempting term
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1_grid, a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec, 1, 0);
            [~,maxindexT]=max(TemptationMatrix_full,[],2);
            midpointsT_jj(:,1,:,curraindex,:,:)=maxindexT;
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii));
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec, 3, 0);
                aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*bothzBind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(reshape(aprime,[N_d1,(maxgap(ii)+1),N_a2,1,N_a2,N_bothz]));
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoints_jj(:,1,:,curraindex,:,:)=maxindex+(loweredge-1);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec, 2, 0);
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*bothzBind;
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(aprime),[N_d1*n2long*N_a2,N_a,N_bothz]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        midpointsT_jj=max(min(midpointsT_jj,n_a1-1),2);
        a1primeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1prime_grid(a1primeindexesT), a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec, 2, 0);
        MostTempting=max(TemptationMatrix_Tii,[],1);
        MostTempting_ford2(:,:,d2_c)=shiftdim(MostTempting,1); % full (d1,a1prime-fine,a2prime) for this d2
    ...
    [V_jj,d2_max]=max(V_ford2,[],3);
    V(:,:,jj)=V_jj-max(MostTempting_ford2,[],3); % subtract the full (d1,d2,a1prime-fine,a2prime) most-tempting term
