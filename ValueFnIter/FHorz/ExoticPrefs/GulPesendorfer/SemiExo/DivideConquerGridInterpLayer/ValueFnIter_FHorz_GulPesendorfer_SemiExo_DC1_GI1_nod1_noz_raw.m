function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC1_GI1_nod1_noz_raw(n_d2,n_a,n_semiz,N_j, d2_gridvals, a_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz,N_d2,'gpuArray'); % per-d2 full (aprime-fine) max of the temptation
% Preallocate
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, n_semiz, d2_gridvals, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, n_semiz, d2_gridvals, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
        ...
        if maxgap(ii)>0
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, n_semiz, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,3);
    ...
    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,n_semiz,d2_gridvals,aprime_grid(aprimeindexes),a_grid,semiz_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
    ...
    TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,n_semiz,d2_gridvals,aprime_grid(aprimeindexesT),a_grid,semiz_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
    ...
    elseif vfoptions.lowmemory==1 % loop semiz
        ...
        for semiz_c=1:N_semiz
            ...
            % n-Monotonicity
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, n_d2, special_n_semiz, d2_gridvals, a_grid, a_grid(level1ii), semiz_val, ReturnFnParamsVec,1);
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, special_n_semiz, d2_gridvals, a_grid, a_grid(level1ii), semiz_val, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer
            [~,maxindex1]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, special_n_semiz, d2_gridvals, a_grid, a_grid(curraindex), semiz_val, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_Nj(:,1,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, special_n_semiz, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, TemptationFnParamsVec,3);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn,n_d2,special_n_semiz,d2_gridvals,aprime_grid(aprimeindexes),a_grid,semiz_val,ReturnFnParamsVec,2);
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,special_n_semiz,d2_gridvals,aprime_grid(aprimeindexes),a_grid,semiz_val,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_Nj=max(min(midpointsT_Nj,n_a-1),2);
            aprimeindexesT=(midpointsT_Nj+(midpointsT_Nj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,special_n_semiz,d2_gridvals,aprime_grid(aprimeindexesT),a_grid,semiz_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            V(:,semiz_c,N_j)=shiftdim(Vtempii-MostTempting,1);
            d_ind=rem(maxindexL2-1,N_d2)+1;
            allind=d_ind+N_d2*aind; % midpoint is n_d-by-1-by-n_a
            Policy(1,:,semiz_c,N_j)=d_ind; % d2
            Policy(2,:,semiz_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1); % midpoint
            Policy(3,:,semiz_c,N_j)=shiftdim(ceil(maxindexL2/N_d2),-1); % aprimeL2ind
    ...
    for d2_c=1:N_d2
        ...
        TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,4);
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL aprime grid for these a (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,4);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
        ...
        TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, aprime_grid(aprimeindexesT), a_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
        MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==1 % loop semiz
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                EVinterp_d2=interp1(a_grid,EV_d2z,aprime_grid);
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_semiz, d2_val, a_grid, a_grid(level1ii), semiz_val, TemptationFnParamsVec,4);
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_semiz, d2_val, a_grid, a_grid(curraindex), semiz_val, TemptationFnParamsVec,4);
                    [~,maxindexT]=max(TemptationMatrix_full,[],1);
                    midpointsT_jj(1,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_semiz, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, TemptationFnParamsVec,5);
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2z(aprimeindexes),[(maxgap(ii)+1),1]); % autoexpand level1iidiff(ii) in 2nd-dim
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,ii,:);
                ...
                ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, special_n_d2, special_n_semiz, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_val, ReturnFnParamsVec,5);
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_semiz, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_val, TemptationFnParamsVec,5);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimeindexes),[n2long,N_a]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);

                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_semiz, d2_val, aprime_grid(aprimeindexesT), a_grid, semiz_val, TemptationFnParamsVec,5);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting_ford2_jj(:,semiz_c,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2

                V_ford2_jj(:,semiz_c,d2_c)=shiftdim(Vtemp,1);
                Policy_ford2_jj(:,semiz_c,d2_c)=shiftdim(maxindex,1);

                allind=aind2; % loweredge is 1-by-n_a
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
        ...
        TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, a_grid, a_grid(level1ii), semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,4);
        entireRHS_d2ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii+DiscountFactorParamsVec*EV_d2;
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            % v's coarse argmax over the FULL aprime grid for these a (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, a_grid, a_grid(curraindex), semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,4);
            [~,maxindexT]=max(TemptationMatrix_full,[],1);
            midpointsT_jj(1,curraindex,:)=maxindexT;
            if maxgap(ii)>0
                loweredge=min(maxindex1(1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
                aprimez=aprimeindexes+N_a*semizind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[(maxgap(ii)+1),1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                [~,maxindex]=max(entireRHS_ii,[],1);
                midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
        ...
        aprimeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)'; % aprime points either side of midpoint
        % aprime possibilities are n2long-by-n_a-by-n_semiz
        ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, special_n_d2, n_semiz, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,jj), ReturnFnParamsVec,5);
        TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
        Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
        aprimez=aprimeindexes+n2aprime*semizind; % the current aprime
        entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimez),[n2long,N_a,N_semiz]);
        [Vtemp,maxindex]=max(entireRHS_ii,[],1);
        ...
        midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
        aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, aprime_grid(aprimeindexesT), a_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
        MostTempting=max(TemptationMatrix_Tii,[],1);
        MostTempting_ford2_jj(:,:,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2
