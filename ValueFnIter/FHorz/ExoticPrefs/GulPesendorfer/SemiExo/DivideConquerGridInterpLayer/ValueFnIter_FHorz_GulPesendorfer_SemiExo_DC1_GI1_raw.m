function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC1_GI1_raw(n_d1,n_d2,n_a,n_z,n_semiz,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
Policy=zeros(4,N_a,N_semiz*N_z,N_j,'gpuArray'); % first dim indexes the optimal choice for d1,d2,aprime and aprime2 (in GI layer)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz*N_z,N_d2,'gpuArray'); % per-d2 full (d1,aprime-fine) max of the temptation
% Preallocate
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, n_bothz, d_gridvals, a_grid, a_grid(level1ii), bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, n_bothz, d_gridvals, a_grid, a_grid(curraindex), bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, n_bothz, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,3);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,n_bothz,d_gridvals,aprime_grid(aprimeindexes),a_grid,bothz_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,n_bothz,d_gridvals,aprime_grid(aprimeindexesT),a_grid,bothz_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        ...
        for z_c=1:N_z
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, [n_semiz,special_n_z], d_gridvals, a_grid, a_grid(level1ii), z_valblock, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer
            [~,maxindex1]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, [n_semiz,special_n_z], d_gridvals, a_grid, a_grid(curraindex), z_valblock, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_Nj(:,1,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, [n_semiz,special_n_z], d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, TemptationFnParamsVec,3);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,[n_semiz,special_n_z],d_gridvals,aprime_grid(aprimeindexes),a_grid,z_valblock,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_Nj=max(min(midpointsT_Nj,n_a-1),2);
            aprimeindexesT=(midpointsT_Nj+(midpointsT_Nj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,[n_semiz,special_n_z],d_gridvals,aprime_grid(aprimeindexesT),a_grid,z_valblock,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            V(:,semizblock,N_j)=shiftdim(Vtempii-MostTempting,1);
            d_ind=rem(maxindexL2-1,N_d)+1;
            allind=d_ind+N_d*aind+N_d*N_a*semizind; % midpoint is n_d-by-1-by-n_a-by-n_semiz
            Policy(1,:,semizblock,N_j)=shiftdim(rem(d_ind-1,N_d1)+1,-1); % d1
            Policy(2,:,semizblock,N_j)=shiftdim(ceil(d_ind/N_d1),-1); % d2
            Policy(3,:,semizblock,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1); % midpoint
            Policy(4,:,semizblock,N_j)=shiftdim(ceil(maxindexL2/N_d),-1); % aprimeL2ind
            ...
            inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
            inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
            PolicyL2flag(1,:,semizblock,N_j) = shiftdim(squeeze(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper)),-1);
        end
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        ...
        for z_c=1:N_bothz
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, special_n_bothz, d_gridvals, a_grid, a_grid(level1ii), z_val, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer
            [~,maxindex1]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, special_n_bothz, d_gridvals, a_grid, a_grid(curraindex), z_val, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_Nj(:,1,curraindex)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d, special_n_bothz, d_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, TemptationFnParamsVec,3);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_Nj(:,1,curraindex)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,special_n_bothz,d_gridvals,aprime_grid(aprimeindexes),a_grid,z_val,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_Nj=max(min(midpointsT_Nj,n_a-1),2);
            aprimeindexesT=(midpointsT_Nj+(midpointsT_Nj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,special_n_bothz,d_gridvals,aprime_grid(aprimeindexesT),a_grid,z_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            V(:,z_c,N_j)=shiftdim(Vtempii-MostTempting,1);
            d_ind=rem(maxindexL2-1,N_d)+1;
            ...
            Policy(3,:,z_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1); % midpoint
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a_grid, a_grid(level1ii), bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a_grid, a_grid(curraindex), bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,3);
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,2);
            ...
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, aprime_grid(aprimeindexesT), a_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a_grid, a_grid(level1ii), z_valblock, TemptationFnParamsVec,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a_grid, a_grid(curraindex), z_valblock, TemptationFnParamsVec,1);
                    [~,maxindexT]=max(TemptationMatrix_full,[],2);
                    midpointsT_jj(:,1,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, TemptationFnParamsVec,3);
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, aprime_grid(aprimeindexes), a_grid, z_valblock, TemptationFnParamsVec,2);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                ...
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, aprime_grid(aprimeindexesT), a_grid, z_valblock, TemptationFnParamsVec,2);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting_ford2_jj(:,semizblock,d2_c)=shiftdim(MostTempting,1); % full (d1,aprime-fine) for this d2
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, a_grid, a_grid(level1ii), z_val, TemptationFnParamsVec,1);
                entireRHS_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii+DiscountFactorParamsVec*shiftdim(EV_z,-1);
                % Treat standard problem as just being the first layer
                [~,maxindex1]=max(entireRHS_ii,[],2);
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, a_grid, a_grid(curraindex), z_val, TemptationFnParamsVec,1);
                    [~,maxindexT]=max(TemptationMatrix_full,[],2);
                    midpointsT_jj(:,1,curraindex)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, TemptationFnParamsVec,3);
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_z(aprimeindexes),[N_d1,(maxgap(ii)+1),1]); % autoexpand level1iidiff(ii) in 3rd-dim
                        [~,maxindex]=max(entireRHS_ii,[],2);
                        midpoints_jj(:,1,curraindex)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii);
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, z_val, TemptationFnParamsVec,2);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_z(aprimeindexes),[N_d1*n2long,N_a]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);

                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, aprime_grid(aprimeindexesT), a_grid, z_val, TemptationFnParamsVec,2);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting_ford2_jj(:,z_c,d2_c)=shiftdim(MostTempting,1); % full (d1,aprime-fine) for this d2
    ...
    V(:,:,N_j)=V_jj-max(MostTempting_ford2_jj,[],3); % subtract the full (d1,d2,aprime-fine) most-tempting term
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a_grid, a_grid(level1ii), bothz_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
            entireRHS_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii+DiscountFactorParamsVec*shiftdim(EV_d2,-1);
            % Treat standard problem as just being the first layer
            [~,maxindex1]=max(entireRHS_ii,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a_grid, a_grid(curraindex), bothz_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_jj(:,1,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,jj), TemptationFnParamsVec,3);
                    aprimez=aprimeindexes+N_a*bothzind2;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,(maxgap(ii)+1),1,N_bothz]); % autoexpand level1iidiff(ii) in 3rd-dim
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_jj(:,1,curraindex,:)=maxindex+(loweredge-1);
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
            aprimez=aprimeindexes+n2aprime*bothzind2; % the current aprime
            entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2(aprimez),[N_d1*n2long,N_a,N_bothz]);
            [Vtemp,maxindex]=max(entireRHS_ii,[],1);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
            aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, n_bothz, d12c_gridvals, aprime_grid(aprimeindexesT), a_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting_ford2_jj(:,:,d2_c)=shiftdim(MostTempting,1); % full (d1,aprime-fine) for this d2
    ...
    % Now we just max over d2, and keep the policy that corresponded to that (including modify the policy to include the d2 decision)
    [V_jj,maxindex]=max(V_ford2_jj,[],3); % max over d2
    V(:,:,jj)=V_jj-max(MostTempting_ford2_jj,[],3); % subtract the full (d1,d2,aprime-fine) most-tempting term
