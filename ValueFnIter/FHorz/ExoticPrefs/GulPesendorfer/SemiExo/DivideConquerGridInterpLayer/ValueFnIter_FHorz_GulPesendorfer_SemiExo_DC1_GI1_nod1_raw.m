function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC1_GI1_nod1_raw(n_d2,n_a,n_z,n_semiz,N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz*N_z,N_d2,'gpuArray'); % per-d2 full (aprime-fine) max of the temptation
% Preallocate
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, n_bothz, d2_gridvals, a_grid, a_grid(level1ii), bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, n_bothz, d2_gridvals, a_grid, a_grid(curraindex), bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, n_bothz, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,3);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,n_bothz,d2_gridvals,aprime_grid(aprimeindexes),a_grid,bothz_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,n_bothz,d2_gridvals,aprime_grid(aprimeindexesT),a_grid,bothz_gridvals_J(:,:,N_j),TemptationFnParamsVec,2);
        MostTempting=max(TemptationMatrix_Tii,[],1);
        V(:,:,N_j)=shiftdim(Vtempii-MostTempting,1);
        d_ind=rem(maxindexL2-1,N_d2)+1;
        allind=d_ind+N_d2*aind+N_d2*N_a*bothzind; % midpoint is n_d-by-1-by-n_a-by-n_bothz
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        ...
        for z_c=1:N_z
            ...
            % n-Monotonicity
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, n_d2, [n_semiz,special_n_z], d2_gridvals, a_grid, a_grid(level1ii), z_valblock, ReturnFnParamsVec,1);
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, [n_semiz,special_n_z], d2_gridvals, a_grid, a_grid(level1ii), z_valblock, TemptationFnParamsVec,1);
            [~,maxindex1]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, [n_semiz,special_n_z], d2_gridvals, a_grid, a_grid(curraindex), z_valblock, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_Nj(:,1,curraindex,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii));
                    aprimeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, n_d2, [n_semiz,special_n_z], d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, ReturnFnParamsVec,3);
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, [n_semiz,special_n_z], d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, TemptationFnParamsVec,3);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,[n_semiz,special_n_z],d2_gridvals,aprime_grid(aprimeindexes),a_grid,z_valblock,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_Nj=max(min(midpointsT_Nj,n_a-1),2);
            aprimeindexesT=(midpointsT_Nj+(midpointsT_Nj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,[n_semiz,special_n_z],d2_gridvals,aprime_grid(aprimeindexesT),a_grid,z_valblock,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            V(:,semizblock,N_j)=shiftdim(Vtempii-MostTempting,1);
            d_ind=rem(maxindexL2-1,N_d2)+1;
            allind=d_ind+N_d2*aind+N_d2*N_a*semizind;
            Policy(1,:,semizblock,N_j)=d_ind; % d2
            Policy(2,:,semizblock,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1); % midpoint
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        ...
        for z_c=1:N_bothz
            ...
            % n-Monotonicity
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, n_d2, special_n_bothz, d2_gridvals, a_grid, a_grid(level1ii), z_val, ReturnFnParamsVec,1);
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, special_n_bothz, d2_gridvals, a_grid, a_grid(level1ii), z_val, TemptationFnParamsVec,1);
            [~,maxindex1]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                % v's coarse argmax over the FULL aprime grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, special_n_bothz, d2_gridvals, a_grid, a_grid(curraindex), z_val, TemptationFnParamsVec,1);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointsT_Nj(:,1,curraindex)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii),n_a-maxgap(ii));
                    aprimeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, n_d2, special_n_bothz, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, ReturnFnParamsVec,3);
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, n_d2, special_n_bothz, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, TemptationFnParamsVec,3);
                    [~,maxindex]=max(ReturnMatrix_ii+TemptationMatrix_ii,[],2);
                    midpoints_Nj(:,1,curraindex)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii);
                    midpoints_Nj(:,1,curraindex)=repelem(loweredge,1,1,length(curraindex));
                end
            end
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,special_n_bothz,d2_gridvals,aprime_grid(aprimeindexes),a_grid,z_val,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointsT_Nj=max(min(midpointsT_Nj,n_a-1),2);
            aprimeindexesT=(midpointsT_Nj+(midpointsT_Nj-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d2,special_n_bothz,d2_gridvals,aprime_grid(aprimeindexesT),a_grid,z_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            V(:,z_c,N_j)=shiftdim(Vtempii-MostTempting,1);
            d_ind=rem(maxindexL2-1,N_d2)+1;
            allind=d_ind+N_d2*aind;
            Policy(1,:,z_c,N_j)=d_ind; % d2
            Policy(2,:,z_c,N_j)=shiftdim(squeeze(midpoints_Nj(allind)),-1); % midpoint
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            % n-Monotonicity
            ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, special_n_d2, n_bothz, d2_val, a_grid, a_grid(level1ii), bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,4); % effectively nod (as just a single d2 point), so use level=2 even for first level
            ...
            % n2long-by-n_a-by-n_bothz
            ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, special_n_d2, n_bothz, d2_gridvals(d2_c,:), aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,5);
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_bothz, d2_gridvals(d2_c,:), aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
            Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
            aprimez=aprimeindexes+n2aprime*bothzind; % the current aprime
            entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez),[n2long,N_a,N_bothz]);
            ...
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_bothz, d2_gridvals(d2_c,:), aprime_grid(aprimeindexesT), a_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
            MostTempting=max(TemptationMatrix_Tii,[],1);
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                entireRHS_d2ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii+DiscountFactorParamsVec*EV_d2z;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[(maxgap(ii)+1),1,N_semiz]);
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,ii,:);
                ...
                ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, special_n_d2, [n_semiz,special_n_z], d2_gridvals(d2_c,:), aprime_grid(aprimeindexes), a_grid, z_valblock, ReturnFnParamsVec,5);
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, [n_semiz,special_n_z], d2_gridvals(d2_c,:), aprime_grid(aprimeindexes), a_grid, z_valblock, TemptationFnParamsVec,5);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                aprimez=aprimeindexes+n2aprime*semizind;
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez),[n2long,N_a,N_semiz]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);

                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, [n_semiz,special_n_z], d2_gridvals(d2_c,:), aprime_grid(aprimeindexesT), a_grid, z_valblock, TemptationFnParamsVec,5);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting_ford2_jj(:,semizblock,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2

                V_ford2_jj(:,semizblock,d2_c)=shiftdim(Vtemp,1);
                Policy_ford2_jj(:,semizblock,d2_c)=shiftdim(maxindex,1);

                allind=(1:1:N_a)+N_a*semizind;
            ...
            for z_c=1:N_bothz
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*EV_d2z(aprimeindexes);
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,curraindex)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,ii);
                ...
                midpoints_jj=max(min(midpoints_jj,n_a-1),2);
                aprimeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                ReturnMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, special_n_d2, special_n_bothz, d2_gridvals(d2_c,:), aprime_grid(aprimeindexes), a_grid, z_val, ReturnFnParamsVec,5);
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_bothz, d2_gridvals(d2_c,:), aprime_grid(aprimeindexes), a_grid, z_val, TemptationFnParamsVec,5);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*EVinterp(aprimeindexes);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);

                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_bothz, d2_gridvals(d2_c,:), aprime_grid(aprimeindexesT), a_grid, z_val, TemptationFnParamsVec,5);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting_ford2_jj(:,z_c,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2

                V_ford2_jj(:,z_c,d2_c)=shiftdim(Vtemp,1);
                Policy_ford2_jj(:,z_c,d2_c)=shiftdim(maxindex,1);

                midpoint_ford2_jj(:,z_c,d2_c)=shiftdim(midpoints_jj,1);
                ...
                inLowerStrict = (maxindex >= 2)         & (maxindex <= n2short+1);
                inUpperStrict = (maxindex >= n2short+3) & (maxindex <= n2long-1);
                PolicyL2flag_ford2_jj(:,z_c,d2_c) = shiftdim(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper),1);
            end
        end
    end
    % Now we just max over d2, and keep the policy that corresponded to that (including modify the policy to include the d2 decision)
    [V_jj,maxindex]=max(V_ford2_jj,[],3); % max over d2
    V(:,:,N_j)=V_jj-max(MostTempting_ford2_jj,[],3); % subtract the full (d2,aprime-fine) most-tempting term
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_bothz, d2_val, aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
            Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
            aprimez=aprimeindexes+n2aprime*bothzind; % the current aprime
            entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez),[n2long,N_a,N_bothz]);
            [Vtemp,maxindex]=max(entireRHS_ii,[],1);
            ...
            midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
            aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_bothz, d2_val, aprime_grid(aprimeindexesT), a_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting_ford2_jj(:,:,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, [n_semiz,special_n_z], d2_val, aprime_grid(aprimeindexes), a_grid, z_valblock, TemptationFnParamsVec,5);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                aprimez=aprimeindexes+n2aprime*semizind;
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez),[n2long,N_a,N_semiz]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);
                ...
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, [n_semiz,special_n_z], d2_val, aprime_grid(aprimeindexesT), a_grid, z_valblock, TemptationFnParamsVec,5);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting_ford2_jj(:,semizblock,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_bothz, d2_val, aprime_grid(aprimeindexes), a_grid, z_val, TemptationFnParamsVec,5);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
                entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*EVinterp(aprimeindexes);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);
                ...
                midpointsT_jj=max(min(midpointsT_jj,n_a-1),2);
                aprimeindexesT=(midpointsT_jj+(midpointsT_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_bothz, d2_val, aprime_grid(aprimeindexesT), a_grid, z_val, TemptationFnParamsVec,5);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting_ford2_jj(:,z_c,d2_c)=shiftdim(MostTempting,1); % full (aprime-fine) for this d2
    ...
    % Now we just max over d2, and keep the policy that corresponded to that (including modify the policy to include the d2 decision)
    [V_jj,maxindex]=max(V_ford2_jj,[],3); % max over d2
    V(:,:,jj)=V_jj-max(MostTempting_ford2_jj,[],3); % subtract the full (d2,aprime-fine) most-tempting term
