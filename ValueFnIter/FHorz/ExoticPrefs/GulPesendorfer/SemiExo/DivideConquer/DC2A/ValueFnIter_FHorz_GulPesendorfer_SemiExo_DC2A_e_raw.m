...
% objective); the most-tempting term is a max over the FULL joint (d1,d2,a1prime,a2prime) choice
% set: a per-d2 full joint (d1,a1prime,a2prime) max is collected alongside each inner solve (from
...
MostTempting_ford2_jj=zeros(N_a,N_semiz*N_z,N_e,N_d2,'gpuArray'); % per-d2 full (d1,a1prime,a2prime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
  ...
  if vfoptions.lowmemory==0
    for d2_c=1:N_d2
        ...
        MostTempting_ford2_jj(curraindex,:,:,d2_c)=shiftdim(max(reshape(TemptationMatrix_ii,[N_d1*N_a1*N_a2,vfoptions.level1n*N_a2,N_bothz,N_e]),[],1),1); % full joint (d1,a1prime,a2prime) grid at the level-1 stations
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            % Most-tempting term over the FULL joint (d1,a1prime,a2prime) grid for these a (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0);
            MostTempting_ford2_jj(curraindex,:,:,d2_c)=shiftdim(max(reshape(TemptationMatrix_full,[N_d1*N_a1*N_a2,level1iidiff(ii)*N_a2,N_bothz,N_e]),[],1),1);
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, n_e, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0);
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, n_e, d12c_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0);
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
  ...
  elseif vfoptions.lowmemory==1 % loop e, vectorise bothz
    for d2_c=1:N_d2
        ...
        for e_c=1:N_e
            ...
            MostTempting_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_ii,[N_d1*N_a1*N_a2,vfoptions.level1n*N_a2,N_bothz]),[],1),1); % full joint (d1,a1prime,a2prime) grid at the level-1 stations
            maxgap=squeeze(max(max(max(max(maxindex1(:,1,:,2:end,:,:)-maxindex1(:,1,:,1:end-1,:,:),[],6),[],5),[],3),[],1));
            for ii=1:(vfoptions.level1n-1)
                curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2ind',level1iidiff(ii),1);
                % Most-tempting term over the FULL joint (d1,a1prime,a2prime) grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1,0);
                MostTempting_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_full,[N_d1*N_a1*N_a2,level1iidiff(ii)*N_a2,N_bothz]),[],1),1);
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2,0);
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d1)+1);
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2,0);
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d1)+1);
    ...
    for d2_c=1:N_d2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                MostTempting_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_ii,[N_d1*N_a1*N_a2,vfoptions.level1n*N_a2,N_semiz]),[],1),1); % full joint (d1,a1prime,a2prime) grid at the level-1 stations
                maxgap=squeeze(max(max(max(max(maxindex1(:,1,:,2:end,:,:)-maxindex1(:,1,:,1:end-1,:,:),[],6),[],5),[],3),[],1));
                for ii=1:(vfoptions.level1n-1)
                    curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2ind',level1iidiff(ii),1);
                    % Most-tempting term over the FULL joint (d1,a1prime,a2prime) grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, [n_semiz,special_n_z], special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_valblock, e_val, TemptationFnParamsVec,1,0);
                    MostTempting_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_full,[N_d1*N_a1*N_a2,level1iidiff(ii)*N_a2,N_semiz]),[],1),1);
                    if maxgap(ii)>0
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, [n_semiz,special_n_z], special_n_e, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_valblock, e_val, TemptationFnParamsVec,2,0);
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, [n_semiz,special_n_z], special_n_e, d12c_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_valblock, e_val, TemptationFnParamsVec,2,0);
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
  ...
  elseif vfoptions.lowmemory==3 % joint bothz / inner e
    for d2_c=1:N_d2
        ...
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                ...
                MostTempting_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_ii,[N_d1*N_a1*N_a2,vfoptions.level1n*N_a2]),[],1),1); % full joint (d1,a1prime,a2prime) grid at the level-1 stations
                maxgap=squeeze(max(max(max(maxindex1(:,1,:,2:end,:)-maxindex1(:,1,:,1:end-1,:),[],5),[],3),[],1));
                for ii=1:(vfoptions.level1n-1)
                    curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2ind',level1iidiff(ii),1);
                    % Most-tempting term over the FULL joint (d1,a1prime,a2prime) grid for these a (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, e_val, TemptationFnParamsVec,1,0);
                    MostTempting_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_full,[N_d1*N_a1*N_a2,level1iidiff(ii)*N_a2]),[],1),1);
                    if maxgap(ii)>0
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, e_val, TemptationFnParamsVec,2,0);
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, e_val, TemptationFnParamsVec,2,0);
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
    ...
    V(:,:,:,N_j)=V_jj-max(MostTempting_ford2_jj,[],4); % subtract the full joint (d1,d2,a1prime,a2prime) most-tempting term
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        DiscountedEV_d2=DiscountFactorParamsVec*reshape(EV_d2,[1,N_a1,N_a2,1,1,N_bothz]); % will autoexpand d in 1st-dim, and e

        % n-Monotonicity
        ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_bothz, n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0);

        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2; % autofill e
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2(reshape(aprime,[N_d1*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_bothz,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2(reshape(aprime,[N_d1*1*N_a2,level1iidiff(ii)*N_a2,N_bothz,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
      ...
      elseif vfoptions.lowmemory==1 % loop e, vectorise bothz
        ...
        EV_d2(isnan(EV_d2))=0; %multiplications of -Inf with 0 gives NaN, this replaces them with zeros (as the zeros come from the transition probabilities)
        EV_d2=sum(EV_d2,2); % sum over z', leaving a singular second dimension
        DiscountedEV_d2=DiscountFactorParamsVec*reshape(EV_d2,[1,N_a1,N_a2,1,1,N_bothz]); % will autoexpand d in 1st-dim
        for e_c=1:N_e
            e_val=e_gridvals_J(e_c,:,N_j);
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1,0);
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, bothz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2(reshape(aprime,[N_d1*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_bothz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d1)+1);
                    a1primeind=rem(ceil(maxindex/N_d1)-1,maxgap(ii)+1)+1-1; % already includes -1
                    ...
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2(reshape(aprime,[N_d1*1*N_a2,level1iidiff(ii)*N_a2,N_bothz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d1)+1);
                    a1primeind=0; %1-1; % already includes -1
      ...
      elseif vfoptions.lowmemory==2 % outer z / inner e, vectorise semiz
        for z_c=1:N_z
            ...
            DiscountedEV_d2z=DiscountFactorParamsVec*reshape(EV_d2z,[1,N_a1,N_a2,1,1,N_semiz]); % will autoexpand d in 1st-dim
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, [n_semiz,special_n_z], special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_valblock, e_val, ReturnFnParamsVec,1,0);
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, [n_semiz,special_n_z], special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_valblock, e_val, TemptationFnParamsVec,1,0);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2z(reshape(aprime,[N_d1*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
                        a1primeind=rem(ceil(maxindex/N_d1)-1,maxgap(ii)+1)+1-1; % already includes -1
                        ...
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2z(reshape(aprime,[N_d1*1*N_a2,level1iidiff(ii)*N_a2,N_semiz]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
                        a1primeind=0; %1-1; % already includes -1
      ...
      elseif vfoptions.lowmemory==3 % joint bothz / inner e
        for z_c=1:N_bothz
            ...
            EV_d2z(isnan(EV_d2z))=0; %multiplications of -Inf with 0 gives NaN, this replaces them with zeros (as the zeros come from the transition probabilities)
            EV_d2z=sum(EV_d2z,2); % sum over z', leaving a singular second dimension
            DiscountedEV_d2z=DiscountFactorParamsVec*reshape(EV_d2z,[1,N_a1,N_a2]); % will autoexpand d in 1st-dim
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_val, e_val, ReturnFnParamsVec,1,0);
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_val, e_val, TemptationFnParamsVec,1,0);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2z(reshape(aprime,[N_d1*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
                        a1primeind=rem(ceil(maxindex/N_d1)-1,maxgap(ii)+1)+1-1; % already includes -1
                        ...
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2z(reshape(aprime,[N_d1*1*N_a2,level1iidiff(ii)*N_a2]));
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
                        a1primeind=0; %1-1; % already includes -1
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2ind',level1iidiff(ii),1);
            % Most-tempting term over the FULL joint (d1,a1prime,a2prime) grid for these a (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,1,0);
            MostTempting_ford2_jj(curraindex,:,:,d2_c)=shiftdim(max(reshape(TemptationMatrix_full,[N_d1*N_a1*N_a2,level1iidiff(ii)*N_a2,N_bothz,N_e]),[],1),1);
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,:,ii,:,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                % loweredge is n_d1-by-1-by-n_a2-by-1-by-n_a2-by-n_bothz-by-n_e
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, n_e, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0);
                aprime=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1,1)+N_a1*a2Bind+N_a*bothzBind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2(reshape(aprime,[N_d1*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_bothz,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, n_e, d12c_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0);
                aprime=repelem(loweredge,1,1,1,level1iidiff(ii),1,1,1)+N_a1*a2Bind+N_a*bothzBind;
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2(reshape(aprime,[N_d1*1*N_a2,level1iidiff(ii)*N_a2,N_bothz,N_e]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
      ...
      elseif vfoptions.lowmemory==1 % loop e, vectorise bothz
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2ind',level1iidiff(ii),1);
                % Most-tempting term over the FULL joint (d1,a1prime,a2prime) grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,1,0);
                MostTempting_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_full,[N_d1*N_a1*N_a2,level1iidiff(ii)*N_a2,N_bothz]),[],1),1);
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii)); % loweredge is n_d1-by-1-by-n_a2-by-1-by-n_a2-by-n_bothz
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2,0);
                    aprime=repelem(a1primeindexes,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*bothzBind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2(reshape(aprime,[N_d1*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_bothz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_bothz, special_n_e, d12c_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, bothz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2,0);
                    aprime=repelem(loweredge,1,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind+N_a*bothzBind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV_d2(reshape(aprime,[N_d1*1*N_a2,level1iidiff(ii)*N_a2,N_bothz]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
    ...
    % Now we just max over d2, and keep the policy that corresponded to that (including modify the policy to include the d2 decision)
    [V_jj,maxindex]=max(V_ford2_jj,[],4); % max over d2
    V(:,:,:,jj)=V_jj-max(MostTempting_ford2_jj,[],4); % subtract the full joint (d1,d2,a1prime,a2prime) most-tempting term
