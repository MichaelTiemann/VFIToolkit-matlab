function [V,Policy3]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC1_nod1_e_raw(n_d2,n_a,n_z,n_semiz, n_e,N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    MostTempting_ford2_jj=zeros(N_a,N_semiz*N_z,N_e,N_d2,'gpuArray'); % per-d2 full aprime max of the temptation
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_bothz, n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_bothz, special_n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,5);
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie;
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    TemptationMatrix_d2iize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a_grid, a_grid(level1ii), z_valblock, e_val, TemptationFnParamsVec,4);
                    MostTempting_ford2_jj(level1ii,semizblock,e_c,d2_c)=shiftdim(max(TemptationMatrix_d2iize,[],1),1); % full aprime grid at the level-1 stations
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        MostTempting_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(max(TemptationMatrix_full,[],1),1);
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                            ...
                            TemptationMatrix_iize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, e_val, TemptationFnParamsVec,5);
                            ...
                            TemptationMatrix_iize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, [n_semiz,special_n_z], special_n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_valblock, e_val, TemptationFnParamsVec,5);
                            entireRHS_ii=ReturnMatrix_iize+TemptationMatrix_iize;
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    TemptationMatrix_d2iize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_bothz, special_n_e, d2_val, a_grid, a_grid(level1ii), z_val, e_val, TemptationFnParamsVec,4);
                    MostTempting_ford2_jj(level1ii,z_c,e_c,d2_c)=shiftdim(max(TemptationMatrix_d2iize,[],1),1); % full aprime grid at the level-1 stations
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        MostTempting_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(max(TemptationMatrix_full,[],1),1);
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(1,ii,:,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                            ...
                            TemptationMatrix_iize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_bothz, special_n_e, d2_val, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, TemptationFnParamsVec,5);
                            entireRHS_ii=ReturnMatrix_iize+TemptationMatrix_iize;
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            ...
                            TemptationMatrix_iize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_bothz, special_n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, TemptationFnParamsVec,5);
                            entireRHS_ii=ReturnMatrix_iize+TemptationMatrix_iize;
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[1,1,N_bothz,N_e]);  % autoexpand level1iidiff(ii) in 2nd-dim
                    V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(entireRHS_ii,1);
                    Policy_ford2_jj(curraindex,:,:,d2_c)=repelem(shiftdim(loweredge,1),level1iidiff(ii),1); % no d1
                end
            end
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_bothz, special_n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,5);
                        aprimez=loweredge+N_a*bothzind;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[1,1,N_bothz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(entireRHS_ii,1);
                        Policy_ford2_jj(curraindex,:,e_c,d2_c)=repelem(shiftdim(loweredge,1),level1iidiff(ii),1);
                    end
                end
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii=ReturnMatrix_d2iize+TemptationMatrix_d2iize+DiscountFactorParamsVec*EV_d2z;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iize+TemptationMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[(maxgap(ii)+1),1,N_semiz]);  % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                            Policy_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(maxindex+(loweredge-1)); % no d1
                        else
                            ...
                            entireRHS_ii=ReturnMatrix_iize+TemptationMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[1,1,N_semiz]);  % autoexpand level1iidiff(ii) in 2nd-dim
                            V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(entireRHS_ii,1);
                            Policy_ford2_jj(curraindex,semizblock,e_c,d2_c)=repelem(shiftdim(loweredge,1),level1iidiff(ii),1);
                        end
                    end
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iize+TemptationMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[(maxgap(ii)+1),1]);  % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            Policy_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(maxindex+(loweredge-1)); % no d1
                        else
                            ...
                            TemptationMatrix_iize=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_bothz, special_n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, TemptationFnParamsVec,5);
                            aprimez=loweredge;
                            entireRHS_ii=ReturnMatrix_iize+TemptationMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[1,1]);  % autoexpand level1iidiff(ii) in 2nd-dim
                            V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(entireRHS_ii,1);
                            Policy_ford2_jj(curraindex,z_c,e_c,d2_c)=repelem(shiftdim(loweredge,1),level1iidiff(ii),1);
                        end
                    end
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_bothz, n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
                    aprimez=loweredge+N_a*bothzind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[1,1,N_bothz,N_e]);  % autoexpand level1iidiff(ii) in 2nd-dim
                    V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(entireRHS_ii,1);
                    Policy_ford2_jj(curraindex,:,:,d2_c)=repelem(shiftdim(loweredge,1),level1iidiff(ii),1); % no d1
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, n_bothz, special_n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,5);
                        aprimez=loweredge+N_a*bothzind;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[1,1,N_bothz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(entireRHS_ii,1);
                        Policy_ford2_jj(curraindex,:,e_c,d2_c)=repelem(shiftdim(loweredge,1),level1iidiff(ii),1);
