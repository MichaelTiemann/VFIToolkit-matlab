function [V,Policy3]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC1_noz_e_raw(n_d1,n_d2,n_a,n_semiz,n_e, N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    MostTempting_ford2_jj=zeros(N_a,N_semiz,N_e,N_d2,'gpuArray'); % per-d2 full (d1,aprime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        for d2_c=1:N_d2
            ...
            MostTempting_ford2_jj(level1ii,:,:,d2_c)=shiftdim(max(reshape(TemptationMatrix_d2ii,[N_d1*N_a,vfoptions.level1n,N_semiz,N_e]),[],1),1); % full (d1,aprime) grid at the level-1 stations
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                MostTempting_ford2_jj(curraindex,:,:,d2_c)=shiftdim(max(reshape(TemptationMatrix_full,[N_d1*N_a,length(curraindex),N_semiz,N_e]),[],1),1);
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2);
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2);
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d1)+1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                MostTempting_ford2_jj(level1ii,:,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_d2iie,[N_d1*N_a,vfoptions.level1n,N_semiz]),[],1),1); % full (d1,aprime) grid at the level-1 stations
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    MostTempting_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_full,[N_d1*N_a,length(curraindex),N_semiz]),[],1),1);
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2);
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2);
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie;
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    MostTempting_ford2_jj(level1ii,semiz_c,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_d2iie,[N_d1*N_a,vfoptions.level1n]),[],1),1); % full (d1,aprime) grid at the level-1 stations
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        MostTempting_ford2_jj(curraindex,semiz_c,e_c,d2_c)=shiftdim(max(reshape(TemptationMatrix_full,[N_d1*N_a,length(curraindex)]),[],1),1);
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                            ...
                            TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, TemptationFnParamsVec,2);
                            entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie;
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            ...
                            TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, TemptationFnParamsVec,2);
                            entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie;
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semiz_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d1)+1);
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1*(maxgap(ii)+1),1,N_semiz,N_e]);  % autoexpand level1iidiff(ii) in 2nd-dim
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d1)+1);
                    allind=dind+N_d1*semizind+N_d1*N_semiz*eind; % loweredge is n_d1-by-1-by-1-by-n_semiz-by-n_e
                    ...
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,1,N_semiz,N_e]); % autoexpand level1iidiff(ii) in 2nd-dim
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d1)+1);
                    allind=dind+N_d1*semizind+N_d1*N_semiz*eind; % loweredge is n_d1-by-1-by-1-by-n_semiz-by-n_e
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2);
                        aprimez=aprimeindexes+N_a*semizBind;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1*(maxgap(ii)+1),1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
                        allind=dind+N_d1*semizind; % loweredge is n_d1-by-1-by-1-by-n_semiz
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2);
                        aprimez=loweredge+N_a*semizBind;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
                        allind=dind+N_d1*semizind; % loweredge is n_d1-by-1-by-1-by-n_semiz
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2semiz(aprimez),[N_d1*(maxgap(ii)+1),1]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semiz_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d1)+1);
                            allind=dind; % loweredge is n_d1-by-1-by-1-by-1
                            ...
                            TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, TemptationFnParamsVec,2);
                            aprimez=loweredge;
                            entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2semiz(aprimez),[N_d1,1]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semiz_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d1)+1);
                            allind=dind; % loweredge is n_d1-by-1-by-1-by-1
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,2);
                    aprimez=aprimeindexes+N_a*semizBind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1*(maxgap(ii)+1),1,N_semiz,N_e]);  % autoexpand level1iidiff(ii) in 2nd-dim
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,2);
                    aprimez=loweredge+N_a*semizBind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,1,N_semiz,N_e]); % autoexpand level1iidiff(ii) in 2nd-dim
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2);
                        aprimez=aprimeindexes+N_a*semizBind;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1*(maxgap(ii)+1),1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                        ...
                        TemptationMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2);
                        aprimez=loweredge+N_a*semizBind;
                        entireRHS_ii=ReturnMatrix_iie+TemptationMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[N_d1,1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
