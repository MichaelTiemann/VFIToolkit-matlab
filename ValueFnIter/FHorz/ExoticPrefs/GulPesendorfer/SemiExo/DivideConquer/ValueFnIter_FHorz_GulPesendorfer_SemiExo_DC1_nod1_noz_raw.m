function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_DC1_nod1_noz_raw(n_d2,n_a,n_semiz,N_j, d2_gridvals, a_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz,N_d2,'gpuArray'); % per-d2 full aprime max of the temptation
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
    ...
    elseif vfoptions.lowmemory==1 % loop semiz
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                MostTempting_ford2_jj(level1ii,semiz_c,d2_c)=shiftdim(max(TemptationMatrix_d2ii,[],1),1); % full aprime grid at the level-1 stations
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    MostTempting_ford2_jj(curraindex,semiz_c,d2_c)=shiftdim(max(TemptationMatrix_full,[],1),1);
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(1,ii,:),n_a-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_semiz, d2_val, reshape(a_grid(loweredge),[1,1,1]), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, TemptationFnParamsVec,5);
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            ...
            for semiz_c=1:N_semiz
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC1(ReturnFn, special_n_d2, special_n_semiz, d2_val, reshape(a_grid(loweredge),[1,1,1]), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, ReturnFnParamsVec,5);
                        TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, special_n_semiz, d2_val, reshape(a_grid(loweredge),[1,1,1]), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, TemptationFnParamsVec,5);
                        entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2z(loweredge),[1,1]); % autoexpand level1iidiff(ii) in 2nd-dim
                        V_ford2_jj(curraindex,semiz_c,d2_c)=shiftdim(entireRHS_ii,1);
                        Policy_ford2_jj(curraindex,semiz_c,d2_c)=repelem(shiftdim(loweredge,1),level1iidiff(ii),1);
                    end
                end
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_semiz, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,5);
                    aprimez=loweredge+N_a*semizind;
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[1,1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                    V_ford2_jj(curraindex,:,d2_c)=shiftdim(entireRHS_ii,1);
                    Policy_ford2_jj(curraindex,:,d2_c)=repelem(shiftdim(loweredge,1),level1iidiff(ii),1);
