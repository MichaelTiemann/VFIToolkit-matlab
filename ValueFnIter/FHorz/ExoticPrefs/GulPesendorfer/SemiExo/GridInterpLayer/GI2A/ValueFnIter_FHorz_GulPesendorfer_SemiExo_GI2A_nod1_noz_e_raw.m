function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_GI2A_nod1_noz_e_raw(n_d2, n_a, n_semiz, n_e, N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2=zeros(N_a,N_semiz,N_e,N_d2,'gpuArray'); % per-d2 fine (a1prime,a2prime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
  ...
  if vfoptions.lowmemory==0
    for d2_c=1:N_d2
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec, 1, 0);
  ...
  elseif vfoptions.lowmemory==1 % loop e, vectorise semiz
    for d2_c=1:N_d2
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1, 0);
            TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec, 1, 0);
            [~,maxindex]=max(ReturnMatrix+TemptationMatrix,[],2);
            ...
            mid_ford2(:,:,e_c,d2_c)=midpoint(maxindexL2a2+N_a2*a12ind+N_a2*N_a*semizind);
  ...
  elseif vfoptions.lowmemory==2 % outer semiz / inner e
    for d2_c=1:N_d2
        ...
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, special_n_semiz, special_n_e, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_val, e_val, ReturnFnParamsVec, 1, 0);
                TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d2, special_n_semiz, special_n_e, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_val, e_val, TemptationFnParamsVec, 1, 0);
                [~,maxindex]=max(ReturnMatrix+TemptationMatrix,[],2);
                ...
                MostTempting_ford2(:,semiz_c,e_c,d2_c)=shiftdim(max(TemptationMatrix_Tii,[],1),1); % fine (a1prime,a2prime) max for this d2
                ...
                % L2 flag (per d2,semiz,e): detect -Inf on the coarse a1 neighbour we'd put weight on (at chosen a2prime)
                ...
                inLowerStrict = (maxindexL2a1 >= 2)         & (maxindexL2a1 <= n2short+1);
                inUpperStrict = (maxindexL2a1 >= n2short+3) & (maxindexL2a1 <= n2long-1);
                flag_ford2(:,semiz_c,e_c,d2_c) = shiftdim(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper), 1);

                V_ford2(:,semiz_c,e_c,d2_c)=shiftdim(Vtempii,1);
                mid_ford2(:,semiz_c,e_c,d2_c)=midpoint(maxindexL2a2+N_a2*a12ind);
    ...
    V_next=sum(reshape(vfoptions.V_Jplus1,[N_a,N_semiz,N_e]).*pi_e_J(1,1,:,N_j+1),3); % integrate over e' first -> [N_a, N_semiz]
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprime),[n2long*N_a2,N_a,N_semiz,N_e]);
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d2, n_semiz, n_e, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec, 1, 0);
        entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*shiftdim(EV,-1);
        [~,maxindex]=max(entireRHS,[],2);
      ...
      elseif vfoptions.lowmemory==1 % loop e, vectorise semiz
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 1, 0);
            TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d2, n_semiz, special_n_e, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec, 1, 0);
            entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*shiftdim(EV,-1);
            [~,maxindex]=max(entireRHS,[],2);
