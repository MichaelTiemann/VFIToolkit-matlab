function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_GI2A_noz_e_raw(n_d1, n_d2, n_a, n_semiz, n_e, N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2=zeros(N_a,N_semiz,N_e,N_d2,'gpuArray'); % per-d2 fine (d1,a1prime,a2prime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
  ...
  if vfoptions.lowmemory==0
    for d2_c=1:N_d2
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec, 1, 0);
  ...
  elseif vfoptions.lowmemory==1 % loop e, vectorise semiz
    for d2_c=1:N_d2
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1, 0);
            TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec, 1, 0);
            [~,maxindex]=max(ReturnMatrix+TemptationMatrix,[],2);
            ...
            mid_ford2(:,:,e_c,d2_c)=midpoint(shiftdim(maxindexL2d1,1)+N_d1*(shiftdim(maxindexL2a2,1)-1)+N_d1*N_a2*a12ind+N_d1*N_a2*N_a*semizind);
  ...
  elseif vfoptions.lowmemory==2 % outer semiz / inner e
    for d2_c=1:N_d2
        ...
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, semiz_val, e_val, ReturnFnParamsVec, 1, 0);
                TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, special_n_semiz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, semiz_val, e_val, TemptationFnParamsVec, 1, 0);
                [~,maxindex]=max(ReturnMatrix+TemptationMatrix,[],2);
                ...
                MostTempting_ford2(:,semiz_c,e_c,d2_c)=shiftdim(max(TemptationMatrix_Tii,[],1),1); % fine (d1,a1prime,a2prime) max for this d2
                ...
                % L2 flag (per d2,semiz,e): detect -Inf on the coarse a1 neighbour we'd put weight on (at chosen d1, a2prime)
                mL2d1 = shiftdim(maxindexL2d1,1);
                mL2a1 = shiftdim(maxindexL2a1,1);
                mL2a2 = shiftdim(maxindexL2a2,1);
                ...
                inLowerStrict = (mL2a1 >= 2)         & (mL2a1 <= n2short+1);
                inUpperStrict = (mL2a1 >= n2short+3) & (mL2a1 <= n2long-1);
                flag_ford2(:,semiz_c,e_c,d2_c) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);

                V_ford2(:,semiz_c,e_c,d2_c)=shiftdim(Vtempii,1);
                d1_ford2(:,semiz_c,e_c,d2_c)=shiftdim(maxindexL2d1,1);
                mid_ford2(:,semiz_c,e_c,d2_c)=midpoint(shiftdim(maxindexL2d1,1)+N_d1*(shiftdim(maxindexL2a2,1)-1)+N_d1*N_a2*a12ind);
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp(aprime),[N_d1*n2long*N_a2,N_a,N_semiz,N_e]);
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_semiz, n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec, 1, 0);
        entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*shiftdim(EV,-1);
        [~,maxindex]=max(entireRHS,[],2);
      ...
      elseif vfoptions.lowmemory==1 % loop e, vectorise semiz
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 1, 0);
            TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A_e(TemptationFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec, 1, 0);
            entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*shiftdim(EV,-1);
            [~,maxindex]=max(entireRHS,[],2);
