function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_GI2A_nod1_noz_raw(n_d2, n_a, n_semiz, N_j, d2_gridvals, a_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2=zeros(N_a,N_semiz,N_d2,'gpuArray'); % per-d2 fine (a1prime,a2prime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
  ...
  if vfoptions.lowmemory==0
    for d2_c=1:N_d2
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 1, 0);
        [~,maxindex]=max(ReturnMatrix+TemptationMatrix,[],2); % max over a1prime (dim 2). maxindex shape: [1,1,N_a2,N_a1,N_a2,N_semiz]
  ...
  elseif vfoptions.lowmemory==1 % loop semiz
    for d2_c=1:N_d2
        ...
        for semiz_c=1:N_semiz
            ...
            TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, special_n_semiz, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_val, TemptationFnParamsVec, 1, 0);
            [~,maxindex]=max(ReturnMatrix+TemptationMatrix,[],2); % max over a1prime (dim 2)
            ...
            MostTempting_ford2(:,semiz_c,d2_c)=shiftdim(max(TemptationMatrix_Tii,[],1),1); % fine (a1prime,a2prime) max for this d2
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d2, n_semiz, d2_val, a1_grid, a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec, 1, 0);
        entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*shiftdim(EV,-1);
        [~,maxindex]=max(entireRHS,[],2);
