function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_GI2A_raw(n_d1, n_d2, n_a, n_z, n_semiz, N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
% SemiExo + GI2A (two-endo grid interpolation), with d1, with z, no e.
% Combines SemiExo d2/pi_semiz iteration with the 2-endo GI2A machinery; bothz=[semiz,z].
...
MostTempting_ford2=zeros(N_a,N_bothz,N_d2,'gpuArray'); % per-d2 fine (d1,a1prime,a2prime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
  ...
  if vfoptions.lowmemory==0
    for d2_c=1:N_d2
        ...
        ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, n_bothz, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 1, 0);
        TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec, 1, 0);
        ...
        mid_ford2(:,:,d2_c)=midpoint(shiftdim(maxindexL2d1,1)+N_d1*(shiftdim(maxindexL2a2,1)-1)+N_d1*N_a2*a12ind+N_d1*N_a2*N_a*bothzind);
  ...
  elseif vfoptions.lowmemory==1 % split: outer z, vectorise semiz
    for d2_c=1:N_d2
        ...
        for z_c=1:N_z
            ...
            ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, z_valblock, ReturnFnParamsVec, 1, 0);
            TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, [n_semiz,special_n_z], d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, z_valblock, TemptationFnParamsVec, 1, 0);
            [~,maxindex]=max(ReturnMatrix+TemptationMatrix,[],2);
            ...
            MostTempting_ford2(:,semizblock,d2_c)=shiftdim(max(TemptationMatrix_Tii,[],1),1); % fine (d1,a1prime,a2prime) max for this d2
            ...
            % L2 flag (per d2,z): detect -Inf on the coarse a1 neighbour we'd put weight on (at chosen d1, a2prime)
            mL2d1 = shiftdim(maxindexL2d1,1);
            mL2a1 = shiftdim(maxindexL2a1,1);
            mL2a2 = shiftdim(maxindexL2a2,1);
            ...
            inLowerStrict = (mL2a1 >= 2)         & (mL2a1 <= n2short+1);
            inUpperStrict = (mL2a1 >= n2short+3) & (mL2a1 <= n2long-1);
            flag_ford2(:,semizblock,d2_c) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);

            V_ford2(:,semizblock,d2_c)=shiftdim(Vtempii,1);
            d1_ford2(:,semizblock,d2_c)=shiftdim(maxindexL2d1,1);
            mid_ford2(:,semizblock,d2_c)=midpoint(shiftdim(maxindexL2d1,1)+N_d1*(shiftdim(maxindexL2a2,1)-1)+N_d1*N_a2*a12ind+N_d1*N_a2*N_a*semizind);
  ...
  elseif vfoptions.lowmemory==2 % joint loop over bothz
    for d2_c=1:N_d2
        ...
        for z_c=1:N_bothz
            ...
            ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, special_n_bothz, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, z_val, ReturnFnParamsVec, 1, 0);
            TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, special_n_bothz, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, z_val, TemptationFnParamsVec, 1, 0);
            [~,maxindex]=max(ReturnMatrix+TemptationMatrix,[],2);
            ...
            MostTempting_ford2(:,z_c,d2_c)=shiftdim(max(TemptationMatrix_Tii,[],1),1); % fine (d1,a1prime,a2prime) max for this d2
            ...
            inLowerStrict = (mL2a1 >= 2)         & (mL2a1 <= n2short+1);
            inUpperStrict = (mL2a1 >= n2short+3) & (mL2a1 <= n2long-1);
            flag_ford2(:,z_c,d2_c) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);

            V_ford2(:,z_c,d2_c)=shiftdim(Vtempii,1);
            d1_ford2(:,z_c,d2_c)=shiftdim(maxindexL2d1,1);
            mid_ford2(:,z_c,d2_c)=midpoint(shiftdim(maxindexL2d1,1)+N_d1*(shiftdim(maxindexL2a2,1)-1)+N_d1*N_a2*a12ind);
...
for reverse_j=1:N_j-1
    ...
    for d2_c=1:N_d2
      ...
      if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, special_n_d, n_bothz, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 1, 0);
        TemptationMatrix=CreateReturnFnMatrix_Disc_DC2A(TemptationFn, special_n_d, n_bothz, d12c_gridvals, a1_grid, a2_grid, a1_grid, a2_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec, 1, 0);
        entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*shiftdim(EV,-1);
        [~,maxindex]=max(entireRHS,[],2);
