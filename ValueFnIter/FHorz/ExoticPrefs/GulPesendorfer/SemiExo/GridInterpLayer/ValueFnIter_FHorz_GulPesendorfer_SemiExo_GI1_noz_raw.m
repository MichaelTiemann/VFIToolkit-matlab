function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_GI1_noz_raw(n_d1,n_d2,n_a,n_semiz,N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz,N_d2,'gpuArray'); % per-d2 fine (d1,aprime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc(TemptationFn, n_d, n_a, n_semiz, d_gridvals, a_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_semiz
            ...
            TemptationMatrix_z=CreateReturnFnMatrix_Disc(TemptationFn, n_d, n_a, special_n_semiz, d_gridvals, a_grid, z_val, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer
            ...
            midpointT=max(min(maxindexT,n_a-1),2);
            aprimeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,special_n_semiz,d_gridvals,aprime_grid(aprimeindexesT),a_grid,z_val,TemptationFnParamsVec,2);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn,n_d,special_n_semiz,d_gridvals,aprime_grid(aprimeindexes),a_grid,z_val,TemptationFnParamsVec,2);
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2=CreateReturnFnMatrix_Disc(TemptationFn, special_n_d, n_a, n_semiz, d12c_gridvals, a_grid, semiz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for z_c=1:N_semiz
                ...
                TemptationMatrix_d2z=CreateReturnFnMatrix_Disc(TemptationFn, special_n_d, n_a, special_n_semiz, d12c_gridvals, a_grid, z_val, TemptationFnParamsVec,1);
                ...
                midpointT=max(min(maxindexT,n_a-1),2);
                aprimeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
                TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_semiz, d12c_gridvals, aprime_grid(aprimeindexesT), a_grid, z_val, TemptationFnParamsVec,2);
                ...
                TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d, special_n_semiz, d12c_gridvals, aprime_grid(aprimeindexes), a_grid, z_val, TemptationFnParamsVec,2);
                Ftemp_ii=ReturnMatrix_d2ii+TemptationMatrix_d2ii;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2=CreateReturnFnMatrix_Disc(TemptationFn, special_n_d, n_a, n_semiz, d12c_gridvals, a_grid, semiz_gridvals_J(:,:,jj), TemptationFnParamsVec,1);
            entireRHS=ReturnMatrix_d2+TemptationMatrix_d2+DiscountFactorParamsVec*shiftdim(EV_d2,-1);
            % Treat standard problem as just being the first layer
            [~,maxindex]=max(entireRHS,[],2);
