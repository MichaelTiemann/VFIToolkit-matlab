function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_GI1_nod1_raw(n_d2,n_a,n_z,n_semiz,N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz*N_z,N_d2,'gpuArray'); % per-d2 fine (aprime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc(TemptationFn, n_d2, n_a, n_bothz, d2_gridvals, a_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        ...
        for z_c=1:N_z
            ...
            TemptationMatrix_z=CreateReturnFnMatrix_Disc(TemptationFn, n_d2, n_a, [n_semiz,special_n_z], d2_gridvals, a_grid, z_valblock, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        ...
        for z_c=1:N_bothz
            ...
            TemptationMatrix_z=CreateReturnFnMatrix_Disc(TemptationFn, n_d2, n_a, special_n_bothz, d2_gridvals, a_grid, z_val, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer
            [~,maxindex]=max(ReturnMatrix_z+TemptationMatrix_z,[],2);
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2=CreateReturnFnMatrix_Disc(TemptationFn, special_n_d2, n_a, n_bothz, d2_val, a_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,0);
            ...
            midpointT=max(min(maxindexT,n_a-1),2);
            aprimeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short)';
            TemptationMatrix_Tii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_bothz, d2_val, aprime_grid(aprimeindexesT), a_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
            ...
            TemptationMatrix_d2ii=CreateReturnFnMatrix_Disc_DC1(TemptationFn, special_n_d2, n_bothz, d2_val, aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,N_j), TemptationFnParamsVec,5);
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                TemptationMatrix_d2z=CreateReturnFnMatrix_Disc(TemptationFn, special_n_d2, n_a, [n_semiz,special_n_z], d2_val, a_grid, z_valblock, TemptationFnParamsVec,0);
                entireRHS_z=ReturnMatrix_d2z+TemptationMatrix_d2z+DiscountFactorParamsVec*EV_d2z;
                % Treat standard problem as just being the first layer
                [~,maxindex]=max(entireRHS_z,[],1); % no d1, loop over d2

                % Most-tempting term: two-stage max of v over the FINE grid, around v's own coarse argmax
                [~,maxindexT]=max(TemptationMatrix_d2z,[],1);
                midpointT=max(min(maxindexT,n_a-1),2);
                aprimeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short)';
                ...
                MostTempting_ford2_jj(:,semizblock,d2_c)=shiftdim(max(TemptationMatrix_Tii,[],1),1); % fine (aprime) max for this d2
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                TemptationMatrix_d2z=CreateReturnFnMatrix_Disc(TemptationFn, special_n_d2, n_a, special_n_bothz, d2_val, a_grid, z_val, TemptationFnParamsVec,0);
                entireRHS_z=ReturnMatrix_d2z+TemptationMatrix_d2z+DiscountFactorParamsVec*EV_d2z;
                % Treat standard problem as just being the first layer
                [~,maxindex]=max(entireRHS_z,[],1); % no d1, loop over d2
                ...
                MostTempting_ford2_jj(:,z_c,d2_c)=shiftdim(max(TemptationMatrix_Tii,[],1),1); % fine (aprime) max for this d2
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2=CreateReturnFnMatrix_Disc(TemptationFn, special_n_d2, n_a, n_bothz, d2_val, a_grid, bothz_gridvals_J(:,:,jj), TemptationFnParamsVec,0);
            entireRHS=ReturnMatrix_d2+TemptationMatrix_d2+DiscountFactorParamsVec*EV_d2;
            % Treat standard problem as just being the first layer
            [~,maxindex]=max(entireRHS,[],1); % no d1, loop over d2
