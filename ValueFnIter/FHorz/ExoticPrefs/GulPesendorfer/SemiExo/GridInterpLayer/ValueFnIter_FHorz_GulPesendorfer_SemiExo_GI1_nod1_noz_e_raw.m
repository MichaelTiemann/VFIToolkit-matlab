function [V,Policy]=ValueFnIter_FHorz_GulPesendorfer_SemiExo_GI1_nod1_noz_e_raw(n_d2,n_a,n_semiz,n_e, N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, TemptationFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, vfoptions)
...
MostTempting_ford2_jj=zeros(N_a,N_semiz,N_e,N_d2,'gpuArray'); % per-d2 fine (aprime) max of the temptation
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d2, n_a, n_semiz, n_e, d2_gridvals, a_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d2, n_a, n_semiz, special_n_e, d2_gridvals, a_grid, semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1);
            % Treat standard problem as just being the first layer
    ...
    elseif vfoptions.lowmemory==2
        ...
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_semize=CreateReturnFnMatrix_Disc_e(TemptationFn, n_d2, n_a, special_n_semiz, special_n_e, d2_gridvals, a_grid, semiz_val, e_val, TemptationFnParamsVec,1);
                % Treat standard problem as just being the first layer
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d2, n_a, n_semiz,n_e, d2_val, a_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,0);
            entireRHS_d2=ReturnMatrix_d2+TemptationMatrix_d2+DiscountFactorParamsVec*EV_d2;
            % Treat standard problem as just being the first layer
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_d2e=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d2, n_a, n_semiz,special_n_e, d2_val, a_grid, semiz_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,0);
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    TemptationMatrix_d2semize=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d2, n_a, special_n_semiz,special_n_e, d2_val, a_grid, semiz_val, e_val, TemptationFnParamsVec,0);
                    entireRHS_d2semize=ReturnMatrix_d2semize+TemptationMatrix_d2semize+DiscountFactorParamsVec*EV_d2semiz;
                    % Treat standard problem as just being the first layer
                    [~,maxindex]=max(entireRHS_d2semize,[],1); % no d1, loop over d2

                    % Most-tempting term: two-stage max of v over the FINE grid, around v's own coarse argmax
                    [~,maxindexT]=max(TemptationMatrix_d2semize,[],1);
                    midpointT=max(min(maxindexT,n_a-1),2);
                    aprimeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short)';
                    ...
                    MostTempting_ford2_jj(:,semiz_c,e_c,d2_c)=shiftdim(max(TemptationMatrix_Tii,[],1),1); % fine (aprime) max for this d2
                    ...
                    TemptationMatrix_d2semizeii=CreateReturnFnMatrix_Disc_DC1_e(TemptationFn, special_n_d2, special_n_semiz, special_n_e, d2_val, aprime_grid(aprimeindexes), a_grid, semiz_val, e_val, TemptationFnParamsVec,5);
                    Ftemp_ii=ReturnMatrix_d2semizeii+TemptationMatrix_d2semizeii;
                    aprimez=aprimeindexes; % the current aprime (single semiz)
                    entireRHS_ii=Ftemp_ii+DiscountFactorParamsVec*reshape(EVinterp_d2semiz(aprimez),[n2long,N_a]);
                    [Vtemp,maxindex]=max(entireRHS_ii,[],1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            TemptationMatrix_d2=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d2, n_a, n_semiz,n_e, d2_val, a_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,0);
            entireRHS_d2=ReturnMatrix_d2+TemptationMatrix_d2+DiscountFactorParamsVec*EV_d2;
            % Treat standard problem as just being the first layer
            [~,maxindex]=max(entireRHS_d2,[],1); % no d1, loop over d2
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d2_c=1:N_d2
            ...
            % Interpolate EV over aprime_grid
            EVinterp_d2=interp1(a_grid,EV_d2,aprime_grid); % (aprime,1,semiz)
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_d2e=CreateReturnFnMatrix_Disc_e(TemptationFn, special_n_d2, n_a, n_semiz, special_n_e, d2_val, a_grid, semiz_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,0);
                entireRHS_d2e=ReturnMatrix_d2e+TemptationMatrix_d2e+DiscountFactorParamsVec*EV_d2;
                % Treat standard problem as just being the first layer
                [~,maxindex]=max(entireRHS_d2e,[],1); % no d1, loop over d2
    ...
    elseif vfoptions.lowmemory==2
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                %Calc the condl expectation term (except beta), which depends on semiz but
                %not on control variables
                EV_d2semiz=EV.*pi_semiz(semiz_c,:);
                EV_d2semiz(isnan(EV_d2semiz))=0; %multiplications of -Inf with 0 gives NaN, this replaces them with zeros (as the zeros come from the transition probabilities)
                EV_d2semiz=sum(EV_d2semiz,2); % sum over semiz', leaving [N_a,1]
