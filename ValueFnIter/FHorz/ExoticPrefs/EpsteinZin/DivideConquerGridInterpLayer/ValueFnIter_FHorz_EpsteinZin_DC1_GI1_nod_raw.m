function [V, Policy]=ValueFnIter_FHorz_EpsteinZin_DC1_GI1_nod_raw(n_a,n_z,N_j, a_grid, z_gridvals_J,pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7, ezc8)
% Divide-and-conquer plus grid-interpolation-layer version of
% ValueFnIter_FHorz_EpsteinZin_nod_raw. Grafts the Epstein-Zin transforms onto
% ValueFnIter_FHorz_DC1_GI1_nod_raw. The interpolation acts on
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            waszero=(ReturnMatrix_ii_z==0);
            ReturnMatrix_ii_z(becareful)=ReturnMatrix_ii_z(becareful).^ezc2(N_j); % in-place transform: ReturnMatrix_ii_z now holds what was temp2 (avoids a full-size copy)
            ReturnMatrix_ii_z(waszero)=-Inf;
            ReturnMatrix_ii_z=ezc1*ReturnMatrix_ii_z+ezc3*DiscountFactorParamsVec*WGmatrix; % warm-glow (zero if not using) % in-place: ReturnMatrix_ii_z now holds what was entireRHS_ii
            temp5=logical(isfinite(ReturnMatrix_ii_z).*(ReturnMatrix_ii_z~=0));
            ReturnMatrix_ii_z(temp5)=ReturnMatrix_ii_z(temp5).^ezc7(N_j);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_z(becareful)=ReturnMatrix_ii_z(becareful).^ezc2(N_j); % in-place transform: ReturnMatrix_ii_z now holds what was temp2 (avoids a full-size copy)
                    ReturnMatrix_ii_z(waszero)=-Inf;
                    ReturnMatrix_ii_z=ezc1*ReturnMatrix_ii_z+ezc3*DiscountFactorParamsVec*reshape(WGmatrix(aprimeindexes),[(maxgap(ii)+1),1]); % in-place: ReturnMatrix_ii_z now holds what was entireRHS_ii
                    temp5=logical(isfinite(ReturnMatrix_ii_z).*(ReturnMatrix_ii_z~=0));
                    ReturnMatrix_ii_z(temp5)=ReturnMatrix_ii_z(temp5).^ezc7(N_j);
                    ReturnMatrix_ii_z(ReturnMatrix_ii_z==0)=-Inf;
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*temp4(reshape(aprimez,[(maxgap(ii)+1),1,N_z]));  % autoexpand the level1iidiff(ii) in 2nd-dim
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*temp4_z(aprimeindexes); % autoexpand the level1iidiff(ii) in 2nd-dim
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                    entireRHS_ii(entireRHS_ii==0)=-Inf;
                    [~,maxindex]=max(entireRHS_ii,[],1);
