function [V,Policy3]=ValueFnIter_FHorz_EpsteinZin_SemiExo_nod1_noz_e_raw(n_d2,n_a,n_semiz,n_e, N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7, ezc8)
% Epstein-Zin version of ValueFnIter_FHorz_SemiExo_nod1_noz_e_raw.
...
% transform chain are applied before each conditional-on-d2 max. The final max
% over d2 then compares fully-transformed values, so it is unaffected.
...
elseif vfoptions.lowmemory==1 % loops over e
    V_ford2_jj=zeros(N_a,N_semiz,N_e,N_d2,'gpuArray'); % [the vNM source preallocates without the N_e dimension, but then indexes (:,:,e_c,d2_c) everywhere, which errors if N_d2>N_e]
    Policy_ford2_jj=zeros(N_a,N_semiz,N_e,N_d2,'gpuArray');
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                entireRHSe=ezc1*temp2+ezc3*DiscountFactorParamsVec*temp4;
