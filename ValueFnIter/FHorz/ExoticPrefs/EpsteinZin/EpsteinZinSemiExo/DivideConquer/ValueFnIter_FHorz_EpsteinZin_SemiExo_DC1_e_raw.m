...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        for d2_c=1:N_d2
            ...
            entireRHS_ii=ReturnMatrix_d2ii+shiftdim(WGmatrix,-1); % warm-glow (zero if not using); (d,aprime,a,bothz,e)
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+reshape(WGmatrix(aprimeindexes),[N_d1*(maxgap(ii)+1),1,N_bothz,N_e]);
                    ...
                    ReturnMatrix_ii(becareful)=(ezc1*ReturnMatrix_ii(becareful).^ezc2(N_j)).^ezc7(N_j);
                    ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                    entireRHS_ii=ReturnMatrix_ii+reshape(WGmatrix(loweredge),[N_d1,1,N_bothz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,:,d2_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d1)+1);
                    allind=dind+N_d1*bothzind+N_d1*N_bothz*eind; % loweredge is n_d1-by-1-by-1-by-n_bothz-by-n_e
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii=ReturnMatrix_d2iie+shiftdim(WGmatrix,-1); % warm-glow (zero if not using); (d,aprime,a,bothz)
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=ReturnMatrix_iie+reshape(WGmatrix(aprimeindexes),[N_d1*(maxgap(ii)+1),1,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        ...
                        ReturnMatrix_iie(becareful)=(ezc1*ReturnMatrix_iie(becareful).^ezc2(N_j)).^ezc7(N_j);
                        ReturnMatrix_iie(ReturnMatrix_iie==0)=-Inf;
                        entireRHS_ii=ReturnMatrix_iie+reshape(WGmatrix(loweredge),[N_d1,1,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford2_jj(curraindex,:,e_c,d2_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1)+1);
                        allind=dind+N_d1*bothzind; % loweredge is n_d1-by-1-by-1-by-n_bothz
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii=ReturnMatrix_d2iize+shiftdim(WGmatrix,-1); % warm-glow (zero if not using); (d,aprime,a,semiz)
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iize+reshape(WGmatrix(aprimeindexes),[N_d1*(maxgap(ii)+1),1,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            ...
                            ReturnMatrix_iize(becareful)=(ezc1*ReturnMatrix_iize(becareful).^ezc2(N_j)).^ezc7(N_j);
                            ReturnMatrix_iize(ReturnMatrix_iize==0)=-Inf;
                            entireRHS_ii=ReturnMatrix_iize+reshape(WGmatrix(loweredge),[N_d1,1,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d1)+1);
                            allind=dind+N_d1*semizind; % loweredge is n_d1-by-1-by-1-by-n_semiz
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_d2iize(becareful)=(ezc1*ReturnMatrix_d2iize(becareful).^ezc2(N_j)).^ezc7(N_j);
                    ReturnMatrix_d2iize(ReturnMatrix_d2iize==0)=-Inf;
                    entireRHS_ii=ReturnMatrix_d2iize+shiftdim(WGmatrix,-1); % warm-glow (zero if not using); (d,aprime,a)
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            ReturnMatrix_iize(becareful)=(ezc1*ReturnMatrix_iize(becareful).^ezc2(N_j)).^ezc7(N_j);
                            ReturnMatrix_iize(ReturnMatrix_iize==0)=-Inf;
                            entireRHS_ii=ReturnMatrix_iize+reshape(WGmatrix(aprimeindexes),[N_d1*(maxgap(ii)+1),1]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            ...
                            ReturnMatrix_iize(becareful)=(ezc1*ReturnMatrix_iize(becareful).^ezc2(N_j)).^ezc7(N_j);
                            ReturnMatrix_iize(ReturnMatrix_iize==0)=-Inf;
                            entireRHS_ii=ReturnMatrix_iize+reshape(WGmatrix(loweredge),[N_d1,1]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d1)+1);
                            allind=dind; % loweredge is n_d1-by-1
