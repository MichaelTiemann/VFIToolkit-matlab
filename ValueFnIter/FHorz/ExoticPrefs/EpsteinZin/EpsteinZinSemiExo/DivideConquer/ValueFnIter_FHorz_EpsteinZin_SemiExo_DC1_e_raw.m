function [V,Policy3]=ValueFnIter_FHorz_EpsteinZin_SemiExo_DC1_e_raw(n_d1,n_d2,n_a,n_z,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7, ezc8)
% Divide-and-conquer version of ValueFnIter_FHorz_EpsteinZin_SemiExo_e_raw.
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_SemiExo_DC1_e_raw:
...
% over the full aprime grid (with the result expanded over d1 via the
% reshape-indexing, as in the vNM code) and indexed exactly where the vNM code
% indexes EV_d2; the return transform and the final ^ezc7 wrap each level's
% entireRHS before its max (a monotone transform, so the divide-and-conquer
% monotonicity logic is unaffected). The final max over d2 then compares
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        for d2_c=1:N_d2
            ...
            ReturnMatrix_d2ii=ezc1*ReturnMatrix_d2ii+ezc3*DiscountFactorParamsVec*shiftdim(WGmatrix,-1); % warm-glow (zero if not using); (d,aprime,a,bothz,e) % in-place: ReturnMatrix_d2ii now holds what was entireRHS_ii
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                    ReturnMatrix_ii=ezc1*ReturnMatrix_ii+ezc3*DiscountFactorParamsVec*reshape(WGmatrix(aprimeindexes),[N_d1*(maxgap(ii)+1),1,N_bothz,N_e]); % in-place: ReturnMatrix_ii now holds what was entireRHS_ii
                    ...
                    ReturnMatrix_ii(waszero)=-Inf;
                    % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                    ReturnMatrix_ii=ezc1*ReturnMatrix_ii+ezc3*DiscountFactorParamsVec*reshape(WGmatrix(loweredge),[N_d1,1,N_bothz,N_e]); % in-place: ReturnMatrix_ii now holds what was entireRHS_ii
                    temp5=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0));
                    ReturnMatrix_ii(temp5)=ReturnMatrix_ii(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                    ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                    [Vtempii,maxindex]=max(ReturnMatrix_ii,[],1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                ReturnMatrix_d2iie=ezc1*ReturnMatrix_d2iie+ezc3*DiscountFactorParamsVec*shiftdim(WGmatrix,-1); % warm-glow (zero if not using); (d,aprime,a,bothz) % in-place: ReturnMatrix_d2iie now holds what was entireRHS_ii
                ...
                % First, we want aprime conditional on (d,1,a,z,e)
                [~,maxindex1]=max(ReturnMatrix_d2iie,[],2);

                % Now, get and store the full (d,aprime)
                [Vtempii,maxindex2]=max(reshape(ReturnMatrix_d2iie,[N_d1*N_a,vfoptions.level1n,N_bothz]),[],1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                        ReturnMatrix_iie=ezc1*ReturnMatrix_iie+ezc3*DiscountFactorParamsVec*reshape(WGmatrix(aprimeindexes),[N_d1*(maxgap(ii)+1),1,N_bothz]); % in-place: ReturnMatrix_iie now holds what was entireRHS_ii
                        ...
                        ReturnMatrix_iie(waszero)=-Inf;
                        % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                        ReturnMatrix_iie=ezc1*ReturnMatrix_iie+ezc3*DiscountFactorParamsVec*reshape(WGmatrix(loweredge),[N_d1,1,N_bothz]); % in-place: ReturnMatrix_iie now holds what was entireRHS_ii
                        temp5=logical(isfinite(ReturnMatrix_iie).*(ReturnMatrix_iie~=0));
                        ReturnMatrix_iie(temp5)=ReturnMatrix_iie(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                        ReturnMatrix_iie(ReturnMatrix_iie==0)=-Inf;
                        [Vtempii,maxindex]=max(ReturnMatrix_iie,[],1);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                    ReturnMatrix_d2iize=ezc1*ReturnMatrix_d2iize+ezc3*DiscountFactorParamsVec*shiftdim(WGmatrix,-1); % warm-glow (zero if not using); (d,aprime,a,semiz) % in-place: ReturnMatrix_d2iize now holds what was entireRHS_ii
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                            ReturnMatrix_iize=ezc1*ReturnMatrix_iize+ezc3*DiscountFactorParamsVec*reshape(WGmatrix(aprimeindexes),[N_d1*(maxgap(ii)+1),1,N_semiz]); % in-place: ReturnMatrix_iize now holds what was entireRHS_ii
                            ...
                            ReturnMatrix_iize(waszero)=-Inf;
                            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                            ReturnMatrix_iize=ezc1*ReturnMatrix_iize+ezc3*DiscountFactorParamsVec*reshape(WGmatrix(loweredge),[N_d1,1,N_semiz]); % in-place: ReturnMatrix_iize now holds what was entireRHS_ii
                            temp5=logical(isfinite(ReturnMatrix_iize).*(ReturnMatrix_iize~=0));
                            ReturnMatrix_iize(temp5)=ReturnMatrix_iize(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                            ReturnMatrix_iize(ReturnMatrix_iize==0)=-Inf;
                            [Vtempii,maxindex]=max(ReturnMatrix_iize,[],1);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_d2iize(waszero)=-Inf;
                    % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                    ReturnMatrix_d2iize=ezc1*ReturnMatrix_d2iize+ezc3*DiscountFactorParamsVec*shiftdim(WGmatrix,-1); % warm-glow (zero if not using); (d,aprime,a) % in-place: ReturnMatrix_d2iize now holds what was entireRHS_ii
                    temp5=logical(isfinite(ReturnMatrix_d2iize).*(ReturnMatrix_d2iize~=0));
                    ReturnMatrix_d2iize(temp5)=ReturnMatrix_d2iize(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                    ReturnMatrix_d2iize(ReturnMatrix_d2iize==0)=-Inf;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            ReturnMatrix_iize(waszero)=-Inf;
                            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                            ReturnMatrix_iize=ezc1*ReturnMatrix_iize+ezc3*DiscountFactorParamsVec*reshape(WGmatrix(aprimeindexes),[N_d1*(maxgap(ii)+1),1]); % in-place: ReturnMatrix_iize now holds what was entireRHS_ii
                            temp5=logical(isfinite(ReturnMatrix_iize).*(ReturnMatrix_iize~=0));
                            ReturnMatrix_iize(temp5)=ReturnMatrix_iize(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                            ReturnMatrix_iize(ReturnMatrix_iize==0)=-Inf;
                            [Vtempii,maxindex]=max(ReturnMatrix_iize,[],1);
                            ...
                            ReturnMatrix_iize(waszero)=-Inf;
                            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                            ReturnMatrix_iize=ezc1*ReturnMatrix_iize+ezc3*DiscountFactorParamsVec*reshape(WGmatrix(loweredge),[N_d1,1]); % in-place: ReturnMatrix_iize now holds what was entireRHS_ii
                            temp5=logical(isfinite(ReturnMatrix_iize).*(ReturnMatrix_iize~=0));
                            ReturnMatrix_iize(temp5)=ReturnMatrix_iize(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                            ReturnMatrix_iize(ReturnMatrix_iize==0)=-Inf;
                            [Vtempii,maxindex]=max(ReturnMatrix_iize,[],1);
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*shiftdim(temp4,-1); % (d,aprime,a,bothz,e)
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4(aprimez),[N_d1*(maxgap(ii)+1),1,N_bothz,N_e]); % autoexpand level1iidiff(ii) in 2nd-dim
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    ...
                    entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4(aprimez),[N_d1,1,N_bothz,N_e]); % autoexpand level1iidiff(ii) in 2nd-dim
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                    entireRHS_ii(entireRHS_ii==0)=-Inf;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*shiftdim(temp4,-1); % (d,aprime,a,bothz)
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        aprimez=aprimeindexes+N_a*bothzBind;
                        entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4(aprimez),[N_d1*(maxgap(ii)+1),1,N_bothz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                        entireRHS_ii(entireRHS_ii==0)=-Inf;
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        ...
                        temp2_ii(ReturnMatrix_ii==0)=-Inf;
                        aprimez=loweredge+N_a*bothzBind;
                        entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4(aprimez),[N_d1,1,N_bothz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                        entireRHS_ii(entireRHS_ii==0)=-Inf;
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
