...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        for d2_c=1:N_d2
            ...
            entireRHS_ii=ReturnMatrix_d2ii+shiftdim(WGmatrix,-1); % warm-glow (zero if not using); (d,aprime,a,bothz)
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+reshape(WGmatrix(aprimeindexes),[N_d1*(maxgap(ii)+1),1,N_bothz]);
                    ...
                    ReturnMatrix_ii(becareful)=(ezc1*ReturnMatrix_ii(becareful).^ezc2(N_j)).^ezc7(N_j);
                    ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                    entireRHS_ii=ReturnMatrix_ii+reshape(WGmatrix(loweredge),[N_d1,1,N_bothz]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford2_jj(curraindex,:,d2_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d1)+1);
                    allind=dind+N_d1*bothzind; % loweredge is n_d-by-1-by-1-by-n_z
