function [V,Policy3]=ValueFnIter_FHorz_EpsteinZin_SemiExo_noz_raw(n_d1,n_d2,n_a,n_semiz,N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7, ezc8)
% Epstein-Zin version of ValueFnIter_FHorz_SemiExo_noz_raw.
...
% are applied (with the result expanded over d1) before each
% conditional-on-d2 max. The final max over d2 then compares
...
if ~isfield(vfoptions,'V_Jplus1')
    WGmatrixkron=kron(WGmatrix,ones(N_d,1)); % rows of the terminal ReturnMatrix are (d1,d2,aprime) with d=(d1,d2) fastest
    if vfoptions.lowmemory==0
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            entireEV=repelem(temp4,N_d1,1,1); % expand over d1 (after the certainty-equivalent transform)
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for z_c=1:N_semiz
                ...
                entireEV_z=repelem(temp4,N_d1,1,1); % expand over d1 (after the certainty-equivalent transform)
                entireRHS_z=ezc1*temp2+ezc3*DiscountFactorParamsVec*entireEV_z;
