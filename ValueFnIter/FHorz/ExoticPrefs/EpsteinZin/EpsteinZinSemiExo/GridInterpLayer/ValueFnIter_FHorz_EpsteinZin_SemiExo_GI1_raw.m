function [V,Policy]=ValueFnIter_FHorz_EpsteinZin_SemiExo_GI1_raw(n_d1,n_d2,n_a,n_z,n_semiz,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7, ezc8)
% Grid-interpolation-layer version of ValueFnIter_FHorz_EpsteinZin_SemiExo_raw.
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_SemiExo_GI1_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ReturnMatrix_ii+reshape(WGmatrixfine(aprimeindexes),[N_d*n2long,N_a,N_bothz]);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                if warmglow==1
                    ...
                    becareful=logical(isfinite(temp4interp).*isfinite(WGmatrixfinebig)); % both are finite
                    temp4interp(becareful)=(sj(N_j)*temp4interp(becareful).^ezc8(N_j)+(1-sj(N_j))*WGmatrixfinebig(becareful).^ezc8(N_j)).^ezc6(N_j);
                    temp4interp((EVinterp_z==0)&(WGmatrixfinebig==0))=0; % Is actually zero
                else % not using warmglow
                    temp4(isfinite(temp4))=(sj(N_j)*temp4(isfinite(temp4)).^ezc8(N_j)).^ezc6(N_j);
                    temp4(EV_d2z==0)=0;
                    temp4interp(isfinite(temp4interp))=(sj(N_j)*temp4interp(isfinite(temp4interp)).^ezc8(N_j)).^ezc6(N_j);
                ...
                entireRHS_z=ezc1*temp2+ezc3*DiscountFactorParamsVec*shiftdim(temp4,-1);
    ...
    elseif vfoptions.lowmemory>=2 % joint loop over bothz
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                % Interpolate EV over aprime_grid (BEFORE the certainty-equivalent power ^ezc6)
                EVinterp_z=interp1(a_grid,EV_d2z,aprime_grid'); % (column, to match WGmatrixfine)
                ...
                if warmglow==1
                    ...
                    temp4interp((EVinterp_z==0)&(WGmatrixfine==0))=0; % Is actually zero
                else % not using warmglow
                    temp4(isfinite(temp4))=(sj(N_j)*temp4(isfinite(temp4)).^ezc8(N_j)).^ezc6(N_j);
                    temp4(EV_d2z==0)=0;
                    temp4interp(isfinite(temp4interp))=(sj(N_j)*temp4interp(isfinite(temp4interp)).^ezc8(N_j)).^ezc6(N_j);
