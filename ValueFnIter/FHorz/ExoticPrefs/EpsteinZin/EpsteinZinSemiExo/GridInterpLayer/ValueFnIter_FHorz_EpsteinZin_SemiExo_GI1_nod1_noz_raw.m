function [V,Policy]=ValueFnIter_FHorz_EpsteinZin_SemiExo_GI1_nod1_noz_raw(n_d2,n_a,n_semiz,N_j, d2_gridvals, a_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7, ezc8)
% Grid-interpolation-layer version of ValueFnIter_FHorz_EpsteinZin_SemiExo_nod1_noz_raw.
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_SemiExo_GI1_nod1_noz_raw.
    ...
    elseif vfoptions.lowmemory>=1
        for d2_c=1:N_d2
            ...
            for z_c=1:N_semiz
                ...
                entireRHS_d2z=ezc1*temp2+ezc3*DiscountFactorParamsVec*temp4;
                temp5=logical(isfinite(entireRHS_d2z).*(entireRHS_d2z~=0));
                entireRHS_d2z(temp5)=entireRHS_d2z(temp5).^ezc7(N_j);
                entireRHS_d2z(entireRHS_d2z==0)=-Inf;
                % Treat standard problem as just being the first layer
    ...
    elseif vfoptions.lowmemory>=1
        for d2_c=1:N_d2
            ...
            for z_c=1:N_semiz
                ...
                entireRHS_d2z=ezc1*temp2+ezc3*DiscountFactorParamsVec*temp4;
                temp5=logical(isfinite(entireRHS_d2z).*(entireRHS_d2z~=0));
                entireRHS_d2z(temp5)=entireRHS_d2z(temp5).^ezc7(jj);
                entireRHS_d2z(entireRHS_d2z==0)=-Inf;
                % Treat standard problem as just being the first layer
                [~,maxindex]=max(entireRHS_d2z,[],1); % no d1, loop over d2
                maxindex=shiftdim(maxindex,-1); % no d1, loop over d2
