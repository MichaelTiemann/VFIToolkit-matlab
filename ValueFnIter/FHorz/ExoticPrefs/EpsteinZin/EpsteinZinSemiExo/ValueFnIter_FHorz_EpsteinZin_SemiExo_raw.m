function [V,Policy3]=ValueFnIter_FHorz_EpsteinZin_SemiExo_raw(n_d1,n_d2,n_a,n_z,n_semiz,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7, ezc8)
% Epstein-Zin version of ValueFnIter_FHorz_SemiExo_raw.
...
% is taken, then the certainty-equivalent power ^ezc6 and the rest of the
...
if ~isfield(vfoptions,'V_Jplus1')
    WGmatrixkron=kron(WGmatrix,ones(N_d,1)); % rows of the terminal ReturnMatrix are (d1,d2,aprime) with d1 fastest, then d2
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        ...
        for z_c=1:N_z
            ...
            % Compose the warm-glow INSIDE the ^ezc7 root (the main-loop composition with the EV term absent)
            ReturnMatrix_z=ezc1*ReturnMatrix_z+ezc3*DiscountFactorParamsVec*WGmatrixkron; % in-place: ReturnMatrix_z now holds what was entireRHS_z
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            entireRHS=ezc1*temp2+ezc3*DiscountFactorParamsVec*repelem(temp4,N_d1,1);
    ...
    elseif vfoptions.lowmemory==1 % parallel over semiz, loop over z
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                entireRHS_z=ezc1*temp2+ezc3*DiscountFactorParamsVec*repelem(temp4,N_d1,1);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                entireEV_z=kron(temp4,ones(N_d1,1));
                entireRHS_z=ezc1*temp2+ezc3*DiscountFactorParamsVec*entireEV_z; %*ones(1,N_a,1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            entireRHS=ezc1*temp2+ezc3*DiscountFactorParamsVec*repelem(temp4,N_d1,1,1); %repelem(temp4,N_d1,N_a,1); but the N_a autofills
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d2_c=1:N_d2
            ...
            for z_c=1:N_bothz
                ...
                entireRHS_z=ezc1*temp2+ezc3*DiscountFactorParamsVec*repelem(temp4,N_d1,1); %*ones(1,N_a,1);
