    ...
    elseif vfoptions.lowmemory>=2
        ...
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii_semize(becareful)=(ezc1*ReturnMatrix_ii_semize(becareful).^ezc2(N_j)).^ezc7(N_j);
                ReturnMatrix_ii_semize(ReturnMatrix_ii_semize==0)=-Inf;
                entireRHS_ii=ReturnMatrix_ii_semize+shiftdim(WGmatrix,-1); % warm-glow (zero if not using)
                % Treat standard problem as just being the first layer
                [~,maxindex1]=max(entireRHS_ii,[],2);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_semize(becareful)=(ezc1*ReturnMatrix_ii_semize(becareful).^ezc2(N_j)).^ezc7(N_j);
                        ReturnMatrix_ii_semize(ReturnMatrix_ii_semize==0)=-Inf;
                        entireRHS_ii=ReturnMatrix_ii_semize+WGmatrix(aprimeindexes); % warm-glow (zero if not using)
                        [~,maxindex]=max(entireRHS_ii,[],2);
                        midpoints_Nj(:,1,curraindex)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
