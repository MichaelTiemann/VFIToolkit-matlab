    ...
    elseif vfoptions.lowmemory==2 % outer z / inner e, vectorize semiz
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii=ReturnMatrix_ii_ze+shiftdim(WGmatrix,-1); % warm-glow (zero if not using)
                % Treat standard problem as just being the first layer
                [~,maxindex1]=max(entireRHS_ii,[],2);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_ze(becareful)=(ezc1*ReturnMatrix_ii_ze(becareful).^ezc2(N_j)).^ezc7(N_j);
                        ReturnMatrix_ii_ze(ReturnMatrix_ii_ze==0)=-Inf;
                        entireRHS_ii=ReturnMatrix_ii_ze+WGmatrix(aprimeindexes); % warm-glow (zero if not using)
                        [~,maxindex]=max(entireRHS_ii,[],2);
                        midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
