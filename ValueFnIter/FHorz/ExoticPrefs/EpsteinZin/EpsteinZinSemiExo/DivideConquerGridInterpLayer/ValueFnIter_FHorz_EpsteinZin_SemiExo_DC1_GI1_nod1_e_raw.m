...
% interpolation. The return transform and the final ^ezc7 wrap each entireRHS
% before its max (a monotone transform, so the divide-and-conquer monotonicity
% logic is unaffected). The warm-glow fn is evaluated exactly on the fine grid
% (no interpolation needed); the coarse passes use its strided subset. The
% final max over d2 compares fully-transformed values, so it is unaffected.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2 % outer z / inner e, vectorize semiz
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                waszero=(ReturnMatrix_ii_ze==0);
                ReturnMatrix_ii_ze(becareful)=ReturnMatrix_ii_ze(becareful).^ezc2(N_j); % in-place transform: ReturnMatrix_ii_ze now holds what was temp2 (avoids a full-size copy)
                ReturnMatrix_ii_ze(waszero)=-Inf;
                % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                ReturnMatrix_ii_ze=ezc1*ReturnMatrix_ii_ze+ezc3*DiscountFactorParamsVec*shiftdim(WGmatrix,-1); % warm-glow (zero if not using) % in-place: ReturnMatrix_ii_ze now holds what was entireRHS_ii
                temp5=logical(isfinite(ReturnMatrix_ii_ze).*(ReturnMatrix_ii_ze~=0));
                ReturnMatrix_ii_ze(temp5)=ReturnMatrix_ii_ze(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_ze(waszero)=-Inf;
                        % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1-branch composition with the EV term absent)
                        ReturnMatrix_ii_ze=ezc1*ReturnMatrix_ii_ze+ezc3*DiscountFactorParamsVec*WGmatrix(aprimeindexes); % warm-glow (zero if not using) % in-place: ReturnMatrix_ii_ze now holds what was entireRHS_ii
                        temp5=logical(isfinite(ReturnMatrix_ii_ze).*(ReturnMatrix_ii_ze~=0));
                        ReturnMatrix_ii_ze(temp5)=ReturnMatrix_ii_ze(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                        ReturnMatrix_ii_ze(ReturnMatrix_ii_ze==0)=-Inf;
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4interp(aprimez),[n2long,N_a,N_bothz,N_e]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                entireRHS_d2ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*temp4;
                temp5=logical(isfinite(entireRHS_d2ii).*(entireRHS_d2ii~=0));
                entireRHS_d2ii(temp5)=entireRHS_d2ii(temp5).^ezc7(N_j);
                entireRHS_d2ii(entireRHS_d2ii==0)=-Inf;
                % Treat standard problem as just being the first layer
                [~,maxindex1]=max(entireRHS_d2ii,[],1); % no d1
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                entireRHS_d2ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*temp4;
                temp5=logical(isfinite(entireRHS_d2ii).*(entireRHS_d2ii~=0));
                entireRHS_d2ii(temp5)=entireRHS_d2ii(temp5).^ezc7(jj);
                entireRHS_d2ii(entireRHS_d2ii==0)=-Inf;
