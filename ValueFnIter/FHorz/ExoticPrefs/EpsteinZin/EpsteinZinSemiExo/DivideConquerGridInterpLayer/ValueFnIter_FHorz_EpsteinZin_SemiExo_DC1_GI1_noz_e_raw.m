function [V,Policy]=ValueFnIter_FHorz_EpsteinZin_SemiExo_DC1_GI1_noz_e_raw(n_d1,n_d2,n_a,n_semiz,n_e, N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8)
% Epstein-Zin version of ValueFnIter_FHorz_SemiExo_DC1_GI1_noz_e_raw.
...
% the certainty-equivalent power ^ezc6 and the rest of the transform chain are
% applied pointwise AFTER the interpolation. The return transform and the
% final ^ezc7 wrap each entireRHS before its max (a monotone transform, so the
% divide-and-conquer monotonicity logic is unaffected). The warm-glow fn is
% evaluated exactly on the fine grid (no interpolation needed); the coarse
% passes use its strided subset; the warm glow is independent of e. The final
% max over d2 compares fully-transformed values, so it is unaffected.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii(becareful)=(ezc1*ReturnMatrix_ii(becareful).^ezc2(N_j)).^ezc7(N_j);
        ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
        entireRHS_ii=ReturnMatrix_ii+reshape(WGmatrixfine(aprimeindexes),[N_d*n2long,N_a,N_semiz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii=ReturnMatrix_ii_e+shiftdim(WGmatrix,-1); % warm-glow (zero if not using)
            % Treat standard problem as just being the first layer: we want aprime conditional on (d,1,a,z,e)
            [~,maxindex1]=max(entireRHS_ii,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_e(becareful)=(ezc1*ReturnMatrix_ii_e(becareful).^ezc2(N_j)).^ezc7(N_j);
                    ReturnMatrix_ii_e(ReturnMatrix_ii_e==0)=-Inf;
                    entireRHS_ii=ReturnMatrix_ii_e+WGmatrix(aprimeindexes); % warm-glow (zero if not using)
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoints_Nj(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
    ...
    temp(V_Jplus1==0)=0;
    temp=sum(temp.*pi_e_J(1,1,:,N_j+1),3); % e'-expectation (d2-independent, on the transformed object)
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4(aprimez),[N_d1,(maxgap(ii)+1),1,N_semiz,N_e]); % autoexpand level1iidiff(ii) in 3rd-dim
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            ...
            entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4interp(aprimez),[N_d1*n2long,N_a,N_semiz,N_e]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4(aprimez),[N_d1,(maxgap(ii)+1),1,1]); % autoexpand level1iidiff(ii) in 3rd-dim
                            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                            entireRHS_ii(entireRHS_ii==0)=-Inf;
                            [~,maxindex]=max(entireRHS_ii,[],2);
                    ...
                    entireRHS_ii=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4interp(aprimez),[N_d1*n2long,N_a,1]);
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                    entireRHS_ii(entireRHS_ii==0)=-Inf;
                    [Vtemp,maxindex]=max(entireRHS_ii,[],1);
...
for reverse_j=1:N_j-1
    ...
    temp(EVpre==0)=0;
    temp=sum(temp.*pi_e_J(1,1,:,jj+1),3); % e'-expectation (d2-independent, on the transformed object)
