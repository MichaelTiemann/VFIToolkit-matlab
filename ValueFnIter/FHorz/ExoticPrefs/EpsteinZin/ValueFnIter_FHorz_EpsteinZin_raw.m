function [V,Policy]=ValueFnIter_FHorz_EpsteinZin_raw(n_d,n_a,n_z,N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            % Compose the warm-glow INSIDE the ^ezc7 root (the main-loop composition with the EV term absent)
            ReturnMatrix_z=ezc1*ReturnMatrix_z+ezc3*DiscountFactorParamsVec*WGmatrix; % in-place: ReturnMatrix_z now holds what was entireRHS_z
...
for reverse_j=1:N_j-1
    ...
    temp(EVpre==0)=0;
    temp(~isfinite(EVpre))=ezc4*EVpre(~isfinite(EVpre)); % ezc4 is 1 or -1, When it is -1 need to flip the sign of the -Inf, so that ezc3 below (which will also be -1) flips it back
