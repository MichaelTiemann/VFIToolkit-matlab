function [V,Policy]=ValueFnIter_FHorz_EpsteinZin_GI1_noz_e_raw(n_d,n_a,n_e,N_j, d_gridvals, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7, ezc8)
% Grid-interpolation-layer version of ValueFnIter_FHorz_EpsteinZin_noz_e_raw.
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_GI1_noz_e_raw. The
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii=ezc1*ReturnMatrix_ii+ezc3*DiscountFactorParamsVec*reshape(WGmatrixfine(aprimeindexes(:)),[N_d*n2long,N_a,N_e]); % in-place: ReturnMatrix_ii now holds what was entireRHS_ii
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_ii_e=ezc1*ReturnMatrix_ii_e+ezc3*DiscountFactorParamsVec*reshape(WGmatrixfine(aprimeindexes(:)),[N_d*n2long,N_a]); % in-place: ReturnMatrix_ii_e now holds what was entireRHS_ii_e
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii_e=ezc1*temp2_ii+ezc3*DiscountFactorParamsVec*reshape(temp4interp(aprimeindexes(:)),[N_d*n2long,N_a]);
