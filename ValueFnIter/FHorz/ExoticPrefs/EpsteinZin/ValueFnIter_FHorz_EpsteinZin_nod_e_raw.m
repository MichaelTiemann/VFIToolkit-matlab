function [V, Policy]=ValueFnIter_FHorz_EpsteinZin_nod_e_raw(n_a,n_z,n_e,N_j, a_grid, z_gridvals_J,e_gridvals_J,pi_z_J,pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8)
...
if warmglow==1
    ...
    if ~isfield(vfoptions,'V_Jplus1')
        ...
        if vfoptions.lowmemory==0 || vfoptions.lowmemory==1
            WGmatrix=WGmatrix.*ones(1,1,N_z); % (aprime,1,z), matching temp4 (spans z at lowmemory 0 and 1, as only e is looped)
        elseif vfoptions.lowmemory==2
            % WGmatrix=WGmatrix; % (aprime,1) column, matching the per-z temp4
        end
    end
else
    WGmatrix=0;
