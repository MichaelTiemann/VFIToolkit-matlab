...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, n_e, d_gridvals, a1prime_grid(a1primeindexes_z), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2);
    ...
    elseif vfoptions.lowmemory==1
        error('lowmem=1 for QH+ExpAssetze N GI2A V_Jplus1 init not yet implemented')
    elseif vfoptions.lowmemory==2
        error('lowmem=2 for QH+ExpAssetze N GI2A V_Jplus1 init not yet implemented')
    end
end
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            midpoint_alt=max(min(maxindex_alt,N_a1-1),2);
            a1primeindexes_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_alt_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, n_e, d_gridvals, a1prime_grid(a1primeindexes_alt), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
            aprimez_alt_z=d2ind_vec + N_d2*(a1primeindexes_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_e-1),-6);
            ...
            isInfLower=(ReturnMatrix_ii_alt_z(linidx_lower)==-Inf);
            isInfUpper=(ReturnMatrix_ii_alt_z(linidx_upper)==-Inf);
            inLowerStrict=(maxindexL2a1_alt>=2)         & (maxindexL2a1_alt<=n2short+1);
            inUpperStrict=(maxindexL2a1_alt>=n2short+3) & (maxindexL2a1_alt<=n2long-1);
            PolicyaltL2flag(1,:,z_c,:,jj)=2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
            % Main
            ...
            aprimez_z=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_e-1),-6);
