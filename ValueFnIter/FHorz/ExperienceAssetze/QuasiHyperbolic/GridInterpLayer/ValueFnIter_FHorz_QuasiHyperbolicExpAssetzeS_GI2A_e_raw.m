...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
            entireRHS_hat_e=ReturnMatrix_e+repelem(DiscountedEV_hat_e,N_d1,1,1,1,1,1,1);
            [~,maxindex_e]=max(entireRHS_hat_e,[],2);
            midpoint_e=max(min(maxindex_e,N_a1-1),2);
            a1primeindexes_e=(midpoint_e+(midpoint_e-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1prime_grid(a1primeindexes_e), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
            aprimez_e=d2ind_vec + N_d2*(a1primeindexes_e-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
            entireRHS_ii_hat_e  =reshape(ReturnMatrix_ii_e+DiscountedEVinterp_hat_e(aprimez_e),  [N_d*n2long*N_a2,N_a,N_z,1]);
            entireRHS_ii_under_e=reshape(ReturnMatrix_ii_e+DiscountedEVinterp_under_e(aprimez_e),[N_d*n2long*N_a2,N_a,N_z,1]);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                midpoint_ze=max(min(maxindex_ze,N_a1-1),2);
                a1primeindexes_ze=(midpoint_ze+(midpoint_ze-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, special_n_e, d_gridvals, a1prime_grid(a1primeindexes_ze), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            midpoint_e=max(min(maxindex_e,N_a1-1),2);
            a1primeindexes_e=(midpoint_e+(midpoint_e-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1prime_grid(a1primeindexes_e), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
            aprimez_e=d2ind_vec + N_d2*(a1primeindexes_e-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
            entireRHS_ii_hat_e  =reshape(ReturnMatrix_ii_e+DiscountedEVinterp_hat_e(aprimez_e),  [N_d*n2long*N_a2,N_a,N_z,1]);
            entireRHS_ii_under_e=reshape(ReturnMatrix_ii_e+DiscountedEVinterp_under_e(aprimez_e),[N_d*n2long*N_a2,N_a,N_z,1]);
            [Vtempii_hat,maxindexL2]=max(entireRHS_ii_hat_e,[],1);
