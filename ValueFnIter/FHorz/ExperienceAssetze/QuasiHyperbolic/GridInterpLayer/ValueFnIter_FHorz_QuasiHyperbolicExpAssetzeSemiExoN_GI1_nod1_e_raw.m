function [V,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetzeSemiExoN_GI1_nod1_e_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,n_e,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Naive quasi-hyperbolic + ExperienceAssetze + SemiExo, GridInterpLayer (GI1, no d1).
...
Policyalt=zeros(4,N_a,N_bothz,N_e,N_j,'gpuArray'); % (d2, d3, midpoint, L2ind)
...
Policy3_ford3_alt=zeros(3,N_a,N_bothz,N_e,N_d3,'gpuArray'); % (d2, midpoint, L2ind)
flag_ford3_alt=2*ones(N_a,N_bothz,N_e,N_d3,'gpuArray');
V_ford3_tilde=zeros(N_a,N_bothz,N_e,N_d3,vfoptions.precision,'gpuArray');
Policy3_ford3_tilde=zeros(3,N_a,N_bothz,N_e,N_d3,'gpuArray'); % (d2, midpoint, L2ind)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_alt=ReturnMatrix_ii_alt+reshape(DiscountedEVinterp_alt(d2a1primea2bothze(:)),[N_d2*n2long,N_a1*N_a2,N_bothz,N_e]);
            ...
            d2a1primea2bothze=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*bothzind+N_d2*N_a1prime*N_a2*N_bothz*shiftdim((0:1:N_e-1),-4);
            entireRHS_ii_tilde=ReturnMatrix_ii_tilde+reshape(DiscountedEVinterp_tilde(d2a1primea2bothze(:)),[N_d2*n2long,N_a1*N_a2,N_bothz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii_alt=ReturnMatrix_ii_alt+reshape(DiscountedEVinterp_alt_e(d2a1primea2bothz(:)),[N_d2*n2long,N_a1*N_a2,N_bothz]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_alt,[],1);
                V_ford3_alt(:,:,e_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d2)+1;
                allind=d_ind+N_d2*aind+N_d2*N_a*bothzBind;
                ...
                d2a1primea2bothz=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*shiftdim((0:1:N_bothz-1),-3);
                entireRHS_ii_tilde=ReturnMatrix_ii_tilde+reshape(DiscountedEVinterp_tilde_e(d2a1primea2bothz(:)),[N_d2*n2long,N_a1*N_a2,N_bothz]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_tilde,[],1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_alt=ReturnMatrix_ii_alt+reshape(DiscountedEVinterp_alt_zbe(d2a1primea2bothz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
                    [Vtempii,maxindexL2]=max(entireRHS_ii_alt,[],1);
                    V_ford3_alt(:,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                    d_ind=rem(maxindexL2-1,N_d2)+1;
                    allind=d_ind+N_d2*aind+N_d2*N_a*semizBind;
                    ...
                    d2a1primea2bothz=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*shiftdim((0:1:N_semiz-1),-3);
                    entireRHS_ii_tilde=ReturnMatrix_ii_tilde+reshape(DiscountedEVinterp_tilde_zbe(d2a1primea2bothz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
                    [Vtempii,maxindexL2]=max(entireRHS_ii_tilde,[],1);
