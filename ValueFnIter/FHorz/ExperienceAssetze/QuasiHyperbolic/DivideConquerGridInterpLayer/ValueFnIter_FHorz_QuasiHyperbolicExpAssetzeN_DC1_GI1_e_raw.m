function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetzeN_DC1_GI1_e_raw(n_d1,n_d2,n_a1,n_a2,n_z,n_e,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting + ExperienceAssetze, Divide-and-Conquer (DC1 over a1prime) with grid interpolation layer (GI1) on a1.
% d2 determines the experience asset a2, a1 is the standard endogenous state.
% aprimeFn=aprimeFn(d2,a2,z,e,...): the experience-asset transition depends on BOTH the current
% markov z and the current i.i.d. e, so a2primeIndex/a2primeProbs carry a z and an e dimension.
% There is only one e in this family (the aprimeFn shock IS the i.i.d. shock of the value fn):
% eprime is integrated out of V with pi_e_J into EVpre BEFORE the a2 lottery, and zprime is
% integrated out with pi_z_J AFTER it; both happen before any discounting, so the dual touches
% only the discount factors, never the lottery.
...
if vfoptions.lowmemory==0
    midpoint=zeros(N_d,1,N_a1,N_a2,N_z,N_e,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
 if vfoptions.lowmemory==0
    ...
    DiscountedEVinterp_alt=permute(interp1(a1_gridvals,permute(DiscountedEV_alt,[2,1,3,4,5,6]),a1prime_grid),[2,1,3,4,5,6]);   % [N_d2,N_a1prime,1,N_a2,N_z,N_e]
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_z,n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind+N_d2*N_a*N_z*eind; % [N_d,maxgap_V+1,1,N_a2,N_z,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_z,N_e]
        ...
        % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2-by-n_z-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z,N_e]; Level=2, Refine=0
        da1primea2z=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind+N_d2*N_a1prime*N_a2*N_z*eind; % [N_d,n2long,N_a1,N_a2,N_z,N_e]; linear index into DiscountedEVinterp_alt [N_d2,N_a1prime,1,N_a2,N_z,N_e]
        entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z,N_e]);
        [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
        Valt(:,:,:,N_j)=shiftdim(Vtempii,1);
        d_indalt=rem(maxindexL2alt-1,N_d)+1;
        allindalt=d_indalt+N_d*aind+N_d*N_a*zeindB; % midpoint is n_d-by-1-by-n_a1-by-n_a2-by-n_z-by-n_e
        Policyalt(1,:,:,:,N_j)=d_indalt; % d2
        Policyalt(2,:,:,:,N_j)=shiftdim(squeeze(midpoint(allindalt)),-1); % a1prime midpoint
        Policyalt(3,:,:,:,N_j)=shiftdim(ceil(maxindexL2alt/N_d),-1); % a1primeL2ind
        % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
        L2offsetalt = ceil(maxindexL2alt/N_d);
        linidx_loweralt = d_indalt                 + N_d*n2long*aind + N_d*n2long*N_a*zeindB;
        linidx_upperalt = d_indalt + N_d*(n2long-1) + N_d*n2long*aind + N_d*n2long*N_a*zeindB;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_z-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind+N_d2*N_a*N_z*eind; % [N_d,maxgap+1,1,N_a2,N_z,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_z,N_e]
        ...
        % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2-by-n_z-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z,N_e]; Level=2, Refine=0
        da1primea2z=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind+N_d2*N_a1prime*N_a2*N_z*eind; % [N_d,n2long,N_a1,N_a2,N_z,N_e]; linear index into DiscountedEVinterp_tilde [N_d2,N_a1prime,1,N_a2,N_z,N_e]
        entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    ...
                    entireRHS_ii_e=ReturnMatrix_ii_dc+DiscountedEV_alt_e(d2aprimez);
            ...
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint, fine index
            % aprime possibilities are n_d2-by-n2long-by-n_a1-by-n_a2-by-n_z
            ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,n2long,n_a1,n_a2,n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
            da1primea2z=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind; % [N_d,n2long,N_a1,N_a2,N_z]; linear index into DiscountedEVinterp_alt_e [N_d2,N_a1prime,1,N_a2,N_z]
            entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt_e(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
            Valt(:,:,e_c,N_j)=shiftdim(Vtempii,1);
            d_indalt=rem(maxindexL2alt-1,N_d)+1;
            allindalt=d_indalt+N_d*aind+N_d*N_a*zindB; % midpoint is n_d2-by-1-by-n_a1-by-n_a2-by-n_z
            Policyalt(1,:,:,e_c,N_j)=d_indalt; % d2
            Policyalt(2,:,:,e_c,N_j)=shiftdim(squeeze(midpoint(allindalt)),-1); % a1prime midpoint
            Policyalt(3,:,:,e_c,N_j)=shiftdim(ceil(maxindexL2alt/N_d),-1); % a1primeL2ind
            % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
            L2offsetalt = ceil(maxindexL2alt/N_d);
            linidx_loweralt = d_indalt                 + N_d*n2long*aind + N_d*n2long*N_a*zindB;
            linidx_upperalt = d_indalt + N_d*(n2long-1) + N_d*n2long*aind + N_d*n2long*N_a*zindB;
            isInfLoweralt = (ReturnMatrix_L2(linidx_loweralt) == -Inf);
            isInfUpperalt = (ReturnMatrix_L2(linidx_upperalt) == -Inf);
            inLowerStrictalt = (L2offsetalt >= 2)         & (L2offsetalt <= n2short+1);
            inUpperStrictalt = (L2offsetalt >= n2short+3) & (L2offsetalt <= n2long-1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_z
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    ...
                    entireRHS_ii_e=ReturnMatrix_ii_dc+DiscountedEV_tilde_e(d2aprimez);
                    [~,maxindex]=max(entireRHS_ii_e,[],2);
                    midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
            ...
            % aprime possibilities are n_d2-by-n2long-by-n_a1-by-n_a2-by-n_z
            ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,n2long,n_a1,n_a2,n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
            da1primea2z=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind; % [N_d,n2long,N_a1,N_a2,N_z]; linear index into DiscountedEVinterp_tilde_e [N_d2,N_a1prime,1,N_a2,N_z]
            entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde_e(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,special_n_z,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                        ...
                        entireRHS_ii_ze=ReturnMatrix_ii_dc+DiscountedEV_alt_ze(d2aprime);
                ...
                ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n2long, n_a1,n_a2,special_n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
                da1primea2=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d,n2long,N_a1,N_a2]; linear index into DiscountedEVinterp_alt_ze [N_d2,N_a1prime,1,N_a2]
                entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt_ze(da1primea2),[N_d*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
                Valt(:,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                d_indalt=rem(maxindexL2alt-1,N_d)+1;
                allindalt=d_indalt+N_d*aind; % midpoint is n_d-by-1-by-n_a1-by-n_a2
                Policyalt(1,:,z_c,e_c,N_j)=d_indalt; % d2
                Policyalt(2,:,z_c,e_c,N_j)=shiftdim(squeeze(midpoint(allindalt)),-1); % a1prime midpoint
                Policyalt(3,:,z_c,e_c,N_j)=shiftdim(ceil(maxindexL2alt/N_d),-1); % a1primeL2ind
                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                L2offsetalt = ceil(maxindexL2alt/N_d);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_z,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                        ...
                        entireRHS_ii_ze=ReturnMatrix_ii_dc+DiscountedEV_tilde_ze(d2aprime);
                        [~,maxindex]=max(entireRHS_ii_ze,[],2);
                        midpoint(:,1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                ...
                % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2
                ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n2long, n_a1,n_a2,special_n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
                da1primea2=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d,n2long,N_a1,N_a2]; linear index into DiscountedEVinterp_tilde_ze [N_d2,N_a1prime,1,N_a2]
                entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde_ze(da1primea2),[N_d*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                % aprime possibilities are n_d-by-maxgap_V(ii)+1-by-1-by-n_a2-by-n_z-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_z,n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind+N_d2*N_a*N_z*eind; % [N_d,maxgap_V+1,1,N_a2,N_z,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2,N_z,N_e]
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprimez);
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoint(:,1,curraindex,:,:,:)=maxindex+(loweredge-1);
        ...
        a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint, fine index
        % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2-by-n_z-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z,N_e]; Level=2, Refine=0
        da1primea2z=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind+N_d2*N_a1prime*N_a2*N_z*eind; % [N_d,n2long,N_a1,N_a2,N_z,N_e]; linear index into DiscountedEVinterp_alt [N_d2,N_a1prime,1,N_a2,N_z,N_e]
        entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z,N_e]);
        [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
        ...
        Policyalt(2,:,:,:,jj)=shiftdim(squeeze(midpoint(allindalt)),-1); % a1prime midpoint
        Policyalt(3,:,:,:,jj)=shiftdim(ceil(maxindexL2alt/N_d),-1); % a1primeL2ind
        % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
        L2offsetalt = ceil(maxindexL2alt/N_d);
        linidx_loweralt = d_indalt                 + N_d*n2long*aind + N_d*n2long*N_a*zeindB;
        linidx_upperalt = d_indalt + N_d*(n2long-1) + N_d*n2long*aind + N_d*n2long*N_a*zeindB;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_z-by-n_e
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind+N_d2*N_a*N_z*eind; % [N_d,maxgap+1,1,N_a2,N_z,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2,N_z,N_e]
                entireRHS_ii=ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprimez);
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoint(:,1,curraindex,:,:,:)=maxindex+(loweredge-1);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                    % aprime possibilities are n_d-by-maxgap_V(ii)+1-by-1-by-n_a2-by-n_z
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind; % [N_d,maxgap_V+1,1,N_a2,N_z]; linear index into DiscountedEV_alt_e [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii_e=ReturnMatrix_ii_dc+DiscountedEV_alt_e(d2aprimez);
                    [~,maxindex]=max(entireRHS_ii_e,[],2);
                    midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
            ...
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint, fine index
            % aprime possibilities are n_d2-by-n2long-by-n_a1-by-n_a2-by-n_z
            ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,n2long,n_a1,n_a2,n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
            da1primea2z=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind; % [N_d,n2long,N_a1,N_a2,N_z]; linear index into DiscountedEVinterp_alt_e [N_d2,N_a1prime,1,N_a2,N_z]
            entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt_e(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
            ...
            Policyalt(2,:,:,e_c,jj)=shiftdim(squeeze(midpoint(allindalt)),-1); % a1prime midpoint
            Policyalt(3,:,:,e_c,jj)=shiftdim(ceil(maxindexL2alt/N_d),-1); % a1primeL2ind
            % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
            L2offsetalt = ceil(maxindexL2alt/N_d);
            linidx_loweralt = d_indalt                 + N_d*n2long*aind + N_d*n2long*N_a*zindB;
            linidx_upperalt = d_indalt + N_d*(n2long-1) + N_d*n2long*aind + N_d*n2long*N_a*zindB;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2-by-n_z
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind; % [N_d,maxgap+1,1,N_a2,N_z]; linear index into DiscountedEV_tilde_e [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii_e=ReturnMatrix_ii_dc+DiscountedEV_tilde_e(d2aprimez);
                    [~,maxindex]=max(entireRHS_ii_e,[],2);
                    midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policyalt(2,:,z_c,e_c,jj)=shiftdim(squeeze(midpoint(allindalt)),-1); % a1prime midpoint
                Policyalt(3,:,z_c,e_c,jj)=shiftdim(ceil(maxindexL2alt/N_d),-1); % a1primeL2ind
                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                L2offsetalt = ceil(maxindexL2alt/N_d);
                linidx_loweralt = d_indalt                 + N_d*n2long*aind;
                linidx_upperalt = d_indalt + N_d*(n2long-1) + N_d*n2long*aind;
