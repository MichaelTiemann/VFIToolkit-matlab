...
% lowmemory=0 full vectorization; lowmemory=1 loops e (z vectorized); lowmemory=2 nested z+e.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            % --- Inlined lm=1 N DC coarse + GI fine step (per e; z vectorized) ---
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
            entireRHS_alt=ReturnMatrix_ii_e+DiscountedEV_alt_e;
            [~,maxindex1_alt]=max(entireRHS_alt,[],2);
            ...
            entireRHS=ReturnMatrix_ii_e+DiscountedEV_e;
            [~,maxindex1]=max(entireRHS,[],2);
            midpoint_z(:,1,:,level1ii,:,:,:,:)=maxindex1;
            maxgap=squeeze(max(max(max(max(max(max( maxindex1(:,1,:,2:end,:,:,:,:)-maxindex1(:,1,:,1:end-1,:,:,:,:), [],8),[],7),[],6),[],5),[],3),[],1));
            for ii=1:(vfoptions.level1n-1)
            ...
            entireRHS_ii_alt=reshape(ReturnMatrix_ii_alt_e+DiscountedEVinterp_alt_e(aprimez_alt),[N_d2*n2long*N_a2,N_a,N_z,1]);
            [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_alt,[],1);
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii_e+DiscountedEVinterp_e(aprimez),[N_d2*n2long*N_a2,N_a,N_z,1]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
