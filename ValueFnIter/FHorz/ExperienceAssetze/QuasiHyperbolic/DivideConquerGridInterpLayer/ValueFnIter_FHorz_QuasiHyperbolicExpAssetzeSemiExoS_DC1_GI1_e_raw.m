function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetzeSemiExoS_DC1_GI1_e_raw(n_d1,n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,n_e,N_j, d12_gridvals, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Sophisticated quasi-hyperbolic + ExperienceAssetze + SemiExo + DivideConquer + GridInterpLayer.
...
% Policy rows: (d1, d2, d3, midpoint, L2ind) + L2flag appended.
%
...
Policy4_ford3=zeros(4,N_a,N_bothz,N_e,N_d3,'gpuArray'); % (d1, d2, midpoint, L2ind)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            Policy4_ford3(1,:,:,:,d3_c)=rem(d_ind-1,N_d1)+1;
            Policy4_ford3(2,:,:,:,d3_c)=ceil(d_ind/N_d1);
            Policy4_ford3(3,:,:,:,d3_c)=shiftdim(squeeze(midpoint(allind)),-1);
            Policy4_ford3(4,:,:,:,d3_c)=shiftdim(ceil(maxindexL2/N_d12),-1);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                Policy4_ford3(1,:,:,e_c,d3_c)=rem(d_ind-1,N_d1)+1;
                Policy4_ford3(2,:,:,e_c,d3_c)=ceil(d_ind/N_d1);
                Policy4_ford3(3,:,:,e_c,d3_c)=shiftdim(squeeze(midpoint(allind)),-1);
                Policy4_ford3(4,:,:,e_c,d3_c)=shiftdim(ceil(maxindexL2/N_d12),-1);
                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                L2offset = ceil(maxindexL2/N_d12);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,vfoptions.level1n,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d123_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,1,0);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                            [~,maxindex]=max(ReturnMatrix_ii,[],2);
                            midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(:,1,ii,:,:);
                    ...
                    midpoint=max(min(midpoint,n_a1(1)-1),2);
                    aprimeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d123_gridvals_val, a1prime_grid(aprimeindexes), a1_gridvals, a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,2,0);
                    [Vtempii,maxindexL2]=max(ReturnMatrix_ii,[],1);
                    Vhat_ford3(:,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                    d_ind=rem(maxindexL2-1,N_d12)+1;
                    allind=d_ind+N_d12*aind+N_d12*N_a*semizBind;
                    Policy4_ford3(1,:,semizblock,e_c,d3_c)=rem(d_ind-1,N_d1)+1;
                    Policy4_ford3(2,:,semizblock,e_c,d3_c)=ceil(d_ind/N_d1);
                    Policy4_ford3(3,:,semizblock,e_c,d3_c)=shiftdim(squeeze(midpoint(allind)),-1);
                    Policy4_ford3(4,:,semizblock,e_c,d3_c)=shiftdim(ceil(maxindexL2/N_d12),-1);
                    % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                    L2offset = ceil(maxindexL2/N_d12);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    Policy4_ford3(1,:,z_c,e_c,d3_c)=rem(d_ind-1,N_d1)+1;
                    Policy4_ford3(2,:,z_c,e_c,d3_c)=ceil(d_ind/N_d1);
                    Policy4_ford3(3,:,z_c,e_c,d3_c)=shiftdim(squeeze(midpoint(allind)),-1);
                    Policy4_ford3(4,:,z_c,e_c,d3_c)=shiftdim(ceil(maxindexL2/N_d12),-1);
                    % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                    L2offset = ceil(maxindexL2/N_d12);
    ...
    Policy(2,:,:,:,N_j)=reshape(Policy4_ford3(2+temp),[1,N_a,N_bothz,N_e]);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d123_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
            entireRHS_ii=ReturnMatrix_ii+repelem(beta0beta*EVreshape,N_d1,1,1,1,1,1);
            ...
            EVfine=reshape(EVinterp(d2a1primea2bothze(:)),[N_d12*n2long,N_a1*N_a2,N_bothz,N_e]);
            ...
            linidx=reshape(maxindexL2,[1,N_a*N_bothz*N_e])+N_d12*n2long*(0:N_a*N_bothz*N_e-1);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                % hat (QH-perceived): F + beta0*beta*EV, full divide-conquer + grid-interp
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,special_n_e, d123_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1,0);
                entireRHS_ii=ReturnMatrix_ii+repelem(beta0beta*EVreshape_e,N_d1,1,1,1,1);
                [~,maxindex1]=max(entireRHS_ii,[],2);
                midpoint(:,1,level1ii,:,:)=maxindex1;
                maxgap=squeeze(max(max(max(maxindex1(:,1,2:end,:,:)-maxindex1(:,1,1:end-1,:,:),[],5),[],4),[],1));
                for ii=1:(vfoptions.level1n-1)
                ...
                EVfine=reshape(EVinterp_e(d2a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
                entireRHS_L2=ReturnMatrix_L2+beta0beta*EVfine;
                [Vtempii,maxindexL2]=max(entireRHS_L2,[],1);
                Vhat_ford3(:,:,e_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d12)+1;
                ...
                % under: F + beta*EV gathered at the hat-argmax (= Vhat + (beta-beta0beta)*EV_at_policy)
                linidx=reshape(maxindexL2,[1,N_a*N_bothz])+N_d12*n2long*(0:N_a*N_bothz-1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    % hat (QH-perceived): F + beta0*beta*EV, full divide-conquer + grid-interp
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,vfoptions.level1n,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d123_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,1,0);
                    entireRHS_ii=ReturnMatrix_ii+repelem(beta0beta*EVreshape_zbe,N_d1,1,1,1,1);
                    [~,maxindex1]=max(entireRHS_ii,[],2);
                    midpoint(:,1,level1ii,:,:)=maxindex1;
                    maxgap=squeeze(max(max(max(maxindex1(:,1,2:end,:,:)-maxindex1(:,1,1:end-1,:,:),[],5),[],4),[],1));
                    for ii=1:(vfoptions.level1n-1)
                    ...
                    EVfine=reshape(EVinterp_zbe(d2a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*EVfine;
                    [Vtempii,maxindexL2]=max(entireRHS_L2,[],1);
                    Vhat_ford3(:,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                    d_ind=rem(maxindexL2-1,N_d12)+1;
                    ...
                    % under: F + beta*EV gathered at the hat-argmax (= Vhat + (beta-beta0beta)*EV_at_policy)
                    linidx=reshape(maxindexL2,[1,N_a*N_semiz])+N_d12*n2long*(0:N_a*N_semiz-1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    % hat (QH-perceived): F + beta0*beta*EV, full divide-conquer + grid-interp
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,vfoptions.level1n,n_a2,special_n_bothz,special_n_e, d123_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
                    entireRHS_ii=ReturnMatrix_ii+repelem(beta0beta*EVreshape_ze,N_d1,1,1,1,1);
                    [~,maxindex1]=max(entireRHS_ii,[],2);
                    midpoint(:,1,level1ii,:)=maxindex1;
                    maxgap=squeeze(max(max(maxindex1(:,1,2:end,:)-maxindex1(:,1,1:end-1,:),[],4),[],1));
                    for ii=1:(vfoptions.level1n-1)
                    ...
                    EVfine=reshape(EVinterp_ze(d2a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                    entireRHS_L2=ReturnMatrix_L2+beta0beta*EVfine;
                    [Vtempii,maxindexL2]=max(entireRHS_L2,[],1);
                    Vhat_ford3(:,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
                    d_ind=rem(maxindexL2-1,N_d12)+1;
                    ...
                    % under: F + beta*EV gathered at the hat-argmax (= Vhat + (beta-beta0beta)*EV_at_policy)
                    linidx=reshape(maxindexL2,[1,N_a])+N_d12*n2long*(0:N_a-1);
...
for reverse_j=1:N_j-1
    ...
    EVpre=sum(Vunderbar(:,:,:,jj+1).*shiftdim(pi_e_J(:,jj),-2),3);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % hat (QH-perceived): F + beta0*beta*EV, full divide-conquer + grid-interp
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d123_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,1,0);
            entireRHS_ii=ReturnMatrix_ii+repelem(beta0beta*EVreshape,N_d1,1,1,1,1,1);
            [~,maxindex1]=max(entireRHS_ii,[],2);
            midpoint(:,1,level1ii,:,:,:)=maxindex1;
            maxgap=squeeze(max(max(max(max(maxindex1(:,1,2:end,:,:,:)-maxindex1(:,1,1:end-1,:,:,:),[],6),[],5),[],4),[],1));
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,n_e, d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    d2aprimeze=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind+N_d2*N_a1*N_a2*N_bothz*shiftdim((0:1:N_e-1),-4);
                    entireRHS_ii=ReturnMatrix_ii+beta0beta*EVreshape(d2aprimeze);
                    [~,maxindex]=max(entireRHS_ii,[],2);
                    midpoint(:,1,curraindex,:,:,:)=maxindex+(loweredge-1);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                % hat (QH-perceived): F + beta0*beta*EV, full divide-conquer + grid-interp
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,special_n_e, d123_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,1,0);
                entireRHS_ii=ReturnMatrix_ii+repelem(beta0beta*EVreshape_e,N_d1,1,1,1,1);
                [~,maxindex1]=max(entireRHS_ii,[],2);
                midpoint(:,1,level1ii,:,:)=maxindex1;
                maxgap=squeeze(max(max(max(maxindex1(:,1,2:end,:,:)-maxindex1(:,1,1:end-1,:,:),[],5),[],4),[],1));
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0);
                        d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind;
                        entireRHS_ii=ReturnMatrix_ii+beta0beta*EVreshape_e(d2aprimez);
                        [~,maxindex]=max(entireRHS_ii,[],2);
                        midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
    ...
    Policy(2,:,:,:,jj)=reshape(Policy4_ford3(2+temp),[1,N_a,N_bothz,N_e]);
