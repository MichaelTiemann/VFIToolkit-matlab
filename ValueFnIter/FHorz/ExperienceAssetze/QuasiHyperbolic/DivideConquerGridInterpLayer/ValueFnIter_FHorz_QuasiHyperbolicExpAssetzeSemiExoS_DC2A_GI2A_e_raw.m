...
Policy4_ford3=zeros(4,N_a,N_bothz,N_e,N_d3,'gpuArray'); % (d12, midpoint, a2prime, L2ind)
flag_ford3=2*ones(1,N_a,N_bothz,N_e,N_d3,'gpuArray'); % L2 flag per d3, aggregated after d3 max
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_hat=reshape(ReturnMatrix_L2+DiscountedEVinterp_hat(aprimez),[N_d12*n2long*N_a2,N_a,N_bothz,N_e]);
            entireRHS_under=reshape(ReturnMatrix_L2+DiscountedEVinterp_under(aprimez),[N_d12*n2long*N_a2,N_a,N_bothz,N_e]);
            [Vtempii_hat,maxindexL2]=max(entireRHS_hat,[],1);
            firstdim=N_d12*n2long*N_a2;
            maxindexfull=maxindexL2 + firstdim*aind + firstdim*N_a*bothzBind + firstdim*N_a*N_bothz*eBind;
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                entireRHS_hat=reshape(ReturnMatrix_L2+DiscountedEVinterp_hat_e(aprime),[N_d12*n2long*N_a2,N_a,N_bothz,1]);
                entireRHS_under=reshape(ReturnMatrix_L2+DiscountedEVinterp_under_e(aprime),[N_d12*n2long*N_a2,N_a,N_bothz,1]);
                [Vtempii_hat,maxindexL2]=max(entireRHS_hat,[],1);
                firstdim=N_d12*n2long*N_a2;
                maxindexfull=maxindexL2 + firstdim*aind + firstdim*N_a*bothzBind;
                Vtempii_under=entireRHS_under(maxindexfull);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_hat_zbe,N_d1,1,1,1,1,1,1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii_ze=ReturnMatrix_inner+DiscountedEV_hat_zbe(d2aprime);
                    ...
                    entireRHS_hat=reshape(ReturnMatrix_L2+DiscountedEVinterp_hat_zbe(aprime),[N_d12*n2long*N_a2,N_a,N_semiz]);
                    entireRHS_under=reshape(ReturnMatrix_L2+DiscountedEVinterp_under_zbe(aprime),[N_d12*n2long*N_a2,N_a,N_semiz]);
                    [Vtempii_hat,maxindexL2]=max(entireRHS_hat,[],1);
                    firstdim=N_d12*n2long*N_a2;
                    maxindexfull=maxindexL2 + firstdim*aind + firstdim*N_a*semizBind;
                    Vtempii_under=entireRHS_under(maxindexfull);
                    Vhat_ford3(:,semizblock,e_c,d3_c)=shiftdim(Vtempii_hat,1);
                    Vunderbar_ford3(:,semizblock,e_c,d3_c)=shiftdim(Vtempii_under,1);
                    d_ind        =rem(maxindexL2-1,N_d12)+1;
                    maxindexL2a1 =rem(floor((maxindexL2-1)/N_d12),n2long)+1;
                    maxindexL2a2 =floor((maxindexL2-1)/(N_d12*n2long))+1;
                    allind=d_ind + N_d12*(maxindexL2a2-1) + N_d12*N_a2*aind + N_d12*N_a2*N_a*semizBind;
