...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n_a1, n_a1,n_a2, n_z, n_e, d_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,0,0);
        entireRHS=ReturnMatrix+repelem(beta*entireEV,N_d1,1,1,1);
        [Vtemp,maxindexalt]=max(entireRHS,[],1);
        Valt(:,:,:,N_j)=shiftdim(Vtemp,1);
        Policyalt(:,:,:,N_j)=shiftdim(maxindexalt,1);
        entireRHS=ReturnMatrix+repelem(beta0beta*entireEV,N_d1,1,1,1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n_a1, n_a1,n_a2, n_z, special_n_e, d_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,0,0);
            entireRHS_e=ReturnMatrix_e+repelem(beta*entireEV_e,N_d1,1,1);
            [Vtemp,maxindexalt]=max(entireRHS_e,[],1);
            Valt(:,:,e_c,N_j)=shiftdim(Vtemp,1);
            Policyalt(:,:,e_c,N_j)=shiftdim(maxindexalt,1);
            entireRHS_e=ReturnMatrix_e+repelem(beta0beta*entireEV_e,N_d1,1,1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n_a1, n_a1,n_a2, special_n_z, special_n_e, d_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,0,0);
                entireRHS_ze=ReturnMatrix_ze+repelem(beta*entireEV_ze,N_d1,1);
                [Vtemp,maxindexalt]=max(entireRHS_ze,[],1);
                Valt(:,z_c,e_c,N_j)=Vtemp;
                Policyalt(:,z_c,e_c,N_j)=maxindexalt;
                entireRHS_ze=ReturnMatrix_ze+repelem(beta0beta*entireEV_ze,N_d1,1);
