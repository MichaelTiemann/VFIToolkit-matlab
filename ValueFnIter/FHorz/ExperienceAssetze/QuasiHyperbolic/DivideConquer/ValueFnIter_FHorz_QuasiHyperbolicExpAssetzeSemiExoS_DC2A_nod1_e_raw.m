function [Vhat,Policy4,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetzeSemiExoS_DC2A_nod1_e_raw(n_d2, n_d3, n_a1, n_a2, n_a3, n_z, n_semiz, n_e, N_j, d2_gridvals, d3_grid, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Sophisticated quasi-hyperbolic + two standard endogenous assets + experienceassetze + semi-exog state, Divide-and-Conquer (DC2A over a1prime). No d1.
...
% Policy4 stores (d2, d3, a1prime, a2prime).
%
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        Policy4(1,curraindex,:,:,N_j)=rem(dind-1,N_d2)+1; % d2
        Policy4(2,curraindex,:,:,N_j)=ceil(dind/N_d2); % d3
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Policy4(1,curraindex,:,e_c,N_j)=rem(dind-1,N_d2)+1; % d2
            Policy4(2,curraindex,:,e_c,N_j)=ceil(dind/N_d2); % d3
            aprimeind=ceil(maxindex2/N_d); % this is the joint (a1prime,a2prime)
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy4(1,curraindex,zind,e_c,N_j)=rem(dind-1,N_d2)+1; % d2
                Policy4(2,curraindex,zind,e_c,N_j)=ceil(dind/N_d2); % d3
                aprimeind=ceil(maxindex2/N_d); % this is the joint (a1prime,a2prime)
    ...
    elseif vfoptions.lowmemory==3
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                ...
                Policy4(1,curraindex,z_c,e_c,N_j)=rem(dind-1,N_d2)+1; % d2
                Policy4(2,curraindex,z_c,e_c,N_j)=ceil(dind/N_d2); % d3
                aprimeind=ceil(maxindex2/N_d); % this is the joint (a1prime,a2prime)
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            DiscountedEV_under=beta*reshape(entireEV,[N_d2,N_a1,N_a2,1,1,N_a3,N_bothz,N_e]);    % exponential
            DiscountedEV_hat=beta0beta*reshape(entireEV,[N_d2,N_a1,N_a2,1,1,N_a3,N_bothz,N_e]); % QH-perceived
            ...
            entireRHS_hat_flat=reshape(entireRHS_hat,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz,N_e]);
            [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
            entireRHS_under=ReturnMatrix_ii_d3+DiscountedEV_under;
            entireRHS_under_flat=reshape(entireRHS_under,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz,N_e]);
            M=vfoptions.level1n*N_a2*N_a3;
            maxindexfull=maxindex2 + (N_d2*N_a1*N_a2)*(0:M-1) + (N_d2*N_a1*N_a2)*M*shiftdim((0:N_bothz-1),-1) + (N_d2*N_a1*N_a2)*M*N_bothz*shiftdim((0:N_e-1),-2);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            aprimeProbs_d3(skipinterp)=0; % effectively skips interpolation
            entireEV=EV1.*aprimeProbs_d3+EV2.*(1-aprimeProbs_d3); % [N_d2*N_a1*N_a2,N_a3,N_bothz,N_e]
            DiscountedEV_under=beta*reshape(entireEV,[N_d2,N_a1,N_a2,1,1,N_a3,N_bothz,N_e]);
            DiscountedEV_hat=beta0beta*reshape(entireEV,[N_d2,N_a1,N_a2,1,1,N_a3,N_bothz,N_e]);
            ...
            for e_c=1:N_e
                ...
                entireRHS_hat=ReturnMatrix_ii_d3+DiscountedEV_hat_e;
                [~,maxindex1]=max(entireRHS_hat,[],2);
                entireRHS_hat_flat=reshape(entireRHS_hat,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz]);
                [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
                entireRHS_under=ReturnMatrix_ii_d3+DiscountedEV_under_e;
                entireRHS_under_flat=reshape(entireRHS_under,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz]);
                M=vfoptions.level1n*N_a2*N_a3;
                maxindexfull=maxindex2 + (N_d2*N_a1*N_a2)*(0:M-1) + (N_d2*N_a1*N_a2)*M*shiftdim((0:N_bothz-1),-1);
                Vtempii_under=entireRHS_under_flat(maxindexfull);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_hat=ReturnMatrix_ii_d3+DiscountedEV_hat_ze;
                    [~,maxindex1]=max(entireRHS_hat,[],2);
                    entireRHS_hat_flat=reshape(entireRHS_hat,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_semiz]);
                    [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
                    entireRHS_under=ReturnMatrix_ii_d3+DiscountedEV_under_ze;
                    entireRHS_under_flat=reshape(entireRHS_under,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_semiz]);
                    M=vfoptions.level1n*N_a2*N_a3;
                    maxindexfull=maxindex2 + (N_d2*N_a1*N_a2)*(0:M-1) + (N_d2*N_a1*N_a2)*M*shiftdim((0:N_semiz-1),-1);
                    Vtempii_under=entireRHS_under_flat(maxindexfull);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_hat=ReturnMatrix_ii_d3+DiscountedEV_hat_ze;
                    [~,maxindex1]=max(entireRHS_hat,[],2);
                    entireRHS_hat_flat=reshape(entireRHS_hat,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]);
                    [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
                    entireRHS_under=ReturnMatrix_ii_d3+DiscountedEV_under_ze;
                    entireRHS_under_flat=reshape(entireRHS_under,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]);
                    M=vfoptions.level1n*N_a2*N_a3;
    ...
    Vhat(:,:,:,N_j)=Vhat_jj;
    Policy4(2,:,:,:,N_j)=shiftdim(d3maxindex,-1); % d3 is just d3maxindex
    d3maxindex_lin=reshape(d3maxindex,[N_a*N_semiz*N_z*N_e,1]);
    d2aprime_ind=reshape(Policy_ford3_hat((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(d3maxindex_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
...
for reverse_j=1:N_j-1
    ...
    [Vhat_jj,d3maxindex]=max(V_ford3_hat,[],4);
    Vhat(:,:,:,jj)=Vhat_jj;
    Policy4(2,:,:,:,jj)=shiftdim(d3maxindex,-1); % d3 is just d3maxindex
    d3maxindex_lin=reshape(d3maxindex,[N_a*N_semiz*N_z*N_e,1]);
    d2aprime_ind=reshape(Policy_ford3_hat((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(d3maxindex_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
