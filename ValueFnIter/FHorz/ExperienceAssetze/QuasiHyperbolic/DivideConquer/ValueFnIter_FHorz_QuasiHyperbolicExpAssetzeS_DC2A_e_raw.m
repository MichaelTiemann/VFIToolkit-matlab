...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
            entireRHS_hat_e=ReturnMatrix_ii_e+repelem(DiscountedEV_hat_e,N_d1,1,1,1,1,1,1);
            [~,maxindex1_e]=max(entireRHS_hat_e,[],2);
            entireRHS_hat_flat=reshape(entireRHS_hat_e,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]);
            [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
            entireRHS_under_e=ReturnMatrix_ii_e+repelem(DiscountedEV_under_e,N_d1,1,1,1,1,1,1);
            entireRHS_under_flat=reshape(entireRHS_under_e,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]);
            M=vfoptions.level1n*N_a2*N_a3;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    firstdim=N_d*(maxgap(ii)+1)*N_a2;
                    Mblock=level1iidiff(ii)*N_a2*N_a3;
                    entireRHS_hat_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat_e(d2aprime_e),[firstdim,Mblock,N_z,1]);
                    entireRHS_under_e=reshape(ReturnMatrix_ii_e+DiscountedEV_under_e(d2aprime_e),[firstdim,Mblock,N_z,1]);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_hat_flat=reshape(entireRHS_hat_ze,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]);
                [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
                entireRHS_under_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_under_ze,N_d1,1,1,1,1,1);
                entireRHS_under_flat=reshape(entireRHS_under_ze,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]);
                M=vfoptions.level1n*N_a2*N_a3;
                ...
                a1pind=rem(floor((maxindex2-1)/N_d),N_a1)+1;
                a2pind=floor((maxindex2-1)/(N_d*N_a1))+1;
                Vhat(curraindex,z_c,e_c,N_j)         =Vtempii_hat;
                Vunderbar(curraindex,z_c,e_c,N_j)    =Vtempii_under;
                Policy(1,curraindex,z_c,e_c,N_j)     =dind;
                Policy(2,curraindex,z_c,e_c,N_j)     =a1pind;
                Policy(3,curraindex,z_c,e_c,N_j)     =a2pind;
                maxgap=squeeze(max(max(max(max( maxindex1_ze(:,1,:,2:end,:,:)-maxindex1_ze(:,1,:,1:end-1,:,:), [],6),[],5),[],3),[],1));
