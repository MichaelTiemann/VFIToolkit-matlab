function [Vhat,Policy4,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetzeSemiExoS_DC2A_e_raw(n_d1, n_d2, n_d3, n_a1, n_a2, n_a3, n_z, n_semiz, n_e, N_j, d12_gridvals, d2_gridvals, d3_grid, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Sophisticated quasi-hyperbolic + two standard endogenous assets + ExperienceAssetze + SemiExo, Divide-and-Conquer (DC2A over a1prime).
% SemiExo graft of ValueFnIter_FHorz_QuasiHyperbolicExpAssetzeS_DC2A_e_raw, following ValueFnIter_FHorz_ExpAssetzeSemiExo_DC2A_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy4(1,curraindex,zind,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                Policy4(2,curraindex,zind,e_c,N_j)=ceil(dind/N_d12); % d3
                aprimeind=ceil(maxindex2/N_d); % this is the joint (a1prime,a2prime)
                Policy4(3,curraindex,zind,e_c,N_j)=rem(aprimeind-1,N_a1)+1; % a1prime
                Policy4(4,curraindex,zind,e_c,N_j)=ceil(aprimeind/N_a1); % a2prime
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policy4(1,curraindex,zind,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                        Policy4(2,curraindex,zind,e_c,N_j)=ceil(dind/N_d12); % d3
                        Policy4(3,curraindex,zind,e_c,N_j)=a1prime_rec; % a1prime
                        Policy4(4,curraindex,zind,e_c,N_j)=a2pind; % a2prime
                    else
                        loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                        ...
                        Policy4(1,curraindex,zind,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                        Policy4(2,curraindex,zind,e_c,N_j)=ceil(dind/N_d12); % d3
                        Policy4(3,curraindex,zind,e_c,N_j)=loweredge(loweredge_idx); % a1prime
                        Policy4(4,curraindex,zind,e_c,N_j)=a2pind; % a2prime
                    end
                end
            end
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_hat_flat=reshape(entireRHS_hat,[N_d12*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz,N_e]);
            ...
            entireRHS_under_flat=reshape(entireRHS_under,[N_d12*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz,N_e]);
            M=vfoptions.level1n*N_a2*N_a3;
            maxindexfull=maxindex2 + (N_d12*N_a1*N_a2)*(0:M-1) + (N_d12*N_a1*N_a2)*M*shiftdim((0:N_bothz-1),-1) + (N_d12*N_a1*N_a2)*M*N_bothz*shiftdim((0:N_e-1),-2);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_hat=reshape(ReturnMatrix_ii_d3+DiscountedEV_hat(d2aprimez),[firstdim,Mblock,N_bothz,N_e]);
                    entireRHS_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[firstdim,Mblock,N_bothz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                [~,maxindex1]=max(entireRHS_hat,[],2);
                entireRHS_hat_flat=reshape(entireRHS_hat,[N_d12*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz]);
                [Vtempii_hat,maxindex2]=max(entireRHS_hat_flat,[],1);
                entireRHS_under=ReturnMatrix_ii_d3+repelem(DiscountedEV_under_e,N_d1,1,1,1,1,1,1);
                entireRHS_under_flat=reshape(entireRHS_under,[N_d12*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz]);
                M=vfoptions.level1n*N_a2*N_a3;
                maxindexfull=maxindex2 + (N_d12*N_a1*N_a2)*(0:M-1) + (N_d12*N_a1*N_a2)*M*shiftdim((0:N_bothz-1),-1);
                Vtempii_under=entireRHS_under_flat(maxindexfull);
                curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                         +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        firstdim=N_d12*(maxgap(ii)+1)*N_a2;
                        Mblock=level1iidiff(ii)*N_a2*N_a3;
                        entireRHS_hat=reshape(ReturnMatrix_ii_d3+DiscountedEV_hat_e(d2aprimez),[firstdim,Mblock,N_bothz]);
                        entireRHS_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under_e(d2aprimez),[firstdim,Mblock,N_bothz]);
                        [Vtempii_hat,maxindex]=max(entireRHS_hat,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                zind=(1:1:N_semiz)+N_semiz*(z_c-1);
                z_val=bothz_gridvals_J(zind,:,N_j);
                DiscountedEV_under_z=DiscountedEV_under(:,:,:,:,:,:,zind,:);
                DiscountedEV_hat_z=DiscountedEV_hat(:,:,:,:,:,:,zind,:);
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    DiscountedEV_under_ze=DiscountedEV_under_z(:,:,:,:,:,:,:,e_c);
                    DiscountedEV_hat_ze=DiscountedEV_hat_z(:,:,:,:,:,:,:,e_c);
                    ...
                    curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                             +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
                    V_ford3_hat(curraindex,zind,e_c,d3_c)=shiftdim(Vtempii_hat,1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            firstdim=N_d12*(maxgap(ii)+1)*N_a2;
                            Mblock=level1iidiff(ii)*N_a2*N_a3;
                            entireRHS_hat=reshape(ReturnMatrix_ii_d3+DiscountedEV_hat_ze(d2aprimez),[firstdim,Mblock,N_semiz]);
                            entireRHS_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under_ze(d2aprimez),[firstdim,Mblock,N_semiz]);
                            [Vtempii_hat,maxindex]=max(entireRHS_hat,[],1);
    ...
    d3maxindex_lin=reshape(d3maxindex,[N_a*N_semiz*N_z*N_e,1]); % This is the value of d3 that corresponds, make it this shape for addition just below
    d12aprime_ind=reshape(Policy_ford3_hat((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(d3maxindex_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
    ...
    Vunderbar(:,:,:,N_j)=reshape(V_ford3_under((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(d3maxindex_lin-1)),[N_a,N_semiz*N_z,N_e]);
end
...
for reverse_j=1:N_j-1
    ...
    % Vunderbar: gather the beta-RHS (already inner-gathered) at the same chosen d3
    Vunderbar(:,:,:,jj)=reshape(V_ford3_under((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(d3maxindex_lin-1)),[N_a,N_semiz*N_z,N_e]);
