function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetzeN_DC2A_nod1_e_raw(n_d2, n_a1, n_a2, n_a3, n_z, n_e, N_j, d2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Naive QH + ExpAssetze (z+e dep aprimeFn), DC2A pattern (nod1).
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
            [~,maxindex1]=max(ReturnMatrix_ii_e,[],2);
            [Vtempii,maxindex2]=max(reshape(ReturnMatrix_ii_e,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]),[],1);
            curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                     + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                     + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
            d2ind  =rem(maxindex2-1,N_d2)+1;
            ...
            Policyalt(1,curraindex,:,e_c,N_j)=d2ind;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 2);
                    ...
                    loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 2);
                    [Vtempii,maxindex]=max(ReturnMatrix_ii_e,[],1);
                    Valt(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    d2ind  =rem(maxindex-1,N_d2)+1;
                    a2pind =floor((maxindex-1)/N_d2)+1;
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_alt(ii)>0
                ...
                ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, n_e, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                ...
                ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, n_e, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
            entireRHS_alt_e=ReturnMatrix_ii_e+DiscountedEV_alt_e;
            [~,maxindex1_alt_e]=max(entireRHS_alt_e,[],2);
            [Vtempii_alt,maxindex2_alt]=max(reshape(entireRHS_alt_e,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]),[],1);
            entireRHS_e=ReturnMatrix_ii_e+DiscountedEV_e;
            [~,maxindex1_e]=max(entireRHS_e,[],2);
            [Vtempii,maxindex2]=max(reshape(entireRHS_e,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]),[],1);
            curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                     + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                     + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
            d2ind_alt=rem(maxindex2_alt-1,N_d2)+1;
            ...
            Policyalt(1,curraindex,:,e_c,N_j)  =d2ind_alt;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap_alt(ii));
                    ReturnMatrix_ii_alt_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                    d2aprime_e=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_alt_e=reshape(ReturnMatrix_ii_alt_e+DiscountedEV_alt_e(d2aprime_e),[N_d2*(maxgap_alt(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    ...
                    a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                    a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                    loweredge_idx=d2ind_alt + N_d2*(a2pind_alt-1) + N_d2*N_a2*a2ind_flat + N_d2*N_a2*N_a2*a3ind_flat + N_d2*N_a2*N_a2*N_a3*zind;
                    a1prime_rec=a1localind_alt+loweredge(loweredge_idx)-1;
                    Policyalt(1,curraindex,:,e_c,N_j)=d2ind_alt;
                    Policyalt(2,curraindex,:,e_c,N_j)=a1prime_rec;
                    Policyalt(3,curraindex,:,e_c,N_j)=a2pind_alt;
                    ...
                    ReturnMatrix_ii_alt_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                    d2aprime_e=(1:1:N_d2)' + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_alt_e=reshape(ReturnMatrix_ii_alt_e+DiscountedEV_alt_e(d2aprime_e),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii_alt,maxindex_alt]=max(entireRHS_alt_e,[],1);
                    Valt(curraindex,:,e_c,N_j)=shiftdim(Vtempii_alt,1);
                    d2ind_alt=rem(maxindex_alt-1,N_d2)+1;
                    a2pind_alt=floor((maxindex_alt-1)/N_d2)+1;
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                    d2aprime_e=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_e=reshape(ReturnMatrix_ii_e+DiscountedEV_e(d2aprime_e),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    ...
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                    d2aprime_e=(1:1:N_d2)' + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_e=reshape(ReturnMatrix_ii_e+DiscountedEV_e(d2aprime_e),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii,maxindex]=max(entireRHS_e,[],1);
                    Vtilde(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    d2ind=rem(maxindex-1,N_d2)+1;
                    a2pind=floor((maxindex-1)/N_d2)+1;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                [Vtempii_alt,maxindex2_alt]=max(reshape(entireRHS_alt_ze,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
                entireRHS_ze=ReturnMatrix_ii_ze+DiscountedEV_ze;
                [~,maxindex1_ze]=max(entireRHS_ze,[],2);
                [Vtempii,maxindex2]=max(reshape(entireRHS_ze,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
                curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                         + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                         + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
                d2ind_alt=rem(maxindex2_alt-1,N_d2)+1;
                ...
                Policyalt(1,curraindex,z_c,e_c,N_j)  =d2ind_alt;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_alt(ii)>0
                        ...
                        a1primeindexes=loweredge+(0:1:maxgap_alt(ii));
                        ReturnMatrix_ii_alt_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, special_n_z, special_n_e, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                        d2aprime_ze=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_alt_ze=reshape(ReturnMatrix_ii_alt_ze+DiscountedEV_alt_ze(d2aprime_ze),[N_d2*(maxgap_alt(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        ...
                        a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                        a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                        loweredge_idx=d2ind_alt + N_d2*(a2pind_alt-1) + N_d2*N_a2*a2ind_flat + N_d2*N_a2*N_a2*a3ind_flat;
                        a1prime_rec=a1localind_alt+loweredge(loweredge_idx)-1;
                        Policyalt(1,curraindex,z_c,e_c,N_j)=d2ind_alt;
                        Policyalt(2,curraindex,z_c,e_c,N_j)=a1prime_rec;
                        Policyalt(3,curraindex,z_c,e_c,N_j)=a2pind_alt;
                        ...
                        ReturnMatrix_ii_alt_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, special_n_z, special_n_e, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                        d2aprime_ze=(1:1:N_d2)' + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_alt_ze=reshape(ReturnMatrix_ii_alt_ze+DiscountedEV_alt_ze(d2aprime_ze),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        [Vtempii_alt,maxindex_alt]=max(entireRHS_alt_ze,[],1);
                        Valt(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii_alt,1);
                        d2ind_alt=rem(maxindex_alt-1,N_d2)+1;
                        a2pind_alt=floor((maxindex_alt-1)/N_d2)+1;
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprime_ze=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_ze=reshape(ReturnMatrix_ii_ze+DiscountedEV_ze(d2aprime_ze),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        ...
                        d2aprime_ze=(1:1:N_d2)' + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_ze=reshape(ReturnMatrix_ii_ze+DiscountedEV_ze(d2aprime_ze),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        [Vtempii,maxindex]=max(entireRHS_ze,[],1);
                        Vtilde(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        d2ind=rem(maxindex-1,N_d2)+1;
                        a2pind=floor((maxindex-1)/N_d2)+1;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_alt(ii)>0
                loweredge=min(maxindex1_alt(:,1,:,ii,:,:,:,:),N_a1-maxgap_alt(ii));
                a1primeindexes=loweredge+(0:1:maxgap_alt(ii));
                ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, n_e, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                d2aprimez=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5) + N_d2*N_a1*N_a2*N_a3*N_z*shiftdim((0:1:N_e-1),-6);
                entireRHS_alt=reshape(ReturnMatrix_ii_alt+DiscountedEV_alt(d2aprimez),[N_d2*(maxgap_alt(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                [Vtempii_alt,maxindex_alt]=max(entireRHS_alt,[],1);
                Valt(curraindex,:,:,jj)=shiftdim(Vtempii_alt,1);
                ...
                loweredge=maxindex1_alt(:,1,:,ii,:,:,:,:);
                ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, n_e, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                d2aprimez=(1:1:N_d2)' + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5) + N_d2*N_a1*N_a2*N_a3*N_z*shiftdim((0:1:N_e-1),-6);
                entireRHS_alt=reshape(ReturnMatrix_ii_alt+DiscountedEV_alt(d2aprimez),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                [Vtempii_alt,maxindex_alt]=max(entireRHS_alt,[],1);
                Valt(curraindex,:,:,jj)=shiftdim(Vtempii_alt,1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            DiscountedEV_alt_e=DiscountedEV_alt(:,:,:,:,:,:,:,e_c);
            DiscountedEV_e    =DiscountedEV    (:,:,:,:,:,:,:,e_c);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 1);
            entireRHS_alt_e=ReturnMatrix_ii_e+DiscountedEV_alt_e;
            [~,maxindex1_alt_e]=max(entireRHS_alt_e,[],2);
            [Vtempii_alt,maxindex2_alt]=max(reshape(entireRHS_alt_e,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]),[],1);
            entireRHS_e=ReturnMatrix_ii_e+DiscountedEV_e;
            ...
            Policyalt(1,curraindex,:,e_c,jj)  =d2ind_alt;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    loweredge=min(maxindex1_alt_e(:,1,:,ii,:,:,:,:),N_a1-maxgap_alt(ii));
                    a1primeindexes=loweredge+(0:1:maxgap_alt(ii));
                    ReturnMatrix_ii_alt_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                    d2aprime_e=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_alt_e=reshape(ReturnMatrix_ii_alt_e+DiscountedEV_alt_e(d2aprime_e),[N_d2*(maxgap_alt(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii_alt,maxindex_alt]=max(entireRHS_alt_e,[],1);
                    Valt(curraindex,:,e_c,jj)=shiftdim(Vtempii_alt,1);
                    ...
                    loweredge=maxindex1_alt_e(:,1,:,ii,:,:,:,:);
                    ReturnMatrix_ii_alt_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                    d2aprime_e=(1:1:N_d2)' + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_alt_e=reshape(ReturnMatrix_ii_alt_e+DiscountedEV_alt_e(d2aprime_e),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii_alt,maxindex_alt]=max(entireRHS_alt_e,[],1);
                    Valt(curraindex,:,e_c,jj)=shiftdim(Vtempii_alt,1);
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1_e(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                    d2aprime_e=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_e=reshape(ReturnMatrix_ii_e+DiscountedEV_e(d2aprime_e),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii,maxindex]=max(entireRHS_e,[],1);
                    Vtilde(curraindex,:,e_c,jj)=shiftdim(Vtempii,1);
                    ...
                    loweredge=maxindex1_e(:,1,:,ii,:,:,:,:);
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                    d2aprime_e=(1:1:N_d2)' + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_e=reshape(ReturnMatrix_ii_e+DiscountedEV_e(d2aprime_e),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,1]);
                    [Vtempii,maxindex]=max(entireRHS_e,[],1);
                    Vtilde(curraindex,:,e_c,jj)=shiftdim(Vtempii,1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policyalt(1,curraindex,z_c,e_c,jj)  =d2ind_alt;
