...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                d12_ind=rem(maxindex-1,N_d12)+1;
                Policy(1,:,semizblock,e_c,N_j)=rem(d12_ind-1,N_d1)+1; % d1
                Policy(2,:,semizblock,e_c,N_j)=ceil(d12_ind/N_d1);    % d2
                Policy(3,:,semizblock,e_c,N_j)=ceil(maxindex/N_d12);  % d3
            end
        end
    ...
    d12_ind=reshape(Policy_ford3_hat((1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(d3maxindex_lin-1)),[1,N_a,N_bothz,N_e]);
    Policy(1,:,:,:,N_j)=rem(d12_ind-1,N_d1)+1; % d1
    Policy(2,:,:,:,N_j)=ceil(d12_ind/N_d1);    % d2
