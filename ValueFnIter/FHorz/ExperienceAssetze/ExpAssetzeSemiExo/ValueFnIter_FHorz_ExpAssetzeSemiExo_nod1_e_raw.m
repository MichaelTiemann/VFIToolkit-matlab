    ...
    elseif vfoptions.lowmemory==2
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                aprimeProbs_slice=aprimeProbs_d2a1a2ze(:,:,z_part,:);

                EV1=reshape(EV_z(aprime_slice(:)),[N_d2*N_a1,N_a2,N_e]);
                EV2=reshape(EV_z(aprimeplus1_slice(:)),[N_d2*N_a1,N_a2,N_e]);

                skipinterp=(EV1==EV2);
                aprimeProbs_z=reshape(aprimeProbs_slice,[N_d2*N_a1,N_a2,N_e]);
                aprimeProbs_z(skipinterp)=0;

                entireEV_z=EV1.*aprimeProbs_z+EV2.*(1-aprimeProbs_z);
                DiscountedEV_z=DiscountFactorParamsVec*repelem(entireEV_z,1,N_a1,1);
