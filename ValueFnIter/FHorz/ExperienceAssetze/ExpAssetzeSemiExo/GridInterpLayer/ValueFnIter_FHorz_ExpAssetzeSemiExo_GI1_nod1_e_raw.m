function [V,Policy]=ValueFnIter_FHorz_ExpAssetzeSemiExo_GI1_nod1_e_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,n_e,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            Policy3_ford3_jj(2,:,:,:,d3_c)=shiftdim(squeeze(midpoint(d_ind+N_d2*aind+N_d2*N_a*bothzBind+N_d2*N_a*N_bothz*eind)),-1);
    ...
    Policy(3,:,:,:,N_j)=reshape(Policy3_ford3_jj(2+temp),[1,N_a,N_bothz,N_e]); % midpoint
    Policy(4,:,:,:,N_j)=reshape(Policy3_ford3_jj(3+temp),[1,N_a,N_bothz,N_e]); % L2ind
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_d3e=ReturnMatrix_d3e+DiscountedEV_zbe;
                    ...
                    entireRHS_ii_d3e=ReturnMatrix_ii_d3e+reshape(DiscountedEVinterp_zbe(d2a1primea2bothz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
                    [Vtempii,maxindexL2]=max(entireRHS_ii_d3e,[],1);
                    V_ford3_jj(:,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                    d_ind=rem(maxindexL2-1,N_d2)+1;
                    allind=d_ind+N_d2*aind+N_d2*N_a*semizBind;
