        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV(d2aprimeze),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    ...
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV(d2aprimeze),[N_d2,level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford3_jj(curraindex,:,:,d3_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d2)+1);
                    a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii_d3e=ReturnMatrix_ii_d3e+DiscountedEV_e;
    ...
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                semizblock=(z_c-1)*N_semiz+(1:1:N_semiz);
                z_valblock=bothz_gridvals_J(semizblock,:,N_j);
                DiscountedEV_zb=DiscountedEV(:,:,:,:,semizblock,:); % [N_d2,N_a1,1,N_a2,N_semiz,N_e]
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    DiscountedEV_zbe=DiscountedEV_zb(:,:,:,:,:,e_c); % [N_d2,N_a1,1,N_a2,N_semiz]
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+DiscountedEV_zbe;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_zbe(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_zbe(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_jj(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
