function [V,Policy2]=ValueFnIter_FHorz_ResidAsset_raw(n_d,n_a,n_r,n_z,N_j, d_grid, a_grid, r_grid, z_gridvals_J, pi_z_J, ReturnFn, rprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, rprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_ResidAsset_Disc(ReturnFn, n_d, n_a, n_r, n_z, d_gridvals, a_grid, r_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,0);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            ReturnMatrix_z=CreateReturnFnMatrix_ResidAsset_Disc(ReturnFn, n_d, n_a, n_r, special_n_z, d_gridvals, a_grid, r_grid, z_val, ReturnFnParamsVec,0);
            %Calc the max and it's index
...
for reverse_j=1:N_j-1
    ...
    [rprimeIndexes,rprimeProbs]=CreateResidualAssetFnMatrix_Case1(rprimeFn, n_d, n_a, n_r, n_z, d_gridvals, a_grid, r_grid, z_gridvals_J(:,:,jj), rprimeFnParamsVec);  % Note, is actually rprime_grid (but r_grid is anyway same for all ages)
    % Note: rprimeIndex is [N_d*N_a*N_a*N_z,1], and rprimeProbs is [N_d*N_a*N_a*N_z,1]
    aprimeIndexes=repelem(repmat((1:1:N_a)',N_a*N_z,1),N_d,1); % aprime over (d,aprime,a,z)
    zprimeIndexes=repelem((1:1:N_z)',N_d*N_a*N_a,1); % zprime over (d,aprime,a,z)
    ...
    EV=reshape(EV,[N_d*N_a,N_a,N_z]);
    % now, it is over (d,a',a,z)

    if vfoptions.lowmemory==0

        ReturnMatrix=CreateReturnFnMatrix_ResidAsset_Disc(ReturnFn, n_d, n_a, n_r, n_z, d_gridvals, a_grid, r_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec,0);
        % (d,aprime,a,r,z)
