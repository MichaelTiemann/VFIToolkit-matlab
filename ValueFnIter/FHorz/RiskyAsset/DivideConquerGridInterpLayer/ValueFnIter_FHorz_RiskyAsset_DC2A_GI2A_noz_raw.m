function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_DC2A_GI2A_noz_raw(n_d1,n_d2,n_d3,n_a1,n_a2,n_a3,n_u,N_j, d1_grid, d2_grid, d3_grid, a1_grid, a2_grid, a3_grid, u_grid, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAsset_DC1_GI1_noz_raw.
...
Policy=zeros(6,N_a,N_j,'gpuArray'); % (1)=d1, (2)=d2, (3)=d3, (4)=a1prime midpoint, (5)=a2prime, (6)=a1prime L2
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, n_d1, n_d3, n_a2, n_a3, d13_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, ReturnFnParamsVec, 2);
    ...
    midpoint=max(min(midpoint,N_a1-1),2);
    a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, n_d1, n_d3, n_a2, n_a3, d13_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, ReturnFnParamsVec, 3);
    aprime=d3ind + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*a2pcol; % linear index into DiscountedEVinterp [N_d3,N_a1fine,N_a2]
    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEVinterp(aprime),[N_d13*n2long*N_a2,N_a]);
