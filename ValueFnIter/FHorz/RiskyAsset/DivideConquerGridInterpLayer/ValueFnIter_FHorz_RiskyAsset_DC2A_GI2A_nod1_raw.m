function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_DC2A_GI2A_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_a3,n_z,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, a3_grid, z_gridvals_J, u_grid, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAsset_DC1_GI1_nod1_raw.
...
Policy=zeros(5,N_a,N_z,N_j,'gpuArray'); % (1)=d2, (2)=d3, (3)=a1prime midpoint, (4)=a2prime, (5)=a1prime L2
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, special_n_z, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 2);
            ...
            Policy(3,:,z_c,N_j)=midpoint(allind);
            Policy(4,:,z_c,N_j)=maxindexL2a2;
            Policy(5,:,z_c,N_j)=maxindexL2a1;
    ...
    if vfoptions.lowmemory==0
        ...
        midpoint=max(min(midpoint,N_a1-1),2);
        a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
        ...
        % L2flag
        ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii,[N_d3*n2long*N_a2,N_a,N_z]);
        linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind;
        linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind;
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, special_n_z, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
            aprime_z=d3col + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*a2pcol;
            entireRHS_ii_z=reshape(ReturnMatrix_ii_z+DiscountedEVinterp_z(aprime_z),[N_d3*n2long*N_a2,N_a]);
            ...
            a1mid=midpoint(allind);
            Policy(2,:,z_c,N_j)=d3_ind;
            Policy(3,:,z_c,N_j)=a1mid;
            Policy(4,:,z_c,N_j)=maxindexL2a2;
            Policy(5,:,z_c,N_j)=maxindexL2a1;

            % L2flag
            ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii_z,[N_d3*n2long*N_a2,N_a]);
            linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind;
            linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind;
            isInfLower=(ReturnMatrix_ii_flat(linidx_lower)==-Inf);
            isInfUpper=(ReturnMatrix_ii_flat(linidx_upper)==-Inf);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        midpoint=max(min(midpoint,N_a1-1),2);
        a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            Policy(3,:,z_c,jj)=a1mid;
            Policy(4,:,z_c,jj)=maxindexL2a2;
            Policy(5,:,z_c,jj)=maxindexL2a1;
