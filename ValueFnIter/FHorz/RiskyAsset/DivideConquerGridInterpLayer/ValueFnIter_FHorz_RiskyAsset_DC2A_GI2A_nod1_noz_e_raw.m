function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_DC2A_GI2A_nod1_noz_e_raw(n_d2,n_d3,n_a1,n_a2,n_a3,n_e,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, a3_grid, e_gridvals_J, u_grid, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAsset_DC1_GI1_nod1_noz_e_raw.
...
Policy=zeros(5,N_a,N_e,N_j,'gpuArray'); % (1)=d2, (2)=d3, (3)=a1prime midpoint, (4)=a2prime, (5)=a1prime L2
...
if vfoptions.lowmemory==0
    midpoint=zeros(N_d3,1,N_a2,N_a1,N_a2,N_a3,N_e,'gpuArray');
else
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2);
        ...
        allind=d3_ind + N_d3*(maxindexL2a2-1) + N_d3*N_a2*aind + N_d3*N_a2*N_a*eBind;
        Policy(2,:,:,N_j)=d3_ind;
        ...
        % L2flag
        linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*eBind;
        linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*eBind;
    ...
    elseif vfoptions.lowmemory>=1
        for e_c=1:N_e
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, special_n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 2);
    ...
    if vfoptions.lowmemory==0
        ...
        % L2flag
        ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii,[N_d3*n2long*N_a2,N_a,N_e]);
        linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*eBind;
        linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*eBind;
    ...
    elseif vfoptions.lowmemory>=1
        for e_c=1:N_e
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, special_n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 3);
            aprime=d3col + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*a2pcol;
            entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEVinterp(aprime),[N_d3*n2long*N_a2,N_a]);
            ...
            % L2flag
            ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii_e,[N_d3*n2long*N_a2,N_a]);
            linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind;
            linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind;
            isInfLower=(ReturnMatrix_ii_flat(linidx_lower)==-Inf);
            isInfUpper=(ReturnMatrix_ii_flat(linidx_upper)==-Inf);
