function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_DC2A_GI2A_noz_e_raw(n_d1,n_d2,n_d3,n_a1,n_a2,n_a3,n_e,n_u,N_j, d1_grid, d2_grid, d3_grid, a1_grid, a2_grid, a3_grid, e_gridvals_J, u_grid, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAsset_DC1_GI1_noz_e_raw.
...
%
% Note: the e variable goes in the shock slot of the z-bearing DC2A return matrix builder.
% DiscountedEV is (d3,a1prime,a2prime): no a3 term (a3prime does not depend on a3), and no
% e term (the expectation has already been taken over e).
...
Policy=zeros(6,N_a,N_e,N_j,'gpuArray'); % (1)=d1, (2)=d2, (3)=d3, (4)=a1prime midpoint, (5)=a2prime, (6)=a1prime L2
...
if vfoptions.lowmemory==0
    midpoint=zeros(N_d13,1,N_a2,N_a1,N_a2,N_a3,N_e,'gpuArray');
else
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, n_e, d13_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2);
        ...
        allind=d_ind + N_d13*(maxindexL2a2-1) + N_d13*N_a2*aind + N_d13*N_a2*N_a*eBind;
        Policy(1,:,:,N_j)=d1_ind;
        Policy(3,:,:,N_j)=d3_ind;
        ...
        % L2flag
        linidx_lower=d_ind                    + N_d13*n2long*(maxindexL2a2-1) + N_d13*n2long*N_a2*aind + N_d13*n2long*N_a2*N_a*eBind;
        linidx_upper=d_ind + N_d13*(n2long-1) + N_d13*n2long*(maxindexL2a2-1) + N_d13*n2long*N_a2*aind + N_d13*n2long*N_a2*N_a*eBind;
    ...
    elseif vfoptions.lowmemory>=1
        for e_c=1:N_e
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, special_n_e, d13_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 2);
    ...
    % DiscountedEV: (d3,a1prime,a2prime); note d1 is not in it, so it has to be indexed out onto d13
    ...
    if vfoptions.lowmemory==0
        % Layer 1
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, n_e, d13_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 1);
        d3aprime=d3ind + N_d3*a1pcol + N_d3*N_a1*a2pcol; % [N_d13,N_a1prime,N_a2prime]
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, n_e, d13_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d3aprime=d3ind + N_d3*(a1primeindexes-1) + N_d3*N_a1*a2pcol; % linear index into DiscountedEV [N_d3,N_a1,N_a2]
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV(d3aprime);
        ...
        aprime=d3ind + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*a2pcol; % linear index into DiscountedEVinterp [N_d3,N_a1fine,N_a2]
        entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEVinterp(aprime),[N_d13*n2long*N_a2,N_a,N_e]);
        ...
        % L2flag
        ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii,[N_d13*n2long*N_a2,N_a,N_e]);
        linidx_lower=d_ind                    + N_d13*n2long*(maxindexL2a2-1) + N_d13*n2long*N_a2*aind + N_d13*n2long*N_a2*N_a*eBind;
        linidx_upper=d_ind + N_d13*(n2long-1) + N_d13*n2long*(maxindexL2a2-1) + N_d13*n2long*N_a2*aind + N_d13*n2long*N_a2*N_a*eBind;
    ...
    elseif vfoptions.lowmemory>=1
        for e_c=1:N_e
            ...
            % Layer 1
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, special_n_e, d13_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 1);
            d3aprime=d3ind + N_d3*a1pcol + N_d3*N_a1*a2pcol;
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, special_n_e, d13_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 3);
            aprime=d3ind + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*a2pcol;
            entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEVinterp(aprime),[N_d13*n2long*N_a2,N_a]);
            ...
            % L2flag
            ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii_e,[N_d13*n2long*N_a2,N_a]);
            linidx_lower=d_ind                    + N_d13*n2long*(maxindexL2a2-1) + N_d13*n2long*N_a2*aind;
            linidx_upper=d_ind + N_d13*(n2long-1) + N_d13*n2long*(maxindexL2a2-1) + N_d13*n2long*N_a2*aind;
            isInfLower=(ReturnMatrix_ii_flat(linidx_lower)==-Inf);
            isInfUpper=(ReturnMatrix_ii_flat(linidx_upper)==-Inf);
