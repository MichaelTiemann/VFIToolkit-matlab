function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_DC2A_GI2A_nod1_e_raw(n_d2,n_d3,n_a1,n_a2,n_a3,n_z,n_e,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, a3_grid, z_gridvals_J, e_gridvals_J, u_grid, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAsset_DC1_GI1_nod1_e_raw.
...
Policy=zeros(5,N_a,N_z,N_e,N_j,'gpuArray'); % (1)=d2, (2)=d3, (3)=a1prime midpoint, (4)=a2prime, (5)=a1prime L2
...
if vfoptions.lowmemory==0
    midpoint=zeros(N_d3,1,N_a2,N_a1,N_a2,N_a3,N_z,N_e,'gpuArray');
elseif vfoptions.lowmemory==1
    special_n_e=ones(1,length(n_e));
    midpoint=zeros(N_d3,1,N_a2,N_a1,N_a2,N_a3,N_z,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2);
        ...
        allind=d3_ind + N_d3*(maxindexL2a2-1) + N_d3*N_a2*aind + N_d3*N_a2*N_a*zBind + N_d3*N_a2*N_a*N_z*eBind;
        Policy(2,:,:,:,N_j)=d3_ind;
        ...
        % L2flag
        linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind + N_d3*n2long*N_a2*N_a*N_z*eBind;
        linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind + N_d3*n2long*N_a2*N_a*N_z*eBind;
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, special_n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 2);
            ...
            allind=d3_ind + N_d3*(maxindexL2a2-1) + N_d3*N_a2*aind + N_d3*N_a2*N_a*zBind;
            Policy(2,:,:,e_c,N_j)=d3_ind;
            Policy(3,:,:,e_c,N_j)=midpoint(allind);
            Policy(4,:,:,e_c,N_j)=maxindexL2a2;
            Policy(5,:,:,e_c,N_j)=maxindexL2a1;

            % L2flag
            linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind;
            linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                midpoint=max(min(midpoint,N_a1-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d3, n_a2, n_a3, special_n_z, special_n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 2);
                ...
                Policy(3,:,z_c,e_c,N_j)=midpoint(allind);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, n_e, d3_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d3aprimez=d3col + N_d3*(a1primeindexes-1) + N_d3*N_a1*a2pcol + N_d3*N_a1*N_a2*shiftdim(zBind,-4); % linear index into DiscountedEV
        ...
        aprimez=d3col + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*a2pcol + N_d3*N_a1fine*N_a2*shiftdim(zBind,-4); % linear index into DiscountedEVinterp
        ...
        % L2flag
        ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii,[N_d3*n2long*N_a2,N_a,N_z,N_e]);
        linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind + N_d3*n2long*N_a2*N_a*N_z*eBind;
        linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind + N_d3*n2long*N_a2*N_a*N_z*eBind;
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, special_n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
            aprimez=d3col + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*a2pcol + N_d3*N_a1fine*N_a2*shiftdim(zBind,-4);
            entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEVinterp(aprimez),[N_d3*n2long*N_a2,N_a,N_z]);
            ...
            a1mid=midpoint(allind);
            Policy(2,:,:,e_c,N_j)=d3_ind;
            Policy(3,:,:,e_c,N_j)=a1mid;
            Policy(4,:,:,e_c,N_j)=maxindexL2a2;
            Policy(5,:,:,e_c,N_j)=maxindexL2a1;

            % L2flag
            ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii_e,[N_d3*n2long*N_a2,N_a,N_z]);
            linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind;
            linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind + N_d3*n2long*N_a2*N_a*zBind;
            isInfLower=(ReturnMatrix_ii_flat(linidx_lower)==-Inf);
            isInfUpper=(ReturnMatrix_ii_flat(linidx_upper)==-Inf);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d3, n_a2, n_a3, special_n_z, special_n_e, d3_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                        d3aprime_z=d3col + N_d3*(a1primeindexes-1) + N_d3*N_a1*a2pcol;
                ...
                midpoint=max(min(midpoint,N_a1-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d3, n_a2, n_a3, special_n_z, special_n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                aprime_z=d3col + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*a2pcol;
                entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze+DiscountedEVinterp_z(aprime_z),[N_d3*n2long*N_a2,N_a]);
                ...
                a1mid=midpoint(allind);
                Policy(2,:,z_c,e_c,N_j)=d3_ind;
                Policy(3,:,z_c,e_c,N_j)=a1mid;
                Policy(4,:,z_c,e_c,N_j)=maxindexL2a2;
                Policy(5,:,z_c,e_c,N_j)=maxindexL2a1;

                % L2flag
                ReturnMatrix_ii_flat=reshape(ReturnMatrix_ii_ze,[N_d3*n2long*N_a2,N_a]);
                linidx_lower=d3_ind                   + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind;
                linidx_upper=d3_ind + N_d3*(n2long-1) + N_d3*n2long*(maxindexL2a2-1) + N_d3*n2long*N_a2*aind;
                isInfLower=(ReturnMatrix_ii_flat(linidx_lower)==-Inf);
                isInfUpper=(ReturnMatrix_ii_flat(linidx_upper)==-Inf);
                ...
                Policy(1,:,z_c,e_c,N_j)=d2index_z(lin);
            end
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, special_n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
            aprimez=d3col + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*a2pcol + N_d3*N_a1fine*N_a2*shiftdim(zBind,-4);
            entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEVinterp(aprimez),[N_d3*n2long*N_a2,N_a,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii_e,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                allind=d3_ind + N_d3*(maxindexL2a2-1) + N_d3*N_a2*aind;
                a1mid=midpoint(allind);
                Policy(2,:,z_c,e_c,jj)=d3_ind;
                Policy(3,:,z_c,e_c,jj)=a1mid;
                Policy(4,:,z_c,e_c,jj)=maxindexL2a2;
                Policy(5,:,z_c,e_c,jj)=maxindexL2a1;
                ...
                % Get the d2Policy
                lin=d3_ind+N_d3*(a1mid-1)+N_d3*N_a1*(maxindexL2a2-1);
                Policy(1,:,z_c,e_c,jj)=d2index_z(lin);
            end
        end
    end
end
