function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_GI2A_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_a3,n_z,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, a3_grid, z_gridvals_J, u_grid, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAsset_GI1_nod1_raw.
...
Policy=zeros(5,N_a,N_z,N_j,'gpuArray'); % (1)=d2, (2)=d3, (3)=a1prime midpoint, (4)=a2prime, (5)=L2ind
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, d3_gridvals, a1_grid, a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 1);
        % [N_d3, N_a1prime, N_a2prime, N_a1, N_a2, N_a3, N_z]
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, d3_gridvals, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2);
        % [N_d3*n2long*N_a2, N_a, N_z]
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            % Layer 1: full ReturnMatrix max for initial midpoint
            ReturnMatrix_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, special_n_z, d3_gridvals, a1_grid, a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 1);
            [~,maxindex]=max(ReturnMatrix_z,[],2);
            midpoint_jj=max(min(maxindex,N_a1-1),2);

            % Grid interpolation layer
            a1primeindexes=(midpoint_jj+(midpoint_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, special_n_z, d3_gridvals, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 2);
    ...
    if vfoptions.lowmemory==0
        ...
        % EV does not depend on a3, so the linear index into DiscountedEVinterp omits an a3 offset
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            aprime_z=(1:1:N_d3)' + N_d3*(a1primeindexesfine-1) + N_d3*N_a1fine*shiftdim((0:1:N_a2-1),-1);
            entireRHS_ii=reshape(ReturnMatrix_ii_z+DiscountedEVinterp_z(aprime_z),[N_d3*n2long*N_a2,N_a]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            V(:,z_c,N_j)=shiftdim(Vtempii,1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        % Layer 1: full ReturnMatrix max for initial midpoint
        ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d3, n_a2, n_a3, n_z, d3_gridvals, a1_grid, a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec, 1);
        entireRHS=ReturnMatrix+DiscountedEV;
        [~,maxindex]=max(entireRHS,[],2);
        midpoint_jj=max(min(maxindex,N_a1-1),2);
