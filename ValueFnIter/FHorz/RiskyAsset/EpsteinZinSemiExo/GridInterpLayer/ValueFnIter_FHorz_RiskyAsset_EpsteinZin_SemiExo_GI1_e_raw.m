function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_SemiExo_GI1_e_raw(n_d1,n_d2,n_d3,n_d4,n_a1,n_a2,n_semiz,n_z,n_e,n_u,N_j, d1_grid, d2_grid, d3_grid, d4_grid, a1_grid, a2_grid, semiz_gridvals_J, z_gridvals_J, e_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
% d1: ReturnFn but not aprimeFn
% d2: aprimeFn but not ReturnFn
% d3: both ReturnFn and aprimeFn
% d4: ReturnFn but not aprimeFn, and determines semiz transitions
% e: iid start-of-period shock (integrated out of the transformed V' before the bothz-expectation)
%
% Grid-interpolation-layer version of the Epstein-Zin riskyasset+semiz solver.
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_RiskyAssetSemiExo_GI1_e_raw.
% V' is transformed ((ezc4*V')^ezc5, masked) BEFORE all the expectations: the
% e- and bothz-expectations and the u-lottery act on the transformed object,
% giving one joint certainty-equivalent over (u,semiz',z',e'). See
% ValueFnIter_FHorz_RiskyAsset_EpsteinZin_SemiExo_GI1_raw for the rest of the
% transform-chain ordering under GI.
...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        % Refine d2 out of the warm-glow (the only continuation term in the terminal period) [ezc9 handles the sign for the max; the warm-glow enters INSIDE the ^ezc7 root]
        [WGmatrix_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3]),[],1);
        DiscountedWGcol=DiscountFactorParamsVec*ezc9*reshape(WGmatrix_onlyd3,[N_d3,1]); % constant in a1prime (and a,bothz,e)
        DiscountedWGcol_d13=repelem(DiscountedWGcol,N_d1,1); % [N_d13,1]
    else
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_bothz,n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
            if warmglow==1
                ...
                entireRHS=ezc1*temp2+DiscountedWGcol_d13; % warm-glow composed INSIDE the ^ezc7 root; constant in a1prime
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,n_bothz,n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
            if warmglow==1
                ...
                entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_bothz,N_e])+DiscountedWGcol_d13,[N_d13*n2long,N_a1*N_a2,N_bothz,N_e]);
                ...
                entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_bothz,N_e])+WGcol_d13,[N_d13*n2long,N_a1*N_a2,N_bothz,N_e]);
            end
            ...
            if warmglow==1
                ...
                d2index_ford4_jj(:,:,:,d4_c)=shiftdim(d2index(d3opt),1); % d2 comes from refining the warm-glow (no a nor bothz nor e in WGmatrix)
            else
    ...
    elseif vfoptions.lowmemory==1
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_d4e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_bothz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1,0);
                if warmglow==1
                    % Modify the Return Function appropriately for Epstein-Zin Preferences (pre-root; the warm-glow enters INSIDE the ^ezc7 root)
                    ...
                    entireRHS_e=ezc1*temp2+DiscountedWGcol_d13; % warm-glow composed INSIDE the ^ezc7 root; constant in a1prime
                    ...
                    entireRHS_e=ReturnMatrix_d4e+WGcol_d13; % warm-glow (zero if not using); constant in a1prime
                end
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,n_bothz,special_n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2,0);
                if warmglow==1
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_bothz])+DiscountedWGcol_d13,[N_d13*n2long,N_a1*N_a2,N_bothz]);
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                    entireRHS_ii(entireRHS_ii==0)=-Inf;
                else
                    ...
                    ReturnMatrix_ii(becareful)=(ezc1*ReturnMatrix_ii(becareful).^ezc2(N_j)).^ezc7(N_j);
                    ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                    entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_bothz])+WGcol_d13,[N_d13*n2long,N_a1*N_a2,N_bothz]);
                end
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                ...
                if warmglow==1
                    ...
                    d2index_ford4_jj(:,:,e_c,d4_c)=shiftdim(d2index(d3opt),1); % d2 comes from refining the warm-glow (no a nor bothz nor e in WGmatrix)
                else
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    if warmglow==1
                        ...
                        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                        temp2_ii(ReturnMatrix_ii==0)=-Inf;
                        entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_semiz])+DiscountedWGcol_d13,[N_d13*n2long,N_a1*N_a2,N_semiz]);
                        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                        entireRHS_ii(entireRHS_ii==0)=-Inf;
                    else
                    ...
                    if warmglow==1
                        ...
                        d2index_ford4_jj(:,semizblock,e_c,d4_c)=shiftdim(d2index(d3opt),1); % d2 comes from refining the warm-glow
                    else
                        d2index_ford4_jj(:,semizblock,e_c,d4_c)=1; % d2 meaningless at j=N_j without warm-glow
                    end
                end
            end
        end
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    if warmglow==1
                        ...
                        entireRHS_ze=ezc1*temp2+DiscountedWGcol_d13; % warm-glow composed INSIDE the ^ezc7 root; constant in a1prime
                        ...
                        entireRHS_ze=ReturnMatrix_ze+WGcol_d13; % warm-glow (zero if not using); constant in a1prime
                    end
                    ...
                    if warmglow==1
                        ...
                        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                        temp2_ii(ReturnMatrix_ii==0)=-Inf;
                        entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2])+DiscountedWGcol_d13,[N_d13*n2long,N_a1*N_a2]);
                        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                        entireRHS_ii(entireRHS_ii==0)=-Inf;
                    else
                        ...
                        ReturnMatrix_ii(becareful)=(ezc1*ReturnMatrix_ii(becareful).^ezc2(N_j)).^ezc7(N_j);
                        ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                        entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2])+WGcol_d13,[N_d13*n2long,N_a1*N_a2]);
                    end
                    [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    Policy(1,:,:,:,N_j)=reshape(d1part,[1,N_a,N_bothz,N_e]);
    Policy(2,:,:,:,N_j)=reshape(d2winner,[1,N_a,N_bothz,N_e]);
    Policy(3,:,:,:,N_j)=reshape(d3part,[1,N_a,N_bothz,N_e]);
    Policy(4,:,:,:,N_j)=reshape(d4winner,[1,N_a,N_bothz,N_e]);
    Policy(5,:,:,:,N_j)=reshape(a1prime_low,[1,N_a,N_bothz,N_e]);
    ...
    EVnext=sum(temp.*shiftdim(pi_e_J(:,N_j+1),-2),3); % [N_a,N_bothz]
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            % Expectation over bothz' (on the transformed object)
            ...
            EVinterp=permute(interp1(a1_gridvals,permute(EV,[2,1,3]),a1prime_grid),[2,1,3]); % [N_d23,N_a1prime,N_bothz]

            % Certainty-equivalent (and mortality-risk/warm-glow) transform, pointwise
            temp4=EV;
            temp4interp=EVinterp;
            if warmglow==1
                WGmatrixbig=WGmatrix.*ones(1,N_a1,N_bothz);
                ...
                WGmatrixbigfine=WGmatrix.*ones(1,N_a1prime,N_bothz);
            ...
            temp4_onlyd3=max(ezc9*ezc3*reshape((~isinf(temp4)).*temp4,[N_d2,N_d3*N_a1,N_bothz]),[],1);
            [temp4interp_onlyd3,d2indexfine]=max(ezc9*ezc3*reshape((~isinf(temp4interp)).*temp4interp,[N_d2,N_d3*N_a1prime,N_bothz]),[],1);
            d2indexfine_resh=reshape(d2indexfine,[N_d3,N_a1prime,N_bothz]);
            ...
            DiscountedEVinterp=DiscountFactorParamsVec*ezc9*reshape(temp4interp_onlyd3,[N_d3,N_a1prime,1,1,N_bothz]);
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_bothz,n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, bothz_gridvals, e_gridvals, ReturnFnParamsVec,1,0);
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            becareful=logical(isfinite(ReturnMatrix_d4).*(ReturnMatrix_d4~=0)); % finite but not zero
            temp2=ReturnMatrix_d4;
            temp2(becareful)=ReturnMatrix_d4(becareful).^ezc2(N_j);
            temp2(ReturnMatrix_d4==0)=-Inf;
            entireRHS=ezc1*temp2+DiscountedEV_d13; % broadcast a2,e
            temp5=logical(isfinite(entireRHS).*(entireRHS~=0));
            entireRHS(temp5)=entireRHS(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
            entireRHS(entireRHS==0)=-Inf;
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,n_bothz,n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals, e_gridvals, ReturnFnParamsVec,2,0);
            becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
            temp2_ii=ReturnMatrix_ii;
            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            da1primez=(1:1:N_d13)'+N_d13*(a1primeindexesfine-1)+N_d13*N_a1prime*zind;
            entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_d13(da1primez(:)),[N_d13*n2long,N_a1*N_a2,N_bothz,N_e]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            Policy_ford4_jj(:,:,:,d4_c)=shiftdim(d_ind,1)+N_d13*(shiftdim(mid_at,1)-1)+N_d13*N_a1*(shiftdim(L2offset,1)-1);
            % d2 lookup — read off the FINE refine at the chosen fine a1prime point (d2indexfine_resh depends on (d3,a1prime,bothz) — not e)
            d3opt=rem(ceil(d_ind/N_d1)-1,N_d3)+1; % [1,N_a,N_bothz,N_e]
            a1mid=midpoint(allind); % [1,N_a,N_bothz,N_e]
            a1fine=(a1mid+(a1mid-1)*n2short)+L2offset-n2short-2; % chosen fine a1prime index
            zlin=shiftdim(gpuArray(0:N_bothz-1),-1); % [1,1,N_bothz]
            linlookup=d3opt+N_d3*(a1fine-1)+N_d3*N_a1prime*zlin; % broadcasts to [1,N_a,N_bothz,N_e]
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_bothz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, bothz_gridvals, e_val, ReturnFnParamsVec,1,0);
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                ...
                entireRHS_e=ezc1*temp2+DiscountedEV_d13;
                temp5=logical(isfinite(entireRHS_e).*(entireRHS_e~=0));
                entireRHS_e(temp5)=entireRHS_e(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                entireRHS_e(entireRHS_e==0)=-Inf;
                ...
                midpoint=max(min(maxindex,n_a1(1)-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,n_bothz,special_n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals, e_val, ReturnFnParamsVec,2,0);
                becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                temp2_ii=ReturnMatrix_ii;
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                da1primez=(1:1:N_d13)'+N_d13*(a1primeindexesfine-1)+N_d13*N_a1prime*zind;
                entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_d13(da1primez(:)),[N_d13*n2long,N_a1*N_a2,N_bothz]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z
                ...
                d2indexfine_reshblock=d2indexfine_resh(:,:,semizblock);
                for e_c=1:N_e
                    e_val=e_gridvals(e_c,:);
                    ...
                    temp2(becareful)=ReturnMatrix_e(becareful).^ezc2(N_j);
                    temp2(ReturnMatrix_e==0)=-Inf;
                    entireRHS_e=ezc1*temp2+DiscountedEVblock_d13;
                    temp5=logical(isfinite(entireRHS_e).*(entireRHS_e~=0));
                    entireRHS_e(temp5)=entireRHS_e(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                    entireRHS_e(entireRHS_e==0)=-Inf;
                    [~,maxindex]=max(entireRHS_e,[],2);
                    ...
                    entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterpblock_d13(da1primez(:)),[N_d13*n2long,N_a1*N_a2,N_semiz]);
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                    entireRHS_ii(entireRHS_ii==0)=-Inf;
                    [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                    ...
                    linlookup=d3opt+N_d3*(a1fine-1)+N_d3*N_a1prime*zlin;
                    d2index_ford4_jj(:,semizblock,e_c,d4_c)=shiftdim(d2indexfine_reshblock(linlookup),1);
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==3
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                ...
                d2indexfine_resh_z=d2indexfine_resh(:,:,z_c);
                for e_c=1:N_e
                    e_val=e_gridvals(e_c,:);
                    ReturnMatrix_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,special_n_bothz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
                    % Modify the Return Function appropriately for Epstein-Zin Preferences
                    ...
                    temp2(becareful)=ReturnMatrix_ze(becareful).^ezc2(N_j);
                    temp2(ReturnMatrix_ze==0)=-Inf;
                    entireRHS_ze=ezc1*temp2+DiscountedEV_z_d13;
                    temp5=logical(isfinite(entireRHS_ze).*(entireRHS_ze~=0));
                    entireRHS_ze(temp5)=entireRHS_ze(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                    entireRHS_ze(entireRHS_ze==0)=-Inf;
                    ...
                    entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_z_d13(da1prime(:)),[N_d13*n2long,N_a1*N_a2]);
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                    entireRHS_ii(entireRHS_ii==0)=-Inf;
                    [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                    ...
                    d2index_ford4_jj(:,z_c,e_c,d4_c)=shiftdim(d2indexfine_resh_z(linlookup),1);
                end
            end
        end
    end
...
for reverse_j=1:N_j-1
    ...
    % Expectation over eprime (on the transformed object)
    EVnext=sum(temp.*shiftdim(pi_e_J(:,jj+1),-2),3); % [N_a,N_bothz]
    ...
    d1part=rem(d13part-1,N_d1)+1;
    d3part=rem(ceil(d13part/N_d1)-1,N_d3)+1;
    Policy(1,:,:,:,jj)=reshape(d1part,[1,N_a,N_bothz,N_e]);
    Policy(2,:,:,:,jj)=reshape(d2winner,[1,N_a,N_bothz,N_e]);
    Policy(3,:,:,:,jj)=reshape(d3part,[1,N_a,N_bothz,N_e]);
    Policy(4,:,:,:,jj)=reshape(d4winner,[1,N_a,N_bothz,N_e]);
    Policy(5,:,:,:,jj)=reshape(a1prime_low,[1,N_a,N_bothz,N_e]);
