function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_SemiExo_GI1_nod1_raw(n_d2,n_d3,n_d4,n_a1,n_a2,n_semiz,n_z,n_u,N_j, d2_grid, d3_grid, d4_grid, a1_grid, a2_grid, semiz_gridvals_J, z_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_RiskyAssetSemiExo_GI1_nod1_raw.
...
% Per-d4 policy encoding (no d4 baked in): d3 + N_d3*(a1prime_mid-1) + N_d3*N_a1*(L2ind-1)
...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        ...
        DiscountedWGcol=DiscountFactorParamsVec*ezc9*reshape(WGmatrix_onlyd3,[N_d3,1]); % constant in a1prime (and a,bothz)
    else
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_bothz, d3_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
            if warmglow==1
                ...
                entireRHS=ezc1*temp2+DiscountedWGcol; % warm-glow composed INSIDE the ^ezc7 root; constant in a1prime
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d3,special_n_d4],n2long,n_a1,n_a2,n_bothz, d3_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
            if warmglow==1
                ...
                entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_bothz])+DiscountedWGcol,[N_d3*n2long,N_a1*N_a2,N_bothz]);
                ...
                entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d3,n2long,N_a1,N_a2,N_bothz])+WGcol,[N_d3*n2long,N_a1*N_a2,N_bothz]);
            end
            ...
            if warmglow==1
                ...
                d2index_ford4_jj(:,:,d4_c)=shiftdim(d2index(d3opt),1); % d2 comes from refining the warm-glow (no a nor bothz in WGmatrix)
            else
        ...
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                ...
                ReturnMatrix_d4z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d3,special_n_d4],n_a1,n_a1,n_a2,special_n_bothz, d3_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, z_val, ReturnFnParamsVec,1,0);
                if warmglow==1
                    % Modify the Return Function appropriately for Epstein-Zin Preferences (pre-root; the warm-glow enters INSIDE the ^ezc7 root)
                    ...
                    entireRHS_z=ezc1*temp2+DiscountedWGcol; % warm-glow composed INSIDE the ^ezc7 root; constant in a1prime
                    ...
                    entireRHS_z=ReturnMatrix_d4z+WGcol; % warm-glow (zero if not using); constant in a1prime
                end
                ...
                ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d3,special_n_d4],n2long,n_a1,n_a2,special_n_bothz, d3_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, ReturnFnParamsVec,2,0);
                if warmglow==1
                    ...
                    entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2])+DiscountedWGcol,[N_d3*n2long,N_a1*N_a2]);
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                    entireRHS_ii(entireRHS_ii==0)=-Inf;
                else
                    ...
                    entireRHS_ii=reshape(reshape(ReturnMatrix_ii_z,[N_d3,n2long,N_a1,N_a2])+WGcol,[N_d3*n2long,N_a1*N_a2]);
                end
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                ...
                if warmglow==1
                    d3opt=d_ind; % no d1 to strip
                    d2index_ford4_jj(:,z_c,d4_c)=shiftdim(d2index(d3opt),1); % d2 comes from refining the warm-glow (no a nor bothz in WGmatrix)
                else
    ...
    Policy(1,:,:,N_j)=reshape(d2winner,[1,N_a,N_bothz]);
    Policy(2,:,:,N_j)=reshape(d3part,[1,N_a,N_bothz]);
    Policy(3,:,:,N_j)=reshape(d4winner,[1,N_a,N_bothz]);
    Policy(4,:,:,N_j)=reshape(a1prime_low,[1,N_a,N_bothz]);
    Policy(5,:,:,N_j)=reshape(L2ind,[1,N_a,N_bothz]);
    Policy(6,:,:,N_j)=reshape(flagwinner,[1,N_a,N_bothz]);
else
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_bothz, d3_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, bothz_gridvals, ReturnFnParamsVec,1,0);
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            becareful=logical(isfinite(ReturnMatrix_d4).*(ReturnMatrix_d4~=0)); % finite but not zero
            temp2=ReturnMatrix_d4;
            temp2(becareful)=ReturnMatrix_d4(becareful).^ezc2(N_j);
            temp2(ReturnMatrix_d4==0)=-Inf;
            entireRHS=ezc1*temp2+DiscountedEV;
            temp5=logical(isfinite(entireRHS).*(entireRHS~=0));
            entireRHS(temp5)=entireRHS(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
            entireRHS(entireRHS==0)=-Inf;
            ...
            entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp(da1primez(:)),[N_d3*n2long,N_a1*N_a2,N_bothz]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            Policy_ford4_jj(:,:,d4_c)=shiftdim(d_ind,1)+N_d3*(shiftdim(mid_at,1)-1)+N_d3*N_a1*(shiftdim(L2offset,1)-1);
            % d2 lookup per (a,bothz) — read off the FINE refine at the chosen fine a1prime point
            d3opt=d_ind; % no d1 to strip
    ...
    elseif vfoptions.lowmemory==1
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z
                ...
                % Level-1 Return at coarse a1prime grid
                ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d3,special_n_d4],n_a1,n_a1,n_a2,[n_semiz,ones(1,length(n_z))], d3_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, z_valblock, ReturnFnParamsVec,1,0);
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_d4).*(ReturnMatrix_d4~=0)); % finite but not zero
                temp2=ReturnMatrix_d4;
                temp2(becareful)=ReturnMatrix_d4(becareful).^ezc2(N_j);
                temp2(ReturnMatrix_d4==0)=-Inf;
                entireRHS=ezc1*temp2+DiscountedEVblock;
                temp5=logical(isfinite(entireRHS).*(entireRHS~=0));
                entireRHS(temp5)=entireRHS(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                entireRHS(entireRHS==0)=-Inf;
                ...
                entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterpblock(da1primez(:)),[N_d3*n2long,N_a1*N_a2,N_semiz]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                ...
                Policy_ford4_jj(:,semizblock,d4_c)=shiftdim(d_ind,1)+N_d3*(shiftdim(mid_at,1)-1)+N_d3*N_a1*(shiftdim(L2offset,1)-1);
                ...
                linlookup=d3opt+N_d3*(a1fine-1)+N_d3*N_a1prime*zlin;
                d2index_ford4_jj(:,semizblock,d4_c)=shiftdim(d2indexfine_reshblock(linlookup),1);
            end
        end
    ...
    elseif vfoptions.lowmemory>=2 % lm2 already does the most-looped variant, so it also serves the higher lowmemory values
        ...
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                ...
                temp2(becareful)=ReturnMatrix_d4z(becareful).^ezc2(N_j);
                temp2(ReturnMatrix_d4z==0)=-Inf;
                entireRHS_z=ezc1*temp2+DiscountedEV_z;
                temp5=logical(isfinite(entireRHS_z).*(entireRHS_z~=0));
                entireRHS_z(temp5)=entireRHS_z(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                entireRHS_z(entireRHS_z==0)=-Inf;
                ...
                entireRHS_ii_z=ezc1*temp2_ii+reshape(DiscountedEVinterp_z(da1prime(:)),[N_d3*n2long,N_a1*N_a2]);
...
for reverse_j=1:N_j-1
    ...
    a1prime_low=midpart-adjust;
    L2ind=adjust.*L2offset+(1-adjust).*(L2offset-n2short-1);
    Policy(1,:,:,jj)=reshape(d2winner,[1,N_a,N_bothz]);
    Policy(2,:,:,jj)=reshape(d3part,[1,N_a,N_bothz]);
    Policy(3,:,:,jj)=reshape(d4winner,[1,N_a,N_bothz]);
    Policy(4,:,:,jj)=reshape(a1prime_low,[1,N_a,N_bothz]);
    Policy(5,:,:,jj)=reshape(L2ind,[1,N_a,N_bothz]);
    Policy(6,:,:,jj)=reshape(flagwinner,[1,N_a,N_bothz]);
end
