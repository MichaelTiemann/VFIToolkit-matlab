...
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_RiskyAssetSemiExo_GI1_noz_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        ...
        DiscountedWGcol=DiscountFactorParamsVec*ezc9*reshape(WGmatrix_onlyd3,[N_d3,1]); % constant in a1prime (and a,semiz,e)
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_semiz,n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
            if warmglow==1
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,n_semiz,n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
            if warmglow==1
                ...
                entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_semiz,N_e])+DiscountedWGcol_d13,[N_d13*n2long,N_a1*N_a2,N_semiz,N_e]);
                ...
                entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_semiz,N_e])+WGcol_d13,[N_d13*n2long,N_a1*N_a2,N_semiz,N_e]);
            end
            ...
            % d2 at j=N_j
            if warmglow==1
                d3opt=rem(ceil(d_ind/N_d1)-1,N_d3)+1; % [1,N_a,N_semiz,N_e]
    ...
    elseif vfoptions.lowmemory==1
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_d4e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_semiz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1,0);
                if warmglow==1
                    % Modify the Return Function appropriately for Epstein-Zin Preferences (pre-root; the warm-glow enters INSIDE the ^ezc7 root)
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,n_semiz,special_n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2,0);
                if warmglow==1
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
    ...
    elseif vfoptions.lowmemory>=2 % lm2 already does the most-looped variant, so it also serves the higher lowmemory values
        for d4_c=1:N_d4
            ...
            for z_c=1:N_semiz
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,special_n_semiz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
                    if warmglow==1
                        % Modify the Return Function appropriately for Epstein-Zin Preferences (pre-root; the warm-glow enters INSIDE the ^ezc7 root)
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,special_n_semiz,special_n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,2,0);
                    if warmglow==1
                        becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                        temp2_ii=ReturnMatrix_ii;
                        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_semiz,n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, semiz_gridvals, e_gridvals, ReturnFnParamsVec,1,0);
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            becareful=logical(isfinite(ReturnMatrix_d4).*(ReturnMatrix_d4~=0)); % finite but not zero
            temp2=ReturnMatrix_d4;
            temp2(becareful)=ReturnMatrix_d4(becareful).^ezc2(N_j);
            ...
            entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_d13(da1primez(:)),[N_d13*n2long,N_a1*N_a2,N_semiz,N_e]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory>=1
        ...
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_semiz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, semiz_gridvals, e_val, ReturnFnParamsVec,1,0);
                % Modify the Return Function appropriately for Epstein-Zin Preferences
...
for reverse_j=1:N_j-1
    ...
    Policy(6,:,:,:,jj)=reshape(L2ind,[1,N_a,N_semiz,N_e]);
    Policy(7,:,:,:,jj)=reshape(flagwinner,[1,N_a,N_semiz,N_e]);
end
