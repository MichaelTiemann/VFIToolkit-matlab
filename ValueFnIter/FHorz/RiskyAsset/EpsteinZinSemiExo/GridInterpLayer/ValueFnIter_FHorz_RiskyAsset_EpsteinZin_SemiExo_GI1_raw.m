...
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_RiskyAssetSemiExo_GI1_raw.
% V' is transformed ((ezc4*V')^ezc5, masked) BEFORE all the expectations: the
% bothz-expectation (per d4, via kron(pi_z,pi_semiz(:,:,d4))) and the u-lottery
% act on the transformed object, giving one joint certainty-equivalent over
...
% in both layers. The warm-glow fn depends only on a2prime, so the u-lottery
% turns it into a function of d23 (constant in a1prime, hence identical on
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_bothz, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0); % [N_d13,N_a1prime,N_a1,N_a2,N_bothz]
            if warmglow==1
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,n_bothz, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
            if warmglow==1
            ...
            L2offset=ceil(maxindexL2/N_d13); % [1,N_a,N_bothz]
        ...
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                ...
                ReturnMatrix_d4z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,special_n_bothz, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, z_val, ReturnFnParamsVec,1,0);
                if warmglow==1
                    % Modify the Return Function appropriately for Epstein-Zin Preferences (pre-root; the warm-glow enters INSIDE the ^ezc7 root)
                    ...
                    entireRHS_z=ezc1*temp2+DiscountedWGcol_d13; % warm-glow composed INSIDE the ^ezc7 root; constant in a1prime
                ...
                ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,special_n_bothz, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, ReturnFnParamsVec,2,0);
                if warmglow==1
                ...
                L2offset=ceil(maxindexL2/N_d13); % [1,N_a]
    ...
    % Cross-d4 max and combine (inlined)
    ...
    Policy(6,:,:,N_j)=reshape(L2ind,[1,N_a,N_bothz]);
    Policy(7,:,:,N_j)=reshape(flagwinner,[1,N_a,N_bothz]);
else
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,n_bothz, d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, bothz_gridvals, ReturnFnParamsVec,1,0); % [N_d13,N_a1prime(=N_a1),N_a1,N_a2,N_bothz]
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            becareful=logical(isfinite(ReturnMatrix_d4).*(ReturnMatrix_d4~=0)); % finite but not zero
            temp2=ReturnMatrix_d4;
            temp2(becareful)=ReturnMatrix_d4(becareful).^ezc2(N_j);
            temp2(ReturnMatrix_d4==0)=-Inf;
            entireRHS=ezc1*temp2+DiscountedEV_d13; % broadcast a2
            temp5=logical(isfinite(entireRHS).*(entireRHS~=0));
            entireRHS(temp5)=entireRHS(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
            entireRHS(entireRHS==0)=-Inf;
            ...
            d3opt=rem(ceil(d_ind/N_d1)-1,N_d3)+1; % [1,N_a,N_bothz]
            a1mid=midpoint(allind); % [1,N_a,N_bothz]
    ...
    elseif vfoptions.lowmemory==1
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z
                ...
                % Level-1 Return at coarse a1prime grid
                ReturnMatrix_d4=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,n_a1,n_a2,[n_semiz,ones(1,length(n_z))], d13_with_d4, a1_gridvals, a1_gridvals, a2_gridvals, z_valblock, ReturnFnParamsVec,1,0);
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_d4).*(ReturnMatrix_d4~=0)); % finite but not zero
                temp2=ReturnMatrix_d4;
                temp2(becareful)=ReturnMatrix_d4(becareful).^ezc2(N_j);
                temp2(ReturnMatrix_d4==0)=-Inf;
                entireRHS=ezc1*temp2+DiscountedEV_d13block; % broadcast a2
                temp5=logical(isfinite(entireRHS).*(entireRHS~=0));
                entireRHS(temp5)=entireRHS(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                entireRHS(entireRHS==0)=-Inf;
                ...
                entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_d13block(da1primez(:)),[N_d13*n2long,N_a1*N_a2,N_semiz]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                ...
                L2offset=ceil(maxindexL2/N_d13); % [1,N_a,N_semiz]
                ...
                % Per-d4 policy encoding: d13 + N_d13*(mid-1) + N_d13*N_a1*(L2offset-1)
                Policy_ford4_jj(:,semizblock,d4_c)=shiftdim(d_ind,1)+N_d13*(shiftdim(mid_at,1)-1)+N_d13*N_a1*(shiftdim(L2offset,1)-1);
                ...
                a1mid=midpoint(allind); % [1,N_a,N_semiz]
                a1fine=(a1mid+(a1mid-1)*n2short)+L2offset-n2short-2; % chosen fine a1prime index
...
for reverse_j=1:N_j-1
    ...
    Policy(6,:,:,jj)=reshape(L2ind,[1,N_a,N_bothz]);
    Policy(7,:,:,jj)=reshape(flagwinner,[1,N_a,N_bothz]);
end
