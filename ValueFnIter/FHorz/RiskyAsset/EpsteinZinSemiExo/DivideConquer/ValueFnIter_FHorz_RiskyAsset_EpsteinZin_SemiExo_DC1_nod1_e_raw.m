function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_SemiExo_DC1_nod1_e_raw(n_d2,n_d3,n_d4,n_a1,n_a2,n_semiz,n_z,n_e,n_u,N_j, d2_grid, d3_grid, d4_grid, a1_grid, a2_grid, semiz_gridvals_J, z_gridvals_J, e_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
%
% Epstein-Zin graft onto ValueFnIter_FHorz_RiskyAssetSemiExo_DC1_nod1_e_raw:
...
Policy=zeros(4,N_a,N_bothz,N_e,N_j,'gpuArray'); % d2, d3, d4, a1prime
...
if warmglow==1
    ...
    if ~isfield(vfoptions,'V_Jplus1')
        % Terminal age: the warm-glow enters INSIDE the ^ezc7 root, so build the temp4-analogue of the main loop (with no EV term)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        if warmglow==1
            % Modify the Return Function appropriately for Epstein-Zin Preferences (pre-root; the warm-glow enters INSIDE the ^ezc7 root)
            ...
            % Time to refine
            % warm-glow, we can refine out d2 [ezc9 handles the sign for the max]
            [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3*N_a1]),[],1);
            % temp4_onlyd3 is over (d3,a1prime); rows of temp2 are (d3,d4,a1prime), so spread it over d4
            temp4_onlyd3=reshape(repmat(reshape(temp4_onlyd3,[N_d3,1,N_a1]),1,N_d4,1),[1,N_d3*N_d4*N_a1]);
            % Now put together entireRHS (warm-glow composed INSIDE the ^ezc7 root)
            ...
            Policy(1,:,:,:,N_j)=d2index(d3index+N_d3*shiftdim(a1primeindex-1,-1)); % d2 (note: no a, z nor e in WGmatrix)
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4,n_a1], [n_a1,n_a2], n_bothz, special_n_e, d3d4a1_gridvals, a1a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                d3index=shiftdim(rem(dindex-1,N_d3)+1,-1);
                a1primeindex=ceil(maxindex/(N_d3*N_d4));
                Policy(1,:,:,e_c,N_j)=d2index(d3index+N_d3*shiftdim(a1primeindex-1,-1)); % d2 (note: no a, z nor e in WGmatrix)
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4,n_a1], [n_a1,n_a2], [n_semiz,ones(1,length(n_z))], special_n_e, d3d4a1_gridvals, a1a2_gridvals, z_valblock, e_val, ReturnFnParamsVec);
                ...
                if warmglow==1
                    ...
                    d3index=shiftdim(rem(dindex-1,N_d3)+1,-1);
                    a1primeindex=ceil(maxindex/(N_d3*N_d4));
                    Policy(1,:,semizblock,e_c,N_j)=d2index(d3index+N_d3*shiftdim(a1primeindex-1,-1)); % d2 (note: no a, z nor e in WGmatrix)
                    Policy(2,:,semizblock,e_c,N_j)=d3index; % d3
                    Policy(3,:,semizblock,e_c,N_j)=ceil(dindex/N_d3); % d4
                    Policy(4,:,semizblock,e_c,N_j)=a1primeindex; % a1prime
                elseif warmglow==0
                    % Modify the Return Function appropriately for Epstein-Zin Preferences
                    becareful=logical(isfinite(ReturnMatrix_e).*(ReturnMatrix_e~=0)); % finite and not zero
                    ReturnMatrix_e(becareful)=(ezc1*ReturnMatrix_e(becareful).^ezc2(N_j)).^ezc7(N_j); % Otherwise can get things like 0 to negative power equals infinity
                    ...
                    dindex=rem(maxindex-1,N_d3*N_d4)+1;
                    Policy(1,:,semizblock,e_c,N_j)=1; % d2 is meaningless in the terminal period
                    Policy(2,:,semizblock,e_c,N_j)=rem(dindex-1,N_d3)+1; % d3
                    Policy(3,:,semizblock,e_c,N_j)=ceil(dindex/N_d3); % d4
                    Policy(4,:,semizblock,e_c,N_j)=ceil(maxindex/(N_d3*N_d4)); % a1prime
                end
            end
        end
    elseif vfoptions.lowmemory==3
        for e_c=1:N_e
            ...
            for z_c=1:N_bothz
                ...
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4,n_a1], [n_a1,n_a2], special_n_bothz, special_n_e, d3d4a1_gridvals, a1a2_gridvals, z_val, e_val, ReturnFnParamsVec);
                ...
                if warmglow==1
                    ...
                    [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3*N_a1]),[],1);
                    % temp4_onlyd3 is over (d3,a1prime); rows of temp2 are (d3,d4,a1prime), so spread it over d4
                    temp4_onlyd3=reshape(repmat(reshape(temp4_onlyd3,[N_d3,1,N_a1]),1,N_d4,1),[N_d3*N_d4*N_a1,1]);
                    % Now put together entireRHS (warm-glow composed INSIDE the ^ezc7 root)
                    entireRHS=ezc1*temp2+DiscountFactorParamsVec*ezc9*temp4_onlyd3;
                    temp5=logical(isfinite(entireRHS).*(entireRHS~=0));
                    entireRHS(temp5)=entireRHS(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
                    entireRHS(entireRHS==0)=-Inf;
                    ...
                    a1primeindex=ceil(maxindex/(N_d3*N_d4));
                    Policy(1,:,z_c,e_c,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a, z nor e in WGmatrix)
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            ...
            RM=reshape(temp2_ii,[N_d3,N_a1,vfoptions.level1n,N_a2,N_bothz,N_e]);
            ...
            entireRHS_ii=reshape(entireRHS_ii,[N_d3,N_a1,vfoptions.level1n,N_a2,N_bothz,N_e]);

            [~,maxindex1]=max(entireRHS_ii,[],2);
            [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d3*N_a1,vfoptions.level1n*N_a2,N_bothz,N_e]),[],1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,n_e, d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*shiftdim(zBind,-2);
                    entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    ...
                    allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBind+N_d3*N_a2*N_bothz*eBind;
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],1,level1iidiff(ii),n_a2,n_bothz,n_e, d3_with_d4, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*shiftdim(zBind,-2);
                    entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEV(d3aprimez),[N_d3,level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                    entireRHS_ii(entireRHS_ii==0)=-Inf;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,n_bothz,ones(1,length(n_e)), d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1,0);
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                ...
                RM=reshape(temp2_ii,[N_d3,N_a1,vfoptions.level1n,N_a2,N_bothz]);
                ...
                entireRHS_ii_e=reshape(entireRHS_ii_e,[N_d3,N_a1,vfoptions.level1n,N_a2,N_bothz]);

                [~,maxindex1]=max(entireRHS_ii_e,[],2);
                [Vtempii,maxindex2]=max(reshape(entireRHS_ii_e,[N_d3*N_a1,vfoptions.level1n*N_a2,N_bothz]),[],1);
                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
                pol_d3_a1=shiftdim(maxindex2,1);
                d3part=rem(pol_d3_a1-1,N_d3)+1;
                a1primepart=ceil(pol_d3_a1/N_d3);
                zidx=repmat(gpuArray(1:N_bothz),size(pol_d3_a1,1),1);
                lin=d3part+N_d3*(a1primepart-1)+N_d3*N_a1*(zidx-1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,ones(1,length(n_e)), d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0);
                        becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                        temp2_ii=ReturnMatrix_ii;
                        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                        temp2_ii(ReturnMatrix_ii==0)=-Inf;
                        d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*shiftdim(zBind,-2);
                        entireRHS_ii_e=reshape(ezc1*temp2_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                        ...
                        loweredge=maxindex1(:,1,ii,:,:);
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],1,level1iidiff(ii),n_a2,n_bothz,ones(1,length(n_e)), d3_with_d4, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0);
                        becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                        temp2_ii=ReturnMatrix_ii;
                        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                        temp2_ii(ReturnMatrix_ii==0)=-Inf;
                        d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*shiftdim(zBind,-2);
                        entireRHS_ii_e=reshape(ezc1*temp2_ii+DiscountedEV(d3aprimez),[N_d3,level1iidiff(ii)*N_a2,N_bothz]);
                        temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                        entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                        entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                        [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,[n_semiz,ones(1,length(n_z))],ones(1,length(n_e)), d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,1,0);
                    % Modify the Return Function appropriately for Epstein-Zin Preferences
                    becareful=logical(isfinite(ReturnMatrix_ii_e).*(ReturnMatrix_ii_e~=0)); % finite and not zero
                    temp2_ii=ReturnMatrix_ii_e;
                    temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
                    ...
                    d3part=rem(pol_d3_a1-1,N_d3)+1;
                    a1primepart=ceil(pol_d3_a1/N_d3);
                    zidx=repmat(gpuArray(1:N_semiz),size(pol_d3_a1,1),1);
                    lin=d3part+N_d3*(a1primepart-1)+N_d3*N_a1*(zidx-1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],ones(1,length(n_e)), d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                            becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                            temp2_ii=ReturnMatrix_ii;
                            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                            temp2_ii(ReturnMatrix_ii==0)=-Inf;
                            d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*shiftdim(zBindblock,-2);
                            entireRHS_ii_e=reshape(ezc1*temp2_ii+DiscountedEVblock(d3aprimez),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                            entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                            entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                            [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                            V_ford4_jj(curraindex,semizblock,e_c,d4_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d3)+1);
                            allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBindblock;
                            pol_d3_a1=shiftdim(maxindex+N_d3*(loweredge(allind)-1),1);
                            d3part=rem(pol_d3_a1-1,N_d3)+1;
                            a1primepart=ceil(pol_d3_a1/N_d3);
                            zidx=repmat(gpuArray(1:N_semiz),size(pol_d3_a1,1),1);
                            ...
                            loweredge=maxindex1(:,1,ii,:,:);
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],ones(1,length(n_e)), d3_with_d4, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                            becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                            temp2_ii=ReturnMatrix_ii;
                            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                            temp2_ii(ReturnMatrix_ii==0)=-Inf;
                            d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*shiftdim(zBindblock,-2);
                            entireRHS_ii_e=reshape(ezc1*temp2_ii+DiscountedEVblock(d3aprimez),[N_d3,level1iidiff(ii)*N_a2,N_semiz]);
                            temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                            entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                            entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                            [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
    ...
    elseif vfoptions.lowmemory==3
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,special_n_bothz,ones(1,length(n_e)), d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
                    % Modify the Return Function appropriately for Epstein-Zin Preferences
                    ...
                    DEV=reshape(DiscountedEV_z,[N_d3,N_a1,1,1]);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_bothz,ones(1,length(n_e)), d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0);
                            becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                            temp2_ii=ReturnMatrix_ii;
                            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                            temp2_ii(ReturnMatrix_ii==0)=-Inf;
                            d3aprime=d3ind+N_d3*(a1primeindexes-1);
                            entireRHS_ii_ze=reshape(ezc1*temp2_ii+DiscountedEV_z(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                            temp5=logical(isfinite(entireRHS_ii_ze).*(entireRHS_ii_ze~=0));
                            ...
                            loweredge=maxindex1(:,1,ii,:);
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],1,level1iidiff(ii),n_a2,special_n_bothz,ones(1,length(n_e)), d3_with_d4, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0);
                            becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                            temp2_ii=ReturnMatrix_ii;
                            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                            temp2_ii(ReturnMatrix_ii==0)=-Inf;
                            d3aprime=d3ind+N_d3*(loweredge-1);
                            entireRHS_ii_ze=reshape(ezc1*temp2_ii+DiscountedEV_z(d3aprime),[N_d3,level1iidiff(ii)*N_a2]);
                            temp5=logical(isfinite(entireRHS_ii_ze).*(entireRHS_ii_ze~=0));
                            entireRHS_ii_ze(temp5)=entireRHS_ii_ze(temp5).^ezc7(N_j);
                            entireRHS_ii_ze(entireRHS_ii_ze==0)=-Inf;
                            [Vtempii,maxindex]=max(entireRHS_ii_ze,[],1);
    ...
    Policy(1,:,:,:,N_j)=reshape(d2_ford4_jj(linidx_d4),[1,N_a,N_bothz,N_e]);
    Policy(2,:,:,:,N_j)=reshape(d3_ford4_jj(linidx_d4),[1,N_a,N_bothz,N_e]);
    Policy(3,:,:,:,N_j)=reshape(d4winner,[1,N_a,N_bothz,N_e]);
    Policy(4,:,:,:,N_j)=reshape(a1prime_ford4_jj(linidx_d4),[1,N_a,N_bothz,N_e]);
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,1,0);
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
            temp2_ii=ReturnMatrix_ii;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,n_e, d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(jj);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    ...
                    loweredge=maxindex1(:,1,ii,:,:,:);
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],1,level1iidiff(ii),n_a2,n_bothz,n_e, d3_with_d4, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(jj);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,n_bothz,ones(1,length(n_e)), d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,1,0);
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_ii_e).*(ReturnMatrix_ii_e~=0)); % finite and not zero
                temp2_ii=ReturnMatrix_ii_e;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,ones(1,length(n_e)), d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0);
                        becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                        temp2_ii=ReturnMatrix_ii;
                        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(jj);
                        temp2_ii(ReturnMatrix_ii==0)=-Inf;
                        ...
                        loweredge=maxindex1(:,1,ii,:,:);
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],1,level1iidiff(ii),n_a2,n_bothz,ones(1,length(n_e)), d3_with_d4, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0);
                        becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                        temp2_ii=ReturnMatrix_ii;
                        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(jj);
                        temp2_ii(ReturnMatrix_ii==0)=-Inf;
    ...
    N=N_a*N_bothz*N_e;
    linidx_d4=(1:1:N)'+N*(reshape(d4winner,[N,1])-1);
    Policy(1,:,:,:,jj)=reshape(d2_ford4_jj(linidx_d4),[1,N_a,N_bothz,N_e]);
    Policy(2,:,:,:,jj)=reshape(d3_ford4_jj(linidx_d4),[1,N_a,N_bothz,N_e]);
    Policy(3,:,:,:,jj)=reshape(d4winner,[1,N_a,N_bothz,N_e]);
    Policy(4,:,:,:,jj)=reshape(a1prime_ford4_jj(linidx_d4),[1,N_a,N_bothz,N_e]);
end
