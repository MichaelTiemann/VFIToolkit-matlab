...
%
% Epstein-Zin graft onto ValueFnIter_FHorz_RiskyAssetSemiExo_DC1_raw:
...
Policy=zeros(5,N_a,N_bothz,N_j,'gpuArray'); % d1, d2, d3, d4, a1prime
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        if warmglow==1
            ...
            Policy(1,:,:,N_j)=d1index(maxindex+N_d3*N_d4*N_a1*aind+N_d3*N_d4*N_a1*N_a*bothzind); % d1
            Policy(2,:,:,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a nor bothz in WGmatrix)
        ...
        for z_c=1:N_bothz
            ...
            if warmglow==1
                ...
                Policy(2,:,z_c,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a nor bothz in WGmatrix)
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            zidx=repmat(gpuArray(1:N_bothz),size(pol_d13_a1,1),1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEV(d3aprimez),[N_d13*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    ...
                    entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEV(d3aprimez),[N_d13,level1iidiff(ii)*N_a2,N_bothz]);
                    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                    entireRHS_ii(entireRHS_ii==0)=-Inf;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            ...
            for z_c=1:N_z
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEVblock(d3aprimez),[N_d13*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                        entireRHS_ii(entireRHS_ii==0)=-Inf;
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        ...
                        entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEVblock(d3aprimez),[N_d13,level1iidiff(ii)*N_a2,N_semiz]);
                        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                        entireRHS_ii(entireRHS_ii==0)=-Inf;
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
