function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_noa1_noz_semiz_raw(n_d1,n_d2,n_d3,n_d4,n_a,n_semiz,n_u,N_j, d1_grid, d2_grid, d3_grid, d4_grid, a_grid, semiz_gridvals_J, u_grid, pi_semiz_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
Policy4=zeros(4,N_a,N_semiz,N_j,'gpuArray'); % (d1,d2,d3,d4)
...
d1index_ford4_jj=zeros(N_d3,N_a,N_semiz,N_d4,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,n_d4], n_a, n_semiz, d1d3d4_gridvals, a_gridvals, semiz_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            Policy4(1,:,:,N_j)=d1index(maxindex+N_d3*N_d4*aind+N_d3*N_d4*N_a*semizind); % d1
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_semiz
            ...
            ReturnMatrix_z=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,n_d4], n_a, special_n_semiz, d1d3d4_gridvals, a_gridvals, z_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                Policy4(2,:,z_c,N_j)=d2index(d3index); % d2, note: no a nor semiz in WGmatrix
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_semiz, d1_d3_special_d4_gridvals, a_gridvals, semiz_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        d1_lookup=d3_ind+N_d3*aind+N_d3*N_a*semizind+N_d3*N_a*N_semiz*shiftdim(maxindex-1,-1);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for d4_c=1:N_d4
            ...
            for z_c=1:N_semiz
                z_val=semiz_gridvals_J(z_c,:,N_j);
                ReturnMatrix_d4z=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, special_n_semiz, d1_d3_special_d4_gridvals, a_gridvals, z_val, ReturnFnParamsVec);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            pi_semizd4=pi_semiz(:,:,d4_c);
            d1_d3_special_d4_gridvals=gpuArray(CreateGridvals([n_d1,n_d3,special_n_d4], [d1_grid; d3_grid; d4_gridvals(d4_c,:)'], 1));
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_semiz, d1_d3_special_d4_gridvals, a_gridvals, semiz_gridvals_J(:,:,jj), ReturnFnParamsVec);
