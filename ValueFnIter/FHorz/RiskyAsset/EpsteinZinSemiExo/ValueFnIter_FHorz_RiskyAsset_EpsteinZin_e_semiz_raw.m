...
Policy=zeros(5,N_a,N_semiz*N_z,N_e,N_j,'gpuArray'); % (d1,d2,d3,d4,a1prime)
...
if warmglow==1
    ...
    % Now just make it the right shape (temp4 spans bothz in all lowmemory tiers of the e raws)
    if isfield(vfoptions,'V_Jplus1')
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        if warmglow==1
            ...
            [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_d4*N_a1,N_a,N_bothz,N_e]),[],1);
            ...
            Policy(2,:,:,:,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a nor z in WGmatrix)
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            if warmglow==1
                ...
                % Time to refine
                % First: ReturnMatrix, we can refine out d1
                [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_d4*N_a1,N_a,N_bothz]),[],1);
                % Second: warm-glow, we can refine out d2
                [WGmatrix_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3*N_a1]),[],1);
                % WGmatrix_onlyd3 is over (d3,a1prime); rows of temp2_onlyd3 are (d3,d4,a1prime), so spread it over d4
                WGmatrix_onlyd3=reshape(repmat(reshape(WGmatrix_onlyd3,[N_d3,1,N_a1]),1,N_d4,1),[1,N_d3*N_d4*N_a1]);
                ...
                Policy(2,:,:,e_c,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a nor z in WGmatrix)
    ...
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4,n_a1], [n_a1,n_a2], [n_semiz,ones(1,length(n_z))], special_n_e, d1d3d4a1_gridvals, a1a2_gridvals, bothz_gridvals_J(semizblock,:,N_j), e_val, ReturnFnParamsVec);
                ...
                if warmglow==1
                    ...
                    d3index=rem(dindex-1,N_d3)+1;
                    a1primeindex=ceil(maxindex/(N_d3*N_d4));
                    Policy(1,:,semizblock,e_c,N_j)=d1index(maxindex+N_d3*N_d4*N_a1*aind+N_d3*N_d4*N_a1*N_a*shiftdim(0:1:N_semiz-1,-1)); % d1
                    Policy(2,:,semizblock,e_c,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a nor z in WGmatrix)
                    Policy(3,:,semizblock,e_c,N_j)=d3index; % d3
                    Policy(4,:,semizblock,e_c,N_j)=shiftdim(ceil(dindex/N_d3),-1); % d4
                    Policy(5,:,semizblock,e_c,N_j)=shiftdim(a1primeindex,-1); % a1prime
                elseif warmglow==0
                    % Modify the Return Function appropriately for Epstein-Zin Preferences
                    becareful=logical(isfinite(ReturnMatrix_ze).*(ReturnMatrix_ze~=0)); % finite and not zero
                    ...
                    dindex=rem(maxindex-1,N_d1*N_d3*N_d4)+1;
                    d1d3_ind=rem(dindex-1,N_d1*N_d3)+1;
                    Policy(1,:,semizblock,e_c,N_j)=shiftdim(rem(d1d3_ind-1,N_d1)+1,-1);
                    Policy(2,:,semizblock,e_c,N_j)=1;
                    Policy(3,:,semizblock,e_c,N_j)=shiftdim(ceil(d1d3_ind/N_d1),-1);
                    Policy(4,:,semizblock,e_c,N_j)=shiftdim(ceil(dindex/(N_d1*N_d3)),-1);
                    Policy(5,:,semizblock,e_c,N_j)=shiftdim(ceil(maxindex/(N_d1*N_d3*N_d4)),-1);
                end
            end
        end
        ...
        for e_c=1:N_e
            ...
            for z_c=1:N_bothz
                ...
                if warmglow==1
                    ...
                    Policy(2,:,z_c,e_c,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a nor z in WGmatrix)
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            % Time to refine
            % First: ReturnMatrix, we can refine out d1
            [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_a1,N_a,N_bothz,N_e]),[],1);
            % Second: EV, we can refine out d2
            [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(temp4)).*temp4,[N_d2,N_d3*N_a1,1,N_bothz,1]),[],1);
        ...
        d1_lookup=d3a1prime_ind+N_d3*N_a1*aind+N_d3*N_a1*N_a*bothzind+N_d3*N_a1*N_a*N_bothz*eind+N_d3*N_a1*N_a*N_bothz*N_e*shiftdim(maxindex-1,-1);
    ...
    elseif vfoptions.lowmemory>=1 % terminal lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                % First: ReturnMatrix, we can refine out d1
                [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_a1,N_a,N_bothz]),[],1);
        ...
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z % outer loop over z (markov)
                ...
                for e_c=1:N_e % inner loop over e
                    e_val=e_gridvals_J(e_c,:,jj);
                    ReturnMatrix_d4ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4,n_a1], [n_a1,n_a2], [n_semiz,ones(1,length(n_z))], special_n_e, d1_d3_special_d4_a1_gridvals, a1a2_gridvals, bothz_gridvals_J(semizblock,:,jj), e_val, ReturnFnParamsVec);
                    ...
                    entireRHS_ze=ezc1*ezc9*shiftdim(temp2_onlyd3,1)+DiscountedEV_onlyd3(:,:,semizblock);
    ...
    elseif vfoptions.lowmemory==3 % outer bothz, inner e
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz % outer loop over bothz
                ...
                for e_c=1:N_e % inner loop over e
                    e_val=e_gridvals_J(e_c,:,jj);
                    ReturnMatrix_d4ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4,n_a1], [n_a1,n_a2], special_n_bothz, special_n_e, d1_d3_special_d4_a1_gridvals, a1a2_gridvals, z_val, e_val, ReturnFnParamsVec);
