...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        if warmglow==1
            ...
            % WGmatrix_onlyd3 is over (d3,a1prime); rows of temp2_onlyd3 are (d3,d4,a1prime), so spread it over d4
            ...
            entireRHS=shiftdim(ezc1*ezc9*temp2_onlyd3+DiscountFactorParamsVec*ezc9*WGmatrix_onlyd3,1);
            ...
            Policy(4,:,:,:,N_j)=shiftdim(ceil(dindex/N_d3),-1); % d4
            Policy(5,:,:,:,N_j)=shiftdim(a1primeindex,-1); % a1prime
        elseif warmglow==0
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            becareful=logical(isfinite(ReturnMatrix).*(ReturnMatrix~=0)); % finite and not zero
            ...
            Policy(2,:,:,:,N_j)=1; % d2 is meaningless
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            if warmglow==1
                ...
                Policy(2,:,:,e_c,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a nor semiz in WGmatrix)
                Policy(3,:,:,e_c,N_j)=d3index; % d3
                Policy(4,:,:,e_c,N_j)=shiftdim(ceil(dindex/N_d3),-1); % d4
                Policy(5,:,:,e_c,N_j)=shiftdim(a1primeindex,-1); % a1prime
            elseif warmglow==0
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_e).*(ReturnMatrix_e~=0)); % finite and not zero
                ...
                dindex=rem(maxindex-1,N_d1*N_d3*N_d4)+1;
                d1d3_ind=rem(dindex-1,N_d1*N_d3)+1;
                Policy(1,:,:,e_c,N_j)=shiftdim(rem(d1d3_ind-1,N_d1)+1,-1);
                Policy(2,:,:,e_c,N_j)=1; % d2 is meaningless
                Policy(3,:,:,e_c,N_j)=shiftdim(ceil(d1d3_ind/N_d1),-1);
                Policy(4,:,:,e_c,N_j)=shiftdim(ceil(dindex/(N_d1*N_d3)),-1);
                Policy(5,:,:,e_c,N_j)=shiftdim(ceil(maxindex/(N_d1*N_d3*N_d4)),-1);
            end
        end
        ...
        for z_c=1:N_semiz % outer loop over semiz
            ...
            for e_c=1:N_e % inner loop over e
                ...
                if warmglow==1
                    ...
                    Policy(2,:,z_c,e_c,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a nor semiz in WGmatrix)
                    Policy(3,:,z_c,e_c,N_j)=d3index; % d3
                    Policy(4,:,z_c,e_c,N_j)=shiftdim(ceil(dindex/N_d3),-1); % d4
                    Policy(5,:,z_c,e_c,N_j)=shiftdim(a1primeindex,-1); % a1prime
                elseif warmglow==0
                    % Modify the Return Function appropriately for Epstein-Zin Preferences
                    becareful=logical(isfinite(ReturnMatrix_ze).*(ReturnMatrix_ze~=0)); % finite and not zero
                    ...
                    dindex=rem(maxindex-1,N_d1*N_d3*N_d4)+1;
                    d1d3_ind=rem(dindex-1,N_d1*N_d3)+1;
                    Policy(1,:,z_c,e_c,N_j)=shiftdim(rem(d1d3_ind-1,N_d1)+1,-1);
                    Policy(2,:,z_c,e_c,N_j)=1; % d2 is meaningless
                    Policy(3,:,z_c,e_c,N_j)=shiftdim(ceil(d1d3_ind/N_d1),-1);
                    Policy(4,:,z_c,e_c,N_j)=shiftdim(ceil(dindex/(N_d1*N_d3)),-1);
                    Policy(5,:,z_c,e_c,N_j)=shiftdim(ceil(maxindex/(N_d1*N_d3*N_d4)),-1);
                end
            end
        end
    end
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4,n_a1], [n_a1,n_a2], n_semiz, n_e, d1_d3_special_d4_a1_gridvals, a1a2_gridvals, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
            ...
            % Time to refine
            % First: ReturnMatrix, we can refine out d1
            [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_a1,N_a,N_semiz,N_e]),[],1);
            % Second: EV, we can refine out d2
            [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(temp4)).*temp4,[N_d2,N_d3*N_a1,1,N_semiz,1]),[],1);
        ...
        % Now we just max over d4
        [V_jj,maxindex]=max(V_ford4_jj,[],4); % max over d4
        V(:,:,:,N_j)=V_jj;
        Policy(4,:,:,:,N_j)=maxindex; % d4
        ...
        % d2 only depends on (d3a1prime, semiz, d4) (no e)
        Policy(2,:,:,:,N_j)=shiftdim(d2index_ford4_jj(d3a1prime_ind+N_d3*N_a1*semizind+N_d3*N_a1*N_semiz*shiftdim(maxindex-1,-1)),-1); % d2
        % d1 depends on (d3a1prime, a, semiz, e, d4)
        d1_lookup=d3a1prime_ind+N_d3*N_a1*aind+N_d3*N_a1*N_a*semizind+N_d3*N_a1*N_a*N_semiz*eind+N_d3*N_a1*N_a*N_semiz*N_e*shiftdim(maxindex-1,-1);
    ...
    elseif vfoptions.lowmemory>=1 % terminal lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_d4e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4,n_a1], [n_a1,n_a2], n_semiz, special_n_e, d1_d3_special_d4_a1_gridvals, a1a2_gridvals, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
                ...
                % First: ReturnMatrix, we can refine out d1
                [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_a1,N_a,N_semiz]),[],1);

                entireRHS_e=ezc1*ezc9*shiftdim(temp2_onlyd3,1)+DiscountedEV_onlyd3;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            pi_semizd4=pi_semiz(:,:,d4_c);
            d1_d3_special_d4_a1_gridvals=gpuArray(CreateGridvals([n_d1,n_d3,special_n_d4,n_a1], [d1_grid; d3_grid; d4_gridvals(d4_c,:)'; a1_grid], 1));
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4,n_a1], [n_a1,n_a2], n_semiz, n_e, d1_d3_special_d4_a1_gridvals, a1a2_gridvals, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec);
        ...
        [V_jj,maxindex]=max(V_ford4_jj,[],4); % max over d4
        V(:,:,:,jj)=V_jj;
        Policy(4,:,:,:,jj)=maxindex; % d4
        maxindex_d4=reshape(maxindex,[N_a*N_semiz*N_e,1]);
        d3a1prime_ind=reshape(Policy_ford4_jj((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindex_d4-1)),[1,N_a,N_semiz,N_e]);
        Policy(3,:,:,:,jj)=shiftdim(rem(d3a1prime_ind-1,N_d3)+1,-1); % d3
        Policy(5,:,:,:,jj)=shiftdim(ceil(d3a1prime_ind/N_d3),-1); % a1prime
        Policy(2,:,:,:,jj)=shiftdim(d2index_ford4_jj(d3a1prime_ind+N_d3*N_a1*semizind+N_d3*N_a1*N_semiz*shiftdim(maxindex-1,-1)),-1); % d2
        d1_lookup=d3a1prime_ind+N_d3*N_a1*aind+N_d3*N_a1*N_a*semizind+N_d3*N_a1*N_a*N_semiz*eind+N_d3*N_a1*N_a*N_semiz*N_e*shiftdim(maxindex-1,-1);
        Policy(1,:,:,:,jj)=shiftdim(d1index_ford4_jj(d1_lookup),-1); % d1
    ...
    elseif vfoptions.lowmemory==1
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_d4e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4,n_a1], [n_a1,n_a2], n_semiz, special_n_e, d1_d3_special_d4_a1_gridvals, a1a2_gridvals, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec);
    ...
    elseif vfoptions.lowmemory>=2 % lm2 already does the most-looped variant, so it also serves the higher lowmemory values
        for d4_c=1:N_d4
            ...
            for z_c=1:N_semiz % outer loop over semiz
                ...
                for e_c=1:N_e % inner loop over e
                    e_val=e_gridvals_J(e_c,:,jj);
                    ReturnMatrix_d4ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4,n_a1], [n_a1,n_a2], special_n_semiz, special_n_e, d1_d3_special_d4_a1_gridvals, a1a2_gridvals, z_val, e_val, ReturnFnParamsVec);
                    ...
                    entireRHS_ze=ezc1*ezc9*shiftdim(temp2_onlyd3,1)+DiscountedEV_onlyd3(:,:,z_c);
