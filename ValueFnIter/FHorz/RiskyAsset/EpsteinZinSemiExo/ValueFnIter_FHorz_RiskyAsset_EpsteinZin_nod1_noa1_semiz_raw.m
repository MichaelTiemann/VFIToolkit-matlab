function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_nod1_noa1_semiz_raw(n_d2,n_d3,n_d4,n_a,n_semiz,n_z,n_u,N_j, d2_grid, d3_grid, d4_grid, a_grid, semiz_gridvals_J, z_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % loop over z (markov), vectorize over semiz
        for z_c=1:N_z
            ...
            ReturnMatrix_z=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d3,n_d4], n_a, [n_semiz,ones(1,length(n_z))], d3d4_gridvals, a_gridvals, z_valblock, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                d3index=rem(maxindex-1,N_d3)+1;
                Policy3(1,:,semizblock,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
                Policy3(2,:,semizblock,N_j)=d3index; % d3
                Policy3(3,:,semizblock,N_j)=shiftdim(ceil(maxindex/N_d3),-1); % d4
            elseif warmglow==0
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_z).*(ReturnMatrix_z~=0)); % finite and not zero
                ...
                V(:,semizblock,N_j)=Vtemp;
                Policy3(1,:,semizblock,N_j)=1; % d2, is meaningless anyway
                Policy3(2,:,semizblock,N_j)=rem(maxindex-1,N_d3)+1; % d3
                Policy3(3,:,semizblock,N_j)=shiftdim(ceil(maxindex/N_d3),-1); % d4
            end
        end
    ...
    elseif vfoptions.lowmemory>=2 % loop over bothz (outer semiz, inner z); also covers any higher lowmemory values
        for z_c=1:N_bothz
            ...
            if warmglow==1
                ...
                V(:,z_c,N_j)=Vtemp;
                d3index=rem(maxindex-1,N_d3)+1;
                Policy3(1,:,z_c,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
    ...
    elseif vfoptions.lowmemory==1 % loop over z (markov), vectorize over semiz
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z
                ...
                ReturnMatrix_d4z=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d3,special_n_d4], n_a, [n_semiz,ones(1,length(n_z))], d3_special_d4_gridvals, a_gridvals, z_valblock, ReturnFnParamsVec);
