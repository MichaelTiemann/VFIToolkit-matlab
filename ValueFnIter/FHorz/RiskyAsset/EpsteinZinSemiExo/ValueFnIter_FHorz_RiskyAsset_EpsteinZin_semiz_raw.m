...
Policy=zeros(5,N_a,N_semiz*N_z,N_j,'gpuArray'); % d1, d2, d3, d4 and a1prime
...
d1index_ford4_jj=zeros(N_d3*N_a1,N_a,N_semiz*N_z,N_d4,'gpuArray'); % d1 refine depends on a
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        elseif warmglow==0
            ...
            Policy(1,:,:,N_j)=shiftdim(rem(d1d3_ind-1,N_d1)+1,-1); % d1
            Policy(2,:,:,N_j)=1; % d2 is meaningless
            Policy(3,:,:,N_j)=shiftdim(ceil(d1d3_ind/N_d1),-1); % d3
        ...
        for z_c=1:N_bothz
            ...
            if warmglow==1
                ...
                Policy(2,:,z_c,N_j)=d2index(d3index+N_d3*(a1primeindex-1)); % d2 (note: no a nor z in WGmatrix)
                Policy(3,:,z_c,N_j)=d3index; % d3
                Policy(4,:,z_c,N_j)=shiftdim(ceil(dindex/N_d3),-1); % d4
                Policy(5,:,z_c,N_j)=shiftdim(a1primeindex,-1); % a1prime
            elseif warmglow==0
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_z).*(ReturnMatrix_z~=0)); % finite and not zero
                ...
                Policy(2,:,z_c,N_j)=1; % d2 is meaningless
    ...
    if vfoptions.lowmemory==0
        ...
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,special_n_d4,n_a1], [n_a1,n_a2], n_bothz, d1_d3_special_d4_a1_gridvals, a1a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec);
    ...
    elseif vfoptions.lowmemory>=2 % loop over bothz (outer semiz, inner z); also covers any higher lowmemory values
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                z_val=bothz_gridvals_J(z_c,:,N_j);
                ReturnMatrix_d4z=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,special_n_d4,n_a1], [n_a1,n_a2], special_n_bothz, d1_d3_special_d4_a1_gridvals, a1a2_gridvals, z_val, ReturnFnParamsVec);
                ...
                % Expectation over u (using pi_u), and then add the lower and upper
                EV_z=sum((EV1_z.*pi_u'),2)+sum((EV2_z.*pi_u'),2); % (d&a1prime,1), sum over u
                ...
                % entireRHS_d4z=ezc1*temp2+ezc3*DiscountFactorParamsVec*temp4;
