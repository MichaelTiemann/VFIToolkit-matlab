function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_noa1_semiz_raw(n_d1,n_d2,n_d3,n_d4,n_a,n_semiz,n_z,n_u,N_j, d1_grid, d2_grid, d3_grid, d4_grid, a_grid, semiz_gridvals_J, z_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
Policy4=zeros(4,N_a,N_semiz*N_z,N_j,'gpuArray'); % (d1,d2,d3,d4)
...
d1index_ford4_jj=zeros(N_d3,N_a,N_semiz*N_z,N_d4,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,n_d4], n_a, n_bothz, d1d3d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            Policy4(1,:,:,N_j)=d1index(maxindex+N_d3*N_d4*aind+N_d3*N_d4*N_a*bothzind); % d1
    ...
    else % lowmemory>=1: terminal has no continuation value, so looping over bothz suffices for every lowmemory level
        for z_c=1:N_bothz
            z_val=bothz_gridvals_J(z_c,:,N_j);
            ReturnMatrix_z=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,n_d4], n_a, special_n_bothz, d1d3d4_gridvals, a_gridvals, z_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                V(:,z_c,N_j)=Vtemp;
                d3index=rem(maxindex-1,N_d3)+1;
                Policy4(1,:,z_c,N_j)=d1index(maxindex+N_d3*N_d4*aind); % d1
                Policy4(2,:,z_c,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
                Policy4(3,:,z_c,N_j)=d3index; % d3
                Policy4(4,:,z_c,N_j)=shiftdim(ceil(maxindex/N_d3),-1); % d4
            elseif warmglow==0
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_z).*(ReturnMatrix_z~=0)); % finite and not zero
                ...
                V(:,z_c,N_j)=Vtemp;
                d1d3_ind=rem(maxindex-1,N_d1*N_d3)+1;
                Policy4(1,:,z_c,N_j)=shiftdim(rem(d1d3_ind-1,N_d1)+1,-1); % d1
                Policy4(2,:,z_c,N_j)=1; % d2, is meaningless anyway
                Policy4(3,:,z_c,N_j)=shiftdim(ceil(d1d3_ind/N_d1),-1); % d3
                Policy4(4,:,z_c,N_j)=shiftdim(ceil(maxindex/(N_d1*N_d3)),-1); % d4
            end
        end
    end
else
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_bothz, d1_d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec);
    ...
    elseif vfoptions.lowmemory==1 % loop over z (markov), vectorize over semiz
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z
                ...
                ReturnMatrix_d4z=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, [n_semiz,ones(1,length(n_z))], d1_d3_special_d4_gridvals, a_gridvals, z_valblock, ReturnFnParamsVec);
                ...
                % Now put together entireRHS, which just depends on d3
                entireRHS_d4z=shiftdim(ezc1*ezc9*temp2_onlyd3+DiscountFactorParamsVec*ezc9*temp4_onlyd3(:,:,:,semizblock),1);
    ...
    elseif vfoptions.lowmemory>=2 % loop over bothz (outer semiz, inner z); also covers any higher lowmemory values
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                z_val=bothz_gridvals_J(z_c,:,N_j);
                ReturnMatrix_d4z=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, special_n_bothz, d1_d3_special_d4_gridvals, a_gridvals, z_val, ReturnFnParamsVec);
                ...
                % Now put together entireRHS, which just depends on d3
                entireRHS_d4z=shiftdim(ezc1*ezc9*temp2_onlyd3+DiscountFactorParamsVec*ezc9*temp4_onlyd3,1);
...
for reverse_j=1:N_j-1
    ...
    if warmglow==1
        ...
        WGmatrix=sum((WG1.*pi_u'),2)+sum((WG2.*pi_u'),2); % (d,1), sum over u
        % WGmatrix is over (d,1)
        % Now just make it the right shape (lowmemory==0 and lowmemory==1 EV is vectorized over bothz)
        if vfoptions.lowmemory==0 || vfoptions.lowmemory==1
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            d1_d3_special_d4_gridvals=gpuArray(CreateGridvals([n_d1,n_d3,special_n_d4], [d1_grid; d3_grid; d4_gridvals(d4_c,:)'], 1));
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_bothz, d1_d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec);
