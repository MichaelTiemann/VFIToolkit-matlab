function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_nod1_noa1_e_semiz_raw(n_d2,n_d3,n_d4,n_a,n_semiz,n_z,n_e,n_u,N_j, d2_grid, d3_grid, d4_grid, a_grid, semiz_gridvals_J, z_gridvals_J, e_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
Policy3=zeros(3,N_a,N_semiz*N_z,N_e,N_j,'gpuArray'); % d2, d3, d4
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4], n_a, n_bothz, n_e, d3d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            entireRHS=ezc1*temp2+DiscountFactorParamsVec*ezc9*shiftdim(repmat(WGmatrix_onlyd3,1,N_d4),1);
            ...
            Policy3(1,:,:,:,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4], n_a, n_bothz, special_n_e, d3d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                V(:,:,e_c,N_j)=Vtemp;
                d3index=rem(maxindex-1,N_d3)+1;
                Policy3(1,:,:,e_c,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
    ...
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4], n_a, [n_semiz,ones(1,length(n_z))], special_n_e, d3d4_gridvals, a_gridvals, bothz_gridvals_J(semizblock,:,N_j), e_val, ReturnFnParamsVec);
                ...
                if warmglow==1
                    ...
                    d3index=rem(maxindex-1,N_d3)+1;
                    Policy3(1,:,semizblock,e_c,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
                    Policy3(2,:,semizblock,e_c,N_j)=d3index; % d3
                    Policy3(3,:,semizblock,e_c,N_j)=shiftdim(ceil(maxindex/N_d3),-1); % d4
                elseif warmglow==0
                    % Modify the Return Function appropriately for Epstein-Zin Preferences
                    becareful=logical(isfinite(ReturnMatrix_ze).*(ReturnMatrix_ze~=0)); % finite and not zero
                    ...
                    V(:,semizblock,e_c,N_j)=Vtemp;
                    Policy3(1,:,semizblock,e_c,N_j)=1; % d2, is meaningless anyway
                    Policy3(2,:,semizblock,e_c,N_j)=rem(maxindex-1,N_d3)+1; % d3
                    Policy3(3,:,semizblock,e_c,N_j)=shiftdim(ceil(maxindex/N_d3),-1); % d4
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==3 % outer bothz, inner e
        for e_c=1:N_e
            ...
            for z_c=1:N_bothz
                ...
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4], n_a, special_n_bothz, special_n_e, d3d4_gridvals, a_gridvals, z_val, e_val, ReturnFnParamsVec);
                ...
                if warmglow==1
                    ...
                    V(:,z_c,e_c,N_j)=Vtemp;
                    d3index=rem(maxindex-1,N_d3)+1;
                    Policy3(1,:,z_c,e_c,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,special_n_d4], n_a, n_bothz, n_e, d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
    ...
    elseif vfoptions.lowmemory>=1 % terminal lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_d4e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,special_n_d4], n_a, n_bothz, special_n_e, d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            d3_special_d4_gridvals=gpuArray(CreateGridvals([n_d3,special_n_d4], [d3_grid; d4_gridvals(d4_c,:)'], 1));
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,special_n_d4], n_a, n_bothz, n_e, d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec);
    ...
    elseif vfoptions.lowmemory==1
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_d4e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,special_n_d4], n_a, n_bothz, special_n_e, d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec);
        ...
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z % outer loop over z (markov)
                ...
                for e_c=1:N_e % inner loop over e
                    e_val=e_gridvals_J(e_c,:,jj);
                    ReturnMatrix_d4ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,special_n_d4], n_a, [n_semiz,ones(1,length(n_z))], special_n_e, d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(semizblock,:,jj), e_val, ReturnFnParamsVec);
                    ...
                    % Now put together entireRHS, which just depends on d3
                    entireRHS_ze=ezc1*temp2+DiscountedEV_onlyd3(:,:,semizblock);
    ...
    elseif vfoptions.lowmemory==3 % outer bothz, inner e
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz % outer loop over bothz
                ...
                for e_c=1:N_e % inner loop over e
                    e_val=e_gridvals_J(e_c,:,jj);
                    ReturnMatrix_d4ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,special_n_d4], n_a, special_n_bothz, special_n_e, d3_special_d4_gridvals, a_gridvals, z_val, e_val, ReturnFnParamsVec);
                    ...
                    % Now put together entireRHS, which just depends on d3
                    entireRHS_ze=ezc1*temp2+DiscountedEV_onlyd3(:,:,z_c);
