...
% ValueFnIter_FHorz_RiskyAssetSemiExo_DC1_GI1_e_raw. V' is transformed
% ((ezc4*V')^ezc5, masked) BEFORE all the expectations: the e' expectation
% (e is iid, integrated first), the (semiz',z') expectation (inside the d4
% loop) and the u-lottery all act on the transformed object, giving one joint
% certainty-equivalent over (u,semiz',z',e'). The interpolation over a1prime
% acts on the transformed expectations object BEFORE the certainty-equivalent
% power ^ezc6 (exact collapses under GI); the ^ezc6 transform is applied
% pointwise to both the coarse and fine objects AFTER the interpolation. d2 is
% refined out AFTER the full transform chain using the ezc9 sign trick (coarse
% and fine separately; the d2 policy is read off the fine refine at the chosen
% fine a1prime point). The return transform and the final ^ezc7 wrap each
% entireRHS before its max at every DC level.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if warmglow==1
        ...
        % Refine d2 out of the warm-glow (the only continuation term in the terminal period) [ezc9 handles the sign for the max]
        [WGmatrix_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3]),[],1);
        % Rows of temp2 are (d1,d3,d4,a1prime): spread the refined warm-glow over d1, d4 and a1prime (it is constant in all three)
        DiscountedWGcol=DiscountFactorParamsVec*ezc9*repmat(reshape(WGmatrix_onlyd3(d3ind),[N_d13,1]),N_d4*N_a1,1); % [N_d13*N_d4*N_a1,1]
        entireRHS=ezc1*temp2+DiscountedWGcol;
        ...
        d2Policy(1,:,:,:,N_j)=d2index(d3part); % d2 comes from refining the warm-glow (no a nor z nor e in WGmatrix)
    else
    ...
    for d4_c=1:N_d4
        ...
        % Expectation over (semiz',z') (on the transformed object)
        ...
        if vfoptions.lowmemory==0
            midpoint=zeros(N_d13,1,N_a1,N_a2,N_bothz,N_e,'gpuArray');

            % n-Monotonicity (coarse DC search at level1ii midpoints)
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d13_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals, e_gridvals, ReturnFnParamsVec,1,0);
            RM=reshape(ReturnMatrix_ii,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2,N_bothz,N_e]);
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,n_e, d13_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals, e_gridvals, ReturnFnParamsVec,3,0);
            ...
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_bothz,N_e])+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_bothz,N_e]),[N_d13*n2long,N_a1*N_a2,N_bothz,N_e]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            ...
            allind=d_ind+N_d13*aind+N_d13*N_a*zBind+N_d13*N_a*N_bothz*eBind;
            ...
            % L2 flag detection (on the transformed return matrix: -Inf marks both infeasible and zero-return points)
            L2offset      = ceil(maxindexL2/N_d13);
            linidx_lower  = d_ind                    + N_d13*n2long*aind + N_d13*n2long*N_a*zBind + N_d13*n2long*N_a*N_bothz*eBind;
            linidx_upper  = d_ind + N_d13*(n2long-1) + N_d13*n2long*aind + N_d13*n2long*N_a*zBind + N_d13*n2long*N_a*N_bothz*eBind;
            ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_bothz,N_e]);
        ...
        elseif vfoptions.lowmemory==1
            ...
            for e_c=1:N_e
                ...
                % n-Monotonicity (coarse DC search at level1ii midpoints)
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,n_bothz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals, e_val, ReturnFnParamsVec,1,0);
                RM=reshape(ReturnMatrix_ii_e,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2,N_bothz]);
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(RM).*(RM~=0)); % finite but not zero
                temp2_ii=RM;
                temp2_ii(becareful)=RM(becareful).^ezc2(N_j);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d13_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals, e_val, ReturnFnParamsVec,3,0);
                        becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                        temp2_ii=ReturnMatrix_ii;
                        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                        temp2_ii(ReturnMatrix_ii==0)=-Inf;
                ...
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                da1primez=d3ind+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*shiftdim(zBind,-2);
                entireRHS_ii_e=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_bothz])+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_bothz]),[N_d13*n2long,N_a1*N_a2,N_bothz]);
                temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                ...
                Policy3_ford4_jj(1,:,:,e_c,d4_c)=d_ind; % d13 index
                ...
                ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_bothz]);
                isInfLower    = (ReturnMatrix_ii_resh(linidx_lower) == -Inf);
                isInfUpper    = (ReturnMatrix_ii_resh(linidx_upper) == -Inf);
                inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
                inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
        ...
        elseif vfoptions.lowmemory==2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    % n-Monotonicity (coarse DC search at level1ii midpoints)
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d13_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,1,0);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d13_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                            becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                            temp2_ii=ReturnMatrix_ii;
                            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                            temp2_ii(ReturnMatrix_ii==0)=-Inf;
                            d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*shiftdim(zBindblock,-2);
                            entireRHS_ii_e=ezc1*temp2_ii+DiscountedEVblock(d3aprimez);
                            temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                            entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                            entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                            [~,maxindex]=max(entireRHS_ii_e,[],2);
                    ...
                    entireRHS_ii_e=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_semiz])+reshape(DiscountedEVinterpblock(da1primez),[N_d13,n2long,N_a1,N_a2,N_semiz]),[N_d13*n2long,N_a1*N_a2,N_semiz]);
                    temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                    entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                    entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                    [Vtempii,maxindexL2]=max(entireRHS_ii_e,[],1);
                    ...
                    Policy3_ford4_jj(1,:,semizblock,e_c,d4_c)=d_ind; % d13 index
                    Policy3_ford4_jj(2,:,semizblock,e_c,d4_c)=shiftdim(squeeze(midpoint(allind)),-1);
                    Policy3_ford4_jj(3,:,semizblock,e_c,d4_c)=shiftdim(ceil(maxindexL2/N_d13),-1);
                    ...
                    linlookup=d3part+N_d3*(a1fine-1)+N_d3*N_a1prime*(zidx-1);
                    d2_ford4_jj(:,semizblock,e_c,d4_c)=d2indexfine_reshblock(linlookup);
                end
            end
        elseif vfoptions.lowmemory==3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    % n-Monotonicity (coarse DC search at level1ii midpoints)
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,special_n_bothz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_val, e_val, ReturnFnParamsVec,1,0);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_bothz,special_n_e, d13_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_val, e_val, ReturnFnParamsVec,3,0);
                            becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                            temp2_ii=ReturnMatrix_ii;
                            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                            temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    ...
                    midpoint=max(min(midpoint,n_a1(1)-1),2);
                    a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d3,special_n_d4],n2long,n_a1,n_a2,special_n_bothz,special_n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_val, e_val, ReturnFnParamsVec,2,0);
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    ...
                    Policy3_ford4_jj(1,:,z_c,e_c,d4_c)=d_ind; % d13 index
    ...
    % Policy3_ford4_jj is [3,N_a,N_bothz,N_e,N_d4]; we want per-(a,z,e) gather along dim 5
