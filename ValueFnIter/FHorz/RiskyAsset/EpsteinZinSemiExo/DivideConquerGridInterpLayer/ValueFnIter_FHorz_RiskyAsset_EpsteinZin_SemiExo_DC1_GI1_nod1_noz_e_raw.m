function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_SemiExo_DC1_GI1_nod1_noz_e_raw(n_d2,n_d3,n_d4,n_a1,n_a2,n_semiz,n_e,n_u,N_j, d2_grid, d3_grid, d4_grid, a1_grid, a2_grid, semiz_gridvals_J, e_gridvals_J, u_grid, pi_semiz_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
% ValueFnIter_FHorz_RiskyAssetSemiExo_DC1_GI1_nod1_noz_e_raw. V' is
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for d4_c=1:N_d4
        ...
        if vfoptions.lowmemory==0
            midpoint=zeros(N_d3,1,N_a1,N_a2,N_semiz,N_e,'gpuArray');

            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,n_semiz,n_e, d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, semiz_gridvals, e_gridvals, ReturnFnParamsVec,1,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,n_semiz,n_e, d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, semiz_gridvals, e_gridvals, ReturnFnParamsVec,3,0);
            ...
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_semiz,N_e])+reshape(DiscountedEVinterp(da1primez),[N_d3,n2long,N_a1,N_a2,N_semiz,N_e]),[N_d3*n2long,N_a1*N_a2,N_semiz,N_e]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            ...
            allind=d_ind+N_d3*aind+N_d3*N_a*zBind+N_d3*N_a*N_semiz*eBind;
            ...
            linidx_lower  = d_ind                   + N_d3*n2long*aind + N_d3*n2long*N_a*zBind + N_d3*n2long*N_a*N_semiz*eBind;
            linidx_upper  = d_ind + N_d3*(n2long-1) + N_d3*n2long*aind + N_d3*n2long*N_a*zBind + N_d3*n2long*N_a*N_semiz*eBind;
            ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_semiz,N_e]);
        ...
        elseif vfoptions.lowmemory==1
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,n_semiz,special_n_e, d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, semiz_gridvals, e_val, ReturnFnParamsVec,1,0);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,n_semiz,special_n_e, d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, semiz_gridvals, e_val, ReturnFnParamsVec,3,0);
                        becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                        temp2_ii=ReturnMatrix_ii;
                        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                        temp2_ii(ReturnMatrix_ii==0)=-Inf;
                ...
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                da1primez=d3ind+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*shiftdim(zBind,-2);
                entireRHS_ii_e=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_semiz])+reshape(DiscountedEVinterp(da1primez),[N_d3,n2long,N_a1,N_a2,N_semiz]),[N_d3*n2long,N_a1*N_a2,N_semiz]);
                temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
        ...
        elseif vfoptions.lowmemory>=2 % lm2 already does the most-looped variant, so it also serves the higher lowmemory values
            ...
            for z_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n_a1,vfoptions.level1n,n_a2,special_n_semiz,special_n_e, d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, semiz_val, e_val, ReturnFnParamsVec,1,0);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_semiz,special_n_e, d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, semiz_val, e_val, ReturnFnParamsVec,3,0);
                            becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                            temp2_ii=ReturnMatrix_ii;
                            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                            temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    ...
                    midpoint=max(min(midpoint,n_a1(1)-1),2);
                    a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d3,special_n_d4],n2long,n_a1,n_a2,special_n_semiz,special_n_e, d3_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, semiz_val, e_val, ReturnFnParamsVec,2,0);
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    da1primez=d3ind+N_d3*(a1primeindexesfine-1);
                    entireRHS_ii_e=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,1])+reshape(DiscountedEVinterp_zc(da1primez),[N_d3,n2long,N_a1,N_a2,1]),[N_d3*n2long,N_a1*N_a2,1]);
                    temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                    entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                    entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                    [Vtempii,maxindexL2]=max(entireRHS_ii_e,[],1);
...
%% Component-form Policy (no d1, no z, with e): rows (d2,d3,d4,a1prime_low,L2,L2flag)
