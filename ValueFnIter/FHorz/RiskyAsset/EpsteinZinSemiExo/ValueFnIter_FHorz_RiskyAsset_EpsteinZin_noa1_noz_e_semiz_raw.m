function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_noa1_noz_e_semiz_raw(n_d1,n_d2,n_d3,n_d4,n_a,n_semiz,n_e,n_u,N_j, d1_grid, d2_grid, d3_grid, d4_grid, a_grid, semiz_gridvals_J, e_gridvals_J, u_grid, pi_semiz_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
Policy4=zeros(4,N_a,N_semiz,N_e,N_j,'gpuArray'); % (d1,d2,d3,d4)
...
d1index_ford4_jj=zeros(N_d3,N_a,N_semiz,N_e,N_d4,'gpuArray');
...
if warmglow==1
    ...
    if ~isfield(vfoptions,'V_Jplus1')
        ...
        WGmatrix(becareful)=0;
            ...
            WGmatrix=repmat(WGmatrix,1,1,N_semiz); % the V_Jplus1 lowmemory>=1 branch consumes the full-semiz temp4 at every rung, so the repmat is unconditional (matches the interior loop)
    end
else
    WGmatrix=0;
end
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4], n_a, n_semiz, n_e, d1d3d4_gridvals, a_gridvals, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_d4,N_a,N_semiz,N_e]),[],1);
            ...
            Policy4(1,:,:,:,N_j)=d1index(maxindex+N_d3*N_d4*aind+N_d3*N_d4*N_a*semizind+N_d3*N_d4*N_a*N_semiz*eind); % d1
            Policy4(2,:,:,:,N_j)=d2index(d3index); % d2, note: no a nor semiz in WGmatrix
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4], n_a, n_semiz, special_n_e, d1d3d4_gridvals, a_gridvals, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                V(:,:,e_c,N_j)=Vtemp;
                d3index=rem(maxindex-1,N_d3)+1;
                Policy4(1,:,:,e_c,N_j)=d1index(maxindex+N_d3*N_d4*aind+N_d3*N_d4*N_a*semizind); % d1
                Policy4(2,:,:,e_c,N_j)=d2index(d3index); % d2, note: no a nor semiz in WGmatrix
        ...
        for z_c=1:N_semiz % outer loop over semiz
            ...
            for e_c=1:N_e % inner loop over e
                ...
                if warmglow==1
                    ...
                    Policy4(2,:,z_c,e_c,N_j)=d2index(d3index); % d2, note: no a nor semiz in WGmatrix
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_semiz, n_e, d1_d3_special_d4_gridvals, a_gridvals, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
            ...
            % Time to refine
            % First: ReturnMatrix, we can refine out d1
            [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3,N_a,N_semiz,N_e]),[],1);
            % Second: EV, we can refine out d2
            [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(temp4)).*temp4,[N_d2,N_d3,1,N_semiz,1]),[],1);
        ...
        Policy4(2,:,:,:,N_j)=shiftdim(d2index_ford4_jj(d3_ind+N_d3*semizind+N_d3*N_semiz*shiftdim(maxindex-1,-1)),-1);
    ...
    elseif vfoptions.lowmemory>=1 % terminal lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_d4e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_semiz, special_n_e, d1_d3_special_d4_gridvals, a_gridvals, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            pi_semizd4=pi_semiz(:,:,d4_c);
            d1_d3_special_d4_gridvals=gpuArray(CreateGridvals([n_d1,n_d3,special_n_d4], [d1_grid; d3_grid; d4_gridvals(d4_c,:)'], 1));
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_semiz, n_e, d1_d3_special_d4_gridvals, a_gridvals, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec);
        ...
        d3_ind=reshape(Policy_ford4_jj((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindex_d4-1)),[1,N_a,N_semiz,N_e]);
        Policy4(3,:,:,:,jj)=d3_ind;
        Policy4(2,:,:,:,jj)=shiftdim(d2index_ford4_jj(d3_ind+N_d3*semizind+N_d3*N_semiz*shiftdim(maxindex-1,-1)),-1);
        d1_lookup=d3_ind+N_d3*aind+N_d3*N_a*semizind+N_d3*N_a*N_semiz*eind+N_d3*N_a*N_semiz*N_e*shiftdim(maxindex-1,-1);
    ...
    elseif vfoptions.lowmemory==1
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_d4e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_semiz, special_n_e, d1_d3_special_d4_gridvals, a_gridvals, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec);
        ...
        for d4_c=1:N_d4
            ...
            for z_c=1:N_semiz % outer loop over semiz
                ...
                for e_c=1:N_e % inner loop over e
                    ...
                    % Now put together entireRHS, which just depends on d3
                    entireRHS_ze=shiftdim(ezc1*ezc9*temp2_onlyd3,1)+DiscountedEV_onlyd3;
