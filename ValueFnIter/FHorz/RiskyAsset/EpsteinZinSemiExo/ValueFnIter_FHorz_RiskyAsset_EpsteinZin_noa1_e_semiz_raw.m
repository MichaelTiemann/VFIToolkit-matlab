function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_noa1_e_semiz_raw(n_d1,n_d2,n_d3,n_d4,n_a,n_semiz,n_z,n_e,n_u,N_j, d1_grid, d2_grid, d3_grid, d4_grid, a_grid, semiz_gridvals_J, z_gridvals_J, e_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
Policy4=zeros(4,N_a,N_semiz*N_z,N_e,N_j,'gpuArray'); % (d1,d2,d3,d4)
...
d1d3d4_gridvals=gpuArray(CreateGridvals([n_d1,n_d3,n_d4],[d1_grid;d3_grid;d4_grid],1));
...
d1index_ford4_jj=zeros(N_d3,N_a,N_semiz*N_z,N_e,N_d4,'gpuArray');
...
if warmglow==1
    ...
    if ~isfield(vfoptions,'V_Jplus1')
        ...
        WGmatrix(becareful)=0;
            ...
            WGmatrix=repmat(WGmatrix,1,1,N_bothz); % the V_Jplus1 lowmemory>=1 branch consumes the full-bothz temp4 at every rung, so the repmat is unconditional (matches the interior loop)
    end
else
    WGmatrix=0;
end
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4], n_a, n_bothz, n_e, d1d3d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_d4,N_a,N_bothz,N_e]),[],1);
            % Second: WGmatrix, we can refine out d2
            [WGmatrix_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3]),[],1);
            % Compose the warm-glow INSIDE the ^ezc7 root (the main-loop composition with the EV term absent); just depends on d3 (and d4)
            entireRHS=shiftdim(ezc1*ezc9*temp2_onlyd3+DiscountFactorParamsVec*ezc9*repmat(WGmatrix_onlyd3,1,N_d4),1);
            ...
            Policy4(1,:,:,:,N_j)=d1index(maxindex+N_d3*N_d4*aind+N_d3*N_d4*N_a*bothzind+N_d3*N_d4*N_a*N_bothz*eind); % d1
            Policy4(2,:,:,:,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
            Policy4(3,:,:,:,N_j)=d3index; % d3
            Policy4(4,:,:,:,N_j)=shiftdim(ceil(maxindex/N_d3),-1); % d4
        elseif warmglow==0
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            becareful=logical(isfinite(ReturnMatrix).*(ReturnMatrix~=0)); % finite and not zero
            ...
            V(:,:,:,N_j)=Vtemp;
            d1d3_ind=rem(maxindex-1,N_d1*N_d3)+1;
            Policy4(1,:,:,:,N_j)=shiftdim(rem(d1d3_ind-1,N_d1)+1,-1); % d1
            Policy4(2,:,:,:,N_j)=1; % d2, is meaningless anyway
            Policy4(3,:,:,:,N_j)=shiftdim(ceil(d1d3_ind/N_d1),-1); % d3
            Policy4(4,:,:,:,N_j)=shiftdim(ceil(maxindex/(N_d1*N_d3)),-1); % d4
        end

    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            e_val=e_gridvals_J(e_c,:,N_j);
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4], n_a, n_bothz, special_n_e, d1d3d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                % Time to refine
                % First: ReturnMatrix, we can refine out d1
                [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_d4,N_a,N_bothz]),[],1);
                % Second: WGmatrix, we can refine out d2
                [WGmatrix_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3]),[],1);
                % Compose the warm-glow INSIDE the ^ezc7 root (the main-loop composition with the EV term absent); just depends on d3 (and d4)
                entireRHS=shiftdim(ezc1*ezc9*temp2_onlyd3+DiscountFactorParamsVec*ezc9*repmat(WGmatrix_onlyd3,1,N_d4),1);
                ...
                V(:,:,e_c,N_j)=Vtemp;
                d3index=rem(maxindex-1,N_d3)+1;
                Policy4(1,:,:,e_c,N_j)=d1index(maxindex+N_d3*N_d4*aind+N_d3*N_d4*N_a*bothzind); % d1
                Policy4(2,:,:,e_c,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
                Policy4(3,:,:,e_c,N_j)=d3index; % d3
                Policy4(4,:,:,e_c,N_j)=shiftdim(ceil(maxindex/N_d3),-1); % d4
            elseif warmglow==0
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_e).*(ReturnMatrix_e~=0)); % finite and not zero
                ...
                V(:,:,e_c,N_j)=Vtemp;
                d1d3_ind=rem(maxindex-1,N_d1*N_d3)+1;
                Policy4(1,:,:,e_c,N_j)=shiftdim(rem(d1d3_ind-1,N_d1)+1,-1); % d1
                Policy4(2,:,:,e_c,N_j)=1; % d2, is meaningless anyway
                Policy4(3,:,:,e_c,N_j)=shiftdim(ceil(d1d3_ind/N_d1),-1); % d3
                Policy4(4,:,:,e_c,N_j)=shiftdim(ceil(maxindex/(N_d1*N_d3)),-1); % d4
            end
        end
    ...
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4], n_a, [n_semiz,ones(1,length(n_z))], special_n_e, d1d3d4_gridvals, a_gridvals, bothz_gridvals_J(semizblock,:,N_j), e_val, ReturnFnParamsVec);
                ...
                if warmglow==1
                    ...
                    % Time to refine
                    % First: ReturnMatrix, we can refine out d1
                    [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_d4,N_a,N_semiz]),[],1);
                    % Second: WGmatrix, we can refine out d2
                    [WGmatrix_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3]),[],1);
                    % Compose the warm-glow INSIDE the ^ezc7 root (the main-loop composition with the EV term absent); just depends on d3 (and d4)
                    entireRHS=shiftdim(ezc1*ezc9*temp2_onlyd3+DiscountFactorParamsVec*ezc9*repmat(WGmatrix_onlyd3,1,N_d4),1);
                    ...
                    d3index=rem(maxindex-1,N_d3)+1;
                    Policy4(1,:,semizblock,e_c,N_j)=d1index(maxindex+N_d3*N_d4*aind+N_d3*N_d4*N_a*semizind); % d1
                    Policy4(2,:,semizblock,e_c,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
                    Policy4(3,:,semizblock,e_c,N_j)=d3index; % d3
                    Policy4(4,:,semizblock,e_c,N_j)=shiftdim(ceil(maxindex/N_d3),-1); % d4
                elseif warmglow==0
                    % Modify the Return Function appropriately for Epstein-Zin Preferences
                    becareful=logical(isfinite(ReturnMatrix_ze).*(ReturnMatrix_ze~=0)); % finite and not zero
                    ...
                    V(:,semizblock,e_c,N_j)=Vtemp;
                    d1d3_ind=rem(maxindex-1,N_d1*N_d3)+1;
                    Policy4(1,:,semizblock,e_c,N_j)=shiftdim(rem(d1d3_ind-1,N_d1)+1,-1); % d1
                    Policy4(2,:,semizblock,e_c,N_j)=1; % d2, is meaningless anyway
                    Policy4(3,:,semizblock,e_c,N_j)=shiftdim(ceil(d1d3_ind/N_d1),-1); % d3
                    Policy4(4,:,semizblock,e_c,N_j)=shiftdim(ceil(maxindex/(N_d1*N_d3)),-1); % d4
                end
            end
        end

    elseif vfoptions.lowmemory==3 % outer bothz, inner e
        for e_c=1:N_e
            e_val=e_gridvals_J(e_c,:,N_j);
            for z_c=1:N_bothz
                z_val=bothz_gridvals_J(z_c,:,N_j);
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4], n_a, special_n_bothz, special_n_e, d1d3d4_gridvals, a_gridvals, z_val, e_val, ReturnFnParamsVec);
                ...
                if warmglow==1
                    ...
                    % Time to refine
                    % First: ReturnMatrix, we can refine out d1
                    [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_d4,N_a]),[],1);
                    % Second: WGmatrix, we can refine out d2
                    [WGmatrix_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3]),[],1);
                    % Compose the warm-glow INSIDE the ^ezc7 root (the main-loop composition with the EV term absent); just depends on d3 (and d4)
                    entireRHS=shiftdim(ezc1*ezc9*temp2_onlyd3+DiscountFactorParamsVec*ezc9*repmat(WGmatrix_onlyd3,1,N_d4),1);
                    ...
                    V(:,z_c,e_c,N_j)=Vtemp;
                    d3index=rem(maxindex-1,N_d3)+1;
                    Policy4(1,:,z_c,e_c,N_j)=d1index(maxindex+N_d3*N_d4*aind); % d1
                    Policy4(2,:,z_c,e_c,N_j)=d2index(d3index); % d2, note: no a nor z in WGmatrix
                    Policy4(3,:,z_c,e_c,N_j)=d3index; % d3
                    Policy4(4,:,z_c,e_c,N_j)=shiftdim(ceil(maxindex/N_d3),-1); % d4
                elseif warmglow==0
                    % Modify the Return Function appropriately for Epstein-Zin Preferences
                    becareful=logical(isfinite(ReturnMatrix_ze).*(ReturnMatrix_ze~=0)); % finite and not zero
                    ReturnMatrix_ze(becareful)=(ezc1*ReturnMatrix_ze(becareful).^ezc2(N_j)).^ezc7(N_j); % Otherwise can get things like 0 to negative power equals infinity
                    ...
                    V(:,z_c,e_c,N_j)=Vtemp;
                    d1d3_ind=rem(maxindex-1,N_d1*N_d3)+1;
                    Policy4(1,:,z_c,e_c,N_j)=shiftdim(rem(d1d3_ind-1,N_d1)+1,-1); % d1
                    Policy4(2,:,z_c,e_c,N_j)=1; % d2, is meaningless anyway
                    Policy4(3,:,z_c,e_c,N_j)=shiftdim(ceil(d1d3_ind/N_d1),-1); % d3
                    Policy4(4,:,z_c,e_c,N_j)=shiftdim(ceil(maxindex/(N_d1*N_d3)),-1); % d4
                end
            end
        end
    end
    ...
    % Using V_Jplus1
    V_Jplus1=reshape(vfoptions.V_Jplus1,[N_a,N_bothz,N_e]);    % First, switch V_Jplus1 into Kron form
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_bothz, n_e, d1_d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
            ...
            EV2=EV((aprimeIndex+1)+N_a*((1:1:N_bothz)-1)); % (d,u,z), the upper aprime

            % Apply the aprimeProbs
            EV1=reshape(EV1,[N_d23,N_u,N_bothz]).*aprimeProbs; % probability of lower grid point
            EV2=reshape(EV2,[N_d23,N_u,N_bothz]).*(1-aprimeProbs); % probability of upper grid point
            ...
            % Time to refine
            % First: ReturnMatrix, we can refine out d1
            [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3,N_a,N_bothz,N_e]),[],1);
            % Second: EV, we can refine out d2
            [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(temp4)).*temp4,[N_d2,N_d3,1,N_bothz,1]),[],1);
        ...
        V(:,:,:,N_j)=V_jj;
        Policy4(4,:,:,:,N_j)=maxindex; % d4 is just maxindex
        maxindex_d4=reshape(maxindex,[N_a*N_bothz*N_e,1]);
        d3_ind=reshape(Policy_ford4_jj((1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex_d4-1)),[1,N_a,N_bothz,N_e]);
        Policy4(3,:,:,:,N_j)=d3_ind;
        Policy4(2,:,:,:,N_j)=shiftdim(d2index_ford4_jj(d3_ind+N_d3*bothzind+N_d3*N_bothz*shiftdim(maxindex-1,-1)),-1);
    ...
    elseif vfoptions.lowmemory>=1 % terminal lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for d4_c=1:N_d4
            ...
            [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(temp4)).*temp4,[N_d2,N_d3,1,N_bothz]),[],1);
            DiscountedEV_onlyd3=DiscountFactorParamsVec*ezc9*shiftdim(temp4_onlyd3,1);
            d2index_ford4_jj(:,:,d4_c)=squeeze(d2index);

            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_d4e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_bothz, special_n_e, d1_d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
                ...
                % First: ReturnMatrix, we can refine out d1
                [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3,N_a,N_bothz]),[],1);
                % Now put together entireRHS, which just depends on d3
                entireRHS_e=shiftdim(ezc1*ezc9*temp2_onlyd3,1)+DiscountedEV_onlyd3;
...
for reverse_j=1:N_j-1
    ...
    if warmglow==1
        ...
        WGmatrix=sum((WG1.*pi_u'),2)+sum((WG2.*pi_u'),2); % (d,1), sum over u
        % WGmatrix is over (d,1)
        % Now just make it the right shape (EV is vectorized over bothz in every lowmemory level)
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            d1_d3_special_d4_gridvals=gpuArray(CreateGridvals([n_d1,n_d3,special_n_d4], [d1_grid; d3_grid; d4_gridvals(d4_c,:)'], 1));
            ReturnMatrix_d4=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_bothz, n_e, d1_d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec);
        ...
        [V_jj,maxindex]=max(V_ford4_jj,[],4); % max over d4
        V(:,:,:,jj)=V_jj;
        Policy4(4,:,:,:,jj)=maxindex; % d4 is just maxindex
        maxindex_d4=reshape(maxindex,[N_a*N_bothz*N_e,1]);
        d3_ind=reshape(Policy_ford4_jj((1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex_d4-1)),[1,N_a,N_bothz,N_e]);
        Policy4(3,:,:,:,jj)=d3_ind;
        Policy4(2,:,:,:,jj)=shiftdim(d2index_ford4_jj(d3_ind+N_d3*bothzind+N_d3*N_bothz*shiftdim(maxindex-1,-1)),-1);
        d1_lookup=d3_ind+N_d3*aind+N_d3*N_a*bothzind+N_d3*N_a*N_bothz*eind+N_d3*N_a*N_bothz*N_e*shiftdim(maxindex-1,-1);
    ...
    elseif vfoptions.lowmemory==1
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_d4e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, n_bothz, special_n_e, d1_d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec);
    ...
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z % outer loop over z (markov)
                semizblock=(z_c-1)*N_semiz+(1:1:N_semiz);
                for e_c=1:N_e % inner loop over e
                    e_val=e_gridvals_J(e_c,:,jj);
                    ReturnMatrix_d4ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, [n_semiz,ones(1,length(n_z))], special_n_e, d1_d3_special_d4_gridvals, a_gridvals, bothz_gridvals_J(semizblock,:,jj), e_val, ReturnFnParamsVec);
                    ...
                    % First: ReturnMatrix, we can refine out d1
                    [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3,N_a,N_semiz]),[],1);
                    % Now put together entireRHS, which just depends on d3
                    entireRHS_ze=shiftdim(ezc1*ezc9*temp2_onlyd3,1)+DiscountedEV_onlyd3(:,:,semizblock);
                    ...
                    V_ford4_jj(:,semizblock,e_c,d4_c)=shiftdim(Vtemp,1);
                    Policy_ford4_jj(:,semizblock,e_c,d4_c)=shiftdim(maxindex,1);
                    d1index_ford4_jj(:,:,semizblock,e_c,d4_c)=shiftdim(d1index,1);
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==3 % outer bothz, inner e
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz % outer loop over bothz
                z_val=bothz_gridvals_J(z_c,:,jj);
                for e_c=1:N_e % inner loop over e
                    e_val=e_gridvals_J(e_c,:,jj);
                    ReturnMatrix_d4ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,special_n_d4], n_a, special_n_bothz, special_n_e, d1_d3_special_d4_gridvals, a_gridvals, z_val, e_val, ReturnFnParamsVec);
                    ...
                    % First: ReturnMatrix, we can refine out d1
                    [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3,N_a]),[],1);
                    % Now put together entireRHS, which just depends on d3
                    entireRHS_ze=shiftdim(ezc1*ezc9*temp2_onlyd3,1)+DiscountedEV_onlyd3(:,:,z_c);
...
Policy=Policy4; % rows: d1, d2, d3, d4 (multi-row form, decoded by UnKronPolicyIndexes4 in the dispatcher, mirroring the vNM pairing)
