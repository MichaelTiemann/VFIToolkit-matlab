function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_DC2A_noz_raw(n_d1,n_d2,n_d3,n_a1,n_a2,n_a3,n_u,N_j, d1_grid, d2_grid, d3_grid, a1_grid, a2_grid, a3_grid, u_grid, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAsset_DC1_noz_raw.
...
Policy=zeros(5,N_a,N_j,'gpuArray'); % (1)=d1, (2)=d2, (3)=d3, (4)=a1prime, (5)=a2prime
...
if ~isfield(vfoptions,'V_Jplus1')
    % Layer 1
    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, n_d1, n_d3, n_a2, n_a3, d13_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, ReturnFnParamsVec, 1);
    % [N_d13, N_a1prime, N_a2prime, level1n, N_a2, N_a3]
    [~,maxindex1]=max(ReturnMatrix_ii,[],2);
    [Vtempii,maxindex2]=max(reshape(ReturnMatrix_ii,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            a1primeindexes=loweredge+(0:1:maxgap(ii));              % [N_d13,maxgap+1,N_a2prime,1,N_a2,N_a3]
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, n_d1, n_d3, n_a2, n_a3, d13_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, ReturnFnParamsVec, 2);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, n_d1, n_d3, n_a2, n_a3, d13_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, ReturnFnParamsVec, 2);
            [Vtempii,maxindex]=max(ReturnMatrix_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii,1);
            dind       =rem(maxindex-1,N_d13)+1;
    ...
    Policy(2,:,N_j)=ones(1,N_a,'gpuArray'); % d2 (terminal: d2 doesn't matter since it's only in the expectations term)
    ...
    [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
    curraindex=repmat(level1ii',N_a2*N_a3,1) ...
             +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
    V(curraindex,N_j)=shiftdim(Vtempii,1);
    dind       =rem(maxindex2-1,N_d13)+1;
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii)); % [N_d13,1,N_a2prime,1,N_a2,N_a3]
            a1primeindexes=loweredge+(0:1:maxgap(ii));              % [N_d13,maxgap+1,N_a2prime,1,N_a2,N_a3]
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, n_d1, n_d3, n_a2, n_a3, d13_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, ReturnFnParamsVec, 3);
            d3aprime=d3col + N_d3*(a1primeindexes-1) + N_d3*N_a1*a2pcol;
            entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprime),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            ...
            loweredge=maxindex1(:,1,:,ii,:,:);
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, n_d1, n_d3, n_a2, n_a3, d13_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, ReturnFnParamsVec, 3);
            d3aprime=d3col + N_d3*(loweredge-1) + N_d3*N_a1*a2pcol;
            entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprime),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii,1);
            dind       =rem(maxindex-1,N_d13)+1;
            a2primepart=floor((maxindex-1)/N_d13)+1;
