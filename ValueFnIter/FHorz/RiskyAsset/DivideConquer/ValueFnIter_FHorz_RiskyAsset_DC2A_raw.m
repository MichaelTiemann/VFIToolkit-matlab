function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_DC2A_raw(n_d1,n_d2,n_d3,n_a1,n_a2,n_a3,n_z,n_u,N_j, d1_grid, d2_grid, d3_grid, a1_grid, a2_grid, a3_grid, z_gridvals_J, u_grid, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAsset_DC1_raw.
...
Policy=zeros(5,N_a,N_z,N_j,'gpuArray'); % (1)=d1, (2)=d2, (3)=d3, (4)=a1prime, (5)=a2prime
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        [Vtempii,maxindex2]=max(reshape(ReturnMatrix_ii,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z]),[],1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                a1primeindexes=loweredge+(0:1:maxgap(ii));                % [N_d13,maxgap+1,N_a2prime,1,N_a2,N_a3,N_z]
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, n_z, d13_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2);
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, n_z, d13_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 2);
                [Vtempii,maxindex]=max(ReturnMatrix_ii,[],1);
                V(curraindex,:,N_j)=shiftdim(Vtempii,1);
                dind       =rem(maxindex-1,N_d13)+1;
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            [Vtempii,maxindex2]=max(reshape(ReturnMatrix_ii_z,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
            ...
            Policy(5,curraindex,z_c,N_j)=a2primepart;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, special_n_z, d13_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 2);
                    ...
                    loweredge=maxindex1(:,1,:,ii,:,:);
                    ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, special_n_z, d13_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 2);
                    [Vtempii,maxindex]=max(ReturnMatrix_ii_z,[],1);
                    V(curraindex,z_c,N_j)=shiftdim(Vtempii,1);
                    dind       =rem(maxindex-1,N_d13)+1;
                    a2primepart=floor((maxindex-1)/N_d13)+1;
                    ...
                    Policy(1,curraindex,z_c,N_j)=d1part;
                    Policy(3,curraindex,z_c,N_j)=d3part;
                    Policy(4,curraindex,z_c,N_j)=loweredge(loweredge_idx);
                    Policy(5,curraindex,z_c,N_j)=a2primepart;
                end
            end
        end
    ...
    if vfoptions.lowmemory==0
        ...
        [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z]),[],1);
        curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                 +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
        V(curraindex,:,N_j)=shiftdim(Vtempii,1);
        dind       =rem(maxindex2-1,N_d13)+1;
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            % Layer 1
            ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, special_n_z, d13_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 1);
            entireRHS_ii_z=ReturnMatrix_ii_z+repelem(DiscountedEV_z,N_d1,1,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii_z=reshape(ReturnMatrix_ii_z+DiscountedEV_z(d3aprime),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                    ...
                    loweredge=maxindex1(:,1,:,ii,:,:);
                    ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d3, n_a2, n_a3, special_n_z, d13_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                    d3aprime=d3col + N_d3*(loweredge-1) + N_d3*N_a1*a2pcol;
                    entireRHS_ii_z=reshape(ReturnMatrix_ii_z+DiscountedEV_z(d3aprime),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                    [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
                    V(curraindex,z_c,N_j)=shiftdim(Vtempii,1);
                    dind       =rem(maxindex-1,N_d13)+1;
                    a2primepart=floor((maxindex-1)/N_d13)+1;
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            Policy(5,curraindex,z_c,jj)=a2primepart;
            % Get the d2Policy
            lin=d3part+N_d3*(a1primepart-1)+N_d3*N_a1*(a2primepart-1);
