function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_nod1_noz_raw(n_d2,n_d3,n_a1,n_a2,n_u, N_j, d2_grid, d3_grid, a1_grid,a2_grid, u_grid, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if warmglow==1
        ...
        Policy(1,:,N_j)=shiftdim(d2index(maxindex),1); % d2, note: no a in WGmatrix
    elseif warmglow==0
        % Modify the Return Function appropriately for Epstein-Zin Preferences
        becareful=logical(isfinite(ReturnMatrix).*(ReturnMatrix~=0)); % finite and not zero
