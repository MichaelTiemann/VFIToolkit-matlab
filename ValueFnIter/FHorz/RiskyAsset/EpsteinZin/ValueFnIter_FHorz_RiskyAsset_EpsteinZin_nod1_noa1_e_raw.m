function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_nod1_noa1_e_raw(n_d2,n_d3,n_a2,n_z,n_e,n_u,N_j, d2_grid, d3_grid, a2_grid, z_gridvals_J, e_gridvals_J, u_grid, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
Policy=zeros(2,N_a2,N_z,N_e,N_j,'gpuArray'); % d2,d3
...
if warmglow==1
    ...
    if ~isfield(vfoptions,'V_Jplus1')
        ...
        % terminal condition of the recursion, composed like any continuation value), so build
        % the temp4-analogue of the main loop (with no EV term):
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, n_d3, n_a2, n_z, n_e, d3_grid, a2_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            % Time to refine (warm-glow composes inside the ^ezc7 root: mirror the main loop with WGmatrix in place of temp4)
            % First: ReturnMatrix, we can refine out d1
            % no d1 here
            % Second: WGmatrix, we can refine out d2
            [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3]),[],1);
            ...
            Policy(1,:,:,:,N_j)=shiftdim(d2index(maxindex),1); % note: no a nor z in WGmatrix
        else
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            becareful=logical(isfinite(ReturnMatrix).*(ReturnMatrix~=0)); % finite but not zero
            ...
            V(:,:,:,N_j)=Vtemp;
            Policy(1,:,:,:,N_j)=1; % is anyway meaningless
            Policy(2,:,:,:,N_j)=maxindex;
        end
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, n_d3, n_a2, n_z, special_n_e, d3_grid, a2_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3]),[],1);
                % Now put together entireRHS, which just depends on d3
                entireRHS_e=ezc1*temp2+shiftdim(DiscountFactorParamsVec*ezc9*temp4_onlyd3,1);
                ...
                V(:,:,e_c,N_j)=Vtemp;
                Policy(1,:,:,e_c,N_j)=1; % is anyway meaningless
                Policy(2,:,:,e_c,N_j)=shiftdim(maxindex,1);
            end
        end
    ...
    elseif vfoptions.lowmemory==2
        ...
        for e_c=1:N_e
            ...
            for z_c=1:N_z
                ...
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, n_d3, n_a2, special_n_z, special_n_e, d3_grid, a2_grid, z_val, e_val, ReturnFnParamsVec);
                ...
                if warmglow==1
                    ...
                    [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3]),[],1);
                    % Now put together entireRHS, which just depends on d3
                    entireRHS_ze=ezc1*temp2+shiftdim(DiscountFactorParamsVec*ezc9*temp4_onlyd3,1);
                    ...
                    V(:,z_c,e_c,N_j)=Vtemp;
                    Policy(2,:,z_c,e_c,N_j)=shiftdim(maxindex,1);
                    ...
                    V(:,z_c,e_c,N_j)=Vtemp;
                    Policy(1,:,z_c,e_c,N_j)=1; % is anyway meaningless
                    Policy(2,:,z_c,e_c,N_j)=maxindex;
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            Policy(2,:,:,e_c,N_j)=shiftdim(maxindex,1);
            Policy(1,:,:,e_c,N_j)=shiftdim(d2index(maxindex+N_d3*zind),1);
        end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                V(:,z_c,e_c,N_j)=Vtemp;
                Policy(2,:,z_c,e_c,N_j)=shiftdim(maxindex,1);
                Policy(1,:,z_c,e_c,N_j)=shiftdim(d2index(maxindex),1);
            end
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, n_d3, n_a2, n_z, n_e, d3_grid, a2_grid, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,aprime,a,z,e)
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, n_d3, n_a2, n_z, special_n_e, d3_grid, a2_grid, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec);
            % (d,aprime,a,z)
            ...
            Policy(1,:,:,e_c,jj)=shiftdim(d2index(maxindex+N_d3*zind),1);
        end
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            % Time to refine
            % Second (out of order): EV, we can refine out d2
            [temp4_onlyd3,d2index]=max(ezc3*ezc9*reshape((~isinf(temp4)).*temp4,[N_d2,N_d3]),[],1);
            ...
            for e_c=1:N_e
                ...
                [Vtemp,maxindex]=max(entireRHS_ze,[],1);
                V(:,z_c,e_c,jj)=Vtemp;
                Policy(2,:,z_c,e_c,jj)=shiftdim(maxindex,1);
                Policy(1,:,z_c,e_c,jj)=shiftdim(d2index(maxindex),1);
            end
        end
    end
