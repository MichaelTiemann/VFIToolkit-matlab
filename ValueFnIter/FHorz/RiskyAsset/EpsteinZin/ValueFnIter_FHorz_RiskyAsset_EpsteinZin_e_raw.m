...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d13,n_a1], [n_a1,n_a2], n_z, n_e, [d13_grid; a1_grid], [a1_grid; a2_grid], z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            [ReturnMatrix_onlyd3,d1index]=max(ezc9*reshape((~isinf(ReturnMatrix)).*ReturnMatrix,[N_d1,N_d3*N_a1,N_a,N_z,N_e]),[],1);
        ...
        elseif warmglow==0
            ...
            Policy(1,:,:,:,N_j)=rem(dindex-1,N_d1)+1;
            Policy(2,:,:,:,N_j)=1; % d2, meaningless
            Policy(3,:,:,:,N_j)=ceil(dindex/N_d1);
            Policy(4,:,:,:,N_j)=ceil(maxindex/N_d13);
        end
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d13,n_a1], [n_a1,n_a2], n_z, special_n_e, [d13_grid; a1_grid], [a1_grid; a2_grid], z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                % Time to refine
                % First: ReturnMatrix, we can refine out d1
                [ReturnMatrix_onlyd3,d1index]=max(ezc9*reshape((~isinf(ReturnMatrix_e)).*ReturnMatrix_e,[N_d1,N_d3*N_a1,N_a,N_z]),[],1);
                % Second: EV, we can refine out d2
                [WGmatrix_onlyd3,d2index]=max(ezc9*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3*N_a1]),[],1);
                % Now put together entireRHS, which just depends on d3
                entireRHS=shiftdim(ezc9*ReturnMatrix_onlyd3+ezc9*WGmatrix_onlyd3,1);
                ...
                Policy(2,:,:,e_c,N_j)=shiftdim(d2index(maxindex),1); % note: no a nor z in WGmatrix
            elseif warmglow==0
                %Calc the max and it's index
                ...
                V(:,:,e_c,N_j)=Vtemp;
                dindex=rem(maxindex-1,N_d13)+1;
                Policy(1,:,:,e_c,N_j)=rem(dindex-1,N_d1)+1;
                Policy(2,:,:,e_c,N_j)=1; % d2, meaningless
                Policy(3,:,:,e_c,N_j)=ceil(dindex/N_d1);
                Policy(4,:,:,e_c,N_j)=ceil(maxindex/N_d13);
            end
        end
    ...
    elseif vfoptions.lowmemory==2
        ...
        for e_c=1:N_e
            ...
            for z_c=1:N_z
                ...
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d13,n_a1], [n_a1,n_a2], special_n_z, special_n_e, [d13_grid; a1_grid], [a1_grid; a2_grid], z_val, e_val, ReturnFnParamsVec);
                ...
                if warmglow==1
                    % Time to refine
                    % First: ReturnMatrix, we can refine out d1
                    [ReturnMatrix_onlyd3,d1index]=max(ezc9*reshape((~isinf(ReturnMatrix_ze)).*ReturnMatrix_ze,[N_d1,N_d3*N_a1,N_a]),[],1);
                    % Second: EV, we can refine out d2
                    [WGmatrix_onlyd3,d2index]=max(ezc9*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3*N_a1]),[],1);
                    % Now put together entireRHS, which just depends on d3
                    entireRHS=shiftdim(ezc9*ReturnMatrix_onlyd3+ezc9*WGmatrix_onlyd3,1);
                    ...
                    Policy(2,:,z_c,e_c,N_j)=shiftdim(d2index(maxindex),1); % note: no a nor z in WGmatrix
                elseif warmglow==0
                    %Calc the max and it's index
                    ...
                    V(:,z_c,e_c,N_j)=Vtemp;
                    dindex=rem(maxindex-1,N_d13)+1;
                    Policy(1,:,z_c,e_c,N_j)=rem(dindex-1,N_d1)+1;
                    Policy(2,:,z_c,e_c,N_j)=1; % d2, meaningless
                    Policy(3,:,z_c,e_c,N_j)=ceil(dindex/N_d1);
                    Policy(4,:,z_c,e_c,N_j)=ceil(maxindex/N_d13);
                end
            end
        end
    ...
    if vfoptions.lowmemory==0
        ...
        % Time to refine
        % First: ReturnMatrix, we can refine out d1
        [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_a1,N_a,N_z,N_e]),[],1);
        % Second: EV, we can refine out d2
        ...
        Policy(2,:,:,:,N_j)=shiftdim(d2index(maxindex+N_d3*N_a1*zind),1); % note: no a in temp4
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            % entireRHS_e=ezc1*temp2+ezc3*DiscountFactorParamsVec*temp4;
            ...
            Policy(2,:,:,e_c,N_j)=shiftdim(d2index(maxindex+N_d3*N_a1*zind),1); % note: no a in temp4
        end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy(2,:,z_c,e_c,N_j)=shiftdim(d2index(maxindex),1); % note: no a in temp4
            end
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d13,n_a1], [n_a1,n_a2], n_z, n_e, [d13_grid; a1_grid], [a1_grid; a2_grid], z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,aprime,a,z,e)
        ...
        Policy(2,:,:,:,jj)=shiftdim(d2index(maxindex+N_d3*N_a1*zind),1); % note: no a in temp4
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d13,n_a1], [n_a1,n_a2], n_z, special_n_e, [d13_grid; a1_grid], [a1_grid; a2_grid], z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec);
            % (d,aprime,a,z)
            ...
            Policy(2,:,:,e_c,jj)=shiftdim(d2index(maxindex+N_d3*N_a1*zind),1); % note: no a in temp4
        end
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy(2,:,z_c,e_c,jj)=shiftdim(d2index(maxindex),1); % note: no a in temp4
            end
        end
    end
