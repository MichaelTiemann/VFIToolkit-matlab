...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if warmglow==1
        ...
        Policy(1,:,N_j)=shiftdim(d1index(maxindex+N_d3*N_a1*aind),1); % d1
        Policy(2,:,N_j)=shiftdim(d2index(maxindex),1); % d2, note: no a in WGmatrix
    elseif warmglow==0
        % Modify the Return Function appropriately for Epstein-Zin Preferences
        becareful=logical(isfinite(ReturnMatrix).*(ReturnMatrix~=0)); % finite and not zero
        ...
        Policy(4,:,N_j)=shiftdim(ceil(maxindex/N_d13),-1);
    end
else
    % Using V_Jplus1
    ...
    Policy(1,:,N_j)=shiftdim(d1index(maxindex+N_d3*N_a1*aind),1);
...
for reverse_j=1:N_j-1
    ...
    Policy(1,:,jj)=shiftdim(d1index(maxindex+N_d3*N_a1*aind),1);
