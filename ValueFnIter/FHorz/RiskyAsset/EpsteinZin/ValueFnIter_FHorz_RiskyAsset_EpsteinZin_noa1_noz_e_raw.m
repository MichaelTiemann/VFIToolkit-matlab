function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_noa1_noz_e_raw(n_d1,n_d2,n_d3,n_a,n_e,n_u, N_j, d1_grid, d2_grid, d3_grid, a_grid, e_gridvals_J, u_grid, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
Policy=zeros(3,N_a,N_e,N_j,'gpuArray'); % three: d1, d2, d3
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d13, n_a,n_e, d13_gridvals, a_gridvals,e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3,N_a,N_e]),[],1);
            % Second: WGmatrix, we can refine out d2
            [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3,1,1]),[],1);
            ...
            Policy(1,:,:,N_j)=shiftdim(d1index(maxindex+N_d3*aind+N_d3*N_a*eind),1); % d1
            Policy(2,:,:,N_j)=shiftdim(d2index(maxindex),1); % d2, note: no a nor e in WGmatrix
        elseif warmglow==0
            % Modify the Return Function appropriately for Epstein-Zin Preferences
            becareful=logical(isfinite(ReturnMatrix).*(ReturnMatrix~=0)); % finite and not zero
            ...
            Policy(2,:,:,N_j)=1; % d2, is meaningless anyway
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d13, n_a, special_n_e, d13_gridvals, a_gridvals, e_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                % Second: WGmatrix, we can refine out d2
                [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3,1]),[],1);
                % Now put together entireRHS, which just depends on d3
                ...
                V(:,e_c,N_j)=Vtemp;
                Policy(3,:,e_c,N_j)=shiftdim(maxindex,1); % d3
                Policy(1,:,e_c,N_j)=shiftdim(d1index(maxindex+N_d3*aind),1); % d1
                Policy(2,:,e_c,N_j)=shiftdim(d2index(maxindex),1); % d2
            elseif warmglow==0
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_e).*(ReturnMatrix_e~=0)); % finite and not zero
                ...
                V(:,e_c,N_j)=Vtemp;
                Policy(1,:,e_c,N_j)=shiftdim(rem(maxindex-1,N_d1)+1,-1);
                Policy(2,:,e_c,N_j)=1; % d2, is meaningless anyway
                Policy(3,:,e_c,N_j)=shiftdim(ceil(maxindex/N_d1),-1);
            end
        end
    end
else
    ...
    % Expectation over u (using pi_u), and then add the lower and upper
    EV=sum((EV1.*pi_u'),2)+sum((EV2.*pi_u'),2); % (d,1), sum over u
    % EV is over (d,1)
    ...
    if vfoptions.lowmemory==0
        ...
        [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(temp4)).*temp4,[N_d2,N_d3,1,1]),[],1);
        % Now put together entireRHS, which just depends on d3
        entireRHS=shiftdim(ezc1*ezc9*temp2_onlyd3+DiscountFactorParamsVec*ezc9*temp4_onlyd3,1);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
       ...
       for e_c=1:N_e
           ...
           [Vtemp,maxindex]=max(entireRHS_e,[],1);
           V(:,e_c,N_j)=Vtemp;
           Policy(3,:,e_c,N_j)=shiftdim(maxindex,1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d13, n_a, n_e, d13_gridvals, a_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,a,e)
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
       ...
       for e_c=1:N_e
           ...
           [Vtemp,maxindex]=max(entireRHS_e,[],1);
           V(:,e_c,jj)=Vtemp;
           Policy(3,:,e_c,jj)=shiftdim(maxindex,1);
