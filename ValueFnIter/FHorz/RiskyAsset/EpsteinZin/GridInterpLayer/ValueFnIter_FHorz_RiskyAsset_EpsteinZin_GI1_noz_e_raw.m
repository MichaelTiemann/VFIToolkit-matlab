...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        ...
        WGcol=ezc9*reshape(WGmatrix_onlyd3,[N_d3,1]); % constant in a1prime (and a,e)
    else
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_e])+WGcol_d13,[N_d13*n2long,N_a1*N_a2,N_e]);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for e_c=1:N_e
            ...
            entireRHS_ii=reshape(reshape(ReturnMatrix_ii_e,[N_d13,n2long,N_a1,N_a2])+WGcol_d13,[N_d13*n2long,N_a1*N_a2]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
