function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_GI1_noz_e_raw(n_d1,n_d2,n_d3,n_a1,n_a2,n_e,n_u,N_j, d1_grid, d2_grid, d3_grid, a1_grid, a2_grid, e_gridvals_J, u_grid, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_RiskyAsset_GI1_noz_e_raw.
% V' is transformed ((ezc4*V')^ezc5, masked) BEFORE all the expectations: the
% e-expectation and the u-lottery act on the transformed object, giving one
% joint certainty-equivalent over (u,e'). The interpolation over a1prime acts
...
Policy=zeros(5,N_a,N_e,N_j,'gpuArray'); % (1)=d1, (2)=d2, (3)=d3, (4)=midpoint, (5)=L2ind
...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        ...
        % Discounted warm-glow (the interior's DiscountedEV with the EV term absent); constant in a1prime (and a,e)
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=reshape(reshape(ezc1*temp2_ii,[N_d13,n2long,N_a1,N_a2,N_e])+DiscountedWG_d13,[N_d13*n2long,N_a1*N_a2,N_e]);
        ...
        if warmglow==1
            Policy(2,:,:,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor e in WGmatrix)
        else
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for e_c=1:N_e
            ...
            % Grid interpolation layer
            aprimeindexes=(midpoint_jj+(midpoint_jj-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d3,n2long,n_a1,n_a2,special_n_e, d13_gridvals, a1prime_grid(aprimeindexes), a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,2,0);
            ...
            % d2, which was not in ReturnFn
            if warmglow==1
                Policy(2,:,e_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor e in WGmatrix)
            else
                Policy(2,:,e_c,N_j)=ones(1,N_a,'gpuArray'); % d2 (terminal: d2 doesn't matter, only in expectations)
            end
        end
    end
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_d13(da1prime(:)),[N_d13*n2long,N_a1*N_a2,N_e]);
        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
        entireRHS_ii(entireRHS_ii==0)=-Inf;
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        Policy(2,:,:,N_j)=reshape(d2indexfine_resh(linlookup),[1,N_a,N_e]);
    ...
    elseif vfoptions.lowmemory>=1
        for e_c=1:N_e
            ...
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            da1prime=(1:1:N_d13)'+N_d13*(a1primeindexesfine-1);
            entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_d13(da1prime(:)),[N_d13*n2long,N_a1*N_a2]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        a1fine=(a1mid+(a1mid-1)*n2short)+L2offset-n2short-2; % chosen fine a1prime index
        linlookup=d3_ind+N_d3*(a1fine-1);
        Policy(2,:,:,jj)=reshape(d2indexfine_resh(linlookup),[1,N_a,N_e]);
