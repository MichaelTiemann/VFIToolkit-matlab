function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_GI1_e_raw(n_d1,n_d2,n_d3,n_a1,n_a2,n_z,n_e,n_u,N_j, d1_grid, d2_grid, d3_grid, a1_grid, a2_grid, z_gridvals_J, e_gridvals_J, u_grid, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
% d1: ReturnFn but not aprimeFn
% d2: aprimeFn but not ReturnFn
% d3: both ReturnFn and aprimeFn
% e: iid start-of-period.
%
% Grid-interpolation-layer version of the Epstein-Zin riskyasset solver.
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_RiskyAsset_GI1_e_raw.
% V' is transformed ((ezc4*V')^ezc5, masked) BEFORE all the expectations: the
% e- and z-expectations and the u-lottery act on the transformed object, giving
% one joint certainty-equivalent over (u,z',e'). The interpolation over a1prime
...
% entireRHS before its max in both layers. The warm-glow fn depends only on
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedWG_d13=repelem(DiscountedWG,N_d1,1); % [N_d13,1]
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS=ezc1*temp2+DiscountedWG_d13; % warm-glow (zero if not using); constant in a1prime
        ...
        entireRHS_ii=reshape(reshape(ezc1*temp2_ii,[N_d13,n2long,N_a1,N_a2,N_z,N_e])+DiscountedWG_d13,[N_d13*n2long,N_a1*N_a2,N_z,N_e]);
        ...
        if warmglow==1
            Policy(2,:,:,:,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z nor e in WGmatrix)
        else
            Policy(2,:,:,:,N_j)=ones(1,N_a,N_z,N_e,'gpuArray'); % d2 (terminal: d2 doesn't matter, only in expectations)
        end
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1 composition with the EV term absent)
            entireRHS_e=ezc1*temp2+DiscountedWG_d13; % warm-glow (zero if not using); constant in a1prime
            ...
            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            entireRHS_ii=reshape(reshape(ezc1*temp2_ii,[N_d13,n2long,N_a1,N_a2,N_z])+DiscountedWG_d13,[N_d13*n2long,N_a1*N_a2,N_z]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            % d2, which was not in ReturnFn
            if warmglow==1
                Policy(2,:,:,e_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z nor e in WGmatrix)
            else
                Policy(2,:,:,e_c,N_j)=ones(1,N_a,N_z,'gpuArray'); % d2 (terminal: d2 doesn't matter, only in expectations)
            end
        end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1 composition with the EV term absent)
                entireRHS_ze=ezc1*temp2+DiscountedWG_d13; % warm-glow (zero if not using); constant in a1prime
                ...
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                entireRHS_ii=reshape(reshape(ezc1*temp2_ii,[N_d13,n2long,N_a1,N_a2])+DiscountedWG_d13,[N_d13*n2long,N_a1*N_a2]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                ...
                % d2, which was not in ReturnFn
                if warmglow==1
                    Policy(2,:,z_c,e_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z nor e in WGmatrix)
                else
                    Policy(2,:,z_c,e_c,N_j)=ones(1,N_a,'gpuArray'); % d2 (terminal: d2 doesn't matter, only in expectations)
                end
            end
        end
    end
    ...
    EVpre=sum(temp.*shiftdim(pi_e_J(:,N_j+1),-2),3); % [N_a,N_z]

    % Expectation over zprime (on the transformed object)
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_d13(da1primez(:)),[N_d13*n2long,N_a1*N_a2,N_z,N_e]);
        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
        entireRHS_ii(entireRHS_ii==0)=-Inf;
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        % Get the d2Policy (read off the fine refine at the chosen fine a1prime point; d2indexfine_resh depends on (d3,a1prime,z) — not e)
        ...
        linlookup=d3_ind+N_d3*(a1fine-1)+N_d3*N_a1prime*zlin; % broadcasts to [1,N_a,N_z,N_e]
    ...
    elseif vfoptions.lowmemory>=1
        ...
        for e_c=1:N_e
            ...
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            da1primez=(1:1:N_d13)'+N_d13*(a1primeindexesfine-1)+N_d13*N_a1prime*zind;
            entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_d13(da1primez(:)),[N_d13*n2long,N_a1*N_a2,N_z]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            Policy(2,:,:,e_c,N_j)=d2indexfine_resh(linlookup);
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    % Expectation over eprime (on the transformed object)
    EVnext=sum(temp.*shiftdim(pi_e_J(:,jj+1),-2),3); % [N_a,N_z]
    ...
    elseif vfoptions.lowmemory>=1
        ...
        for e_c=1:N_e
            ...
            zlin=shiftdim(gpuArray(0:N_z-1),-1); % [1,1,N_z]
            linlookup=d3_ind+N_d3*(a1fine-1)+N_d3*N_a1prime*zlin;
            Policy(2,:,:,e_c,jj)=d2indexfine_resh(linlookup);
        end
    end
end
