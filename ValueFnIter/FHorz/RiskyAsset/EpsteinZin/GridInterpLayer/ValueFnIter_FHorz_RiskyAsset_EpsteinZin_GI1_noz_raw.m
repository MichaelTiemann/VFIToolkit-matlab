...
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_RiskyAsset_GI1_noz_raw.
% V' is transformed ((ezc4*V')^ezc5, masked) BEFORE all the expectations: the
% u-lottery acts on the transformed object, giving one joint certainty-equivalent
% over u. The interpolation over a1prime acts on the transformed expectations
% object BEFORE the certainty-equivalent power ^ezc6 (exact collapses under GI);
% the ^ezc6 transform is applied pointwise to both the coarse and fine objects
...
% and the final ^ezc7 wrap each entireRHS before its max in both layers.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    ReturnMatrix=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, n_d1,n_d3,n_a1,n_a1,n_a2, d13_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, ReturnFnParamsVec,1,0);
    ...
    Policy(4,:,N_j)=midpoint_jj(allind);      % midpoint
    Policy(5,:,N_j)=ceil(maxindexL2/N_d13);   % L2ind
    ...
    if warmglow==1
        Policy(2,:,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a in WGmatrix)
    else
        Policy(2,:,N_j)=ones(1,N_a,'gpuArray'); % d2 (terminal: d2 doesn't matter, only in expectations)
    end
    ...
    Policy(5,:,N_j)=ceil(maxindexL2/N_d13);
...
for reverse_j=1:N_j-1
    ...
    Policy(5,:,jj)=ceil(maxindexL2/N_d13);
