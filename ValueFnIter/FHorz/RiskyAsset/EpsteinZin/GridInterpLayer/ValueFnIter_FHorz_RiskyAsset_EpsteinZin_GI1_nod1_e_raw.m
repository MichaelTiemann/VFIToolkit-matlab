function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_GI1_nod1_e_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_e,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, z_gridvals_J, e_gridvals_J, u_grid, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
% d2: aprimeFn but not ReturnFn
% d3: both ReturnFn and aprimeFn
% No d1. e: iid start-of-period.
%
% Grid-interpolation-layer version of the Epstein-Zin riskyasset solver.
% Grafts the Epstein-Zin transforms onto ValueFnIter_FHorz_RiskyAsset_GI1_nod1_e_raw.
...
Policy=zeros(4,N_a,N_z,N_e,N_j,'gpuArray'); % (1)=d2, (2)=d3, (3)=midpoint, (4)=L2ind
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=reshape(reshape(ezc1*temp2_ii,[N_d3,n2long,N_a1,N_a2,N_z,N_e])+DiscountedWG,[N_d3*n2long,N_a1*N_a2,N_z,N_e]);
        ...
        if warmglow==1
            Policy(1,:,:,:,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z nor e in WGmatrix)
        else
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1 composition with the EV term absent)
            entireRHS_e=ezc1*temp2+DiscountedWG; % warm-glow (zero if not using); constant in a1prime
            ...
            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            entireRHS_ii=reshape(reshape(ezc1*temp2_ii,[N_d3,n2long,N_a1,N_a2,N_z])+DiscountedWG,[N_d3*n2long,N_a1*N_a2,N_z]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            % d2, which was not in ReturnFn
            if warmglow==1
                Policy(1,:,:,e_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z nor e in WGmatrix)
            else
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1 composition with the EV term absent)
                entireRHS_ze=ezc1*temp2+DiscountedWG; % warm-glow (zero if not using); constant in a1prime
                ...
                % d2, which was not in ReturnFn
                if warmglow==1
                    Policy(1,:,z_c,e_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z nor e in WGmatrix)
                else
    ...
    EVnext=sum(temp.*shiftdim(pi_e_J(:,N_j+1),-2),3);
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp(da1primez(:)),[N_d3*n2long,N_a1*N_a2,N_z,N_e]);
        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
        entireRHS_ii(entireRHS_ii==0)=-Inf;
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        Policy(1,:,:,:,N_j)=d2indexfine_resh(linlookup);
    ...
    elseif vfoptions.lowmemory>=1
        ...
        for e_c=1:N_e
            ...
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            da1primez=(1:1:N_d3)'+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*zind;
            entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp(da1primez(:)),[N_d3*n2long,N_a1*N_a2,N_z]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            Policy(1,:,:,e_c,N_j)=d2indexfine_resh(linlookup);
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        zlin=shiftdim(gpuArray(0:N_z-1),-1); % [1,1,N_z]
        linlookup=d3_ind+N_d3*(a1fine-1)+N_d3*N_a1prime*zlin; % broadcasts to [1,N_a,N_z,N_e]
        Policy(1,:,:,:,jj)=d2indexfine_resh(linlookup);
    ...
    elseif vfoptions.lowmemory>=1
        ...
        for e_c=1:N_e
            ...
            zlin=shiftdim(gpuArray(0:N_z-1),-1); % [1,1,N_z]
            linlookup=d3_ind+N_d3*(a1fine-1)+N_d3*N_a1prime*zlin;
            Policy(1,:,:,e_c,jj)=d2indexfine_resh(linlookup);
        end
    end
end
