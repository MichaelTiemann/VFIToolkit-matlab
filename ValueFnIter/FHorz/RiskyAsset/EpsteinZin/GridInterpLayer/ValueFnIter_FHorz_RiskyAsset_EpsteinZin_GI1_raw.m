...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        ...
        WGcol=ezc9*reshape(WGmatrix_onlyd3,[N_d3,1]); % constant in a1prime (and a,z)
    else
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            entireRHS_z=ReturnMatrix_z+WGcol_d13; % warm-glow (zero if not using); constant in a1prime
