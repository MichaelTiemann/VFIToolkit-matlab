...
% joint certainty-equivalent over (u,z'). The interpolation over a1prime acts
% on the transformed expectations object BEFORE the certainty-equivalent power
% ^ezc6 (exact collapses under GI); the ^ezc6 transform is applied pointwise to
% both the coarse and fine objects AFTER the interpolation. d2 is refined out
% AFTER the full transform chain using the ezc9 sign trick (coarse and fine
...
% before its max in both layers. The warm-glow fn depends only on a2prime, so
% the u-lottery turns it into a function of d23 (constant in a1prime, hence
...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        ...
        % Discounted warm-glow (the interior's DiscountedEV with the EV term absent); constant in a1prime (and a,z)
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d3,n2long,n_a1,n_a2,n_z, d13_gridvals, a1prime_grid(aprimeindexes), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
        ...
        if warmglow==1
            Policy(2,:,:,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z in WGmatrix)
        else
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1 composition with the EV term absent)
            entireRHS_z=ezc1*temp2+DiscountedWG_d13; % warm-glow (zero if not using); constant in a1prime
            ...
            % d2, which was not in ReturnFn
            if warmglow==1
                Policy(2,:,z_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z in WGmatrix)
            else
                Policy(2,:,z_c,N_j)=ones(1,N_a,'gpuArray'); % d2 (terminal: d2 doesn't matter, only in expectations)
            end
        end
    end
else % V_Jplus1
    ...
    EV=temp.*shiftdim(pi_z_J(:,:,N_j)',-1); % [N_a,N_z,N_z(prime)]
    EV(isnan(EV))=0;
    ...
    if vfoptions.lowmemory==0
        ...
        temp2(becareful)=ReturnMatrix(becareful).^ezc2(N_j);
        temp2(ReturnMatrix==0)=-Inf;
        entireRHS=ezc1*temp2+DiscountedEV_d13; % broadcast over a2 (which is dim 4)
        temp5=logical(isfinite(entireRHS).*(entireRHS~=0));
        entireRHS(temp5)=entireRHS(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
        entireRHS(entireRHS==0)=-Inf;
        [~,maxindex]=max(entireRHS,[],2);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            d2indexfine_z=d2indexfine_resh(:,:,z_c); % [N_d3,N_a1prime]
            ...
            entireRHS_ii=ezc1*temp2_ii+reshape(DiscountedEVinterp_z(da1prime(:)),[N_d13*n2long,N_a1*N_a2]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            Policy(2,:,z_c,N_j)=d2indexfine_z(linlookup);
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    % Expectation over zprime (on the transformed object)
    EV=temp.*shiftdim(pi_z_J(:,:,jj)',-1); % [N_a,N_z,N_z(prime)]
    EV(isnan(EV))=0;
    EV=sum(EV,2);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            a1fine=(a1mid+(a1mid-1)*n2short)+L2offset-n2short-2; % chosen fine a1prime index
            linlookup=d3_ind+N_d3*(a1fine-1);
            Policy(2,:,z_c,jj)=d2indexfine_z(linlookup);
        end
    end
end
