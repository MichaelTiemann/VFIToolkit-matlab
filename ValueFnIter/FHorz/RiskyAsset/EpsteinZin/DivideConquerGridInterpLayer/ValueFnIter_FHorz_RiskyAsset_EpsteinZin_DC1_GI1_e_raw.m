...
% ValueFnIter_FHorz_RiskyAsset_DC1_GI1_e_raw. V' is transformed
...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        ...
        DiscountedWG=DiscountFactorParamsVec*ezc9*reshape(WGmatrix_onlyd3(d3ind),[N_d13,1]);
    else
        DiscountedWG=zeros(N_d13,1,'gpuArray');
    end
    ...
    if vfoptions.lowmemory==0
        ...
        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
        temp2_ii(ReturnMatrix_ii==0)=-Inf;
        entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_z,N_e])+DiscountedWG,[N_d13*n2long,N_a1*N_a2,N_z,N_e]);
        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
        entireRHS_ii(entireRHS_ii==0)=-Inf;
        ...
        allind=d_ind+N_d13*aind+N_d13*N_a*zindB+N_d13*N_a*N_z*eBind;
        ...
        linidx_lower  = d_ind                    + N_d13*n2long*aind + N_d13*n2long*N_a*zindB + N_d13*n2long*N_a*N_z*eBind;
        linidx_upper  = d_ind + N_d13*(n2long-1) + N_d13*n2long*aind + N_d13*n2long*N_a*zindB + N_d13*n2long*N_a*N_z*eBind;
        ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_z,N_e]);
        ...
        if warmglow==1
            Policy(2,:,:,:,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z,e in WGmatrix)
        end
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d13_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0);
                    becareful=logical(isfinite(ReturnMatrix_ii_e).*(ReturnMatrix_ii_e~=0)); % finite but not zero
                    temp2_ii=ReturnMatrix_ii_e;
                    temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
                    temp2_ii(ReturnMatrix_ii_e==0)=-Inf;
            ...
            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_z])+DiscountedWG,[N_d13*n2long,N_a1*N_a2,N_z]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_z]);
            isInfLower    = (ReturnMatrix_ii_resh(linidx_lower) == -Inf);
            isInfUpper    = (ReturnMatrix_ii_resh(linidx_upper) == -Inf);
            inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
            inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
            ...
            % d2, which was not in ReturnFn
            if warmglow==1
                Policy(2,:,:,e_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z,e in WGmatrix)
            end
        end
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                midpoint_jj=zeros(N_d13,1,N_a1,N_a2,1,'gpuArray');
                % Layer 1
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,special_n_z,special_n_e, d13_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_ii_e).*(ReturnMatrix_ii_e~=0)); % finite but not zero
                temp2_ii=ReturnMatrix_ii_e;
                temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_z,special_n_e, d13_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0);
                        becareful=logical(isfinite(ReturnMatrix_ii_e).*(ReturnMatrix_ii_e~=0)); % finite but not zero
                        temp2_ii=ReturnMatrix_ii_e;
                        temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
                        temp2_ii(ReturnMatrix_ii_e==0)=-Inf;
                ...
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,1])+DiscountedWG,[N_d13*n2long,N_a1*N_a2,1]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                ...
                Policy(4,:,z_c,e_c,N_j)=shiftdim(squeeze(midpoint_jj(allind)),-1);
                Policy(5,:,z_c,e_c,N_j)=shiftdim(ceil(maxindexL2/N_d13),-1);
                ...
                % d2, which was not in ReturnFn
                if warmglow==1
                    Policy(2,:,z_c,e_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z,e in WGmatrix)
                end
            end
        end
    end
    ...
    if warmglow==0
        % d2, which was not in ReturnFn
        Policy(2,:,:,:,N_j)=ones(1,N_a,N_z,N_e,'gpuArray'); % d2 (because this is terminal period, choice of d2 is not actually doing anything (as it is only in the expectations term)
    end
    ...
    if vfoptions.lowmemory==0
        % Layer 1
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,n_z,n_e, d13_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
        % [N_d13, level1n, N_a1, N_a2, N_z, N_e]; broadcast DiscountedEV [N_d3,N_a1,1,1,N_z] over d1, a2 and e
        % Modify the Return Function appropriately for Epstein-Zin Preferences
        becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
        temp2_ii=ReturnMatrix_ii;
        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
        temp2_ii(ReturnMatrix_ii==0)=-Inf;
        temp2_ii=reshape(temp2_ii,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2,N_z,N_e]);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*shiftdim(zindB,-2); % [N_d13,maxgap+1,1,N_a2,N_z,N_e]; e broadcasts via the index array
        ...
        entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_z,N_e])+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_z,N_e]),[N_d13*n2long,N_a1*N_a2,N_z,N_e]);
        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
        entireRHS_ii(entireRHS_ii==0)=-Inf;
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        Policy(2,:,:,:,N_j)=shiftdim(d2indexfine_resh(linlookup),-1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii_e==0)=-Inf;
            temp2_ii=reshape(temp2_ii,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2,N_z]);
            ...
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            da1primez=d3ind+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*shiftdim(zindB,-2);
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_z])+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_z]),[N_d13*n2long,N_a1*N_a2,N_z]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            linlookup=d3part+N_d3*(a1fine-1)+N_d3*N_a1prime*(zidx-1);
            Policy(2,:,:,e_c,N_j)=shiftdim(d2indexfine_resh(linlookup),-1);
        end
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            z_val=z_gridvals_J(z_c,:,N_j);
            ...
            for e_c=1:N_e
                ...
                temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii_e==0)=-Inf;
                temp2_ii=reshape(temp2_ii,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2,1]);
                DEV=reshape(DiscountedEV_z,[1,N_d3,N_a1,1,1,1]);
                entireRHS_ii_e=ezc1*temp2_ii+DEV;
                ...
                entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,1])+reshape(DiscountedEVinterp_z(da1primez),[N_d13,n2long,N_a1,N_a2,1]),[N_d13*n2long,N_a1*N_a2,1]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                ...
                Policy(2,:,z_c,e_c,N_j)=shiftdim(d2indexfine_z(linlookup),-1);
            end
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        zidx=repmat(gpuArray(reshape(1:N_z,[1,N_z,1])),N_a,1,N_e);
        linlookup=d3part+N_d3*(a1fine-1)+N_d3*N_a1prime*(zidx-1);
        Policy(2,:,:,:,jj)=shiftdim(d2indexfine_resh(linlookup),-1);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d13_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0);
                    becareful=logical(isfinite(ReturnMatrix_ii_e).*(ReturnMatrix_ii_e~=0)); % finite but not zero
                    temp2_ii=ReturnMatrix_ii_e;
                    temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(jj);
                    temp2_ii(ReturnMatrix_ii_e==0)=-Inf;
            ...
            zidx=repmat(gpuArray(1:N_z),N_a,1);
            linlookup=d3part+N_d3*(a1fine-1)+N_d3*N_a1prime*(zidx-1);
            Policy(2,:,:,e_c,jj)=shiftdim(d2indexfine_resh(linlookup),-1);
        end
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy(4,:,z_c,e_c,jj)=shiftdim(squeeze(midpoint_jj(allind)),-1);
                Policy(5,:,z_c,e_c,jj)=shiftdim(ceil(maxindexL2/N_d13),-1);
                ...
                a1fine=(a1mid+(a1mid-1)*n2short)+L2offset-n2short-2; % chosen fine a1prime index
                linlookup=d3_ind+N_d3*(a1fine-1);
                Policy(2,:,z_c,e_c,jj)=shiftdim(d2indexfine_z(linlookup),-1);
            end
        end
    end
end
