...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        ...
        WGcol=ezc9*reshape(WGmatrix_onlyd3(d3ind),[N_d13,1]); % constant in a1prime (and a,z,e)
    else
        WGcol=zeros(N_d13,1,'gpuArray');
    end
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii(becareful)=(ezc1*ReturnMatrix_ii(becareful).^ezc2(N_j)).^ezc7(N_j);
        ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
        entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_z,N_e])+WGcol,[N_d13*n2long,N_a1*N_a2,N_z,N_e]);
        ...
        ReturnMatrix_ii_resh=reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_z,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_ii(becareful)=(ezc1*ReturnMatrix_ii(becareful).^ezc2(N_j)).^ezc7(N_j);
            ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
            entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_z])+WGcol,[N_d13*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii(becareful)=(ezc1*ReturnMatrix_ii(becareful).^ezc2(N_j)).^ezc7(N_j);
                ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,1])+WGcol,[N_d13*n2long,N_a1*N_a2,1]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
