function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_DC1_GI1_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, z_gridvals_J, u_grid, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
% ValueFnIter_FHorz_RiskyAsset_DC1_GI1_nod1_raw. V' is transformed
% ((ezc4*V')^ezc5, masked) BEFORE all the expectations: the z'-expectation and
% the u-lottery act on the transformed object, giving one joint
% certainty-equivalent over (u,z'). The interpolation over a1prime acts on the
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1 composition with the EV term absent)
            entireRHS_ii_z=ezc1*temp2_ii+DiscountedWG;
            ...
            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2])+DiscountedWG,[N_d3*n2long,N_a1*N_a2]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            Policy(3,:,z_c,N_j)=shiftdim(squeeze(midpoint_jj(allind)),-1);
            Policy(4,:,z_c,N_j)=shiftdim(ceil(maxindexL2/N_d3),-1);
            ...
            % d2, which was not in ReturnFn
            if warmglow==1
                Policy(1,:,z_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z in WGmatrix)
            end
        end
    end
    ...
    if warmglow==0
        % d2, which was not in ReturnFn
        Policy(1,:,:,N_j)=ones(1,N_a,N_z,'gpuArray'); % d2 (because this is terminal period, choice of d2 is not actually doing anything (as it is only in the expectations term)
    end
    ...
    % Get EV in terms of next period endogenous states (z'-expectation on the transformed object)
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii_z=ezc1*temp2_ii+DiscountedEV_z(d3aprime);
                    temp5=logical(isfinite(entireRHS_ii_z).*(entireRHS_ii_z~=0));
                    entireRHS_ii_z(temp5)=entireRHS_ii_z(temp5).^ezc7(N_j);
                    entireRHS_ii_z(entireRHS_ii_z==0)=-Inf;
                    [~,maxindex]=max(entireRHS_ii_z,[],2);
            ...
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2])+reshape(DiscountedEVinterp_z(da1prime),[N_d3,n2long,N_a1,N_a2]),[N_d3*n2long,N_a1*N_a2]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            Policy(1,:,z_c,N_j)=shiftdim(d2indexfine_z(linlookup),-1);
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            Policy(3,:,z_c,jj)=shiftdim(squeeze(midpoint_jj(allind)),-1);
            Policy(4,:,z_c,jj)=shiftdim(ceil(maxindexL2/N_d3),-1);
            ...
            a1fine=(a1mid+(a1mid-1)*n2short)+L2offset-n2short-2; % chosen fine a1prime index
            linlookup=d3_ind+N_d3*(a1fine-1);
            Policy(1,:,z_c,jj)=shiftdim(d2indexfine_z(linlookup),-1);
        end
    end
end
