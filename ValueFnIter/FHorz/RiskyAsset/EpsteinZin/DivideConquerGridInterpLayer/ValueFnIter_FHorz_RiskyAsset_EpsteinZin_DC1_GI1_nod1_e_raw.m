function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_DC1_GI1_nod1_e_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_e,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, z_gridvals_J, e_gridvals_J, u_grid, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
% ValueFnIter_FHorz_RiskyAsset_DC1_GI1_nod1_e_raw. V' is transformed
% ((ezc4*V')^ezc5, masked) BEFORE all the expectations: the e'-expectation, the
% z'-expectation and the u-lottery act on the transformed object, giving one
% joint certainty-equivalent over (u,z',e'). The interpolation over a1prime
...
% entireRHS before its max at every DC level. The warm-glow fn depends only on
% a2prime, so the u-lottery turns it into a function of d23 (constant in
% a1prime, hence identical on coarse and fine grids).
...
if ~isfield(vfoptions,'V_Jplus1')
    if warmglow==1
        ...
        % Discounted warm-glow (the interior's DiscountedEV with the EV term absent); constant in a1prime (and a,z,e)
        DiscountedWG=DiscountFactorParamsVec*ezc9*reshape(WGmatrix_onlyd3(d3ind),[N_d3,1]);
    else
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ezc1*temp2_ii+DiscountedWG; % warm-glow (zero if not using); constant in a1prime so does not affect the layer maxes over a1prime
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                entireRHS_ii=ezc1*temp2_ii+DiscountedWG;
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        ...
        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
        temp2_ii(ReturnMatrix_ii==0)=-Inf;
        entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_z,N_e])+DiscountedWG,[N_d3*n2long,N_a1*N_a2,N_z,N_e]);
        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
        entireRHS_ii(entireRHS_ii==0)=-Inf;
        ...
        allind=d3_ind+N_d3*aind+N_d3*N_a*zindB+N_d3*N_a*N_z*eBind;
        ...
        linidx_lower  = d3_ind                   + N_d3*n2long*aind + N_d3*n2long*N_a*zindB + N_d3*n2long*N_a*N_z*eBind;
        linidx_upper  = d3_ind + N_d3*(n2long-1) + N_d3*n2long*aind + N_d3*n2long*N_a*zindB + N_d3*n2long*N_a*N_z*eBind;
        ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_z,N_e]);
        ...
        if warmglow==1
            Policy(1,:,:,:,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z,e in WGmatrix)
        end
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1 composition with the EV term absent)
            entireRHS_ii_e=ezc1*temp2_ii+DiscountedWG;
            ...
            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_z])+DiscountedWG,[N_d3*n2long,N_a1*N_a2,N_z]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_z]);
            isInfLower    = (ReturnMatrix_ii_resh(linidx_lower) == -Inf);
            isInfUpper    = (ReturnMatrix_ii_resh(linidx_upper) == -Inf);
            inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
            inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
            ...
            % d2, which was not in ReturnFn
            if warmglow==1
                Policy(1,:,:,e_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z,e in WGmatrix)
            end
        end
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                midpoint_jj=zeros(N_d3,1,N_a1,N_a2,1,'gpuArray');
                % Layer 1
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d3,n_a1,vfoptions.level1n,n_a2,special_n_z,special_n_e, d3_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_ii_e).*(ReturnMatrix_ii_e~=0)); % finite but not zero
                temp2_ii=ReturnMatrix_ii_e;
                temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_z,special_n_e, d3_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0);
                        becareful=logical(isfinite(ReturnMatrix_ii_e).*(ReturnMatrix_ii_e~=0)); % finite but not zero
                        temp2_ii=ReturnMatrix_ii_e;
                        temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
                        temp2_ii(ReturnMatrix_ii_e==0)=-Inf;
                ...
                midpoint_jj=max(min(midpoint_jj,n_a1(1)-1),2);
                a1primeindexesfine=(midpoint_jj+(midpoint_jj-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d3,n2long,n_a1,n_a2,special_n_z,special_n_e, d3_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,2,0);
                becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
                temp2_ii=ReturnMatrix_ii;
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,1])+DiscountedWG,[N_d3*n2long,N_a1*N_a2,1]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                ...
                Policy(2,:,z_c,e_c,N_j)=d3_ind; % d3
                Policy(3,:,z_c,e_c,N_j)=shiftdim(squeeze(midpoint_jj(allind)),-1);
                Policy(4,:,z_c,e_c,N_j)=shiftdim(ceil(maxindexL2/N_d3),-1);

                % L2flag
                L2offset      = ceil(maxindexL2/N_d3);
                linidx_lower  = d3_ind                   + N_d3*n2long*aind;
                linidx_upper  = d3_ind + N_d3*(n2long-1) + N_d3*n2long*aind;
                ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,1]);
                isInfLower    = (ReturnMatrix_ii_resh(linidx_lower) == -Inf);
                isInfUpper    = (ReturnMatrix_ii_resh(linidx_upper) == -Inf);
                inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
                inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
                ...
                % d2, which was not in ReturnFn
                if warmglow==1
                    Policy(1,:,z_c,e_c,N_j)=d2index(d3_ind); % d2 comes from refining the warm-glow (no a nor z,e in WGmatrix)
                end
            end
        end
    end
    ...
    if warmglow==0
        % d2, which was not in ReturnFn
        Policy(1,:,:,:,N_j)=ones(1,N_a,N_z,N_e,'gpuArray'); % d2 (because this is terminal period, choice of d2 is not actually doing anything (as it is only in the expectations term)
    end
    ...
    EV=sum(temp.*shiftdim(pi_e_J(:,N_j+1),-2),3); % [N_a,N_z]

    % z'-expectation on the transformed object
    ...
    EVinterp=permute(interp1(a1_gridvals,permute(EV,[2,1,3]),a1prime_grid),[2,1,3]); % [N_d23,N_a1prime,N_z]

    % Certainty-equivalent (and mortality-risk/warm-glow) transform, pointwise
    temp4=EV;
    temp4interp=EVinterp;
    if warmglow==1
        WGmatrixbig=WGmatrix.*ones(1,N_a1,N_z);
        ...
        WGmatrixbigfine=WGmatrix.*ones(1,N_a1prime,N_z);
    ...
    temp4_onlyd3=max(ezc9*ezc3*reshape((~isinf(temp4)).*temp4,[N_d2,N_d3*N_a1,N_z]),[],1);
    [temp4interp_onlyd3,d2indexfine]=max(ezc9*ezc3*reshape((~isinf(temp4interp)).*temp4interp,[N_d2,N_d3*N_a1prime,N_z]),[],1);
    d2indexfine_resh=reshape(d2indexfine,[N_d3,N_a1prime,N_z]);
    ...
    DiscountedEVinterp=DiscountFactorParamsVec*ezc9*reshape(temp4interp_onlyd3,[N_d3,N_a1prime,1,1,N_z]);

    if vfoptions.lowmemory==0
        % Layer 1
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d3,n_a1,vfoptions.level1n,n_a2,n_z,n_e, d3_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);
        % [N_d3, level1n, N_a1, N_a2, N_z, N_e]; broadcast DiscountedEV [N_d3,N_a1,1,1,N_z,1] over a2 and e
        % Modify the Return Function appropriately for Epstein-Zin Preferences
        becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite but not zero
        temp2_ii=ReturnMatrix_ii;
        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
        temp2_ii(ReturnMatrix_ii==0)=-Inf;
        RM=reshape(temp2_ii,[N_d3,N_a1,vfoptions.level1n,N_a2,N_z,N_e]);
        DEV=reshape(DiscountedEV,[N_d3,N_a1,1,1,N_z,1]);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*shiftdim(zindB,-2); % [N_d3,maxgap+1,1,N_a2,N_z,N_e]; e broadcasts via the index array
        ...
        entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_z,N_e])+reshape(DiscountedEVinterp(da1primez),[N_d3,n2long,N_a1,N_a2,N_z,N_e]),[N_d3*n2long,N_a1*N_a2,N_z,N_e]);
        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
        entireRHS_ii(entireRHS_ii==0)=-Inf;
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii_e==0)=-Inf;
            RM=reshape(temp2_ii,[N_d3,N_a1,vfoptions.level1n,N_a2,N_z]);
            DEV=reshape(DiscountedEV,[N_d3,N_a1,1,1,N_z]);
            ...
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            da1primez=d3ind+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*shiftdim(zindB,-2);
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,N_z])+reshape(DiscountedEVinterp(da1primez),[N_d3,n2long,N_a1,N_a2,N_z]),[N_d3*n2long,N_a1*N_a2,N_z]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            d2indexfine_z=d2indexfine_resh(:,:,z_c);
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                midpoint_jj=zeros(N_d3,1,N_a1,N_a2,1,'gpuArray');
                % Layer 1
                ...
                temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii_e==0)=-Inf;
                RM=reshape(temp2_ii,[N_d3,N_a1,vfoptions.level1n,N_a2,1]);
                DEV=reshape(DiscountedEV_z,[N_d3,N_a1,1,1,1]);
                entireRHS_ii_e=ezc1*RM+DEV;
                temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii_e=ezc1*temp2_ii+DiscountedEV_z(d3aprimez);
                        temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                        entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                        entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                        [~,maxindex]=max(entireRHS_ii_e,[],2);
                ...
                entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d3,n2long,N_a1,N_a2,1])+reshape(DiscountedEVinterp_z(da1primez),[N_d3,n2long,N_a1,N_a2,1]),[N_d3*n2long,N_a1*N_a2,1]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                ...
                Policy(1,:,z_c,e_c,N_j)=shiftdim(d2indexfine_z(linlookup),-1);
            end
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    % e'-expectation on the transformed object
    EV=sum(temp.*shiftdim(pi_e_J(:,jj+1),-2),3); % [N_a,N_z]
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d3_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0);
                    becareful=logical(isfinite(ReturnMatrix_ii_e).*(ReturnMatrix_ii_e~=0)); % finite but not zero
                    temp2_ii=ReturnMatrix_ii_e;
                    temp2_ii(becareful)=ReturnMatrix_ii_e(becareful).^ezc2(jj);
                    temp2_ii(ReturnMatrix_ii_e==0)=-Inf;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy(3,:,z_c,e_c,jj)=shiftdim(squeeze(midpoint_jj(allind)),-1);
                Policy(4,:,z_c,e_c,jj)=shiftdim(ceil(maxindexL2/N_d3),-1);
                ...
                a1fine=(a1mid+(a1mid-1)*n2short)+L2offset-n2short-2; % chosen fine a1prime index
                linlookup=d3_ind+N_d3*(a1fine-1);
                Policy(1,:,z_c,e_c,jj)=shiftdim(d2indexfine_z(linlookup),-1);
            end
        end
    end
end
