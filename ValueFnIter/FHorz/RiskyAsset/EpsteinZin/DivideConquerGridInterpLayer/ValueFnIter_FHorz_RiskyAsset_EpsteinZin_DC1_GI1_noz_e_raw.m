...
% ValueFnIter_FHorz_RiskyAsset_DC1_GI1_noz_e_raw. V' is transformed
...
Policy=zeros(5,N_a,N_e,N_j,'gpuArray'); % (1)=d1, (2)=d2, (3)=d3, (4)=a1prime midpoint, (5)=a1primeL2ind
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
        temp2_ii(ReturnMatrix_ii==0)=-Inf;
        entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_e])+DiscountedWG,[N_d13*n2long,N_a1*N_a2,N_e]);
        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
        entireRHS_ii(entireRHS_ii==0)=-Inf;
        ...
        ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_e]);
        ...
        if warmglow==1
            d3part=rem(ceil(d3_ind/N_d1)-1,N_d3)+1; % [1,N_a,N_e]
            Policy(2,:,:,N_j)=d2index(d3part); % d2 comes from refining the warm-glow (no a nor e in WGmatrix)
        end
    ...
    elseif vfoptions.lowmemory>=1
        ...
        for e_c=1:N_e
            ...
            if warmglow==1
                ...
                Policy(2,:,e_c,N_j)=d2index(d3part); % d2 comes from refining the warm-glow (no a nor e in WGmatrix)
            end
        end
    end
    ...
    if vfoptions.lowmemory==0
        ...
        temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
        temp2_ii(ReturnMatrix_ii==0)=-Inf;
        temp2_ii=reshape(temp2_ii,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2,N_e]);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                d3aprime=d3ind+N_d3*(a1primeindexes-1); % [N_d13,maxgap+1,1,N_a2,N_e]; broadcasts across e via the index array
        ...
        entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2,N_e])+reshape(DiscountedEVinterp(da1prime),[N_d13,n2long,N_a1,N_a2,N_e]),[N_d13*n2long,N_a1*N_a2,N_e]);
        temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
        entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
        entireRHS_ii(entireRHS_ii==0)=-Inf;
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory>=1
        ...
        for e_c=1:N_e
            ...
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2])+reshape(DiscountedEVinterp(da1prime),[N_d13,n2long,N_a1,N_a2]),[N_d13*n2long,N_a1*N_a2]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
