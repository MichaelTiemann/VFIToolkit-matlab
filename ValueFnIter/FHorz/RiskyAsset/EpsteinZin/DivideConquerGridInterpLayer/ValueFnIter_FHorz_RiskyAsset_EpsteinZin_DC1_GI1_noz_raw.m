...
% ValueFnIter_FHorz_RiskyAsset_DC1_GI1_noz_raw. V' is transformed
% ((ezc4*V')^ezc5, masked) BEFORE all the expectations: the u-lottery acts on
% the transformed object, giving one joint certainty-equivalent over u. The
% interpolation over a1prime acts on the transformed expectations object
% BEFORE the certainty-equivalent power ^ezc6 (exact collapses under GI); the
% ^ezc6 transform is applied pointwise to both the coarse and fine objects
% AFTER the interpolation. d2 is refined out AFTER the full transform chain
% using the ezc9 sign trick (coarse and fine separately; the d2 policy is read
% off the fine refine at the chosen fine a1prime point). The return transform
% and the final ^ezc7 wrap each entireRHS before its max at every DC level.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if warmglow==1
        ...
        Policy(2,:,N_j)=d2index(d3part); % d2 comes from refining the warm-glow (no a nor z in WGmatrix)
    else
    ...
    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
    temp2_ii(ReturnMatrix_ii==0)=-Inf;
    d3aprime=d3ind+N_d3*((1:1:N_a1)-1); % [N_d13,N_a1]
    entireRHS_ii=ezc1*temp2_ii+DiscountedEV(d3aprime); % [N_d13,N_a1(a1prime),level1n,N_a2]
    temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
    entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);  % matlab otherwise puts 0 to negative power to infinity
    entireRHS_ii(entireRHS_ii==0)=-Inf;
