...
% ValueFnIter_FHorz_RiskyAsset_DC1_GI1_raw. V' is transformed
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii==0)=-Inf;
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2])+DiscountedWG,[N_d13*n2long,N_a1*N_a2]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            ReturnMatrix_ii_resh=reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2]);
            isInfLower    = (ReturnMatrix_ii_resh(linidx_lower) == -Inf);
            isInfUpper    = (ReturnMatrix_ii_resh(linidx_upper) == -Inf);
            inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
            inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            temp2_ii(becareful)=ReturnMatrix_ii_z(becareful).^ezc2(N_j);
            temp2_ii(ReturnMatrix_ii_z==0)=-Inf;
            temp2_ii=reshape(temp2_ii,[N_d1,N_d3,N_a1,vfoptions.level1n,N_a2]);
            DEV=reshape(DiscountedEV_z,[1,N_d3,N_a1,1,1]);
            entireRHS_ii_z=ezc1*temp2_ii+DEV;
            ...
            entireRHS_ii=reshape(ezc1*reshape(temp2_ii,[N_d13,n2long,N_a1,N_a2])+reshape(DiscountedEVinterp_z(da1prime),[N_d13,n2long,N_a1,N_a2]),[N_d13*n2long,N_a1*N_a2]);
            temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
            entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
            entireRHS_ii(entireRHS_ii==0)=-Inf;
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            Policy(2,:,z_c,N_j)=shiftdim(d2indexfine_z(linlookup),-1);
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            a1fine=(a1mid+(a1mid-1)*n2short)+L2offset-n2short-2; % chosen fine a1prime index
            linlookup=d3_ind+N_d3*(a1fine-1);
            Policy(2,:,z_c,jj)=shiftdim(d2indexfine_z(linlookup),-1);
        end
    end
end
