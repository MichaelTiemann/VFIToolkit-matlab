...
Policy=zeros(2,N_a2,N_z,N_j,'gpuArray'); % d2, d3
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d3, n_a2, n_z, d3_grid, a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            V(:,:,N_j)=Vtemp;
            Policy(1,:,:,N_j)=shiftdim(d2index(maxindex),1); % d2, note: no a nor z in WGmatrix
            ...
            V(:,:,N_j)=Vtemp;
            Policy(1,:,:,N_j)=1; % d2, is anyway meaningless
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        ...
        for z_c=1:N_z
            ...
            if warmglow==1
                ...
                V(:,z_c,N_j)=Vtemp;
                Policy(1,:,z_c,N_j)=shiftdim(d2index(maxindex),1); % d2, note: no a nor z in WGmatrix
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d3, n_a2, n_z, d3_grid, a2_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,aprime,a,z)
