function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_u,N_j, d2_grid, d3_grid, a1_grid,a2_grid, z_gridvals_J, u_grid, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
n_d3a1=[n_d3,n_a1];
d3a1_gridvals=CreateGridvals(n_d3a1,[d3_grid; a1_grid],1);
n_a1a2=[n_a1,n_a2];
a1a2_gridvals=CreateGridvals(n_a1a2,[a1_grid; a2_grid],1);
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0

        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d3a1, n_a1a2, n_z, d3a1_gridvals, a1a2_gridvals, z_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        elseif warmglow==0
            ...
            Policy(2,:,:,N_j)=rem(maxindex-1,N_d3)+1; % d3
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        ...
        for z_c=1:N_z
            ...
            if warmglow==1
                ...
                % no d1 here
                % Second: WGmatrix, we can refine out d2
                [temp4_onlyd3,d2index]=max(ezc9*ezc3*reshape((~isinf(WGmatrix)).*WGmatrix,[N_d2,N_d3*N_a1,1]),[],1);
                % Now put together entireRHS, which just depends on d3
                ...
                Policy(1,:,z_c,N_j)=shiftdim(d2index(maxindex),1); % note: no a nor z in WGmatrix
            elseif warmglow==0
                % Modify the Return Function appropriately for Epstein-Zin Preferences
                becareful=logical(isfinite(ReturnMatrix_z).*(ReturnMatrix_z~=0)); % finite and not zero
                ...
                V(:,z_c,N_j)=Vtemp;
                Policy(1,:,z_c,N_j)=1; % d2, meaningless
                Policy(2,:,z_c,N_j)=rem(maxindex-1,N_d3)+1;
                Policy(3,:,z_c,N_j)=ceil(maxindex/N_d3);
            end
        end
    end
else
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d3a1, n_a1a2, n_z, d3a1_gridvals, a1a2_gridvals, z_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,aprime,a,z)
