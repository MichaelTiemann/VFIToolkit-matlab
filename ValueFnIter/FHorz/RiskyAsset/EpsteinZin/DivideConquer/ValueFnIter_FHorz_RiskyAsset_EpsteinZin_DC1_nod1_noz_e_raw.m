function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_DC1_nod1_noz_e_raw(n_d2,n_d3,n_a1,n_a2,n_e,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, e_gridvals_J, u_grid, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
% d2: aprimeFn but not ReturnFn
% d3: both ReturnFn and aprimeFn
% No d1, no z, with e.
%
% Epstein-Zin graft onto ValueFnIter_FHorz_RiskyAsset_DC1_nod1_noz_e_raw: V' is
% transformed by ^ezc5 (masked) once per age before the expectations (one joint
% certainty-equivalent over (u,e'): the e-expectation and the u-lottery are both
% taken on the transformed object); temp4 (post-certainty-equivalent
% continuation) is refined over d2 using ezc9*max(ezc9*.) and indexed exactly
% where the vNM code indexes DiscountedEV; the ^ezc7 mask wraps each level's
% entireRHS before its max (a monotone transform, so the divide-and-conquer
% monotonicity logic is unaffected).
...
Policy=zeros(3,N_a,N_e,N_j,'gpuArray'); % (1)=d2, (2)=d3, (3)=a1prime
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    % Refine d2 out of the (pre-root) warm-glow term [WGmatrix is zeros if no warm-glow, so d2 policy is just 1]
    ...
    % Discounted warm-glow (the interior's DiscountedEV with the EV term absent; no e in WGmatrix)
    DiscountedWG=DiscountFactorParamsVec*ezc9*reshape(WGmatrix_onlyd3,[N_d3,N_a1,1,1]);
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ezc1*temp2_ii+DiscountedWG; % warm-glow (zero if not using)
        ...
        pol_d3_a1=shiftdim(maxindex2,1); % (level1n*N_a2, N_e)
        ...
        % Get the d2Policy [note: no e in WGmatrix]
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedWG(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_e]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                ...
                allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*eBind;
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d3,1,level1iidiff(ii),n_a2,n_e, d3_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                temp2_ii=ReturnMatrix_ii;
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                d3aprime=d3ind+N_d3*(loweredge-1);
                entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedWG(d3aprime),[N_d3,level1iidiff(ii)*N_a2,N_e]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory>=1
        for e_c=1:N_e
            ...
            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1 composition with the EV term absent)
            entireRHS_ii_e=ezc1*temp2_ii+DiscountedWG; % warm-glow (zero if not using)
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_e, d3_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0);
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprime=d3ind+N_d3*(a1primeindexes-1);
                    entireRHS_ii_e=reshape(ezc1*temp2_ii+DiscountedWG(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                    entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                    entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                    ...
                    loweredge=maxindex1(:,1,ii,:);
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d3,1,level1iidiff(ii),n_a2,special_n_e, d3_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_val, ReturnFnParamsVec,3,0);
                    becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                    temp2_ii=ReturnMatrix_ii;
                    temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(N_j);
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprime=d3ind+N_d3*(loweredge-1);
                    entireRHS_ii_e=reshape(ezc1*temp2_ii+DiscountedWG(d3aprime),[N_d3,level1iidiff(ii)*N_a2]);
                    temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                    entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                    entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
    ...
    EVnext=reshape(vfoptions.V_Jplus1,[N_a,N_e]);
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_ii=ezc1*temp2_ii+DiscountedEV;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                d3aprime=d3ind+N_d3*(a1primeindexes-1);
                entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEV(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_e]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                ...
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
                d3aprime=d3ind+N_d3*(loweredge-1);
                entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEV(d3aprime),[N_d3,level1iidiff(ii)*N_a2,N_e]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory>=1
        for e_c=1:N_e
            ...
            entireRHS_ii_e=ezc1*temp2_ii+DiscountedEV;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprime=d3ind+N_d3*(a1primeindexes-1);
                    entireRHS_ii_e=reshape(ezc1*temp2_ii+DiscountedEV(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                    entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                    entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    ...
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprime=d3ind+N_d3*(loweredge-1);
                    entireRHS_ii_e=reshape(ezc1*temp2_ii+DiscountedEV(d3aprime),[N_d3,level1iidiff(ii)*N_a2]);
                    temp5=logical(isfinite(entireRHS_ii_e).*(entireRHS_ii_e~=0));
                    entireRHS_ii_e(temp5)=entireRHS_ii_e(temp5).^ezc7(N_j);
                    entireRHS_ii_e(entireRHS_ii_e==0)=-Inf;
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                loweredge=maxindex1(:,1,ii,:,:);
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,n_d3,1,level1iidiff(ii),n_a2,n_e, d3_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                becareful=logical(isfinite(ReturnMatrix_ii).*(ReturnMatrix_ii~=0)); % finite and not zero
                temp2_ii=ReturnMatrix_ii;
                temp2_ii(becareful)=ReturnMatrix_ii(becareful).^ezc2(jj);
                temp2_ii(ReturnMatrix_ii==0)=-Inf;
