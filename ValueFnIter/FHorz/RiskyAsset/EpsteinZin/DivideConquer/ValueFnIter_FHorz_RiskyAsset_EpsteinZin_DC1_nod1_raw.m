function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_DC1_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, z_gridvals_J, u_grid, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
% d2: aprimeFn but not ReturnFn
% d3: both ReturnFn and aprimeFn
%
% No d1 variant: ReturnFn depends on (d3,...) only.
%
% Epstein-Zin graft onto ValueFnIter_FHorz_RiskyAsset_DC1_nod1_raw: V' is
% transformed by ^ezc5 (masked) once per age before the expectations; the
% u-lottery is taken on the transformed object; temp4 (post-certainty-equivalent
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    % Discounted warm-glow (the interior's DiscountedEV with the EV term absent; no z in WGmatrix)
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedWG(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                ...
                entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedWG(d3aprime),[N_d3,level1iidiff(ii)*N_a2,N_z]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            % Compose the warm-glow INSIDE the ^ezc7 root (the V_Jplus1 composition with the EV term absent)
            entireRHS_ii_z=ezc1*temp2_ii+DiscountedWG; % warm-glow (zero if not using)
            ...
            % Get the d2Policy
            lin=d3part+N_d3*(a1primepart-1);
            Policy(1,curraindex,z_c,N_j)=d2index_resh(lin);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprime=d3ind+N_d3*(a1primeindexes-1);
                    entireRHS_ii_z=reshape(ezc1*temp2_ii+DiscountedWG(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                    temp5=logical(isfinite(entireRHS_ii_z).*(entireRHS_ii_z~=0));
                    entireRHS_ii_z(temp5)=entireRHS_ii_z(temp5).^ezc7(N_j);
                    entireRHS_ii_z(entireRHS_ii_z==0)=-Inf;
                    ...
                    temp2_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprime=d3ind+N_d3*(loweredge-1);
                    entireRHS_ii_z=reshape(ezc1*temp2_ii+DiscountedWG(d3aprime),[N_d3,level1iidiff(ii)*N_a2]);
                    temp5=logical(isfinite(entireRHS_ii_z).*(entireRHS_ii_z~=0));
                    entireRHS_ii_z(temp5)=entireRHS_ii_z(temp5).^ezc7(N_j);
                    entireRHS_ii_z(entireRHS_ii_z==0)=-Inf;
                    [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                ...
                entireRHS_ii=reshape(ezc1*temp2_ii+DiscountedEV(d3aprimez),[N_d3,level1iidiff(ii)*N_a2,N_z]);
                temp5=logical(isfinite(entireRHS_ii).*(entireRHS_ii~=0));
                entireRHS_ii(temp5)=entireRHS_ii(temp5).^ezc7(N_j);
                entireRHS_ii(entireRHS_ii==0)=-Inf;
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for z_c=1:N_z
            ...
            entireRHS_ii_z=ezc1*temp2_ii+DiscountedEV_z;
