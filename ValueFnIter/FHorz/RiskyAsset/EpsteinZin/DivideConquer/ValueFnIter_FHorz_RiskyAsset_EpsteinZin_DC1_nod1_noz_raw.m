...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii+WGmatrix_onlyd3(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii+WGmatrix_onlyd3(d3aprime),[N_d3,level1iidiff(ii)*N_a2]);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii,1);
            dind=(rem(maxindex-1,N_d3)+1);
            allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii));
