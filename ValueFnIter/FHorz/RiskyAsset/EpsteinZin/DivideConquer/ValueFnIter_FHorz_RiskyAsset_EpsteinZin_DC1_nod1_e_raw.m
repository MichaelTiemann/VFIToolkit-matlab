    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=reshape(ReturnMatrix_ii+WGmatrix_onlyd3(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z,N_e]);
                ...
                entireRHS_ii=reshape(ReturnMatrix_ii+WGmatrix_onlyd3(d3aprime),[N_d3,level1iidiff(ii)*N_a2,N_z,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                dind=(rem(maxindex-1,N_d3)+1);
                allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBind+N_d3*N_a2*N_z*eBind;
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprime=d3ind+N_d3*(a1primeindexes-1);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii+WGmatrix_onlyd3(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                    ...
                    ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                    d3aprime=d3ind+N_d3*(loweredge-1);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii+WGmatrix_onlyd3(d3aprime),[N_d3,level1iidiff(ii)*N_a2,N_z]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    V(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d3)+1);
                    allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBind;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii_ze=ReturnMatrix_ii_ze+WGmatrix_onlyd3; % warm-glow (zero if not using)
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                        d3aprime=d3ind+N_d3*(a1primeindexes-1);
                        entireRHS_ii_ze=reshape(ReturnMatrix_ii+WGmatrix_onlyd3(d3aprime),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                        ...
                        ReturnMatrix_ii(ReturnMatrix_ii==0)=-Inf;
                        d3aprime=d3ind+N_d3*(loweredge-1);
                        entireRHS_ii_ze=reshape(ReturnMatrix_ii+WGmatrix_onlyd3(d3aprime),[N_d3,level1iidiff(ii)*N_a2]);
                        [Vtempii,maxindex]=max(entireRHS_ii_ze,[],1);
                        V(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d3)+1);
                        allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii));
