...
Policy=zeros(4,N_a,N_e,N_j,'gpuArray'); % d1, d2, d3, a1prime
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        if warmglow==1
            ...
            [temp2_onlyd3,d1index]=max(ezc9*reshape((~isinf(temp2)).*temp2,[N_d1,N_d3*N_a1,N_a,N_e]),[],1);
            ...
            Policy(3,:,:,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1); % d3
            Policy(4,:,:,N_j)=shiftdim(ceil(maxindex/N_d3),1); % a1prime
            Policy(1,:,:,N_j)=shiftdim(d1index(maxindex+N_d3*N_a1*aind+N_d3*N_a1*N_a*eind),1); % d1
    ...
    elseif vfoptions.lowmemory>=1 % lm1 already does the most-looped variant, so it also serves the higher lowmemory values
        for e_c=1:N_e
            ...
            if warmglow==1
                ...
                V(:,e_c,N_j)=Vtemp;
                Policy(3,:,e_c,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1); % d3
                Policy(4,:,e_c,N_j)=shiftdim(ceil(maxindex/N_d3),1); % a1prime
                Policy(1,:,e_c,N_j)=shiftdim(d1index(maxindex+N_d3*N_a1*aind),1); % d1
    ...
    % So I will add a check for when this happens, and then overwrite those (by setting aprimeProbs to zero)
    aprimeProbs=repmat(a2primeProbs,N_a1,1);  % [N_d*N_a1,N_u]
    skipinterp=logical(temp(aprimeIndex(:))==temp(aprimeplus1Index(:))); % Note, probably just do this off of a2prime values
    aprimeProbs(skipinterp)=0;

    % Switch EV from being in terms of aprime to being in terms of d (in expectation because of the u shocks)
    EV1=aprimeProbs.*reshape(temp(aprimeIndex),[N_d23*N_a1,N_u]); % (d&a1prime,u), the lower aprime
    EV2=(1-aprimeProbs).*reshape(temp(aprimeplus1Index),[N_d23*N_a1,N_u]); % (d&a1prime,u), the upper aprime
    % Already applied the probabilities from interpolating onto grid

    % Expectation over u (using pi_u), and then add the lower and upper
    EV=sum((EV1.*pi_u'),2)+sum((EV2.*pi_u'),2); % (d&a1prime,1), sum over u
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d13,n_a1], [n_a1,n_a2], n_e, d13a1_gridvals, a12_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,a,e)
