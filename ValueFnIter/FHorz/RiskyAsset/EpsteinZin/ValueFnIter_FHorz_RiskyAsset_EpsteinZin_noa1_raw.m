...
Policy=zeros(3,N_a2,N_z,N_j,'gpuArray'); % d1, d2, d3
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d13, n_a2, n_z, d13_gridvals, a2_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            Policy(1,:,:,N_j)=shiftdim(d1index(maxindex+N_d3*aind+N_d3*N_a2*zind),1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, n_d13, n_a2, n_z, d13_gridvals, a2_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,aprime,a,z)
        ...
        Policy(1,:,:,jj)=shiftdim(d1index(maxindex+N_d3*aind+N_d3*N_a2*zind),1);
