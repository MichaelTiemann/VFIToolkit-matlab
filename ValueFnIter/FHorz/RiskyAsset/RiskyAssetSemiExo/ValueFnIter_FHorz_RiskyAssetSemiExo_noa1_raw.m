function [V,Policy]=ValueFnIter_FHorz_RiskyAssetSemiExo_noa1_raw(n_d1,n_d2,n_d3,n_d4,n_a,n_semiz,n_z,n_u,N_j, d1_grid, d2_grid, d3_grid, d4_grid, a_grid, semiz_gridvals_J, z_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
Policy=zeros(4,N_a,N_semiz*N_z,N_j,'gpuArray'); % d1, d2, d3, d4
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            EV=sum((EV1.*pi_u'),2)+sum((EV2.*pi_u'),2); % (d23,1,bothz)

            [ReturnMatrix_onlyd3,d1index]=max(reshape(ReturnMatrix_d4,[N_d1,N_d3,N_a,N_bothz]),[],1);
    ...
    elseif vfoptions.lowmemory==1 % loop over z (markov), vectorize over semiz
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z
                ...
                [ReturnMatrix_onlyd3,d1index]=max(reshape(ReturnMatrix_d4z,[N_d1,N_d3,N_a,N_semiz]),[],1);
    ...
    elseif vfoptions.lowmemory==2 % loop over bothz (outer semiz, inner z)
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                ...
                [ReturnMatrix_onlyd3,d1index]=max(reshape(ReturnMatrix_d4z,[N_d1,N_d3,N_a]),[],1);
