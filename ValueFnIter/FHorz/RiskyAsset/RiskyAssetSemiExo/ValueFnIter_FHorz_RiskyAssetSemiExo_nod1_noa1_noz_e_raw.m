function [V,Policy]=ValueFnIter_FHorz_RiskyAssetSemiExo_nod1_noa1_noz_e_raw(n_d2,n_d3,n_d4,n_a,n_semiz,n_e,n_u,N_j, d2_grid, d3_grid, d4_grid, a_grid, semiz_gridvals_J, e_gridvals_J, u_grid, pi_semiz_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
Policy=zeros(3,N_a,N_semiz,N_e,N_j,'gpuArray'); % (d2,d3,d4)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        Policy(2,:,:,:,N_j)=d3_ind;
        Policy(1,:,:,:,N_j)=shiftdim(d2index_ford4_jj(d3_ind+N_d3*semizind+N_d3*N_semiz*shiftdim(maxindex-1,-1)),-1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        d3_ind=reshape(Policy_ford4_jj((1:1:N_a*N_semiz*N_e)'+(N_a*N_semiz*N_e)*(maxindex_d4-1)),[1,N_a,N_semiz,N_e]);
        Policy(2,:,:,:,jj)=d3_ind;
        Policy(1,:,:,:,jj)=shiftdim(d2index_ford4_jj(d3_ind+N_d3*semizind+N_d3*N_semiz*shiftdim(maxindex-1,-1)),-1);
    ...
    elseif vfoptions.lowmemory==2
        for d4_c=1:N_d4
            ...
            for z_c=1:N_semiz % outer loop over semiz
                DiscountedEV_onlyd3=shiftdim(DiscountFactorParamsVec*EV_onlyd3(:,:,:,z_c),1);
                ...
                for e_c=1:N_e % inner loop over e
                    ...
                    entireRHS_ze=ReturnMatrix_d4ze+DiscountedEV_onlyd3;
