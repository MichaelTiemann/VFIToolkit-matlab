function [V,Policy]=ValueFnIter_FHorz_RiskyAssetSemiExo_nod1_noz_raw(n_d2,n_d3,n_d4,n_a1,n_a2,n_semiz,n_u,N_j, d2_grid, d3_grid, d4_grid, a1_grid, a2_grid, semiz_gridvals_J, u_grid, pi_semiz_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
Policy=zeros(4,N_a,N_semiz,N_j,vfoption.indexT,'gpuArray'); % d2, d3, d4, a1prime
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_semiz
            ...
            dindex=rem(maxindex-1,N_d3*N_d4)+1;
            Policy(1,:,z_c,N_j)=1;
            Policy(2,:,z_c,N_j)=rem(dindex-1,N_d3)+1;
            Policy(3,:,z_c,N_j)=shiftdim(ceil(dindex/N_d3),-1);
            Policy(4,:,z_c,N_j)=shiftdim(ceil(maxindex/(N_d3*N_d4)),-1);
        end
    end
else
