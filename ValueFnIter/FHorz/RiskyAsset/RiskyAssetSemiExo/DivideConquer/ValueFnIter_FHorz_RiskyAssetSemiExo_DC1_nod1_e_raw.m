        ...
        for d4_c=1:N_d4
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    ...
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3,level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,:,d4_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d3)+1);
                    allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBind+N_d3*N_a2*N_bothz*eBind;
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*shiftdim(zBind,-2);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        ...
                        d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*shiftdim(zBind,-2);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3,level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                        V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d3)+1);
                        allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBind;
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEVblock(d3aprimez),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                            V_ford4_jj(curraindex,semizblock,e_c,d4_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d3)+1);
                            ...
                            entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEVblock(d3aprimez),[N_d3,level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                            V_ford4_jj(curraindex,semizblock,e_c,d4_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d3)+1);
                            allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBindblock;
        ...
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    RM=reshape(ReturnMatrix_ii_ze,[N_d3,N_a1,vfoptions.level1n,N_a2]);
