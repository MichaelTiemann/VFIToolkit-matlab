function [V,Policy]=ValueFnIter_FHorz_RiskyAssetSemiExo_DC2A_nod1_noz_e_raw(n_d2,n_d3,n_d4,n_a1,n_a2,n_a3,n_semiz,n_e,n_u,N_j, d2_grid, d3_grid, d4_grid, a1_grid, a2_grid, a3_grid, semiz_gridvals_J, e_gridvals_J, u_grid, pi_semiz_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAssetSemiExo_DC1_nod1_noz_e_raw.
...
Policy=zeros(5,N_a,N_semiz,N_e,N_j,'gpuArray'); % d2, d3, d4, a1prime, a2prime
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4,n_a1,n_a2], [n_a1,n_a2,n_a3], n_semiz, n_e, d3d4a1a2_gridvals, a1a2a3_gridvals, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4,n_a1,n_a2], [n_a1,n_a2,n_a3], n_semiz, special_n_e, d3d4a1a2_gridvals, a1a2a3_gridvals, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
    ...
    elseif vfoptions.lowmemory>=2 % lm2 already does the most-looped variant, so it also serves the higher lowmemory values
        for e_c=1:N_e
            ...
            for z_c=1:N_semiz
                ...
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d3,n_d4,n_a1,n_a2], [n_a1,n_a2,n_a3], special_n_semiz, special_n_e, d3d4a1a2_gridvals, a1a2a3_gridvals, z_val, e_val, ReturnFnParamsVec);
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,n_e, d3_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1);
            % [N_d3, N_a1prime, N_a2prime, level1n, N_a2, N_a3, N_semiz, N_e]
            entireRHS_ii=ReturnMatrix_ii+DiscountedEV;

            [~,maxindex1]=max(entireRHS_ii,[],2);
            [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d3*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_semiz,N_e]),[],1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii)); % [N_d3,1,N_a2prime,1,N_a2,N_a3,N_semiz,N_e]
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,n_e, d3_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz,N_e]);
                    ...
                    allind=d3part+N_d3*(a2primepart-1)+N_d3*N_a2*a2Bind_flat+N_d3*N_a2*N_a2*a3Bind_flat+N_d3*N_a2*N_a2*N_a3*zBind+N_d3*N_a2*N_a2*N_a3*N_semiz*eBind;
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,n_e, d3_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,:,d4_c)=shiftdim(Vtempii,1);
                    d3part     =rem(maxindex-1,N_d3)+1;
    ...
    elseif vfoptions.lowmemory>=1
        ...
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,ones(1,length(n_e)), d3_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,ones(1,length(n_e)), d3_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                        ...
                        loweredge=maxindex1(:,1,:,ii,:,:,:);
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,ones(1,length(n_e)), d3_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                        V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
                        d3part     =rem(maxindex-1,N_d3)+1;
                        a2primepart=floor((maxindex-1)/N_d3)+1;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,n_e, d3_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,1);
            entireRHS_ii=ReturnMatrix_ii+DiscountedEV;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,n_e, d3_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,:,d4_c)=shiftdim(Vtempii,1);
                    ...
                    loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,n_e, d3_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,:,d4_c)=shiftdim(Vtempii,1);
    ...
    elseif vfoptions.lowmemory>=1
        ...
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,ones(1,length(n_e)), d3_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,1);
                entireRHS_ii_e=ReturnMatrix_ii_e+DiscountedEV;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,:,ii,:,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,ones(1,length(n_e)), d3_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                        V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
                        ...
                        loweredge=maxindex1(:,1,:,ii,:,:,:);
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0,[n_d3,special_n_d4],n_a2,n_a3,n_semiz,ones(1,length(n_e)), d3_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, semiz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                        V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
