function [V,Policy]=ValueFnIter_FHorz_RiskyAssetSemiExo_DC2A_e_raw(n_d1,n_d2,n_d3,n_d4,n_a1,n_a2,n_a3,n_semiz,n_z,n_e,n_u,N_j, d1_grid, d2_grid, d3_grid, d4_grid, a1_grid, a2_grid, a3_grid, semiz_gridvals_J, z_gridvals_J, e_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_e_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAssetSemiExo_DC1_e_raw.
...
Policy=zeros(6,N_a,N_bothz,N_e,N_j,'gpuArray'); % d1, d2, d3, d4, a1prime, a2prime
...
a1a2prime_ford4_jj=zeros(N_a,N_bothz,N_e,N_d4,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4,n_a1,n_a2], [n_a1,n_a2,n_a3], n_bothz, special_n_e, d1d3d4a1a2_gridvals, a1a2a3_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4,n_a1,n_a2], [n_a1,n_a2,n_a3], [n_semiz,ones(1,length(n_z))], special_n_e, d1d3d4a1a2_gridvals, a1a2a3_gridvals, z_valblock, e_val, ReturnFnParamsVec);
                [Vtemp,maxindex]=max(ReturnMatrix_e,[],1);
                ...
                a1a2primepart=ceil(maxindex/(N_d1*N_d3*N_d4));
                Policy(5,:,semizblock,e_c,N_j)=rem(a1a2primepart-1,N_a1)+1; % a1prime
                Policy(6,:,semizblock,e_c,N_j)=floor((a1a2primepart-1)/N_a1)+1; % a2prime
            end
        end
    elseif vfoptions.lowmemory==3
        for e_c=1:N_e
            ...
            for z_c=1:N_bothz
                ...
                ReturnMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(ReturnFn, [n_d1,n_d3,n_d4,n_a1,n_a2], [n_a1,n_a2,n_a3], special_n_bothz, special_n_e, d1d3d4a1a2_gridvals, a1a2a3_gridvals, z_val, e_val, ReturnFnParamsVec);
    ...
    EVnext=sum(V_Jplus1.*shiftdim(pi_e_J(:,N_j+1),-2),3); % [N_a,N_bothz]
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,n_e, d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1);
            RM=reshape(ReturnMatrix_ii,[N_d1,N_d3,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz,N_e]);
            DEV=reshape(DiscountedEV,[1,N_d3,N_a1,N_a2,1,1,1,N_bothz,1]);
            entireRHS_ii=RM+DEV;
            entireRHS_ii=reshape(entireRHS_ii,[N_d13,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz,N_e]);

            [~,maxindex1]=max(entireRHS_ii,[],2);
            [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz,N_e]),[],1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,n_e, d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz,N_e]);
                    ...
                    allind=dind+N_d13*(a2primepart-1)+N_d13*N_a2*a2Bind_flat+N_d13*N_a2*N_a2*a3Bind_flat+N_d13*N_a2*N_a2*N_a3*zBind+N_d13*N_a2*N_a2*N_a3*N_bothz*eBind;
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,n_e, d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,:,d4_c)=shiftdim(Vtempii,1);
                    dind       =rem(maxindex-1,N_d13)+1;
                    d1part     =rem(maxindex-1,N_d1)+1;
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,ones(1,length(n_e)), d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1);
                RM=reshape(ReturnMatrix_ii_e,[N_d1,N_d3,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz]);
                DEV=reshape(DiscountedEV,[1,N_d3,N_a1,N_a2,1,1,1,N_bothz]);
                entireRHS_ii_e=RM+DEV;
                entireRHS_ii_e=reshape(entireRHS_ii_e,[N_d13,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz]);

                [~,maxindex1]=max(entireRHS_ii_e,[],2);
                [Vtempii,maxindex2]=max(reshape(entireRHS_ii_e,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz]),[],1);
                curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                         +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
                ...
                a1primepart=rem(floor((pol_d13_a1a2-1)/N_d13),N_a1)+1;
                a2primepart=floor((pol_d13_a1a2-1)/(N_d13*N_a1))+1;
                zidx=repmat(gpuArray(1:N_bothz),size(pol_d13_a1a2,1),1);
                lin=d3part+N_d3*(a1primepart-1)+N_d3*N_a1*(a2primepart-1)+N_d3*N_a1*N_a2*(zidx-1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,ones(1,length(n_e)), d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                        ...
                        a1primepart=shiftdim(a1primepart,1);
                        a2primepart=shiftdim(a2primepart,1);
                        zidx=repmat(gpuArray(1:N_bothz),size(d3part,1),1);
                        lin=d3part+N_d3*(a1primepart-1)+N_d3*N_a1*(a2primepart-1)+N_d3*N_a1*N_a2*(zidx-1);
                        d1_ford4_jj(curraindex,:,e_c,d4_c)=d1part;
                        d3_ford4_jj(curraindex,:,e_c,d4_c)=d3part;
                        a1a2prime_ford4_jj(curraindex,:,e_c,d4_c)=a1primepart+N_a1*(a2primepart-1);
                        ...
                        loweredge=maxindex1(:,1,:,ii,:,:,:);
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,ones(1,length(n_e)), d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                        V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
                        dind       =rem(maxindex-1,N_d13)+1;
                        d1part     =rem(maxindex-1,N_d1)+1;
    ...
    elseif vfoptions.lowmemory==2
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,[n_semiz,ones(1,length(n_z))],ones(1,length(n_e)), d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_valblock, e_val, ReturnFnParamsVec,1);
                    RM=reshape(ReturnMatrix_ii_e,[N_d1,N_d3,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_semiz]);
                    DEV=reshape(DiscountedEVblock,[1,N_d3,N_a1,N_a2,1,1,1,N_semiz]);
                    entireRHS_ii_e=RM+DEV;
                    ...
                    a1a2prime_ford4_jj(curraindex,semizblock,e_c,d4_c)=a1primepart+N_a1*(a2primepart-1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,:,ii,:,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,[n_semiz,ones(1,length(n_z))],ones(1,length(n_e)), d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_valblock, e_val, ReturnFnParamsVec,3);
                            d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBindblock,-4);
                            entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEVblock(d3aprimez),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                            V_ford4_jj(curraindex,semizblock,e_c,d4_c)=shiftdim(Vtempii,1);
                            dind       =rem(maxindex-1,N_d13)+1;
                            d1part     =rem(maxindex-1,N_d1)+1;
                            ...
                            a1localind =rem(floor((maxindex-1)/N_d13),maxgap(ii)+1)+1;
                            a2primepart=floor((maxindex-1)/(N_d13*(maxgap(ii)+1)))+1;
                            allind=dind+N_d13*(a2primepart-1)+N_d13*N_a2*a2Bind_flat+N_d13*N_a2*N_a2*a3Bind_flat+N_d13*N_a2*N_a2*N_a3*zBindblock;
                            a1primepart=a1localind+loweredge(allind)-1;
                            d1part=shiftdim(d1part,1);
                            d3part=shiftdim(d3part,1);
                            a1primepart=shiftdim(a1primepart,1);
                            ...
                            loweredge=maxindex1(:,1,:,ii,:,:,:);
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,[n_semiz,ones(1,length(n_z))],ones(1,length(n_e)), d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_valblock, e_val, ReturnFnParamsVec,3);
                            d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBindblock,-4);
                            entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEVblock(d3aprimez),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                            V_ford4_jj(curraindex,semizblock,e_c,d4_c)=shiftdim(Vtempii,1);
                            dind       =rem(maxindex-1,N_d13)+1;
                            d1part     =rem(maxindex-1,N_d1)+1;
    ...
    elseif vfoptions.lowmemory==3
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,special_n_bothz,ones(1,length(n_e)), d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec,1);
                    RM=reshape(ReturnMatrix_ii_ze,[N_d1,N_d3,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3]);
                    DEV=reshape(DiscountedEV_z,[1,N_d3,N_a1,N_a2,1,1,1]);
                    entireRHS_ii_ze=RM+DEV;
                    entireRHS_ii_ze=reshape(entireRHS_ii_ze,[N_d13,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3]);
                    ...
                    a1a2prime_ford4_jj(curraindex,z_c,e_c,d4_c)=a1primepart+N_a1*(a2primepart-1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,special_n_bothz,ones(1,length(n_e)), d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec,3);
                            d3aprime=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol;
                            entireRHS_ii_ze=reshape(ReturnMatrix_ii+DiscountedEV_z(d3aprime),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                            ...
                            a1localind =rem(floor((maxindex-1)/N_d13),maxgap(ii)+1)+1;
                            a2primepart=floor((maxindex-1)/(N_d13*(maxgap(ii)+1)))+1;
                            allind=dind+N_d13*(a2primepart-1)+N_d13*N_a2*a2Bind_flat+N_d13*N_a2*N_a2*a3Bind_flat;
                            a1primepart=a1localind+loweredge(allind)-1;
                            d1part=shiftdim(d1part,1);
                            d3part=shiftdim(d3part,1);
                            a1primepart=shiftdim(a1primepart,1);
                            ...
                            loweredge=maxindex1(:,1,:,ii,:,:);
                            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,special_n_bothz,ones(1,length(n_e)), d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec,3);
                            d3aprime=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol;
                            entireRHS_ii_ze=reshape(ReturnMatrix_ii+DiscountedEV_z(d3aprime),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                            [Vtempii,maxindex]=max(entireRHS_ii_ze,[],1);
                            V_ford4_jj(curraindex,z_c,e_c,d4_c)=shiftdim(Vtempii,1);
                            dind       =rem(maxindex-1,N_d13)+1;
                            d1part     =rem(maxindex-1,N_d1)+1;
    ...
    a1a2primepart=reshape(a1a2prime_ford4_jj(linidx_d4),[1,N_a,N_bothz,N_e]);
    Policy(5,:,:,:,N_j)=rem(a1a2primepart-1,N_a1)+1; % a1prime
    Policy(6,:,:,:,N_j)=floor((a1a2primepart-1)/N_a1)+1; % a2prime
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,n_e, d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,1);
            RM=reshape(ReturnMatrix_ii,[N_d1,N_d3,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz,N_e]);
            DEV=reshape(DiscountedEV,[1,N_d3,N_a1,N_a2,1,1,1,N_bothz,1]);
            entireRHS_ii=RM+DEV;
            entireRHS_ii=reshape(entireRHS_ii,[N_d13,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz,N_e]);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,n_e, d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,:,d4_c)=shiftdim(Vtempii,1);
                    ...
                    loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,n_e, d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,:,d4_c)=shiftdim(Vtempii,1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d4_c=1:N_d4
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,ones(1,length(n_e)), d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,1);
                RM=reshape(ReturnMatrix_ii_e,[N_d1,N_d3,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz]);
                DEV=reshape(DiscountedEV,[1,N_d3,N_a1,N_a2,1,1,1,N_bothz]);
                entireRHS_ii_e=RM+DEV;
                entireRHS_ii_e=reshape(entireRHS_ii_e,[N_d13,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz]);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,:,ii,:,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,ones(1,length(n_e)), d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                        V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
                        ...
                        loweredge=maxindex1(:,1,:,ii,:,:,:);
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz,ones(1,length(n_e)), d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                        entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                        V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
