...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    [V(:,:,:,N_j),Policy(:,:,:,N_j)]=internal_per_j_e(EVpre,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d3,n_d4,n_a1,n_a2,n_semiz,n_e,N_d2,N_d3,N_d4,N_d23,N_d2d3,N_a1,N_a2,N_a,N_semiz,N_e,N_u,...
        d3_gridvals,d4_grid,d4_gridvals,special_n_d4,a1_gridvals,a2_gridvals,semiz_gridvals_J(:,:,N_j),e_gridvals_J(:,:,N_j),pi_semiz,pi_u_col,...
...
for reverse_j=1:N_j-1
    ...
    [V(:,:,:,jj),Policy(:,:,:,jj)]=internal_per_j_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d3,n_d4,n_a1,n_a2,n_semiz,n_e,N_d2,N_d3,N_d4,N_d23,N_d2d3,N_a1,N_a2,N_a,N_semiz,N_e,N_u,...
        d3_gridvals,d4_grid,d4_gridvals,special_n_d4,a1_gridvals,a2_gridvals,semiz_gridvals_J(:,:,jj),e_gridvals_J(:,:,jj),pi_semiz,pi_u_col,...
        level1ii,level1iidiff,a2Bind,zBind,eBind,d3ind,V_ford4_jj,Policy_ford4_jj,vfoptions);
end
...
%% Per-period inner (no d1, no z, with e). EVnext has shape [N_a,N_semiz]
function [V_jj,Policy_jj]=internal_per_j_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
    n_d3,n_d4,n_a1,n_a2,n_semiz,n_e,N_d2,N_d3,N_d4,N_d23,N_d2d3,N_a1,N_a2,N_a,N_semiz_count,N_e_count,N_u,...
    d3_gridvals,d4_grid,d4_gridvals,special_n_d4,a1_gridvals,a2_gridvals,semiz_gridvals,e_gridvals,pi_semiz,pi_u_col,...
    level1ii,level1iidiff,a2Bind,zBind,eBind,d3ind,V_ford4_jj,Policy_ford4_jj,vfoptions)
...
if vfoptions.lowmemory==0
    for d4_c=1:N_d4
        ...
        entireRHS_ii=RM+DEV;
        entireRHS_ii=reshape(entireRHS_ii,[N_d3,vfoptions.level1n,N_a1,N_a2,N_semiz_count,N_e_count]);

        [~,maxindex1]=max(entireRHS_ii,[],2);
        [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d3*N_a1,vfoptions.level1n*N_a2,N_semiz_count,N_e_count]),[],1);
        ...
        Policy_ford4_jj(curraindex,:,:,d4_c)=encodePolicy_with_d2lookup_nod1_e(shiftdim(maxindex2,1),N_d2,N_d3,d2index_resh,N_a1,N_semiz_count,N_e_count);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz_count,N_e_count]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford4_jj(curraindex,:,:,d4_c)=shiftdim(Vtempii,1);
                dind=(rem(maxindex-1,N_d3)+1);
                allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBind+N_d3*N_a2*N_semiz_count*eBind;
                pol_d3_a1=maxindex+N_d3*(loweredge(allind)-1);
                Policy_ford4_jj(curraindex,:,:,d4_c)=encodePolicy_with_d2lookup_nod1_e(shiftdim(pol_d3_a1,1),N_d2,N_d3,d2index_resh,N_a1,N_semiz_count,N_e_count);
            else
                loweredge=maxindex1(:,1,ii,:,:,:);
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d3,1,level1iidiff(ii),n_a2,n_semiz,n_e, d3_with_d4, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, semiz_gridvals, e_gridvals, ReturnFnParamsVec,3,0);
                d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*zBind;
                entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3,level1iidiff(ii)*N_a2,N_semiz_count,N_e_count]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V_ford4_jj(curraindex,:,:,d4_c)=shiftdim(Vtempii,1);
                dind=(rem(maxindex-1,N_d3)+1);
                allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBind+N_d3*N_a2*N_semiz_count*eBind;
...
elseif vfoptions.lowmemory>=1
    ...
    for d4_c=1:N_d4
        ...
        for e_c=1:N_e_count
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d3,n_a1,vfoptions.level1n,n_a2,n_semiz,ones(1,length(n_e)), d3_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, semiz_gridvals, e_val, ReturnFnParamsVec,1,0);
            ...
            entireRHS_ii_e=reshape(entireRHS_ii_e,[N_d3,vfoptions.level1n,N_a1,N_a2,N_semiz_count]);

            [~,maxindex1]=max(entireRHS_ii_e,[],2);
            [Vtempii,maxindex2]=max(reshape(entireRHS_ii_e,[N_d3*N_a1,vfoptions.level1n*N_a2,N_semiz_count]),[],1);
            curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
            V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
            Policy_ford4_jj(curraindex,:,e_c,d4_c)=encodePolicy_with_d2lookup_nod1(shiftdim(maxindex2,1),N_d2,N_d3,d2index_resh,N_a1,N_semiz_count);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_semiz,ones(1,length(n_e)), d3_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, semiz_gridvals, e_val, ReturnFnParamsVec,3,0);
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*zBind;
                    entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz_count]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d3)+1);
                    allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBind;
                    pol_d3_a1=maxindex+N_d3*(loweredge(allind)-1);
                    Policy_ford4_jj(curraindex,:,e_c,d4_c)=encodePolicy_with_d2lookup_nod1(shiftdim(pol_d3_a1,1),N_d2,N_d3,d2index_resh,N_a1,N_semiz_count);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,n_d3,1,level1iidiff(ii),n_a2,n_semiz,ones(1,length(n_e)), d3_with_d4, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, semiz_gridvals, e_val, ReturnFnParamsVec,3,0);
                    d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*zBind;
                    entireRHS_ii_e=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d3,level1iidiff(ii)*N_a2,N_semiz_count]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    V_ford4_jj(curraindex,:,e_c,d4_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d3)+1);
                    allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii))+N_d3*N_a2*zBind;
