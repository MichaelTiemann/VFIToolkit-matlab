...
Policy=zeros(6,N_a,N_bothz,N_j,'gpuArray'); % d1, d2, d3, d4, a1prime, a2prime
...
a1a2prime_ford4_jj=zeros(N_a,N_bothz,N_d4,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,n_d4,n_a1,n_a2], [n_a1,n_a2,n_a3], n_bothz, d1d3d4a1a2_gridvals, a1a2a3_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        for z_c=1:N_bothz
            ...
            ReturnMatrix_z=CreateReturnFnMatrix_Case2_Disc(ReturnFn, [n_d1,n_d3,n_d4,n_a1,n_a2], [n_a1,n_a2,n_a3], special_n_bothz, d1d3d4a1a2_gridvals, a1a2a3_gridvals, z_val, ReturnFnParamsVec);
            ...
            a1a2primepart=ceil(maxindex/(N_d1*N_d3*N_d4));
            Policy(5,:,z_c,N_j)=rem(a1a2primepart-1,N_a1)+1; % a1prime
            Policy(6,:,z_c,N_j)=floor((a1a2primepart-1)/N_a1)+1; % a2prime
        end
    end
else
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz, d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,1);
            ...
            [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz]),[],1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:),N_a1-maxgap(ii)); % [N_d13,1,N_a2prime,1,N_a2,N_a3,N_bothz]
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz, d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz, d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,d4_c)=shiftdim(Vtempii,1);
                    dind       =rem(maxindex-1,N_d13)+1;
                    d1part     =rem(maxindex-1,N_d1)+1;
    ...
    elseif vfoptions.lowmemory==1
        for d4_c=1:N_d4
            ...
            for z_c=1:N_z
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,[n_semiz,ones(1,length(n_z))], d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_valblock, ReturnFnParamsVec,1);
                ...
                [~,maxindex1]=max(entireRHS_ii,[],2);
                [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_semiz]),[],1);
                curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                         +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
                ...
                a1a2prime_ford4_jj(curraindex,semizblock,d4_c)=a1primepart+N_a1*(a2primepart-1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,[n_semiz,ones(1,length(n_z))], d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_valblock, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBindblock,-4);
                        entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEVblock(d3aprimez),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford4_jj(curraindex,semizblock,d4_c)=shiftdim(Vtempii,1);
                        dind       =rem(maxindex-1,N_d13)+1;
                        d1part     =rem(maxindex-1,N_d1)+1;
                        ...
                        loweredge=maxindex1(:,1,:,ii,:,:,:);
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,[n_semiz,ones(1,length(n_z))], d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_valblock, ReturnFnParamsVec,3);
                        d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBindblock,-4);
                        entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEVblock(d3aprimez),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford4_jj(curraindex,semizblock,d4_c)=shiftdim(Vtempii,1);
                        dind       =rem(maxindex-1,N_d13)+1;
                        d1part     =rem(maxindex-1,N_d1)+1;
    ...
    elseif vfoptions.lowmemory>=2 % lm2 already does the most-looped variant, so it also serves the higher lowmemory values
        for d4_c=1:N_d4
            ...
            for z_c=1:N_bothz
                ...
                ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,ones(1,length(n_bothz)), d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec,1);
                RM=reshape(ReturnMatrix_ii_z,[N_d1,N_d3,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3]);
                DEV=reshape(DiscountedEV_z,[1,N_d3,N_a1,N_a2,1,1,1]);
                entireRHS_ii_z=RM+DEV;
                entireRHS_ii_z=reshape(entireRHS_ii_z,[N_d13,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3]);

                [~,maxindex1]=max(entireRHS_ii_z,[],2);
                [Vtempii,maxindex2]=max(reshape(entireRHS_ii_z,[N_d13*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
                curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                         +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,ones(1,length(n_bothz)), d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec,3);
                        d3aprime=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol;
                        entireRHS_ii_z=reshape(ReturnMatrix_ii+DiscountedEV_z(d3aprime),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        ...
                        loweredge=maxindex1(:,1,:,ii,:,:);
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,ones(1,length(n_bothz)), d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec,3);
                        d3aprime=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol;
                        entireRHS_ii_z=reshape(ReturnMatrix_ii+DiscountedEV_z(d3aprime),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
                        V_ford4_jj(curraindex,z_c,d4_c)=shiftdim(Vtempii,1);
                        dind       =rem(maxindex-1,N_d13)+1;
                        d1part     =rem(maxindex-1,N_d1)+1;
    ...
    a1a2primepart=reshape(a1a2prime_ford4_jj(linidx_d4),[1,N_a,N_bothz]);
    Policy(5,:,:,N_j)=rem(a1a2primepart-1,N_a1)+1; % a1prime
    Policy(6,:,:,N_j)=floor((a1a2primepart-1)/N_a1)+1; % a2prime
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d4_c=1:N_d4
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz, d13_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,1);
            RM=reshape(ReturnMatrix_ii,[N_d1,N_d3,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz]);
            DEV=reshape(DiscountedEV,[1,N_d3,N_a1,N_a2,1,1,1,N_bothz]);
            entireRHS_ii=RM+DEV;
            entireRHS_ii=reshape(entireRHS_ii,[N_d13,N_a1,N_a2,vfoptions.level1n,N_a2,N_a3,N_bothz]);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz, d13_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,d4_c)=shiftdim(Vtempii,1);
                    ...
                    loweredge=maxindex1(:,1,:,ii,:,:,:);
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1,[n_d3,special_n_d4],n_a2,n_a3,n_bothz, d13_with_d4, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,3);
                    d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*a2pcol+N_d3*N_a1*N_a2*shiftdim(zBind,-4);
                    entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV(d3aprimez),[N_d13*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford4_jj(curraindex,:,d4_c)=shiftdim(Vtempii,1);
