...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for d4_c=1:N_d4
        ...
        if vfoptions.lowmemory==0
            ...
            entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_bothz,N_e])+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_bothz,N_e]),[N_d13*n2long,N_a1*N_a2,N_bothz,N_e]);
            ...
            ReturnMatrix_ii_resh=reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_bothz,N_e]);
            ...
            % d2 lookup (d2index_resh has no e dim)
            d3part=rem(ceil(shiftdim(d_ind,1)/N_d1)-1,N_d3)+1; % [N_a, N_bothz, N_e]
        ...
        elseif vfoptions.lowmemory==1
            ...
            for e_c=1:N_e
                ...
                da1primez=d3ind+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*shiftdim(zBind,-2);
                entireRHS_ii_e=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_bothz])+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_bothz]),[N_d13*n2long,N_a1*N_a2,N_bothz]);
                ...
                % d2 lookup (d2index_resh has no e dim)
                d3part=rem(ceil(shiftdim(d_ind,1)/N_d1)-1,N_d3)+1; % [N_a, N_bothz]
                a1mid=squeeze(midpoint(allind));
        ...
        elseif vfoptions.lowmemory==2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_e=reshape(reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_semiz])+reshape(DiscountedEVinterpblock(da1primez),[N_d13,n2long,N_a1,N_a2,N_semiz]),[N_d13*n2long,N_a1*N_a2,N_semiz]);
                    [Vtempii,maxindexL2]=max(entireRHS_ii_e,[],1);
                    ...
                    % d2 lookup (d2index_reshblock has no e dim)
                    d3part=rem(ceil(shiftdim(d_ind,1)/N_d1)-1,N_d3)+1; % [N_a, N_semiz]
                    a1mid=squeeze(midpoint(allind));
    ...
    % Cross-d4 max (dim 4 since shape is [N_a,N_bothz,N_e,N_d4])
