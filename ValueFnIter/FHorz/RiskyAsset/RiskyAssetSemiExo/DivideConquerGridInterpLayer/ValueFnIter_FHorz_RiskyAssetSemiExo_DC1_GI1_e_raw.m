...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    [V(:,:,:,N_j),Policy3(:,:,:,:,N_j),PolicyL2flag(:,:,:,:,N_j),d2Policy(:,:,:,:,N_j),d4Policy(:,:,:,:,N_j)]=internal_per_j_e(EVpre,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d1,n_d3,n_d4,n_a1,n_a2,n_bothz,n_e,N_d1,N_d2,N_d3,N_d4,N_d13,N_d23,N_a1,N_a1prime,N_a2,N_a,N_bothz,N_e,N_u,...
...
for reverse_j=1:N_j-1
    ...
    [V(:,:,:,jj),Policy3(:,:,:,:,jj),PolicyL2flag(:,:,:,:,jj),d2Policy(:,:,:,:,jj),d4Policy(:,:,:,:,jj)]=internal_per_j_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d1,n_d3,n_d4,n_a1,n_a2,n_bothz,n_e,N_d1,N_d2,N_d3,N_d4,N_d13,N_d23,N_a1,N_a1prime,N_a2,N_a,N_bothz,N_e,N_u,...
...
%% Encode single-index Policy (with d1, z, e)
...
%% Per-period inner (with e). EVnext already integrated over e: [N_a,N_bothz]
function [V_jj,Policy3_jj,PolicyL2flag_jj,d2Policy_jj,d4Policy_jj]=internal_per_j_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
    n_d1,n_d3,n_d4,n_a1,n_a2,n_bothz,n_e,N_d1,N_d2,N_d3,N_d4,N_d13,N_d23,N_a1,N_a1prime,N_a2,N_a,N_bothz_count,N_e_count,N_u,...
...
for d4_c=1:N_d4
    ...
    if vfoptions.lowmemory==0
        midpoint=zeros(N_d13,1,N_a1,N_a2,N_bothz_count,N_e_count,'gpuArray');
        ...
        entireRHS_ii=reshape(ReturnMatrix_ii+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_bothz_count,N_e_count]),[N_d13*n2long,N_a1*N_a2,N_bothz_count,N_e_count]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        V_ford4_jj(:,:,:,d4_c)=shiftdim(Vtempii,1);
        d_ind=rem(maxindexL2-1,N_d13)+1;
        allind=d_ind+N_d13*aind+N_d13*N_a*zBind+N_d13*N_a*N_bothz_count*eBind;
        ...
        linidx_lower  = d_ind                    + N_d13*n2long*aind + N_d13*n2long*N_a*zBind + N_d13*n2long*N_a*N_bothz_count*eBind;
        linidx_upper  = d_ind + N_d13*(n2long-1) + N_d13*n2long*aind + N_d13*n2long*N_a*zBind + N_d13*n2long*N_a*N_bothz_count*eBind;
        ReturnMatrix_ii_resh=reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_bothz_count,N_e_count]);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e_count
            ...
            midpoint=zeros(N_d13,1,N_a1,N_a2,N_bothz_count,'gpuArray');

            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,n_bothz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals, e_val, ReturnFnParamsVec,1,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d13_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals, e_val, ReturnFnParamsVec,3,0);
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*zBind;
            ...
            da1primez=d3ind+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*zBind;
            entireRHS_ii_e=reshape(ReturnMatrix_ii+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_bothz_count]),[N_d13*n2long,N_a1*N_a2,N_bothz_count]);
            ...
            ReturnMatrix_ii_resh=reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_bothz_count]);
            isInfLower    = (ReturnMatrix_ii_resh(linidx_lower) == -Inf);
            isInfUpper    = (ReturnMatrix_ii_resh(linidx_upper) == -Inf);
            inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
            inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
