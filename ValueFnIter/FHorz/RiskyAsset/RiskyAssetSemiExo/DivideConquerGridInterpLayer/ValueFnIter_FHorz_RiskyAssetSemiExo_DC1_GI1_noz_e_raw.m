...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    [V(:,:,:,N_j),Policy3(:,:,:,:,N_j),PolicyL2flag(:,:,:,:,N_j),d2Policy(:,:,:,:,N_j),d4Policy(:,:,:,:,N_j)]=internal_per_j_noz_e(EVpre,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d1,n_d3,n_d4,n_a1,n_a2,n_semiz,n_e,N_d1,N_d2,N_d3,N_d4,N_d13,N_d23,N_a1,N_a1prime,N_a2,N_a,N_semiz,N_e,N_u,...
        d13_gridvals,d4_gridvals,a1_gridvals,a1prime_grid,a2_gridvals,semiz_gridvals_J(:,:,N_j),e_gridvals_J(:,:,N_j),pi_semiz,pi_u_col,...
...
for reverse_j=1:N_j-1
    ...
    [V(:,:,:,jj),Policy3(:,:,:,:,jj),PolicyL2flag(:,:,:,:,jj),d2Policy(:,:,:,:,jj),d4Policy(:,:,:,:,jj)]=internal_per_j_noz_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d1,n_d3,n_d4,n_a1,n_a2,n_semiz,n_e,N_d1,N_d2,N_d3,N_d4,N_d13,N_d23,N_a1,N_a1prime,N_a2,N_a,N_semiz,N_e,N_u,...
        d13_gridvals,d4_gridvals,a1_gridvals,a1prime_grid,a2_gridvals,semiz_gridvals_J(:,:,jj),e_gridvals_J(:,:,jj),pi_semiz,pi_u_col,...
        level1ii,level1iidiff,n2short,n2long,aind,zBind,eBind,d3ind,V_ford4_jj,Policy3_ford4_jj,flag_ford4_jj,d2_ford4_jj,vfoptions);
end
...
%% Encode single-index Policy (with d1, no z, with e)
d1part=rem(Policy3(1,:,:,:,:)-1,N_d1)+1;
d3part=rem(ceil(Policy3(1,:,:,:,:)/N_d1)-1,N_d3)+1;
d2part=d2Policy;
d4part=d4Policy;
N_d=N_d1*N_d2*N_d3*N_d4;
Policy=shiftdim(d1part+N_d1*(d2part-1)+N_d1*N_d2*(d3part-1)+N_d1*N_d2*N_d3*(d4part-1)+N_d1*N_d2*N_d3*N_d4*(Policy3(2,:,:,:,:)-1)+N_d*N_a1*(Policy3(3,:,:,:,:)-1)+N_d*N_a1*(n2short+2)*(PolicyL2flag-1),1);

end


%% Per-period inner (no z, with e). EVnext already integrated over e: [N_a,N_semiz]
function [V_jj,Policy3_jj,PolicyL2flag_jj,d2Policy_jj,d4Policy_jj]=internal_per_j_noz_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
    n_d1,n_d3,n_d4,n_a1,n_a2,n_semiz,n_e,N_d1,N_d2,N_d3,N_d4,N_d13,N_d23,N_a1,N_a1prime,N_a2,N_a,N_semiz_count,N_e_count,N_u,...
    d13_gridvals,d4_gridvals,a1_gridvals,a1prime_grid,a2_gridvals,semiz_gridvals,e_gridvals,pi_semiz,pi_u_col,...
    level1ii,level1iidiff,n2short,n2long,aind,zBind,eBind,d3ind,V_ford4_jj,Policy3_ford4_jj,flag_ford4_jj,d2_ford4_jj,vfoptions)
...
for d4_c=1:N_d4
    ...
    if vfoptions.lowmemory==0
        midpoint=zeros(N_d13,1,N_a1,N_a2,N_semiz_count,N_e_count,'gpuArray');
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n2long,n_a1,n_a2,n_semiz,n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, semiz_gridvals, e_gridvals, ReturnFnParamsVec,2,0);
        da1primez=d3ind+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*zBind;
        entireRHS_ii=reshape(ReturnMatrix_ii+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_semiz_count,N_e_count]),[N_d13*n2long,N_a1*N_a2,N_semiz_count,N_e_count]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        V_ford4_jj(:,:,:,d4_c)=shiftdim(Vtempii,1);
        d_ind=rem(maxindexL2-1,N_d13)+1;
        allind=d_ind+N_d13*aind+N_d13*N_a*zBind+N_d13*N_a*N_semiz_count*eBind;
        ...
        linidx_lower  = d_ind                    + N_d13*n2long*aind + N_d13*n2long*N_a*zBind + N_d13*n2long*N_a*N_semiz_count*eBind;
        linidx_upper  = d_ind + N_d13*(n2long-1) + N_d13*n2long*aind + N_d13*n2long*N_a*zBind + N_d13*n2long*N_a*N_semiz_count*eBind;
        ReturnMatrix_ii_resh=reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_semiz_count,N_e_count]);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e_count
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,n_semiz,special_n_e, d13_with_d4, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, semiz_gridvals, e_val, ReturnFnParamsVec,1,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_semiz,special_n_e, d13_with_d4, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, semiz_gridvals, e_val, ReturnFnParamsVec,3,0);
                    d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*zBind;
            ...
            midpoint=max(min(midpoint,n_a1(1)-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n2long,n_a1,n_a2,n_semiz,special_n_e, d13_with_d4, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, semiz_gridvals, e_val, ReturnFnParamsVec,2,0);
            da1primez=d3ind+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*zBind;
            entireRHS_ii_e=reshape(ReturnMatrix_ii+reshape(DiscountedEVinterp(da1primez),[N_d13,n2long,N_a1,N_a2,N_semiz_count]),[N_d13*n2long,N_a1*N_a2,N_semiz_count]);
            ...
            ReturnMatrix_ii_resh=reshape(ReturnMatrix_ii,[N_d13,n2long,N_a1,N_a2,N_semiz_count]);
            isInfLower    = (ReturnMatrix_ii_resh(linidx_lower) == -Inf);
            isInfUpper    = (ReturnMatrix_ii_resh(linidx_upper) == -Inf);
            inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
            inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
