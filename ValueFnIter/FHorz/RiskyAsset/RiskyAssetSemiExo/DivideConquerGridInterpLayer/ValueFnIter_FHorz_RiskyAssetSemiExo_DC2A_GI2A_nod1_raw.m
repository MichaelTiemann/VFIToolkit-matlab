function [V,Policy]=ValueFnIter_FHorz_RiskyAssetSemiExo_DC2A_GI2A_nod1_raw(n_d2,n_d3,n_d4,n_a1,n_a2,n_a3,n_semiz,n_z,n_u,N_j, d2_grid, d3_grid, d4_grid, a1_grid, a2_grid, a3_grid, semiz_gridvals_J, z_gridvals_J, u_grid, pi_semiz_J, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Two standard endogenous assets version of ValueFnIter_FHorz_RiskyAssetSemiExo_DC1_GI1_nod1_raw.
...
Policy3=zeros(3,N_a,N_bothz,N_j,'gpuArray'); % (1)=d3, (2)=joint(a1prime midpoint,a2prime), (3)=L2ind
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for d4_c=1:N_d4
        ...
        if vfoptions.lowmemory==0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0,[n_d3,special_n_d4], n_a2, n_a3, n_bothz, d3_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, bothz_gridvals, ReturnFnParamsVec, 1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap(ii));                % [N_d3,maxgap+1,N_a2prime,1,N_a2,N_a3,N_bothz]
                    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0,[n_d3,special_n_d4], n_a2, n_a3, n_bothz, d3_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals, ReturnFnParamsVec, 3);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0,[n_d3,special_n_d4], n_a2, n_a3, n_bothz, d3_with_d4, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals, ReturnFnParamsVec, 3);
        ...
        elseif vfoptions.lowmemory==1
            for z_c=1:N_z
                ...
                midpoint=max(min(midpoint,N_a1-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0,[n_d3,special_n_d4], n_a2, n_a3, [n_semiz,ones(1,length(n_z))], d3_with_d4, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_valblock, ReturnFnParamsVec, 3);
        ...
        elseif vfoptions.lowmemory>=2 % lm2 already does the most-looped variant, so it also serves the higher lowmemory values
            for z_c=1:N_bothz
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0,[n_d3,special_n_d4], n_a2, n_a3, special_n_bothz, d3_with_d4, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0,[n_d3,special_n_d4], n_a2, n_a3, special_n_bothz, d3_with_d4, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                ...
                midpoint=max(min(midpoint,N_a1-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0,[n_d3,special_n_d4], n_a2, n_a3, special_n_bothz, d3_with_d4, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
...
Policy=zeros(7,N_a,N_bothz,N_j,'gpuArray'); % rows: (d2,d3,d4,a1prime,a2prime,L2,L2flag)
Policy(1,:,:,:)=d2Policy;
Policy(2,:,:,:)=Policy3(1,:,:,:);
Policy(3,:,:,:)=d4Policy;
