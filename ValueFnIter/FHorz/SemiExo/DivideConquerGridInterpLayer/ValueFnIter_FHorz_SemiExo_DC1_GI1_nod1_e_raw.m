function [V,Policy]=ValueFnIter_FHorz_SemiExo_DC1_GI1_nod1_e_raw(n_d2,n_a,n_z,n_semiz, n_e,N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[(maxgap(ii)+1),1,N_bothz,N_e]); % autoexpand level1iidiff(ii) in 2nd-dim
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                entireRHS_d2ii=ReturnMatrix_d2iie+DiscountFactorParamsVec*EV_d2;
                % Treat standard problem as just being the first layer
                [~,maxindex1]=max(entireRHS_d2ii,[],1); % no d1
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        aprimez=aprimeindexes+N_a*bothzind;
                        entireRHS_ii=ReturnMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[(maxgap(ii)+1),1,N_bothz]); % autoexpand level1iidiff(ii) in 2nd-dim
                        [~,maxindex]=max(entireRHS_ii,[],1);
                        midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(1,ii,:);
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_d2ii=ReturnMatrix_d2iie+DiscountFactorParamsVec*EV_d2z;
                    % Treat standard problem as just being the first layer
                    [~,maxindex1]=max(entireRHS_d2ii,[],1); % no d1
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[(maxgap(ii)+1),1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [~,maxindex]=max(entireRHS_ii,[],1);
                            midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(1,ii,:);
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2z(aprimeindexes),[(maxgap(ii)+1),1]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [~,maxindex]=max(entireRHS_ii,[],1);
                            midpoints_jj(1,curraindex)=maxindex+(loweredge-1);
                        else
                            loweredge=maxindex1(1,ii,:);
