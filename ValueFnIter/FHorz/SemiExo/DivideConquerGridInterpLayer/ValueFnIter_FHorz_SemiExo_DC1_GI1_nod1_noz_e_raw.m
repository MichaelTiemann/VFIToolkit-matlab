function [V,Policy]=ValueFnIter_FHorz_SemiExo_DC1_GI1_nod1_noz_e_raw(n_d2,n_a,n_semiz,n_e, N_j, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, n_semiz, special_n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_semize=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, n_d2, special_n_semiz, special_n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), semiz_val, e_val, ReturnFnParamsVec,3);
        ...
        for d2_c=1:N_d2
            ...
            for semiz_c=1:N_semiz
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_d2semiie=ReturnMatrix_d2semiie+DiscountFactorParamsVec*EV_d2semiz;
                    % Treat standard problem as just being the first layer
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iisemize+DiscountFactorParamsVec*reshape(EV_d2semiz(aprimeindexes),[(maxgap(ii)+1),1]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [~,maxindex]=max(entireRHS_ii,[],1);
                    ...
                    entireRHS_ii=ReturnMatrix_d2ii+DiscountFactorParamsVec*reshape(EVinterp_d2semiz(aprimeindexes),[n2long,N_a]);
                    [Vtemp,maxindex]=max(entireRHS_ii,[],1);
