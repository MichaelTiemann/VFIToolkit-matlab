function [V,Policy]=ValueFnIter_FHorz_SemiExo_DC2A_GI2A_noz_e_raw(n_d1, n_d2, n_a, n_semiz, n_e, N_j, d1_gridvals, d2_gridvals, a_grid, semiz_gridvals_J, e_gridvals_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
      ...
      if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprime,[N_d1,(maxgap(ii)+1),N_a2,1,N_a2,N_semiz,N_e]));
        ...
        entireRHS_ii=ReturnMatrix_ii+reshape(DiscountedEVinterp(aprime),[N_d1*n2long*N_a2,N_a,N_semiz,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(curraindex), a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3, 0);
                    aprime=a1primeindexes+N_a1*a2ind+N_a1*N_a2*semizBind;
                    entireRHS_ii=ReturnMatrix_ii+DiscountedEV(reshape(aprime,[N_d1,(maxgap(ii)+1),N_a2,1,N_a2,N_semiz]));
                    [~,maxindex]=max(entireRHS_ii,[],2);% max over a1prime
                    midpoints_jj(:,1,:,curraindex,:,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:,:);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_e(ReturnFn, special_n_d, n_semiz, special_n_e, d12c_gridvals, a1prime_grid(a1primeindexes), a2_grid, a1_grid, a2_grid, semiz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 2, 0);
            aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*semizBind;
            entireRHS_ii=ReturnMatrix_ii+reshape(DiscountedEVinterp(aprime),[N_d1*n2long*N_a2,N_a,N_semiz]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            maxindexL2d1=rem(maxindexL2-1,N_d1)+1;
            maxindexL2a=ceil(maxindexL2/N_d1);
            maxindexL2a1=rem(maxindexL2a-1,n2long)+1;
      ...
      elseif vfoptions.lowmemory==2 % outer semiz / inner e
        for semiz_c=1:N_semiz
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii=ReturnMatrix_ii+reshape(DiscountedEV_d2zinterp(aprime),[N_d1*n2long*N_a2,N_a]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                maxindexL2d1=rem(maxindexL2-1,N_d1)+1;
                maxindexL2a=ceil(maxindexL2/N_d1);
                maxindexL2a1=rem(maxindexL2a-1,n2long)+1;
