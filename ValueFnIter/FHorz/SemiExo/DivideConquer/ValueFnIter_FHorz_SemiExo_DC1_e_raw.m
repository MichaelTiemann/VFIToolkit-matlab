function [V,Policy3]=ValueFnIter_FHorz_SemiExo_DC1_e_raw(n_d1,n_d2,n_a,n_z,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii=ReturnMatrix_d2iize+DiscountFactorParamsVec*shiftdim(EV_d2z,-1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[N_d1*(maxgap(ii)+1),1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d1)+1);
                            allind=dind+N_d1*semizind; % loweredge is n_d1-by-1-by-1-by-n_semiz
                            ...
                            entireRHS_ii=ReturnMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[N_d1,1,N_semiz]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d1)+1);
                            allind=dind+N_d1*semizind; % loweredge is n_d1-by-1-by-1-by-n_semiz
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[N_d1*(maxgap(ii)+1),1]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d1)+1);
                            allind=dind; % loweredge is n_d1-by-1
                            ...
                            ReturnMatrix_iize=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d, special_n_bothz, special_n_e, d12c_gridvals, a_grid(loweredge), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, ReturnFnParamsVec,2);
                            aprimez=loweredge;
                            entireRHS_ii=ReturnMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[N_d1,1]); % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            dind=(rem(maxindex-1,N_d1)+1);
                            allind=dind; % loweredge is n_d1-by-1
