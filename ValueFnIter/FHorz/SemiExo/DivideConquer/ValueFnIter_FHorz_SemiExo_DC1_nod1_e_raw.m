function [V,Policy3]=ValueFnIter_FHorz_SemiExo_DC1_nod1_e_raw(n_d2,n_a,n_z,n_semiz, n_e,N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
        ...
        for d2_c=1:N_d2
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[1,1,N_bothz,N_e]);  % autoexpand level1iidiff(ii) in 2nd-dim
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_iie=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, n_bothz, special_n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,5);
                        aprimez=loweredge+N_a*bothzind;
                        entireRHS_ii=ReturnMatrix_iie+DiscountFactorParamsVec*reshape(EV_d2(aprimez),[1,1,N_bothz]); % autoexpand level1iidiff(ii) in 2nd-dim
        ...
        for d2_c=1:N_d2
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii=ReturnMatrix_d2iize+DiscountFactorParamsVec*EV_d2z;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[(maxgap(ii)+1),1,N_semiz]);  % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(Vtempii,1);
                            Policy_ford2_jj(curraindex,semizblock,e_c,d2_c)=shiftdim(maxindex+(loweredge-1)); % no d1
                        else
                            ...
                            entireRHS_ii=ReturnMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[1,1,N_semiz]);  % autoexpand level1iidiff(ii) in 2nd-dim
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=ReturnMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[(maxgap(ii)+1),1]);  % autoexpand level1iidiff(ii) in 2nd-dim
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(Vtempii,1);
                            Policy_ford2_jj(curraindex,z_c,e_c,d2_c)=shiftdim(maxindex+(loweredge-1)); % no d1
                        else
                            ...
                            ReturnMatrix_iize=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn, special_n_d2, special_n_bothz, special_n_e, d2_val, reshape(a_grid(loweredge),loweredgesize), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), z_val, e_val, ReturnFnParamsVec,5);
                            aprimez=loweredge;
                            entireRHS_ii=ReturnMatrix_iize+DiscountFactorParamsVec*reshape(EV_d2z(aprimez),[1,1]);  % autoexpand level1iidiff(ii) in 2nd-dim
