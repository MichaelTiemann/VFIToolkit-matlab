function [V,Policy3]=ValueFnIter_FHorz_ExpAssetsemiz_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% experienceassetsemiz, no d1: d2 determines experience asset, d3 determines semi-exog state
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % loop over z (markov), vectorize over semiz
        for z_c=1:N_z
            ...
            d_ind=rem(maxindex-1,N_d23)+1;
            Policy3(1,:,semizblock,N_j)=rem(d_ind-1,N_d2)+1;
            Policy3(2,:,semizblock,N_j)=ceil(d_ind/N_d2);
            Policy3(3,:,semizblock,N_j)=ceil(maxindex/N_d23);
        end
    ...
    % aprime depends on (d2, a1, a2, current_semiz); independent of d3 and z -- compute once
    ...
    aprimeIndex_full=repmat(aprimeIndex,1,1,N_z); % [N_d2*N_a1, N_a2, N_bothz] (semiz fast, so tile over z)
