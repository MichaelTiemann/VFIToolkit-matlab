function [V,Policy]=ValueFnIter_FHorz_ExpAssetsemiz_DC1_GI1_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
            ...
            for z_c=1:N_z
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(curraindex), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                ...
                Policy3_ford3_jj(1,:,semizblock,d3_c)=d_ind;
                Policy3_ford3_jj(2,:,semizblock,d3_c)=shiftdim(squeeze(midpoint(allind)),-1);
                Policy3_ford3_jj(3,:,semizblock,d3_c)=shiftdim(ceil(maxindexL2/N_d2),-1);
                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                L2offset = ceil(maxindexL2/N_d2);
    ...
    elseif vfoptions.lowmemory==2
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                Policy3_ford3_jj(2,:,z_c,d3_c)=shiftdim(squeeze(midpoint(allind)),-1);
                Policy3_ford3_jj(3,:,z_c,d3_c)=shiftdim(ceil(maxindexL2/N_d2),-1);
                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                L2offset = ceil(maxindexL2/N_d2);
    ...
    Policy(1,:,:,N_j)=reshape(Policy3_ford3_jj(1+temp),[1,N_a,N_bothz]); % d2
    Policy(3,:,:,N_j)=reshape(Policy3_ford3_jj(2+temp),[1,N_a,N_bothz]); % midpoint
    Policy(4,:,:,N_j)=reshape(Policy3_ford3_jj(3+temp),[1,N_a,N_bothz]); % L2
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3=ReturnMatrix_ii_d3+reshape(DiscountedEVinterp(d2a1primea2bothz(:)),[N_d2*n2long,N_a1*N_a2,N_bothz]);
    ...
    elseif vfoptions.lowmemory==1 % loop over z (markov), vectorize over semiz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+DiscountedEV;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_d3z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(curraindex), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind; % linear index into DiscountedEV [N_d2,N_a1,1,N_a2,N_semiz]
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+reshape(DiscountedEVinterp(d2a1primea2semiz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
    ...
    elseif vfoptions.lowmemory==2
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+DiscountedEV_z;
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+reshape(DiscountedEVinterp_z(d2a1primea2(:)),[N_d2*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3z,[],1);
                V_ford3_jj(:,z_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d2)+1;
                allind=d_ind+N_d2*aind;
