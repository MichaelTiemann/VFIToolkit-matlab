...
% ValueFnIter_FHorz_ExpAssetSemiExo_DC2A_GI2A_raw; the EV pipeline differs
...
Vtilde=zeros(N_a,N_bothz,N_j,'gpuArray'); % QH-perceived value (beta0*beta)
Valt=zeros(N_a,N_bothz,N_j,'gpuArray'); % exponential value (beta); drives the backward recursion
...
Policyalt=zeros(5,N_a,N_bothz,N_j,'gpuArray'); % exponential-discounter policy
...
d12_ford3_alt=zeros(N_a,N_bothz,N_d3,'gpuArray');
d12_ford3_tilde=zeros(N_a,N_bothz,N_d3,'gpuArray');
joint_ford3_alt=zeros(N_a,N_bothz,N_d3,'gpuArray');
joint_ford3_tilde=zeros(N_a,N_bothz,N_d3,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            [Vtempii,maxindexL2]=max(ReturnMatrix_ii,[],1);
            Vtilde(:,zind,N_j)=shiftdim(Vtempii,1);
            ...
            d12_ind=rem(d_ind-1,N_d12)+1;
            Policy(1,:,zind,N_j)=rem(d12_ind-1,N_d1)+1; % d1
            Policy(2,:,zind,N_j)=ceil(d12_ind/N_d1); % d2
            Policy(3,:,zind,N_j)=ceil(d_ind/N_d12); % d3
            Policy(4,:,zind,N_j)=midpoint(allind)+N_a1*(maxindexL2a2-1); % joint(a1prime midpoint,a2prime)
            Policy(5,:,zind,N_j)=maxindexL2a1; % a1primeL2ind
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
            ...
            entireRHS_ii_d3=reshape(ReturnMatrix_L2+DiscountedEVinterp_alt(aprimez),[N_d12*n2long*N_a2,N_a,N_bothz]);
            ...
            d12_ford3_alt(:,:,d3_c)=shiftdim(d_ind,1);
            joint_ford3_alt(:,:,d3_c)=shiftdim(midpoint(allind)+N_a1*(maxindexL2a2-1),1);
            ...
            ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
            aprimez=d2ind_vec + N_d2*(a1primeindexesfine-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            entireRHS_ii_d3=reshape(ReturnMatrix_L2+DiscountedEVinterp_tilde(aprimez),[N_d12*n2long*N_a2,N_a,N_bothz]);
            [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
            ...
            d12_ford3_tilde(:,:,d3_c)=shiftdim(d_ind,1);
            joint_ford3_tilde(:,:,d3_c)=shiftdim(midpoint(allind)+N_a1*(maxindexL2a2-1),1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                %% Valt (beta)
                entireRHS_ii_d3=ReturnMatrix_ii_d3+repelem(DiscountedEV_alt_z,N_d1,1,1,1,1,1,1);
                [~,maxindex1]=max(entireRHS_ii_d3,[],2);
                midpoint(:,1,:,level1ii,:,:,:)=maxindex1;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,:,ii,:,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_semiz, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                ...
                midpoint=max(min(midpoint,N_a1-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_semiz, d123_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                ...
                entireRHS_ii_d3=reshape(ReturnMatrix_L2+DiscountedEVinterp_alt_z(aprime_z),[N_d12*n2long*N_a2,N_a,N_semiz]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
                ...
                d12_ford3_alt(:,zind,d3_c)=shiftdim(d_ind,1);
                joint_ford3_alt(:,zind,d3_c)=shiftdim(midpoint(allind)+N_a1*(maxindexL2a2-1),1);
                ...
                %% Vtilde (beta0*beta)
                entireRHS_ii_d3=ReturnMatrix_ii_d3+repelem(DiscountedEV_tilde_z,N_d1,1,1,1,1,1,1);
                [~,maxindex1]=max(entireRHS_ii_d3,[],2);
                midpoint(:,1,:,level1ii,:,:,:)=maxindex1;
                ...
                ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_semiz, d123_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                aprime_z=d2ind_vec + N_d2*(a1primeindexesfine-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_semiz-1),-5);
                entireRHS_ii_d3=reshape(ReturnMatrix_L2+DiscountedEVinterp_tilde_z(aprime_z),[N_d12*n2long*N_a2,N_a,N_semiz]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
                ...
                d12_ford3_tilde(:,zind,d3_c)=shiftdim(d_ind,1);
                joint_ford3_tilde(:,zind,d3_c)=shiftdim(midpoint(allind)+N_a1*(maxindexL2a2-1),1);
            ...
            for z_c=1:N_bothz
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,:,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_bothz, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                ...
                midpoint=max(min(midpoint,N_a1-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_bothz, d123_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                ...
                entireRHS_ii_z=reshape(ReturnMatrix_L2+DiscountedEVinterp_alt_z(aprime_z),[N_d12*n2long*N_a2,N_a]);
                ...
                d12_ford3_alt(:,z_c,d3_c)=shiftdim(d_ind,1);
                joint_ford3_alt(:,z_c,d3_c)=shiftdim(midpoint(allind)+N_a1*(maxindexL2a2-1),1);
                ...
                ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_bothz, d123_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                aprime_z=d2ind_vec + N_d2*(a1primeindexesfine-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
                entireRHS_ii_z=reshape(ReturnMatrix_L2+DiscountedEVinterp_tilde_z(aprime_z),[N_d12*n2long*N_a2,N_a]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_z,[],1);
                ...
                d12_ford3_tilde(:,z_c,d3_c)=shiftdim(d_ind,1);
                joint_ford3_tilde(:,z_c,d3_c)=shiftdim(midpoint(allind)+N_a1*(maxindexL2a2-1),1);
    ...
    [V_jj,d3_max]=max(V_ford3_tilde,[],3); % max over d3
    Vtilde(:,:,N_j)=V_jj;
    Policy(3,:,:,N_j)=shiftdim(d3_max,-1); % d3 is just the maximising index
    M=N_a*N_bothz;
    d3_max_lin=reshape(d3_max,[M,1]);
    idx=(1:M)'+M*(d3_max_lin-1);
    d12sel=reshape(d12_ford3_tilde(idx),[1,N_a,N_bothz]);
    Policy(1,:,:,N_j)=rem(d12sel-1,N_d1)+1; % d1
    Policy(2,:,:,N_j)=ceil(d12sel/N_d1); % d2
    Policy(4,:,:,N_j)=reshape(joint_ford3_tilde(idx),[1,N_a,N_bothz]); % joint(a1prime midpoint,a2prime)
    Policy(5,:,:,N_j)=reshape(L2a1_ford3_tilde(idx),[1,N_a,N_bothz]); % a1primeL2ind
    ...
    Valt(:,:,N_j)=V_jjalt;
    Policyalt(3,:,:,N_j)=shiftdim(d3_maxalt,-1); % d3 is just the maximising index
    Malt=N_a*N_bothz;
    d3_max_linalt=reshape(d3_maxalt,[Malt,1]);
    idxalt=(1:Malt)'+Malt*(d3_max_linalt-1);
    d12selalt=reshape(d12_ford3_alt(idxalt),[1,N_a,N_bothz]);
    Policyalt(1,:,:,N_j)=rem(d12selalt-1,N_d1)+1; % d1
    Policyalt(2,:,:,N_j)=ceil(d12selalt/N_d1); % d2
    Policyalt(4,:,:,N_j)=reshape(joint_ford3_alt(idxalt),[1,N_a,N_bothz]); % joint(a1prime midpoint,a2prime)
    Policyalt(5,:,:,N_j)=reshape(L2a1_ford3_alt(idxalt),[1,N_a,N_bothz]); % a1primeL2ind
...
for reverse_j=1:N_j-1
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,:,ii,:,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                    d2aprimez=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                    entireRHS_ii_d3=ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprimez);
                    [~,maxindex_inner]=max(entireRHS_ii_d3,[],2);
                    midpoint(:,1,:,curra1inner,:,:,:)=maxindex_inner+(loweredge-1);
            ...
            midpoint=max(min(midpoint,N_a1-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
            aprimez=d2ind_vec + N_d2*(a1primeindexesfine-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            entireRHS_ii_d3=reshape(ReturnMatrix_L2+DiscountedEVinterp_alt(aprimez),[N_d12*n2long*N_a2,N_a,N_bothz]);
            [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
    ...
    Policy(4,:,:,jj)=reshape(joint_ford3_tilde(idx),[1,N_a,N_bothz]); % joint(a1prime midpoint,a2prime)
    Policy(5,:,:,jj)=reshape(L2a1_ford3_tilde(idx),[1,N_a,N_bothz]); % a1primeL2ind
    PolicyL2flag(1,:,:,jj)=reshape(L2flag_ford3_tilde(idx),[1,N_a,N_bothz]);

    % Max over d3 for the exponential-discounter (alt) pass
    [V_jjalt,d3_maxalt]=max(V_ford3_alt,[],3); % max over d3
    Valt(:,:,jj)=V_jjalt;
    Policyalt(3,:,:,jj)=shiftdim(d3_maxalt,-1); % d3 is just the maximising index
    Malt=N_a*N_bothz;
    d3_max_linalt=reshape(d3_maxalt,[Malt,1]);
    idxalt=(1:Malt)'+Malt*(d3_max_linalt-1);
    d12selalt=reshape(d12_ford3_alt(idxalt),[1,N_a,N_bothz]);
    Policyalt(1,:,:,jj)=rem(d12selalt-1,N_d1)+1; % d1
    Policyalt(2,:,:,jj)=ceil(d12selalt/N_d1); % d2
    Policyalt(4,:,:,jj)=reshape(joint_ford3_alt(idxalt),[1,N_a,N_bothz]); % joint(a1prime midpoint,a2prime)
    Policyalt(5,:,:,jj)=reshape(L2a1_ford3_alt(idxalt),[1,N_a,N_bothz]); % a1primeL2ind
