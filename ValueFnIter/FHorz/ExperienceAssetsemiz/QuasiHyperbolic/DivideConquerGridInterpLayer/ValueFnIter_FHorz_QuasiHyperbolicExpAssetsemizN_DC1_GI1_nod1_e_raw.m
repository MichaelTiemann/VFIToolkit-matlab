...
Policyalt=zeros(4,N_a,N_bothz,N_e,N_j,'gpuArray'); % exponential discounter optimal [d; a1prime midpoint; a1primeL2ind]
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+DiscountedEV_alt;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_V(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap_V(ii));
                            a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                            ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_V(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(d2a1primea2semiz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
                    [Vtempii,maxindexL2alt]=max(entireRHS_ii_d3ze,[],1);
                    V_ford3_alt(:,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                    d_indalt=rem(maxindexL2alt-1,N_d2)+1;
                    allindalt=d_indalt+N_d2*aind+N_d2*N_a*semizBind;
                    Policy3_ford3_alt(1,:,semizblock,e_c,d3_c)=d_indalt;
                    Policy3_ford3_alt(2,:,semizblock,e_c,d3_c)=shiftdim(squeeze(midpoint(allindalt)),-1);
                    Policy3_ford3_alt(3,:,semizblock,e_c,d3_c)=shiftdim(ceil(maxindexL2alt/N_d2),-1);
                    % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                    L2offsetalt = ceil(maxindexL2alt/N_d2);
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+DiscountedEV_tilde;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                            d2aprimez=(1:1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind;
                    ...
                    ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,2,0);
                    d2a1primea2semiz=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*semizind;
                    entireRHS_ii_d3ze=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde(d2a1primea2semiz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
