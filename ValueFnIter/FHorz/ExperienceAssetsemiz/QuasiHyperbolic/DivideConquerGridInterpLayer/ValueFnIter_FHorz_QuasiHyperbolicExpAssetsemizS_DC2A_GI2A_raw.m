...
V_ford3_under=zeros(N_a,N_bothz,N_d3,'gpuArray'); % beta-value gathered at the hat argmax, per d3
d12_ford3_hat=zeros(N_a,N_bothz,N_d3,'gpuArray');
joint_ford3_hat=zeros(N_a,N_bothz,N_d3,'gpuArray');
L2a1_ford3_hat=zeros(N_a,N_bothz,N_d3,'gpuArray');
L2flag_ford3_hat=2*ones(N_a,N_bothz,N_d3,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            [Vtempii,maxindexL2]=max(ReturnMatrix_ii,[],1);
            Vhat(:,zind,N_j)=shiftdim(Vtempii,1);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3=reshape(ReturnMatrix_ii_d3,[N_d12*n2long*N_a2,N_a,N_bothz])+beta0beta*EVfine;
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                entireRHS_ii_d3=ReturnMatrix_ii_d3+repelem(beta0beta*entireEV_z,N_d1,1,1,1,1,1,1);
                [~,maxindex1]=max(entireRHS_ii_d3,[],2);
                midpoint(:,1,:,level1ii,:,:,:)=maxindex1;
                ...
                d12_ford3_hat(:,zind,d3_c)=shiftdim(d_ind,1);
                joint_ford3_hat(:,zind,d3_c)=shiftdim(midpoint(allind)+N_a1*(maxindexL2a2-1),1);
                L2a1_ford3_hat(:,zind,d3_c)=shiftdim(maxindexL2a1,1);
                ...
                inLowerStrict=(maxindexL2a1>=2)         & (maxindexL2a1<=n2short+1);
                inUpperStrict=(maxindexL2a1>=n2short+3) & (maxindexL2a1<=n2long-1);
                L2flag_ford3_hat(:,zind,d3_c)=shiftdim(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper),1);
                % Vunderbar: the undiscounted interpolated continuation gathered at the hat argmax
    ...
    d12sel=reshape(d12_ford3_hat(idx),[1,N_a,N_bothz]);
...
for reverse_j=1:N_j-1
    ...
    EVpre=Vunderbar(:,:,jj+1); % [N_a,N_bothz]  -- continuation is Vunderbar (the beta-discounted value at the hat argmax)
