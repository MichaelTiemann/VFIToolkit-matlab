function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetsemizS_DC1_GI1_raw(n_d1,n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,N_j, d12_gridvals, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Sophisticated quasi-hyperbolic discounting + ExperienceAsset driven by semiz, Divide-and-Conquer (DC1 over a1prime) with grid interpolation layer (GI1) on a1.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % loop over z (markov), vectorize over semiz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                Policy4_ford3_hat(3,:,semizblock,d3_c)=shiftdim(squeeze(midpoint(allind)),-1); % a1prime midpoint
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            EVnodiscinterp=permute(interp1(a1_gridvals,permute(EVnodisc,[2,1,3,4,5]),a1prime_grid),[2,1,3,4,5]); % [N_d2,N_a1prime,1,N_a2,N_bothz]
            % d1-dim is implicit singleton in EVnodisc/EVnodiscinterp, broadcasts at use sites
            ...
            entireRHS_ii_d3=ReturnMatrix_ii_d3+repelem(beta0beta*EVnodisc,N_d1,1,1,1,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind; % [N_d12,maxgap+1,1,N_a2,N_bothz]; linear index into EVnodisc [N_d2,N_a1,1,N_a2,N_bothz]
                    entireRHS_ii_d3=ReturnMatrix_ii_d3+beta0beta*EVnodisc(d2aprimez);
            ...
            EVfine=reshape(EVnodiscinterp(d2a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
    ...
    elseif vfoptions.lowmemory==1 % loop over z (markov), vectorize over semiz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                EVnodisc=reshape(entireEV,[N_d2,N_a1,1,N_a2,N_semiz]); % undiscounted, beta0beta applied at use sites
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+repelem(beta0beta*EVnodisc,N_d1,1,1,1,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind; % linear index into EVnodisc [N_d2,N_a1,1,N_a2,N_semiz]
                        entireRHS_ii_d3z=ReturnMatrix_ii_d3z+beta0beta*EVnodisc(d2aprimez);
                ...
                EVfine=reshape(EVnodiscinterp(d2a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
                ...
                V_ford3_under(:,semizblock,d3_c)=V_ford3_hat(:,semizblock,d3_c)+(beta-beta0beta)*EV_at_policy;
            end
        end
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                z_val=bothz_gridvals_J(z_c,:,N_j);
                EVnodisc_z=EVnodisc(:,:,:,:,z_c);
                EVnodiscinterp_z=EVnodiscinterp(:,:,:,:,z_c);
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+repelem(beta0beta*EVnodisc_z,N_d1,1,1,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d12,maxgap+1,1,N_a2]; linear index into EVnodisc_z [N_d2,N_a1,1,N_a2]
                        entireRHS_ii_d3z=ReturnMatrix_ii_d3z+beta0beta*EVnodisc_z(d2aprime);
                        [~,maxindex]=max(entireRHS_ii_d3z,[],2);
                        midpoint(:,1,curraindex,:)=maxindex+(loweredge-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
                ...
                EVfine=reshape(EVnodiscinterp_z(d2a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+beta0beta*EVfine;
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3z,[],1);
                V_ford3_hat(:,z_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d12)+1;
