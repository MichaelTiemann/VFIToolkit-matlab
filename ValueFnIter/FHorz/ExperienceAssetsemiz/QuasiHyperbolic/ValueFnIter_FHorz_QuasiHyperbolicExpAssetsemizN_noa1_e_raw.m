...
if vfoptions.lowmemory==2
    special_n_semiz=[n_semiz,ones(1,length(n_z))]; % semiz vectorised, z scalar
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                d12_ind=rem(maxindex-1,N_d12)+1;
                Policy3alt(1,:,semizblock,e_c,N_j)=rem(d12_ind-1,N_d1)+1;
                Policy3alt(2,:,semizblock,e_c,N_j)=ceil(d12_ind/N_d1);
                Policy3alt(3,:,semizblock,e_c,N_j)=ceil(maxindex/N_d12);
            end
        end
    elseif vfoptions.lowmemory==3
        for e_c=1:N_e
    ...
    aprimeIndex_full=repmat(a2primeIndex,1,1,N_z);   % [N_d2, N_a2, N_bothz] (semiz fast, so tile over z)
    aprimeplus1Index_full=repmat(a2primeIndex+1,1,1,N_z);
    aprimeProbs_full=repmat(a2primeProbs,1,1,N_z);
    ...
    for d3_c=1:N_d3
        ...
        entireEV=EV1.*aprimeProbs_d3+EV2.*(1-aprimeProbs_d3); % [N_d2, N_a2, N_bothz]
...
for reverse_j=1:N_j-1
    ...
    aprimeIndex_full=repmat(a2primeIndex,1,1,N_z);
    aprimeplus1Index_full=repmat(a2primeIndex+1,1,1,N_z);
    aprimeProbs_full=repmat(a2primeProbs,1,1,N_z);
