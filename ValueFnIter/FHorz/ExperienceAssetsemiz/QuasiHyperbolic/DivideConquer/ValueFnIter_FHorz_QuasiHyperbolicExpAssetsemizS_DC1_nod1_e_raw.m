function [Vhat,Policy3,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetsemizS_DC1_nod1_e_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,n_e,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii_d3e=ReturnMatrix_ii_d3e+repelem(DiscountedEV_hat,1,1,1,1,1);
                entireRHS_ii_d3e_under=ReturnMatrix_ii_d3e+repelem(DiscountedEV_under,1,1,1,1,1);

                [~,maxindex1]=max(entireRHS_ii_d3e,[],2);
                entireRHS_ii_d3e_flat=reshape(entireRHS_ii_d3e,[N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz]);
                [Vtempii,maxindex2]=max(entireRHS_ii_d3e_flat,[],1);

                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                V_ford3_hat(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                entireRHS_ii_d3e_under_flat=reshape(entireRHS_ii_d3e_under,[N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz]);
                maxindexfull=maxindex2+N_d2*N_a1*(0:vfoptions.level1n*N_a2-1)+N_d2*N_a1*vfoptions.level1n*N_a2*shiftdim((0:N_bothz-1),-1);
                Vtempii_under=entireRHS_ii_d3e_under_flat(maxindexfull);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprimez=(1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3e+DiscountedEV_hat(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        entireRHS_ii_under=reshape(ReturnMatrix_ii_d3e+DiscountedEV_under(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_hat(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        ...
                        d2aprimez=(1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3e+DiscountedEV_hat(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_bothz]);
                        entireRHS_ii_under=reshape(ReturnMatrix_ii_d3e+DiscountedEV_under(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_hat(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+repelem(DiscountedEV_hat,1,1,1,1,1);
                    entireRHS_ii_d3ze_under=ReturnMatrix_ii_d3ze+repelem(DiscountedEV_under,1,1,1,1,1);

                    [~,maxindex1]=max(entireRHS_ii_d3ze,[],2);
                    entireRHS_ii_d3ze_flat=reshape(entireRHS_ii_d3ze,[N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                    [Vtempii,maxindex2]=max(entireRHS_ii_d3ze_flat,[],1);

                    curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                    V_ford3_hat(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                    entireRHS_ii_d3ze_under_flat=reshape(entireRHS_ii_d3ze_under,[N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                    maxindexfull=maxindex2+N_d2*N_a1*(0:vfoptions.level1n*N_a2-1)+N_d2*N_a1*vfoptions.level1n*N_a2*shiftdim((0:N_semiz-1),-1);
                    Vtempii_under=entireRHS_ii_d3ze_under_flat(maxindexfull);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_hat(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            entireRHS_ii_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_under(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_hat(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_hat(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
                            entireRHS_ii_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_under(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_hat(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+DiscountedEV_z_hat;
                    entireRHS_ii_d3ze_under=ReturnMatrix_ii_d3ze+DiscountedEV_z_under;

                    [~,maxindex1]=max(entireRHS_ii_d3ze,[],2);
                    entireRHS_ii_d3ze_flat=reshape(entireRHS_ii_d3ze,[N_d2*N_a1,vfoptions.level1n*N_a2]);
                    [Vtempii,maxindex2]=max(entireRHS_ii_d3ze_flat,[],1);

                    curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                    V_ford3_hat(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
                    entireRHS_ii_d3ze_under_flat=reshape(entireRHS_ii_d3ze_under,[N_d2*N_a1,vfoptions.level1n*N_a2]);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_hat(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                            entireRHS_ii_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_under(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_hat(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_hat(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                            entireRHS_ii_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_under(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_hat(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
