...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % loop over z, vectorize over semiz
        for z_c=1:N_z
            ...
            dind=rem(maxindex2-1,N_d)+1;
            d12_ind=rem(dind-1,N_d12)+1;
            Policy3alt(1,curraindex,semizblock,N_j)=rem(d12_ind-1,N_d1)+1;
            Policy3alt(2,curraindex,semizblock,N_j)=ceil(d12_ind/N_d1);
            Policy3alt(3,curraindex,semizblock,N_j)=ceil(dind/N_d12);
            Policy3alt(4,curraindex,semizblock,N_j)=ceil(maxindex2/N_d);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy3alt(2,curraindex,semizblock,N_j)=ceil(d12_ind/N_d1);
                    Policy3alt(3,curraindex,semizblock,N_j)=ceil(dind/N_d12);
                    Policy3alt(4,curraindex,semizblock,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
    ...
    elseif vfoptions.lowmemory==1 % loop over z, vectorize over semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii_d3z,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]),[],1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap_V(ii));
                        a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                        ReturnMatrix_ii_d3z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],maxgap_V(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z_dc+DiscountedEV_alt(d2aprimez),[N_d12*(maxgap_V(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                        V_ford3_alt(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dindalt=(rem(maxindexalt-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        allindalt=dindalt+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*shiftdim((0:1:N_semiz-1),-1);
                        Policy_ford3_alt(curraindex,semizblock,d3_c)=shiftdim(maxindexalt+N_d1*N_d2*(loweredge(allindalt)-1),1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
                        ReturnMatrix_ii_d3z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z_dc+DiscountedEV_alt(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                        V_ford3_alt(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dindalt=(rem(maxindexalt-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_d3z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z_dc+DiscountedEV_tilde(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_tilde(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        allind=dind+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*shiftdim((0:1:N_semiz-1),-1);
                        Policy_ford3_tilde(curraindex,semizblock,d3_c)=shiftdim(maxindex+N_d1*N_d2*(loweredge(allind)-1),1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
                        ReturnMatrix_ii_d3z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z_dc+DiscountedEV_tilde(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_tilde(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
