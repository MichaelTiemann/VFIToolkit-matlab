...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        Policy3(4,curraindex,:,N_j)=ceil(maxindex2/N_d); % a1prime
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                d12_ind=rem(dind-1,N_d12)+1;
                Policy3(1,curraindex,:,N_j)=rem(d12_ind-1,N_d1)+1;
                Policy3(2,curraindex,:,N_j)=ceil(d12_ind/N_d1);
                Policy3(3,curraindex,:,N_j)=ceil(dind/N_d12);
                Policy3(4,curraindex,:,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
            else
    ...
    elseif vfoptions.lowmemory==1 % loop over z, vectorize over semiz
        for z_c=1:N_z
            ...
            dind=rem(maxindex2-1,N_d)+1;
            d12_ind=rem(dind-1,N_d12)+1;
            Policy3(1,curraindex,semizblock,N_j)=rem(d12_ind-1,N_d1)+1;
            Policy3(2,curraindex,semizblock,N_j)=ceil(d12_ind/N_d1);
            Policy3(3,curraindex,semizblock,N_j)=ceil(dind/N_d12);
            Policy3(4,curraindex,semizblock,N_j)=ceil(maxindex2/N_d);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy3(2,curraindex,semizblock,N_j)=ceil(d12_ind/N_d1);
                    Policy3(3,curraindex,semizblock,N_j)=ceil(dind/N_d12);
                    Policy3(4,curraindex,semizblock,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for z_c=1:N_bothz
            ...
            dind=rem(maxindex2-1,N_d)+1;
            d12_ind=rem(dind-1,N_d12)+1;
            Policy3(1,curraindex,z_c,N_j)=rem(d12_ind-1,N_d1)+1;
            Policy3(2,curraindex,z_c,N_j)=ceil(d12_ind/N_d1);
            Policy3(3,curraindex,z_c,N_j)=ceil(dind/N_d12);
            Policy3(4,curraindex,z_c,N_j)=ceil(maxindex2/N_d);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy3(2,curraindex,z_c,N_j)=ceil(d12_ind/N_d1);
                    Policy3(3,curraindex,z_c,N_j)=ceil(dind/N_d12);
                    Policy3(4,curraindex,z_c,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
    ...
    aprimeIndex_full=repmat(aprimeIndex,1,1,N_z); % [N_d2*N_a1, N_a2, N_bothz]
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz, d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                    ...
                    loweredge=maxindex1(:,1,ii,:,:);
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz, d123_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
    ...
    elseif vfoptions.lowmemory==1 % loop over z, vectorize over semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+repelem(DiscountedEV_hat,N_d1,1,1,1,1);
                entireRHS_ii_d3z_under=ReturnMatrix_ii_d3z+repelem(DiscountedEV_under,N_d1,1,1,1,1);

                [~,maxindex1]=max(entireRHS_ii_d3z,[],2);
                entireRHS_ii_d3z_flat=reshape(entireRHS_ii_d3z,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                [Vtempii,maxindex2]=max(entireRHS_ii_d3z_flat,[],1);

                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                V_ford3_hat(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                entireRHS_ii_d3z_under_flat=reshape(entireRHS_ii_d3z_under,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_d3z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z+DiscountedEV_hat(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        entireRHS_ii_under=reshape(ReturnMatrix_ii_d3z+DiscountedEV_under(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_hat(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        ...
                        Policy_ford3_hat(curraindex,semizblock,d3_c)=shiftdim(maxindex+N_d1*N_d2*(loweredge(allind)-1),1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
                        ReturnMatrix_ii_d3z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z+DiscountedEV_hat(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                        entireRHS_ii_under=reshape(ReturnMatrix_ii_d3z+DiscountedEV_under(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_hat(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
            ...
            for z_c=1:N_bothz
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_bothz, d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0);
                        ...
                        loweredge=maxindex1(:,1,ii,:);
                        ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,special_n_bothz, d123_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0);
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    loweredge=maxindex1(:,1,ii,:,:);
                    ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz, d123_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_hat(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_bothz]);
                    entireRHS_ii_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_bothz]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
