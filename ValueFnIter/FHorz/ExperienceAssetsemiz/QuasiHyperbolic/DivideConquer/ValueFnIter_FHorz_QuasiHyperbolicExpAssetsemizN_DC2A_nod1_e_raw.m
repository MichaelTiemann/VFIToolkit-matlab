...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        Policy3alt(3,curraindex,:,:,N_j)=ceil(maxindex2/N_d); % joint(a1prime,a2prime)
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                Policy3alt(1,curraindex,:,:,N_j)=rem(dind-1,N_d2)+1; % d2
                Policy3alt(2,curraindex,:,:,N_j)=ceil(dind/N_d2); % d3
                Policy3alt(3,curraindex,:,:,N_j)=a1prime_rec+N_a1*(a2pind-1); % joint(a1prime,a2prime)
            else
                ...
                Policy3alt(1,curraindex,:,:,N_j)=rem(dind-1,N_d2)+1; % d2
                Policy3alt(2,curraindex,:,:,N_j)=ceil(dind/N_d2); % d3
                Policy3alt(3,curraindex,:,:,N_j)=loweredge(loweredge_idx)+N_a1*(a2pind-1); % joint(a1prime,a2prime)
            end
        end
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Policy3alt(3,curraindex,:,e_c,N_j)=ceil(maxindex2/N_d); % joint(a1prime,a2prime)
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy3alt(1,curraindex,:,e_c,N_j)=rem(dind-1,N_d2)+1; % d2
                    Policy3alt(2,curraindex,:,e_c,N_j)=ceil(dind/N_d2); % d3
                    Policy3alt(3,curraindex,:,e_c,N_j)=a1prime_rec+N_a1*(a2pind-1); % joint(a1prime,a2prime)
                else
                    loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                    ...
                    Policy3alt(1,curraindex,:,e_c,N_j)=rem(dind-1,N_d2)+1; % d2
                    Policy3alt(2,curraindex,:,e_c,N_j)=ceil(dind/N_d2); % d3
                    Policy3alt(3,curraindex,:,e_c,N_j)=loweredge(loweredge_idx)+N_a1*(a2pind-1); % joint(a1prime,a2prime)
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy3alt(3,curraindex,zind,e_c,N_j)=ceil(maxindex2/N_d); % joint(a1prime,a2prime)
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policy3alt(1,curraindex,zind,e_c,N_j)=rem(dind-1,N_d2)+1; % d2
                        Policy3alt(2,curraindex,zind,e_c,N_j)=ceil(dind/N_d2); % d3
                        Policy3alt(3,curraindex,zind,e_c,N_j)=a1prime_rec+N_a1*(a2pind-1); % joint(a1prime,a2prime)
                    else
                        loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                        ...
                        Policy3alt(1,curraindex,zind,e_c,N_j)=rem(dind-1,N_d2)+1; % d2
                        Policy3alt(2,curraindex,zind,e_c,N_j)=ceil(dind/N_d2); % d3
                        Policy3alt(3,curraindex,zind,e_c,N_j)=loweredge(loweredge_idx)+N_a1*(a2pind-1); % joint(a1prime,a2prime)
                    end
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==3
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                ...
                Policy3alt(3,curraindex,z_c,e_c,N_j)=ceil(maxindex2/N_d); % joint(a1prime,a2prime)
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policy3alt(1,curraindex,z_c,e_c,N_j)=rem(dind-1,N_d2)+1; % d2
                        Policy3alt(2,curraindex,z_c,e_c,N_j)=ceil(dind/N_d2); % d3
                        Policy3alt(3,curraindex,z_c,e_c,N_j)=a1prime_rec+N_a1*(a2pind-1); % joint(a1prime,a2prime)
                    else
                        loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                        ...
                        Policy3alt(1,curraindex,z_c,e_c,N_j)=rem(dind-1,N_d2)+1; % d2
                        Policy3alt(2,curraindex,z_c,e_c,N_j)=ceil(dind/N_d2); % d3
                        Policy3alt(3,curraindex,z_c,e_c,N_j)=loweredge(loweredge_idx)+N_a1*(a2pind-1); % joint(a1prime,a2prime)
                    end
                end
            end
        end
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprimez=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_alt(d2aprimez),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_alt(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        dind      =rem(maxindex-1,N_d2)+1;
                        a1localind=rem(floor((maxindex-1)/N_d2),maxgap(ii)+1)+1;
                        ...
                        d2aprimez=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_alt(d2aprimez),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_alt(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        dind   =rem(maxindex-1,N_d2)+1;
                        a2pind =floor((maxindex-1)/N_d2)+1;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                        d2aprimez=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_tilde(d2aprimez),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_tilde(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        dind      =rem(maxindex-1,N_d2)+1;
                        a1localind=rem(floor((maxindex-1)/N_d2),maxgap(ii)+1)+1;
                        ...
                        ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d123_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                        d2aprimez=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_tilde(d2aprimez),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_tilde(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        dind   =rem(maxindex-1,N_d2)+1;
                        a2pind =floor((maxindex-1)/N_d2)+1;
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_z_alt(d2aprimez),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_alt(curraindex,zind,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind      =rem(maxindex-1,N_d2)+1;
                            a1localind=rem(floor((maxindex-1)/N_d2),maxgap(ii)+1)+1;
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_z_alt(d2aprimez),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_alt(curraindex,zind,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind   =rem(maxindex-1,N_d2)+1;
                            a2pind =floor((maxindex-1)/N_d2)+1;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_semiz, special_n_e, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                            d2aprimez=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_semiz-1),-5);
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_z_tilde(d2aprimez),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_tilde(curraindex,zind,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind      =rem(maxindex-1,N_d2)+1;
                            a1localind=rem(floor((maxindex-1)/N_d2),maxgap(ii)+1)+1;
                            ...
                            ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_semiz, special_n_e, d123_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                            d2aprimez=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_semiz-1),-5);
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_z_tilde(d2aprimez),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_tilde(curraindex,zind,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind   =rem(maxindex-1,N_d2)+1;
                            a2pind =floor((maxindex-1)/N_d2)+1;
                ...
                for e_c=1:N_e
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_z_alt(d2aprime),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_alt(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind      =rem(maxindex-1,N_d2)+1;
                            a1localind=rem(floor((maxindex-1)/N_d2),maxgap(ii)+1)+1;
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_z_alt(d2aprime),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_alt(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind   =rem(maxindex-1,N_d2)+1;
                            a2pind =floor((maxindex-1)/N_d2)+1;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d123_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                            d2aprime=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_z_tilde(d2aprime),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_tilde(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind      =rem(maxindex-1,N_d2)+1;
                            a1localind=rem(floor((maxindex-1)/N_d2),maxgap(ii)+1)+1;
                            ...
                            ReturnMatrix_ii_d3=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d123_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                            d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3+DiscountedEV_z_tilde(d2aprime),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_tilde(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind   =rem(maxindex-1,N_d2)+1;
                            a2pind =floor((maxindex-1)/N_d2)+1;
    ...
    Policy3alt(1,:,:,:,N_j)=rem(d2aprime_ind-1,N_d2)+1; % d2
    Policy3alt(3,:,:,:,N_j)=ceil(d2aprime_ind/N_d2); % joint(a1prime,a2prime)
...
for reverse_j=1:N_j-1
    ...
    maxindex=reshape(maxindex,[N_a*N_semiz*N_z*N_e,1]); % This is the value of d3 that corresponds, make it this shape for addition just below
    d2aprime_ind=reshape(Policy_ford3_alt((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex-1)),[1,N_a,N_semiz*N_z,N_e]);
    Policy3alt(1,:,:,:,jj)=rem(d2aprime_ind-1,N_d2)+1; % d2
    Policy3alt(3,:,:,:,jj)=ceil(d2aprime_ind/N_d2); % joint(a1prime,a2prime)
    % Max over d3 for tilde (QH-perceived), and keep the policy that corresponded to that
    % Now we just max over d3, and keep the policy that corresponded to that (including modify the policy to include the d3 decision)
    [V_jj,maxindex]=max(V_ford3_tilde,[],4); % max over d3
    Vtilde(:,:,:,jj)=V_jj;
