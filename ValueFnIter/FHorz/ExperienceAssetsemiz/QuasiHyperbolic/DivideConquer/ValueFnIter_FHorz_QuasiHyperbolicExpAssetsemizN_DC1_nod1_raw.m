function [Vtilde,Policy3,Valt,Policy3alt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetsemizN_DC1_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        Policy3alt(3,curraindex,:,N_j)=ceil(maxindex2/N_d23); % a1prime
    ...
    elseif vfoptions.lowmemory==1 % loop over z, vectorize over semiz
        for z_c=1:N_z
            ...
            dind=rem(maxindex2-1,N_d23)+1;
            Policy3alt(1,curraindex,semizblock,N_j)=rem(dind-1,N_d2)+1;
            Policy3alt(2,curraindex,semizblock,N_j)=ceil(dind/N_d2);
            Policy3alt(3,curraindex,semizblock,N_j)=ceil(maxindex2/N_d23);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy3alt(1,curraindex,semizblock,N_j)=rem(dind-1,N_d2)+1;
                    Policy3alt(2,curraindex,semizblock,N_j)=ceil(dind/N_d2);
                    Policy3alt(3,curraindex,semizblock,N_j)=ceil(maxindex/N_d23+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    ReturnMatrix_ii_d3_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                    ...
                    a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                    allindalt=dindalt+N_d2*a2ind+N_d2*N_a2*bothzind;
                    Policy_ford3_alt(curraindex,:,d3_c)=shiftdim(maxindexalt+N_d2*(loweredge(allindalt)-1),1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
                    ReturnMatrix_ii_d3_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_d3_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                    d2aprimez=(1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
    ...
    elseif vfoptions.lowmemory==1 % loop over z, vectorize over semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+repelem(DiscountedEV_alt,1,1,1,1,1);

                [~,maxindex1]=max(entireRHS_ii_d3z,[],2);
                [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii_d3z,[N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]),[],1);

                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                V_ford3_alt(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                Policy_ford3_alt(curraindex,semizblock,d3_c)=shiftdim(maxindex2alt,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap_V(ii));
                        a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                        ReturnMatrix_ii_d3z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap_V(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=(1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z_dc+DiscountedEV_alt(d2aprimez),[N_d2*(maxgap_V(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                        V_ford3_alt(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dindalt=(rem(maxindexalt-1,N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        allindalt=dindalt+N_d2*a2ind+N_d2*N_a2*shiftdim((0:1:N_semiz-1),-1);
                        Policy_ford3_alt(curraindex,semizblock,d3_c)=shiftdim(maxindexalt+N_d2*(loweredge(allindalt)-1),1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
                        ReturnMatrix_ii_d3z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d23_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=(1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z_dc+DiscountedEV_alt(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                        V_ford3_alt(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dindalt=(rem(maxindexalt-1,N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+repelem(DiscountedEV_tilde,1,1,1,1,1);

                [~,maxindex1]=max(entireRHS_ii_d3z,[],2);
                [Vtempii,maxindex2]=max(reshape(entireRHS_ii_d3z,[N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]),[],1);

                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                V_ford3_tilde(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                Policy_ford3_tilde(curraindex,semizblock,d3_c)=shiftdim(maxindex2,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_d3z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=(1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z_dc+DiscountedEV_tilde(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_tilde(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        allind=dind+N_d2*a2ind+N_d2*N_a2*shiftdim((0:1:N_semiz-1),-1);
                        Policy_ford3_tilde(curraindex,semizblock,d3_c)=shiftdim(maxindex+N_d2*(loweredge(allind)-1),1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
                        ReturnMatrix_ii_d3z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d23_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0);
                        d2aprimez=(1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z_dc+DiscountedEV_tilde(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_tilde(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                entireRHS_ii_z=ReturnMatrix_ii_z+repelem(DiscountedEV_z_alt,1,1,1,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),N_a1-maxgap_V(ii));
                        a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                        ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap_V(ii)+1,level1iidiff(ii),n_a2,special_n_bothz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0);
                        ...
                        loweredge=maxindex1(:,1,ii,:);
                        ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],1,level1iidiff(ii),n_a2,special_n_bothz, d23_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0);
                ...
                entireRHS_ii_z=ReturnMatrix_ii_z+repelem(DiscountedEV_z_tilde,1,1,1,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),N_a1-maxgap(ii));
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_bothz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, ReturnFnParamsVec,3,0);
                        d2aprime=(1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2);
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap_V(ii));
                    a1primeindexes=loweredge+(0:1:maxgap_V(ii));
                    ReturnMatrix_ii_d3_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap_V(ii)+1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    d2aprimez=(1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3_dc+DiscountedEV_alt(d2aprimez),[N_d2*(maxgap_V(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                    [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                    V_ford3_alt(curraindex,:,d3_c)=shiftdim(Vtempii,1);
                    ...
                    loweredge=maxindex1(:,1,ii,:,:);
                    ReturnMatrix_ii_d3_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    d2aprimez=(1:N_d2)'+N_d2*(loweredge-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3_dc+DiscountedEV_alt(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_bothz]);
                    [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                    V_ford3_alt(curraindex,:,d3_c)=shiftdim(Vtempii,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                    a1primeindexes=loweredge+(0:1:maxgap(ii));
                    ReturnMatrix_ii_d3_dc=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,n_bothz, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    d2aprimez=(1:N_d2)'+N_d2*(a1primeindexes-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                    entireRHS_ii=reshape(ReturnMatrix_ii_d3_dc+DiscountedEV_tilde(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford3_tilde(curraindex,:,d3_c)=shiftdim(Vtempii,1);
