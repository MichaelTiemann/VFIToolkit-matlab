...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3_under=ReturnMatrix_ii_d3+repelem(DiscountedEV_under,N_d1,1,1,1,1); % broadcasts over e

            [~,maxindex1]=max(entireRHS_ii_d3,[],2);
            entireRHS_ii_d3_flat=reshape(entireRHS_ii_d3,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz,N_e]);
            [Vtempii,maxindex2]=max(entireRHS_ii_d3_flat,[],1);

            curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
            V_ford3_hat(curraindex,:,:,d3_c)=shiftdim(Vtempii,1);
            entireRHS_ii_d3_under_flat=reshape(entireRHS_ii_d3_under,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz,N_e]);
            maxindexfull=maxindex2+N_d1*N_d2*N_a1*(0:vfoptions.level1n*N_a2-1)+N_d1*N_d2*N_a1*vfoptions.level1n*N_a2*shiftdim((0:N_bothz-1),-1)+N_d1*N_d2*N_a1*vfoptions.level1n*N_a2*N_bothz*shiftdim((0:N_e-1),-2);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii_d3e_under=ReturnMatrix_ii_d3e+repelem(DiscountedEV_under,N_d1,1,1,1,1);

                [~,maxindex1]=max(entireRHS_ii_d3e,[],2);
                entireRHS_ii_d3e_flat=reshape(entireRHS_ii_d3e,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz]);
                [Vtempii,maxindex2]=max(entireRHS_ii_d3e_flat,[],1);

                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                V_ford3_hat(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                entireRHS_ii_d3e_under_flat=reshape(entireRHS_ii_d3e_under,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz]);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii_under=reshape(ReturnMatrix_ii_d3e+DiscountedEV_under(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_hat(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
                        ...
                        entireRHS_ii_under=reshape(ReturnMatrix_ii_d3e+DiscountedEV_under(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_hat(curraindex,:,e_c,d3_c)=shiftdim(Vtempii,1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_d3ze_under=ReturnMatrix_ii_d3ze+repelem(DiscountedEV_under,N_d1,1,1,1,1);

                    [~,maxindex1]=max(entireRHS_ii_d3ze,[],2);
                    entireRHS_ii_d3ze_flat=reshape(entireRHS_ii_d3ze,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                    [Vtempii,maxindex2]=max(entireRHS_ii_d3ze_flat,[],1);

                    curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                    V_ford3_hat(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                    entireRHS_ii_d3ze_under_flat=reshape(entireRHS_ii_d3ze_under,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_hat(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            entireRHS_ii_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_under(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_hat(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_hat(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                            entireRHS_ii_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_under(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_hat(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii,1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_d3ze_under=ReturnMatrix_ii_d3ze+repelem(DiscountedEV_z_under,N_d1,1,1,1,1);

                    [~,maxindex1]=max(entireRHS_ii_d3ze,[],2);
                    entireRHS_ii_d3ze_flat=reshape(entireRHS_ii_d3ze,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2]);
                    [Vtempii,maxindex2]=max(entireRHS_ii_d3ze_flat,[],1);

                    curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                    V_ford3_hat(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
                    entireRHS_ii_d3ze_under_flat=reshape(entireRHS_ii_d3ze_under,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2]);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_under(d2aprime),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_hat(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
                            ...
                            entireRHS_ii_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_under(d2aprime),[N_d12,level1iidiff(ii)*N_a2]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            V_ford3_hat(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii,1);
