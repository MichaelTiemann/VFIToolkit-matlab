...
% z is ordinary exogenous markov state (present), semiz is semi-exog state
% aprimeFn = aprimeFn(d2, a2, semiz, ...) -- the experience asset is driven by semiz
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        Policy3alt(4,:,:,N_j)=ceil(maxindex/N_d); % a1prime
    elseif vfoptions.lowmemory==1 % loop over z (markov), vectorize over semiz
        for z_c=1:N_z
            ...
            d_ind=rem(maxindex-1,N_d)+1;
            d12_ind=rem(d_ind-1,N_d12)+1;
            Policy3alt(1,:,semizblock,N_j)=rem(d12_ind-1,N_d1)+1;
            Policy3alt(2,:,semizblock,N_j)=ceil(d12_ind/N_d1);
            Policy3alt(3,:,semizblock,N_j)=ceil(d_ind/N_d12);
            Policy3alt(4,:,semizblock,N_j)=ceil(maxindex/N_d);
        end
    ...
    % aprime depends on (d2, a1, a2, current_semiz); independent of d1, d3 and z -- compute once
    ...
    aprimeIndex=repelem((1:1:N_a1)',N_d2,N_a2,N_semiz)+N_a1*repmat(a2primeIndex-1,N_a1,1,1); % [N_d2*N_a1, N_a2, N_semiz]
    aprimeplus1Index=repelem((1:1:N_a1)',N_d2,N_a2,N_semiz)+N_a1*repmat(a2primeIndex,N_a1,1,1);
    aprimeProbs_d2a1a2semiz=repmat(a2primeProbs,N_a1,1,1); % [N_d2*N_a1, N_a2, N_semiz]
    % Expand to current_bothz = (semiz, z) with semiz fastest; aprime depends on semiz, so tile over z
