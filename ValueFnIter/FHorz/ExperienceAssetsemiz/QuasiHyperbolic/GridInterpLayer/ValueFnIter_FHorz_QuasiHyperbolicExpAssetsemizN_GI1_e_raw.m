...
Policyalt=zeros(5,N_a,N_bothz,N_e,N_j,'gpuArray'); % exponential discounter optimal choice
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % midpoint indexing with e (5D): need a separate path; just store via shiftdim
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_d3=ReturnMatrix_d3+beta*undiscountedEV; % broadcasts over e
            ...
            entireRHS_ii_alt=ReturnMatrix_ii_alt+beta*reshape(undiscountedEVinterp(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz,N_e]); % midpoint (hence gather) is e-dependent at lowmemory=0
            ...
            entireRHS_d3=ReturnMatrix_d3+beta0beta*undiscountedEV; % broadcasts over e
            ...
            d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*bothzind;
            entireRHS_ii_tilde=ReturnMatrix_ii_tilde+beta0beta*reshape(undiscountedEVinterp(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz,N_e]); % midpoint (hence gather) is e-dependent at lowmemory=0
    ...
    elseif vfoptions.lowmemory==3 % joint loop over bothz, inner e
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                z_val=bothz_gridvals_J(z_c,:,N_j);
                undiscountedEV_z=repelem(undiscountedEV(:,:,:,:,z_c),N_d1,1);
                undiscountedEVinterp_z=repelem(undiscountedEVinterp(:,:,:,:,z_c),N_d1,1);
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_d3ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,n_a1,n_a2,special_n_bothz,special_n_e, d123_gridvals_val, a1_gridvals, a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
    ...
    % Max over d3 (dim 4 of V_ford3_tilde since e is dim 3)
    [V_jj,maxindex]=max(V_ford3_tilde,[],4);
    Vtilde(:,:,:,N_j)=V_jj;
    Policy(3,:,:,:,N_j)=shiftdim(maxindex,-1);
    maxindex=reshape(maxindex,[N_a*N_bothz*N_e,1]);
