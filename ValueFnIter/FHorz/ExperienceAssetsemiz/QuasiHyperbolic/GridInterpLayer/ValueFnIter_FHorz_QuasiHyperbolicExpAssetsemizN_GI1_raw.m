...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % loop over z, vectorize semiz
        for d3_c=1:N_d3
            d123_gridvals_val=[d12_gridvals,repelem(d3_grid(d3_c),N_d12,1)];
            for z_c=1:N_z
                ...
                Policy4_ford3_tilde(1,:,semizblock,d3_c)=rem(d_ind-1,N_d1)+1;
                Policy4_ford3_tilde(2,:,semizblock,d3_c)=ceil(d_ind/N_d1);
                Policy4_ford3_tilde(3,:,semizblock,d3_c)=shiftdim(squeeze(midpoint(allind)),-1);
                Policy4_ford3_tilde(4,:,semizblock,d3_c)=shiftdim(ceil(maxindexL2/N_d12),-1);
                L2offset=ceil(maxindexL2/N_d12);
                ...
                inLowerStrict=(L2offset>=2)&(L2offset<=n2short+1);
                inUpperStrict=(L2offset>=n2short+3)&(L2offset<=n2long-1);
                flag_ford3_tilde(:,semizblock,d3_c)=shiftdim(2+(inLowerStrict&isInfLower)-(inUpperStrict&isInfUpper),1);
            end
        end
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            undiscountedEV=repelem(undiscountedEV,N_d1,1);
            undiscountedEVinterp=repelem(undiscountedEVinterp,N_d1,1);
            ...
            midpoint=max(min(maxindex,n_a1(1)-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,n_bothz, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
            d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*bothzind;
            entireRHS_ii_alt=ReturnMatrix_ii_alt+beta*reshape(undiscountedEVinterp(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
            ...
            midpoint=max(min(maxindex,n_a1(1)-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_tilde=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,n_bothz, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
            d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*bothzind;
            entireRHS_ii_tilde=ReturnMatrix_ii_tilde+beta0beta*reshape(undiscountedEVinterp(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
    ...
    elseif vfoptions.lowmemory==1 % loop over z, vectorize semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                % alt pass (exponential discounter): F + beta*EV
                entireRHS_d3z=ReturnMatrix_d3z+beta*undiscountedEV;

                [~,maxindex]=max(entireRHS_d3z,[],2);

                midpoint=max(min(maxindex,n_a1(1)-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_valblock, ReturnFnParamsVec,2,0);
                d12a1primea2semiz=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*semizind;
                entireRHS_ii_alt=ReturnMatrix_ii_alt+beta*reshape(undiscountedEVinterp(d12a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_alt,[],1);
                V_ford3_alt(:,semizblock,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d12)+1;
                allind=d_ind+N_d12*aind+N_d12*N_a*semizBind;
                Policy4_ford3_alt(1,:,semizblock,d3_c)=rem(d_ind-1,N_d1)+1;
                Policy4_ford3_alt(2,:,semizblock,d3_c)=ceil(d_ind/N_d1);
                Policy4_ford3_alt(3,:,semizblock,d3_c)=shiftdim(squeeze(midpoint(allind)),-1);
                Policy4_ford3_alt(4,:,semizblock,d3_c)=shiftdim(ceil(maxindexL2/N_d12),-1);
                L2offset=ceil(maxindexL2/N_d12);
                linidx_lower=d_ind+N_d12*n2long*aind+N_d12*n2long*N_a*semizBind;
                linidx_upper=d_ind+N_d12*(n2long-1)+N_d12*n2long*aind+N_d12*n2long*N_a*semizBind;
                isInfLower=(ReturnMatrix_ii_alt(linidx_lower)==-Inf);
                ...
                % tilde pass (QH-perceived): F + beta0*beta*EV
                entireRHS_d3z=ReturnMatrix_d3z+beta0beta*undiscountedEV;

                [~,maxindex]=max(entireRHS_d3z,[],2);

                midpoint=max(min(maxindex,n_a1(1)-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_tilde=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_valblock, ReturnFnParamsVec,2,0);
                d12a1primea2semiz=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*semizind;
                entireRHS_ii_tilde=ReturnMatrix_ii_tilde+beta0beta*reshape(undiscountedEVinterp(d12a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_tilde,[],1);
                V_ford3_tilde(:,semizblock,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d12)+1;
                allind=d_ind+N_d12*aind+N_d12*N_a*semizBind;
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                % alt pass (exponential discounter): F + beta*EV
                entireRHS_d3z=ReturnMatrix_d3z+beta*undiscountedEV_z;

                [~,maxindex]=max(entireRHS_d3z,[],2);

                midpoint=max(min(maxindex,n_a1(1)-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,special_n_bothz, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, ReturnFnParamsVec,2,0);
                d12a1primea2=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind;
                entireRHS_ii_alt=ReturnMatrix_ii_alt+beta*reshape(undiscountedEVinterp_z(d12a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_alt,[],1);
                V_ford3_alt(:,z_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d12)+1;
                allind=d_ind+N_d12*aind;
                Policy4_ford3_alt(1,:,z_c,d3_c)=rem(d_ind-1,N_d1)+1;
                Policy4_ford3_alt(2,:,z_c,d3_c)=ceil(d_ind/N_d1);
                Policy4_ford3_alt(3,:,z_c,d3_c)=shiftdim(squeeze(midpoint(allind)),-1);
                Policy4_ford3_alt(4,:,z_c,d3_c)=shiftdim(ceil(maxindexL2/N_d12),-1);
                L2offset=ceil(maxindexL2/N_d12);
                linidx_lower=d_ind+N_d12*n2long*aind;
                linidx_upper=d_ind+N_d12*(n2long-1)+N_d12*n2long*aind;
                isInfLower=(ReturnMatrix_ii_alt(linidx_lower)==-Inf);
                ...
                % tilde pass (QH-perceived): F + beta0*beta*EV
                entireRHS_d3z=ReturnMatrix_d3z+beta0beta*undiscountedEV_z;

                [~,maxindex]=max(entireRHS_d3z,[],2);

                midpoint=max(min(maxindex,n_a1(1)-1),2);
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_tilde=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,special_n_bothz, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, ReturnFnParamsVec,2,0);
                d12a1primea2=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind;
                entireRHS_ii_tilde=ReturnMatrix_ii_tilde+beta0beta*reshape(undiscountedEVinterp_z(d12a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_tilde,[],1);
                V_ford3_tilde(:,z_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d12)+1;
                allind=d_ind+N_d12*aind;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            midpoint=max(min(maxindex,n_a1(1)-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,n_bothz, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0);
            d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*bothzind;
            entireRHS_ii_alt=ReturnMatrix_ii_alt+beta*reshape(undiscountedEVinterp(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
            [Vtempii,maxindexL2]=max(entireRHS_ii_alt,[],1);
            V_ford3_alt(:,:,d3_c)=shiftdim(Vtempii,1);
            ...
            midpoint=max(min(maxindex,n_a1(1)-1),2);
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_tilde=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,n_bothz, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0);
            d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*bothzind;
            entireRHS_ii_tilde=ReturnMatrix_ii_tilde+beta0beta*reshape(undiscountedEVinterp(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
            [Vtempii,maxindexL2]=max(entireRHS_ii_tilde,[],1);
            V_ford3_tilde(:,:,d3_c)=shiftdim(Vtempii,1);
