function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetN_DC1_GI1_noz_e_raw(n_d1, n_d2,n_a1,n_a2,n_e,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid,e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV_alt=beta*reshape(EV,[N_d2,N_a1,1,N_a2]); % exponential
    DiscountedEV_tilde=beta0beta*reshape(EV,[N_d2,N_a1,1,N_a2]); % QH-perceived
    % Interpolate EV over aprime_grid
    DiscountedEVinterp_alt=permute(interp1(a1_gridvals,permute(DiscountedEV_alt,[2,1,3,4]),a1prime_grid),[2,1,3,4]);   % [N_d2,N_a1prime,1,N_a2]
    DiscountedEVinterp_tilde=permute(interp1(a1_gridvals,permute(DiscountedEV_tilde,[2,1,3,4]),a1prime_grid),[2,1,3,4]);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d,maxgap_V+1,1,N_a2,N_e]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2]
        ...
        ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,n2long,n_a1,n_a2,n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
        da1primea2=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d,n2long,N_a1,N_a2,N_e]; linear index into DiscountedEVinterp_alt [N_d2,N_a1prime,1,N_a2]
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d,maxgap+1,1,N_a2,N_e]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2]
        ...
        % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,n2long,n_a1,n_a2,n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
        da1primea2=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d,n2long,N_a1,N_a2,N_e]; linear index into DiscountedEVinterp_tilde [N_d2,N_a1prime,1,N_a2]
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d,maxgap_V+1,1,N_a2]; linear index into DiscountedEV_alt [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime);
            ...
            da1primea2=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d,n2long,N_a1,N_a2]; linear index into DiscountedEVinterp_alt [N_d2,N_a1prime,1,N_a2]
            entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(da1primea2),[N_d*n2long,N_a1*N_a2]);
            [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
            Valt(:,e_c,N_j)=shiftdim(Vtempii,1);
            d_indalt=rem(maxindexL2alt-1,N_d)+1;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d,maxgap+1,1,N_a2]; linear index into DiscountedEV_tilde [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_e=ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime);
                    [~,maxindex]=max(entireRHS_ii_e,[],2);
                    midpoint(:,1,curraindex,:)=maxindex+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
            ...
            % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2
            ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,n2long,n_a1,n_a2,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_val, ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
            da1primea2=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d,n2long,N_a1,N_a2]; linear index into DiscountedEVinterp_tilde [N_d2,N_a1prime,1,N_a2]
            entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_tilde(da1primea2),[N_d*n2long,N_a1*N_a2]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
...
for reverse_j=1:N_j-1
    ...
    EV=sum(pi_e_J(:,jj+1)'.*Valt(:,:,jj+1),2);    % Expectations over e
    ...
    if vfoptions.lowmemory==0
        ...
        a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint, fine index
        % aprime possibilities are n_d-by-n2long-by-n_a1-by-n_a2-by-n_e
        ReturnMatrix_L2=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d2,n2long,n_a1,n_a2,n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, e_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
        da1primea2=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind; % [N_d,n2long,N_a1,N_a2,N_e]; linear index into DiscountedEVinterp_alt [N_d2,N_a1prime,1,N_a2]
        entireRHS_ii=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(da1primea2),[N_d*n2long,N_a1*N_a2,N_e]);
        [Vtempii,maxindexL2alt]=max(entireRHS_ii,[],1);
