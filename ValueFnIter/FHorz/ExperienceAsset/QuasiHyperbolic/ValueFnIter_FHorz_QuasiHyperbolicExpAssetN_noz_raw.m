function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetN_noz_raw(n_d1, n_d2,n_a1,n_a2,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    entireRHS_alt=ReturnMatrix+beta*repelem(EV,N_d1,N_a1);
    [Vtemp_alt,maxindex_alt]=max(entireRHS_alt,[],1);
    Valt(:,N_j)=shiftdim(Vtemp_alt,1);
    Policyalt(:,N_j)=shiftdim(maxindex_alt,1);
    entireRHS_tilde=ReturnMatrix+beta0beta*repelem(EV,N_d1,N_a1);
...
for reverse_j=1:N_j-1
    ...
    if length(n_a2)==1
        ...
        aprime_lh=a1prime_offsets+N_a1*repmat(loIdx_1+n_a2_1*loIdx_2-1,N_a1,1);
        aprime_hh=a1prime_offsets+N_a1*repmat((loIdx_1+1)+n_a2_1*loIdx_2-1,N_a1,1);
        V_ll=reshape(Valt(aprime_ll(:),jj+1),[N_d2*N_a1,N_a2]);
        V_hl=reshape(Valt(aprime_hl(:),jj+1),[N_d2*N_a1,N_a2]);
        V_lh=reshape(Valt(aprime_lh(:),jj+1),[N_d2*N_a1,N_a2]);
        V_hh=reshape(Valt(aprime_hh(:),jj+1),[N_d2*N_a1,N_a2]);
