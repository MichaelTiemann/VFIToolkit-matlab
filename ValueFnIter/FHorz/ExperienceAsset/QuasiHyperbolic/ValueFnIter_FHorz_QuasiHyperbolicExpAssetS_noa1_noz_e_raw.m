function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetS_noa1_noz_e_raw(n_d1,n_d2,n_a2,n_e,N_j, d_gridvals, d2_gridvals, a2_grid,e_gridvals_J,pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_hat=ReturnMatrix+beta0beta*repelem(EV,N_d1,1,1);
        [Vtemp,maxindex]=max(entireRHS_hat,[],1);
        entireRHS_under=ReturnMatrix+beta*repelem(EV,N_d1,1,1);
        maxindexfull=maxindex+N_d12*(0:1:N_a-1)+shiftdim(N_d12*N_a*(0:1:N_e-1),-1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            % hat: argmax of F + beta0*beta*EV; under: the beta-RHS gathered at that argmax
            entireRHS_hat=ReturnMatrix_e+beta0beta*repelem(EV,N_d1,1);
            [Vtemp,maxindex]=max(entireRHS_hat,[],1);
            entireRHS_under=ReturnMatrix_e+beta*repelem(EV,N_d1,1);
...
for reverse_j=1:N_j-1
    ...
    EVpre=sum(pi_e_J(:,jj+1)'.*Vunderbar(:,:,jj+1),2);
