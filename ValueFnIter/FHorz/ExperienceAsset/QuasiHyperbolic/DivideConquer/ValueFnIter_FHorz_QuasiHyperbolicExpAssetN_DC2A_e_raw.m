function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetN_DC2A_e_raw(n_d1, n_d2, n_a1, n_a2, n_a3, n_z, n_e, N_j, d_gridvals, d2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
% with-d1 _e analog of ValueFnIter_FHorz_ExpAsset_DC2A_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,N_e]),[],1);
        curraindex=repmat(level1ii',N_a2*N_a3,1) +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
        dindalt   =rem(maxindex2alt-1,N_d)+1;
        a1pindalt =rem(floor((maxindex2alt-1)/N_d),N_a1)+1;
        a2pindalt =floor((maxindex2alt-1)/(N_d*N_a1))+1;
        Valt(curraindex,:,:,N_j)       =shiftdim(Vtempii,1);
        Policyalt(1,curraindex,:,:,N_j)=dindalt;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprimez),[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                dindalt      =rem(maxindexalt-1,N_d)+1;
                a1localindalt=rem(floor((maxindexalt-1)/N_d),maxgap_V(ii)+1)+1;
                a2pindalt    =floor((maxindexalt-1)/(N_d*(maxgap_V(ii)+1)))+1;
                a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                loweredge_idxalt=dindalt + N_d*(a2pindalt-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat + N_d*N_a2*N_a2*N_a3*zind + N_d*N_a2*N_a2*N_a3*N_z*eind;
                ...
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprimez),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                dindalt   =rem(maxindexalt-1,N_d)+1;
                a2pindalt =floor((maxindexalt-1)/N_d)+1;
                a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                loweredge_idxalt=dindalt + N_d*(a2pindalt-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat + N_d*N_a2*N_a2*N_a3*zind + N_d*N_a2*N_a2*N_a3*N_z*eind;
                Policyalt(1,curraindex,:,:,N_j)=dindalt;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d2aprimez=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprimez),[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d2aprimez=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprimez),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_z,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                dind   =rem(maxindex-1,N_d)+1;
                a2pind =floor((maxindex-1)/N_d)+1;
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_val, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 1);
            %% Valt (beta)
            entireRHS_ii_z=ReturnMatrix_ii_z+repelem(DiscountedEV_alt_z,N_d1,1,1,1,1,1);
            [~,maxindex1]=max(entireRHS_ii_z,[],2);
            [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii_z,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,1,N_e]),[],1);
            curraindex=repmat(level1ii',N_a2*N_a3,1) +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
            dindalt   =rem(maxindex2alt-1,N_d)+1;
            a1pindalt =rem(floor((maxindex2alt-1)/N_d),N_a1)+1;
            a2pindalt =floor((maxindex2alt-1)/(N_d*N_a1))+1;
            Valt(curraindex,z_c,:,N_j)       =shiftdim(Vtempii,1);
            Policyalt(1,curraindex,z_c,:,N_j)=dindalt;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    entireRHS_ii_z=reshape(ReturnMatrix_ii_z_dc+DiscountedEV_alt_z(d2aprime),[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,1,N_e]);
                    ...
                    a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                    a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                    loweredge_idxalt=dindalt + N_d*(a2pindalt-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat + N_d*N_a2*N_a2*N_a3*eind;
                    a1prime_recalt=a1localindalt+loweredge(loweredge_idxalt)-1;
                    Policyalt(1,curraindex,z_c,:,N_j)=dindalt;
                    ...
                    loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                    ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                    d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_z=reshape(ReturnMatrix_ii_z_dc+DiscountedEV_alt_z(d2aprime),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,1,N_e]);
                    [Vtempii,maxindexalt]=max(entireRHS_ii_z,[],1);
                    Valt(curraindex,z_c,:,N_j)=shiftdim(Vtempii,1);
                    dindalt   =rem(maxindexalt-1,N_d)+1;
                    a2pindalt =floor((maxindexalt-1)/N_d)+1;
            ...
            %% Vtilde (beta0*beta)
            entireRHS_ii_z=ReturnMatrix_ii_z+repelem(DiscountedEV_tilde_z,N_d1,1,1,1,1,1);
            [~,maxindex1]=max(entireRHS_ii_z,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                    d2aprime=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_z=reshape(ReturnMatrix_ii_z_dc+DiscountedEV_tilde_z(d2aprime),[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,1,N_e]);
                    ...
                    ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                    d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_z=reshape(ReturnMatrix_ii_z_dc+DiscountedEV_tilde_z(d2aprime),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,1,N_e]);
                    [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
                    Vtilde(curraindex,z_c,:,N_j)=shiftdim(Vtempii,1);
                    dind   =rem(maxindex-1,N_d)+1;
                    a2pind =floor((maxindex-1)/N_d)+1;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 1);
                %% Valt (beta)
                entireRHS_ii_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_alt_z,N_d1,1,1,1,1,1);
                [~,maxindex1]=max(entireRHS_ii_ze,[],2);
                [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii_ze,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
                curraindex=repmat(level1ii',N_a2*N_a3,1) +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
                dindalt   =rem(maxindex2alt-1,N_d)+1;
                a1pindalt =rem(floor((maxindex2alt-1)/N_d),N_a1)+1;
                a2pindalt =floor((maxindex2alt-1)/(N_d*N_a1))+1;
                Valt(curraindex,z_c,e_c,N_j)       =shiftdim(Vtempii,1);
                Policyalt(1,curraindex,z_c,e_c,N_j)=dindalt;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        d2aprime=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze_dc+DiscountedEV_alt_z(d2aprime),[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        ...
                        a2ind_flat=repmat(repelem((0:N_a2-1),1,level1iidiff(ii)),1,N_a3);
                        a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                        loweredge_idxalt=dindalt + N_d*(a2pindalt-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat;
                        a1prime_recalt=a1localindalt+loweredge(loweredge_idxalt)-1;
                        Policyalt(1,curraindex,z_c,e_c,N_j)=dindalt;
                        ...
                        d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze_dc+DiscountedEV_alt_z(d2aprime),[N_d*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        [Vtempii,maxindexalt]=max(entireRHS_ii_ze,[],1);
                        Valt(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        dindalt   =rem(maxindexalt-1,N_d)+1;
                        a2pindalt =floor((maxindexalt-1)/N_d)+1;
                ...
                %% Vtilde (beta0*beta)
                entireRHS_ii_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_tilde_z,N_d1,1,1,1,1,1);
                [~,maxindex1]=max(entireRHS_ii_ze,[],2);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                        d2aprime=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze_dc+DiscountedEV_tilde_z(d2aprime),[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        ...
                        ReturnMatrix_ii_ze_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, special_n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                        d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                        entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze_dc+DiscountedEV_tilde_z(d2aprime),[N_d*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                        [Vtempii,maxindex]=max(entireRHS_ii_ze,[],1);
                        Vtilde(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                        dind   =rem(maxindex-1,N_d)+1;
                        a2pind =floor((maxindex-1)/N_d)+1;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        Policyalt(1,curraindex,:,:,jj)=dindalt;
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            Policyalt(1,curraindex,z_c,:,jj)=dindalt;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                    ReturnMatrix_ii_z_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_z, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                    d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_z=reshape(ReturnMatrix_ii_z_dc+DiscountedEV_alt_z(d2aprime),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,1,N_e]);
                    [Vtempii,maxindexalt]=max(entireRHS_ii_z,[],1);
                    Valt(curraindex,z_c,:,jj)=shiftdim(Vtempii,1);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policyalt(1,curraindex,z_c,e_c,jj)=dindalt;
