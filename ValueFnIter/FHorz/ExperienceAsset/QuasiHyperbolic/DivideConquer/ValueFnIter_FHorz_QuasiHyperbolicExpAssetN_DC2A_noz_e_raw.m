function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetN_DC2A_noz_e_raw(n_d1, n_d2, n_a1, n_a2, n_a3, n_e, N_j, d_gridvals, d2_gridvals, a1_grid, a2_gridvals, a3_grid, e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    DiscountedEV_tilde=beta0beta*reshape(EV,[N_d2,N_a1,N_a2,1,1,N_a3]);   % QH-perceived
    ...
    if vfoptions.lowmemory==0
        ...
        [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_e]),[],1);
        curraindex=repmat(level1ii',N_a2*N_a3,1) +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_e]);
                ...
                loweredge=maxindex1(:,1,:,ii,:,:,:);
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_e]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,N_j)=shiftdim(Vtempii,1);
                dindalt   =rem(maxindexalt-1,N_d)+1;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d2aprime=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime),[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_e]);
                ...
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                Vtilde(curraindex,:,N_j)=shiftdim(Vtempii,1);
                dind   =rem(maxindex-1,N_d)+1;
                a2pind =floor((maxindex-1)/N_d)+1;
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 1);
            %% Valt (beta)
            entireRHS_ii_e=ReturnMatrix_ii_e+repelem(DiscountedEV_alt,N_d1,1,1,1,1,1);
            [~,maxindex1]=max(entireRHS_ii_e,[],2);
            [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii_e,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
            curraindex=repmat(level1ii',N_a2*N_a3,1) +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
            dindalt   =rem(maxindex2alt-1,N_d)+1;
            a1pindalt =rem(floor((maxindex2alt-1)/N_d),N_a1)+1;
            a2pindalt =floor((maxindex2alt-1)/(N_d*N_a1))+1;
            Valt(curraindex,e_c,N_j)       =shiftdim(Vtempii,1);
            Policyalt(1,curraindex,e_c,N_j)=dindalt;
            Policyalt(2,curraindex,e_c,N_j)=a1pindalt;
            Policyalt(3,curraindex,e_c,N_j)=a2pindalt;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    d2aprime=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_alt(d2aprime),[N_d*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                    ...
                    a1prime_recalt=a1localindalt+loweredge(loweredge_idxalt)-1;
                    Policyalt(1,curraindex,e_c,N_j)=dindalt;
                    Policyalt(2,curraindex,e_c,N_j)=a1prime_recalt;
                    Policyalt(3,curraindex,e_c,N_j)=a2pindalt;
                else
                    loweredge=maxindex1(:,1,:,ii,:,:);
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 3);
                    d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_alt(d2aprime),[N_d*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                    [Vtempii,maxindexalt]=max(entireRHS_ii_e,[],1);
                    Valt(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                    dindalt   =rem(maxindexalt-1,N_d)+1;
                    a2pindalt =floor((maxindexalt-1)/N_d)+1;
                    ...
                    loweredge_idxalt=dindalt + N_d*(a2pindalt-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat;
                    Policyalt(1,curraindex,e_c,N_j)=dindalt;
                    Policyalt(2,curraindex,e_c,N_j)=loweredge(loweredge_idxalt);
                    Policyalt(3,curraindex,e_c,N_j)=a2pindalt;
                end
            end

            %% Vtilde (beta0*beta)
            entireRHS_ii_e=ReturnMatrix_ii_e+repelem(DiscountedEV_tilde,N_d1,1,1,1,1,1);
            [~,maxindex1]=max(entireRHS_ii_e,[],2);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 3);
                    d2aprime=d2ind_vec + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_tilde(d2aprime),[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                    ...
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 3);
                    d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_e=reshape(ReturnMatrix_ii_e_dc+DiscountedEV_tilde(d2aprime),[N_d*N_a2,level1iidiff(ii)*N_a2*N_a3]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    Vtilde(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                    dind   =rem(maxindex-1,N_d)+1;
                    a2pind =floor((maxindex-1)/N_d)+1;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                loweredge=maxindex1(:,1,:,ii,:,:,:);
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                d2aprime=d2ind_vec + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_e]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                Valt(curraindex,:,jj)=shiftdim(Vtempii,1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Policyalt(1,curraindex,e_c,jj)=dindalt;
            Policyalt(2,curraindex,e_c,jj)=a1pindalt;
            Policyalt(3,curraindex,e_c,jj)=a2pindalt;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    ...
                    a1prime_recalt=a1localindalt+loweredge(loweredge_idxalt)-1;
                    Policyalt(1,curraindex,e_c,jj)=dindalt;
                    Policyalt(2,curraindex,e_c,jj)=a1prime_recalt;
                    Policyalt(3,curraindex,e_c,jj)=a2pindalt;
                else
                    loweredge=maxindex1(:,1,:,ii,:,:);
                    ReturnMatrix_ii_e_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, n_d2, n_a2, n_a3, special_n_e, d_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, e_val, ReturnFnParamsVec, 3);
                    ...
                    loweredge_idxalt=dindalt + N_d*(a2pindalt-1) + N_d*N_a2*a2ind_flat + N_d*N_a2*N_a2*a3ind_flat;
                    Policyalt(1,curraindex,e_c,jj)=dindalt;
                    Policyalt(2,curraindex,e_c,jj)=loweredge(loweredge_idxalt);
                    Policyalt(3,curraindex,e_c,jj)=a2pindalt;
                end
            end
