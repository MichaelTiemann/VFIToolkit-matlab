function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetS_DC1_nod1_noz_raw(n_d2,n_a1,n_a2,N_j, d2_gridvals, a1_gridvals, a2_grid, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    % Now, get and store the full (d,aprime)
    entireRHS_ii_flat=reshape(entireRHS_ii,[N_d2*N_a1,vfoptions.level1n*N_a2]);
    [Vtempii,maxindex2]=max(entireRHS_ii_flat,[],1);

    % Store
    curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
    Vhat(curraindex,N_j)=shiftdim(Vtempii,1);
    entireRHS_ii_under_flat=reshape(entireRHS_ii_under,[N_d2*N_a1,vfoptions.level1n*N_a2]);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            a1primeindexes=loweredge+(0:1:maxgap(ii));
            % aprime possibilities are n_d-by-maxgap(ii)+1-by-1-by-n_a2
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, 0, n_d2, maxgap(ii)+1, level1iidiff(ii), n_a2, d2_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, ReturnFnParamsVec,3,0); % Level=2, Refine=0
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
            entireRHS_ii_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
            ...
            loweredge=maxindex1(:,1,ii,:);
            % Just use aprime(ii) for everything
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_noz(ReturnFn, 0, n_d2, 1, level1iidiff(ii), n_a2, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, ReturnFnParamsVec,3,0); % Level=2, Refine=0
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEV_hat(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
            entireRHS_ii_under=reshape(ReturnMatrix_ii+DiscountedEV_under(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            Vhat(curraindex,N_j)=shiftdim(Vtempii,1);
