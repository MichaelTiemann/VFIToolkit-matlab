function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetN_DC2A_nod1_noz_raw(n_d2, n_a1, n_a2, n_a3, N_j, d2_gridvals, a1_grid, a2_gridvals, a3_grid, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
% _noz analog of ValueFnIter_FHorz_ExpAsset_DC2A_nod1_raw (no exogenous z state).
...
Policyalt=zeros(3,N_a,N_j,'gpuArray'); % channels: 1=d2, 2=a1prime, 3=a2prime
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    % [N_d2, N_a1, N_a2, level1n, N_a2, N_a3]
    [~,maxindex1]=max(ReturnMatrix_ii,[],2);
    [Vtempii,maxindex2]=max(reshape(ReturnMatrix_ii,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, 0, n_d2, n_a2, n_a3, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, ReturnFnParamsVec, 2);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, 0, n_d2, n_a2, n_a3, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, ReturnFnParamsVec, 2);
            [Vtempii,maxindex]=max(ReturnMatrix_ii,[],1);
            Valt(curraindex,N_j)=shiftdim(Vtempii,1);
            d2ind  =rem(maxindex-1,N_d2)+1;
    ...
    [Vtempii,maxindex2alt]=max(reshape(entireRHS_ii,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
    curraindex=repmat(level1ii',N_a2*N_a3,1) +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap_V(ii)>0
            ...
            entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d2*(maxgap_V(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            ...
            loweredge=maxindex1(:,1,:,ii,:,:);
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, 0, n_d2, n_a2, n_a3, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, ReturnFnParamsVec, 3);
            d2aprime=(1:1:N_d2)' + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
            entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_alt(d2aprime),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
            Valt(curraindex,N_j)=shiftdim(Vtempii,1);
            d2indalt  =rem(maxindexalt-1,N_d2)+1;
    ...
    [~,maxindex1]=max(entireRHS_ii,[],2);
    [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d2*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]),[],1);
    curraindex=repmat(level1ii',N_a2*N_a3,1) +N_a1*repmat(repelem(a2ind,vfoptions.level1n,1),N_a3,1) +N_a1*N_a2*repelem(a3ind,vfoptions.level1n*N_a2,1);
    d2ind  =rem(maxindex2-1,N_d2)+1;
    a1pind =rem(floor((maxindex2-1)/N_d2),N_a1)+1;
    a2pind =floor((maxindex2-1)/(N_d2*N_a1))+1;
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, 0, n_d2, n_a2, n_a3, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, ReturnFnParamsVec, 3);
            d2aprime=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
            entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime),[N_d2*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_noz(ReturnFn, 0, n_d2, n_a2, n_a3, d2_gridvals, a1_grid(loweredge), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, ReturnFnParamsVec, 3);
            d2aprime=(1:1:N_d2)' + N_d2*(loweredge-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
            entireRHS_ii=reshape(ReturnMatrix_ii_dc+DiscountedEV_tilde(d2aprime),[N_d2*1*N_a2,level1iidiff(ii)*N_a2*N_a3]);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            Vtilde(curraindex,N_j)=shiftdim(Vtempii,1);
            d2ind  =rem(maxindex-1,N_d2)+1;
            a2pind =floor((maxindex-1)/N_d2)+1;
