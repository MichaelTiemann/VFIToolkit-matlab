function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetN_noa1_noz_e_raw(n_d1,n_d2,n_a2,n_e,N_j, d_gridvals, d2_gridvals, a2_grid,e_gridvals_J,pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS_alt=ReturnMatrix+beta*repelem(EV,N_d1,1,1);
        [Vtemp_alt,maxindex_alt]=max(entireRHS_alt,[],1);
        Valt(:,:,N_j)=shiftdim(Vtemp_alt,1);
        Policyalt(:,:,N_j)=shiftdim(maxindex_alt,1);
        entireRHS_tilde=ReturnMatrix+beta0beta*repelem(EV,N_d1,1,1);
...
for reverse_j=1:N_j-1
    ...
    EVpre=sum(pi_e_J(:,jj+1)'.*Valt(:,:,jj+1),2);
