function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetS_nod1_noz_raw(n_d2,n_a1,n_a2,N_j, d2_gridvals, a1_gridvals, a2_grid, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
for reverse_j=1:N_j-1
    ...
    if length(n_a2)==1
        ...
        aprime_lh=a1prime_offsets+N_a1*repmat(loIdx_1+n_a2_1*loIdx_2-1,N_a1,1);
        aprime_hh=a1prime_offsets+N_a1*repmat((loIdx_1+1)+n_a2_1*loIdx_2-1,N_a1,1);
        V_ll=reshape(Vunderbar(aprime_ll(:),jj+1),[N_d2*N_a1,N_a2]);
        V_hl=reshape(Vunderbar(aprime_hl(:),jj+1),[N_d2*N_a1,N_a2]);
        V_lh=reshape(Vunderbar(aprime_lh(:),jj+1),[N_d2*N_a1,N_a2]);
        V_hh=reshape(Vunderbar(aprime_hh(:),jj+1),[N_d2*N_a1,N_a2]);
