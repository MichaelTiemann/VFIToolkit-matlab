function [V,Policy]=ValueFnIter_FHorz_ExpAsset_nod1_noa1_raw(n_d2,n_a2,n_z,N_j, d2_gridvals, a2_grid,z_gridvals_J,pi_z_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn,n_d2, n_a2, n_z, d2_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), ReturnFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            ReturnMatrix_z=CreateReturnFnMatrix_Case2_Disc(ReturnFn,n_d2, n_a2, special_n_z, d2_gridvals, a2_gridvals, z_val, ReturnFnParamsVec); % with only the experience asset, can just use Case2 command
            %Calc the max and it's index
    ...
    if length(n_a2)==1
        ...
        V_ll=reshape(EVpre(aprime_ll(:),:),[N_d2,N_a2,N_z]);
        V_hl=reshape(EVpre(aprime_hl(:),:),[N_d2,N_a2,N_z]);
        V_lh=reshape(EVpre(aprime_lh(:),:),[N_d2,N_a2,N_z]);
        V_hh=reshape(EVpre(aprime_hh(:),:),[N_d2,N_a2,N_z]);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc(ReturnFn,n_d2, n_a2, n_z, d2_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), ReturnFnParamsVec); % with only the experience asset, can just use Case2 command
