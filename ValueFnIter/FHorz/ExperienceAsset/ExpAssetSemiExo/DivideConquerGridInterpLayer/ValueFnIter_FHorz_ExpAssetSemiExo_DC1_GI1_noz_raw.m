function [V,Policy]=ValueFnIter_FHorz_ExpAssetSemiExo_DC1_GI1_noz_raw(n_d1,n_d2,n_d3,n_a1,n_a2,n_semiz,N_j, d12_gridvals, d2_gridvals, d3_grid, a1_gridvals, a2_grid, semiz_gridvals_J, pi_semiz_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
PolicyL2flag=2*ones(1,N_a,N_semiz,N_j,vfoptions.indexT,'gpuArray'); % L2 flag: 1=all to lower, 2=usual, 3=all to upper
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_semiz
                ...
                inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
                inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
                flag_ford3_jj(:,z_c,d3_c) = squeeze(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper));
            end
        end
    end
    ...
    Policy(1,:,:,N_j)=reshape(Policy4_ford3_jj(1+temp),[1,N_a,N_semiz]);
    Policy(2,:,:,N_j)=reshape(Policy4_ford3_jj(2+temp),[1,N_a,N_semiz]);
    Policy(4,:,:,N_j)=reshape(Policy4_ford3_jj(3+temp),[1,N_a,N_semiz]);
    Policy(5,:,:,N_j)=reshape(Policy4_ford3_jj(4+temp),[1,N_a,N_semiz]);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for z_c=1:N_semiz
                ...
                entireRHS_ii_d3_z=ReturnMatrix_ii_d3_z+repelem(DiscountedEV_z,N_d1,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii_d3_z=ReturnMatrix_ii_d3_z+DiscountedEV_z(d2aprime);
                ...
                entireRHS_ii_d3=ReturnMatrix_ii_d3+reshape(DiscountedEVinterp_z(d2a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
                V_ford3_jj(:,z_c,d3_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexL2-1,N_d12)+1;
                allind=d_ind+N_d12*aind; % midpoint is n_d12-by-1-by-n_a1-by-n_a2
...
for reverse_j=1:N_j-1
    ...
    maxindex=reshape(maxindex,[N_a*N_semiz,1]); % This is the value of d that corresponds, make it this shape for addition just below
    temp=4*( (1:1:N_a*N_semiz)'+(N_a*N_semiz)*(maxindex-1) -1);
    Policy(1,:,:,jj)=reshape(Policy4_ford3_jj(1+temp),[1,N_a,N_semiz]);
    Policy(2,:,:,jj)=reshape(Policy4_ford3_jj(2+temp),[1,N_a,N_semiz]);
    Policy(4,:,:,jj)=reshape(Policy4_ford3_jj(3+temp),[1,N_a,N_semiz]);
    Policy(5,:,:,jj)=reshape(Policy4_ford3_jj(4+temp),[1,N_a,N_semiz]);
    PolicyL2flag(1,:,:,jj)=reshape(flag_ford3_jj((1:1:N_a*N_semiz)'+(N_a*N_semiz)*(maxindex-1)),[1,N_a,N_semiz]);
end
