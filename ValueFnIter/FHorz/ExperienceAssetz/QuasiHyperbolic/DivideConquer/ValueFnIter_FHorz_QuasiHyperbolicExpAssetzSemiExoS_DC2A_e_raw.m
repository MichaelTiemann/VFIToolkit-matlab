...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Policy(1,curraindex,:,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
            Policy(2,curraindex,:,e_c,N_j)=ceil(dind/N_d12); % d3
            aprimeind=ceil(maxindex2/N_d); % this is the joint (a1prime,a2prime)
            Policy(3,curraindex,:,e_c,N_j)=rem(aprimeind-1,N_a1)+1; % a1prime
            Policy(4,curraindex,:,e_c,N_j)=ceil(aprimeind/N_a1); % a2prime
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy(1,curraindex,:,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                    Policy(2,curraindex,:,e_c,N_j)=ceil(dind/N_d12); % d3
                    Policy(3,curraindex,:,e_c,N_j)=a1prime_rec; % a1prime
                    Policy(4,curraindex,:,e_c,N_j)=a2pind; % a2prime
                else
                    loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                    ...
                    Policy(1,curraindex,:,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                    Policy(2,curraindex,:,e_c,N_j)=ceil(dind/N_d12); % d3
                    Policy(3,curraindex,:,e_c,N_j)=loweredge(loweredge_idx); % a1prime
                    Policy(4,curraindex,:,e_c,N_j)=a2pind; % a2prime
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy(1,curraindex,zind,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                Policy(2,curraindex,zind,e_c,N_j)=ceil(dind/N_d12); % d3
                aprimeind=ceil(maxindex2/N_d); % this is the joint (a1prime,a2prime)
                Policy(3,curraindex,zind,e_c,N_j)=rem(aprimeind-1,N_a1)+1; % a1prime
                Policy(4,curraindex,zind,e_c,N_j)=ceil(aprimeind/N_a1); % a2prime
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policy(1,curraindex,zind,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                        Policy(2,curraindex,zind,e_c,N_j)=ceil(dind/N_d12); % d3
                        Policy(3,curraindex,zind,e_c,N_j)=a1prime_rec; % a1prime
                        Policy(4,curraindex,zind,e_c,N_j)=a2pind; % a2prime
                    else
                        loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                        ...
                        Policy(1,curraindex,zind,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                        Policy(2,curraindex,zind,e_c,N_j)=ceil(dind/N_d12); % d3
                        Policy(3,curraindex,zind,e_c,N_j)=loweredge(loweredge_idx); % a1prime
                        Policy(4,curraindex,zind,e_c,N_j)=a2pind; % a2prime
                    end
                end
            end
    ...
    elseif vfoptions.lowmemory==3
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                ...
                Policy(1,curraindex,z_c,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                Policy(2,curraindex,z_c,e_c,N_j)=ceil(dind/N_d12); % d3
                aprimeind=ceil(maxindex2/N_d); % this is the joint (a1prime,a2prime)
                Policy(3,curraindex,z_c,e_c,N_j)=rem(aprimeind-1,N_a1)+1; % a1prime
                Policy(4,curraindex,z_c,e_c,N_j)=ceil(aprimeind/N_a1); % a2prime
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policy(1,curraindex,z_c,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                        Policy(2,curraindex,z_c,e_c,N_j)=ceil(dind/N_d12); % d3
                        Policy(3,curraindex,z_c,e_c,N_j)=a1prime_rec; % a1prime
                        Policy(4,curraindex,z_c,e_c,N_j)=a2pind; % a2prime
                    else
                        loweredge=maxindex1(:,1,:,ii,:,:,:,:);
                        ...
                        Policy(1,curraindex,z_c,e_c,N_j)=d12_ind; % d12 (composite d1,d2)
                        Policy(2,curraindex,z_c,e_c,N_j)=ceil(dind/N_d12); % d3
                        Policy(3,curraindex,z_c,e_c,N_j)=loweredge(loweredge_idx); % a1prime
                        Policy(4,curraindex,z_c,e_c,N_j)=a2pind; % a2prime
                    end
                end
            end
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_under_flat=reshape(ReturnMatrix_ii_d3+repelem(DiscountedEV_under,N_d1,1,1,1,1,1,1),[N_d12*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_bothz,N_e]);
            maxindexfull=maxindex2+(N_d12*N_a1*N_a2)*(0:1:(vfoptions.level1n*N_a2*N_a3)-1)+shiftdim((N_d12*N_a1*N_a2)*(vfoptions.level1n*N_a2*N_a3)*(0:1:(N_bothz)-1),-1)+shiftdim((N_d12*N_a1*N_a2)*(vfoptions.level1n*N_a2*N_a3)*(N_bothz)*(0:1:(N_e)-1),-2);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d12*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz,N_e]);
                    maxindexfull=maxindex+(N_d12*(maxgap(ii)+1)*N_a2)*(0:1:(level1iidiff(ii)*N_a2*N_a3)-1)+shiftdim((N_d12*(maxgap(ii)+1)*N_a2)*(level1iidiff(ii)*N_a2*N_a3)*(0:1:(N_bothz)-1),-1)+shiftdim((N_d12*(maxgap(ii)+1)*N_a2)*(level1iidiff(ii)*N_a2*N_a3)*(N_bothz)*(0:1:(N_e)-1),-2);
                    ...
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    entireRHS_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d12*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz,N_e]);
                    maxindexfull=maxindex+(N_d12*1*N_a2)*(0:1:(level1iidiff(ii)*N_a2*N_a3)-1)+shiftdim((N_d12*1*N_a2)*(level1iidiff(ii)*N_a2*N_a3)*(0:1:(N_bothz)-1),-1)+shiftdim((N_d12*1*N_a2)*(level1iidiff(ii)*N_a2*N_a3)*(N_bothz)*(0:1:(N_e)-1),-2);
                    V_ford3_under(curraindex,:,:,d3_c)=shiftdim(entireRHS_under(maxindexfull),1);
                    V_ford3_hat(curraindex,:,:,d3_c)=shiftdim(Vtempii,1);
                    dind   =rem(maxindex-1,N_d12)+1;
            ...
            V_ford3_under(curraindex,zind,e_c,d3_c)=shiftdim(Vtempii_under,1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            V_ford3_under(curraindex,zind,e_c,d3_c)=shiftdim(entireRHS_under(maxindexfull),1);
                            V_ford3_hat(curraindex,zind,e_c,d3_c)=shiftdim(Vtempii,1);
                            dind      =rem(maxindex-1,N_d12)+1;
                            a1localind=rem(floor((maxindex-1)/N_d12),maxgap(ii)+1)+1;
                            a2pind    =floor((maxindex-1)/(N_d12*(maxgap(ii)+1)))+1;
