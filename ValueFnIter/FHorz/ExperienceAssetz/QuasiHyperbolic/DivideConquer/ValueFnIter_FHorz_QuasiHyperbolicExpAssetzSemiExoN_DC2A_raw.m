...
Policyalt=zeros(4,N_a,N_semiz*N_z,N_j,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            Policyalt(1,curraindex,zind,N_j)=d12_ind; % d12 (composite d1,d2)
            Policyalt(2,curraindex,zind,N_j)=ceil(dind/N_d12); % d3
            aprimeind=ceil(maxindex2/N_d); % this is the joint (a1prime,a2prime)
            Policyalt(3,curraindex,zind,N_j)=rem(aprimeind-1,N_a1)+1; % a1prime
            Policyalt(4,curraindex,zind,N_j)=ceil(aprimeind/N_a1); % a2prime
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policyalt(1,curraindex,zind,N_j)=d12_ind; % d12 (composite d1,d2)
                    Policyalt(2,curraindex,zind,N_j)=ceil(dind/N_d12); % d3
                    Policyalt(3,curraindex,zind,N_j)=a1prime_rec; % a1prime
                    Policyalt(4,curraindex,zind,N_j)=a2pind; % a2prime
                else
                    loweredge=maxindex1(:,1,:,ii,:,:,:);
                    ...
                    Policyalt(1,curraindex,zind,N_j)=d12_ind; % d12 (composite d1,d2)
                    Policyalt(2,curraindex,zind,N_j)=ceil(dind/N_d12); % d3
                    Policyalt(3,curraindex,zind,N_j)=loweredge(loweredge_idx); % a1prime
                    Policyalt(4,curraindex,zind,N_j)=a2pind; % a2prime
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_bothz
            ...
            Policyalt(1,curraindex,z_c,N_j)=d12_ind; % d12 (composite d1,d2)
            Policyalt(2,curraindex,z_c,N_j)=ceil(dind/N_d12); % d3
            aprimeind=ceil(maxindex2/N_d); % this is the joint (a1prime,a2prime)
            Policyalt(3,curraindex,z_c,N_j)=rem(aprimeind-1,N_a1)+1; % a1prime
            Policyalt(4,curraindex,z_c,N_j)=ceil(aprimeind/N_a1); % a2prime
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policyalt(1,curraindex,z_c,N_j)=d12_ind; % d12 (composite d1,d2)
                    Policyalt(2,curraindex,z_c,N_j)=ceil(dind/N_d12); % d3
                    Policyalt(3,curraindex,z_c,N_j)=a1prime_rec; % a1prime
                    Policyalt(4,curraindex,z_c,N_j)=a2pind; % a2prime
                else
                    loweredge=maxindex1(:,1,:,ii,:,:);
                    ...
                    Policyalt(1,curraindex,z_c,N_j)=d12_ind; % d12 (composite d1,d2)
                    Policyalt(2,curraindex,z_c,N_j)=ceil(dind/N_d12); % d3
                    Policyalt(3,curraindex,z_c,N_j)=loweredge(loweredge_idx); % a1prime
                    Policyalt(4,curraindex,z_c,N_j)=a2pind; % a2prime
                end
            end
        end
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    ...
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                    ...
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(loweredge_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_tilde(ii)>0
                    ...
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                    ...
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(loweredge_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            % --- alt pass (exponential beta) ---
                ...
                V_ford3_alt(curraindex,zind,d3_c)=shiftdim(Vtempii_alt,1);
                Policy_ford3_alt(curraindex,zind,d3_c)=shiftdim(maxindex2_alt,1);
            ...
            % --- tilde pass (QH-perceived beta0*beta) ---
                ...
                V_ford3_tilde(curraindex,zind,d3_c)=shiftdim(Vtempii_tilde,1);
                Policy_ford3_tilde(curraindex,zind,d3_c)=shiftdim(maxindex2_tilde,1);
            ...
            % --- alt narrow band ---
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_alt(ii)>0
                        loweredge_alt=min(maxindex1_alt(:,1,:,ii,:,:,:),N_a1-maxgap_alt(ii));
                        a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                        ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_semiz, d123_gridvals, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                        ...
                        a1prime_rec=a1localind+loweredge_alt(loweredge_idx)-1;
                        Policy_ford3_alt(curraindex,zind,d3_c)=shiftdim(dind_alt + N_d12*(a1prime_rec-1) + N_d12*N_a1*(a2pind-1),1);
                    else
                        loweredge_alt=maxindex1_alt(:,1,:,ii,:,:,:);
                        ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_semiz, d123_gridvals, a1_grid(loweredge_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                        ...
                        a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                        loweredge_idx=dind_alt + N_d12*(a2pind-1) + N_d12*N_a2*a2ind_flat + N_d12*N_a2*N_a2*a3ind_flat + N_d12*N_a2*N_a2*N_a3*semizind;
                        Policy_ford3_alt(curraindex,zind,d3_c)=shiftdim(dind_alt + N_d12*(loweredge_alt(loweredge_idx)-1) + N_d12*N_a1*(a2pind-1),1);
                    end
                end
            ...
            % --- tilde narrow band ---
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_tilde(ii)>0
                        loweredge_tilde=min(maxindex1_tilde(:,1,:,ii,:,:,:),N_a1-maxgap_tilde(ii));
                        a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                        ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_semiz, d123_gridvals, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                        ...
                        a1prime_rec=a1localind+loweredge_tilde(loweredge_idx)-1;
                        Policy_ford3_tilde(curraindex,zind,d3_c)=shiftdim(dind_tilde + N_d12*(a1prime_rec-1) + N_d12*N_a1*(a2pind-1),1);
                    else
                        loweredge_tilde=maxindex1_tilde(:,1,:,ii,:,:,:);
                        ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_semiz, d123_gridvals, a1_grid(loweredge_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                        ...
                        a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                        loweredge_idx=dind_tilde + N_d12*(a2pind-1) + N_d12*N_a2*a2ind_flat + N_d12*N_a2*N_a2*a3ind_flat + N_d12*N_a2*N_a2*N_a3*semizind;
                        Policy_ford3_tilde(curraindex,zind,d3_c)=shiftdim(dind_tilde + N_d12*(loweredge_tilde(loweredge_idx)-1) + N_d12*N_a1*(a2pind-1),1);
                    end
                end
            end
        end
            ...
            % --- alt narrow band ---
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_alt(ii)>0
                        loweredge_alt=min(maxindex1_alt(:,1,:,ii,:,:),N_a1-maxgap_alt(ii));
                        a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                        ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_bothz, d123_gridvals, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                        ...
                        a1prime_rec=a1localind+loweredge_alt(loweredge_idx)-1;
                        Policy_ford3_alt(curraindex,z_c,d3_c)=shiftdim(dind_alt + N_d12*(a1prime_rec-1) + N_d12*N_a1*(a2pind-1),1);
                    else
                        loweredge_alt=maxindex1_alt(:,1,:,ii,:,:);
                        ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_bothz, d123_gridvals, a1_grid(loweredge_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                        ...
                        a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                        loweredge_idx=dind_alt + N_d12*(a2pind-1) + N_d12*N_a2*a2ind_flat + N_d12*N_a2*N_a2*a3ind_flat;
                        Policy_ford3_alt(curraindex,z_c,d3_c)=shiftdim(dind_alt + N_d12*(loweredge_alt(loweredge_idx)-1) + N_d12*N_a1*(a2pind-1),1);
                    end
                end
            ...
            % --- tilde narrow band ---
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_tilde(ii)>0
                        loweredge_tilde=min(maxindex1_tilde(:,1,:,ii,:,:),N_a1-maxgap_tilde(ii));
                        a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                        ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_bothz, d123_gridvals, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                        ...
                        a1prime_rec=a1localind+loweredge_tilde(loweredge_idx)-1;
                        Policy_ford3_tilde(curraindex,z_c,d3_c)=shiftdim(dind_tilde + N_d12*(a1prime_rec-1) + N_d12*N_a1*(a2pind-1),1);
                    else
                        loweredge_tilde=maxindex1_tilde(:,1,:,ii,:,:);
                        ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_bothz, d123_gridvals, a1_grid(loweredge_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                        ...
                        a3ind_flat=repelem((0:N_a3-1),1,level1iidiff(ii)*N_a2);
                        loweredge_idx=dind_tilde + N_d12*(a2pind-1) + N_d12*N_a2*a2ind_flat + N_d12*N_a2*N_a2*a3ind_flat;
                        Policy_ford3_tilde(curraindex,z_c,d3_c)=shiftdim(dind_tilde + N_d12*(loweredge_tilde(loweredge_idx)-1) + N_d12*N_a1*(a2pind-1),1);
                    end
                end
            end
        end
    ...
    d12aprime_ind=reshape(Policy_ford3_alt((1:1:N_a*N_semiz*N_z)'+(N_a*N_semiz*N_z)*(maxindex-1)),[1,N_a,N_semiz*N_z]);
    ...
    maxindex=reshape(maxindex,[N_a*N_semiz*N_z,1]); % This is the value of d3 that corresponds, make it this shape for addition just below
    d12aprime_ind=reshape(Policy_ford3_tilde((1:1:N_a*N_semiz*N_z)'+(N_a*N_semiz*N_z)*(maxindex-1)),[1,N_a,N_semiz*N_z]);
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    loweredge_alt=min(maxindex1_alt(:,1,:,ii,:,:,:),N_a1-maxgap_alt(ii));
                    a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                    d2aprimez_alt=d2ind_vec + N_d2*(a1primeindexes_alt-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                    entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3_alt+DiscountedEV_alt(d2aprimez_alt),[N_d12*(maxgap_alt(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                    [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                    V_ford3_alt(curraindex,:,d3_c)=shiftdim(Vtempii_alt,1);
                    ...
                    loweredge_alt=maxindex1_alt(:,1,:,ii,:,:,:);
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(loweredge_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                    d2aprimez_alt=d2ind_vec + N_d2*(loweredge_alt-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                    entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3_alt+DiscountedEV_alt(d2aprimez_alt),[N_d12*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                    [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                    V_ford3_alt(curraindex,:,d3_c)=shiftdim(Vtempii_alt,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_tilde(ii)>0
                    loweredge_tilde=min(maxindex1_tilde(:,1,:,ii,:,:,:),N_a1-maxgap_tilde(ii));
                    a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                    d2aprimez_tilde=d2ind_vec + N_d2*(a1primeindexes_tilde-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                    entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde(d2aprimez_tilde),[N_d12*(maxgap_tilde(ii)+1)*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                    [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                    V_ford3_tilde(curraindex,:,d3_c)=shiftdim(Vtempii_tilde,1);
                    ...
                    loweredge_tilde=maxindex1_tilde(:,1,:,ii,:,:,:);
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, d123_gridvals, a1_grid(loweredge_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                    d2aprimez_tilde=d2ind_vec + N_d2*(loweredge_tilde-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                    entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde(d2aprimez_tilde),[N_d12*1*N_a2,level1iidiff(ii)*N_a2*N_a3,N_bothz]);
                    [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                    V_ford3_tilde(curraindex,:,d3_c)=shiftdim(Vtempii_tilde,1);
