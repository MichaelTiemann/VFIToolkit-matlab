...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        Policy(3,curraindex,:,:,N_j)=ceil(maxindex2/N_d23);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            dind=rem(maxindex2-1,N_d23)+1;
            Policy(1,curraindex,:,e_c,N_j)=rem(dind-1,N_d2)+1;
            Policy(2,curraindex,:,e_c,N_j)=ceil(dind/N_d2);
            Policy(3,curraindex,:,e_c,N_j)=ceil(maxindex2/N_d23);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy(1,curraindex,:,e_c,N_j)=rem(dind-1,N_d2)+1;
                    Policy(2,curraindex,:,e_c,N_j)=ceil(dind/N_d2);
                    Policy(3,curraindex,:,e_c,N_j)=ceil(maxindex/N_d23+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
    ...
    elseif vfoptions.lowmemory==2 % split: vectorize semiz, outer loop z, inner loop e
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                dind=rem(maxindex2-1,N_d23)+1;
                Policy(1,curraindex,semizblock,e_c,N_j)=rem(dind-1,N_d2)+1;
                Policy(2,curraindex,semizblock,e_c,N_j)=ceil(dind/N_d2);
                Policy(3,curraindex,semizblock,e_c,N_j)=ceil(maxindex2/N_d23);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policy(1,curraindex,semizblock,e_c,N_j)=rem(dind-1,N_d2)+1;
                        Policy(2,curraindex,semizblock,e_c,N_j)=ceil(dind/N_d2);
                        Policy(3,curraindex,semizblock,e_c,N_j)=ceil(maxindex/N_d23+loweredge(allind)-1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
    ...
    elseif vfoptions.lowmemory==3 % joint loop over bothz (outer), inner loop e
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                ...
                dind=rem(maxindex2-1,N_d23)+1;
                Policy(1,curraindex,z_c,e_c,N_j)=rem(dind-1,N_d2)+1;
                Policy(2,curraindex,z_c,e_c,N_j)=ceil(dind/N_d2);
                Policy(3,curraindex,z_c,e_c,N_j)=ceil(maxindex2/N_d23);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policy(1,curraindex,z_c,e_c,N_j)=rem(dind-1,N_d2)+1;
                        Policy(2,curraindex,z_c,e_c,N_j)=ceil(dind/N_d2);
                        Policy(3,curraindex,z_c,e_c,N_j)=ceil(maxindex/N_d23+loweredge(allind)-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_under_flat=reshape(ReturnMatrix_ii_d3+repelem(DiscountedEV_under,1,1,1,1,1),[N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz,N_e]);
            maxindexfull=maxindex2+(N_d2*N_a1)*(0:1:(vfoptions.level1n*N_a2)-1)+shiftdim((N_d2*N_a1)*(vfoptions.level1n*N_a2)*(0:1:(N_bothz)-1),-1)+shiftdim((N_d2*N_a1)*(vfoptions.level1n*N_a2)*(N_bothz)*(0:1:(N_e)-1),-2);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    maxindexfull=maxindex+(N_d2*(maxgap(ii)+1))*(0:1:(level1iidiff(ii)*N_a2)-1)+shiftdim((N_d2*(maxgap(ii)+1))*(level1iidiff(ii)*N_a2)*(0:1:(N_bothz)-1),-1)+shiftdim((N_d2*(maxgap(ii)+1))*(level1iidiff(ii)*N_a2)*(N_bothz)*(0:1:(N_e)-1),-2);
                    ...
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    entireRHS_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    maxindexfull=maxindex+(N_d2)*(0:1:(level1iidiff(ii)*N_a2)-1)+shiftdim((N_d2)*(level1iidiff(ii)*N_a2)*(0:1:(N_bothz)-1),-1)+shiftdim((N_d2)*(level1iidiff(ii)*N_a2)*(N_bothz)*(0:1:(N_e)-1),-2);
                    V_ford3_under(curraindex,:,:,d3_c)=shiftdim(entireRHS_under(maxindexfull),1);
                    V_ford3_hat(curraindex,:,:,d3_c)=shiftdim(Vtempii,1);
                    dind=(rem(maxindex-1,N_d2)+1);
                    a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            % Vunderbar: value of the hat-policy under the long-run beta (gather, no second argmax)
            entireRHS_under_flat=reshape(ReturnMatrix_ii_d3e+repelem(DiscountedEV_under,1,1,1,1,1),[N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz]);
            ...
            V_ford3_under(curraindex,:,e_c,d3_c)=shiftdim(Vtempii_under,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        entireRHS_under=reshape(ReturnMatrix_ii_d3e+DiscountedEV_under(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        ...
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        entireRHS_under=reshape(ReturnMatrix_ii_d3e+DiscountedEV_under(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_bothz]);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+repelem(DiscountedEV_z_hat,1,1,1,1,1);
                ...
                % Vunderbar: value of the hat-policy under the long-run beta (gather, no second argmax)
                entireRHS_under_flat=reshape(ReturnMatrix_ii_d3ze+repelem(DiscountedEV_z_under,1,1,1,1,1),[N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                ...
                V_ford3_under(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii_under,1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_hat(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            entireRHS_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_under(d2aprimez),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            ...
                            entireRHS_ii=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_hat(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            entireRHS_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_under(d2aprimez),[N_d2,level1iidiff(ii)*N_a2,N_semiz]);
    ...
    elseif vfoptions.lowmemory==3 % joint loop over bothz (outer), inner loop e
        for d3_c=1:N_d3
            ...
            % Vunderbar: value of the hat-policy under the long-run beta (gather, no second argmax)
            entireRHS_under_flat=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_under,[N_d2*N_a1,vfoptions.level1n*N_a2]);
            ...
            V_ford3_under(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii_under,1);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            ...
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            entireRHS_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_under(d2aprime),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                            ...
                            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                            entireRHS_under=reshape(ReturnMatrix_ii_d3ze+DiscountedEV_z_under(d2aprime),[N_d2,level1iidiff(ii)*N_a2]);
