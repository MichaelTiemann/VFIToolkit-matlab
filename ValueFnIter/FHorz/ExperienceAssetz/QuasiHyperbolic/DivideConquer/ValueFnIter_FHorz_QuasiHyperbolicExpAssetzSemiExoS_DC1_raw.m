...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        Policy(1,curraindex,:,N_j)=rem(d12_ind-1,N_d1)+1; % d1
        Policy(2,curraindex,:,N_j)=ceil(d12_ind/N_d1); % d2
        Policy(3,curraindex,:,N_j)=ceil(dind/N_d12); % d3
        Policy(4,curraindex,:,N_j)=ceil(maxindex2/N_d); % a1prime
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        ...
        for z_c=1:N_z
            ...
            dind=rem(maxindex2-1,N_d)+1;
            d12_ind=rem(dind-1,N_d12)+1;
            Policy(1,curraindex,semizblock,N_j)=rem(d12_ind-1,N_d1)+1;
            Policy(2,curraindex,semizblock,N_j)=ceil(d12_ind/N_d1);
            Policy(3,curraindex,semizblock,N_j)=ceil(dind/N_d12);
            Policy(4,curraindex,semizblock,N_j)=ceil(maxindex2/N_d);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy(2,curraindex,semizblock,N_j)=ceil(d12_ind/N_d1);
                    Policy(3,curraindex,semizblock,N_j)=ceil(dind/N_d12);
                    Policy(4,curraindex,semizblock,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for z_c=1:N_bothz
            ...
            dind=rem(maxindex2-1,N_d)+1;
            d12_ind=rem(dind-1,N_d12)+1;
            Policy(1,curraindex,z_c,N_j)=rem(d12_ind-1,N_d1)+1;
            Policy(2,curraindex,z_c,N_j)=ceil(d12_ind/N_d1);
            Policy(3,curraindex,z_c,N_j)=ceil(dind/N_d12);
            Policy(4,curraindex,z_c,N_j)=ceil(maxindex2/N_d);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policy(2,curraindex,z_c,N_j)=ceil(d12_ind/N_d1);
                    Policy(3,curraindex,z_c,N_j)=ceil(dind/N_d12);
                    Policy(4,curraindex,z_c,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_under_flat=reshape(ReturnMatrix_ii_d3+repelem(DiscountedEV_under,N_d1,1,1,1,1),[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz]);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                    ...
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    entireRHS_under=reshape(ReturnMatrix_ii_d3+DiscountedEV_under(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_bothz]);
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+repelem(DiscountedEV_z_hat,N_d1,1,1,1,1);

                [~,maxindex1]=max(entireRHS_ii_d3z,[],2);
                [Vtempii,maxindex2]=max(reshape(entireRHS_ii_d3z,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]),[],1);
                % Vunderbar: value of the hat-policy under the long-run beta (gather, no second argmax)
                entireRHS_under_flat=reshape(ReturnMatrix_ii_d3z+repelem(DiscountedEV_z_under,N_d1,1,1,1,1),[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z+DiscountedEV_z_hat(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        entireRHS_under=reshape(ReturnMatrix_ii_d3z+DiscountedEV_z_under(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        ...
                        dind=(rem(maxindex-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        allind=dind+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*semizind;
                        ...
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z+DiscountedEV_z_hat(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        entireRHS_under=reshape(ReturnMatrix_ii_d3z+DiscountedEV_z_under(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                % Vunderbar: value of the hat-policy under the long-run beta (gather, no second argmax)
                entireRHS_under_flat=reshape(ReturnMatrix_ii_z+repelem(DiscountedEV_z_under,N_d1,1,1,1),[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2]);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_under=reshape(ReturnMatrix_ii_z+DiscountedEV_z_under(d2aprime),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                        ...
                        [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
                        entireRHS_under=reshape(ReturnMatrix_ii_z+DiscountedEV_z_under(d2aprime),[N_d12,level1iidiff(ii)*N_a2]);
