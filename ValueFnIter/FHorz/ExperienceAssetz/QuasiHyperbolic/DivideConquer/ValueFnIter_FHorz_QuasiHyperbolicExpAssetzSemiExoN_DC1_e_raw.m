...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        Policyalt(4,curraindex,:,:,N_j)=ceil(maxindex2/N_d);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            dind=rem(maxindex2-1,N_d)+1;
            d12_ind=rem(dind-1,N_d12)+1;
            Policyalt(1,curraindex,:,e_c,N_j)=rem(d12_ind-1,N_d1)+1;
            Policyalt(2,curraindex,:,e_c,N_j)=ceil(d12_ind/N_d1);
            Policyalt(3,curraindex,:,e_c,N_j)=ceil(dind/N_d12);
            Policyalt(4,curraindex,:,e_c,N_j)=ceil(maxindex2/N_d);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    Policyalt(2,curraindex,:,e_c,N_j)=ceil(d12_ind/N_d1);
                    Policyalt(3,curraindex,:,e_c,N_j)=ceil(dind/N_d12);
                    Policyalt(4,curraindex,:,e_c,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
    ...
    elseif vfoptions.lowmemory==2 % split: vectorize semiz, outer loop z, inner loop e
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                dind=rem(maxindex2-1,N_d)+1;
                d12_ind=rem(dind-1,N_d12)+1;
                Policyalt(1,curraindex,semizblock,e_c,N_j)=rem(d12_ind-1,N_d1)+1;
                Policyalt(2,curraindex,semizblock,e_c,N_j)=ceil(d12_ind/N_d1);
                Policyalt(3,curraindex,semizblock,e_c,N_j)=ceil(dind/N_d12);
                Policyalt(4,curraindex,semizblock,e_c,N_j)=ceil(maxindex2/N_d);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policyalt(2,curraindex,semizblock,e_c,N_j)=ceil(d12_ind/N_d1);
                        Policyalt(3,curraindex,semizblock,e_c,N_j)=ceil(dind/N_d12);
                        Policyalt(4,curraindex,semizblock,e_c,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
    ...
    elseif vfoptions.lowmemory==3 % joint loop over bothz (outer), inner loop e
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                ...
                dind=rem(maxindex2-1,N_d)+1;
                d12_ind=rem(dind-1,N_d12)+1;
                Policyalt(1,curraindex,z_c,e_c,N_j)=rem(d12_ind-1,N_d1)+1;
                Policyalt(2,curraindex,z_c,e_c,N_j)=ceil(d12_ind/N_d1);
                Policyalt(3,curraindex,z_c,e_c,N_j)=ceil(dind/N_d12);
                Policyalt(4,curraindex,z_c,e_c,N_j)=ceil(maxindex2/N_d);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policyalt(2,curraindex,z_c,e_c,N_j)=ceil(d12_ind/N_d1);
                        Policyalt(3,curraindex,z_c,e_c,N_j)=ceil(dind/N_d12);
                        Policyalt(4,curraindex,z_c,e_c,N_j)=ceil(maxindex/N_d+loweredge(allind)-1);
                    else
                        loweredge=maxindex1(:,1,ii,:);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3_alt=ReturnMatrix_ii_d3+repelem(DiscountedEV_alt,N_d1,1,1,1,1); % broadcasts over e

            [~,maxindex1_alt]=max(entireRHS_ii_d3_alt,[],2);
            [Vtempii_alt,maxindex2_alt]=max(reshape(entireRHS_ii_d3_alt,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz,N_e]),[],1);
            ...
            entireRHS_ii_d3_tilde=ReturnMatrix_ii_d3+repelem(DiscountedEV_tilde,N_d1,1,1,1,1); % broadcasts over e

            [~,maxindex1_tilde]=max(entireRHS_ii_d3_tilde,[],2);
            [Vtempii_tilde,maxindex2_tilde]=max(reshape(entireRHS_ii_d3_tilde,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz,N_e]),[],1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    ...
                    entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3_alt+DiscountedEV_alt(d2aprimez_alt),[N_d12*(maxgap_alt(ii)+1),level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                    V_ford3_alt(curraindex,:,:,d3_c)=shiftdim(Vtempii_alt,1);
                    dind_alt=(rem(maxindex_alt-1,N_d1*N_d2)+1);
                    a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                    eind=shiftdim((0:1:N_e-1),-2);
                    allind_alt=dind_alt+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*bothzind+N_d1*N_d2*N_a2*N_bothz*eind;
                    ...
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz,n_e, d123_gridvals_val, a1_gridvals(loweredge_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                    d2aprimez_alt=d2ind+N_d2*(loweredge_alt-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                    entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3_alt+DiscountedEV_alt(d2aprimez_alt),[N_d12,level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                    V_ford3_alt(curraindex,:,:,d3_c)=shiftdim(Vtempii_alt,1);
                    dind_alt=(rem(maxindex_alt-1,N_d1*N_d2)+1);
                    a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_tilde(ii)>0
                    ...
                    entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde(d2aprimez_tilde),[N_d12*(maxgap_tilde(ii)+1),level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                    V_ford3_tilde(curraindex,:,:,d3_c)=shiftdim(Vtempii_tilde,1);
                    dind_tilde=(rem(maxindex_tilde-1,N_d1*N_d2)+1);
                    a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                    eind=shiftdim((0:1:N_e-1),-2);
                    allind_tilde=dind_tilde+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*bothzind+N_d1*N_d2*N_a2*N_bothz*eind;
                    ...
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz,n_e, d123_gridvals_val, a1_gridvals(loweredge_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                    d2aprimez_tilde=d2ind+N_d2*(loweredge_tilde-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                    entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde(d2aprimez_tilde),[N_d12,level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                    V_ford3_tilde(curraindex,:,:,d3_c)=shiftdim(Vtempii_tilde,1);
                    dind_tilde=(rem(maxindex_tilde-1,N_d1*N_d2)+1);
                    a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            % --- alt pass (exponential beta) ---
                ...
                entireRHS_ii_d3e_alt=ReturnMatrix_ii_d3e+repelem(DiscountedEV_alt,N_d1,1,1,1,1);

                [~,maxindex1_alt]=max(entireRHS_ii_d3e_alt,[],2);
                [Vtempii_alt,maxindex2_alt]=max(reshape(entireRHS_ii_d3e_alt,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz]),[],1);
            ...
            % --- tilde pass (QH-perceived beta0*beta) ---
                ...
                entireRHS_ii_d3e_tilde=ReturnMatrix_ii_d3e+repelem(DiscountedEV_tilde,N_d1,1,1,1,1);

                [~,maxindex1_tilde]=max(entireRHS_ii_d3e_tilde,[],2);
                [Vtempii_tilde,maxindex2_tilde]=max(reshape(entireRHS_ii_d3e_tilde,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_bothz]),[],1);
            ...
            % --- alt narrow band ---
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_alt(ii)>0
                        ...
                        d2aprimez_alt=d2ind+N_d2*(a1primeindexes_alt-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                        entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3e_alt+DiscountedEV_alt(d2aprimez_alt),[N_d12*(maxgap_alt(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                        V_ford3_alt(curraindex,:,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                        dind_alt=(rem(maxindex_alt-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        allind_alt=dind_alt+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*shiftdim((0:1:N_bothz-1),-1);
                        Policy_ford3_alt(curraindex,:,e_c,d3_c)=shiftdim(maxindex_alt+N_d1*N_d2*(loweredge_alt(allind_alt)-1),1);
                    else
                        loweredge_alt=maxindex1_alt(:,1,ii,:,:);
                        ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d123_gridvals_val, a1_gridvals(loweredge_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0);
                        d2aprimez_alt=d2ind+N_d2*(loweredge_alt-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                        entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3e_alt+DiscountedEV_alt(d2aprimez_alt),[N_d12,level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                        V_ford3_alt(curraindex,:,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                        dind_alt=(rem(maxindex_alt-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
            ...
            % --- tilde narrow band ---
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_tilde(ii)>0
                        ...
                        d2aprimez_tilde=d2ind+N_d2*(a1primeindexes_tilde-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                        entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3e_tilde+DiscountedEV_tilde(d2aprimez_tilde),[N_d12*(maxgap_tilde(ii)+1),level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                        V_ford3_tilde(curraindex,:,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                        dind_tilde=(rem(maxindex_tilde-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        allind_tilde=dind_tilde+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*shiftdim((0:1:N_bothz-1),-1);
                        Policy_ford3_tilde(curraindex,:,e_c,d3_c)=shiftdim(maxindex_tilde+N_d1*N_d2*(loweredge_tilde(allind_tilde)-1),1);
                    else
                        loweredge_tilde=maxindex1_tilde(:,1,ii,:,:);
                        ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d123_gridvals_val, a1_gridvals(loweredge_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0);
                        d2aprimez_tilde=d2ind+N_d2*(loweredge_tilde-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                        entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3e_tilde+DiscountedEV_tilde(d2aprimez_tilde),[N_d12,level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                        V_ford3_tilde(curraindex,:,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                        dind_tilde=(rem(maxindex_tilde-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                % --- alt pass (exponential beta) ---
                    ...
                    entireRHS_ii_d3ze_alt=ReturnMatrix_ii_d3ze+repelem(DiscountedEV_z_alt,N_d1,1,1,1,1);

                    [~,maxindex1_alt]=max(entireRHS_ii_d3ze_alt,[],2);
                    [Vtempii_alt,maxindex2_alt]=max(reshape(entireRHS_ii_d3ze_alt,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]),[],1);
                ...
                % --- tilde pass (QH-perceived beta0*beta) ---
                    ...
                    entireRHS_ii_d3ze_tilde=ReturnMatrix_ii_d3ze+repelem(DiscountedEV_z_tilde,N_d1,1,1,1,1);

                    [~,maxindex1_tilde]=max(entireRHS_ii_d3ze_tilde,[],2);
                    [Vtempii_tilde,maxindex2_tilde]=max(reshape(entireRHS_ii_d3ze_tilde,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_semiz]),[],1);
                ...
                % --- alt narrow band ---
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_alt(ii)>0
                            ...
                            entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3ze_alt+DiscountedEV_z_alt(d2aprimez_alt),[N_d12*(maxgap_alt(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                            V_ford3_alt(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                            dind_alt=(rem(maxindex_alt-1,N_d1*N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                            allind_alt=dind_alt+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*shiftdim((0:1:N_semiz-1),-1);
                            Policy_ford3_alt(curraindex,semizblock,e_c,d3_c)=shiftdim(maxindex_alt+N_d1*N_d2*(loweredge_alt(allind_alt)-1),1);
                        else
                            loweredge_alt=maxindex1_alt(:,1,ii,:,:);
                            ReturnMatrix_ii_d3ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d123_gridvals_val, a1_gridvals(loweredge_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                            d2aprimez_alt=d2ind+N_d2*(loweredge_alt-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                            entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3ze_alt+DiscountedEV_z_alt(d2aprimez_alt),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                            V_ford3_alt(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                            dind_alt=(rem(maxindex_alt-1,N_d1*N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                ...
                % --- tilde narrow band ---
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_tilde(ii)>0
                            ...
                            entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3ze_tilde+DiscountedEV_z_tilde(d2aprimez_tilde),[N_d12*(maxgap_tilde(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                            V_ford3_tilde(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                            dind_tilde=(rem(maxindex_tilde-1,N_d1*N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                            allind_tilde=dind_tilde+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*shiftdim((0:1:N_semiz-1),-1);
                            Policy_ford3_tilde(curraindex,semizblock,e_c,d3_c)=shiftdim(maxindex_tilde+N_d1*N_d2*(loweredge_tilde(allind_tilde)-1),1);
                        else
                            loweredge_tilde=maxindex1_tilde(:,1,ii,:,:);
                            ReturnMatrix_ii_d3ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d123_gridvals_val, a1_gridvals(loweredge_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                            d2aprimez_tilde=d2ind+N_d2*(loweredge_tilde-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_semiz-1),-3);
                            entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3ze_tilde+DiscountedEV_z_tilde(d2aprimez_tilde),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                            [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                            V_ford3_tilde(curraindex,semizblock,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                            dind_tilde=(rem(maxindex_tilde-1,N_d1*N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
    ...
    elseif vfoptions.lowmemory==3 % joint loop over bothz (outer), inner loop e
        for d3_c=1:N_d3
            ...
            % --- alt pass (exponential beta) ---
                    ...
                    [~,maxindex1_alt]=max(entireRHS_ii_d3ze_alt,[],2);
                    [Vtempii_alt,maxindex2_alt]=max(reshape(entireRHS_ii_d3ze_alt,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2]),[],1);
            ...
            % --- tilde pass (QH-perceived beta0*beta) ---
                    ...
                    [~,maxindex1_tilde]=max(entireRHS_ii_d3ze_tilde,[],2);
                    [Vtempii_tilde,maxindex2_tilde]=max(reshape(entireRHS_ii_d3ze_tilde,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2]),[],1);
            ...
            % --- alt narrow band ---
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_alt(ii)>0
                            ...
                            entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3ze_alt+DiscountedEV_z_alt(d2aprime_alt),[N_d12*(maxgap_alt(ii)+1),level1iidiff(ii)*N_a2]);
                            [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                            V_ford3_alt(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                            dind_alt=(rem(maxindex_alt-1,N_d1*N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                            allind_alt=dind_alt+N_d1*N_d2*a2ind;
                            Policy_ford3_alt(curraindex,z_c,e_c,d3_c)=shiftdim(maxindex_alt+N_d1*N_d2*(loweredge_alt(allind_alt)-1),1);
                        else
                            loweredge_alt=maxindex1_alt(:,1,ii,:);
                            ReturnMatrix_ii_d3ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,special_n_bothz,special_n_e, d123_gridvals_val, a1_gridvals(loweredge_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0);
                            d2aprime_alt=d2ind+N_d2*(loweredge_alt-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2);
                            entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3ze_alt+DiscountedEV_z_alt(d2aprime_alt),[N_d12,level1iidiff(ii)*N_a2]);
                            [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                            V_ford3_alt(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                            dind_alt=(rem(maxindex_alt-1,N_d1*N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
            ...
            % --- tilde narrow band ---
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_tilde(ii)>0
                            ...
                            entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3ze_tilde+DiscountedEV_z_tilde(d2aprime_tilde),[N_d12*(maxgap_tilde(ii)+1),level1iidiff(ii)*N_a2]);
                            [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                            V_ford3_tilde(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                            dind_tilde=(rem(maxindex_tilde-1,N_d1*N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                            allind_tilde=dind_tilde+N_d1*N_d2*a2ind;
                            Policy_ford3_tilde(curraindex,z_c,e_c,d3_c)=shiftdim(maxindex_tilde+N_d1*N_d2*(loweredge_tilde(allind_tilde)-1),1);
                        else
                            loweredge_tilde=maxindex1_tilde(:,1,ii,:);
                            ReturnMatrix_ii_d3ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,special_n_bothz,special_n_e, d123_gridvals_val, a1_gridvals(loweredge_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0);
                            d2aprime_tilde=d2ind+N_d2*(loweredge_tilde-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2);
                            entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3ze_tilde+DiscountedEV_z_tilde(d2aprime_tilde),[N_d12,level1iidiff(ii)*N_a2]);
                            [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                            V_ford3_tilde(curraindex,z_c,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                            dind_tilde=(rem(maxindex_tilde-1,N_d1*N_d2)+1);
                            a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
        ...
        for d3_c=1:N_d3
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    ...
                    loweredge_alt=maxindex1_alt(:,1,ii,:,:,:);
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz,n_e, d123_gridvals_val, a1_gridvals(loweredge_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    d2aprimez_alt=d2ind+N_d2*(loweredge_alt-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                    entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3_alt+DiscountedEV_alt(d2aprimez_alt),[N_d12,level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                    V_ford3_alt(curraindex,:,:,d3_c)=shiftdim(Vtempii_alt,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_tilde(ii)>0
                    ...
                    loweredge_tilde=maxindex1_tilde(:,1,ii,:,:,:);
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz,n_e, d123_gridvals_val, a1_gridvals(loweredge_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    d2aprimez_tilde=d2ind+N_d2*(loweredge_tilde-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                    entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde(d2aprimez_tilde),[N_d12,level1iidiff(ii)*N_a2,N_bothz,N_e]);
                    [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                    V_ford3_tilde(curraindex,:,:,d3_c)=shiftdim(Vtempii_tilde,1);
            ...
            % --- alt narrow band ---
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_alt(ii)>0
                        ...
                        loweredge_alt=maxindex1_alt(:,1,ii,:,:);
                        ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d123_gridvals_val, a1_gridvals(loweredge_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0);
                        d2aprimez_alt=d2ind+N_d2*(loweredge_alt-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                        entireRHS_ii_alt=reshape(ReturnMatrix_ii_d3e_alt+DiscountedEV_alt(d2aprimez_alt),[N_d12,level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii_alt,maxindex_alt]=max(entireRHS_ii_alt,[],1);
                        V_ford3_alt(curraindex,:,e_c,d3_c)=shiftdim(Vtempii_alt,1);
            ...
            % --- tilde narrow band ---
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_tilde(ii)>0
                        ...
                        loweredge_tilde=maxindex1_tilde(:,1,ii,:,:);
                        ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d123_gridvals_val, a1_gridvals(loweredge_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0);
                        d2aprimez_tilde=d2ind+N_d2*(loweredge_tilde-1)+N_d2*N_a1*shiftdim((0:1:N_a2-1),-2)+N_d2*N_a1*N_a2*shiftdim((0:1:N_bothz-1),-3);
                        entireRHS_ii_tilde=reshape(ReturnMatrix_ii_d3e_tilde+DiscountedEV_tilde(d2aprimez_tilde),[N_d12,level1iidiff(ii)*N_a2,N_bothz]);
                        [Vtempii_tilde,maxindex_tilde]=max(entireRHS_ii_tilde,[],1);
                        V_ford3_tilde(curraindex,:,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
