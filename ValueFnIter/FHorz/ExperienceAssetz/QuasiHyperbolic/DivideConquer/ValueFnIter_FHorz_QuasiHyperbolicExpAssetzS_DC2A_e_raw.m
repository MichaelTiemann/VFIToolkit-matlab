...
% Sophisticated QH + ExpAssetz, DC2A pattern (with d1, +e).
% E pre-integration: EVpre = sum_e' pi_e(e') * Vunderbar(:,:,e',jj+1).
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            e_val=e_gridvals_J(e_c,:,N_j);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, n_d2, n_a2, n_a3, n_z, special_n_e, d_gridvals, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
            entireRHS_hat_e=ReturnMatrix_ii_e+repelem(DiscountedEV_hat,N_d1,1,1,1,1,1,1);
            [~,maxindex1]=max(entireRHS_hat_e,[],2);
            entireRHS_hat_e_flat=reshape(entireRHS_hat_e,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]);
            [Vtempii_hat,maxindex2]=max(entireRHS_hat_e_flat,[],1);
            entireRHS_under_e=ReturnMatrix_ii_e+repelem(DiscountedEV_under,N_d1,1,1,1,1,1,1);
            entireRHS_under_e_flat=reshape(entireRHS_under_e,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3,N_z,1]);
            M=vfoptions.level1n*N_a2*N_a3;
            maxindexfull=maxindex2 + (N_d*N_a1*N_a2)*(0:M-1) + (N_d*N_a1*N_a2)*M*shiftdim((0:N_z-1),-1);
            Vtempii_under=entireRHS_under_e_flat(maxindexfull);
            curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                     + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                     + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
            dind   =rem(maxindex2-1,N_d)+1;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    firstdim=N_d*(maxgap(ii)+1)*N_a2;
                    Mblock=level1iidiff(ii)*N_a2*N_a3;
                    entireRHS_hat_e=reshape(ReturnMatrix_ii_e+DiscountedEV_hat(d2aprimez),[firstdim,Mblock,N_z,1]);
                    entireRHS_under_e=reshape(ReturnMatrix_ii_e+DiscountedEV_under(d2aprimez),[firstdim,Mblock,N_z,1]);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_hat_ze_flat=reshape(entireRHS_hat_ze,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]);
                [Vtempii_hat,maxindex2]=max(entireRHS_hat_ze_flat,[],1);
                entireRHS_under_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_under_ze,N_d1,1,1,1,1,1);
                entireRHS_under_ze_flat=reshape(entireRHS_under_ze,[N_d*N_a1*N_a2,vfoptions.level1n*N_a2*N_a3]);
                M=vfoptions.level1n*N_a2*N_a3;
                maxindexfull=maxindex2 + (N_d*N_a1*N_a2)*(0:M-1);
                Vtempii_under=entireRHS_under_ze_flat(maxindexfull);
                curraindex=repmat(level1ii',N_a2*N_a3,1) ...
                         + N_a1   *repmat(repelem((0:N_a2-1)',vfoptions.level1n,1),N_a3,1) ...
                         + N_a1*N_a2*repelem((0:N_a3-1)',vfoptions.level1n*N_a2,1);
                dind   =rem(maxindex2-1,N_d)+1;
