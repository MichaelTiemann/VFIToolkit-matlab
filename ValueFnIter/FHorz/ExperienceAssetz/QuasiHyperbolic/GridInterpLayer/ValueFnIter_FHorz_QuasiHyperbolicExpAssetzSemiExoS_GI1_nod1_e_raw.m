...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            Policy3_ford3_hat(2,:,:,:,d3_c)=shiftdim(squeeze(midpoint_hat(d_ind+N_d2*aind+N_d2*N_a*bothzBind+N_d2*N_a*N_bothz*eind)),-1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+reshape(DiscountedEVinterp_hat(d2a1primea2semiz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
                    [Vtempii,maxindexL2]=max(entireRHS_ii_d3ze,[],1);
                % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
                entireRHS_under=ReturnMatrix_ii_d3ze+reshape(DiscountedEVinterp_under(d2a1primea2semiz(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
