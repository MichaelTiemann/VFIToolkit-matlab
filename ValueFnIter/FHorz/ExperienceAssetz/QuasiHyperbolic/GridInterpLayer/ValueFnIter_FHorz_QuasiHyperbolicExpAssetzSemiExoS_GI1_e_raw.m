...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % midpoint_hat indexing with e (5D): need a separate path; just store via shiftdim
            Policy4_ford3_hat(3,:,:,:,d3_c)=shiftdim(squeeze(midpoint_hat(d_ind+N_d12*aind+N_d12*N_a*bothzBind+N_d12*N_a*N_bothz*eind)),-1);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3=ReturnMatrix_ii_d3+reshape(DiscountedEVinterp_hat(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz,1]); % broadcasts over e
            [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
            % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
            entireRHS_under=ReturnMatrix_ii_d3+reshape(DiscountedEVinterp_under(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz,1]);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_d3e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,n_bothz,special_n_e, d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2,0);
                d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*shiftdim((0:N_bothz-1),-3);
                entireRHS_ii_d3e=ReturnMatrix_ii_d3e+reshape(DiscountedEVinterp_hat(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3e,[],1);
            % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
            entireRHS_under=ReturnMatrix_ii_d3e+reshape(DiscountedEVinterp_under(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    entireRHS_d3ze=ReturnMatrix_d3ze+DiscountedEV_hat;
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+reshape(DiscountedEVinterp_hat(d12a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
                    [Vtempii,maxindexL2]=max(entireRHS_ii_d3ze,[],1);
                % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
                entireRHS_under=ReturnMatrix_ii_d3ze+reshape(DiscountedEVinterp_under(d12a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
    ...
    elseif vfoptions.lowmemory==3 % joint loop over bothz (outer), inner loop e
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                z_val=bothz_gridvals_J(z_c,:,N_j);
                DiscountedEV_z_hat=repelem(DiscountedEV_hat(:,:,:,:,z_c),N_d1,1);
                DiscountedEV_z_under=repelem(DiscountedEV_under(:,:,:,:,z_c),N_d1,1);
                DiscountedEVinterp_z_hat=repelem(DiscountedEVinterp_hat(:,:,:,:,z_c),N_d1,1);
                DiscountedEVinterp_z_under=repelem(DiscountedEVinterp_under(:,:,:,:,z_c),N_d1,1);
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_d3ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,n_a1,n_a2,special_n_bothz,special_n_e, d123_gridvals_val, a1_gridvals, a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
                    ...
                    entireRHS_ii_d3ze=ReturnMatrix_ii_d3ze+reshape(DiscountedEVinterp_z_hat(d12a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                    [Vtempii,maxindexL2]=max(entireRHS_ii_d3ze,[],1);
            % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
            entireRHS_under=ReturnMatrix_ii_d3ze+reshape(DiscountedEVinterp_z_under(d12a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*bothzind;
            entireRHS_ii_d3=ReturnMatrix_ii_d3+reshape(DiscountedEVinterp_hat(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz,N_e]);
            [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
            % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
