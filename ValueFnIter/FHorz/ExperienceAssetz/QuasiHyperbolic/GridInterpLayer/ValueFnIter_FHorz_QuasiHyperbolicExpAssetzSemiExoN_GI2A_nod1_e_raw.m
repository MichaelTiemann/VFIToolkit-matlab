function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetzSemiExoN_GI2A_nod1_e_raw(n_d2, n_d3, n_a1, n_a2, n_a3, n_z, n_semiz, n_e, N_j, d2_gridvals, d3_grid, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
...
Policy4_ford3_tilde=zeros(4,N_a,N_bothz,N_e,N_d3,'gpuArray'); % (d2, a1prime-midpoint, a2prime, a1primeL2ind)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            midpoint_alt=max(min(maxindex,N_a1-1),2);

            a1primeindexes=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
    ...
    Policyalt(1,:,:,:,N_j)=reshape(Policy4_ford3_alt(1+temp),[1,N_a,N_bothz,N_e]); % d2
    Policyalt(3,:,:,:,N_j)=reshape(Policy4_ford3_alt(2+temp),[1,N_a,N_bothz,N_e]); % a1prime midpoint_alt
    Policyalt(4,:,:,:,N_j)=reshape(Policy4_ford3_alt(3+temp),[1,N_a,N_bothz,N_e]); % a2prime
    Policyalt(5,:,:,:,N_j)=reshape(Policy4_ford3_alt(4+temp),[1,N_a,N_bothz,N_e]); % a1primeL2ind
    PolicyL2flagalt(1,:,:,:,N_j)=reshape(flag_ford3_alt((1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1)),[1,N_a,N_bothz,N_e]);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
            entireRHS_d3_alt=ReturnMatrix_d3+DiscountedEV_alt; % broadcasts over e
            [~,maxindex_alt]=max(entireRHS_d3_alt,[],2);
            midpoint_alt=max(min(maxindex_alt,N_a1-1),2);

            a1primeindexes=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
            ...
            entireRHS_ii_d3_alt=reshape(ReturnMatrix_ii_d3_alt+DiscountedEVinterp_alt(aprimez),[N_d2*n2long*N_a2,N_a,N_bothz,N_e]); % broadcasts over e
            ...
            % --- tilde pass ---
            entireRHS_d3_tilde=ReturnMatrix_d3+DiscountedEV_tilde; % broadcasts over e
            [~,maxindex_tilde]=max(entireRHS_d3_tilde,[],2);
            midpoint_tilde=max(min(maxindex_tilde,N_a1-1),2);

            a1primeindexes=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
            aprimez=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            entireRHS_ii_d3_tilde=reshape(ReturnMatrix_ii_d3_tilde+DiscountedEVinterp_tilde(aprimez),[N_d2*n2long*N_a2,N_a,N_bothz,N_e]); % broadcasts over e
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                entireRHS_d3e_alt=ReturnMatrix_d3e+DiscountedEV_alt;
                [~,maxindex_alt]=max(entireRHS_d3e_alt,[],2);
                midpoint_alt=max(min(maxindex_alt,N_a1-1),2);

                a1primeindexes=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                ...
                entireRHS_ii_d3e_alt=reshape(ReturnMatrix_ii_d3e_alt+DiscountedEVinterp_alt(aprime),[N_d2*n2long*N_a2,N_a,N_bothz]);
                ...
                inLowerStrict_alt=(maxindexL2a1_alt>=2)         & (maxindexL2a1_alt<=n2short+1);
                inUpperStrict_alt=(maxindexL2a1_alt>=n2short+3) & (maxindexL2a1_alt<=n2long-1);
                flag_ford3_alt(:,:,e_c,d3_c)=shiftdim(2 + (inLowerStrict_alt & isInfLower_alt) - (inUpperStrict_alt & isInfUpper_alt),1);

            % --- tilde pass ---
                entireRHS_d3e_tilde=ReturnMatrix_d3e+DiscountedEV_tilde;
                [~,maxindex_tilde]=max(entireRHS_d3e_tilde,[],2);
                midpoint_tilde=max(min(maxindex_tilde,N_a1-1),2);

                a1primeindexes=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                aprime=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                entireRHS_ii_d3e_tilde=reshape(ReturnMatrix_ii_d3e_tilde+DiscountedEVinterp_tilde(aprime),[N_d2*n2long*N_a2,N_a,N_bothz]);
                ...
                inLowerStrict_tilde=(maxindexL2a1_tilde>=2)         & (maxindexL2a1_tilde<=n2short+1);
                inUpperStrict_tilde=(maxindexL2a1_tilde>=n2short+3) & (maxindexL2a1_tilde<=n2long-1);
                flag_ford3_tilde(:,:,e_c,d3_c)=shiftdim(2 + (inLowerStrict_tilde & isInfLower_tilde) - (inUpperStrict_tilde & isInfUpper_tilde),1);
            end
        end
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                % --- alt pass ---
                    ...
                    a1primeindexes=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_semiz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_valblock, e_val, ReturnFnParamsVec, 3);
                    ...
                    entireRHS_ii_d3e_alt=reshape(ReturnMatrix_ii_d3e_alt+DiscountedEVinterp_alt(aprime),[N_d2*n2long*N_a2,N_a,N_semiz]);
                    [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_d3e_alt,[],1);
                    ...
                    inLowerStrict_alt=(maxindexL2a1_alt>=2)         & (maxindexL2a1_alt<=n2short+1);
                    inUpperStrict_alt=(maxindexL2a1_alt>=n2short+3) & (maxindexL2a1_alt<=n2long-1);
                    flag_ford3_alt(:,semizblock,e_c,d3_c)=shiftdim(2 + (inLowerStrict_alt & isInfLower_alt) - (inUpperStrict_alt & isInfUpper_alt),1);
                ...
                % --- tilde pass ---
                    ...
                    a1primeindexes=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_semiz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_valblock, e_val, ReturnFnParamsVec, 3);
                    aprime=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_semiz-1),-5);
                    entireRHS_ii_d3e_tilde=reshape(ReturnMatrix_ii_d3e_tilde+DiscountedEVinterp_tilde(aprime),[N_d2*n2long*N_a2,N_a,N_semiz]);
                    [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_d3e_tilde,[],1);
                    ...
                    inLowerStrict_tilde=(maxindexL2a1_tilde>=2)         & (maxindexL2a1_tilde<=n2short+1);
                    inUpperStrict_tilde=(maxindexL2a1_tilde>=n2short+3) & (maxindexL2a1_tilde<=n2long-1);
                    flag_ford3_tilde(:,semizblock,e_c,d3_c)=shiftdim(2 + (inLowerStrict_tilde & isInfLower_tilde) - (inUpperStrict_tilde & isInfUpper_tilde),1);
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==3 % joint bothz, inner e
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                    entireRHS_d3ze_alt=ReturnMatrix_d3ze+DiscountedEV_z_alt;
                    [~,maxindex_alt]=max(entireRHS_d3ze_alt,[],2);
                    midpoint_alt=max(min(maxindex_alt,N_a1-1),2);

                    a1primeindexes=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_d3ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                    aprime=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_d3ze_alt=reshape(ReturnMatrix_ii_d3ze_alt+DiscountedEVinterp_z_alt(aprime),[N_d2*n2long*N_a2,N_a]);
                    ...
                    inLowerStrict_alt=(maxindexL2a1_alt>=2)         & (maxindexL2a1_alt<=n2short+1);
                    inUpperStrict_alt=(maxindexL2a1_alt>=n2short+3) & (maxindexL2a1_alt<=n2long-1);
                    flag_ford3_alt(:,z_c,e_c,d3_c)=shiftdim(2 + (inLowerStrict_alt & isInfLower_alt) - (inUpperStrict_alt & isInfUpper_alt),1);

            % --- tilde pass ---
                    entireRHS_d3ze_tilde=ReturnMatrix_d3ze+DiscountedEV_z_tilde;
                    [~,maxindex_tilde]=max(entireRHS_d3ze_tilde,[],2);
                    midpoint_tilde=max(min(maxindex_tilde,N_a1-1),2);

                    a1primeindexes=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_d3ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, e_val, ReturnFnParamsVec, 3);
                    aprime=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_d3ze_tilde=reshape(ReturnMatrix_ii_d3ze_tilde+DiscountedEVinterp_z_tilde(aprime),[N_d2*n2long*N_a2,N_a]);
                    ...
                    inLowerStrict_tilde=(maxindexL2a1_tilde>=2)         & (maxindexL2a1_tilde<=n2short+1);
                    inUpperStrict_tilde=(maxindexL2a1_tilde>=n2short+3) & (maxindexL2a1_tilde<=n2long-1);
                    flag_ford3_tilde(:,z_c,e_c,d3_c)=shiftdim(2 + (inLowerStrict_tilde & isInfLower_tilde) - (inUpperStrict_tilde & isInfUpper_tilde),1);
                end
            end
        end
    end
    ...
    temp=4*((1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1)-1);
    Policyalt(1,:,:,:,N_j)=reshape(Policy4_ford3_alt(1+temp),[1,N_a,N_bothz,N_e]); % d2
    Policyalt(3,:,:,:,N_j)=reshape(Policy4_ford3_alt(2+temp),[1,N_a,N_bothz,N_e]); % a1prime midpoint
    Policyalt(4,:,:,:,N_j)=reshape(Policy4_ford3_alt(3+temp),[1,N_a,N_bothz,N_e]); % a2prime
    Policyalt(5,:,:,:,N_j)=reshape(Policy4_ford3_alt(4+temp),[1,N_a,N_bothz,N_e]); % a1primeL2ind
    PolicyL2flagalt(1,:,:,:,N_j)=reshape(flag_ford3_alt((1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1)),[1,N_a,N_bothz,N_e]);
    ...
    maxindex=reshape(maxindex,[N_a*N_bothz*N_e,1]);
    temp=4*((1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1)-1);
    Policy(1,:,:,:,N_j)=reshape(Policy4_ford3_tilde(1+temp),[1,N_a,N_bothz,N_e]); % d2
    Policy(3,:,:,:,N_j)=reshape(Policy4_ford3_tilde(2+temp),[1,N_a,N_bothz,N_e]); % a1prime midpoint
    Policy(4,:,:,:,N_j)=reshape(Policy4_ford3_tilde(3+temp),[1,N_a,N_bothz,N_e]); % a2prime
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            a1primeindexes=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
            aprimez=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            entireRHS_ii_d3_alt=reshape(ReturnMatrix_ii_d3_alt+DiscountedEVinterp_alt(aprimez),[N_d2*n2long*N_a2,N_a,N_bothz,N_e]); % broadcasts over e
            [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_d3_alt,[],1);
            V_ford3_alt(:,:,:,d3_c)=shiftdim(Vtempii_alt,1);
            ...
            a1primeindexes=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
            aprimez=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            entireRHS_ii_d3_tilde=reshape(ReturnMatrix_ii_d3_tilde+DiscountedEVinterp_tilde(aprimez),[N_d2*n2long*N_a2,N_a,N_bothz,N_e]); % broadcasts over e
            [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_d3_tilde,[],1);
            V_ford3_tilde(:,:,:,d3_c)=shiftdim(Vtempii_tilde,1);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                ...
                a1primeindexes=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                aprime=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                entireRHS_ii_d3e_alt=reshape(ReturnMatrix_ii_d3e_alt+DiscountedEVinterp_alt(aprime),[N_d2*n2long*N_a2,N_a,N_bothz]);
                [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_d3e_alt,[],1);
                V_ford3_alt(:,:,e_c,d3_c)=shiftdim(Vtempii_alt,1);
            ...
            % --- tilde pass ---
                ...
                a1primeindexes=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexes), a2_gridvals, a1_grid, a2_gridvals, a3_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                aprime=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                entireRHS_ii_d3e_tilde=reshape(ReturnMatrix_ii_d3e_tilde+DiscountedEVinterp_tilde(aprime),[N_d2*n2long*N_a2,N_a,N_bothz]);
                [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_d3e_tilde,[],1);
                V_ford3_tilde(:,:,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
    ...
    maxindex=reshape(maxindex,[N_a*N_bothz*N_e,1]);
    temp=4*((1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1)-1);
    Policyalt(1,:,:,:,jj)=reshape(Policy4_ford3_alt(1+temp),[1,N_a,N_bothz,N_e]); % d2
    Policyalt(3,:,:,:,jj)=reshape(Policy4_ford3_alt(2+temp),[1,N_a,N_bothz,N_e]); % a1prime midpoint
    Policyalt(4,:,:,:,jj)=reshape(Policy4_ford3_alt(3+temp),[1,N_a,N_bothz,N_e]); % a2prime
    Policyalt(5,:,:,:,jj)=reshape(Policy4_ford3_alt(4+temp),[1,N_a,N_bothz,N_e]); % a1primeL2ind
    PolicyL2flagalt(1,:,:,:,jj)=reshape(flag_ford3_alt((1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1)),[1,N_a,N_bothz,N_e]);
    ...
    maxindex=reshape(maxindex,[N_a*N_bothz*N_e,1]);
    temp=4*((1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1)-1);
    Policy(1,:,:,:,jj)=reshape(Policy4_ford3_tilde(1+temp),[1,N_a,N_bothz,N_e]); % d2
    Policy(3,:,:,:,jj)=reshape(Policy4_ford3_tilde(2+temp),[1,N_a,N_bothz,N_e]); % a1prime midpoint
    Policy(4,:,:,:,jj)=reshape(Policy4_ford3_tilde(3+temp),[1,N_a,N_bothz,N_e]); % a2prime
