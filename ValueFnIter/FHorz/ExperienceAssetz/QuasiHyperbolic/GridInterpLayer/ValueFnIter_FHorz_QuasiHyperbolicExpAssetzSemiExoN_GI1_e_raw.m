...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % midpoint_alt indexing with e (5D): need a separate path; just store via shiftdim
            Policy4_ford3_alt(3,:,:,:,d3_c)=shiftdim(squeeze(midpoint_alt(d_ind+N_d12*aind+N_d12*N_a*bothzBind+N_d12*N_a*N_bothz*eind)),-1);
    ...
    elseif vfoptions.lowmemory==2 % split: vectorize semiz, outer loop z, inner loop e
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_d3ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,n_a1,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d123_gridvals_val, a1_gridvals, a1_gridvals, a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,1,0);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            Policy4_ford3_alt(3,:,:,:,d3_c)=shiftdim(squeeze(midpoint_alt(d_ind_alt+N_d12*aind+N_d12*N_a*bothzBind+N_d12*N_a*N_bothz*eind)),-1);
            Policy4_ford3_alt(4,:,:,:,d3_c)=shiftdim(ceil(maxindexL2_alt/N_d12),-1);
            L2offset_alt=ceil(maxindexL2_alt/N_d12);
            linidx_lower_alt=d_ind_alt+N_d12*n2long*aind+N_d12*n2long*N_a*bothzBind+N_d12*n2long*N_a*N_bothz*eind;
            linidx_upper_alt=d_ind_alt+N_d12*(n2long-1)+N_d12*n2long*aind+N_d12*n2long*N_a*bothzBind+N_d12*n2long*N_a*N_bothz*eind;
            ...
            Policy4_ford3_tilde(3,:,:,:,d3_c)=shiftdim(squeeze(midpoint_tilde(d_ind_tilde+N_d12*aind+N_d12*N_a*bothzBind+N_d12*N_a*N_bothz*eind)),-1);
            Policy4_ford3_tilde(4,:,:,:,d3_c)=shiftdim(ceil(maxindexL2_tilde/N_d12),-1);
            L2offset_tilde=ceil(maxindexL2_tilde/N_d12);
            linidx_lower_tilde=d_ind_tilde+N_d12*n2long*aind+N_d12*n2long*N_a*bothzBind+N_d12*n2long*N_a*N_bothz*eind;
            linidx_upper_tilde=d_ind_tilde+N_d12*(n2long-1)+N_d12*n2long*aind+N_d12*n2long*N_a*bothzBind+N_d12*n2long*N_a*N_bothz*eind;
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                ...
                d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine_alt-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*shiftdim((0:N_bothz-1),-3);
                entireRHS_ii_d3e_alt=ReturnMatrix_ii_d3e_alt+reshape(DiscountedEVinterp_alt(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
            ...
            % --- tilde pass ---
                ...
                d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine_tilde-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*shiftdim((0:N_bothz-1),-3);
                entireRHS_ii_d3e_tilde=ReturnMatrix_ii_d3e_tilde+reshape(DiscountedEVinterp_tilde(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                % --- alt pass ---
                    ...
                    d12a1primea2semiz=(1:1:N_d12)'+N_d12*(a1primeindexesfine_alt-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*semizind;
                    entireRHS_ii_d3ze_alt=ReturnMatrix_ii_d3ze_alt+reshape(DiscountedEVinterp_alt(d12a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
                ...
                % --- tilde pass ---
                    ...
                    d12a1primea2semiz=(1:1:N_d12)'+N_d12*(a1primeindexesfine_tilde-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*semizind;
                    entireRHS_ii_d3ze_tilde=ReturnMatrix_ii_d3ze_tilde+reshape(DiscountedEVinterp_tilde(d12a1primea2semiz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
    ...
    elseif vfoptions.lowmemory==3 % joint loop over bothz (outer), inner loop e
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                z_val=bothz_gridvals_J(z_c,:,N_j);
                DiscountedEV_z_alt=repelem(DiscountedEV_alt(:,:,:,:,z_c),N_d1,1);
                DiscountedEVinterp_z_alt=repelem(DiscountedEVinterp_alt(:,:,:,:,z_c),N_d1,1);
                DiscountedEV_z_tilde=repelem(DiscountedEV_tilde(:,:,:,:,z_c),N_d1,1);
                DiscountedEVinterp_z_tilde=repelem(DiscountedEVinterp_tilde(:,:,:,:,z_c),N_d1,1);
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_d3ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,[n_d2,1],n_a1,n_a1,n_a2,special_n_bothz,special_n_e, d123_gridvals_val, a1_gridvals, a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);
            ...
            % --- alt pass ---
                    ...
                    d12a1primea2=(1:1:N_d12)'+N_d12*(a1primeindexesfine_alt-1)+N_d12*N_a1prime*a2ind;
                    entireRHS_ii_d3ze_alt=ReturnMatrix_ii_d3ze_alt+reshape(DiscountedEVinterp_z_alt(d12a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                    [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_d3ze_alt,[],1);
            ...
            % --- tilde pass ---
                    ...
                    d12a1primea2=(1:1:N_d12)'+N_d12*(a1primeindexesfine_tilde-1)+N_d12*N_a1prime*a2ind;
                    entireRHS_ii_d3ze_tilde=ReturnMatrix_ii_d3ze_tilde+reshape(DiscountedEVinterp_z_tilde(d12a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                    [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_d3ze_tilde,[],1);
    ...
    % Max over d3 (dim 4 of V_ford3_jj since e is dim 3)
    % Max over d3 (alt)
    [V_jj,maxindex]=max(V_ford3_alt,[],4);
    Valt(:,:,:,N_j)=V_jj;
    Policyalt(3,:,:,:,N_j)=shiftdim(maxindex,-1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine_alt-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*bothzind;
            entireRHS_ii_d3_alt=ReturnMatrix_ii_d3_alt+reshape(DiscountedEVinterp_alt(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz,N_e]);
            [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_d3_alt,[],1);
            ...
            % L2flag for dealing with -Inf in ReturnFn
            L2offset_alt=ceil(maxindexL2_alt/N_d12);
            linidx_lower_alt=d_ind_alt+N_d12*n2long*aind+N_d12*n2long*N_a*bothzBind+N_d12*n2long*N_a*N_bothz*eind;
            linidx_upper_alt=d_ind_alt+N_d12*(n2long-1)+N_d12*n2long*aind+N_d12*n2long*N_a*bothzBind+N_d12*n2long*N_a*N_bothz*eind;
            isInfLower_alt=(ReturnMatrix_ii_d3_alt(linidx_lower_alt)==-Inf);
            ...
            d12a1primea2bothz=(1:1:N_d12)'+N_d12*(a1primeindexesfine_tilde-1)+N_d12*N_a1prime*a2ind+N_d12*N_a1prime*N_a2*bothzind;
            entireRHS_ii_d3_tilde=ReturnMatrix_ii_d3_tilde+reshape(DiscountedEVinterp_tilde(d12a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz,N_e]);
            [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_d3_tilde,[],1);
