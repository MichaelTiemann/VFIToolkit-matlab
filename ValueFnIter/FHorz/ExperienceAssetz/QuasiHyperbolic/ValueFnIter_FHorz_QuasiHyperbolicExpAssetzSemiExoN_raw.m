...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        for z_c=1:N_z
            ...
            Policyalt(3,:,semizblock,N_j)=ceil(d_ind/N_d12);
            Policyalt(4,:,semizblock,N_j)=ceil(maxindex/N_d);
        end
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for z_c=1:N_bothz
            ...
            Policyalt(3,:,z_c,N_j)=ceil(d_ind/N_d12);
            Policyalt(4,:,z_c,N_j)=ceil(maxindex/N_d);
        end
    end
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                % alt (exponential): F + beta*EV
                entireRHS_d3z=ReturnMatrix_d3z+beta*repelem(entireEV_z,N_d1,N_a1,1);
                ...
                % tilde (QH-perceived): F + beta0*beta*EV
                entireRHS_d3z=ReturnMatrix_d3z+beta0beta*repelem(entireEV_z,N_d1,N_a1,1);
                [Vtemp,maxindex]=max(entireRHS_d3z,[],1);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                % alt (exponential): F + beta*EV
                entireRHS_d3z=ReturnMatrix_d3z+beta*repelem(entireEV_z,N_d1,N_a1);
                [Vtemp,maxindex]=max(entireRHS_d3z,[],1);
                V_ford3_alt(:,z_c,d3_c)=Vtemp;
                Policy_ford3_alt(:,z_c,d3_c)=maxindex;

                % tilde (QH-perceived): F + beta0*beta*EV
                entireRHS_d3z=ReturnMatrix_d3z+beta0beta*repelem(entireEV_z,N_d1,N_a1);
                [Vtemp,maxindex]=max(entireRHS_d3z,[],1);
