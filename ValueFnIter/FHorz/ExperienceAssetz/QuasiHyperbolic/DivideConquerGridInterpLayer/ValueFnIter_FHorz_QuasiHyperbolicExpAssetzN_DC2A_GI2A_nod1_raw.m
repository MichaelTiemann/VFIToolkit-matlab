...
if vfoptions.lowmemory==0
    ...
    midpoint_alt=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,N_z,'gpuArray');
elseif vfoptions.lowmemory==1
    special_n_z=ones(1,length(n_z),vfoptions.precision,'gpuArray');
    midpoint_z    =zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,'gpuArray');
    midpoint_alt_z=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,'gpuArray');
end
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            midpoint_z(:,1,:,level1ii,:,:)=maxindex1_z;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    midpoint_z(:,1,:,curra1inner,:,:)=maxindex_inner+(loweredge-1);
                else
                    loweredge=maxindex1_z(:,1,:,ii,:,:);
                    midpoint_z(:,1,:,curra1inner,:,:)=repelem(loweredge,1,1,1,level1iidiff(ii),1,1);
                end
            end
            midpoint_z=max(min(midpoint_z,N_a1-1),2);
            a1primeindexesfine_z=(midpoint_z+(midpoint_z-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, special_n_z, d2_gridvals, a1prime_grid(a1primeindexesfine_z), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 2);
            [Vtempii,maxindexL2]=max(ReturnMatrix_ii_z,[],1);
            Valt(:,z_c,N_j)=Vtempii(:);
            d_ind        =rem(maxindexL2-1,N_d2)+1;
            maxindexL2a1 =rem(floor((maxindexL2-1)/N_d2),n2long)+1;
            maxindexL2a2 =floor((maxindexL2-1)/(N_d2*n2long))+1;
            allind_z=d_ind + N_d2*(maxindexL2a2-1) + N_d2*N_a2*aind;
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_alt(ii)>0
                ...
                ReturnMatrix_inner=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
        ...
        ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, d2_gridvals, a1prime_grid(a1primeindexesfine_alt), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
        aprimez_alt=(1:1:N_d2)' + N_d2*(a1primeindexesfine_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
        entireRHS_ii_alt=reshape(ReturnMatrix_ii_alt+DiscountedEVinterp_alt(aprimez_alt),[N_d2*n2long*N_a2,N_a,N_z]);
        ...
        allind_alt=d_ind_alt + N_d2*(maxindexL2a2_alt-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*zindB;
        ...
        RM_alt_flat=reshape(ReturnMatrix_ii_alt,[N_d2*n2long*N_a2,N_a,N_z]);
        linidx_lower=d_ind_alt                   + N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*zindB;
        linidx_upper=d_ind_alt + N_d2*(n2long-1) + N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*zindB;
        ...
        % --- GI fine: current-self ---
        midpoint=max(min(midpoint,N_a1-1),2);
        a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, d2_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
        ...
        RM_flat=reshape(ReturnMatrix_ii,[N_d2*n2long*N_a2,N_a,N_z]);
        linidx_lower=d_ind                   + N_d2*n2long*(maxindexL2a2-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*zindB;
        linidx_upper=d_ind + N_d2*(n2long-1) + N_d2*n2long*(maxindexL2a2-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*zindB;
        inLowerStrict=(maxindexL2a1>=2)         & (maxindexL2a1<=n2short+1);
        inUpperStrict=(maxindexL2a1>=n2short+3) & (maxindexL2a1<=n2long-1);
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            midpoint_alt_z(:,1,:,level1ii,:,:)=maxindex1_alt_z;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    ...
                    a1primeindexes=loweredge+(0:1:maxgap_alt(ii));
                    ReturnMatrix_inner_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, special_n_z, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
                    ...
                    midpoint_alt_z(:,1,:,curra1inner,:,:)=maxindex_inner+(loweredge-1);
                else
                    loweredge=maxindex1_alt_z(:,1,:,ii,:,:);
                    midpoint_alt_z(:,1,:,curra1inner,:,:)=repelem(loweredge,1,1,1,level1iidiff(ii),1,1);
                end
            end
            ...
            ReturnMatrix_ii_alt_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, special_n_z, d2_gridvals, a1prime_grid(a1primeindexesfine_alt), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
            aprimez_alt_z=(1:1:N_d2)' + N_d2*(a1primeindexesfine_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
            entireRHS_ii_alt_z=reshape(ReturnMatrix_ii_alt_z+DiscountedEVinterp_alt_z(aprimez_alt_z),[N_d2*n2long*N_a2,N_a]);
            [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_alt_z,[],1);
            Valt(:,z_c,N_j)=Vtempii_alt(:);
            d_ind_alt        =rem(maxindexL2_alt-1,N_d2)+1;
            maxindexL2a1_alt =rem(floor((maxindexL2_alt-1)/N_d2),n2long)+1;
            maxindexL2a2_alt =floor((maxindexL2_alt-1)/(N_d2*n2long))+1;
            allind_alt_z=d_ind_alt + N_d2*(maxindexL2a2_alt-1) + N_d2*N_a2*aind;
            Policyalt(1,:,z_c,N_j)=d_ind_alt;
            Policyalt(2,:,z_c,N_j)=midpoint_alt_z(allind_alt_z);
            Policyalt(3,:,z_c,N_j)=maxindexL2a2_alt;
            Policyalt(4,:,z_c,N_j)=maxindexL2a1_alt;
            RM_alt_flat=reshape(ReturnMatrix_ii_alt_z,[N_d2*n2long*N_a2,N_a]);
            ...
            inLowerStrict=(maxindexL2a1_alt>=2)         & (maxindexL2a1_alt<=n2short+1);
            inUpperStrict=(maxindexL2a1_alt>=n2short+3) & (maxindexL2a1_alt<=n2long-1);
            PolicyaltL2flag(1,:,z_c,N_j)=2 + (inLowerStrict & (RM_alt_flat(linidx_lower)==-Inf)) - (inUpperStrict & (RM_alt_flat(linidx_upper)==-Inf));

            % GI fine curr
            midpoint_z=max(min(midpoint_z,N_a1-1),2);
            a1primeindexesfine=(midpoint_z+(midpoint_z-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, special_n_z, d2_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_val, ReturnFnParamsVec, 3);
            aprimez_z=(1:1:N_d2)' + N_d2*(a1primeindexesfine-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
            entireRHS_ii_z=reshape(ReturnMatrix_ii_z+DiscountedEVinterp_z(aprimez_z),[N_d2*n2long*N_a2,N_a]);
            ...
            RM_flat=reshape(ReturnMatrix_ii_z,[N_d2*n2long*N_a2,N_a]);
            linidx_lower=d_ind                   + N_d2*n2long*(maxindexL2a2-1) + N_d2*n2long*N_a2*aind;
            linidx_upper=d_ind + N_d2*(n2long-1) + N_d2*n2long*(maxindexL2a2-1) + N_d2*n2long*N_a2*aind;
            inLowerStrict=(maxindexL2a1>=2)         & (maxindexL2a1<=n2short+1);
            inUpperStrict=(maxindexL2a1>=n2short+3) & (maxindexL2a1<=n2long-1);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_alt(ii)>0
                loweredge=min(maxindex1_alt(:,1,:,ii,:,:,:),N_a1-maxgap_alt(ii));
                a1primeindexes=loweredge+(0:1:maxgap_alt(ii));
                ReturnMatrix_inner=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, d2_gridvals, a1_grid(a1primeindexes), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                d2aprimez=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                entireRHS_inner=ReturnMatrix_inner+DiscountedEV_alt(d2aprimez);
                [~,maxindex_inner]=max(entireRHS_inner,[],2);
                midpoint_alt(:,1,:,curra1inner,:,:,:)=maxindex_inner+(loweredge-1);
        ...
        midpoint_alt=max(min(midpoint_alt,N_a1-1),2);
        a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_ii_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, d2_gridvals, a1prime_grid(a1primeindexesfine_alt), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
        aprimez_alt=(1:1:N_d2)' + N_d2*(a1primeindexesfine_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
        entireRHS_ii_alt=reshape(ReturnMatrix_ii_alt+DiscountedEVinterp_alt(aprimez_alt),[N_d2*n2long*N_a2,N_a,N_z]);
        [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_alt,[],1);
        ...
        midpoint=max(min(midpoint,N_a1-1),2);
        a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
        ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, d2_gridvals, a1prime_grid(a1primeindexesfine), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
        aprimez=(1:1:N_d2)' + N_d2*(a1primeindexesfine-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
        entireRHS_ii=reshape(ReturnMatrix_ii+DiscountedEVinterp(aprimez),[N_d2*n2long*N_a2,N_a,N_z]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
