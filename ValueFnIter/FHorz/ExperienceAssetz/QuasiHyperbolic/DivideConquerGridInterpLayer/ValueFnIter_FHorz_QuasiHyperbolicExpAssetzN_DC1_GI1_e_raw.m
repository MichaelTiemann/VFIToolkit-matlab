function [Vtilde,Policy,Valt,Policyalt]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetzN_DC1_GI1_e_raw(n_d1,n_d2,n_a1,n_a2,n_z,n_e,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, beta0)
% Naive quasi-hyperbolic discounting + ExperienceAssetz, Divide-and-Conquer (DC1 over a1prime) with grid interpolation layer (GI1) on a1.
% d2 determines the experience asset a2, a1 is the standard endogenous state.
% z enters the experience-asset transition (aprimeFn=aprimeFn(d2,a2,z,...)), so
% a2primeIndex/a2primeProbs carry the current-z dimension; zprime is integrated out with pi_z_J
% inside EV before any discounting, so the dual touches only the discount factors, not the lottery.
...
for reverse_j=1:N_j-1
    ...
    EVpre=sum(shiftdim(pi_e_J(:,jj+1),-2).*Valt(:,:,:,jj+1),3); % integrate out e
