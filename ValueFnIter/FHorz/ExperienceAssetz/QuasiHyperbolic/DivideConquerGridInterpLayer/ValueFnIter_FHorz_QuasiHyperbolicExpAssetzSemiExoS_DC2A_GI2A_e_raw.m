...
if vfoptions.lowmemory==0
    midpoint_hat=zeros(N_d12,1,N_a2,N_a1,N_a2,N_a3,N_bothz,N_e,'gpuArray');
elseif vfoptions.lowmemory==1
    midpoint_hat=zeros(N_d12,1,N_a2,N_a1,N_a2,N_a3,N_bothz,1,'gpuArray');
...
Policy4_ford3_hat=zeros(4,N_a,N_bothz,N_e,N_d3,'gpuArray'); % (d12, midpoint, a2prime, L2ind)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3=reshape(ReturnMatrix_ii_d3+DiscountedEVinterp_hat(aprimez),[N_d12*n2long*N_a2,N_a,N_bothz,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                e_val=e_gridvals_J(e_c,:,N_j);
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d123_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
                entireRHS_ii_e=ReturnMatrix_ii_e+repelem(DiscountedEV_hat,N_d1,1,1,1,1,1,1);
                ...
                entireRHS_ii_e=reshape(ReturnMatrix_ii_e+DiscountedEVinterp_hat(aprime),[N_d12*n2long*N_a2,N_a,N_bothz,1]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_e,[],1);
            % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
            entireRHS_under=reshape(ReturnMatrix_ii_e+DiscountedEVinterp_under(aprime),[N_d12*n2long*N_a2,N_a,N_bothz,1]);
            maxindexfull=maxindexL2+(N_d12*n2long*N_a2)*(0:1:(N_a)-1)+shiftdim((N_d12*n2long*N_a2)*(N_a)*(0:1:(N_bothz)-1),-1)+shiftdim((N_d12*n2long*N_a2)*(N_a)*(N_bothz)*(0:1:(1)-1),-2);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, [n_semiz,ones(1,length(n_z))], special_n_e, d123_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, z_valblock, e_val, ReturnFnParamsVec, 1);
                    entireRHS_ii_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_zb_hat,N_d1,1,1,1,1,1,1);
                    ...
                    entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze+DiscountedEVinterp_zb_hat(aprime),[N_d12*n2long*N_a2,N_a,N_semiz]);
                    [Vtempii,maxindexL2]=max(entireRHS_ii_ze,[],1);
            % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
            entireRHS_under=reshape(ReturnMatrix_ii_ze+DiscountedEVinterp_zb_under(aprime),[N_d12*n2long*N_a2,N_a,N_semiz]);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, n_d1, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d123_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, z_val, e_val, ReturnFnParamsVec, 1);
                    entireRHS_ii_ze=ReturnMatrix_ii_ze+repelem(DiscountedEV_z_hat,N_d1,1,1,1,1,1);
                    [~,maxindex1]=max(entireRHS_ii_ze,[],2);
                    midpoint_hat(:,1,:,level1ii,:,:)=maxindex1;
                    maxgap=squeeze(max(max(max(max( maxindex1(:,1,:,2:end,:,:)-maxindex1(:,1,:,1:end-1,:,:), [],6),[],5),[],3),[],1));
                    for ii=1:(vfoptions.level1n-1)
                    ...
                    entireRHS_ii_ze=reshape(ReturnMatrix_ii_ze+DiscountedEVinterp_z_hat(aprime),[N_d12*n2long*N_a2,N_a]);
                    [Vtempii,maxindexL2]=max(entireRHS_ii_ze,[],1);
            % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
            entireRHS_under=reshape(ReturnMatrix_ii_ze+DiscountedEVinterp_z_under(aprime),[N_d12*n2long*N_a2,N_a]);
