...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        for d3_c=1:N_d3
            ...
            % Just keep the 'midpoint_hat' version of maxindex1 [as GI]
            ...
            % Turn this into the 'midpoint_hat'
            midpoint_hat=max(min(midpoint_hat,n_a1(1)-1),2); % avoid the top end (inner), and avoid the bottom end (outer)
            % midpoint_hat is n_d12-1-by-n_a1-by-n_a2-by-n_bothz
            a1primeindexesfine=(midpoint_hat+(midpoint_hat-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint_hat
            ...
            allind=d_ind+N_d12*aind+N_d12*N_a*bothzBind; % midpoint_hat is n_d12-by-1-by-n_a1-by-n_a2-by-n_bothz
            Policy4_ford3_hat(1,:,:,d3_c)=rem(d_ind-1,N_d1)+1; % d1
            Policy4_ford3_hat(2,:,:,d3_c)=ceil(d_ind/N_d1); % d2
            Policy4_ford3_hat(3,:,:,d3_c)=shiftdim(squeeze(midpoint_hat(allind)),-1); % a1prime midpoint_hat
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                % Turn this into the 'midpoint_hat'
                midpoint_hat=max(min(midpoint_hat,n_a1(1)-1),2); % avoid the top end (inner), and avoid the bottom end (outer)
                % midpoint_hat is n_d12-1-by-n_a1-by-n_a2-by-n_semiz
                a1primeindexesfine=(midpoint_hat+(midpoint_hat-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint_hat
                ...
                d_ind=rem(maxindexL2-1,N_d12)+1;
                allind=d_ind+N_d12*aind+N_d12*N_a*semizBind; % midpoint_hat is n_d12-by-1-by-n_a1-by-n_a2-by-n_semiz
                Policy4_ford3_hat(1,:,semizblock,d3_c)=rem(d_ind-1,N_d1)+1; % d1
                Policy4_ford3_hat(2,:,semizblock,d3_c)=ceil(d_ind/N_d1); % d2
                Policy4_ford3_hat(3,:,semizblock,d3_c)=shiftdim(squeeze(midpoint_hat(allind)),-1); % a1prime midpoint_hat
                Policy4_ford3_hat(4,:,semizblock,d3_c)=shiftdim(ceil(maxindexL2/N_d12),-1); % a1primeL2ind
                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                L2offset = ceil(maxindexL2/N_d12);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                % Turn this into the 'midpoint_hat'
                midpoint_hat=max(min(midpoint_hat,n_a1(1)-1),2); % avoid the top end (inner), and avoid the bottom end (outer)
                % midpoint_hat is n_d12-1-by-n_a1-by-n_a2
                a1primeindexesfine=(midpoint_hat+(midpoint_hat-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint_hat
                ...
                d_ind=rem(maxindexL2-1,N_d12)+1;
                allind=d_ind+N_d12*aind; % midpoint_hat is n_d12-by-1-by-n_a1-by-n_a2
                Policy4_ford3_hat(1,:,z_c,d3_c)=rem(d_ind-1,N_d1)+1; % d1
                Policy4_ford3_hat(2,:,z_c,d3_c)=ceil(d_ind/N_d1); % d2
                Policy4_ford3_hat(3,:,z_c,d3_c)=shiftdim(squeeze(midpoint_hat(allind)),-1); % a1prime midpoint_hat
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            DiscountedEVinterp_under=permute(interp1(a1_gridvals,permute(DiscountedEV_under,[2,1,3,4,5]),a1prime_grid),[2,1,3,4,5]); % [N_d2,N_a1prime,1,N_a2,N_bothz]
            % d1-dim is implicit singleton in DiscountedEV_under/DiscountedEVinterp_under, broadcasts at use sites

            % n-Monotonicity
            % Interpolate EV over aprime_grid
            DiscountedEVinterp_hat=permute(interp1(a1_gridvals,permute(DiscountedEV_hat,[2,1,3,4,5]),a1prime_grid),[2,1,3,4,5]); % [N_d2,N_a1prime,1,N_a2,N_bothz]
            % d1-dim is implicit singleton in DiscountedEV_hat/DiscountedEVinterp_hat, broadcasts at use sites
            ...
            entireRHS_ii_d3=ReturnMatrix_ii_d3+reshape(DiscountedEVinterp_hat(d2a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
            [Vtempii,maxindexL2]=max(entireRHS_ii_d3,[],1);
            % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
            entireRHS_under=ReturnMatrix_ii_d3+reshape(DiscountedEVinterp_under(d2a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_bothz]);
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                DiscountedEV_under=beta*EVbase_qh;
                DiscountedEV_hat=beta0beta*EVbase_qh;
                DiscountedEVinterp_under=permute(interp1(a1_gridvals,permute(DiscountedEV_under,[2,1,3,4,5]),a1prime_grid),[2,1,3,4,5]); % [N_d2,N_a1prime,1,N_a2,N_semiz]

                % n-Monotonicity
                DiscountedEVinterp_hat=permute(interp1(a1_gridvals,permute(DiscountedEV_hat,[2,1,3,4,5]),a1prime_grid),[2,1,3,4,5]); % [N_d2,N_a1prime,1,N_a2,N_semiz]
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        % loweredge is n_d12-by-1-by-1-by-n_a2-by-n_semiz
                        a1primeindexes=loweredge+(0:1:maxgap(ii));
                        ReturnMatrix_ii_d3z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, ReturnFnParamsVec,3,0); % Level=3, Refine=0
                        d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind; % linear index into DiscountedEV_hat [N_d2,N_a1,1,N_a2,N_semiz]
                ...
                % midpoint_hat is n_d12-1-by-n_a1-by-n_a2-by-n_semiz
                a1primeindexesfine=(midpoint_hat+(midpoint_hat-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint_hat
                ReturnMatrix_ii_d3z=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,[n_d2,1],n2long,n_a1,n_a2,[n_semiz,ones(1,length(n_z))], d123_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_valblock, ReturnFnParamsVec,2,0); % [N_d12,N_a1prime,N_a1,N_a2,N_semiz]; Level=2, Refine=0
                d2a1primea2bothz=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*semizind;
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+reshape(DiscountedEVinterp_hat(d2a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3z,[],1);
                % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
                entireRHS_under=ReturnMatrix_ii_d3z+reshape(DiscountedEVinterp_under(d2a1primea2bothz(:)),[N_d12*n2long,N_a1*N_a2,N_semiz]);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+repelem(DiscountedEV_z_hat,N_d1,1,1,1);
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+reshape(DiscountedEVinterp_z_hat(d2a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3z,[],1);
            % Vunderbar: the beta fine-RHS gathered at the hat argmax (not re-maximised)
            entireRHS_under=ReturnMatrix_ii_d3z+reshape(DiscountedEVinterp_z_under(d2a1primea2(:)),[N_d12*n2long,N_a1*N_a2]);
