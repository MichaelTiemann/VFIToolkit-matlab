function [Vhat,Policy,Vunderbar]=ValueFnIter_FHorz_QuasiHyperbolicExpAssetzS_DC2A_GI2A_nod1_e_raw(n_d2, n_a1, n_a2, n_a3, n_z, n_e, N_j, d2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
% Sophisticated QH + ExpAssetz, DC2A_GI2A pattern (nod1, +e).
% Single DC coarse + GI fine pass on entireRHS_hat (beta0*beta) -> Vhat, Policy.
% Vunderbar = lookup at the hat-maximizer in entireRHS_under (beta only).
% lowmemory=0 full vectorization; lowmemory=1 loops z; lowmemory=2 nested z+e.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            a1primeindexesfine_e=(midpoint_z+(midpoint_z-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, n_d2, n_a2, n_a3, n_z, special_n_e, d2_gridvals, a1prime_grid(a1primeindexesfine_e), a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 2);
            [Vtempii,maxindexL2]=max(ReturnMatrix_ii_e,[],1);
            Vhat(:,:,e_c,N_j)=shiftdim(Vtempii,1);
            d_ind        =rem(maxindexL2-1,N_d2)+1;
            maxindexL2a1 =rem(floor((maxindexL2-1)/N_d2),n2long)+1;
            maxindexL2a2 =floor((maxindexL2-1)/(N_d2*n2long))+1;
            allind_e=d_ind + N_d2*(maxindexL2a2-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*zindB;
            Policy(1,:,:,e_c,N_j)=d_ind;
            Policy(2,:,:,e_c,N_j)=midpoint_z(allind_e);
    ...
    if vfoptions.lowmemory==0
        ...
        Vunderbar(:,:,:,N_j)=shiftdim(entireRHS_ii_under(maxindexfull),1);
        d_ind        =rem(maxindexL2-1,N_d2)+1;
        maxindexL2a1 =rem(floor((maxindexL2-1)/N_d2),n2long)+1;
        maxindexL2a2 =floor((maxindexL2-1)/(N_d2*n2long))+1;
        allind=d_ind + N_d2*(maxindexL2a2-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*zindB + N_d2*N_a2*N_a*N_z*eindB;
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            entireRHS_hat_e=ReturnMatrix_ii_e+DiscountedEV_hat;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=(1:1:N_d2)' + N_d2*(a1primeindexes-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_z-1),-5);
                    entireRHS_inner=ReturnMatrix_inner_e+DiscountedEV_hat(d2aprimez);
                    [~,maxindex_inner]=max(entireRHS_inner,[],2);
                    midpoint_z(:,1,:,curra1inner,:,:,:,:)=maxindex_inner+(loweredge-1);
                else
                    loweredge=maxindex1(:,1,:,ii,:,:,:,:);
            ...
            [Vtempii,maxindexL2]=max(entireRHS_ii_hat_e,[],1);
            Vhat(:,:,e_c,N_j)=shiftdim(Vtempii,1);
            firstdim=N_d2*n2long*N_a2;
            maxindexfull_e=maxindexL2 + firstdim*aind + firstdim*N_a*zindB;
            Vunderbar(:,:,e_c,N_j)=shiftdim(entireRHS_ii_under_e(maxindexfull_e),1);
            d_ind        =rem(maxindexL2-1,N_d2)+1;
            maxindexL2a1 =rem(floor((maxindexL2-1)/N_d2),n2long)+1;
            maxindexL2a2 =floor((maxindexL2-1)/(N_d2*n2long))+1;
            allind_e=d_ind + N_d2*(maxindexL2a2-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*zindB;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Vunderbar(:,z_c,e_c,N_j)=entireRHS_ii_under_ze(maxindexfull_ze);
                d_ind        =rem(maxindexL2-1,N_d2)+1;
                maxindexL2a1 =rem(floor((maxindexL2-1)/N_d2),n2long)+1;
                maxindexL2a2 =floor((maxindexL2-1)/(N_d2*n2long))+1;
                allind_ze=d_ind + N_d2*(maxindexL2a2-1) + N_d2*N_a2*aind;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        firstdim=N_d2*n2long*N_a2;
        maxindexfull=maxindexL2 + firstdim*aind + firstdim*N_a*zindB + firstdim*N_a*N_z*eindB;
        Vunderbar(:,:,:,jj)=shiftdim(entireRHS_ii_under(maxindexfull),1);
        d_ind        =rem(maxindexL2-1,N_d2)+1;
        maxindexL2a1 =rem(floor((maxindexL2-1)/N_d2),n2long)+1;
        maxindexL2a2 =floor((maxindexL2-1)/(N_d2*n2long))+1;
        allind=d_ind + N_d2*(maxindexL2a2-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*zindB + N_d2*N_a2*N_a*N_z*eindB;
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            firstdim=N_d2*n2long*N_a2;
            maxindexfull_e=maxindexL2 + firstdim*aind + firstdim*N_a*zindB;
            Vunderbar(:,:,e_c,jj)=shiftdim(entireRHS_ii_under_e(maxindexfull_e),1);
            d_ind        =rem(maxindexL2-1,N_d2)+1;
            maxindexL2a1 =rem(floor((maxindexL2-1)/N_d2),n2long)+1;
            maxindexL2a2 =floor((maxindexL2-1)/(N_d2*n2long))+1;
            allind_e=d_ind + N_d2*(maxindexL2a2-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*zindB;
            Policy(1,:,:,e_c,jj)=d_ind;
            Policy(2,:,:,e_c,jj)=midpoint_z(allind_e);
