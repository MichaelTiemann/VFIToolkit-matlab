...
% Preallocate
if vfoptions.lowmemory==0
    midpoint_alt=zeros(N_d2,1,N_a1,N_a2,N_bothz,N_e,'gpuArray');
    midpoint_tilde=zeros(N_d2,1,N_a1,N_a2,N_bothz,N_e,'gpuArray');
elseif vfoptions.lowmemory==1
    midpoint_alt=zeros(N_d2,1,N_a1,N_a2,N_bothz,'gpuArray');
    midpoint_tilde=zeros(N_d2,1,N_a1,N_a2,N_bothz,'gpuArray');
elseif vfoptions.lowmemory==2
    midpoint_alt=zeros(N_d2,1,N_a1,N_a2,N_semiz,'gpuArray');
    midpoint_tilde=zeros(N_d2,1,N_a1,N_a2,N_semiz,'gpuArray');
elseif vfoptions.lowmemory==3
    midpoint_alt=zeros(N_d2,1,N_a1,N_a2,'gpuArray');
    midpoint_tilde=zeros(N_d2,1,N_a1,N_a2,'gpuArray');
end
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                Policy3_ford3_alt(2,:,:,e_c,d3_c)=shiftdim(squeeze(midpoint_alt(allind)),-1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,1,0);
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap(ii)>0
                            loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                            a1primeindexes=loweredge+(0:1:maxgap(ii));
                            ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                    ...
                    midpoint_alt=max(min(midpoint_alt,n_a1(1)-1),2);
                    a1primeindexesfine=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,2,0);
                    ...
                    Policy3_ford3_alt(1,:,semizblock,e_c,d3_c)=d_ind;
                    Policy3_ford3_alt(2,:,semizblock,e_c,d3_c)=shiftdim(squeeze(midpoint_alt(allind)),-1);
                    Policy3_ford3_alt(3,:,semizblock,e_c,d3_c)=shiftdim(ceil(maxindexL2/N_d2),-1);
                    % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                    L2offset = ceil(maxindexL2/N_d2);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    Policy3_ford3_alt(2,:,z_c,e_c,d3_c)=shiftdim(squeeze(midpoint_alt(allind)),-1);
    ...
    Policyalt(1,:,:,:,N_j)=reshape(Policy3_ford3_alt(1+temp),[1,N_a,N_bothz,N_e]);
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);

            entireRHS_ii_d3_alt=ReturnMatrix_ii_d3_alt+DiscountedEV_alt; % broadcasts over e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    ...
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_alt(ii)+1,level1iidiff(ii),n_a2,n_bothz,n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                    d2aprimez_alt=(1:1:N_d2)'+N_d2*(a1primeindexes_alt-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind;
            ...
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,n_bothz,n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
            d2a1primea2bothz_alt=(1:1:N_d2)'+N_d2*(a1primeindexesfine_alt-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*bothzind;
            entireRHS_ii_d3_alt=ReturnMatrix_ii_d3_alt+reshape(DiscountedEVinterp_alt(d2a1primea2bothz_alt(:)),[N_d2*n2long,N_a1*N_a2,N_bothz,N_e]);
            ...
            allind_alt=d_ind_alt+N_d2*aind+N_d2*N_a*bothzBind+N_d2*N_a*N_bothz*eBind;
            ...
            linidx_lower_alt = d_ind_alt                  + N_d2*n2long*aind + N_d2*n2long*N_a*bothzBind + N_d2*n2long*N_a*N_bothz*eBind;
            linidx_upper_alt = d_ind_alt + N_d2*(n2long-1) + N_d2*n2long*aind + N_d2*n2long*N_a*bothzBind + N_d2*n2long*N_a*N_bothz*eBind;
            ...
            % --- tilde pass ---
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,1,0);

            entireRHS_ii_d3_tilde=ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde; % broadcasts over e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_tilde(ii)>0
                    ...
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_tilde(ii)+1,level1iidiff(ii),n_a2,n_bothz,n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3,0);
                    d2aprimez_tilde=(1:1:N_d2)'+N_d2*(a1primeindexes_tilde-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind;
            ...
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,n_bothz,n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2,0);
            d2a1primea2bothz_tilde=(1:1:N_d2)'+N_d2*(a1primeindexesfine_tilde-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*bothzind;
            entireRHS_ii_d3_tilde=ReturnMatrix_ii_d3_tilde+reshape(DiscountedEVinterp_tilde(d2a1primea2bothz_tilde(:)),[N_d2*n2long,N_a1*N_a2,N_bothz,N_e]);
            ...
            allind_tilde=d_ind_tilde+N_d2*aind+N_d2*N_a*bothzBind+N_d2*N_a*N_bothz*eBind;
            ...
            linidx_lower_tilde = d_ind_tilde                  + N_d2*n2long*aind + N_d2*n2long*N_a*bothzBind + N_d2*n2long*N_a*N_bothz*eBind;
            linidx_upper_tilde = d_ind_tilde + N_d2*(n2long-1) + N_d2*n2long*aind + N_d2*n2long*N_a*bothzBind + N_d2*n2long*N_a*N_bothz*eBind;
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1,0);

                entireRHS_ii_d3e_alt=ReturnMatrix_ii_d3e_alt+DiscountedEV_alt;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_alt(ii)>0
                        loweredge_alt=min(maxindex1_alt(:,1,ii,:,:),N_a1-maxgap_alt(ii));
                        a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                        ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_alt(ii)+1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0);
                        d2aprimez_alt=(1:1:N_d2)'+N_d2*(a1primeindexes_alt-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind;
                ...
                midpoint_alt=max(min(midpoint_alt,n_a1(1)-1),2);
                a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,n_bothz,special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2,0);
                d2a1primea2bothz_alt=(1:1:N_d2)'+N_d2*(a1primeindexesfine_alt-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*bothzind;
                entireRHS_ii_d3e_alt=ReturnMatrix_ii_d3e_alt+reshape(DiscountedEVinterp_alt(d2a1primea2bothz_alt(:)),[N_d2*n2long,N_a1*N_a2,N_bothz]);
                ...
                Policy3_ford3_alt(1,:,:,e_c,d3_c)=d_ind_alt;
                Policy3_ford3_alt(2,:,:,e_c,d3_c)=shiftdim(squeeze(midpoint_alt(allind_alt)),-1);
                Policy3_ford3_alt(3,:,:,e_c,d3_c)=shiftdim(ceil(maxindexL2_alt/N_d2),-1);
                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                L2offset_alt = ceil(maxindexL2_alt/N_d2);
            ...
            % --- tilde pass ---
                ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,1,0);

                entireRHS_ii_d3e_tilde=ReturnMatrix_ii_d3e_tilde+DiscountedEV_tilde;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_tilde(ii)>0
                        loweredge_tilde=min(maxindex1_tilde(:,1,ii,:,:),N_a1-maxgap_tilde(ii));
                        a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                        ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_tilde(ii)+1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3,0);
                        d2aprimez_tilde=(1:1:N_d2)'+N_d2*(a1primeindexes_tilde-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind;
                ...
                midpoint_tilde=max(min(midpoint_tilde,n_a1(1)-1),2);
                a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,n_bothz,special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2,0);
                d2a1primea2bothz_tilde=(1:1:N_d2)'+N_d2*(a1primeindexesfine_tilde-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*bothzind;
                entireRHS_ii_d3e_tilde=ReturnMatrix_ii_d3e_tilde+reshape(DiscountedEVinterp_tilde(d2a1primea2bothz_tilde(:)),[N_d2*n2long,N_a1*N_a2,N_bothz]);
                ...
                Policy3_ford3_tilde(1,:,:,e_c,d3_c)=d_ind_tilde;
                Policy3_ford3_tilde(2,:,:,e_c,d3_c)=shiftdim(squeeze(midpoint_tilde(allind_tilde)),-1);
                Policy3_ford3_tilde(3,:,:,e_c,d3_c)=shiftdim(ceil(maxindexL2_tilde/N_d2),-1);
                % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                L2offset_tilde = ceil(maxindexL2_tilde/N_d2);
    ...
    elseif vfoptions.lowmemory==2 % split: vectorize semiz, outer loop z, inner loop e
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                % --- alt pass ---
                    ReturnMatrix_ii_d3ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,1,0);

                    entireRHS_ii_d3ze_alt=ReturnMatrix_ii_d3ze_alt+DiscountedEV_alt;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_alt(ii)>0
                            loweredge_alt=min(maxindex1_alt(:,1,ii,:,:),N_a1-maxgap_alt(ii));
                            a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                            ReturnMatrix_ii_d3ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_alt(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                            d2aprimez_alt=(1:1:N_d2)'+N_d2*(a1primeindexes_alt-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind;
                    ...
                    midpoint_alt=max(min(midpoint_alt,n_a1(1)-1),2);
                    a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_d3ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a1_gridvals, a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,2,0);
                    d2a1primea2bothz_alt=(1:1:N_d2)'+N_d2*(a1primeindexesfine_alt-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*semizind;
                    entireRHS_ii_d3ze_alt=ReturnMatrix_ii_d3ze_alt+reshape(DiscountedEVinterp_alt(d2a1primea2bothz_alt(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
                    ...
                    Policy3_ford3_alt(1,:,semizblock,e_c,d3_c)=d_ind_alt;
                    Policy3_ford3_alt(2,:,semizblock,e_c,d3_c)=shiftdim(squeeze(midpoint_alt(allind_alt)),-1);
                    Policy3_ford3_alt(3,:,semizblock,e_c,d3_c)=shiftdim(ceil(maxindexL2_alt/N_d2),-1);
                    % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                    L2offset_alt = ceil(maxindexL2_alt/N_d2);
                ...
                % --- tilde pass ---
                    ReturnMatrix_ii_d3ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,1,0);

                    entireRHS_ii_d3ze_tilde=ReturnMatrix_ii_d3ze_tilde+DiscountedEV_tilde;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_tilde(ii)>0
                            loweredge_tilde=min(maxindex1_tilde(:,1,ii,:,:),N_a1-maxgap_tilde(ii));
                            a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                            ReturnMatrix_ii_d3ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_tilde(ii)+1,level1iidiff(ii),n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,3,0);
                            d2aprimez_tilde=(1:1:N_d2)'+N_d2*(a1primeindexes_tilde-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*semizind;
                    ...
                    midpoint_tilde=max(min(midpoint_tilde,n_a1(1)-1),2);
                    a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_d3ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,[n_semiz,ones(1,length(n_z))],special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a1_gridvals, a2_gridvals, z_valblock, e_val, ReturnFnParamsVec,2,0);
                    d2a1primea2bothz_tilde=(1:1:N_d2)'+N_d2*(a1primeindexesfine_tilde-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*semizind;
                    entireRHS_ii_d3ze_tilde=ReturnMatrix_ii_d3ze_tilde+reshape(DiscountedEVinterp_tilde(d2a1primea2bothz_tilde(:)),[N_d2*n2long,N_a1*N_a2,N_semiz]);
                    ...
                    Policy3_ford3_tilde(1,:,semizblock,e_c,d3_c)=d_ind_tilde;
                    Policy3_ford3_tilde(2,:,semizblock,e_c,d3_c)=shiftdim(squeeze(midpoint_tilde(allind_tilde)),-1);
                    Policy3_ford3_tilde(3,:,semizblock,e_c,d3_c)=shiftdim(ceil(maxindexL2_tilde/N_d2),-1);
                    % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                    L2offset_tilde = ceil(maxindexL2_tilde/N_d2);
        ...
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_ii_d3ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,special_n_bothz,special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);

                    entireRHS_ii_d3ze_alt=ReturnMatrix_ii_d3ze_alt+DiscountedEV_z_alt;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_alt(ii)>0
                            loweredge_alt=min(maxindex1_alt(:,1,ii,:),N_a1-maxgap_alt(ii));
                            a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                            ReturnMatrix_ii_d3ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_alt(ii)+1,level1iidiff(ii),n_a2,special_n_bothz,special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0);
                            d2aprime_alt=(1:1:N_d2)'+N_d2*(a1primeindexes_alt-1)+N_d2*N_a1*a2ind;
                    ...
                    midpoint_alt=max(min(midpoint_alt,n_a1(1)-1),2);
                    a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_d3ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,special_n_bothz,special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,2,0);
                    d2a1primea2_alt=(1:1:N_d2)'+N_d2*(a1primeindexesfine_alt-1)+N_d2*N_a1prime*a2ind;
                    entireRHS_ii_d3ze_alt=ReturnMatrix_ii_d3ze_alt+reshape(DiscountedEVinterp_z_alt(d2a1primea2_alt(:)),[N_d2*n2long,N_a1*N_a2]);
                    [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_d3ze_alt,[],1);
                    V_ford3_alt(:,z_c,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                    d_ind_alt=rem(maxindexL2_alt-1,N_d2)+1;
                    allind_alt=d_ind_alt+N_d2*aind;
                    Policy3_ford3_alt(1,:,z_c,e_c,d3_c)=d_ind_alt;
                    Policy3_ford3_alt(2,:,z_c,e_c,d3_c)=shiftdim(squeeze(midpoint_alt(allind_alt)),-1);
                    Policy3_ford3_alt(3,:,z_c,e_c,d3_c)=shiftdim(ceil(maxindexL2_alt/N_d2),-1);
                    % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                    L2offset_alt = ceil(maxindexL2_alt/N_d2);
            ...
            % --- tilde pass ---
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_ii_d3ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,special_n_bothz,special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_val, e_val, ReturnFnParamsVec,1,0);

                    entireRHS_ii_d3ze_tilde=ReturnMatrix_ii_d3ze_tilde+DiscountedEV_z_tilde;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_tilde(ii)>0
                            loweredge_tilde=min(maxindex1_tilde(:,1,ii,:),N_a1-maxgap_tilde(ii));
                            a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                            ReturnMatrix_ii_d3ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_tilde(ii)+1,level1iidiff(ii),n_a2,special_n_bothz,special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, ReturnFnParamsVec,3,0);
                            d2aprime_tilde=(1:1:N_d2)'+N_d2*(a1primeindexes_tilde-1)+N_d2*N_a1*a2ind;
                    ...
                    midpoint_tilde=max(min(midpoint_tilde,n_a1(1)-1),2);
                    a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_d3ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,special_n_bothz,special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a1_gridvals, a2_gridvals, z_val, e_val, ReturnFnParamsVec,2,0);
                    d2a1primea2_tilde=(1:1:N_d2)'+N_d2*(a1primeindexesfine_tilde-1)+N_d2*N_a1prime*a2ind;
                    entireRHS_ii_d3ze_tilde=ReturnMatrix_ii_d3ze_tilde+reshape(DiscountedEVinterp_z_tilde(d2a1primea2_tilde(:)),[N_d2*n2long,N_a1*N_a2]);
                    [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_d3ze_tilde,[],1);
                    V_ford3_tilde(:,z_c,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                    d_ind_tilde=rem(maxindexL2_tilde-1,N_d2)+1;
                    allind_tilde=d_ind_tilde+N_d2*aind;
                    Policy3_ford3_tilde(1,:,z_c,e_c,d3_c)=d_ind_tilde;
                    Policy3_ford3_tilde(2,:,z_c,e_c,d3_c)=shiftdim(squeeze(midpoint_tilde(allind_tilde)),-1);
                    Policy3_ford3_tilde(3,:,z_c,e_c,d3_c)=shiftdim(ceil(maxindexL2_tilde/N_d2),-1);
                    % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
                    L2offset_tilde = ceil(maxindexL2_tilde/N_d2);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,1,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    loweredge_alt=min(maxindex1_alt(:,1,ii,:,:,:),N_a1-maxgap_alt(ii));
                    a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_alt(ii)+1,level1iidiff(ii),n_a2,n_bothz,n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    d2aprimez_alt=(1:1:N_d2)'+N_d2*(a1primeindexes_alt-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind;
                    entireRHS_ii_d3_alt=ReturnMatrix_ii_d3_alt+DiscountedEV_alt(d2aprimez_alt);
                    [~,maxindex_alt]=max(entireRHS_ii_d3_alt,[],2);
                    midpoint_alt(:,1,curraindex_alt,:,:,:)=maxindex_alt+(loweredge_alt-1);
            ...
            midpoint_alt=max(min(midpoint_alt,n_a1(1)-1),2);
            a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,n_bothz,n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0);
            d2a1primea2bothz_alt=(1:1:N_d2)'+N_d2*(a1primeindexesfine_alt-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*bothzind;
            entireRHS_ii_d3_alt=ReturnMatrix_ii_d3_alt+reshape(DiscountedEVinterp_alt(d2a1primea2bothz_alt(:)),[N_d2*n2long,N_a1*N_a2,N_bothz,N_e]);
            [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_d3_alt,[],1);
            ...
            % --- tilde pass ---
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,1,0);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_tilde(ii)>0
                    loweredge_tilde=min(maxindex1_tilde(:,1,ii,:,:,:),N_a1-maxgap_tilde(ii));
                    a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_tilde(ii)+1,level1iidiff(ii),n_a2,n_bothz,n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,3,0);
                    d2aprimez_tilde=(1:1:N_d2)'+N_d2*(a1primeindexes_tilde-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind;
                    entireRHS_ii_d3_tilde=ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde(d2aprimez_tilde);
                    [~,maxindex_tilde]=max(entireRHS_ii_d3_tilde,[],2);
                    midpoint_tilde(:,1,curraindex_tilde,:,:,:)=maxindex_tilde+(loweredge_tilde-1);
            ...
            midpoint_tilde=max(min(midpoint_tilde,n_a1(1)-1),2);
            a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,n_bothz,n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2,0);
            d2a1primea2bothz_tilde=(1:1:N_d2)'+N_d2*(a1primeindexesfine_tilde-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*bothzind;
            entireRHS_ii_d3_tilde=ReturnMatrix_ii_d3_tilde+reshape(DiscountedEVinterp_tilde(d2a1primea2bothz_tilde(:)),[N_d2*n2long,N_a1*N_a2,N_bothz,N_e]);
            [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_d3_tilde,[],1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,1,0);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_alt(ii)>0
                        loweredge_alt=min(maxindex1_alt(:,1,ii,:,:),N_a1-maxgap_alt(ii));
                        a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                        ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_alt(ii)+1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_alt), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0);
                        d2aprimez_alt=(1:1:N_d2)'+N_d2*(a1primeindexes_alt-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind;
                        entireRHS_ii_d3e_alt=ReturnMatrix_ii_d3e_alt+DiscountedEV_alt(d2aprimez_alt);
                        [~,maxindex_alt]=max(entireRHS_ii_d3e_alt,[],2);
                        midpoint_alt(:,1,curraindex_alt,:,:)=maxindex_alt+(loweredge_alt-1);
                ...
                midpoint_alt=max(min(midpoint_alt,n_a1(1)-1),2);
                a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_d3e_alt=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,n_bothz,special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2,0);
                d2a1primea2bothz_alt=(1:1:N_d2)'+N_d2*(a1primeindexesfine_alt-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*bothzind;
                entireRHS_ii_d3e_alt=ReturnMatrix_ii_d3e_alt+reshape(DiscountedEVinterp_alt(d2a1primea2bothz_alt(:)),[N_d2*n2long,N_a1*N_a2,N_bothz]);
                [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_d3e_alt,[],1);
                V_ford3_alt(:,:,e_c,d3_c)=shiftdim(Vtempii_alt,1);
            ...
            % --- tilde pass ---
                ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n_a1,vfoptions.level1n,n_a2,n_bothz,special_n_e, d23_gridvals_val, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,1,0);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_tilde(ii)>0
                        loweredge_tilde=min(maxindex1_tilde(:,1,ii,:,:),N_a1-maxgap_tilde(ii));
                        a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                        ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],maxgap_tilde(ii)+1,level1iidiff(ii),n_a2,n_bothz,special_n_e, d23_gridvals_val, a1_gridvals(a1primeindexes_tilde), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,3,0);
                        d2aprimez_tilde=(1:1:N_d2)'+N_d2*(a1primeindexes_tilde-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*bothzind;
                        entireRHS_ii_d3e_tilde=ReturnMatrix_ii_d3e_tilde+DiscountedEV_tilde(d2aprimez_tilde);
                        [~,maxindex_tilde]=max(entireRHS_ii_d3e_tilde,[],2);
                        midpoint_tilde(:,1,curraindex_tilde,:,:)=maxindex_tilde+(loweredge_tilde-1);
                ...
                midpoint_tilde=max(min(midpoint_tilde,n_a1(1)-1),2);
                a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_d3e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, 0,[n_d2,1],n2long,n_a1,n_a2,n_bothz,special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a1_gridvals, a2_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2,0);
                d2a1primea2bothz_tilde=(1:1:N_d2)'+N_d2*(a1primeindexesfine_tilde-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*bothzind;
                entireRHS_ii_d3e_tilde=ReturnMatrix_ii_d3e_tilde+reshape(DiscountedEVinterp_tilde(d2a1primea2bothz_tilde(:)),[N_d2*n2long,N_a1*N_a2,N_bothz]);
                [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_d3e_tilde,[],1);
                V_ford3_tilde(:,:,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
    ...
    maxindex=reshape(maxindex,[N_a*N_bothz*N_e,1]);
    temp=3*( (1:1:N_a*N_bothz*N_e)'+(N_a*N_bothz*N_e)*(maxindex-1) -1);
    Policyalt(1,:,:,:,jj)=reshape(Policy3_ford3_alt(1+temp),[1,N_a,N_bothz,N_e]);
