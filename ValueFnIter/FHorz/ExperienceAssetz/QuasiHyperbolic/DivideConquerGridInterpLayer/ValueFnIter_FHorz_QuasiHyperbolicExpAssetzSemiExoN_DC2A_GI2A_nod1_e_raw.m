...
if vfoptions.lowmemory==0
    midpoint_alt=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,N_bothz,N_e,'gpuArray');
    midpoint_tilde=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,N_bothz,N_e,'gpuArray');
elseif vfoptions.lowmemory==1
    midpoint_alt=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,N_bothz,1,'gpuArray');
    midpoint_tilde=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,N_bothz,1,'gpuArray');
elseif vfoptions.lowmemory==2
    midpoint_alt=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,N_semiz,'gpuArray');
    midpoint_tilde=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,N_semiz,'gpuArray');
elseif vfoptions.lowmemory==3
    midpoint_alt=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,'gpuArray');
    midpoint_tilde=zeros(N_d2,1,N_a2,N_a1,N_a2,N_a3,'gpuArray');
end
...
Policy4_ford3_tilde=zeros(4,N_a,N_bothz,N_e,N_d3,'gpuArray'); % (d2, midpoint, a2prime, L2ind)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 1);
            entireRHS_ii_d3_alt=ReturnMatrix_ii_d3_alt+DiscountedEV_alt;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    ...
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                    d2aprimez_alt=(1:1:N_d2)' + N_d2*(a1primeindexes_alt-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            ...
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
            aprimez=(1:1:N_d2)' + N_d2*(a1primeindexesfine_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            entireRHS_ii_d3_alt=reshape(ReturnMatrix_ii_d3_alt+DiscountedEVinterp_alt(aprimez),[N_d2*n2long*N_a2,N_a,N_bothz,N_e]);
            ...
            allind_alt=d_ind_alt + N_d2*(maxindexL2a2_alt-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*bothzBind + N_d2*N_a2*N_a*N_bothz*eBind;
            ...
            ReturnMatrix_ii_flat_alt=reshape(ReturnMatrix_ii_d3_alt,[N_d2*n2long*N_a2,N_a,N_bothz,N_e]);
            linidx_lower_alt=d_ind_alt                + N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*bothzBind + N_d2*n2long*N_a2*N_a*N_bothz*eBind;
            linidx_upper_alt=d_ind_alt + N_d2*(n2long-1)+ N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*bothzBind + N_d2*n2long*N_a2*N_a*N_bothz*eBind;
            ...
            % --- tilde pass ---
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 1);
            entireRHS_ii_d3_tilde=ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_tilde(ii)>0
                    ...
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
                    d2aprimez_tilde=(1:1:N_d2)' + N_d2*(a1primeindexes_tilde-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            ...
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec, 3);
            aprimez=(1:1:N_d2)' + N_d2*(a1primeindexesfine_tilde-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            entireRHS_ii_d3_tilde=reshape(ReturnMatrix_ii_d3_tilde+DiscountedEVinterp_tilde(aprimez),[N_d2*n2long*N_a2,N_a,N_bothz,N_e]);
            ...
            maxindexL2a1_tilde =rem(floor((maxindexL2_tilde-1)/N_d2),n2long)+1;
            maxindexL2a2_tilde =floor((maxindexL2_tilde-1)/(N_d2*n2long))+1;
            allind_tilde=d_ind_tilde + N_d2*(maxindexL2a2_tilde-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*bothzBind + N_d2*N_a2*N_a*N_bothz*eBind;
            ...
            ReturnMatrix_ii_flat_tilde=reshape(ReturnMatrix_ii_d3_tilde,[N_d2*n2long*N_a2,N_a,N_bothz,N_e]);
            linidx_lower_tilde=d_ind_tilde                + N_d2*n2long*(maxindexL2a2_tilde-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*bothzBind + N_d2*n2long*N_a2*N_a*N_bothz*eBind;
            linidx_upper_tilde=d_ind_tilde + N_d2*(n2long-1)+ N_d2*n2long*(maxindexL2a2_tilde-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*bothzBind + N_d2*n2long*N_a2*N_a*N_bothz*eBind;
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                ReturnMatrix_ii_e_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
                entireRHS_ii_e_alt=ReturnMatrix_ii_e_alt+DiscountedEV_alt;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_alt(ii)>0
                        loweredge_alt=min(maxindex1_alt(:,1,:,ii,:,:,:,:),N_a1-maxgap_alt(ii));
                        a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                        ReturnMatrix_ii_e_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                        d2aprime_alt=(1:1:N_d2)' + N_d2*(a1primeindexes_alt-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                ...
                midpoint_alt=max(min(midpoint_alt,N_a1-1),2);
                a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_e_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                aprime=(1:1:N_d2)' + N_d2*(a1primeindexesfine_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                entireRHS_ii_e_alt=reshape(ReturnMatrix_ii_e_alt+DiscountedEVinterp_alt(aprime),[N_d2*n2long*N_a2,N_a,N_bothz,1]);
                [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_e_alt,[],1);
                V_ford3_alt(:,:,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                d_ind_alt        =rem(maxindexL2_alt-1,N_d2)+1;
                maxindexL2a1_alt =rem(floor((maxindexL2_alt-1)/N_d2),n2long)+1;
                maxindexL2a2_alt =floor((maxindexL2_alt-1)/(N_d2*n2long))+1;
                allind_alt=d_ind_alt + N_d2*(maxindexL2a2_alt-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*bothzBind;
                ...
                ReturnMatrix_ii_flat_alt=reshape(ReturnMatrix_ii_e_alt,[N_d2*n2long*N_a2,N_a,N_bothz,1]);
                linidx_lower_alt=d_ind_alt                + N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*bothzBind;
                linidx_upper_alt=d_ind_alt + N_d2*(n2long-1)+ N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*bothzBind;
                isInfLower_alt=(ReturnMatrix_ii_flat_alt(linidx_lower_alt)==-Inf);
                isInfUpper_alt=(ReturnMatrix_ii_flat_alt(linidx_upper_alt)==-Inf);
                inLowerStrict_alt=(maxindexL2a1_alt>=2)         & (maxindexL2a1_alt<=n2short+1);
                inUpperStrict_alt=(maxindexL2a1_alt>=n2short+3) & (maxindexL2a1_alt<=n2long-1);
            ...
            % --- tilde pass ---
                ReturnMatrix_ii_e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 1);
                entireRHS_ii_e_tilde=ReturnMatrix_ii_e_tilde+DiscountedEV_tilde;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_tilde(ii)>0
                        loweredge_tilde=min(maxindex1_tilde(:,1,:,ii,:,:,:,:),N_a1-maxgap_tilde(ii));
                        a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                        ReturnMatrix_ii_e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                        d2aprime_tilde=(1:1:N_d2)' + N_d2*(a1primeindexes_tilde-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                ...
                midpoint_tilde=max(min(midpoint_tilde,N_a1-1),2);
                a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec, 3);
                aprime=(1:1:N_d2)' + N_d2*(a1primeindexesfine_tilde-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                entireRHS_ii_e_tilde=reshape(ReturnMatrix_ii_e_tilde+DiscountedEVinterp_tilde(aprime),[N_d2*n2long*N_a2,N_a,N_bothz,1]);
                [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_e_tilde,[],1);
                V_ford3_tilde(:,:,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                d_ind_tilde        =rem(maxindexL2_tilde-1,N_d2)+1;
                maxindexL2a1_tilde =rem(floor((maxindexL2_tilde-1)/N_d2),n2long)+1;
                maxindexL2a2_tilde =floor((maxindexL2_tilde-1)/(N_d2*n2long))+1;
                allind_tilde=d_ind_tilde + N_d2*(maxindexL2a2_tilde-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*bothzBind;
                ...
                ReturnMatrix_ii_flat_tilde=reshape(ReturnMatrix_ii_e_tilde,[N_d2*n2long*N_a2,N_a,N_bothz,1]);
                linidx_lower_tilde=d_ind_tilde                + N_d2*n2long*(maxindexL2a2_tilde-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*bothzBind;
                linidx_upper_tilde=d_ind_tilde + N_d2*(n2long-1)+ N_d2*n2long*(maxindexL2a2_tilde-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*bothzBind;
                isInfLower_tilde=(ReturnMatrix_ii_flat_tilde(linidx_lower_tilde)==-Inf);
                isInfUpper_tilde=(ReturnMatrix_ii_flat_tilde(linidx_upper_tilde)==-Inf);
                inLowerStrict_tilde=(maxindexL2a1_tilde>=2)         & (maxindexL2a1_tilde<=n2short+1);
                inUpperStrict_tilde=(maxindexL2a1_tilde>=n2short+3) & (maxindexL2a1_tilde<=n2long-1);
    ...
    elseif vfoptions.lowmemory==2 % outer z (markov), inner e, vectorize semiz
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_ii_ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, [n_semiz,ones(1,length(n_z))], special_n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, z_valblock, e_val, ReturnFnParamsVec, 1);
                    entireRHS_ii_ze_alt=ReturnMatrix_ii_ze_alt+DiscountedEV_zb_alt;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_alt(ii)>0
                            loweredge_alt=min(maxindex1_alt(:,1,:,ii,:,:,:),N_a1-maxgap_alt(ii));
                            a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                            ReturnMatrix_ii_ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, [n_semiz,ones(1,length(n_z))], special_n_e, d23_gridvals_val, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, z_valblock, e_val, ReturnFnParamsVec, 3);
                            d2aprime_alt=(1:1:N_d2)' + N_d2*(a1primeindexes_alt-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_semiz-1),-5);
                    ...
                    midpoint_alt=max(min(midpoint_alt,N_a1-1),2);
                    a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, [n_semiz,ones(1,length(n_z))], special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, z_valblock, e_val, ReturnFnParamsVec, 3);
                    aprime=(1:1:N_d2)' + N_d2*(a1primeindexesfine_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_semiz-1),-5);
                    entireRHS_ii_ze_alt=reshape(ReturnMatrix_ii_ze_alt+DiscountedEVinterp_zb_alt(aprime),[N_d2*n2long*N_a2,N_a,N_semiz]);
                    [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_ze_alt,[],1);
                    V_ford3_alt(:,semizblock,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                    d_ind_alt        =rem(maxindexL2_alt-1,N_d2)+1;
                    maxindexL2a1_alt =rem(floor((maxindexL2_alt-1)/N_d2),n2long)+1;
                    maxindexL2a2_alt =floor((maxindexL2_alt-1)/(N_d2*n2long))+1;
                    allind_alt=d_ind_alt + N_d2*(maxindexL2a2_alt-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*semizBind;
                    ...
                    ReturnMatrix_ii_flat_alt=reshape(ReturnMatrix_ii_ze_alt,[N_d2*n2long*N_a2,N_a,N_semiz]);
                    linidx_lower_alt=d_ind_alt                + N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*semizBind;
                    linidx_upper_alt=d_ind_alt + N_d2*(n2long-1)+ N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*semizBind;
                    isInfLower_alt=(ReturnMatrix_ii_flat_alt(linidx_lower_alt)==-Inf);
                    isInfUpper_alt=(ReturnMatrix_ii_flat_alt(linidx_upper_alt)==-Inf);
                    inLowerStrict_alt=(maxindexL2a1_alt>=2)         & (maxindexL2a1_alt<=n2short+1);
                    inUpperStrict_alt=(maxindexL2a1_alt>=n2short+3) & (maxindexL2a1_alt<=n2long-1);
            ...
            % --- tilde pass ---
                ...
                for e_c=1:N_e
                    e_val=e_gridvals_J(e_c,:,N_j);
                    ReturnMatrix_ii_ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, [n_semiz,ones(1,length(n_z))], special_n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, z_valblock, e_val, ReturnFnParamsVec, 1);
                    entireRHS_ii_ze_tilde=ReturnMatrix_ii_ze_tilde+DiscountedEV_zb_tilde;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_tilde(ii)>0
                            loweredge_tilde=min(maxindex1_tilde(:,1,:,ii,:,:,:),N_a1-maxgap_tilde(ii));
                            a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                            ReturnMatrix_ii_ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, [n_semiz,ones(1,length(n_z))], special_n_e, d23_gridvals_val, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, z_valblock, e_val, ReturnFnParamsVec, 3);
                            d2aprime_tilde=(1:1:N_d2)' + N_d2*(a1primeindexes_tilde-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_semiz-1),-5);
                    ...
                    midpoint_tilde=max(min(midpoint_tilde,N_a1-1),2);
                    a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, [n_semiz,ones(1,length(n_z))], special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, z_valblock, e_val, ReturnFnParamsVec, 3);
                    aprime=(1:1:N_d2)' + N_d2*(a1primeindexesfine_tilde-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_semiz-1),-5);
                    entireRHS_ii_ze_tilde=reshape(ReturnMatrix_ii_ze_tilde+DiscountedEVinterp_zb_tilde(aprime),[N_d2*n2long*N_a2,N_a,N_semiz]);
                    [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_ze_tilde,[],1);
                    V_ford3_tilde(:,semizblock,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                    d_ind_tilde        =rem(maxindexL2_tilde-1,N_d2)+1;
                    maxindexL2a1_tilde =rem(floor((maxindexL2_tilde-1)/N_d2),n2long)+1;
                    maxindexL2a2_tilde =floor((maxindexL2_tilde-1)/(N_d2*n2long))+1;
                    allind_tilde=d_ind_tilde + N_d2*(maxindexL2a2_tilde-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*semizBind;
                    ...
                    ReturnMatrix_ii_flat_tilde=reshape(ReturnMatrix_ii_ze_tilde,[N_d2*n2long*N_a2,N_a,N_semiz]);
                    linidx_lower_tilde=d_ind_tilde                + N_d2*n2long*(maxindexL2a2_tilde-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*semizBind;
                    linidx_upper_tilde=d_ind_tilde + N_d2*(n2long-1)+ N_d2*n2long*(maxindexL2a2_tilde-1) + N_d2*n2long*N_a2*aind + N_d2*n2long*N_a2*N_a*semizBind;
                    isInfLower_tilde=(ReturnMatrix_ii_flat_tilde(linidx_lower_tilde)==-Inf);
                    isInfUpper_tilde=(ReturnMatrix_ii_flat_tilde(linidx_upper_tilde)==-Inf);
                    inLowerStrict_tilde=(maxindexL2a1_tilde>=2)         & (maxindexL2a1_tilde<=n2short+1);
                    inUpperStrict_tilde=(maxindexL2a1_tilde>=n2short+3) & (maxindexL2a1_tilde<=n2long-1);
        ...
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_ii_ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, z_val, e_val, ReturnFnParamsVec, 1);
                    entireRHS_ii_ze_alt=ReturnMatrix_ii_ze_alt+DiscountedEV_z_alt;
                    [~,maxindex1_alt]=max(entireRHS_ii_ze_alt,[],2);
                    midpoint_alt(:,1,:,level1ii,:,:)=maxindex1_alt;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_alt(ii)>0
                            loweredge_alt=min(maxindex1_alt(:,1,:,ii,:,:),N_a1-maxgap_alt(ii));
                            a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                            ReturnMatrix_ii_ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d23_gridvals_val, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, z_val, e_val, ReturnFnParamsVec, 3);
                            d2aprime_alt=(1:1:N_d2)' + N_d2*(a1primeindexes_alt-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    ...
                    midpoint_alt=max(min(midpoint_alt,N_a1-1),2);
                    a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_ze_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, z_val, e_val, ReturnFnParamsVec, 3);
                    aprime=(1:1:N_d2)' + N_d2*(a1primeindexesfine_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_ze_alt=reshape(ReturnMatrix_ii_ze_alt+DiscountedEVinterp_z_alt(aprime),[N_d2*n2long*N_a2,N_a]);
                    [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_ze_alt,[],1);
                    V_ford3_alt(:,z_c,e_c,d3_c)=shiftdim(Vtempii_alt,1);
                    d_ind_alt        =rem(maxindexL2_alt-1,N_d2)+1;
                    maxindexL2a1_alt =rem(floor((maxindexL2_alt-1)/N_d2),n2long)+1;
                    maxindexL2a2_alt =floor((maxindexL2_alt-1)/(N_d2*n2long))+1;
                    allind_alt=d_ind_alt + N_d2*(maxindexL2a2_alt-1) + N_d2*N_a2*aind;
                    ...
                    ReturnMatrix_ii_flat_alt=reshape(ReturnMatrix_ii_ze_alt,[N_d2*n2long*N_a2,N_a]);
                    linidx_lower_alt=d_ind_alt                + N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind;
                    linidx_upper_alt=d_ind_alt + N_d2*(n2long-1)+ N_d2*n2long*(maxindexL2a2_alt-1) + N_d2*n2long*N_a2*aind;
                    isInfLower_alt=(ReturnMatrix_ii_flat_alt(linidx_lower_alt)==-Inf);
                    isInfUpper_alt=(ReturnMatrix_ii_flat_alt(linidx_upper_alt)==-Inf);
                    inLowerStrict_alt=(maxindexL2a1_alt>=2)         & (maxindexL2a1_alt<=n2short+1);
                    inUpperStrict_alt=(maxindexL2a1_alt>=n2short+3) & (maxindexL2a1_alt<=n2long-1);
            ...
            % --- tilde pass ---
                ...
                for e_c=1:N_e
                    ...
                    ReturnMatrix_ii_ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, z_val, e_val, ReturnFnParamsVec, 1);
                    entireRHS_ii_ze_tilde=ReturnMatrix_ii_ze_tilde+DiscountedEV_z_tilde;
                    [~,maxindex1_tilde]=max(entireRHS_ii_ze_tilde,[],2);
                    midpoint_tilde(:,1,:,level1ii,:,:)=maxindex1_tilde;
                    ...
                    for ii=1:(vfoptions.level1n-1)
                        ...
                        if maxgap_tilde(ii)>0
                            loweredge_tilde=min(maxindex1_tilde(:,1,:,ii,:,:),N_a1-maxgap_tilde(ii));
                            a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                            ReturnMatrix_ii_ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d23_gridvals_val, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, z_val, e_val, ReturnFnParamsVec, 3);
                            d2aprime_tilde=(1:1:N_d2)' + N_d2*(a1primeindexes_tilde-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4);
                    ...
                    midpoint_tilde=max(min(midpoint_tilde,N_a1-1),2);
                    a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                    ReturnMatrix_ii_ze_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, z_val, e_val, ReturnFnParamsVec, 3);
                    aprime=(1:1:N_d2)' + N_d2*(a1primeindexesfine_tilde-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4);
                    entireRHS_ii_ze_tilde=reshape(ReturnMatrix_ii_ze_tilde+DiscountedEVinterp_z_tilde(aprime),[N_d2*n2long*N_a2,N_a]);
                    [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_ze_tilde,[],1);
                    V_ford3_tilde(:,z_c,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
                    d_ind_tilde        =rem(maxindexL2_tilde-1,N_d2)+1;
                    maxindexL2a1_tilde =rem(floor((maxindexL2_tilde-1)/N_d2),n2long)+1;
                    maxindexL2a2_tilde =floor((maxindexL2_tilde-1)/(N_d2*n2long))+1;
                    allind_tilde=d_ind_tilde + N_d2*(maxindexL2a2_tilde-1) + N_d2*N_a2*aind;
                    ...
                    ReturnMatrix_ii_flat_tilde=reshape(ReturnMatrix_ii_ze_tilde,[N_d2*n2long*N_a2,N_a]);
                    linidx_lower_tilde=d_ind_tilde                + N_d2*n2long*(maxindexL2a2_tilde-1) + N_d2*n2long*N_a2*aind;
                    linidx_upper_tilde=d_ind_tilde + N_d2*(n2long-1)+ N_d2*n2long*(maxindexL2a2_tilde-1) + N_d2*n2long*N_a2*aind;
                    isInfLower_tilde=(ReturnMatrix_ii_flat_tilde(linidx_lower_tilde)==-Inf);
                    isInfUpper_tilde=(ReturnMatrix_ii_flat_tilde(linidx_upper_tilde)==-Inf);
                    inLowerStrict_tilde=(maxindexL2a1_tilde>=2)         & (maxindexL2a1_tilde<=n2short+1);
                    inUpperStrict_tilde=(maxindexL2a1_tilde>=n2short+3) & (maxindexL2a1_tilde<=n2long-1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 1);
            entireRHS_ii_d3_alt=ReturnMatrix_ii_d3_alt+DiscountedEV_alt;
            [~,maxindex1_alt]=max(entireRHS_ii_d3_alt,[],2);
            midpoint_alt(:,1,:,level1ii,:,:,:,:)=maxindex1_alt;
            maxgap_alt=squeeze(max(max(max(max(max(max( maxindex1_alt(:,1,:,2:end,:,:,:,:)-maxindex1_alt(:,1,:,1:end-1,:,:,:,:), [],8),[],7),[],6),[],5),[],3),[],1));
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_alt(ii)>0
                    loweredge_alt=min(maxindex1_alt(:,1,:,ii,:,:,:,:),N_a1-maxgap_alt(ii));
                    a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                    ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                    d2aprimez_alt=(1:1:N_d2)' + N_d2*(a1primeindexes_alt-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                    entireRHS_ii_d3_alt=ReturnMatrix_ii_d3_alt+DiscountedEV_alt(d2aprimez_alt);
                    [~,maxindex_inner_alt]=max(entireRHS_ii_d3_alt,[],2);
                    midpoint_alt(:,1,:,curra1inner,:,:,:,:)=maxindex_inner_alt+(loweredge_alt-1);
            ...
            midpoint_alt=max(min(midpoint_alt,N_a1-1),2);
            a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_d3_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
            aprimez=(1:1:N_d2)' + N_d2*(a1primeindexesfine_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            entireRHS_ii_d3_alt=reshape(ReturnMatrix_ii_d3_alt+DiscountedEVinterp_alt(aprimez),[N_d2*n2long*N_a2,N_a,N_bothz,N_e]);
            [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_d3_alt,[],1);
            V_ford3_alt(:,:,:,d3_c)=shiftdim(Vtempii_alt,1);
            ...
            % --- tilde pass ---
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 1);
            entireRHS_ii_d3_tilde=ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde;
            [~,maxindex1_tilde]=max(entireRHS_ii_d3_tilde,[],2);
            midpoint_tilde(:,1,:,level1ii,:,:,:,:)=maxindex1_tilde;
            maxgap_tilde=squeeze(max(max(max(max(max(max( maxindex1_tilde(:,1,:,2:end,:,:,:,:)-maxindex1_tilde(:,1,:,1:end-1,:,:,:,:), [],8),[],7),[],6),[],5),[],3),[],1));
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_tilde(ii)>0
                    loweredge_tilde=min(maxindex1_tilde(:,1,:,ii,:,:,:,:),N_a1-maxgap_tilde(ii));
                    a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                    ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
                    d2aprimez_tilde=(1:1:N_d2)' + N_d2*(a1primeindexes_tilde-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                    entireRHS_ii_d3_tilde=ReturnMatrix_ii_d3_tilde+DiscountedEV_tilde(d2aprimez_tilde);
                    [~,maxindex_inner_tilde]=max(entireRHS_ii_d3_tilde,[],2);
                    midpoint_tilde(:,1,:,curra1inner,:,:,:,:)=maxindex_inner_tilde+(loweredge_tilde-1);
            ...
            midpoint_tilde=max(min(midpoint_tilde,N_a1-1),2);
            a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_ii_d3_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec, 3);
            aprimez=(1:1:N_d2)' + N_d2*(a1primeindexesfine_tilde-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
            entireRHS_ii_d3_tilde=reshape(ReturnMatrix_ii_d3_tilde+DiscountedEVinterp_tilde(aprimez),[N_d2*n2long*N_a2,N_a,N_bothz,N_e]);
            [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_d3_tilde,[],1);
            V_ford3_tilde(:,:,:,d3_c)=shiftdim(Vtempii_tilde,1);
    ...
    elseif vfoptions.lowmemory==1
        for d3_c=1:N_d3
            ...
            % --- alt pass ---
                ReturnMatrix_ii_e_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 1);
                entireRHS_ii_e_alt=ReturnMatrix_ii_e_alt+DiscountedEV_alt;
                [~,maxindex1_alt]=max(entireRHS_ii_e_alt,[],2);
                midpoint_alt(:,1,:,level1ii,:,:,:,:)=maxindex1_alt;
                maxgap_alt=squeeze(max(max(max(max(max(max( maxindex1_alt(:,1,:,2:end,:,:,:,:)-maxindex1_alt(:,1,:,1:end-1,:,:,:,:), [],8),[],7),[],6),[],5),[],3),[],1));
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_alt(ii)>0
                        loweredge_alt=min(maxindex1_alt(:,1,:,ii,:,:,:,:),N_a1-maxgap_alt(ii));
                        a1primeindexes_alt=loweredge_alt+(0:1:maxgap_alt(ii));
                        ReturnMatrix_ii_e_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1_grid(a1primeindexes_alt), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                        d2aprime_alt=(1:1:N_d2)' + N_d2*(a1primeindexes_alt-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                        entireRHS_ii_e_alt=ReturnMatrix_ii_e_alt+DiscountedEV_alt(d2aprime_alt);
                        [~,maxindex_inner_alt]=max(entireRHS_ii_e_alt,[],2);
                        midpoint_alt(:,1,:,curra1inner,:,:,:,:)=maxindex_inner_alt+(loweredge_alt-1);
                ...
                midpoint_alt=max(min(midpoint_alt,N_a1-1),2);
                a1primeindexesfine_alt=(midpoint_alt+(midpoint_alt-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_e_alt=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_alt), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                aprime=(1:1:N_d2)' + N_d2*(a1primeindexesfine_alt-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                entireRHS_ii_e_alt=reshape(ReturnMatrix_ii_e_alt+DiscountedEVinterp_alt(aprime),[N_d2*n2long*N_a2,N_a,N_bothz,1]);
                [Vtempii_alt,maxindexL2_alt]=max(entireRHS_ii_e_alt,[],1);
                V_ford3_alt(:,:,e_c,d3_c)=shiftdim(Vtempii_alt,1);
            ...
            % --- tilde pass ---
                ReturnMatrix_ii_e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 1);
                entireRHS_ii_e_tilde=ReturnMatrix_ii_e_tilde+DiscountedEV_tilde;
                [~,maxindex1_tilde]=max(entireRHS_ii_e_tilde,[],2);
                midpoint_tilde(:,1,:,level1ii,:,:,:,:)=maxindex1_tilde;
                maxgap_tilde=squeeze(max(max(max(max(max(max( maxindex1_tilde(:,1,:,2:end,:,:,:,:)-maxindex1_tilde(:,1,:,1:end-1,:,:,:,:), [],8),[],7),[],6),[],5),[],3),[],1));
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_tilde(ii)>0
                        loweredge_tilde=min(maxindex1_tilde(:,1,:,ii,:,:,:,:),N_a1-maxgap_tilde(ii));
                        a1primeindexes_tilde=loweredge_tilde+(0:1:maxgap_tilde(ii));
                        ReturnMatrix_ii_e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1_grid(a1primeindexes_tilde), a2_gridvals, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                        d2aprime_tilde=(1:1:N_d2)' + N_d2*(a1primeindexes_tilde-1) + N_d2*N_a1*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                        entireRHS_ii_e_tilde=ReturnMatrix_ii_e_tilde+DiscountedEV_tilde(d2aprime_tilde);
                        [~,maxindex_inner_tilde]=max(entireRHS_ii_e_tilde,[],2);
                        midpoint_tilde(:,1,:,curra1inner,:,:,:,:)=maxindex_inner_tilde+(loweredge_tilde-1);
                ...
                midpoint_tilde=max(min(midpoint_tilde,N_a1-1),2);
                a1primeindexesfine_tilde=(midpoint_tilde+(midpoint_tilde-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_ii_e_tilde=CreateReturnFnMatrix_ExpAsset_Disc_DC2A_e(ReturnFn, 0, [n_d2,1], n_a2, n_a3, n_bothz, special_n_e, d23_gridvals_val, a1prime_grid(a1primeindexesfine_tilde), a2_gridvals, a1_grid, a2_gridvals, a3_gridvals, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec, 3);
                aprime=(1:1:N_d2)' + N_d2*(a1primeindexesfine_tilde-1) + N_d2*N_a1fine*shiftdim((0:1:N_a2-1),-1) + N_d2*N_a1fine*N_a2*shiftdim((0:1:N_a3-1),-4) + N_d2*N_a1fine*N_a2*N_a3*shiftdim((0:1:N_bothz-1),-5);
                entireRHS_ii_e_tilde=reshape(ReturnMatrix_ii_e_tilde+DiscountedEVinterp_tilde(aprime),[N_d2*n2long*N_a2,N_a,N_bothz,1]);
                [Vtempii_tilde,maxindexL2_tilde]=max(entireRHS_ii_e_tilde,[],1);
                V_ford3_tilde(:,:,e_c,d3_c)=shiftdim(Vtempii_tilde,1);
