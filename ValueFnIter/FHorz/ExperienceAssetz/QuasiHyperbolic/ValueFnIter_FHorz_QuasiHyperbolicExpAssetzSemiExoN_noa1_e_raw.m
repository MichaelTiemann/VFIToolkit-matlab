...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % loop e
        for e_c=1:N_e
            ...
            Valt(:,:,e_c,N_j)=Vtemp;
            d12_ind=rem(maxindex-1,N_d12)+1;
            Policyalt(1,:,:,e_c,N_j)=rem(d12_ind-1,N_d1)+1;
            Policyalt(2,:,:,e_c,N_j)=ceil(d12_ind/N_d1);
            Policyalt(3,:,:,e_c,N_j)=ceil(maxindex/N_d12);
        end
    elseif vfoptions.lowmemory==2 % split: vectorize semiz, outer loop z, inner loop e
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                d12_ind=rem(maxindex-1,N_d12)+1;
                Policyalt(1,:,semizblock,e_c,N_j)=rem(d12_ind-1,N_d1)+1;
                Policyalt(2,:,semizblock,e_c,N_j)=ceil(d12_ind/N_d1);
                Policyalt(3,:,semizblock,e_c,N_j)=ceil(maxindex/N_d12);
            end
        end
    elseif vfoptions.lowmemory==3 % joint loop over bothz (outer), inner loop e
        for z_c=1:N_bothz
            ...
            for e_c=1:N_e
                ...
                d12_ind=rem(maxindex-1,N_d12)+1;
                Policyalt(1,:,z_c,e_c,N_j)=rem(d12_ind-1,N_d1)+1;
                Policyalt(2,:,z_c,e_c,N_j)=ceil(d12_ind/N_d1);
                Policyalt(3,:,z_c,e_c,N_j)=ceil(maxindex/N_d12);
            end
        end
    end
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_d3=ReturnMatrix_d3+beta*repelem(entireEV,N_d1,1,1); % autofills e dimension
            ...
            entireRHS_d3=ReturnMatrix_d3+beta0beta*repelem(entireEV,N_d1,1,1); % autofills e dimension
            [Vtemp,maxindex]=max(entireRHS_d3,[],1);
    ...
    elseif vfoptions.lowmemory==1 % loop e
        for d3_c=1:N_d3
            ...
            for e_c=1:N_e
                ...
                % alt (exponential): F + beta*EV
                entireRHS_d3e=ReturnMatrix_d3e+beta*repelem(entireEV,N_d1,1,1);
                [Vtemp,maxindex]=max(entireRHS_d3e,[],1);
                V_ford3_alt(:,:,e_c,d3_c)=shiftdim(Vtemp,1);
                Policy_ford3_alt(:,:,e_c,d3_c)=shiftdim(maxindex,1);

                % tilde (QH-perceived): F + beta0*beta*EV
                entireRHS_d3e=ReturnMatrix_d3e+beta0beta*repelem(entireEV,N_d1,1,1);
                [Vtemp,maxindex]=max(entireRHS_d3e,[],1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                for e_c=1:N_e
                    ...
                    % alt (exponential): F + beta*EV
                    entireRHS_d3ze=ReturnMatrix_d3ze+beta*repelem(entireEV_z,N_d1,1,1);
                    ...
                    % tilde (QH-perceived): F + beta0*beta*EV
                    entireRHS_d3ze=ReturnMatrix_d3ze+beta0beta*repelem(entireEV_z,N_d1,1,1);
                    [Vtemp,maxindex]=max(entireRHS_d3ze,[],1);
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                for e_c=1:N_e
                    ...
                    % alt (exponential): F + beta*EV
                    entireRHS_d3ze=ReturnMatrix_d3ze+beta*repelem(entireEV_z,N_d1,1);
                    [Vtemp,maxindex]=max(entireRHS_d3ze,[],1);
                    V_ford3_alt(:,z_c,e_c,d3_c)=Vtemp;
                    Policy_ford3_alt(:,z_c,e_c,d3_c)=maxindex;

                    % tilde (QH-perceived): F + beta0*beta*EV
                    entireRHS_d3ze=ReturnMatrix_d3ze+beta0beta*repelem(entireEV_z,N_d1,1);
                    [Vtemp,maxindex]=max(entireRHS_d3ze,[],1);
                    V_ford3_tilde(:,z_c,e_c,d3_c)=Vtemp;
                    Policy_ford3_tilde(:,z_c,e_c,d3_c)=maxindex;
