...
Policy4_ford3_jj=zeros(4,N_a,N_bothz,N_d3,'gpuArray'); % (d2, a1prime-midpoint, a2prime, a1primeL2ind)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    Policy(1,:,:,N_j)=reshape(Policy4_ford3_jj(1+temp),[1,N_a,N_bothz]); % d2
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                ReturnMatrix_d3z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, [n_d2,1], n_a2, n_a3, special_n_semiz, d23_gridvals_val, a1_grid, a2_gridvals, a1_grid, a2_gridvals, a3_grid, z_valblock, ReturnFnParamsVec, 1);
                entireRHS_d3z=ReturnMatrix_d3z+DiscountedEV;
                ...
                entireRHS_ii_d3z=reshape(ReturnMatrix_ii_d3z+DiscountedEVinterp(aprime_z),[N_d2*n2long*N_a2,N_a,N_semiz]);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                entireRHS_ii_d3z=reshape(ReturnMatrix_ii_d3z+DiscountedEVinterp_z(aprime_z),[N_d2*n2long*N_a2,N_a]);
                [Vtempii,maxindexL2]=max(entireRHS_ii_d3z,[],1);
                V_ford3_jj(:,z_c,d3_c)=shiftdim(Vtempii,1);
...
for reverse_j=1:N_j-1
    ...
    maxindex=reshape(maxindex,[N_a*N_bothz,1]);
    temp=4*((1:1:N_a*N_bothz)'+(N_a*N_bothz)*(maxindex-1)-1);
    Policy(1,:,:,jj)=reshape(Policy4_ford3_jj(1+temp),[1,N_a,N_bothz]); % d2
