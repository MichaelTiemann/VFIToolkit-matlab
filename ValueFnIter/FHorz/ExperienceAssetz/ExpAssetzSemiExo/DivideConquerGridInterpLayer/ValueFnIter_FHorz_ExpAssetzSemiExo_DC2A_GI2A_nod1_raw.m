...
Policy4_ford3_jj=zeros(4,N_a,N_bothz,N_d3,'gpuArray'); % (d2, midpoint, a2prime, L2ind)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                V_ford3_jj(:,semizblock,d3_c)=shiftdim(Vtempii,1);
                d_ind        =rem(maxindexL2-1,N_d2)+1;
                maxindexL2a1 =rem(floor((maxindexL2-1)/N_d2),n2long)+1;
                maxindexL2a2 =floor((maxindexL2-1)/(N_d2*n2long))+1;
                allind=d_ind + N_d2*(maxindexL2a2-1) + N_d2*N_a2*aind + N_d2*N_a2*N_a*semizBind;
                Policy4_ford3_jj(1,:,semizblock,d3_c)=d_ind;
                Policy4_ford3_jj(2,:,semizblock,d3_c)=midpoint(allind);
                Policy4_ford3_jj(3,:,semizblock,d3_c)=maxindexL2a2;
                Policy4_ford3_jj(4,:,semizblock,d3_c)=maxindexL2a1;
                ...
                inLowerStrict=(maxindexL2a1>=2)         & (maxindexL2a1<=n2short+1);
                inUpperStrict=(maxindexL2a1>=n2short+3) & (maxindexL2a1<=n2long-1);
                flag_ford3_jj(1,:,semizblock,d3_c)=2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
            end
        end
    ...
    elseif vfoptions.lowmemory==2 % joint: loop over bothz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                Policy4_ford3_jj(1,:,z_c,d3_c)=d_ind;
                Policy4_ford3_jj(2,:,z_c,d3_c)=midpoint(allind);
                Policy4_ford3_jj(3,:,z_c,d3_c)=maxindexL2a2;
                Policy4_ford3_jj(4,:,z_c,d3_c)=maxindexL2a1;
                ...
                inLowerStrict=(maxindexL2a1>=2)         & (maxindexL2a1<=n2short+1);
                inUpperStrict=(maxindexL2a1>=n2short+3) & (maxindexL2a1<=n2long-1);
                flag_ford3_jj(1,:,z_c,d3_c)=2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
            end
        end
    end
    ...
    Policy(3,:,:,N_j)=reshape(Policy4_ford3_jj(2+temp),[1,N_a,N_bothz]);
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                ReturnMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc_DC2A(ReturnFn, 0, [n_d2,1], n_a2, n_a3, [n_semiz,ones(1,length(n_z))], d23_gridvals_val, a1_grid, a2_gridvals, a1_grid(level1ii), a2_gridvals, a3_gridvals, z_valblock, ReturnFnParamsVec, 1);
                entireRHS_ii_z=ReturnMatrix_ii_z+DiscountedEV_zb;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii_z=ReturnMatrix_ii_z+DiscountedEV_zb(d2aprime);
                ...
                entireRHS_ii_z=reshape(ReturnMatrix_ii_z+DiscountedEVinterp_zb(aprime),[N_d2*n2long*N_a2,N_a,N_semiz]);
...
for reverse_j=1:N_j-1
    ...
    Policy(3,:,:,jj)=reshape(Policy4_ford3_jj(2+temp),[1,N_a,N_bothz]);
