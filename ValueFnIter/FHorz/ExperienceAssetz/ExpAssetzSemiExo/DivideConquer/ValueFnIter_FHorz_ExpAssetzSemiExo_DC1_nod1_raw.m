function [V,Policy3]=ValueFnIter_FHorz_ExpAssetzSemiExo_DC1_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_semiz,N_j, d2_gridvals, d3_grid, a1_gridvals, a2_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        ...
        for z_c=1:N_z
            ...
            curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
            V(curraindex,semizblock,N_j)=shiftdim(Vtempii,1);
            dind=rem(maxindex2-1,N_d23)+1;
            Policy3(1,curraindex,semizblock,N_j)=rem(dind-1,N_d2)+1; % d2
            Policy3(2,curraindex,semizblock,N_j)=ceil(dind/N_d2); % d3
            Policy3(3,curraindex,semizblock,N_j)=ceil(maxindex2/N_d23); % a1prime
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            entireRHS_ii_d3=ReturnMatrix_ii_d3+repelem(DiscountedEV,1,1,1,1,1);
    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                DiscountedEV=DiscountFactorParamsVec*reshape(entireEV_z,[N_d2,N_a1,1,N_a2,N_semiz]);
                ...
                curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
                V_ford3_jj(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                Policy_ford3_jj(curraindex,semizblock,d3_c)=shiftdim(maxindex2,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        Policy_ford3_jj(curraindex,semizblock,d3_c)=shiftdim(maxindex+N_d2*(loweredge(allind)-1),1);
                    else
                        loweredge=maxindex1(:,1,ii,:,:);
    ...
    elseif vfoptions.lowmemory==2 % joint loop over bothz
        for d3_c=1:N_d3
            ...
            for z_c=1:N_bothz
                ...
                DiscountedEV_z=DiscountFactorParamsVec*reshape(entireEV_z,[N_d2,N_a1,1,N_a2]);
                ...
                entireRHS_ii_z=ReturnMatrix_ii_z+repelem(DiscountedEV_z,1,1,1,1);
