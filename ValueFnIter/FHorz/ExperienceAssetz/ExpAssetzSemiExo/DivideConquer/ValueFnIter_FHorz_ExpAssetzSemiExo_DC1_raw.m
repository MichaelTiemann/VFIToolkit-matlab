    ...
    elseif vfoptions.lowmemory==1 % split: loop z (markov), vectorize semiz
        ...
        for d3_c=1:N_d3
            ...
            for z_c=1:N_z
                ...
                DiscountedEV_z=DiscountFactorParamsVec*reshape(entireEV_z,[N_d2,N_a1,1,N_a2,N_semiz]);
                ...
                entireRHS_ii_d3z=ReturnMatrix_ii_d3z+repelem(DiscountedEV_z,N_d1,1,1,1,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z+DiscountedEV_z(d2aprimez),[N_d12*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_jj(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
                        ...
                        entireRHS_ii=reshape(ReturnMatrix_ii_d3z+DiscountedEV_z(d2aprimez),[N_d12,level1iidiff(ii)*N_a2,N_semiz]);
                        [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                        V_ford3_jj(curraindex,semizblock,d3_c)=shiftdim(Vtempii,1);
                        dind=(rem(maxindex-1,N_d1*N_d2)+1);
                        a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii));
