function [V, Policy]=ValueFnIter_InfHorz_ExpAsset_Refine_GI1_raw(V0,n_d1,n_d2,n_a1,n_a2,n_z, d1_gridvals, d2_grid, a1_gridvals, a2_grid, z_gridvals, pi_z, ReturnFn, aprimeFn, Parameters, DiscountFactorParamsVec, ReturnFnParamsVec, aprimeFnParamNames, vfoptions)
...
a1prime_grid=interp1(1:1:n_a(1),a1_grid,linspace(1,n_a(1),n_a(1)+(n_a(1)-1)*n2short));
n_a1prime=length(aprime_grid);
N_a1prime=prod(n_a1prime);

a1prime_gridvals=CreateGridVals(n_a1prime,a1prime_grid,1);
...
elseif vfoptions.lowmemory==1 % loop over z
    ...
    ReturnMatrixfine=zeros(N_d2*N_a1prime,N_a,N_z,'gpuArray'); % 'refined' return matrix
    dstar=zeros(N_d2*N_a1prime,N_a,N_z,'gpuArray');
    l_z=length(n_z);
...
temp=(1:1:N_d2)'+N_d1*((1:vfoptions.ngridinterp+1:n_aprime)-1);
ReturnMatrix=ReturnMatrixfine(temp(:),:,:);
...
aprimeIndex=repelem((1:1:N_a1)',N_d2,N_a2)+N_a1*repmat((a2primeIndex-1),N_a1prime,1); % [N_d2*N_a1,N_a2]
aprimeplus1Index=repelem((1:1:N_a1)',N_d2,N_a2)+N_a1*repmat(a2primeIndex,N_a1prime,1); % [N_d2*N_a1,N_a2]
aprimeProbs=repmat(a2primeProbs,N_a1,1,N_z);  % [N_d2*N_a1,N_a2,N_z]

%% First, just use the rough grid without interpolation to get a decent initial guess
currdist=Inf;
tempcounter=1;
while currdist>(vfoptions.multigridswitch*vfoptions.tolerance) && tempcounter<=vfoptions.maxiter
    ...
    entireRHS=ReturnMatrix+DiscountFactorParamsVec*repelem(EV,N_d1,N_a1,1);
    ...
    if isfinite(currdist) && currdist/vfoptions.tolerance>10 && vfoptions.maxhowards>0 % Use Howards Policy Fn Iteration Improvement
        ...
        Ftemp=reshape(ReturnMatrixfine(Policy+N_d2*N_a1*(0:1:N_a-1)'+N_d2*N_a1*N_a*(0:1:N_z-1)),[N_a*N_z,1]);
...
%% Switch to the fine grid with interpolation
currdist=1;
while currdist>vfoptions.tolerance && tempcounter<=vfoptions.maxiter
    ...
    % Have to reshape, permute to get a1prime as first dimension (so can interp1, which can only be done on first dimension), then permute and reshape back
    EVinterp=reshape(permute(interp1(a1_grid,permute(reshape(EV,[N_d2,N_a1prime,N_a2*1*N_z]),[2,1,3]),a1prime_grid),[2,1,3]),[N_d2*N_a1prime,N_a2,1,N_z]);

    entireRHS=ReturnMatrixfine+DiscountFactorParamsVec*repelem(EVinterp,1,N_a1,1,1);
    ...
    if isfinite(currdist) && currdist/vfoptions.tolerance>10 && vfoptions.maxhowards>0 % Use Howards Policy Fn Iteration Improvement
        ...
        Ftemp=reshape(ReturnMatrixfine(Policy+N_d2*N_a1prime*(0:1:N_a-1)'+N_d2*N_a1prime*N_a*(0:1:N_z-1)),[N_a*N_z,1]);
        Policy_d2ind=rem(Policy(:)-1,N_d2)+1;
        Policy_a1primeind=ceil(Policy(:)/N_d2); % size(Policy_a1primeind) is [N_a*N_z,1]
        ...
        a2primeind=a2primeIndex(temp); % this is the lower grid point index for a2prime in terms of (a1,a2,z)
        % combine a1prime, a2prime, and z
        aprimeind=Policy_a1primeind+N_a1prime*(a2primeind-1); %+N_a1*N_a2*repelem((0:1:N_z-1)',N_a1*N_a2,1);
        aprimeind=reshape(aprimeind,[N_a*N_z,1]);
        aprimeplus1ind=aprimeind+N_a1prime; % add one to a2prime index, which means adding N_a1prime*1
        aprimeProbs_Howards=reshape(a2primeProbs(temp),[N_a*N_z,1]); %  a2primeProbs is [N_d2,N_a2]
...
% Policy is currently
temppolicyindex=reshape(Policy,[1,N_a*N_z])+N_a1prime*gpuArray(0:1:N_a1prime*N_z-1);
Policy=reshape(dstar(temppolicyindex),[N_a,N_z]) + N_d1*(Policy-1); % Add in d1, but keep in a fully vectorized form

%% Split up the fine index into lower grid point and L2 index
