...
% QH policy (Policy) - separate fine-grid index into L1 and L2
...
Policyalt=zeros(3,N_a,N_z,'gpuArray'); % +1 channel for PolicyL2flag
Policyalt(1,:,:)=reshape(L1,[1,N_a,N_z]);
Policyalt(2,:,:)=reshape(L2,[1,N_a,N_z]);
...
isInfUpper = (ReturnMatrixfine(linidx_upper(:)) == -Inf);
inInterior = (L2 >= 2) & (L2 <= n2short+1);
Policyalt(3,:,:) = reshape(2 + (inInterior & isInfLower) - (inInterior & isInfUpper), [1,N_a,N_z]);
