function [VKron,Policy]=ValueFnIter_InfHorz_Refine_postGI_raw(VKron,n_d,n_a,n_z,d_gridvals,a_grid,z_gridvals,pi_z,ReturnFn,DiscountFactorParamsVec,ReturnFnParams,vfoptions)
...
% The rest, except putting d back into Policy at the end, is all just copy-paste from ValueFnIter_InfHorz_postGI_nod_raw()
...
while currdist>(vfoptions.multigridswitch*vfoptions.tolerance) && tempcounter<=vfoptions.maxiter
    ...
    if isfinite(currdist) && currdist/vfoptions.tolerance>10 && tempcounter<vfoptions.maxhowards
        ...
        for Howards_counter=1:vfoptions.howards
            ...
            EVKrontemp=reshape(sum(EVKrontemp,2),[N_a,N_z]); % interpolate EV
