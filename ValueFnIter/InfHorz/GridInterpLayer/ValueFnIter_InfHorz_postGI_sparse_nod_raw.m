...
%
% vfoptions.lowmemory has no effect here, the same as for every other nod postGI raw. In the Refine
% (with d) raws, lowmemory=1 loops over z so as to never hold the [N_d,...] raw return matrix that
% gets max'd over d. Without a d there is no such intermediate to avoid: the return matrices are
% already their final size, so building them one z at a time would not lower the peak.
