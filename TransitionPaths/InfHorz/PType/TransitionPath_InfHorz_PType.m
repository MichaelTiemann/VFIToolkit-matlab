...
for ii=1:PTypeStructure.N_i
    ...
    if ~isfield(PTypeStructure.(iistr).vfoptions,'divideandconquer')
        ...
        if PTypeStructure.(iistr).vfoptions.divideandconquer==1
            if isscalar(n_a)
                PTypeStructure.(iistr).vfoptions.level1n=floor(sqrt(n_a(1)));
            elseif length(n_a)==2
                PTypeStructure.(iistr).vfoptions.level1n=[floor(sqrt(n_a(1))),n_a(2)]; % default DC2A: level1n(2)==n_a(2) triggers DC2A branch
            end
        end
    end
