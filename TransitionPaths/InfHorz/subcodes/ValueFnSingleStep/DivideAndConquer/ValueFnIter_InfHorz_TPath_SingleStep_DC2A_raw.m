function [V,Policy2]=ValueFnIter_InfHorz_TPath_SingleStep_DC2A_raw(Vnext,n_d,n_a,n_z, d_gridvals, a_grid, z_gridvals, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% DC2A: two endogenous states, divide-and-conquer on the first endo state, but not on the second endo state
...
elseif vfoptions.lowmemory>=2
    error('vfoptions.lowmemory>=2 not supported for ValueFnIter_InfHorz_TPath_SingleStep_DC2A_raw')
end
...
if vfoptions.lowmemory==0
    ...
    % n-Monotonicity
    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_z, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_gridvals, ReturnFnParamsVec,1,0);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals, ReturnFnParamsVec,2,1);
            ...
            entireRHS_ii=reshape(entireRHS_ii,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, n_z, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals, ReturnFnParamsVec,2,1);
    ...
    for z_c=1:N_z
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, special_n_z, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsVec,2,1);
                aprimez=a1primeindexes+N_a1*shiftdim((0:1:N_a2-1),-1); % the current aprimeii(ii):aprimeii(ii+1)
                % ReturnMatrix_ii (Refine=1) is [N_d*(maxgap+1)*N_a2, level1iidiff, N_a2]; EV-indexed reshape is [..., 1, N_a2]. Broadcast over level1iidiff at dim 2.
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV_z(reshape(aprimez,[N_d*(maxgap(ii)+1)*N_a2,1,N_a2]));
                entireRHS_ii=reshape(entireRHS_ii,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,z_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                ...
                %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii)); % already includes -1
                allind=dind+N_d*a2primeind+N_d*N_a2*a2ind; % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2
                Policy(curraindex,z_c)=shiftdim(maxindexfix+N_d*(loweredge(allind)-1),1);
            else
                loweredge=maxindex1(:,1,:,ii,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A(ReturnFn, n_d, special_n_z, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsVec,2,1);
                aprimez=loweredge+N_a1*shiftdim((0:1:N_a2-1),-1); % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV_z(reshape(aprimez,[N_d*1*N_a2,1,N_a2]));
                entireRHS_ii=reshape(entireRHS_ii,[N_d*1*N_a2,level1iidiff(ii)*N_a2]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,z_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
