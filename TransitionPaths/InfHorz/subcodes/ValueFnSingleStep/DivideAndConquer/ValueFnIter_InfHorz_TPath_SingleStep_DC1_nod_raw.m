function [V, Policy]=ValueFnIter_InfHorz_TPath_SingleStep_DC1_nod_raw(Vnext,n_a,n_z, a_grid, z_gridvals,pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
elseif vfoptions.lowmemory>=2
    error('vfoptions.lowmemory>=2 not supported for ValueFnIter_InfHorz_TPath_SingleStep_DC1_nod_raw')
end
...
elseif vfoptions.lowmemory==1
    for z_c=1:N_z
        ...
        V(level1ii,z_c)=shiftdim(Vtempii,1);
        Policy(level1ii,z_c)=shiftdim(maxindex1,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV_z(aprimeindexes); % autofill level1iidiff(ii) in 2nd dimension
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,z_c)=shiftdim(Vtempii,1);
                Policy(curraindex,z_c)=shiftdim(maxindex+loweredge-1,1);
            else
                loweredge=maxindex1(1,ii);
                % Just use aprime(ii) for everything
                % No need for reshape(a_grid(loweredge),...) as is just a single point
                ...
                entireRHS_ii=ReturnMatrix_ii+DiscountedEV_z(loweredge);
                % can skip max() over as just a single point
                V(curraindex,z_c)=shiftdim(entireRHS_ii,1);
                Policy(curraindex,z_c)=loweredge;
            end
        end
    end
end
