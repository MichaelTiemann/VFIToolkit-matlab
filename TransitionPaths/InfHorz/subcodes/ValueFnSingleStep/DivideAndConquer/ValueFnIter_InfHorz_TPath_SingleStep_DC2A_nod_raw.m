function [V, Policy]=ValueFnIter_InfHorz_TPath_SingleStep_DC2A_nod_raw(Vnext,n_a,n_z, a_grid, z_gridvals,pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
elseif vfoptions.lowmemory>=2
    error('vfoptions.lowmemory>=2 not supported for ValueFnIter_InfHorz_TPath_SingleStep_DC2A_nod_raw')
end
...
if vfoptions.lowmemory==0
    ...
    % n-Monotonicity
    ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_z, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, z_gridvals, ReturnFnParamsVec,1);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_z, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals, ReturnFnParamsVec,2);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, n_z, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals, ReturnFnParamsVec,2);
...
elseif vfoptions.lowmemory==1
    for z_c=1:N_z
        ...
        entireRHS_ii=ReturnMatrix_ii+DiscountFactorParamsVec*reshape(EV_z,[N_a1,N_a2]); % autoexpand (a)

        % Calc the max and it's index
        [~,maxindex1]=max(entireRHS_ii,[],1);

        % Now, get and store the full aprime
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_z, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsVec,2);
                aprimez=repelem(aprimeindexes,1,1,level1iidiff(ii),1)+N_a1*(0:1:N_a2-1); % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii+DiscountFactorParamsVec*EV_z(reshape(aprimez,[(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,z_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
                ...
                loweredge=maxindex1(1,:,ii,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii=CreateReturnFnMatrix_Disc_DC2A_nod(ReturnFn, special_n_z, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsVec,2);
                aprimez=repelem(loweredge,1,1,level1iidiff(ii),1)+N_a1*(0:1:N_a2-1); % the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii+DiscountFactorParamsVec*EV_z(reshape(aprimez,[1*N_a2,level1iidiff(ii)*N_a2]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,z_c)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
