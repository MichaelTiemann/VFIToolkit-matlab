function [vfoptions,simoptions]=SetupNonStandardEndoStates_FHorz_TPath(n_d,n_a,d_grid,a_grid,vfoptions,simoptions,n_z,z_grid,N_j,Parameters)
% Note: the last four inputs (n_z,z_grid,N_j,Parameters) are only needed for experienceassetz (the aprimeFn depends on z)
...
% Note: reuses the setup_experienceasset struct name, so that all the Step1/Step2 plumbing is shared
if isfield(vfoptions,'experienceassetz')
    if vfoptions.experienceassetz>=1
        if nargin<10
            error('experienceassetz transition path requires SetupNonStandardEndoStates_FHorz_TPath to be called with n_z,z_grid,N_j,Parameters (this caller does not yet support experienceassetz)')
        end
        % To be able to pass them to internal commands, we store all the setup in vfoptions and simoptions.
        if ~isfield(vfoptions,'l_dexperienceasset')
            vfoptions.l_dexperienceasset=1; % by default, only one decision variable influences the experienceasset
        ...
        % Build z_gridvals_J [N_z,l_z,N_j], needed to evaluate the aprimeFn (e.g., by Step2 of the transition path)
        ...
        if N_a1>0 % set up for divide-and-conquer
            if vfoptions.divideandconquer==1
                ...
                % ExpAssetz only divide-and-conquers a1, so drop any trailing level1n entries
                if length(vfoptions.level1n)>length(n_a1)
                    vfoptions.level1n=vfoptions.level1n(1:length(n_a1));
                end
                vfoptions.level1n=min(vfoptions.level1n,n_a1); % Otherwise causes errors
        ...
        simoptions.setup_experienceasset.a2_grid=a2_grid;
        simoptions.setup_experienceasset.d_grid=d_grid; % Note: bit silly, as is already in simoptions.d_grid
        simoptions.setup_experienceasset.z_gridvals_J=z_gridvals_J_expassetz; % [N_z,l_z,N_j]: experienceassetz needs z to evaluate the aprimeFn
    end
end