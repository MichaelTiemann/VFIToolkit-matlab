function [V,Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_ExpAssetz_GI1_e_raw(V,n_d1,n_d2,n_a1,n_a2,n_z,n_e,N_j, d_gridvals,d2_gridvals,a1_gridvals,a2_grid, z_gridvals_J,e_gridvals_J, pi_z_J,pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ReturnMatrix=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n_a1, n_a1,n_a2, n_z,n_e,N_j, d_gridvals, a1_gridvals, a1_gridvals, a2_grid, z_gridvals_J,e_gridvals_J, ReturnFnParamsAgeMatrix,1,1); % Level=1, Refine=1
    % fastOLG: ReturnMatrix is [N_d1,N_d2*N_a1prime,N_a1,N_a2,N_j,N_z,N_e] (d1 split out so DiscountedEV broadcasts over it)

    entireRHS=reshape(ReturnMatrix+reshape(DiscountedEV,[1,N_d2*N_a1,1,N_a2,N_j,N_z]),[N_d,N_a1,N_a1,N_a2,N_j,N_z,N_e]);
    ...
    % midpoint is N_d-by-1-by-N_a1-by-N_a2-by-N_j-by-N_z-by-N_e
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        ReturnMatrix_e=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n_a1, n_a1,n_a2, n_z,special_n_e,N_j, d_gridvals, a1_gridvals, a1_gridvals, a2_grid, z_gridvals_J,e_val, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0
        % fastOLG: ReturnMatrix_e is [N_d,N_a1prime,N_a1,N_a2,N_j,N_z]

        entireRHS_e=ReturnMatrix_e+repelem(DiscountedEV,N_d1,1,1,1,1,1); % expand d2 → d=(d1,d2) to match ReturnMatrix_e dim 1

        [~,maxindex1]=max(entireRHS_e,[],2);

        midpoint=max(min(maxindex1,n_a1(1)-1),2);
        % midpoint is N_d-by-1-by-N_a1-by-N_a2-by-N_j-by-N_z
...
elseif vfoptions.lowmemory==2
    ...
    for z_c=1:N_z
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_ze=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc_e(ReturnFn, n_d1, n_d2, n_a1, n_a1,n_a2, special_n_z,special_n_e,N_j, d_gridvals, a1_gridvals, a1_gridvals, a2_grid, z_val,e_val, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0
            % fastOLG: ReturnMatrix_ze is [N_d,N_a1prime,N_a1,N_a2,N_j]

            entireRHS_ze=ReturnMatrix_ze+repelem(DiscountedEV_z,N_d1,1,1,1,1); % expand d2 → d=(d1,d2) to match ReturnMatrix_ze dim 1
