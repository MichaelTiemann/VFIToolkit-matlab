function [V, Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_ExpAssetz_GI1_nod1_e_raw(V,n_d2,n_a1,n_a2,n_z,n_e,N_j, d2_gridvals,a1_gridvals,a2_grid, z_gridvals_J,e_gridvals_J, pi_z_J,pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ReturnMatrix=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc_e(ReturnFn, 0, n_d2, n_a1, n_a1,n_a2, n_z,n_e,N_j, d2_gridvals, a1_gridvals, a1_gridvals, a2_grid, z_gridvals_J,e_gridvals_J, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0
    % fastOLG: ReturnMatrix is [N_d2,N_a1prime,N_a1,N_a2,N_j,N_z,N_e]

    entireRHS=ReturnMatrix+DiscountedEV;

    % First, we want a1prime conditional on (d,1,a1,a2,j,z,e)
    ...
    % midpoint is N_d2-by-1-by-N_a1-by-N_a2-by-N_j-by-N_z-by-N_e
    ...
    d2aprimejz=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*jind+N_d2*N_a1prime*N_a2*N_j*zind;
    ...
    linidx_lower=reshape(dind,[1,N_a1*N_a2,N_j,N_z,N_e])                   +N_d2*n2long*gpuArray(0:1:N_a-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-1)+N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_z-1),-2)+N_d2*n2long*N_a*N_j*N_z*shiftdim(gpuArray(0:1:N_e-1),-3);
    linidx_upper=reshape(dind,[1,N_a1*N_a2,N_j,N_z,N_e])+N_d2*(n2long-1)   +N_d2*n2long*gpuArray(0:1:N_a-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-1)+N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_z-1),-2)+N_d2*n2long*N_a*N_j*N_z*shiftdim(gpuArray(0:1:N_e-1),-3);
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        ReturnMatrix_e=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc_e(ReturnFn, 0, n_d2, n_a1, n_a1,n_a2, n_z,special_n_e,N_j, d2_gridvals, a1_gridvals, a1_gridvals, a2_grid, z_gridvals_J,e_val, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0
        % fastOLG: ReturnMatrix_e is [N_d2,N_a1prime,N_a1,N_a2,N_j,N_z]

        entireRHS_e=ReturnMatrix_e+DiscountedEV;

        [~,maxindex1]=max(entireRHS_e,[],2);

        midpoint=max(min(maxindex1,n_a1(1)-1),2);
        % midpoint is N_d2-by-1-by-N_a1-by-N_a2-by-N_j-by-N_z
        ...
        % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
        L2offset=ceil(maxindexL2/N_d2);
        linidx_lower=reshape(dind,[1,N_a1*N_a2,N_j,N_z])                   +N_d2*n2long*gpuArray(0:1:N_a-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-1)+N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_z-1),-2);
        linidx_upper=reshape(dind,[1,N_a1*N_a2,N_j,N_z])+N_d2*(n2long-1)   +N_d2*n2long*gpuArray(0:1:N_a-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-1)+N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_z-1),-2);
        isInfLower=(ReturnMatrix_ii(linidx_lower)==-Inf);
        isInfUpper=(ReturnMatrix_ii(linidx_upper)==-Inf);
        inLowerStrict=(L2offset>=2)         & (L2offset<=n2short+1);
        inUpperStrict=(L2offset>=n2short+3) & (L2offset<=n2long-1);
...
elseif vfoptions.lowmemory==2
    ...
    for z_c=1:N_z
        ...
        for e_c=1:N_e
            ...
            ReturnMatrix_ze=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc_e(ReturnFn, 0, n_d2, n_a1, n_a1,n_a2, special_n_z,special_n_e,N_j, d2_gridvals, a1_gridvals, a1_gridvals, a2_grid, z_val,e_val, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0
            % fastOLG: ReturnMatrix_ze is [N_d2,N_a1prime,N_a1,N_a2,N_j]
            ...
            midpoint=max(min(maxindex1,n_a1(1)-1),2);
            % midpoint is N_d2-by-1-by-N_a1-by-N_a2-by-N_j
            a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ...
            % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
            L2offset=ceil(maxindexL2/N_d2);
            linidx_lower=reshape(dind,[1,N_a1*N_a2,N_j])                   +N_d2*n2long*gpuArray(0:1:N_a-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-1);
            linidx_upper=reshape(dind,[1,N_a1*N_a2,N_j])+N_d2*(n2long-1)   +N_d2*n2long*gpuArray(0:1:N_a-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-1);
            isInfLower=(ReturnMatrix_ii(linidx_lower)==-Inf);
            isInfUpper=(ReturnMatrix_ii(linidx_upper)==-Inf);
            inLowerStrict=(L2offset>=2)         & (L2offset<=n2short+1);
            inUpperStrict=(L2offset>=n2short+3) & (L2offset<=n2long-1);
