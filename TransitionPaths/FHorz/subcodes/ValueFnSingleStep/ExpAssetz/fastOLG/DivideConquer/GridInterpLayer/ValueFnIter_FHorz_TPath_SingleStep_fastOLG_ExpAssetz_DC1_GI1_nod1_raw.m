function [V, Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_ExpAssetz_DC1_GI1_nod1_raw(V,n_d2,n_a1,n_a2,n_z,N_j, d2_gridvals,a1_gridvals,a2_grid, z_gridvals_J,pi_z_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
Policy3=zeros(4,N_a,N_j,N_z,'gpuArray'); %first dim indexes the optimal choice for d and a1prime rest of dimensions a,z (d2, midpoint, a1primeL2ind, L2flag)
...
elseif vfoptions.EVpre==1
    ...
    % Need to add the indexes for j to the aprimeIndex, remember fastOLG so V is (a,j)-by-z
    Vlower=reshape(V(aprimeIndex+shiftdim(N_a*gpuArray(0:1:N_j-1),-1),:),[N_d2*N_a1,N_a2,N_j,N_z,N_z]); % (d2*a1prime,a2,j,z,zprime)
    Vupper=reshape(V(aprimeplus1Index+shiftdim(N_a*gpuArray(0:1:N_j-1),-1),:),[N_d2*N_a1,N_a2,N_j,N_z,N_z]);
    % Skip interpolation when upper and lower are equal (otherwise can cause numerical rounding errors)
    skipinterp=(Vlower==Vupper);
    aprimeProbs(skipinterp)=0; % effectively skips interpolation
...
if vfoptions.lowmemory==0
    % n-Monotonicity
    ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, n_a1, vfoptions.level1n,n_a2, n_z,N_j, d2_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_grid, z_gridvals_J, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, maxgap(ii)+1, level1iidiff(ii),n_a2, n_z,N_j, d2_gridvals, a1_gridvals(aprimeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J, ReturnFnParamsAgeMatrix,3,0); % Level=3, Refine=0
    ...
    ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, n2long, n_a1,n_a2, n_z,N_j, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_grid, z_gridvals_J, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
...
elseif vfoptions.lowmemory==1
    ...
    for z_c=1:N_z
        ...
        % n-Monotonicity
        ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, n_a1, vfoptions.level1n,n_a2, special_n_z,N_j, d2_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_grid, z_val, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, maxgap(ii)+1, level1iidiff(ii),n_a2, special_n_z,N_j, d2_gridvals, a1_gridvals(aprimeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsAgeMatrix,3,0); % Level=3, Refine=0
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, n2long, n_a1,n_a2, special_n_z,N_j, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_grid, z_val, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
        ...
        Policy3(1,:,:,z_c)=dind; % d2
        Policy3(2,:,:,z_c)=shiftdim(squeeze(midpoint(allind)),-1); % a1prime midpoint
        Policy3(3,:,:,z_c)=shiftdim(ceil(maxindexL2/N_d2),-1); % a1primeL2ind
        % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
        L2offset=ceil(maxindexL2/N_d2);
        ...
        inLowerStrict=(L2offset>=2)         & (L2offset<=n2short+1);
        inUpperStrict=(L2offset>=n2short+3) & (L2offset<=n2long-1);
        Policy3(4,:,:,z_c)=shiftdim(2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper),-1);
