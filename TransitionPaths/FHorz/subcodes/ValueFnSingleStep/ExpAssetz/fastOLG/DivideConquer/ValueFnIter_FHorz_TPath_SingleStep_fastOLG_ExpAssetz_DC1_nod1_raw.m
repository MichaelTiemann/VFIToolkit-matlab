function [V, Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_ExpAssetz_DC1_nod1_raw(V,n_d2,n_a1,n_a2,n_z,N_j, d2_gridvals,a1_gridvals,a2_grid, z_gridvals_J,pi_z_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
% Note: The fact that we are here means a1 is 1-dimensional, so a1_gridvals is just a1_grid
...
if vfoptions.lowmemory==0
    ...
    [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d2*N_a1,vfoptions.level1n*N_a2,N_j,N_z]),[],1);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, maxgap(ii)+1, level1iidiff(ii),n_a2, n_z,N_j, d2_gridvals, a1_gridvals(aprimeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
            ...
            entireRHS_ii=ReturnMatrix_ii+repelem(reshape(DiscountedEV(d2aprimejz),[N_d2*(maxgap(ii)+1),N_a2,N_j,N_z]),1,level1iidiff(ii),1,1);
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, 1, level1iidiff(ii),n_a2, n_z,N_j, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_gridvals_J, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
            ...
            entireRHS_ii=ReturnMatrix_ii+repelem(reshape(DiscountedEV(d2aprimejz),[N_d2,N_a2,N_j,N_z]),1,level1iidiff(ii),1,1);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,:,:)=shiftdim(Vtempii,1);
            % maxindex does not need reworking, as with expasset there is no a2prime
            % the a1prime is relative to loweredge(allind), need to 'add' the loweredge
...
elseif vfoptions.lowmemory==1
    ...
    for z_c=1:N_z
        ...
        % n-Monotonicity
        ReturnMatrix_ii_z=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, n_a1, vfoptions.level1n,n_a2, special_n_z,N_j, d2_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_grid, z_val, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0
        ...
        % Now, get and store the full (d,aprime)
        [Vtempii,maxindex2]=max(reshape(entireRHS_ii_z,[N_d2*N_a1,vfoptions.level1n*N_a2,N_j]),[],1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_z=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, maxgap(ii)+1, level1iidiff(ii),n_a2, special_n_z,N_j, d2_gridvals, a1_gridvals(aprimeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
                d2aprimej=d2ind+N_d2*(aprimeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*jind; % with the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii_z=ReturnMatrix_ii_z+repelem(reshape(DiscountedEV_z(d2aprimej),[N_d2*(maxgap(ii)+1),N_a2,N_j]),1,level1iidiff(ii),1);
                ...
                Policy(curraindex,:,z_c)=shiftdim(maxindex+N_d2*(loweredge(allind)-1),1);
            else
                loweredge=maxindex1(:,1,ii,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_z=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, 0, n_d2, 1, level1iidiff(ii),n_a2, special_n_z,N_j, d2_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_val, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
                d2aprimej=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*jind; % with the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii_z=ReturnMatrix_ii_z+repelem(reshape(DiscountedEV_z(d2aprimej),[N_d2,N_a2,N_j]),1,level1iidiff(ii),1);
                [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
                V(curraindex,:,z_c)=shiftdim(Vtempii,1);
                % maxindex does not need reworking, as with expasset there is no a2prime
                % the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                ...
                allind=reshape(dind,[1,1,level1iidiff(ii),N_a2,N_j])+N_d2*a2ind+N_d2*N_a2*jind; % loweredge is n_d-by-1-by-1-by-n_a2-by-N_j
                allind=reshape(allind,[1,level1iidiff(ii)*N_a2,N_j]);
                Policy(curraindex,:,z_c)=shiftdim(maxindex+N_d2*(loweredge(allind)-1),1); % loweredge
            end
        end
    end
end
