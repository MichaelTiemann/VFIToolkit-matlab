function [V, Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_DC1_GI1_nod_noz_e_raw(V,n_a,n_e,N_j, a_grid,e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    entireRHS_ii=ReturnMatrix_ii+DiscountedEV; % (aprime,a and j,e), autofills a for expectation term
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            % loweredge is 1-by-1-by-N_j-by-n_e
            aprimeindexes=loweredge+(0:1:maxgap(ii))'; % ' due to no d
            % aprimeindexes is 1-by-maxgap(ii)+1-by-N_j-by-n_e
            ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_Disc_DC1_nod(ReturnFn,n_e,N_j,a_grid(aprimeindexes),a_grid(level1ii(ii)+1:level1ii(ii+1)-1),e_gridvals_J,ReturnFnParamsAgeMatrix,2);
            aprimej=repelem(aprimeindexes,1,level1iidiff(ii),1)+N_a*jind;
            entireRHS_ii=ReturnMatrix_ii+reshape(DiscountedEV(aprimej(:)),[maxgap(ii)+1,level1iidiff(ii),N_j,N_e]);
...
elseif vfoptions.lowmemory==1
    ...
    V=zeros(N_a*N_j,N_e,vfoptions.precision,'gpuArray'); % preallocate: V is over (a,j,e)

    for e_c=1:N_e
        e_vals=e_gridvals_J(1,1,:,e_c,:); % e_gridvals_J has shape (1,1,N_j,N_e,l_e) for fastOLG without z
        ...
        entireRHS_ii=ReturnMatrix_ii+DiscountedEV; % (aprime,a and j,z), autofills j for expectation term
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_Disc_DC1_nod(ReturnFn,special_n_e,N_j,a_grid(aprimeindexes),a_grid(level1ii(ii)+1:level1ii(ii+1)-1),e_vals,ReturnFnParamsAgeMatrix,2);
                aprimej=repelem(aprimeindexes,1,level1iidiff(ii),1)+N_a*jind;
