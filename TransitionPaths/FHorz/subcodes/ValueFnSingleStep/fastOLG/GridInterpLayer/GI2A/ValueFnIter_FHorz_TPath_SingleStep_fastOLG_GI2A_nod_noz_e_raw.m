function [V,Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_GI2A_nod_noz_e_raw(V,n_a,n_e,N_j, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% fastOLG GI2A (no d, no z, has e): parallelize over "age" (j), grid-interpolation
...
% For broadcasting against a1primeindexes of shape (n2long, N_a2, N_a1, N_a2, N_j, N_e):
a2ind=gpuArray(0:1:N_a2-1);               % (1,N_a2)              -> a2prime axis (dim 2)
jind =shiftdim(gpuArray(0:1:N_j-1),-3);   % (1,1,1,1,N_j)         -> j axis (dim 5)
eBind=shiftdim(gpuArray(0:1:N_e-1),-4);   % (1,1,1,1,1,N_e)       -> e axis (dim 6)
...
if vfoptions.lowmemory==0
    ...
    ReturnMatrix=CreateReturnFnMatrix_fastOLG_Disc_DC2A_nod(ReturnFn, n_e, N_j, a1_grid, a2_grid, a1_grid, a2_grid, e_gridvals_J, ReturnFnParamsAgeMatrix, 1);
    % Shape: (a1prime, a2prime, a1, a2, j, e)

    entireRHS=ReturnMatrix+DiscountedEV; % EV is (a1prime,a2prime,1,1,j) — broadcasts on dims 3,4 and the trailing e dim

    % argmax over a1prime: result shape (1, a2prime, a1, a2, j, e)
    [~,maxindex]=max(entireRHS,[],1);

    % Turn this into the 'midpoint'
    midpoint=max(min(maxindex,n_a1-1),2); % avoid the top end (inner), and avoid the bottom end (outer)
    % midpoint is (1, n_a2, n_a1, n_a2, N_j, n_e)
    a1primeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short)'; % a1prime points either side of midpoint (column vector)
    % a1primeindexes shape: (n2long, n_a2, n_a1, n_a2, N_j, n_e)
    ...
    aprime=a1primeindexes+N_a1fine*a2ind+N_a1fine*N_a2*jind; % shape (n2long, N_a2, N_a1, N_a2, N_j, N_e)
    ...
    % Map back to midpoint per (a,j,e): linear index into midpoint(1,a2prime,a1,a2,j,e) treated as (a2prime,a,j,e)
    midpoint_pick=maxindexL2a2+N_a2*a12ind+N_a2*N_a*jBind+N_a2*N_a*N_j*eind;
    ...
    linidx_lower = 1                  + n2long*(maxindexL2a2-1) + n2long*N_a2*aBind + n2long*N_a2*N_a*jBind + n2long*N_a2*N_a*N_j*eind;
    linidx_upper = n2long*maxindexL2a2                          + n2long*N_a2*aBind + n2long*N_a2*N_a*jBind + n2long*N_a2*N_a*N_j*eind;
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        ReturnMatrix_e=CreateReturnFnMatrix_fastOLG_Disc_DC2A_nod(ReturnFn, special_n_e, N_j, a1_grid, a2_grid, a1_grid, a2_grid, e_vals, ReturnFnParamsAgeMatrix, 1);
