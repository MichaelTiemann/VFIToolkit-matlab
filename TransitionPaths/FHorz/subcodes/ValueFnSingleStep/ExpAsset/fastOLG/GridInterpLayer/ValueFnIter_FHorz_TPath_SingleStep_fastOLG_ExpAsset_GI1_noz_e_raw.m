function [V,Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_ExpAsset_GI1_noz_e_raw(V,n_d1,n_d2,n_a1,n_a2,n_e,N_j, d_gridvals,d2_gridvals,a1_gridvals,a2_grid, e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
Policy3=zeros(4,N_a,N_j,N_e,vfoptions.indexT,'gpuArray'); % first dim indexes the optimal choice for d and a1prime rest of dimensions a,e (d, midpoint, a1primeL2ind, L2flag)
...
if vfoptions.lowmemory==0
    ReturnMatrix=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, n_a1, n_a1,n_a2, n_e,N_j, d_gridvals, a1_gridvals, a1_gridvals, a2_grid, e_gridvals_J, ReturnFnParamsAgeMatrix,1,1); % Level=1, Refine=1
    % fastOLG: ReturnMatrix is [N_d1,N_d2*N_a1prime,N_a1,N_a2,N_j,N_e] (d1 split out so DiscountedEV broadcasts over it)

    entireRHS=reshape(ReturnMatrix+reshape(DiscountedEV,[1,N_d2*N_a1,1,N_a2,N_j]),[N_d,N_a1,N_a1,N_a2,N_j,N_e]);

    % First, we want a1prime conditional on (d,1,a1,a2,j,e)
    ...
    % midpoint is N_d-by-1-by-N_a1-by-N_a2-by-N_j-by-N_e
    a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short); % aprime points either side of midpoint, fine index
    % aprime possibilities are N_d-by-n2long-by-N_a1-by-N_a2-by-N_j-by-N_e
    ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, n2long, n_a1,n_a2, n_e,N_j, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_grid, e_gridvals_J, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
    d2aprimej=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*jind;
    entireRHS_ii=ReturnMatrix_ii+DiscountedEVinterp(reshape(d2aprimej,[N_d*n2long,N_a1*N_a2,N_j,N_e]));
    [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    V=shiftdim(Vtempii,1);
    dind=rem(maxindexL2-1,N_d)+1;
    allind=reshape(dind,[1,1,1,N_a1*N_a2,N_j,N_e])+N_d*aind+N_d*N_a*jind+N_d*N_a*N_j*eind; % midpoint is N_d-by-1-by-N_a1-by-N_a2-by-N_j-by-N_e
    ...
    linidx_lower=reshape(dind,[1,N_a1*N_a2,N_j,N_e])                  +N_d*n2long*gpuArray(0:1:N_a-1)+N_d*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-1)+N_d*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_e-1),-2);
    linidx_upper=reshape(dind,[1,N_a1*N_a2,N_j,N_e])+N_d*(n2long-1)   +N_d*n2long*gpuArray(0:1:N_a-1)+N_d*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-1)+N_d*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_e-1),-2);
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        ReturnMatrix_e=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, n_a1, n_a1,n_a2, special_n_e,N_j, d_gridvals, a1_gridvals, a1_gridvals, a2_grid, e_val, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0
        % fastOLG: ReturnMatrix_e is [N_d,N_a1prime,N_a1,N_a2,N_j]

        entireRHS_e=ReturnMatrix_e+repelem(DiscountedEV,N_d1,1,1,1,1); % expand d2 → d=(d1,d2) to match ReturnMatrix_e dim 1
        ...
        ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, n2long, n_a1,n_a2, special_n_e,N_j, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_grid, e_val, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
        d2aprimej=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*jind;
...
%% fastOLG with e (no z), so need to output to take certain shapes
