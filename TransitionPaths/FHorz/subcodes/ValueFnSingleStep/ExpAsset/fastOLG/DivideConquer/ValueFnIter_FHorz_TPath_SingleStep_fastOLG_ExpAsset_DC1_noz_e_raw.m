function [V,Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_ExpAsset_DC1_noz_e_raw(V,n_d1,n_d2,n_a1,n_a2,n_e,N_j, d_gridvals,d2_gridvals,a1_gridvals,a2_grid, e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if vfoptions.EVpre==0
    ...
    EVpre=[sum(V(N_a+1:end,:).*pi_e_J(1:end-N_a,:),2); zeros(N_a,1,'gpuArray')]; % pi_e_J already kron-expanded to (a-j,e) upstream; j=N_j zero-padded
...
DiscountedEV=DiscountFactorParamsVec.*reshape(EV,[N_d2,N_a1,1,N_a2,N_j]); % (d2,a1prime,1,a2,j)
...
if vfoptions.lowmemory==0
    ...
    [Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d*N_a1,vfoptions.level1n*N_a2,N_j,N_e]),[],1);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, maxgap(ii)+1, level1iidiff(ii),n_a2, n_e,N_j, d_gridvals, a1_gridvals(aprimeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_gridvals_J, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
            d2aprimejz=d2ind+N_d2*(aprimeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*jind; % with the current aprimeii(ii):aprimeii(ii+1)
            entireRHS_ii=ReturnMatrix_ii+repelem(reshape(DiscountedEV(d2aprimejz),[N_d*(maxgap(ii)+1),N_a2,N_j,N_e]),1,level1iidiff(ii),1,1);
            ...
            allind=reshape(dind,[1,1,level1iidiff(ii),N_a2,N_j,N_e])+N_d*a2ind+N_d*N_a2*jind+N_d*N_a2*N_j*eind; % loweredge is n_d-by-1-by-1-by-n_a2-by-N_j-by-n_e
            allind=reshape(allind,[1,level1iidiff(ii)*N_a2,N_j,N_e]);
            Policy(curraindex,:,:)=shiftdim(maxindex+N_d*(loweredge(allind)-1),1);
        else
            loweredge=maxindex1(:,1,ii,:,:,:);
            % Just use aprime(ii) for everything
            ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, 1, level1iidiff(ii),n_a2, n_e,N_j, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_gridvals_J, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
            d2aprimejz=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*jind; % with the current aprimeii(ii):aprimeii(ii+1)
            entireRHS_ii=ReturnMatrix_ii+repelem(reshape(DiscountedEV(d2aprimejz),[N_d,N_a2,N_j,N_e]),1,level1iidiff(ii),1,1);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,:,:)=shiftdim(Vtempii,1);
            % maxindex does not need reworking, as with expasset there is no a2prime
            % the a1prime is relative to loweredge(allind), need to 'add' the loweredge
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        % n-Monotonicity
        ReturnMatrix_ii_z=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, n_a1, vfoptions.level1n,n_a2, special_n_e,N_j, d_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_grid, e_val, ReturnFnParamsAgeMatrix,1,0); % Level=1, Refine=0

        entireRHS_ii_z=ReturnMatrix_ii_z+repelem(DiscountedEV,N_d1,1,1,1,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_z=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, maxgap(ii)+1, level1iidiff(ii),n_a2, special_n_e,N_j, d_gridvals, a1_gridvals(aprimeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_val, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
                d2aprimej=d2ind+N_d2*(aprimeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*jind; % with the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii_z=ReturnMatrix_ii_z+repelem(reshape(DiscountedEV(d2aprimej),[N_d*(maxgap(ii)+1),N_a2,N_j]),1,level1iidiff(ii),1);
                ...
                loweredge=maxindex1(:,1,ii,:,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_z=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, 1, level1iidiff(ii),n_a2, special_n_e,N_j, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_val, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
                d2aprimej=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*jind; % with the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii_z=ReturnMatrix_ii_z+repelem(reshape(DiscountedEV(d2aprimej),[N_d,N_a2,N_j]),1,level1iidiff(ii),1);
                [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
                V(curraindex,:,e_c)=shiftdim(Vtempii,1);
                % maxindex does not need reworking, as with expasset there is no a2prime
                % the a1prime is relative to loweredge(allind), need to 'add' the loweredge
