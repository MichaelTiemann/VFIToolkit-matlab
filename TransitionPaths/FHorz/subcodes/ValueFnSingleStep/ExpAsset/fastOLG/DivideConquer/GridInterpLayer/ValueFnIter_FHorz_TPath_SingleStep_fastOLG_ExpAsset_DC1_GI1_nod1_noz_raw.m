function [V,Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_ExpAsset_DC1_GI1_nod1_noz_raw(V,n_d2,n_a1,n_a2,N_j, d2_gridvals,a1_gridvals,a2_grid, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc_noz(ReturnFn, 0, n_d2, maxgap(ii)+1, level1iidiff(ii),n_a2,N_j, d2_gridvals, a1_gridvals(aprimeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, ReturnFnParamsAgeMatrix,3,0); % Level=3, Refine=0
    ...
    ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc_noz(ReturnFn, 0, n_d2, n2long, n_a1,n_a2,N_j, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_grid, ReturnFnParamsAgeMatrix,2,0); % Level=2, Refine=0
    ...
    linidx_lower=dind                  + N_d2*n2long*gpuArray(0:N_a-1) + N_d2*n2long*N_a*shiftdim(gpuArray(0:N_j-1),-1);
    linidx_upper=dind + N_d2*(n2long-1) + N_d2*n2long*gpuArray(0:N_a-1) + N_d2*n2long*N_a*shiftdim(gpuArray(0:N_j-1),-1);
