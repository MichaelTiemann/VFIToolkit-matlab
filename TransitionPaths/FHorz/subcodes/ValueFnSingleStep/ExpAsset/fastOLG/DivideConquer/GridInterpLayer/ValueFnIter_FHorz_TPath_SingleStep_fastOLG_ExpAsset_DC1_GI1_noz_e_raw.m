function [V,Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_ExpAsset_DC1_GI1_noz_e_raw(V,n_d1,n_d2,n_a1,n_a2,n_e,N_j, d_gridvals,d2_gridvals,a1_gridvals,a2_grid, e_gridvals_J, pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    midpoint=zeros(N_d,1,N_a1,N_a2,N_j,N_e,'gpuArray');
...
if vfoptions.lowmemory==0
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, maxgap(ii)+1, level1iidiff(ii),n_a2, n_e,N_j, d_gridvals, a1_gridvals(aprimeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_gridvals_J, ReturnFnParamsAgeMatrix,3,0); % Level=3, Refine=0
            d2aprimej=d2ind+N_d2*(aprimeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*jind; % with the current aprimeii(ii):aprimeii(ii+1)
            entireRHS_ii=ReturnMatrix_ii+reshape(DiscountedEV(d2aprimej),[N_d,(maxgap(ii)+1),1,N_a2,N_j,N_e]);
    ...
    linidx_lower=dind                  + N_d*n2long*reshape(gpuArray(0:N_a-1),[1,N_a,1,1]) + N_d*n2long*N_a*shiftdim(gpuArray(0:N_j-1),-1) + N_d*n2long*N_a*N_j*shiftdim(gpuArray(0:N_e-1),-2);
    linidx_upper=dind + N_d*(n2long-1) + N_d*n2long*reshape(gpuArray(0:N_a-1),[1,N_a,1,1]) + N_d*n2long*N_a*shiftdim(gpuArray(0:N_j-1),-1) + N_d*n2long*N_a*N_j*shiftdim(gpuArray(0:N_e-1),-2);
    ...
    for e_c=1:N_e
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_ExpAsset_Disc(ReturnFn, n_d1, n_d2, maxgap(ii)+1, level1iidiff(ii),n_a2, special_n_e,N_j, d_gridvals, a1_gridvals(aprimeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_val, ReturnFnParamsAgeMatrix,3,0); % Level=3, Refine=0
                d2aprimej=d2ind+N_d2*(aprimeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*jind; % with the current aprimeii(ii):aprimeii(ii+1)
                entireRHS_ii=ReturnMatrix_ii+reshape(DiscountedEV(d2aprimej),[N_d,(maxgap(ii)+1),1,N_a2,N_j]);
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
            else
                loweredge=maxindex1(:,1,ii,:,:,:);
