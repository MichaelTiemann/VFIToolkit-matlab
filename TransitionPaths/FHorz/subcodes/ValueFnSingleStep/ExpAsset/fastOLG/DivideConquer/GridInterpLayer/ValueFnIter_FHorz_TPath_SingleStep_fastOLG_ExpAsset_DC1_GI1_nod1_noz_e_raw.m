function [V, Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_ExpAsset_DC1_GI1_nod1_noz_e_raw(V,n_d2,n_a1,n_a2,n_e,N_j, d2_gridvals,a1_gridvals,a2_grid, e_gridvals_J,pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    linidx_lower=dind                   + N_d2*n2long*reshape(gpuArray(0:N_a-1),[1,N_a,1,1]) + N_d2*n2long*N_a*shiftdim(gpuArray(0:N_j-1),-1) + N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:N_e-1),-2);
    linidx_upper=dind + N_d2*(n2long-1) + N_d2*n2long*reshape(gpuArray(0:N_a-1),[1,N_a,1,1]) + N_d2*n2long*N_a*shiftdim(gpuArray(0:N_j-1),-1) + N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:N_e-1),-2);
