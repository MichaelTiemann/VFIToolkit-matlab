function [V,Policy,Vhat]=ValueFnIter_FHorz_TPath_SingleStep_QHS_fastOLG_noz_e_raw(V,n_d,n_a,n_e,N_j, d_gridvals, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    entireRHS_under=ReturnMatrix+DiscountedEV_under; %(d,aprime)-by-(a,j,e)
    maxindexfull=Policy+(N_d*N_a)*(0:1:N_a-1)+(N_d*N_a*N_a)*shiftdim((0:1:N_j-1),-1)+(N_d*N_a*N_a*N_j)*shiftdim((0:1:N_e-1),-2);
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        % Now Vunderbar
        entireRHS_under_e=ReturnMatrix_e+DiscountedEV_under; %(d,aprime)-by-(a,j)
