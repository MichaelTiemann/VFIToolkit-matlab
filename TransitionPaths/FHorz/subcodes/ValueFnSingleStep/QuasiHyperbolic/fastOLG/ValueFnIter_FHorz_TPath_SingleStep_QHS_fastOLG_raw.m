function [V,Policy,Vhat]=ValueFnIter_FHorz_TPath_SingleStep_QHS_fastOLG_raw(V,n_d,n_a,n_z,N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
DiscountedEV_under=repelem(DiscountedEV_under,N_d,1,1,1); % [d & aprime,1,j,z]
...
if vfoptions.lowmemory==0
    ...
    entireRHS_under=ReturnMatrix+DiscountedEV_under; %  [d & aprime,a,j,z]
    maxindexfull=Policy+(N_d*N_a)*(0:1:N_a-1)+(N_d*N_a*N_a)*shiftdim((0:1:N_j-1),-1)+(N_d*N_a*N_a*N_j)*shiftdim((0:1:N_z-1),-2);
...
elseif vfoptions.lowmemory==1
    ...
    for z_c=1:N_z
        ...
        % Now Vunderbar
        entireRHS_under_z=ReturnMatrix_z+DiscountedEV_under_z; %(d,aprime)-by-(a,j)
