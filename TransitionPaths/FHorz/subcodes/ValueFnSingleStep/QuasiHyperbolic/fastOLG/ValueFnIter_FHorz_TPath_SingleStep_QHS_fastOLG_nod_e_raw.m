function [V, Policy, Vhat]=ValueFnIter_FHorz_TPath_SingleStep_QHS_fastOLG_nod_e_raw(V,n_a,n_z,n_e,N_j, a_grid, z_gridvals_J,e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    entireRHS_under=ReturnMatrix+DiscountedEV_under; %(aprime)-by-(a,j)-by-z-by-e
    maxindexfull=Policy+N_a*(0:1:N_a-1)+(N_a*N_a)*shiftdim((0:1:N_j-1),-1)+(N_a*N_a*N_j)*shiftdim((0:1:N_z-1),-2)+(N_a*N_a*N_j*N_z)*shiftdim((0:1:N_e-1),-3);
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        % Now Vunderbar
        entireRHS_under_e=ReturnMatrix_e+DiscountedEV_under; %(aprime)-by-(a,j)-by-z
        maxindexfull=maxindex+N_a*(0:1:N_a-1)+(N_a*N_a)*shiftdim((0:1:N_j-1),-1)+(N_a*N_a*N_j)*shiftdim((0:1:N_z-1),-2);
...
elseif vfoptions.lowmemory==2
    ...
    for z_c=1:N_z
        ...
        for e_c=1:N_e
            ...
            % Now Vunderbar
            entireRHS_under_ze=ReturnMatrix_ze+DiscountedEV_under_z; %(aprime)-by-(a,j)
