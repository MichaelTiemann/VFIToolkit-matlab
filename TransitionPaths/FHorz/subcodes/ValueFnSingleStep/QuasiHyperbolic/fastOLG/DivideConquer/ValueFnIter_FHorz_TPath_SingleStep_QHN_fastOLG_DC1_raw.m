function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_fastOLG_DC1_raw(V,n_d,n_a,n_z,N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
% fastOLG, so a-j-z
Policy=zeros(N_a,N_j,N_z,vfoptions.indexT,'gpuArray'); % first dim indexes the optimal choice for d and aprime
...
V=zeros(N_a,N_j,N_z,vfoptions.precision,'gpuArray'); % V is over (a,j); for Naive QH carries Valt

DiscountedEV=shiftdim(reshape(DiscountFactor_J,[1,1,N_j]).*EV,-1); % [1,N_a,1,N_j,N_z] beta_j*EV; 1st dim will autofill d, 3rd dim will autofill a
...
if vfoptions.lowmemory==0
    ...
    for ii=1:(vfoptions.level1n-1)
        if maxgap_V(ii)>0
            loweredge=min(maxindex1(:,1,ii,:,:),n_a-maxgap_V(ii)); % maxindex1(:,ii), but avoid going off top of grid when we add maxgap(ii) points
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV(aprimez(:)),[N_d*(maxgap_V(ii)+1),1,N_j,N_z]); % broadcast level1iidiff at dim 2
            [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
            V(level1ii(ii)+1:level1ii(ii+1)-1,:,:)=shiftdim(Vtempii,1);
            Policyalt(level1ii(ii)+1:level1ii(ii+1)-1,:,:)=shiftdim(maxindexalt+N_d*(loweredge(rem(maxindexalt-1,N_d)+1+N_d*jBind+N_d*N_j*zBind)-1),1); % loweredge(given the d)
        else
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV(aprimez(:)),[N_d,1,N_j,N_z]); % broadcast level1iidiff at dim 2
            [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
            V(level1ii(ii)+1:level1ii(ii+1)-1,:,:)=shiftdim(Vtempii,1);
            Policyalt(level1ii(ii)+1:level1ii(ii+1)-1,:,:)=shiftdim(maxindexalt+N_d*(loweredge(rem(maxindexalt-1,N_d)+1+N_d*jBind+N_d*N_j*zBind)-1),1); % loweredge(given the d)
        end
    ...
    % Store
    Policy(level1ii,:,:)=maxindex2; % combined (d,aprime)
...
elseif vfoptions.lowmemory==1
    ...
    for z_c=1:N_z
        ...
        Policyalt(level1ii,:,z_c)=maxindex2+N_d*(maxindex1d-1); % d,aprime
        ...
        for ii=1:(vfoptions.level1n-1)
            if maxgap_V(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV_z(aprime(:)),[N_d*(maxgap_V(ii)+1),1,N_j]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                V(level1ii(ii)+1:level1ii(ii+1)-1,:,z_c)=shiftdim(Vtempii,1);
                Policyalt(level1ii(ii)+1:level1ii(ii+1)-1,:,z_c)=shiftdim(maxindexalt+N_d*(loweredge(rem(maxindexalt-1,N_d)+1+N_d*jBind)-1),1);
            else
                loweredge=maxindex1(:,1,ii,:);
                ...
                entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV_z(aprime(:)),[N_d,1,N_j]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                V(level1ii(ii)+1:level1ii(ii+1)-1,:,z_c)=shiftdim(Vtempii,1);
                Policyalt(level1ii(ii)+1:level1ii(ii+1)-1,:,z_c)=shiftdim(maxindexalt+N_d*(loweredge(rem(maxindexalt-1,N_d)+1+N_d*jBind)-1),1);
            end
        ...
        Policy(level1ii,:,z_c)=maxindex2;
        Vtilde(level1ii,:,z_c)=shiftdim(Vtempii,1);
