function [V,Policy,Vhat]=ValueFnIter_FHorz_TPath_SingleStep_QHS_fastOLG_DC1_e_raw(V,n_d,n_a,n_z,n_e,N_j, d_gridvals, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% fastOLG just means parallelize over "age" (j)
% fastOLG is done as (a,j,z,e), rather than standard (a,z,e,j)
% V is (a,j)-by-z-by-e; for Sophisticated QH carries Vunderbar
...
Vhat=zeros(N_a,N_j,N_z,N_e,'gpuArray'); % beta0*beta-step value (snapshot of V before Vunderbar transform)
...
V=zeros(N_a,N_j,N_z,N_e,vfoptions.precision,'gpuArray'); % V is over (a,j,z,e); for Sophisticated QH carries Vunderbar
...
if vfoptions.lowmemory==0
    ...
    entireRHS_ii=ReturnMatrix_ii+Beta0DiscountedEV; % (d,aprime,a and j,z,e), autofills a and e for expectation term
...
elseif vfoptions.lowmemory==2
    ...
    for z_c=1:N_z
        ...
        for e_c=1:N_e
            ...
            Policy(level1ii,:,z_c,e_c)=maxindex2+N_d*(maxindex1d-1);
