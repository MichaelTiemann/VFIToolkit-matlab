function [V,Policy,Vhat]=ValueFnIter_FHorz_TPath_SingleStep_QHS_fastOLG_DC1_GI1_e_raw(V,n_d,n_a,n_z,n_e,N_j, d_gridvals, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
% fastOLG, so a-j-z-e
Policy=zeros(4,N_a,N_j,N_z,N_e,vfoptions.indexT,'gpuArray'); % first dim indexes the optimal choice for d and aprime (d, midpoint, L2, L2 flag)
...
DiffDiscountedEVinterp=reshape(betaminusbeta0beta_J,[1,1,N_j]).*EVinterp; % [n2aprime fine,1,j,z]
...
if vfoptions.lowmemory==0
    ...
    EVfine=reshape(DiffDiscountedEVinterp(aprimejz(:)),[N_d*n2long,N_a,N_j,N_z,N_e]);
    ...
    linidx=reshape(maxindexL2,[1,N_a*N_j*N_z*N_e])+N_d*n2long*(0:N_a*N_j*N_z*N_e-1);
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        aprimejz=aprimeindexes+n2aprime*jind+n2aprime*N_j*zind;
        EVfine_e=reshape(DiffDiscountedEVinterp(aprimejz(:)),[N_d*n2long,N_a,N_j,N_z]);
        ...
        linidx_e=reshape(maxindexL2,[1,N_a*N_j*N_z])+N_d*n2long*(0:N_a*N_j*N_z-1);
...
elseif vfoptions.lowmemory==2
    ...
    for z_c=1:N_z
        ...
        for e_c=1:N_e
            ...
            EVfine_ze=reshape(DiffDiscountedEVinterp_z(aprimej(:)),[N_d*n2long,N_a,N_j]);
            ...
            linidx_ze=reshape(maxindexL2,[1,N_a*N_j])+N_d*n2long*(0:N_a*N_j-1);
            EV_at_policy_ze=reshape(EVfine_ze(linidx_ze),[N_a*N_j,1]);
            Vhat(:,z_c,e_c)=reshape(Vhat_ze,[N_a*N_j,1]);
            V(:,z_c,e_c)=Vhat(:,z_c,e_c)+EV_at_policy_ze;
        end
    end
end
