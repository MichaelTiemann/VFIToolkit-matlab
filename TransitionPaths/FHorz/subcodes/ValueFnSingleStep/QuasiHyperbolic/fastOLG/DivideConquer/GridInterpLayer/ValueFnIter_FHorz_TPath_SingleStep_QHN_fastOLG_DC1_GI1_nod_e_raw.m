function [V, Policy, Policyalt, Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_fastOLG_DC1_GI1_nod_e_raw(V,n_a,n_z,n_e,N_j, a_grid, z_gridvals_J,e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    midpoints_jj=zeros(1,N_a,N_j,N_z,N_e,'gpuArray');
...
% fastOLG will be N_aprime by N_a*N_j*N_z*N_e (note: N_aprime is just equal to N_a)

beta_J=prod(CreateAgeMatrixFromParams(Parameters, DiscountFactorParamNames,N_j,vfoptions.precision),2);
...
if vfoptions.lowmemory==0
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV_alt(aprimejz(:)),[maxgap(ii)+1,level1iidiff(ii),N_j,N_z,N_e]);
    ...
    ReturnMatrix_L2=CreateReturnFnMatrix_fastOLG_Disc_DC1_nod_e(ReturnFn,n_z,n_e,N_j,aprime_grid(aprimeindexes),a_grid,z_gridvals_J,e_gridvals_J,ReturnFnParamsAgeMatrix,2);
    aprimejz=aprimeindexes+n2aprime*jind+n2aprime*N_j*zind;
    entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(aprimejz(:)),[n2long,N_a,N_j,N_z,N_e]);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            ReturnMatrix_ii_dc=CreateReturnFnMatrix_fastOLG_Disc_DC1_nod_e(ReturnFn,n_z,n_e,N_j,a_grid(aprimeindexes),a_grid(level1ii(ii)+1:level1ii(ii+1)-1),z_gridvals_J,e_gridvals_J,ReturnFnParamsAgeMatrix,2);
            aprimejz=repelem(aprimeindexes,1,level1iidiff(ii),1)+N_a*jind+N_a*N_j*zind;
            entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV(aprimejz(:)),[maxgap(ii)+1,level1iidiff(ii),N_j,N_z,N_e]);
            [~,maxindex]=max(entireRHS_ii,[],1);
            midpoints_jj(1,curraindex,:,:,:)=maxindex+(loweredge-1);
        else
            loweredge=maxindex1(1,ii,:,:,:);
    ...
    ReturnMatrix_L2=CreateReturnFnMatrix_fastOLG_Disc_DC1_nod_e(ReturnFn,n_z,n_e,N_j,aprime_grid(aprimeindexes),a_grid,z_gridvals_J,e_gridvals_J,ReturnFnParamsAgeMatrix,2);
    aprimejz=aprimeindexes+n2aprime*jind+n2aprime*N_j*zind;
    entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp(aprimejz(:)),[n2long,N_a,N_j,N_z,N_e]);
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        % n-Monotonicity
        ReturnMatrix_ii=CreateReturnFnMatrix_fastOLG_Disc_DC1_nod_e(ReturnFn, n_z,special_n_e,N_j, a_grid, a_grid(level1ii), z_gridvals_J,e_vals, ReturnFnParamsAgeMatrix,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                loweredge=min(maxindex1(1,ii,:,:),n_a-maxgap(ii));
                aprimeindexes=loweredge+(0:1:maxgap(ii))';
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_fastOLG_Disc_DC1_nod_e(ReturnFn,n_z,special_n_e,N_j,a_grid(aprimeindexes),a_grid(level1ii(ii)+1:level1ii(ii+1)-1),z_gridvals_J,e_vals,ReturnFnParamsAgeMatrix,2);
                aprimejz=repelem(aprimeindexes,1,level1iidiff(ii),1)+N_a*jind+N_a*N_j*zind;
        ...
        midpoints_jj=max(min(midpoints_jj,n_a-1),2);
        aprimeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
        ReturnMatrix_L2=CreateReturnFnMatrix_fastOLG_Disc_DC1_nod_e(ReturnFn,n_z,special_n_e,N_j,aprime_grid(aprimeindexes),a_grid,z_gridvals_J,e_vals,ReturnFnParamsAgeMatrix,2);
        aprimej=aprimeindexes+n2aprime*jind+n2aprime*N_j*zind;
        entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(aprimej(:)),[n2long,N_a,N_j,N_z]);
        [Vtempii,maxindexL2alt]=max(entireRHS_L2,[],1);
        V(:,:,:,e_c)=shiftdim(Vtempii,1);
        Policyalt(1,:,:,:,e_c)=shiftdim(squeeze(midpoints_jj),-1);
...
elseif vfoptions.lowmemory==2
    ...
    for z_c=1:N_z
        ...
        for e_c=1:N_e
            ...
            midpoints_jj=max(min(midpoints_jj,n_a-1),2);
            aprimeindexes=(midpoints_jj+(midpoints_jj-1)*n2short)+(-n2short-1:1:1+n2short)';
            ReturnMatrix_L2=CreateReturnFnMatrix_fastOLG_Disc_DC1_nod_e(ReturnFn,special_n_z,special_n_e,N_j,aprime_grid(aprimeindexes),a_grid,z_vals,e_vals,ReturnFnParamsAgeMatrix,2);
            ...
            Policyalt(1,:,:,z_c,e_c)=shiftdim(squeeze(midpoints_jj),-1);
            Policyalt(2,:,:,z_c,e_c)=shiftdim(maxindexL2alt,-1);

            isInfLoweralt    = (ReturnMatrix_L2(1,     :,:) == -Inf);
            isInfUpperalt    = (ReturnMatrix_L2(n2long,:,:) == -Inf);
            inLowerStrictalt = (maxindexL2alt >= 2)         & (maxindexL2alt <= n2short+1);
            inUpperStrictalt = (maxindexL2alt >= n2short+3) & (maxindexL2alt <= n2long-1);
            Policyalt(3,:,:,z_c,e_c) = shiftdim(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt),-1);
