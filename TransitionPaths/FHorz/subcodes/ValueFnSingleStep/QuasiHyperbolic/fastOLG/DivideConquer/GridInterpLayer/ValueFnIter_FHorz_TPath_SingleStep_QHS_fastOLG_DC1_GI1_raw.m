function [V,Policy,Vhat]=ValueFnIter_FHorz_TPath_SingleStep_QHS_fastOLG_DC1_GI1_raw(V,n_d,n_a,n_z,N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% fastOLG just means parallelize over "age" (j)
% fastOLG is done as (a,j,z), rather than standard (a,z,j)
% V is (a,j)-by-z (carries Vunderbar for Sophisticated QH)
...
if vfoptions.lowmemory==0
    ...
    EVfine=reshape(DiffDiscountedEVinterp(aprimejz(:)),[N_d*n2long,N_a,N_j,N_z]); % (beta - beta0beta)*EVinterp values at fine grid
    ...
    linidx=reshape(maxindexL2,[1,N_a*N_j*N_z])+N_d*n2long*(0:N_a*N_j*N_z-1);
    EV_at_policy=reshape(EVfine(linidx),[N_a*N_j,N_z]);
...
elseif vfoptions.lowmemory==1
    ...
    for z_c=1:N_z
        ...
        EVfine_z=reshape(DiffDiscountedEVinterp_z(aprimej(:)),[N_d*n2long,N_a,N_j]);
        entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_z(aprimej(:)),[N_d*n2long,N_a,N_j]);
        [Vhat_z,maxindexL2]=max(entireRHS_L2,[],1);
        d_ind=rem(maxindexL2-1,N_d)+1;
        ...
        linidx_z=reshape(maxindexL2,[1,N_a*N_j])+N_d*n2long*(0:N_a*N_j-1);
        EV_at_policy_z=reshape(EVfine_z(linidx_z),[N_a*N_j,1]);
        Vhat(:,z_c)=reshape(Vhat_z,[N_a*N_j,1]);
        V(:,z_c)=Vhat(:,z_c)+EV_at_policy_z;
    end
end
