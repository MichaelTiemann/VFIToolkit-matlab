...
% fastOLG, so a-j-e
Policy=zeros(4,N_a,N_j,N_e,vfoptions.indexT,'gpuArray'); % first dim indexes the optimal choice for d and aprime (d, midpoint, L2, L2 flag)
...
DiffDiscountedEVinterp=reshape(betaminusbeta0beta_J,[1,1,N_j]).*EVinterp; % [n2aprime fine,1,j]
...
if vfoptions.lowmemory==0
    ...
    EVfine=reshape(DiffDiscountedEVinterp(aprimej(:)),[N_d*n2long,N_a,N_j,N_e]);
