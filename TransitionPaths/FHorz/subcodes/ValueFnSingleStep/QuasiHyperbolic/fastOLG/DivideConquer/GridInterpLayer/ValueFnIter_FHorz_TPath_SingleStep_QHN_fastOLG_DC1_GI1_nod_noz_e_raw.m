function [V, Policy, Policyalt, Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_fastOLG_DC1_GI1_nod_noz_e_raw(V,n_a,n_e,N_j, a_grid,e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
Policyalt=zeros(3,N_a,N_j,N_e,'gpuArray'); % exponential discounter optimal (midpoint, L2, L2 flag)
...
if vfoptions.lowmemory==0
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV_alt(aprimej(:)),[maxgap(ii)+1,level1iidiff(ii),N_j,N_e]);
    ...
    entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(aprimej(:)),[n2long,N_a,N_j,N_e]);
    ...
    for e_c=1:N_e
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                aprimej=repelem(aprimeindexes,1,level1iidiff(ii),1)+N_a*jind;
                entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV_alt(aprimej(:)),[maxgap(ii)+1,level1iidiff(ii),N_j]);
                [~,maxindex]=max(entireRHS_ii,[],1);
                midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
            else
                loweredge=maxindex1(1,ii,:);
        ...
        aprimej=aprimeindexes+n2aprime*jind;
        entireRHS_L2=ReturnMatrix_L2+reshape(DiscountedEVinterp_alt(aprimej(:)),[n2long,N_a,N_j]);
        [Vtempii,maxindexL2alt]=max(entireRHS_L2,[],1);
