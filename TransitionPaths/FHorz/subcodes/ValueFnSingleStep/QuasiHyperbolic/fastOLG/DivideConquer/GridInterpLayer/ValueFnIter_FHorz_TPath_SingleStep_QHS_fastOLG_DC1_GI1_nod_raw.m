function [V, Policy, Vhat]=ValueFnIter_FHorz_TPath_SingleStep_QHS_fastOLG_DC1_GI1_nod_raw(V,n_a,n_z,N_j, a_grid, z_gridvals_J,pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
% fastOLG, so a-j-z
Policy=zeros(3,N_a,N_j,N_z,vfoptions.indexT,'gpuArray'); % first dim indexes the optimal choice for aprime (midpoint, L2, L2 flag)
...
if vfoptions.lowmemory==0
    ...
    EVfine=reshape(DiffDiscountedEVinterp(aprimejz(:)),[n2long,N_a,N_j,N_z]);
    ...
    linidx=reshape(maxindexL2,[1,N_a*N_j*N_z])+n2long*(0:N_a*N_j*N_z-1);
...
elseif vfoptions.lowmemory==1
    ...
    for z_c=1:N_z
        ...
        EVfine_z=reshape(DiffDiscountedEVinterp_z(aprimej(:)),[n2long,N_a,N_j]);
        ...
        linidx_z=reshape(maxindexL2,[1,N_a*N_j])+n2long*(0:N_a*N_j-1);
