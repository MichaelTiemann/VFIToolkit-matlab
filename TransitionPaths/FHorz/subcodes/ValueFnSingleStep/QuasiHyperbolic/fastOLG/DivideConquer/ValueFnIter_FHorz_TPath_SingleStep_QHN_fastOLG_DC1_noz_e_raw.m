function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_fastOLG_DC1_noz_e_raw(V,n_d,n_a,n_e,N_j, d_gridvals, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
V=zeros(N_a,N_j,N_e,vfoptions.precision,'gpuArray'); % V is over (a,j,e); for Naive QH carries Valt

DiscountedEV=shiftdim(reshape(DiscountFactor_J,[1,1,N_j]).*reshape(EV,[N_a,1,N_j]),-1); % [1,aprime,1,j] beta_j*EV
...
if vfoptions.lowmemory==0
    ...
    entireRHS_ii=ReturnMatrix_ii+DiscountedEV; % (d,aprime,a and j,e), autofills a for expectation term
    ...
    for ii=1:(vfoptions.level1n-1)
        if maxgap_V(ii)>0
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV(aprime(:)),[N_d*(maxgap_V(ii)+1),1,N_j,N_e]);
            ...
            entireRHS_ii=ReturnMatrix_ii_dc+reshape(DiscountedEV(aprime(:)),[N_d,1,N_j,N_e]);
            [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
            V(level1ii(ii)+1:level1ii(ii+1)-1,:,:)=shiftdim(Vtempii,1);
            d_ind=rem(maxindexalt-1,N_d)+1;
            allind=d_ind+N_d*jBind+N_d*N_j*eBind;
    ...
    for e_c=1:N_e
        ...
        for ii=1:(vfoptions.level1n-1)
            if maxgap_V(ii)>0
                ...
                aprime=aprimeindexes+N_a*jind;
                entireRHS_ii=ReturnMatrix_ii_e_dc+reshape(DiscountedEV(aprime(:)),[N_d*(maxgap_V(ii)+1),1,N_j]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                ...
                aprime=loweredge+N_a*jind;
                entireRHS_ii=ReturnMatrix_ii_e_dc+reshape(DiscountedEV(aprime(:)),[N_d,1,N_j]);
                [Vtempii,maxindexalt]=max(entireRHS_ii,[],1);
                V(level1ii(ii)+1:level1ii(ii+1)-1,:,e_c)=shiftdim(Vtempii,1);
                d_ind=rem(maxindexalt-1,N_d)+1;
                allind=d_ind+N_d*jBind;
