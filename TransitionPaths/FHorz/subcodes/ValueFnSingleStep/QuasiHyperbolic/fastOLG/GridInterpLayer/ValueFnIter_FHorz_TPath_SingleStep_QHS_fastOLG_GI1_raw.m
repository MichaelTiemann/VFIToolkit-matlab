function [V,Policy,Vhat]=ValueFnIter_FHorz_TPath_SingleStep_QHS_fastOLG_GI1_raw(V,n_d,n_a,n_z,N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% fastOLG just means parallelize over "age" (j)
% fastOLG is done as (a,j,z), rather than standard (a,z,j)
% V is (a,j)-by-z (V carries Vunderbar for Sophisticated)
% Policy is (a,j,z)
...
EVinterp_d=repelem(shiftdim(EVinterp,-1),N_d,1,1,1); % [d,aprime,1,j,z], undiscounted, for Vunderbar correction
...
if vfoptions.lowmemory==0
    ...
    EVfine=reshape(EVinterp_d(daprimej(:)),[N_d*n2long,N_a,N_j,N_z]);
    ...
    Vhat=reshape(Vhatii,[N_a*N_j,N_z]); % snapshot pre-Vunderbar
...
elseif vfoptions.lowmemory==1
    ...
    for z_c=1:N_z
        ...
        EVfine_z=reshape(EVinterp_d_z(daprimej(:)),[N_d*n2long,N_a,N_j]);
        ...
        EV_at_policy_z=reshape(EVfine_z(linidx_z),[N_a,N_j]);
        Vtemp=shiftdim(Vhatii,1)+reshape(beta_J-beta0beta_J,[1,N_j]).*EV_at_policy_z;
        V(:,z_c)=reshape(Vtemp,[N_a*N_j,1]);
        Vhat(:,z_c)=reshape(Vhatii,[N_a*N_j,1]); % snapshot pre-Vunderbar
    end
end
