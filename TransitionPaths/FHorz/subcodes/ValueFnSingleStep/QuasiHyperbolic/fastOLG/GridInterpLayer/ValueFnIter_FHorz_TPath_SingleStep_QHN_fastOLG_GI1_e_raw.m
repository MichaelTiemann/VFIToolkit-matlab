function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_fastOLG_GI1_e_raw(V,n_d,n_a,n_z,n_e,N_j, d_gridvals, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
Policy=zeros(4,N_a,N_j,N_z,N_e,vfoptions.indexT,'gpuArray'); %first dim indexes the optimal choice for d and aprime (d, midpoint, aprimeL2ind, L2flag)
Policyalt=zeros(4,N_a,N_j,N_z,N_e,'gpuArray');
...
if vfoptions.lowmemory==0
    ...
    entireRHS_alt=ReturnMatrix+DiscountedEV_alt; %  [d,aprime,a,j,z,e]
    ...
    ReturnMatrix_iialt=CreateReturnFnMatrix_fastOLG_Disc_DC1_e(ReturnFn,n_d,n_z,n_e,N_j,d_gridvals,aprime_grid(aprimeindexesalt),a_grid, z_gridvals_J,e_gridvals_J, ReturnFnParamsAgeMatrix,2);
    daprimejalt=(1:1:N_d)'+N_d*(aprimeindexesalt-1)+N_d*n2aprime*jind+N_d*n2aprime*N_j*zind;
    entireRHS_iialt=ReturnMatrix_iialt+reshape(DiscountedEVinterp_alt(daprimejalt(:)),[N_d*n2long,N_a,N_j,N_z,N_e]);
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        ReturnMatrix_iialt=CreateReturnFnMatrix_fastOLG_Disc_DC1_e(ReturnFn,n_d,n_z,special_n_e,N_j,d_gridvals,aprime_grid(aprimeindexesalt),a_grid,z_gridvals_J,e_vals,ReturnFnParamsAgeMatrix,2);
        daprimejalt=(1:1:N_d)'+N_d*(aprimeindexesalt-1)+N_d*n2aprime*jind+N_d*n2aprime*N_j*zind;
        ...
        Vtilde(:,:,e_c)=reshape(Vtildetemp,[N_a*N_j,N_z]);
        d_ind=rem(maxindexL2-1,N_d)+1;
...
elseif vfoptions.lowmemory==2
    ...
    for z_c=1:N_z
        ...
        for e_c=1:N_e
            ...
            %% Valt-step (beta)
            entireRHS_alt_ze=ReturnMatrix_ze+DiscountedEV_alt_z;
            [~,maxindex1alt]=max(entireRHS_alt_ze,[],2);
            midpointalt=max(min(maxindex1alt,n_a-1),2);
            aprimeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_iialt=CreateReturnFnMatrix_fastOLG_Disc_DC1_e(ReturnFn,n_d,special_n_z,special_n_e,N_j,d_gridvals,aprime_grid(aprimeindexesalt),a_grid,z_vals,e_vals,ReturnFnParamsAgeMatrix,2);
            ...
            Policyalt(2,:,:,z_c,e_c)=shiftdim(squeeze(midpointalt(allindalt)),-1);
            ...
            [Vtildetemp,maxindexL2]=max(entireRHS_ii,[],1);
            Vtilde(:,z_c,e_c)=reshape(Vtildetemp,[N_a*N_j,1]);
            d_ind=rem(maxindexL2-1,N_d)+1;
