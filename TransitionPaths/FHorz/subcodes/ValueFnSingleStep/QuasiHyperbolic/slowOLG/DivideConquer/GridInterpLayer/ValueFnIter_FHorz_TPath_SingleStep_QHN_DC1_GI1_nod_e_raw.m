function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_DC1_GI1_nod_e_raw(V,n_a,n_z,n_e,N_j, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
Policy=zeros(3,N_a,N_z,N_e,N_j,vfoptions.indexT,'gpuArray'); % [midpoint; aprimeL2ind; L2flag]
...
if vfoptions.lowmemory>=3
    error('vfoptions.lowmemory=K not supported for ValueFnIter_FHorz_TPath_SingleStep_QHN_DC1_GI1_nod_e_raw')
end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                inLowerStrictalt = (maxindexL2alt >= 2)         & (maxindexL2alt <= n2short+1);
                inUpperStrictalt = (maxindexL2alt >= n2short+3) & (maxindexL2alt <= n2long-1);
                Policyalt(3,:,z_c,e_c,jj) = shiftdim(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt),-1);
                %% Policy (beta0*beta) — QH choice
                ...
                [Vtildeii,maxindexL2]=max(entireRHS_L2,[],1);
                Vtilde(:,z_c,e_c,jj)=shiftdim(Vtildeii,1);
