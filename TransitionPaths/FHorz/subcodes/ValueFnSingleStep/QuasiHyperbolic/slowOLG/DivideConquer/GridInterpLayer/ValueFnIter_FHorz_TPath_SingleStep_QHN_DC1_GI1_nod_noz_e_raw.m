function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_DC1_GI1_nod_noz_e_raw(V,n_a,n_e,N_j, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
Policy=zeros(3,N_a,N_e,N_j,vfoptions.indexT,'gpuArray'); % [midpoint; aprimeL2ind; L2flag]
...
if vfoptions.lowmemory>=2
    error('vfoptions.lowmemory=K not supported for ValueFnIter_FHorz_TPath_SingleStep_QHN_DC1_GI1_nod_noz_e_raw')
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_dc+beta*EV(reshape(aprimeindexes(:),[maxgap_V(ii)+1,1,N_e]));
        ...
        entireRHS_L2=ReturnMatrix_L2+beta*reshape(EVinterp(aprimeindexes(:)),[n2long,N_a,N_e]);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                aprimeindexes=loweredge+(0:1:maxgap(ii))';
                ReturnMatrix_ii_dc=CreateReturnFnMatrix_Disc_DC1_nod(ReturnFn, n_e, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                entireRHS_ii=ReturnMatrix_ii_dc+beta0beta*EV(reshape(aprimeindexes(:),[maxgap(ii)+1,1,N_e]));
                [~,maxindex]=max(entireRHS_ii,[],1);
                midpoints_jj(1,curraindex,:)=maxindex+(loweredge-1);
            else
                loweredge=maxindex1(1,ii,:);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            inLowerStrictalt = (maxindexL2alt >= 2)         & (maxindexL2alt <= n2short+1);
            inUpperStrictalt = (maxindexL2alt >= n2short+3) & (maxindexL2alt <= n2long-1);
            Policyalt(3,:,e_c,jj) = shiftdim(2 + (inLowerStrictalt & isInfLoweralt) - (inUpperStrictalt & isInfUpperalt),-1);
            %% Policy (beta0*beta) — QH choice
            entireRHS_ii=ReturnMatrix_ii+beta0beta*EV;
            [~,maxindex1]=max(entireRHS_ii,[],1);
            midpoints_jj(1,level1ii)=maxindex1;
            ...
            [Vtildeii,maxindexL2]=max(entireRHS_L2,[],1);
            Vtilde(:,e_c,jj)=shiftdim(Vtildeii,1);
