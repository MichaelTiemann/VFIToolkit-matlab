function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_nod_raw(V,n_a,n_z,N_j, a_grid,z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        % First Valt
        entireRHS_alt=ReturnMatrix+beta*EV.*ones(1,N_a,1); % Use the two-future-periods discount factor
        [Vtemp,maxindex_alt]=max(entireRHS_alt,[],1);
        V(:,:,jj)=shiftdim(Vtemp,1);
        Policyalt(:,:,jj)=shiftdim(maxindex_alt,1);
        % Now Policy
        entireRHS=ReturnMatrix+beta0beta*EV.*ones(1,N_a,1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            % First Valt
            entireRHS_alt_z=ReturnMatrix_z+beta*EV_z;
