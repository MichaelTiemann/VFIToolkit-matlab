function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_e_raw(V,n_d,n_a,n_z,n_e,N_j, d_gridvals, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if vfoptions.lowmemory>=3
    error('vfoptions.lowmemory=K not supported for ValueFnIter_FHorz_TPath_SingleStep_QHN_e_raw')
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        % First Valt
        entireRHS_alt=ReturnMatrix+beta*repmat(entireEV,1,N_a,1,N_e); % Use the two-future-periods discount factor
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            % First Valt
            entireRHS_alt_e=ReturnMatrix_e+beta*entireEV; %.*ones(1,N_a,1);
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                % First Valt
                entireRHS_alt_ze=ReturnMatrix_ze+beta*entireEV_z; %*ones(1,N_a,1);
