function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_GI1_e_raw(V,n_d,n_a,n_z,n_e,N_j, d_gridvals, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if vfoptions.lowmemory>=3
    error('vfoptions.lowmemory>=3 not supported for ValueFnIter_FHorz_TPath_SingleStep_QHN_GI1_e_raw')
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_L2alt=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn,n_d,n_z,n_e,d_gridvals,aprime_grid(aprimeindexesalt),a_grid,z_gridvals_J(:,:,jj),e_gridvals_J(:,:,jj),ReturnFnParamsVec,2);
        aprimezalt=aprimeindexesalt+n2aprime*zBind;
        entireRHS_L2alt=ReturnMatrix_L2alt+beta*reshape(EVinterp(aprimezalt(:)),[N_d*n2long,N_a,N_z,N_e]);
        [Vtempii,maxindexL2alt]=max(entireRHS_L2alt,[],1);

        d_indalt = rem(maxindexL2alt-1,N_d)+1;
        L2offsetalt = ceil(maxindexL2alt/N_d);
        linidx_loweralt = d_indalt + N_d*n2long*aind + N_d*n2long*N_a*zind + N_d*n2long*N_a*N_z*eind;
        linidx_upperalt = d_indalt + N_d*(n2long-1) + N_d*n2long*aind + N_d*n2long*N_a*zind + N_d*n2long*N_a*N_z*eind;
        ...
        allindalt=d_indalt+N_d*aind+N_d*N_a*zind+N_d*N_a*N_z*eind;
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            [~,maxindexalt]=max(entireRHS_e,[],2);
            midpointalt=max(min(maxindexalt,n_a-1),2);
            aprimeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_L2alt_e=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn,n_d,n_z,special_n_e,d_gridvals,aprime_grid(aprimeindexesalt),a_grid,z_gridvals_J(:,:,jj),e_val,ReturnFnParamsVec,2);
            aprimezalt=aprimeindexesalt+n2aprime*zBind;
            entireRHS_L2alt_e=ReturnMatrix_L2alt_e+beta*reshape(EVinterp(aprimezalt(:)),[N_d*n2long,N_a,N_z]);
            [Vtempii,maxindexL2alt]=max(entireRHS_L2alt_e,[],1);
            ...
            isInfLoweralt = (ReturnMatrix_L2alt_e(linidx_loweralt) == -Inf);
            isInfUpperalt = (ReturnMatrix_L2alt_e(linidx_upperalt) == -Inf);
            inLowerStrictalt = (L2offsetalt >= 2)         & (L2offsetalt <= n2short+1);
            inUpperStrictalt = (L2offsetalt >= n2short+3) & (L2offsetalt <= n2long-1);
            ...
            Policyalt(2,:,:,e_c,jj)=shiftdim(squeeze(midpointalt(allindalt)),-1);
            Policyalt(3,:,:,e_c,jj)=shiftdim(ceil(maxindexL2alt/N_d),-1);
            ...
            midpoint=max(min(maxindex,n_a-1),2);
            aprimeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_L2_e=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn,n_d,n_z,special_n_e,d_gridvals,aprime_grid(aprimeindexes),a_grid,z_gridvals_J(:,:,jj),e_val,ReturnFnParamsVec,2);
            aprimez=aprimeindexes+n2aprime*zBind;
            entireRHS_L2_e=ReturnMatrix_L2_e+beta0beta*reshape(EVinterp(aprimez(:)),[N_d*n2long,N_a,N_z]);
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                %% Valt (beta)
                entireRHS_ze=ReturnMatrix_ze+beta*shiftdim(EV_z,-1);
                [~,maxindexalt]=max(entireRHS_ze,[],2);
                midpointalt=max(min(maxindexalt,n_a-1),2);
                aprimeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_L2alt_ze=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn,n_d,special_n_z,special_n_e,d_gridvals,aprime_grid(aprimeindexesalt),a_grid,z_val,e_val,ReturnFnParamsVec,2);
                entireRHS_L2alt_ze=ReturnMatrix_L2alt_ze+beta*reshape(EVinterp_z(aprimeindexesalt(:)),[N_d*n2long,N_a]);
                [Vtempii,maxindexL2alt]=max(entireRHS_L2alt_ze,[],1);

                d_indalt = rem(maxindexL2alt-1,N_d)+1;
                L2offsetalt = ceil(maxindexL2alt/N_d);
                linidx_loweralt = d_indalt + N_d*n2long*aind;
                linidx_upperalt = d_indalt + N_d*(n2long-1) + N_d*n2long*aind;
                isInfLoweralt = (ReturnMatrix_L2alt_ze(linidx_loweralt) == -Inf);
                isInfUpperalt = (ReturnMatrix_L2alt_ze(linidx_upperalt) == -Inf);
                inLowerStrictalt = (L2offsetalt >= 2)         & (L2offsetalt <= n2short+1);
                inUpperStrictalt = (L2offsetalt >= n2short+3) & (L2offsetalt <= n2long-1);
                ...
                Policyalt(2,:,z_c,e_c,jj)=shiftdim(squeeze(midpointalt(allindalt)),-1);
                Policyalt(3,:,z_c,e_c,jj)=shiftdim(ceil(maxindexL2alt/N_d),-1);

                %% Vtilde (beta0*beta)
                entireRHS_ze=ReturnMatrix_ze+beta0beta*shiftdim(EV_z,-1);
                [~,maxindex]=max(entireRHS_ze,[],2);
                midpoint=max(min(maxindex,n_a-1),2);
                aprimeindexes=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
                ReturnMatrix_L2_ze=CreateReturnFnMatrix_Disc_DC1_e(ReturnFn,n_d,special_n_z,special_n_e,d_gridvals,aprime_grid(aprimeindexes),a_grid,z_val,e_val,ReturnFnParamsVec,2);
                entireRHS_L2_ze=ReturnMatrix_L2_ze+beta0beta*reshape(EVinterp_z(aprimeindexes(:)),[N_d*n2long,N_a]);
                [Vtempii_qh,maxindexL2]=max(entireRHS_L2_ze,[],1);

                d_ind = rem(maxindexL2-1,N_d)+1;
                L2offset = ceil(maxindexL2/N_d);
                linidx_lower = d_ind + N_d*n2long*aind;
                linidx_upper = d_ind + N_d*(n2long-1) + N_d*n2long*aind;
                isInfLower = (ReturnMatrix_L2_ze(linidx_lower) == -Inf);
                isInfUpper = (ReturnMatrix_L2_ze(linidx_upper) == -Inf);
                inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
                inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
                PolicyL2flag(1,:,z_c,e_c,jj) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);

                Vtilde(:,z_c,e_c,jj)=shiftdim(Vtempii_qh,1);
                allind=d_ind+N_d*aind;
