function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_GI1_noz_e_raw(V,n_d,n_a,n_e,N_j, d_gridvals, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
elseif vfoptions.lowmemory>=2
    error('vfoptions.lowmemory>=2 not supported for ValueFnIter_FHorz_TPath_SingleStep_QHN_GI1_noz_e_raw')
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_L2alt=CreateReturnFnMatrix_Disc_DC1(ReturnFn,n_d,n_e,d_gridvals,aprime_grid(aprimeindexesalt),a_grid,e_gridvals_J(:,:,jj),ReturnFnParamsVec,2);
        entireRHS_L2alt=ReturnMatrix_L2alt+beta*reshape(EVinterp(aprimeindexesalt(:)),[N_d*n2long,N_a,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            midpointalt=max(min(maxindexalt,n_a-1),2);
            aprimeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short);
            ReturnMatrix_L2alt_e=CreateReturnFnMatrix_Disc_DC1(ReturnFn,n_d,special_n_e,d_gridvals,aprime_grid(aprimeindexesalt),a_grid,e_val,ReturnFnParamsVec,2);
            entireRHS_L2alt_e=ReturnMatrix_L2alt_e+beta*reshape(EVinterp(aprimeindexesalt(:)),[N_d*n2long,N_a]);
            ...
            entireRHS_L2_e=ReturnMatrix_L2_e+beta0beta*reshape(EVinterp(aprimeindexes(:)),[N_d*n2long,N_a]);
