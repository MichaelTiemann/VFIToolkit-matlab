function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_GI1_nod_noz_e_raw(V,n_a,n_e,N_j, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_L2alt=CreateReturnFnMatrix_Disc_DC1_nod(ReturnFn,n_e,aprime_grid(aprimeindexesalt),a_grid,e_gridvals_J(:,:,jj),ReturnFnParamsVec,2);
        entireRHS_L2alt=ReturnMatrix_L2alt+beta*reshape(EVinterp(aprimeindexesalt(:)),[n2long,N_a,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            midpointalt=max(min(maxindexalt,n_a-1),2);
            aprimeindexesalt=(midpointalt+(midpointalt-1)*n2short)+(-n2short-1:1:1+n2short)';
            ReturnMatrix_L2alt_e=CreateReturnFnMatrix_Disc_DC1_nod(ReturnFn,special_n_e,aprime_grid(aprimeindexesalt),a_grid,e_val,ReturnFnParamsVec,2);
            entireRHS_L2alt_e=ReturnMatrix_L2alt_e+beta*reshape(EVinterp(aprimeindexesalt(:)),[n2long,N_a]);
