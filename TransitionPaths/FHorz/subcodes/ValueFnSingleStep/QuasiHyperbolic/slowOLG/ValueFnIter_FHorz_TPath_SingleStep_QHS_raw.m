function [V,Policy,Vhat]=ValueFnIter_FHorz_TPath_SingleStep_QHS_raw(V,n_d,n_a,n_z,N_j, d_gridvals, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
elseif vfoptions.lowmemory>=2
    error('vfoptions.lowmemory=K not supported for ValueFnIter_FHorz_TPath_SingleStep_QHS_raw')
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        % For sophisticated we compute Policy (which is Policyhat)
        % and then we compute Vunderbar.
        % First Policy
        entireRHS=ReturnMatrix+beta0beta*repmat(entireEV,1,N_a,1); % Use the today-to-tomorrow discount factor
        ...
        % Now Vunderbar
        entireRHS=ReturnMatrix+beta*repmat(entireEV,1,N_a,1); % Use the two-future-periods discount factor
        maxindexfull=maxindex+N_d*N_a*(0:1:N_a-1)+shiftdim(N_d*N_a*N_a*(0:1:N_z-1),-1);
        V(:,:,jj)=entireRHS(maxindexfull); % Evaluate time-inconsistent policy using two-future-periods discount rate
