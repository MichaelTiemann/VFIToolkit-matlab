function [V,Policy,Policyalt,Vtilde]=ValueFnIter_FHorz_TPath_SingleStep_QHN_nod_noz_raw(V,n_a,N_j, a_grid, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
for reverse_j=1:N_j-1
    ...
    % First Valt
    entireRHS_alt=ReturnMatrix+beta*VKronNext_j*ones(1,N_a,1);
    [Vtemp,maxindex_alt]=max(entireRHS_alt,[],1);
    V(:,jj)=Vtemp;
    Policyalt(:,jj)=maxindex_alt;
    % Now Policy
    entireRHS=ReturnMatrix+beta0beta*VKronNext_j*ones(1,N_a,1);
    [Vtilde_tmp,maxindex]=max(entireRHS,[],1);
    Vtilde(:,jj)=Vtilde_tmp;
    Policy(:,jj)=maxindex;
end
