function [V,Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_SemiExo_e_raw(V,n_d1,n_d2,n_a,n_z,n_semiz,n_e,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
DiscountedEV=repelem(DiscountedEV,N_d1,1,1,1); % [(d1,d2) & aprime,1,j,bothz], d1 indexes fastest, then d2, then aprime
...
% Policy contains the joint (d1,d2,aprime) index: d1 indexes fastest, then d2, then aprime
