function [V,Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_SemiExo_GI1_raw(V,n_d1,n_d2,n_a,n_z,n_semiz,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, pi_z_J, pi_semiz_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
% V=zeros(N_a*N_j,N_bothz,'gpuArray');
Policy=zeros(4,N_a,N_j,N_bothz,'gpuArray'); %first dim indexes the optimal choice for d and aprime rest of dimensions a,z (channels: d, midpoint, aprimeL2ind, L2flag)
...
if vfoptions.lowmemory==0
    ...
    ReturnMatrix=CreateReturnFnMatrix_fastOLG_Disc_DC1(ReturnFn, n_d, n_bothz, N_j, d_gridvals, a_grid, a_grid, bothz_gridvals_J, ReturnFnParamsAgeMatrix,1);
...
elseif vfoptions.lowmemory==1
    ...
    for z_c=1:N_bothz
        bothz_vals=bothz_gridvals_J(1,1,1,:,z_c,:); % bothz_gridvals_J has shape (j,prod(n_bothz),l_z) for fastOLG
        DiscountedEV_z=DiscountedEV(:,:,:,:,z_c);
        DiscountedEVinterp_z=DiscountedEVinterp(:,:,:,:,z_c);

        ReturnMatrix_z=CreateReturnFnMatrix_fastOLG_Disc_DC1(ReturnFn, n_d, special_n_bothz, N_j, d_gridvals, a_grid, a_grid, bothz_vals, ReturnFnParamsAgeMatrix,1);
