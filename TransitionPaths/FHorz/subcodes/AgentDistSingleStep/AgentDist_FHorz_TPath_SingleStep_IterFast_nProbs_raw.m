function AgentDist=AgentDist_FHorz_TPath_SingleStep_IterFast_nProbs_raw(AgentDist,Policy_aprimejz,PolicyProbs,N_a,N_z,N_j,pi_z_J_sim,II,exceptlastj,exceptfirstj,justfirstj,jequalOneDist)
...
% Policy_aprimejz is [N_a*(N_j-1)*N_z,N_probs], already except last j
% PolicyProbs is [N_a*(N_j-1)*N_z,N_probs], already except last j

% precomputed:
% II=repelem((1:1:N_a*(N_j-1)*N_z)',1,N_probs);
% policyexceptlastj=repmat((1:1:N_a)',(N_j-1)*N_z*N_probs,1)+repmat(repelem(N_a*(0:1:N_j-2)',N_a,1),N_z*N_probs,1)+repelem(N_a*N_j*(0:1:N_z*N_probs-1)',N_a*(N_j-1),1);
...
Gammatranspose=sparse(gather(Policy_aprimejz),II,gather(PolicyProbs),N_a*(N_j-1)*N_z,N_a*(N_j-1)*N_z);
% Note: N_j-1, not N_j
