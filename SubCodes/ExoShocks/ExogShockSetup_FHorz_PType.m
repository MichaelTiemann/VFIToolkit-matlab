...
if zdependsonptype==0
    ...
    if prod(n_z)==0
        ...
        elseif gridpiboth==2 % For agent dist, we don't use grid
            ...
            if isfield(options,'ExogShockFn')
                pi_z_J=zeros(prod(n_z),prod(n_z),N_jpiz,options.precision,'gpuArray');
                for jj=1:N_jpiz
                    ExogShockFnParamsVec=CreateVectorFromParams(Parameters, options.ExogShockFnParamNames,jj,options.precision);
                    ExogShockFnParamsCell=cell(length(ExogShockFnParamsVec),1);
                    for ii=1:length(ExogShockFnParamsVec)
                ...
                if ndims(pi_z)==2 % age-independent: broadcast
                    pi_z_J=pi_z.*ones(1,1,N_jpiz,options.precision,'gpuArray');
    ...
    for ii=1:length(Names_i)
        ...
        if prod(n_z_temp)==0
            ...
            if gridpiboth==1 % for most FnsToEvaluate, we don't use pi_z
                pi_z_J.(Names_i{ii})=[];
                % Now just do z_gridvals_J
                z_gridvals_J_temp=zeros(prod(n_z_temp),length(n_z_temp),N_j_temp,options.precision,'gpuArray');
                if isfield(options,'ExogShockFn')
                    for jj=1:N_j_temp
                ...
                elseif all(size(z_grid_temp)==[prod(n_z_temp),length(n_z_temp)]) % joint grid
                    z_gridvals_J_temp=z_grid_temp.*ones(1,1,N_j_temp,options.precision,'gpuArray');
            ...
            elseif gridpiboth==2 % For agent dist, we don't use grid
                ...
                if isfield(options,'ExogShockFn')
                    pi_z_J_temp=zeros(prod(n_z_temp),prod(n_z_temp),N_jpiz_temp,options.precision,'gpuArray');
                    ...
                    if ndims(pi_z_temp)==2 % age-independent: broadcast
                        pi_z_J_temp=pi_z_temp.*ones(1,1,N_jpiz_temp,options.precision,'gpuArray');
            ...
            elseif gridpiboth==3
                ...
                if isfield(options,'ExogShockFn')
                    ...
                    elseif all(size(z_grid_temp)==[prod(n_z_temp),length(n_z_temp)]) % joint grid
                        z_gridvals_J_temp=z_grid_temp.*ones(1,1,N_j_temp,options.precision,'gpuArray');
                    elseif all(size(z_grid_temp)==[sum(n_z_temp),1]) % basic grid
                        z_gridvals_J_temp=CreateGridvals(n_z_temp,z_grid_temp,1).*ones(1,1,N_j_temp,options.precision,'gpuArray');
                    else
                        error('z_grid_temp size does not match any expected shape')
                    end
                end
            ...
            if gridpiboth==1 % for most FnsToEvaluate, we don't use pi_z
                ...
                elseif ndims(z_grid_temp)==3
                    ...
                    elseif all(size(z_grid_temp)==[prod(n_z_temp),length(n_z_temp),N_i]) % joint-grid, depend on ptype
                        z_gridvals_J_temp=z_grid_temp(:,:,ii).*ones(1,1,N_j_temp,options.precision,'gpuArray');
                    else
            ...
            elseif gridpiboth==2 % For agent dist, we don't use grid
                ...
                elseif size(pi_z_temp,ndims(pi_z_temp))==N_i
                    ...
                    if ndims(pi_z_temp_ii)==2 % age-independent: broadcast
                        pi_z_J_temp=pi_z_temp_ii.*ones(1,1,N_jpiz_temp,options.precision,'gpuArray');
...
if edependsonptype==0
    if prod(n_e)==0
        ...
        elseif gridpiboth==2 % For agent dist, we don't use grid
            ...
            if isfield(options,'EiidShockFn')
                options.pi_e_J=zeros(prod(options.n_e),N_jpie,options.precision,'gpuArray');
                for jj=1:N_j
                    EiidShockFnParamsVec=CreateVectorFromParams(Parameters, options.EiidShockFnParamNames,jj,options.precision);
                    EiidShockFnParamsCell=cell(length(EiidShockFnParamsVec),1);
                    for ii=1:length(EiidShockFnParamsVec)
                ...
                if size(options.pi_e,2)==1 % age-independent: broadcast
                    options.pi_e_J=options.pi_e.*ones(1,N_jpie,options.precision,'gpuArray');
    ...
    for ii=1:length(Names_i)
        ...
        if prod(n_e_temp)==0
            ...
            if gridpiboth==1 % for most FnsToEvaluate, we don't use pi_z
                options.pi_e_J.(Names_i{ii})=[];
                % Now just do e_gridvals_J
                e_gridvals_J_temp=zeros(prod(n_e_temp),length(n_e_temp),N_j_temp,options.precision,'gpuArray');
                if isfield(options,'EiidShockFn')
                    for jj=1:N_j_temp
                ...
                elseif all(size(e_grid_temp)==[prod(n_e_temp),length(n_e_temp)]) % joint grid
                    e_gridvals_J_temp=e_grid_temp.*ones(1,1,N_j_temp,options.precision,'gpuArray');
                elseif all(size(e_grid_temp)==[sum(n_e_temp),1]) % basic grid
                    e_gridvals_J_temp=CreateGridvals(n_e_temp,e_grid_temp,1).*ones(1,1,N_j_temp,options.precision,'gpuArray');
                else
            ...
            elseif gridpiboth==2 % For agent dist, we don't use grid
                ...
                if isfield(options,'EiidShockFn')
                    pi_e_J_temp=zeros(prod(n_e_temp),N_jpie_temp,options.precision,'gpuArray');
                    for jj=1:N_j_temp
                        EiidShockFnParamsVec=CreateVectorFromParams(Parameters, options.EiidShockFnParamNames.(Names_i{ii}),jj,options.precision);
                        EiidShockFnParamsCell=cell(length(EiidShockFnParamsVec),1);
                        for pp=1:length(EiidShockFnParamsVec)
                    ...
                    if size(pi_e_temp,2)==1 % age-independent: broadcast
                        pi_e_J_temp=pi_e_temp.*ones(1,N_jpie_temp,options.precision,'gpuArray');
            ...
            if gridpiboth==1 % for most FnsToEvaluate, we don't use pi_e
                ...
                elseif ndims(e_grid_temp)==3
                    ...
                    elseif all(size(e_grid_temp)==[prod(n_e_temp),length(n_e_temp),N_i])
                        e_gridvals_J_temp=e_grid_temp(:,:,ii).*ones(1,1,N_j_temp,options.precision,'gpuArray');
                    else
            ...
            elseif gridpiboth==2 % For agent dist, we don't use grid
                ...
                elseif size(pi_e_temp,ndims(pi_e_temp))==N_i
                    ...
                    if size(pi_e_temp_ii,2)==1 % age-independent: broadcast
                        pi_e_J_temp=pi_e_temp_ii.*ones(1,N_jpie_temp,options.precision,'gpuArray');
