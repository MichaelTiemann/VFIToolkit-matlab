function Policy=UnKronPolicyIndexes2_FHorz_z(PolicyKron, n_daprime1, n_daprime2, n_a, n_z, N_j, vfoptions)
% For models with z, two daprime dimensions, finite horizon.
% Can input vfoptions OR simoptions
% Input: PolicyKron=zeros(2,N_a,N_z,N_j);
%        PolicyKron(1,:,:,:) indexes the optimal choice for daprime1
%        PolicyKron(2,:,:,:) indexes the optimal choice for daprime2
%        If vfoptions.gridinterplayer==1, PolicyKron is (4,N_a,N_z,N_j): rows 1-2 are the
%        Kron indices, rows 3 and 4 are L2 and L2flag (passed through unchanged).
% Output: Policy is (l_daprime1+l_daprime2,n_a,n_z,N_j);
