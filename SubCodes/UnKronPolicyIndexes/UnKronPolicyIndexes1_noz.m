function Policy=UnKronPolicyIndexes1_noz(PolicyKron, n_daprime1, n_a, vfoptions)
% For models with no z (and no e, no semiz), single daprime dimension.
% Can input vfoptions OR simoptions
% Input: PolicyKron=zeros(1,N_a); % indexes the optimal choice for daprime1
%        If vfoptions.gridinterplayer==1, PolicyKron is (3,N_a): row 1 is the
%        Kron index, rows 2 and 3 are L2 and L2flag (passed through unchanged).
% Output: Policy is (l_daprime1,n_a);
%         If vfoptions.gridinterplayer==1, Policy is (l_daprime1+2,n_a).
...
if vfoptions.gridinterplayer==1
    Policy=[mod(floor(double(PolicyKron(1,:)-1)./divisors),n_daprime1(:))+1;
            PolicyKron(2,:);
            PolicyKron(3,:)];
    Policy=reshape(Policy,[l_daprime1+2,n_a]);
else
    Policy=mod(floor(double(PolicyKron-1)./divisors),n_daprime1(:))+1;
    Policy=reshape(Policy,[l_daprime1,n_a]);
end
