function Policy=UnKronPolicyIndexes2_z_e(PolicyKron, n_daprime1, n_daprime2, n_a, n_z, n_e, vfoptions)
% For models with z and e, two daprime dimensions.
% Can input vfoptions OR simoptions
% Input: PolicyKron=zeros(2,N_a,N_z,N_e);
%        PolicyKron(1,:,:,:) indexes the optimal choice for daprime1
%        PolicyKron(2,:,:,:) indexes the optimal choice for daprime2
%        If vfoptions.gridinterplayer==1, PolicyKron is (4,N_a,N_z,N_e): rows 1-2 are the
%        Kron indices, rows 3 and 4 are L2 and L2flag (passed through unchanged).
% Output: Policy is (l_daprime1+l_daprime2,n_a,n_z,n_e);
%         If vfoptions.gridinterplayer==1, Policy is (l_daprime1+l_daprime2+2,n_a,n_z,n_e).
...
if vfoptions.gridinterplayer==1
    Policy=[mod(floor(double(PolicyKron(1,:)-1)./divisors1),n_daprime1(:))+1;
            ...
            PolicyKron(3,:);
            PolicyKron(4,:)];
    Policy=reshape(Policy,[l_daprime1+l_daprime2+2,n_a,n_z,n_e]);
else
    Policy=[mod(floor(double(PolicyKron(1,:)-1)./divisors1),n_daprime1(:))+1;
            mod(floor(double(PolicyKron(2,:)-1)./divisors2),n_daprime2(:))+1];
    Policy=reshape(Policy,[l_daprime1+l_daprime2,n_a,n_z,n_e]);
end
