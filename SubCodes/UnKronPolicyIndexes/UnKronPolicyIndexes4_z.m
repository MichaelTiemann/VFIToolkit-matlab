function Policy=UnKronPolicyIndexes4_z(PolicyKron, n_daprime1, n_daprime2, n_daprime3, n_daprime4, n_a, n_z, vfoptions)
% For models with z, four daprime dimensions.
% Can input vfoptions OR simoptions
% Input: PolicyKron=zeros(4,N_a,N_z);
%        PolicyKron(k,:,:) indexes the optimal choice for daprime k (k=1,2,3,4)
%        If vfoptions.gridinterplayer==1, PolicyKron is (6,N_a,N_z): rows 1-4 are the
%        Kron indices, rows 5 and 6 are L2 and L2flag (passed through unchanged).
% Output: Policy is (l_daprime1+l_daprime2+l_daprime3+l_daprime4,n_a,n_z);
%         If vfoptions.gridinterplayer==1, Policy is (l_daprime1+l_daprime2+l_daprime3+l_daprime4+2,n_a,n_z).
...
if vfoptions.gridinterplayer==1
    Policy=[mod(floor(double(PolicyKron(1,:)-1)./divisors1),n_daprime1(:))+1;
            ...
            PolicyKron(5,:);
            PolicyKron(6,:)];
    Policy=reshape(Policy,[l_daprime1+l_daprime2+l_daprime3+l_daprime4+2,n_a,n_z]);
else
    Policy=[mod(floor(double(PolicyKron(1,:)-1)./divisors1),n_daprime1(:))+1;
            mod(floor(double(PolicyKron(2,:)-1)./divisors2),n_daprime2(:))+1;
            mod(floor(double(PolicyKron(3,:)-1)./divisors3),n_daprime3(:))+1;
            mod(floor(double(PolicyKron(4,:)-1)./divisors4),n_daprime4(:))+1];
    Policy=reshape(Policy,[l_daprime1+l_daprime2+l_daprime3+l_daprime4,n_a,n_z]);
end
