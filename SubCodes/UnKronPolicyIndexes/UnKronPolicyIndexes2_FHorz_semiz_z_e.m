function Policy=UnKronPolicyIndexes2_FHorz_semiz_z_e(PolicyKron, n_daprime1, n_daprime2, n_a, n_semiz, n_z, n_e, N_j, vfoptions)
% For models with semiz, z and e, two daprime dimensions, finite horizon.
% Can input vfoptions OR simoptions
% Input: PolicyKron=zeros(2,N_a,N_semiz,N_z,N_e,N_j);
%        PolicyKron(1,:,:,:,:,:) indexes the optimal choice for daprime1
%        PolicyKron(2,:,:,:,:,:) indexes the optimal choice for daprime2
%        If vfoptions.gridinterplayer==1, PolicyKron is (4,N_a,N_semiz,N_z,N_e,N_j): rows 1-2 are the
%        Kron indices, rows 3 and 4 are L2 and L2flag (passed through unchanged).
% Output: Policy is (l_daprime1+l_daprime2,n_a,n_semiz,n_z,n_e,N_j);
%         If vfoptions.gridinterplayer==1, Policy is (l_daprime1+l_daprime2+2,n_a,n_semiz,n_z,n_e,N_j).
...
if vfoptions.gridinterplayer==1
    Policy=[mod(floor((PolicyKron(1,:)-1)./divisors1),n_daprime1(:))+1;
            ...
            PolicyKron(3,:);
            PolicyKron(4,:)];
    Policy=reshape(Policy,[l_daprime1+l_daprime2+2,n_a,n_semiz,n_z,n_e,N_j]);
else
    Policy=[mod(floor((PolicyKron(1,:)-1)./divisors1),n_daprime1(:))+1;
            mod(floor((PolicyKron(2,:)-1)./divisors2),n_daprime2(:))+1];
    Policy=reshape(Policy,[l_daprime1+l_daprime2,n_a,n_semiz,n_z,n_e,N_j]);
end
