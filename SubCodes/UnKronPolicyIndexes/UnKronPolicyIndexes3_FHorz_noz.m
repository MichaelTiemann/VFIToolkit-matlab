function Policy=UnKronPolicyIndexes3_FHorz_noz(PolicyKron, n_daprime1, n_daprime2, n_daprime3, n_a, N_j, vfoptions)
% For models with no z (and no e, no semiz), three daprime dimensions, finite horizon.
% Can input vfoptions OR simoptions
% Input: PolicyKron=zeros(3,N_a,N_j);
%        PolicyKron(k,:,:) indexes the optimal choice for daprime k (k=1,2,3)
%        If vfoptions.gridinterplayer==1, PolicyKron is (5,N_a,N_j): rows 1-3 are the
%        Kron indices, rows 4 and 5 are L2 and L2flag (passed through unchanged).
% Output: Policy is (l_daprime1+l_daprime2+l_daprime3,n_a,N_j);
%         If vfoptions.gridinterplayer==1, Policy is (l_daprime1+l_daprime2+l_daprime3+2,n_a,N_j).
