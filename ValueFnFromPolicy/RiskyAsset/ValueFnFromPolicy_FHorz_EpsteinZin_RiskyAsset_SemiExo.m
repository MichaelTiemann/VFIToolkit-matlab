function V=ValueFnFromPolicy_FHorz_EpsteinZin_RiskyAsset_SemiExo(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8)
...
% (n_semiz>0). Mirrors ValueFnFromPolicy_FHorz_RiskyAsset_SemiExo with the
% Epstein-Zin transform chain: V' is transformed by ^ezc5 BEFORE all the
% expectations (one joint certainty-equivalent over (u,semizprime,zprime,eprime);
% mirroring the EZ riskyasset VFI raws and ValueFnFromPolicy_FHorz_EpsteinZin_SemiExo,
% e' is summed first, then z' is collapsed, then the d_semiz-dependent semiz'
% transition is collapsed per d_semiz choice, and finally the u-lottery over
% a2prime is applied via the per-state (a1prime,a2prime,d_semiz) lookup on the
% collapsed object), with the certainty-equivalent power ^ezc6 applied
% pointwise after the per-state lookup. The warm-glow of bequests is over the
% risky asset a2prime (as in the EZ riskyasset VFI raws): transformed by ^ezc5
% and averaged over the policy's a2prime u-lottery before being folded into the
% sj/ezc8 structure. The ezc1-ezc8/sj/warmglow preamble is done by the caller
...
if vfoptions.gridinterplayer==1
    V=ValueFnFromPolicy_FHorz_EpsteinZin_RiskyAsset_SemiExo_GI(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8);
    return
end
...
if ~isfield(vfoptions,'refine_d')
    error('ValueFnFromPolicy_FHorz_EpsteinZin_RiskyAsset_SemiExo requires vfoptions.refine_d (with a 4th entry for the semiz decision)')
end
...
if N_d==0
    error('ValueFnFromPolicy_FHorz_EpsteinZin_RiskyAsset_SemiExo: riskyasset+semiz requires at least one decision variable')
end
l_d=length(n_d);
...
%% PolicyValues (PolicyInd2Val_FHorz handles riskyasset internally -- drops a2prime, auto-adds n_semiz and n_e)
...
for reverse_j=0:N_j-1
    ...
    if jj==N_j
        ...
        temp2=F_jj;
        temp2(becareful)=F_jj(becareful).^ezc2(N_j);
        temp2(F_jj==0)=-Inf;
        if warmglow==1
        ...
        % Step 4: per-state lookup + u-lottery, on the transformed-and-collapsed object.
        % Mirrors the vNM riskyasset semiz FromPolicy's skipinterp+isnan so policies landing
        % on infeasible-on-both-sides next-states give the same finite V as the argmax V.
        ...
        if N_e==0
            ...
            if N_z==0
                ...
                a2pPrb(EV_lo==EV_up)=0; % skipinterp (on the transformed EV, as the EZ VFI raws)
