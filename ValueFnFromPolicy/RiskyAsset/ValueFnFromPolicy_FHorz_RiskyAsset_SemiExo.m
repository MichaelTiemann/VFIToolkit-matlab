function varargout=ValueFnFromPolicy_FHorz_RiskyAsset_SemiExo(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions)
% Compute V from a given Policy when the model has riskyasset (vfoptions.riskyasset==1)
% AND semi-exogenous shocks (n_semiz>0).
%
% Structural cousin of ValueFnFromPolicy_FHorz_ExpAssetu_SemiExo (same u-shock structure). The only
% substantive differences are: (i) the aprime-from-policy helper is CreateaprimePolicyRiskyAsset
% (riskyasset's aprimeFn ignores current a2); (ii) the d split is set by vfoptions.refine_d, the
% 'refine' split the VFI codes use ([d1,d2,d3,d4]): d1 ReturnFn only, d2 aprimeFn only, d3 both,
% d4 the semiz decision. aprimeFn takes (d2,d3,u); ReturnFn takes (d1,d3,d4,a1prime,a1,a2,semiz,...).

%% Dispatch to GI subfn if gridinterplayer==1
if vfoptions.gridinterplayer==1
    V=ValueFnFromPolicy_FHorz_RiskyAsset_SemiExo_GI(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions);
    varargout={V};
    return
end
...
if ~isfield(vfoptions,'refine_d')
    error('ValueFnFromPolicy_FHorz_RiskyAsset_SemiExo requires vfoptions.refine_d (with a 4th entry for the semiz decision)')
end
...
if N_d==0
    error('ValueFnFromPolicy_FHorz_RiskyAsset_SemiExo: riskyasset+semiz requires at least one decision variable')
end
l_d=length(n_d);
l_a=length(n_a);
