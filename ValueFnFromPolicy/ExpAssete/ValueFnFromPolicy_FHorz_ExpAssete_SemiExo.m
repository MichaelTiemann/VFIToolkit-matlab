function varargout=ValueFnFromPolicy_FHorz_ExpAssete_SemiExo(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions)
...
%
% Combines:
%   - ExpAssete: a2 is the experience asset; a2prime is determined by aprimeFn(d_expasset,a2,e,...) where
%     e is the iid start-of-period shock (REQUIRED). aprimeFn does not depend on semiz or z.
...
if vfoptions.gridinterplayer==1
    V=ValueFnFromPolicy_FHorz_ExpAssete_SemiExo_GI(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions);
    varargout={V};
    return
end
...
if N_e==0
    error('ValueFnFromPolicy_FHorz_ExpAssete_SemiExo: experienceassete requires N_e>0 (aprimeFn depends on e)')
end
if N_d==0
    error('ValueFnFromPolicy_FHorz_ExpAssete_SemiExo: experienceassete+semiz requires at least one decision variable')
end
l_d=length(n_d);
...
% aprimeFnParamNames: leading inputs are (d_expasset..., a2, e...)
...
% Joint Markov-like shock dim = [semiz, z] (z may be empty); e is a separate axis
if N_z==0
    n_shocks=n_semiz;
else
...
%% PolicyValues (PolicyInd2Val_FHorz handles experienceassete internally; auto-adds n_semiz and n_e)
PolicyValues=PolicyInd2Val_FHorz(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,vfoptions,1);
l_daprime=size(PolicyValues,1); % = l_d + l_a1
% PolicyValues shape: [l_daprime, N_a, N_shocks*N_e, N_j]
...
%% Joint Markov-like shock gridvals for ReturnFn (semiz, then z if present)
if N_z==0
...
for reverse_j=0:N_j-1
    ...
    if jj==N_j
        ...
        % Step 4: per-state lookup -- per-e iteration since a2primeIndex/Policy all depend on e
        if N_z==0
            ...
            for e_c=1:N_e
                ...
                EV_lo=reshape(EVnext_byd2(lo_idx(:)),[N_a, N_semiz]);
                EV_up=reshape(EVnext_byd2(up_idx(:)),[N_a, N_semiz]);
