function [V,Valt]=ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssete_SemiExo(Policy,Policyalt,isNaive,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions, beta0)
...
%   - the e aprime machinery + semiz reconstruction (cf ValueFnFromPolicy_FHorz_ExpAsseteSemiExo)
%   - the QH dual-value (Naive/Sophisticated) idiom with d_semiz indirection
%     (cf ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetze_SemiExo and _QuasiHyperbolic_ExpAssete).
...
% pi_semiz depends on the policy's d_semiz choice (last l_dsemiz components of d). For Naive,
% Vtilde looks up EVnext at Policy's d_semiz/a2prime; Valt looks up EVnext at Policyalt's.
%
...
if vfoptions.gridinterplayer==1
    [V,Valt]=ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssete_SemiExo_GI(Policy,Policyalt,isNaive,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions, beta0);
    return
end

%% Setup (mirrors ValueFnFromPolicy_FHorz_ExpAsseteSemiExo)
...
if N_e==0
    error('ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssete_SemiExo: experienceassete requires N_e>0 (aprimeFn depends on e)')
end
if N_d==0
    error('ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssete_SemiExo: experienceassete+semiz requires at least one decision variable')
end
l_d=length(n_d);
...
% PolicyInd2Val_FHorz handles experienceassete internally; auto-adds n_semiz and n_e
...
for reverse_j=0:N_j-1
    ...
    if jj==N_j
        ...
        for e_c=1:N_e
            ...
            a1p_e=reshape(a1prime_idx(:,:,e_c,jj),[N_a, N_semiz, N_zeff]);
            d2_e =reshape(d_semiz_idx(:,:,e_c,jj),[N_a, N_semiz, N_zeff]);
            a2pIdx_e=reshape(a2primeIndex(:,block),[N_a, N_semiz, N_zeff]);
            a2pPrb_e=reshape(a2primeProbs(:,block),[N_a, N_semiz, N_zeff]);
            ...
            EV_lo=reshape(EVnext_byd2(lo_idx(:)),[N_a, N_semiz, N_zeff]);
            EV_up=reshape(EVnext_byd2(up_idx(:)),[N_a, N_semiz, N_zeff]);
        ...
        if isNaive
            ...
            for e_c=1:N_e
                block=(e_c-1)*N_shocks + (1:N_shocks);
                a1p_e=reshape(a1prime_idx_alt(:,:,e_c,jj),[N_a, N_semiz, N_zeff]);
                d2_e =reshape(d_semiz_idx_alt(:,:,e_c,jj),[N_a, N_semiz, N_zeff]);
                a2pIdx_e=reshape(a2primeIndex_alt(:,block),[N_a, N_semiz, N_zeff]);
                a2pPrb_e=reshape(a2primeProbs_alt(:,block),[N_a, N_semiz, N_zeff]);
                aprime_low_e=a1p_e+N_a1*(a2pIdx_e-1);
                aprime_up_e =a1p_e+N_a1*(a2pIdx_e);
                base_off=reshape(N_a*(SZ_grid(:)-1)+N_a*N_semiz*(Z_grid(:)-1)+N_a*N_semiz*N_zeff*(d2_e(:)-1), [N_a, N_semiz, N_zeff]);
                lo_idx=aprime_low_e+base_off;
