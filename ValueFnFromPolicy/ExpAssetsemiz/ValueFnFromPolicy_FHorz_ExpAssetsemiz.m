function varargout=ValueFnFromPolicy_FHorz_ExpAssetsemiz(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions)
...
if vfoptions.gridinterplayer==1
    V=ValueFnFromPolicy_FHorz_ExpAssetsemiz_GI(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions);
    varargout={V};
    return
end
...
if N_d==0
    error('ValueFnFromPolicy_FHorz_ExpAssetsemiz: experienceassetsemiz requires at least one decision variable')
end
l_d=length(n_d);
...
%% PolicyValues (PolicyInd2Val_FHorz handles experienceassetsemiz internally; auto-adds n_semiz and n_e)
...
for reverse_j=0:N_j-1
    ...
    if jj==N_j
        ...
        if N_e==0
            ...
            a2pIdx=reshape(a2primeIndex,[N_a, N_semiz, N_zloc]);
            a2pPrb=reshape(a2primeProbs,[N_a, N_semiz, N_zloc]);
            ...
            for e_c=1:N_e
                ...
                a1p_e=reshape(a1prime_idx(:,:,e_c,jj),[N_a, N_semiz, N_zloc]);
                d2_e =reshape(d_semiz_idx(:,:,e_c,jj),[N_a, N_semiz, N_zloc]);
                a2pIdx_e=reshape(a2primeIndex(:,block),[N_a, N_semiz, N_zloc]);
                a2pPrb_e=reshape(a2primeProbs(:,block),[N_a, N_semiz, N_zloc]);
