function [V,Valt]=ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetsemiz(Policy,Policyalt,isNaive,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions, beta0)
...
%
% Structural base: ValueFnFromPolicy_FHorz_ExpAssetsemiz (the semiz-driven a2prime interpolation and
% the d_semiz indirection are carried over untouched). QH bookkeeping mirrors
% ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAsset_SemiExo.
%
...
if vfoptions.gridinterplayer==1
    [V,Valt]=ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetsemiz_GI(Policy,Policyalt,isNaive,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions, beta0);
    return
end

%% Setup (mirrors ValueFnFromPolicy_FHorz_ExpAssetsemiz)
...
if N_d==0
    error('ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetsemiz: experienceassetsemiz requires at least one decision variable')
end
l_d=length(n_d);
...
% aprimeFnParamNames: leading inputs are (d_expasset..., a2, semiz...)
...
for reverse_j=0:N_j-1
    ...
    % Shapes:
    %   N_e==0: [N_a, N_shocks]            (where N_shocks = N_semiz*N_zloc)
    ...
    if jj==N_j
        ...
        % Step 4: per-state lookup -- combine d_semiz indirection and a2 interpolation. Pass 1 is
        % Policy (giving the reported value's continuation); pass 2, when Naive, is Policyalt.
        for pass=1:(1+isNaive)
            ...
            if N_e==0
                ...
                a1p_r=reshape(a1p,[N_a, N_semiz, N_zloc]);
                d2_r =reshape(d2_jj,[N_a, N_semiz, N_zloc]);
                a2pIdx=reshape(a2pi,[N_a, N_semiz, N_zloc]);
                a2pPrb=reshape(a2pp,[N_a, N_semiz, N_zloc]);
                ...
                for e_c=1:N_e
                    ...
                    a1p_e=reshape(a1pi(:,:,e_c,jj),[N_a, N_semiz, N_zloc]);
                    d2_e =reshape(d2i(:,:,e_c,jj),[N_a, N_semiz, N_zloc]);
                    a2pIdx_e=reshape(a2pi(:,block),[N_a, N_semiz, N_zloc]);
                    a2pPrb_e=reshape(a2pp(:,block),[N_a, N_semiz, N_zloc]);
