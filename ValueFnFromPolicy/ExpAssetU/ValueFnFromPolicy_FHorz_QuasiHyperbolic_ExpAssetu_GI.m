function [V,Valt]=ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetu_GI(Policy,Policyalt,isNaive,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions, beta0)
...
%
% Structural base: ValueFnFromPolicy_FHorz_ExpAssetu_GI (the 2x2 interpolated lookup and the u
% integration are carried over untouched). QH bookkeeping mirrors
% ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAsset_GI.
%
% Under GI, Policy stores an extra L2 (layer-2 fine-grid) index at the end, used to interpolate a1prime
% between two adjacent a1_grid points. a2prime continues to be interpolated via aprimeFn, which here
% also takes the iid shock u, so a2primeIndex/a2primeProbs carry a trailing N_u dimension. Per-state
% lookup is a 2x2 interpolation (lower/upper a1 x lower/upper a2) at each u, and u is then integrated
% out with pi_u inside the pass loop, before any discount factor is applied.
%
...
if N_d==0
    error('ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetu_GI: experienceassetu requires at least one decision variable')
end
l_d=length(n_d);
l_a=length(n_a);
...
for reverse_j=0:N_j-1
    ...
    % Step 1: a2primeIndex, a2primeProbs (adds u dim) (for each policy)
    ...
    if jj==N_j
        ...
        if N_z==0 && N_e==0
            EVnext=Vdrive(:,jj+1);
        elseif N_z==0 && N_e>0
            EVnext=sum(Vdrive(:,:,jj+1) .* shiftdim(vfoptions.pi_e_J(:,jj+1), -1), 2);
        elseif N_z>0 && N_e==0
        ...
        for pass=1:(1+isNaive)
            ...
            elseif N_z>0 && N_e==0
                ...
                a2l=reshape(a2pi,[N_a,N_z,N_e,N_u]); a2u=a2l+1;
                wa2l=reshape(a2pp,[N_a,N_z,N_e,N_u]); wa2u=1-wa2l;
