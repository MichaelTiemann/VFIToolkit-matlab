function [V,Valt]=ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetu_SemiExo_GI(Policy,Policyalt,isNaive,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions, beta0)
% Compute V from a given Policy when the model has experienceassetu (vfoptions.experienceassetu>=1),
% semi-exogenous shocks (n_semiz>0), the grid interpolation layer (vfoptions.gridinterplayer==1),
...
%
% Structural base: ValueFnFromPolicy_FHorz_ExpAssetu_SemiExo_GI (the 2x2 corner interpolation, the
% d_semiz indirection, the u integration and every gather stride are carried over untouched). QH
% bookkeeping mirrors ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAsset_SemiExo_GI and
% ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetu_GI.
%
% Per-state EVnext lookup combines four pieces:
%   - a1 fine-grid interpolation (lower/upper a1 grid point + L2 weight)
%   - a2 interpolation via a2primeIndex/a2primeProbs (from CreateaprimePolicyExperienceAssetu)
%   - d_semiz indirection: pick the d2-slice of EVnext_byd2 via d_semiz_idx
%   - sum over the iid u shock with pi_u (u does NOT enter Policy)
%
...
% u is integrated out with pi_u inside each pass, before any discount factor is applied.
...
if N_d==0
    error('ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetu_SemiExo_GI: experienceassetu+semiz requires at least one decision variable')
end
l_d=length(n_d);
l_a=length(n_a);
...
l_a1=length(n_a1);
l_a2=length(n_a2);
l_aprime=l_a1; % Policy stores a1prime only (plus L2 in GI); a2prime implicit
...
%% PolicyValues (PolicyInd2Val_FHorz handles experienceassetu+GI internally)
...
for reverse_j=0:N_j-1
    ...
    % Step 1: a2primeIndex, a2primeProbs at this age (at Policy, and at Policyalt if Naive) -- helper adds the u dim
    ...
    if jj==N_j
        ...
        % Step 4: per-state 2x2 corner interpolation on EVnext_byd2 at d_semiz-selected slice, summed over u with pi_u.
        ...
        % Both passes integrate u out with pi_u before Step 5 applies any discount factor.
        for pass=1:(1+isNaive)
            ...
            if N_e==0
                ...
                if N_z==0
                    ...
                    a2l=reshape(a2pi,[N_a, N_semiz, N_z, N_u]); a2u=a2l+1;
                    wa2l=reshape(a2pp,[N_a, N_semiz, N_z, N_u]); wa2u=1-wa2l;
                ...
                if N_z==0
                    ...
                    for e_c=1:N_e
                        ...
                        a1l_e=reshape(a1Lo(:,:,e_c,jj),[N_a, N_semiz]);
                        a1u_e=reshape(a1Up(:,:,e_c,jj),[N_a, N_semiz]);
                        wa1l_e=reshape(wLo(:,:,e_c,jj),[N_a, N_semiz]);
                        wa1u_e=reshape(wUp(:,:,e_c,jj),[N_a, N_semiz]);
                        a2l_e=reshape(a2pi(:,block,:),[N_a, N_semiz, N_u]);
                        a2u_e=a2l_e+1;
                        wa2l_e=reshape(a2pp(:,block,:),[N_a, N_semiz, N_u]);
                        wa2u_e=1-wa2l_e;
                        ...
                        EVnext_pass(:,:,e_c)=sum(per_u .* shiftdim(pi_u,-2), 3);
                    end
                    EVnext_pass(isnan(EVnext_pass))=0; % zero corner weights times -Inf next-states give NaN
                else
                    ...
                    for e_c=1:N_e
                        block=(e_c-1)*N_shocks + (1:N_shocks);
                        a1l_e=reshape(a1Lo(:,:,e_c,jj),[N_a, N_semiz, N_z]);
                        a1u_e=reshape(a1Up(:,:,e_c,jj),[N_a, N_semiz, N_z]);
                        wa1l_e=reshape(wLo(:,:,e_c,jj),[N_a, N_semiz, N_z]);
                        wa1u_e=reshape(wUp(:,:,e_c,jj),[N_a, N_semiz, N_z]);
                        a2l_e=reshape(a2pi(:,block,:),[N_a, N_semiz, N_z, N_u]);
                        a2u_e=a2l_e+1;
                        wa2l_e=reshape(a2pp(:,block,:),[N_a, N_semiz, N_z, N_u]);
                        wa2u_e=1-wa2l_e;
                        ...
                        EV_UU=reshape(EVnext_byd2(lin_UU(:)),[N_a, N_semiz, N_z, N_u]);
                        per_u=wa1l_e.*wa2l_e.*EV_LL + wa1l_e.*wa2u_e.*EV_LU + wa1u_e.*wa2l_e.*EV_UL + wa1u_e.*wa2u_e.*EV_UU;
                        EVnext_pass(:,:,:,e_c)=sum(per_u .* shiftdim(pi_u,-3), 4);
                    end
                    EVnext_pass(isnan(EVnext_pass))=0; % zero corner weights times -Inf next-states give NaN
                end
            end
