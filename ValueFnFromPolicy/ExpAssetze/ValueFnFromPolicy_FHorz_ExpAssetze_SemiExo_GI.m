function varargout=ValueFnFromPolicy_FHorz_ExpAssetze_SemiExo_GI(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions)
...
%   - a2 interpolation via a2primeIndex/a2primeProbs (from CreateaprimePolicyExperienceAssetze)
%   - d_semiz indirection: pick the d2-slice of EVnext_byd2 via d_semiz_idx
% aprimeFn = aprimeFn(d_expasset, a2, z, e, ...) -- depends on Markov z (REQUIRED) and iid e (REQUIRED).
...
if N_z==0 || N_e==0
    error('ValueFnFromPolicy_FHorz_ExpAssetze_SemiExo_GI: experienceassetze requires both N_z>0 and N_e>0 (aprimeFn depends on z and e)')
end
if N_d==0
    error('ValueFnFromPolicy_FHorz_ExpAssetze_SemiExo_GI: experienceassetze+semiz requires at least one decision variable')
end
l_d=length(n_d);
l_z=length(n_z);
...
if isscalar(n_a)
    ...
    V=ValueFnFromPolicy_FHorz_ExpAssetze_SemiExo(Policy,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions);
    varargout={V};
    return
end
n_a1=n_a(1:end-1);
...
%% PolicyValues (PolicyInd2Val_FHorz handles experienceassetze+GI internally)
