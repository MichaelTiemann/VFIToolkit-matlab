...
% Combines:
%   - the ze aprime machinery + semiz reconstruction + GI 2x2-corner lookup
%     (cf ValueFnFromPolicy_FHorz_ExpAssetze_SemiExo_GI, the structural base)
%   - the QH dual-value (Naive/Sophisticated) idiom with d_semiz indirection
%     (cf ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetze_SemiExo and _QuasiHyperbolic_ExpAssetze_GI).
...
% choice (last l_dsemiz components of d). Under GI, Policy carries an L2 fine-grid index for a1prime;
% the per-state lookup is a 2x2 corner interpolation (a1 low/up x a2 low/up) combined with the
% d_semiz-dependent bothz transition and e'-integration.
% For Naive, Vtilde looks up EVnext at Policy's GI indices/a2prime/d_semiz;
% Valt looks up EVnext at Policyalt's GI indices/a2prime/d_semiz.
%
...
%% Setup (mirrors ValueFnFromPolicy_FHorz_ExpAssetze_SemiExo_GI)
...
if N_z==0 || N_e==0
    error('ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetze_SemiExo_GI: experienceassetze requires both N_z>0 and N_e>0 (aprimeFn depends on z and e)')
end
if N_d==0
    error('ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetze_SemiExo_GI: experienceassetze+semiz requires at least one decision variable')
end
l_d=length(n_d);
l_z=length(n_z);
