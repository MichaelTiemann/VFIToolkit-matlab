function [V,Valt]=ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAsset_SemiExo_GI(Policy,Policyalt,isNaive,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, vfoptions, beta0)
...
%
% Structural base: ValueFnFromPolicy_FHorz_ExpAsset_SemiExo_GI (the 2x2 corner interpolation, the
% d_semiz indirection and every gather stride are carried over untouched). QH bookkeeping mirrors
% ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAsset_GI and ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAsset_SemiExo.
%
% Per-state EVnext lookup combines three pieces:
%   - a1 fine-grid interpolation (lower/upper a1 grid point + L2 weight)
%   - a2 interpolation via a2primeIndex/a2primeProbs (from CreateaprimePolicyExperienceAsset)
%   - d_semiz indirection: pick the d2-slice of EVnext_byd2 via d_semiz_idx
%
% i.e. a 2x2 corner interpolation (a1_low/up x a2_low/up), evaluated on the d_semiz-selected EVnext slice.
%
...
if N_d==0
    error('ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAsset_SemiExo_GI: experienceasset+semiz requires at least one decision variable')
end
l_d=length(n_d);
l_a=length(n_a);
...
%% PolicyValues (PolicyInd2Val_FHorz handles experienceasset+GI internally; auto-adds n_semiz and n_e)
...
size_first=l_d+l_a1+1; % under GI we want d, a1mid, L2 → l_d + l_a1 + 1 channels
...
for reverse_j=0:N_j-1
    ...
    % Step 1: a2primeIndex, a2primeProbs at this age (at Policy, and at Policyalt if Naive)
    ...
    if jj==N_j
        ...
        % Step 4: per-state 2×2 corner interpolation on EVnext_byd2 at the d_semiz-selected slice.
        % Corners: (a1_low, a2_low), (a1_low, a2_up), (a1_up, a2_low), (a1_up, a2_up)
        % aprime_kron(a1_corner, a2_corner) = a1_corner + N_a1*(a2_corner - 1)
        % Weights: wa1l*wa2l, wa1l*wa2u, wa1u*wa2l, wa1u*wa2u
            ...
            if N_e==0
                ...
                if N_z==0
                    ...
                    for e_c=1:N_e
                        ...
                        a2l_e=reshape(a2pi(:,block),[N_a,N_semiz]);
                        a2u_e=a2l_e+1;
                        wa2l_e=reshape(a2pp(:,block),[N_a,N_semiz]);
                        wa2u_e=1-wa2l_e;
                        ...
                        EV_LL=EVnext_byd2(lin_LL); EV_LU=EVnext_byd2(lin_LU);
                        EV_UL=EVnext_byd2(lin_UL); EV_UU=EVnext_byd2(lin_UU);
                        EVnext_pass(:,:,e_c)=reshape( wa1l_e(:).*wa2l_e(:).*EV_LL + wa1l_e(:).*wa2u_e(:).*EV_LU + wa1u_e(:).*wa2l_e(:).*EV_UL + wa1u_e(:).*wa2u_e(:).*EV_UU, [N_a, N_semiz]);
                    end
                else
                    ...
                    for e_c=1:N_e
                        ...
                        a2l_e=reshape(a2pi(:,block),[N_a,N_semiz,N_z]);
                        a2u_e=a2l_e+1;
                        wa2l_e=reshape(a2pp(:,block),[N_a,N_semiz,N_z]);
                        wa2u_e=1-wa2l_e;
                        ...
                        EV_LL=EVnext_byd2(lin_LL); EV_LU=EVnext_byd2(lin_LU);
                        EV_UL=EVnext_byd2(lin_UL); EV_UU=EVnext_byd2(lin_UU);
                        EVnext_pass(:,:,:,e_c)=reshape( wa1l_e(:).*wa2l_e(:).*EV_LL + wa1l_e(:).*wa2u_e(:).*EV_LU + wa1u_e(:).*wa2l_e(:).*EV_UL + wa1u_e(:).*wa2u_e(:).*EV_UU, [N_a, N_semiz, N_z]);
                    end
                end
            end
