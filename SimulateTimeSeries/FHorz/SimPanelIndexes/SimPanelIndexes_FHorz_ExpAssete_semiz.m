function SimPanel=SimPanelIndexes_FHorz_ExpAssete_semiz(InitialDist,Policy,n_d,n_a,n_z,N_j,pi_z_J, Parameters, simoptions)
...
if N_e==0
    error('Cannot use simoptions.experienceassete=1 with no e variables (e is required)')
end
...
% aprimeFnParamNames in same fashion: (d2, a2, e) are the leading inputs
l_a2=length(n_a2);
...
%%
% e required for experienceassete, so N_semizze always contains N_e
if N_z==0
...
whichisdforexpassete=length(n_d)-l_d3;  % is just saying which is the decision variable that influences the experience asset (it is the 'second last' decision variable)
for jj=1:N_j
    ...
    elseif l_a==3 % experience asset and two other assets
        ...
        error('Not yet implemented experienceassete with length(n_a)>3')
    end
...
%% e required: only the (no-z, e) and (z, e) branches matter
if N_z==0 % no z, e
