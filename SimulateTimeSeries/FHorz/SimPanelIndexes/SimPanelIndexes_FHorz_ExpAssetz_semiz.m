function SimPanel=SimPanelIndexes_FHorz_ExpAssetz_semiz(InitialDist,Policy,n_d,n_a,n_z,N_j,z_gridvals_J,pi_z_J, Parameters, simoptions)
...
N_z=prod(n_z);
if N_z==0
    error('Cannot use simoptions.experienceassetz=1 with no z variables (z is required)')
end
l_z=length(n_z);
...
n_d2=n_d(end-simoptions.l_dsemiz); % decision variable that controls experience asset
if length(n_d)>2
    n_d1=n_d(1:end-2);
    l_d1=length(n_d1);
else
...
% aprimeFnParamNames in same fashion: (d2, a2, z) are the leading inputs
l_a2=length(n_a2);
...
% size(semiz_gridvals_J)=[prod(n_semiz),length(n_semiz),N_j]
...
%%
% z required for experienceassetz, so N_semizze always contains N_z
if N_e==0
...
whichisdforexpassetz=length(n_d)-l_d3;  % is just saying which is the decision variable that influences the experience asset (it is the 'second last' decision variable)
for jj=1:N_j
    ...
    elseif l_a==3 % experience asset and two other assets
        ...
        error('Not yet implemented experienceassetz with length(n_a)>3')
    end
...
if simoptions.gridinterplayer==1
    ...
    aprimeProbs_upper=reshape(shiftdim(double(Policy(end-1,:,:,:)-1)/(simoptions.ngridinterp+1),1),[N_a,N_semizze,1,N_j]); % probability of upper grid point (from L2 index; end-1 because end is now L2flag)
...
%% z required: only the (z, no-e) and (z, e) branches matter
if N_e==0 % z, no e
