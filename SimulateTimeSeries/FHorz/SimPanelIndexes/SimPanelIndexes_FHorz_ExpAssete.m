function SimPanel=SimPanelIndexes_FHorz_ExpAssete(InitialDist,Policy,n_d,n_a,n_z,N_j,pi_z_J, Parameters, simoptions)
...
PolicyProbs=zeros(N_a,N_bothze,2,N_j,'gpuArray'); % The third dimension is lower/upper grid point
whichisdforexpassete=length(n_d);  % is just saying which is the decision variable that influences the experience asset (it is the 'last' decision variable)
for jj=1:N_j
