...
if N_z==0
    ...
    for ff=1:length(FnsToEvaluate)
        CellOverAgeOfParamValues=CreateCellOverAgeFromParams(Parameters,FnsToEvaluateParamNames(ff).Names,N_j,3,simoptions.precision); % j in 3nd dimension: (a,z,j,l_d+l_a), so we want j to be after N_a and N_z
