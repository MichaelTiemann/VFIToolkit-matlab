function AggVars=EvalFnOnAgentDist_AggVars_InfHorz_withV(V,StationaryDist, Policy, FnsToEvaluate, FnsToEvalNames, Parameters, FnsToEvaluateParamNames, n_d, n_a, n_z, d_grid, a_grid, z_grid, Parallel, simoptions)
% Evaluates the aggregate value (weighted sum/integral) for each element of FnsToEvaluate
% Note to self: I want to delete this _withV, but cannot currently do so as it is used by the firm entry/exit models (they need reworking before this can be deleted).
%
...
% EvalFnOnAgendDist_AggVars_InfHorz() again. Some of the results are then
