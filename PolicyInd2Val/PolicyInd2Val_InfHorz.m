function PolicyValues=PolicyInd2Val_InfHorz(Policy,n_d,n_a,n_z,d_grid,a_grid,vfoptions,outputkron)
...
if ~isgpuarray(Policy)
    if gpuDeviceCount==0
        ...
        PolicyValues=PolicyInd2Val_InfHorz_cpu(Policy,n_d,n_a,n_z,d_grid,a_grid,vfoptions,outputkron);
        return
    else
...
if isfield(vfoptions,'inheritanceasset') && vfoptions.inheritanceasset>0
    l_aprime=l_a-1;
    aprime_grid=a_grid(1:sum(n_aprime(1:end-1)));
    n_aprime=n_aprime(1:end-1);
end
    ...
    if l_d==0
        ...
        if size(d_grid,2)==1
            ...
            if l_d>1
                if l_d>2
                    for ii=2:l_d-1
                        PolicyValues(ii,:)=d_grid(cumsum_n_d(ii-1)+Policy(ii,:));
                    end
                end
                PolicyValues(l_d,:)=d_grid(cumsum_n_d(end-1)+Policy(l_d,:));
            end
        else % using d_gridvals (and must be that l_d>1)
            d_gridvals=gpuArray(d_grid);
            % Rebuild the joint row index from the per-variable d indexes, then read each
