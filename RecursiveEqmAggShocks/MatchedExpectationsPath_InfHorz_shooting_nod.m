...
while TransPathConvergence>1 && itercounter<=recursiveeqmoptions.maxiter
    ...
    if isscalar(n_a)
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,1:T-1,:),[N_a*(T-1)*N_z,1]); % aprime index
    elseif length(n_a)==2
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,1:T-1,:)+n_a(1)*(PolicyIndexesPath(2,:,1:T-1,:)-1),[N_a*(T-1)*N_z,1]);
    elseif length(n_a)==3
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,1:T-1,:)+n_a(1)*(PolicyIndexesPath(2,:,1:T-1,:)-1)+n_a(1)*n_a(2)*(PolicyIndexesPath(3,:,1:T-1,:)-1),[N_a*(T-1)*N_z,1]);
    elseif length(n_a)==4
    ...
    PolicyValuesPath=PolicyInd2Val_InfHorz_TPath(PolicyIndexesPath,0,n_a,n_z,T,[],a_grid,vfoptions,1);
    PolicyValuesPath=permute(reshape(PolicyValuesPath,[size(PolicyValuesPath,1),N_a,N_z,T]),[2,4,3,1]); %[N_a,T,N_z,l_aprime]
    ...
    %% AggVars
    tic;
    AggVarsPath=EvalFnOnAgentDist_AgeConditionalAggVars_FHorz_fastOLG(AgentDistPath.*AgeWeights_T,[],PolicyValuesPath, FnsToEvaluateCell,FnsToEvaluateParamNames,AggVarNames,Parameters,T,0,l_a,l_a,l_z,N_a,N_z,a_gridvals,z_gridvals_T_fastOLG,1);
    ...
    if recursiveeqmoptions.verbose>=2
        ...
        if isscalar(n_S)
            ...
            for tt=1:T-1
                fig4colourmap(ss_ind_T(tt),tt)=coloursformatchheat(tt);
            end
        elseif length(n_S)==2
            ...
            for tt=1:T-1
                fig4colourmap(ss_ind_T_d1(tt),tt)=coloursformatchheat(tt);
                fig4colourmap(n_S(1)+ss_ind_T_d2(tt),tt)=coloursformatchheat(tt);
            end
        end
        ...
        fprintf('Current distance between old and new price path,     99.9th percentile:  %2.8f \n', pricedistquantiles(4))
        fprintf('Current distance between old and new price path, max (L-infinity norm):  %2.8f \n', CurrentPathDist_price)
    end
