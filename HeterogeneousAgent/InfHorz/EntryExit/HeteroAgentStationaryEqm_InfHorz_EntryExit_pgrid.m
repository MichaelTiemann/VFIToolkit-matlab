...
%     [V,Policy]=ValueFnIter_InfHorz(V0Kron, n_d,n_a,n_z,d_grid,a_grid,z_grid, pi_z, ReturnFn,Parameters, DiscountFactorParamNames,ReturnFnParamNames,vfoptions);
    if simoptions.endogenousexit==1
        [V,Policy,ExitPolicy]=ValueFnIter_InfHorz(n_d,n_a,n_z,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
    ...
    elseif simoptions.endogenousexit==2 % Mixture of both endog and exog exit (which occurs at end of period)
        [V,Policy,PolicyWhenExiting,ExitPolicy]=ValueFnIter_InfHorz(n_d,n_a,n_z,d_grid,a_grid,z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
        % With entry-exit will need to keep the value function to be able to evaluate entry condition.
        Parameters.(EntryExitParamNames.CondlProbOfSurvival{1})=1-gather(ExitPolicy);
    else
...
%     StationaryDistKron=StationaryDist_InfHorz(Policy,n_d,n_a,n_z,pi_z,simoptions);
    StationaryDistKron=StationaryDist_InfHorz(Policy,n_d,n_a,n_z,pi_z, simoptions,Parameters,EntryExitParamNames);
%     Parameters.(EntryExitParamNames.MassOfExistingAgents{1})=MassOfExistingAgents;

    if simoptions.endogenousexit==2
        AggVars=EvalFnOnAgentDist_AggVars_InfHorz(StationaryDistKron, Policy, FnsToEvaluateFn, Parameters, FnsToEvaluateParamNames, n_d, n_a, n_z, d_grid, a_grid, z_grid, simoptions.parallel,simoptions,EntryExitParamNames, PolicyWhenExiting);
    else
